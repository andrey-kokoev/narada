...
const scanFileExtensions = new Set(['.js', '.ts', '.ts', '.tsx']);
...
function hasIgnoredFragment(path: any) {
  const normalized = path.replaceAll('\\', '/');
  return ignoredPathFragments.some((fragment: any) => normalized.includes(fragment.replaceAll('\\', '/')));
}

function extensionOf(path: any) {
  const match = /\.[^.\\/]+$/.exec(path);
  return match?.[0] ?? '';
}

function isTestFile(path: any) {
  return /(?:^|[\\/.])(?:test|spec)\.[cm]?[jt]sx?$/.test(path) || /[\\/]test[\\/]/.test(path);
}

function isProductionPackageSource(path: any) {
  const normalized = path.replaceAll('\\', '/');
  return normalized.includes('/src/') || normalized.includes('/bin/');
}

function walk(dir: any, visitor: any) {
...
walk(packagesRoot, (filePath: any) => {
