...
function listFiles(dir: any) {
  if (!existsSync(dir)) return [];
  const out: string[] = [];
  const scan = (current: any) => {
...
function copyDist(source: any, target: any) {
  rmSync(target, { recursive: true, force: true });
  mkdirSync(target, { recursive: true });
  cpSync(source, target, { recursive: true });
}

function compareFiles(leftDir: any, rightDir: any, extension: any) {
  const left = listFiles(leftDir).filter((file: any) => file.endsWith(extension));
  const right = listFiles(rightDir).filter((file: any) => file.endsWith(extension));
  const names = Array.from(new Set([...left, ...right])).sort();
  return names.map((file: any) => {
...
async function main() {
  ...
  const result = {
    ...
    js_byte_equal: jsComparison.every((item: any) => item.equal),
    runtime_export_keys_equal: JSON.stringify(tscExportKeys) === JSON.stringify(oxbuildExportKeys),
    declaration_equal: dtsComparison.every((item: any) => item.equal),
    declaration_emitted: dtsComparison.every((item: any) => item.right_exists),
