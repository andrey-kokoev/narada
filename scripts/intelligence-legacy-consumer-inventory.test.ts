...
} from './intelligence-legacy-consumer-inventory.ts';
...
test('fails closed for compound, qualified, and contradictory documentation claims', () => {
  ...
  assert.equal(inventory.zero_authoritative_consumers, false);
  assert.deepEqual(
    inventory.violations.map(({ symbol }: any) => symbol).sort(),
...
test('does not infer negation from words unrelated to the legacy symbol', () => {
  ...
  assert.ok(inventory.violations.every(({ classification }: any) => classification === 'stale_governance_authority'));
});
...
test('does not blanket-admit authoritative governance decisions', () => {
  ...
  assert.equal(
    inventory.violations.find(({ path }: any) => path === '.ai/decisions/stale.md')?.classification,
    'stale_governance_authority',
  );
  assert.equal(inventory.violations[0].classification, 'stale_governance_authority');
  assert.equal(
    inventory.references.find((entry: any) => entry.path === '.ai/decisions/canonical.md')?.classification,
    'governance_negative_contract',
  );
  assert.equal(
    inventory.references.find((entry: any) => entry.path === '.ai/reviews/historical.md')?.classification,
    'governance_evidence',
  );
  assert.deepEqual(
    inventory.violations
      .filter(({ path }: any) => path !== '.ai/decisions/stale.md')
      .map(({ path }: any) => path)
      .sort(),
    [
...
test('admits an explicit runtime rejection boundary', () => {
  const inventory = scanLegacyIntelligenceEntries([
    ...
    {
      path: 'packages/agent-start/bin/verify-registered-site-launchers.ts',
  ...
  assert.ok(inventory.references.every(({ classification }: any) => classification === 'runtime_rejection_boundary'));
});
...
test('rejects positive runtime reads paired with unrelated deletes', () => {
  const inventory = scanLegacyIntelligenceEntries([
    ...
    {
      path: 'packages/nars-provider-runtime/src/canonical-protocol-adapters.ts',
      content: 'const active = env.OPENAI_MODEL; delete cache.entry;\n',
    },
  ]);
  assert.equal(inventory.zero_authoritative_consumers, false);
  assert.equal(inventory.violations.length, 3);
  assert.ok(inventory.violations.every(({ classification }: any) => classification === 'runtime_or_configuration_authority'));
});
...
test('rejects quoted lists without exact negative dataflow', () => {
  const inventory = scanLegacyIntelligenceEntries([
    ...
    {
      path: 'packages/nars-capability-gateway/src/mcp-runtime.ts',
...
test('rejects whole-file and cross-scope list tricks', () => {
  ...
  assert.equal(inventory.zero_authoritative_consumers, false);
  assert.deepEqual(
    [...new Set(inventory.violations.map(({ symbol }: any) => symbol))].sort(),
...
test('rejects retired package dependencies and surviving artifacts', () => {
  ...
  assert.equal(inventory.zero_authoritative_consumers, false);
  assert.deepEqual(
    inventory.violations.map((entry: any) => entry.classification),
...
test('distinguishes stale documentation from a negative contract', () => {
  ...
  assert.equal(
    inventory.references.find((entry: any) => entry.path === 'docs/canonical.md')?.classification,
