...
function findDirectOutput(file: any) {
  const text = readFileSync(join(COMMANDS_DIR, file), "utf8");
  return text
    .split("\n")
    .map((lineText: any, idx: any) => ({ file, line: idx + 1, text: lineText.trim() }))
    .filter((match: any) => DIRECT_OUTPUT_RE.test(match.text));
}

function allowanceKey(file: any, allowanceIndex: any) {
...
for (const file of commandFiles()) {
  ...
  for (const match of findDirectOutput(file)) {
    const allowanceIndex = allowances.findIndex((allowance: any) => allowance.pattern.test(match.text));
    if (allowanceIndex=== -1) {
...
const allowanceCount = Object.values(allowlist).reduce((sum: any, entries: any) => sum + entries.length, 0);
if (reportMode) {
  const totalDirectOutput = Array.from(matchedAllowances.values()).reduce((sum: any, entry: any) => sum + entry.count, 0);
