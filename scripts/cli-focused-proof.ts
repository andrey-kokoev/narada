...
function usage() {
  console.error("Usage: node scripts/cli-focused-proof.ts <suite> [--case <name>]");
  process.exit(1);
}
...
function parseCaseFromCommandLinePattern(pattern: any) {
...
async function runTaskPromoteRecommendationProof(selectedCase: any) {
  const { taskPromoteRecommendationCommand, openTaskLifecycleStore, ExitCode } = await loadCli();

  function setupRepo(tempDir: any) {
    ...
    try {
      ...
      const seedLifecycle = (taskId: any, taskNumber: any, status: any) => {
  ...
  function listPromotionRows(tempDir: any) {
    const store = openTaskLifecycleStore(tempDir);
    try {
  ...
  const cases: Record<string, any> = {
    ...
    dependency_missing: async () => {
      ...
      try {
        ...
        const rows = listPromotionRows(tempDir);
        const promotion = JSON.parse(rows[0].promotion_json);
        const depCheck = promotion.validation_results.find((v: any) => v.check === "dependencies");
...
async function runTaskRosterProof(selectedCase: any) {
  ...
  function setupRepo(tempDir: any, options: any= {}) {
  ...
  function createRepo(options: any= {}) {
  ...
  const cases: Record<string, any> = {
    ...
    show_verbose: async (tempDir: any) => {
    ...
    assign_success: async (tempDir: any) => {
      writeFileSync(
      ...
      if (compactMode) {
        ...
        try {
          ...
          assert(roster.some((a: any) => a.agent_id === "test-agent" && a.task_number === 385), "roster task not updated");
        ...
        assert(roster.agents.find((a: any) => a.agent_id === "test-agent")?.task === 385, "roster task not updated");
    ...
    assign_then_done_compact: async (tempDir: any) => {
      writeFileSync(
    ...
    review_success: async (tempDir: any) => {
      writeFileSync(
    ...
    done_strict_complete: async (tempDir: any) => {
      writeFileSync(
        join(tempDir, ".ai", "do-not-open", "tasks", "20260420-388-test.md"),
        "---\ntask_id: 388\nstatus: claimed\n---\n\n# Task 388\n\n## Acceptance Criteria\n- [x] Done\n\n## Execution Notes\nCompleted.\n\n## Verification\nChecked.\n",
      );
    ...
    idle_preserves_last_done: async (tempDir: any) => {
      writeFileSync(
      ...
      assert(result.exitCode === ExitCode.SUCCESS, `expected success, got ${result.exitCode}`);
      const roster = await loadRoster(tempDir);
      const agent = roster.agents.find((a: any) => a.agent_id === "test-agent");
