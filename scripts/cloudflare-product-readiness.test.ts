...
function readyOperatorCheck() {
  return {
    ...
    operation: {
      ...
      persistence_posture: { durable_boundaries: BOUNDARY_KEYS.map((key: any) => ({ key, status: 'available' })) },
      recovery_posture: {
        recovery_boundaries: BOUNDARY_KEYS.map((key: any) => ({ key, status: 'recoverable' })),
        recovery_gaps: [],
      },
...
test('cloudflare product readiness fails closed when required scheduler evidence is stale', () => {
  const source: any = readyOperatorCheck();
