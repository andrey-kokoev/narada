...
function spawnChecked(command: string, args: string[], options: any = {}) {
  const result = spawnSync(command, args, options);
  if (result.error) throw result.error;
  if (result.status !== 0) throw new Error(`command failed: ${command} ${args.join(' ')} (${result.status})`);
  return result;
}

function makeFixture({ stale = false, dirty = false }: any= {}) {
  ...
  if (stale) {
    spawnChecked('touch', ['-t', '202001010000', join(root, 'packages', 'layers', 'cli', 'dist', 'main.js')], {});
    spawnChecked('touch', ['-t', '202001010000', join(root, 'packages', 'layers', 'cli', 'dist', 'mcp-main.js')], {});
    spawnChecked('touch', ['-t', '202001010000', join(root, 'packages', 'layers', 'control-plane', 'dist', 'index.js')], {});
    spawnChecked('touch', ['-t', '202001010000', join(root, 'packages', 'task-governance', 'dist', 'index.js')], {});
    spawnChecked('touch', ['-t', '203001010000', join(root, 'packages', 'layers', 'cli', 'src', 'main.ts')], {});
  }
  if (dirty) {
...
function run(shim: any, args: any, env: any= {}) {
