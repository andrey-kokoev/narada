...
} from '../packages/cloudflare-carrier/src/cloudflare-operator-console.ts';
...
assert.match(consoleCheck.body, /Mailbox Status Posture/);
assert.match(consoleCheck.body, /Site File Change Review/);
assert.match(consoleCheck.body, /focused_status/);
assert.match(consoleCheck.body, /next_task/);
assert.match(consoleCheck.body, /mailbox_status_shadow_read_count/);
assert.match(consoleCheck.body, /mailbox_status_authority/);
assert.match(consoleCheck.body, /mailbox_shadow_target_locus/);
assert.match(consoleCheck.body, /mailbox_send_admission/);
assert.match(consoleCheck.body, /mailbox_mutation_admission/);
assert.match(consoleCheck.body, /mailbox_authority_partition/);
...
assert.match(consoleCheck.body, /flightDeckFocusSiteFileChangeProposal/);
assert.match(consoleCheck.body, /Focus Next Action/);
assert.match(consoleCheck.body, /Focus File Change Proposal/);
...
const smoke = await runJsonCommand('live-carrier-smoke', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const projectionWriteSmoke = await runJsonCommand('task-lifecycle-projection-write-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-task-lifecycle-projection-write-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
assert.ok(siteList.body.sites.some((site: any) => site.site_id === siteId));
assert.ok(Array.isArray(siteList.body.site_product_statuses));
const focusedSiteProductStatus = siteList.body.site_product_statuses.find((status: any) => status.site_id === siteId) ?? null;
...
if (sitePostureRoute.status=== 'needs_attention') {
  ...
  assert.ok(siteList.body.site_product_statuses.some((status: any) => status.site_id === sitePostureRoute.target));
} else {
...
assert.equal(operationRead.body.sessions?.some((session: any) => session.carrier_session_id === smoke.carrier_session_id), true);
assert.equal(operationRead.body.tasks?.some((task: any) => task.carrier_session_id === smoke.carrier_session_id), true);
assert.equal(operationRead.body.carrier_evidence?.some((entry: any) => entry.carrier_session_id === smoke.carrier_session_id && entry.ok === true), true);
...
assert.ok(operationRead.body.operations.some((operation: any) => operation.operation_id === operationId));

const liveCommandStates: any = commandStatesForOperationProduct(operationRead.body, { session_id: smoke.carrier_session_id });
...
if (operationPostureRoute.status=== 'needs_attention') {
  ...
  assert.ok(operations.some((operation: any) => operation.operation_id === operationPostureRoute.target));
} else {
...
assert.ok((cloudflareContinuityPull.packet?.decisions ?? []).some((decision: any) => (
...
const taskLifecycleAssignmentWriteSmoke = await runJsonCommand('task-lifecycle-assignment-write-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-task-lifecycle-assignment-write-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const taskLifecycleRoleResolutionWriteSmoke = await runJsonCommand('task-lifecycle-role-resolution-write-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-task-lifecycle-role-resolution-write-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const taskLifecycleRosterMutationWriteSmoke = await runJsonCommand('task-lifecycle-roster-mutation-write-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-task-lifecycle-roster-mutation-write-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const mailboxStatusShadowSmoke = await runJsonCommand('mailbox:status-shadow-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-mailbox-status-shadow-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
if (mailboxSourceAccount) {
  mailboxStatusSourceSmoke = await runJsonCommand('mailbox:status-source-smoke:live', [
    'node',
    'packages/cloudflare-carrier/scripts/cloudflare-carrier-mailbox-status-source-live-smoke.ts',
    '--url',
    workerUrl,
    '--token-file',
    tokenFile,
...
const mailboxDraftReplyProposalSmoke = await runJsonCommand('mailbox:draft-reply-proposal-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-mailbox-draft-reply-proposal-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const siteFileChangeProposalSmoke = await runJsonCommand('site-file-change:proposal-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-site-file-change-proposal-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const siteFileMaterializationSmoke = await runJsonCommand('site-file:materialization-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-site-file-materialization-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const repositoryPublicationSmoke = await runJsonCommand('repository-publication:smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-repository-publication-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
const repositoryPublicationReadiness = await runJsonCommand('repository-publication:readiness-smoke:live', [
  'node',
  'packages/cloudflare-carrier/scripts/cloudflare-carrier-repository-publication-readiness-live-smoke.ts',
  '--url',
  workerUrl,
  '--token-file',
  tokenFile,
...
assert.equal(operationPersistencePosture?.durable_boundaries.some((boundary: any) => boundary.key === 'site_file_materialization_store' && boundary.status === 'available'), true);
assert.equal(operationPersistencePosture?.durable_boundaries.some((boundary: any) => boundary.key === 'local_ingress_request_queue' && boundary.status === 'available'), true);
assert.equal(operationPersistencePosture?.durable_boundaries.some((boundary: any) => boundary.key === 'repository_publication_request_queue' && boundary.status === 'available'), true);
assert.equal(operationPersistencePosture?.durable_boundaries.some((boundary: any) => boundary.key === 'repository_publication_evidence_store' && boundary.status === 'available'), true);
...
assert.match(operationPersistencePosture?.next_action, /^(monitor_persistence_posture|session_snapshot|site_registry|carrier_evidence_index|site_file_materialization_store|task_lifecycle_store|session_without_replayed_evidence|task_projection_without_task_store_binding|carrier_evidence_replay_degraded)$/);
...
assert.equal(operationRecoveryPosture?.recovery_boundaries.some((boundary: any) => boundary.key === 'site_file_materialization_store' && boundary.status === 'recoverable'), true);
assert.equal(operationRecoveryPosture?.recovery_boundaries.some((boundary: any) => boundary.key === 'repository_publication_request_queue' && boundary.status === 'recoverable'), true);
assert.equal(operationRecoveryPosture?.recovery_boundaries.some((boundary: any) => boundary.key === 'repository_publication_evidence_store' && boundary.status === 'recoverable'), true);
...
assert.equal(operationRecoveryPosture?.evidence_session_count, new Set(operationReadAfterContinuity.body.carrier_evidence.filter((group: any) => group.ok === true && (group.events || []).length > 0).map((group: any) => group.carrier_session_id)).size);
...
assert.equal(projectionWriteTask.task_lifecycle_projection_records?.some((record: any) => record.projection_id === projectionWriteSmoke.projection_write_admission_id || record.source_evidence_ref === `source-evidence:${projectionWriteSmoke.task_id}:finished-row`), true);
...
const recordedMailboxDraftReplyProposal = mailboxDraftReplyProposals.find((proposal: any) => proposal.proposal_id === mailboxDraftReplyProposalSmoke.proposal_id);
...
const recordedSiteFileChangeProposal = siteFileChangeProposals.find((proposal: any) => proposal.proposal_id === siteFileChangeProposalSmoke.proposal_id);
...
const recordedSiteFileMaterialization = siteFileMaterializations.find((materialization: any) => materialization.materialization_id === siteFileMaterializationSmoke.materialization_id);
...
const recordedWebhookDelayDirectiveDelivery = webhookDelayDirectiveDeliveries.find((delivery: any) => delivery.delivery_id === webhookDelayDirectiveDelivery.body.record?.delivery_id);
...
assert.equal(operationReadAfterContinuity.body.sessions.some((session: any) => session.carrier_session_id === webhookDelayDirectiveDelivery.body.carrier_session_id), true);
assert.equal(operationReadAfterContinuity.body.carrier_evidence.some((entry: any) => entry.carrier_session_id === webhookDelayDirectiveDelivery.body.carrier_session_id && entry.ok === true && entry.events.some((event: any) => event.event_kind === 'directive_receipt_recorded')), true);
...
const report = {
  ...
  operation: {
    ...
    listed_on_site_read: operations.some((operation: any) => operation.operation_id === operationId),
    ...
    smoke_session_bound: operationRead.body.sessions.some((session: any) => session.carrier_session_id === smoke.carrier_session_id),
  },
  ...
  continuity: {
    ...
    windows_packet_ids: continuitySecond.windows_packets?.map((packet: any) => packet.packet_id) ?? [],
...
function continuityCommand() {
  return [
    'node',
    'scripts/site-continuity-loop.ts',
    'sync-cloudflare',
    '--site',
    siteId,
    '--url',
...
function siteContinuitySchedulerHealthCommand() {
  return [
    'node',
    'packages/cloudflare-carrier/scripts/cloudflare-site-continuity-scheduler.ts',
    '--action',
    'health',
    '--live',
...
function cloudflareContinuityPullCommand() {
  return [
    'node',
    'packages/cloudflare-carrier/scripts/cloudflare-site-continuity-sync.ts',
    'pull-cloudflare',
    '--site',
    siteId,
    '--url',
...
function providerLivenessSchedulerReadbackCommand() {
  return [
    'node',
    'packages/cloudflare-carrier/scripts/cloudflare-carrier-provider-liveness-scheduler.ts',
    '--action',
    'status',
    '--live',
  ];
...
function loadLocalEnv(pathUrl: any) {
...
async function writeLocalEnv({ workerUrl, tokenFile }: any) {
...
async function readableFileStat(path: any) {
  try {
...
async function readConsole(baseUrl: any) {
...
function verifyOperatorConsoleStructure(html: any, script: any) {
  const ids = [...html.matchAll(/\bid="([^"]+)"/g)].map((match: any) => match[1]);
  const idSet = new Set(ids);
  const duplicateIds = ids.filter((id: any, index: any) => ids.indexOf(id) !== index);
  ...
  const elReferenceIds = [...new Set([...script.matchAll(/\bel\('([^']+)'\)/g)].map((match: any) => match[1]))].sort();
  const missingElTargets = elReferenceIds.filter((id: any) => !idSet.has(id));
...
async function postCarrier(baseUrl: any, token: any, body: any) {
...
function expectedSiteProductOverview(statuses: any= []) {
  const actionableStatus = statuses.find((status: any) => {
    const nextAction = status?.next_action || 'monitor_site';
    return nextAction !== 'monitor_site';
  }) ?? null;
  return {
    health_counts: statuses.reduce((counts: any, status: any) => {
    ...
    action_counts: countBy(statuses, (status: any) => status?.next_action || 'monitor_site'),
...
function sitePostureRouteInvariant(overview: any= {}, focusedSiteId: any= '') {
...
function expectedOperationPostureOverview(operations: any= [], product: any= {}, context: any= {}) {
  const items = operationWorkQueueItemsForCheck(operations, product, context);
  const healthCounts: Record<string, number> = { ready: 0, needs_attention: 0 };
  const actionCounts: Record<string, number> = {};
  const reasonCounts: Record<string, number> = {};
  const commandStateCounts: Record<string, number> = {};
  ...
  const next = items.find((item: any) => item.status === 'needs_attention')
...
function operationPostureRouteInvariant(overview: any= {}, activeOperationId: any= '') {
...
function assertProviderSchedulerPosture(posture: any, providerKind: any) {
...
function assertProviderLivenessSchedulerReadback(readback: any) {
...
function assertSiteContinuitySchedulerHealth(readback: any) {
...
function assertLocalCloudContinuityBridgeState(bridge: any) {
...
function operationWorkQueueItemsForCheck(operations: any= [], product: any= {}, context: any= {}) {
  const activeOperationId = context.activeOperationId || '';
...
function operationPathForCheck(operation: any= {}, product: any= {}, context: any= {}) {
  const operationId = operation.operation_id || context.activeOperationId || '';
  const sessions = (product.sessions || []).filter((session: any) => !session.operation_id || session.operation_id === operationId);
  ...
  const attention = operationAttentionForCheck(product).filter((item: any) => !item.operation_id || item.operation_id === operationId);
  const openTasks = tasks.filter((task: any) => taskLifecycleStatusForCheck(task) === 'open');
...
function operationScopeLoadedForCheck(operation: any= {}, product: any= {}, context: any= {}) {
  const operationId = operation.operation_id || context.activeOperationId || '';
  return Boolean(operationId && product.operation?.operation_id === operationId);
}

function operationEventsForCheck(operation: any= {}, product: any= {}, context: any= {}) {
  const operationId = operation.operation_id || context.activeOperationId || '';
  if (!operationId) return [];
  const seen = new Set();
  return (product.carrier_evidence || []).flatMap((entry: any) => entry.events || []).filter((event: any) => {
...
function operationTasksForCheck(operation: any= {}, product: any= {}, context: any= {}) {
  const operationId = operation.operation_id || context.activeOperationId || '';
  if (!operationId) return [];
  const siteId = product.site?.site_id || product.operation?.site_id || context.siteId || '';
  return (product.tasks || []).filter((task: any) => task.operation_id === operationId || task.site_id === siteId);
}

function operationAttentionForCheck(product: any= {}) {
  const tasks = product.tasks || [];
  const seen = new Set();
  return (product.carrier_evidence || []).flatMap((entry: any) => entry.events || [])
...
function operationPostureReasonForCheck(item: any= {}) {
  const action = item.command?.next_action || 'inspect_operation';
...
function taskLifecycleStatusForCheck(task: any= {}) {
  const status = String(task.status || '').toLowerCase();
...
function countBy(items: any= [], keyForItem: any) {
  return items.reduce((counts: any, item: any) => {
...
function countNestedValues(items: any= [], key: any) {
  return items.reduce((counts: any, item: any) => {
...
function commandStatesForOperationProduct(product: any= {}, focus: any= {}) {
  ...
  const focusSession = sessions.find((session: any) => session.carrier_session_id === focusSessionId) || sessions[0] || null;
  const focusEvidence = evidenceGroups.find((entry: any) => entry.carrier_session_id === focusSessionId) || evidenceGroups[0] || null;
  const evidenceEvents = focusEvidence?.events ?? [];
  const focusTask = tasks.find((task: any) => task.carrier_session_id === focusSessionId) || tasks[0] || null;
  const taskEvidenceCount = focusTask
    ? evidenceEvents.filter((event: any) => JSON.stringify(event.payload || {}).includes(focusTask.task_id)).length
    : 0;
  const hasOperationEvidence = evidenceGroups.some((entry: any) => (entry.events || []).length > 0);
  const openTasks = tasks.filter((task: any) => ['open', 'todo', 'pending'].includes(String(task.status || '').toLowerCase()));
  ...
  const evidenceEvent = evidenceEvents[0] || (product.carrier_evidence || []).flatMap((entry: any) => entry.events || [])[0] || {};
  return {
    ...
    authority: classifyCloudflareAuthorityCommandState({
      ...
      refusal_count: authorityDecisions.filter((decision: any) => ['refuse', 'deny'].includes(String(decision.action || '').toLowerCase())).length,
      unresolved_locus_count: authorityDecisions.filter((decision: any) => !decision.authority_locus || decision.authority_locus === 'unresolved').length,
...
async function checkHumanOperatorSession({ workerUrl, siteId, operatorCookieFile, required, requireAction = false }: any) {
...
async function verifyHumanOperatorResidentDispatchAction({ workerUrl, siteId, cookieHeader, principal }: any) {
  ...
  const recorded = (readback.body.resident_dispatch_decisions ?? []).find((decision: any) => decision.dispatch_decision_id === dispatchDecisionId);
...
async function getOperatorSession(baseUrl: any, cookieHeader: any) {
...
async function postCarrierWithCookie(baseUrl: any, cookieHeader: any, body: any) {
  const response = await fetch(new URL('/api/carrier', withTrailingSlash(baseUrl)), {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
...
function normalizeCookieHeader(value: any) {
  const trimmed = String(value ?? '').trim();
  if (!trimmed) return '';
  if (/^cookie\s*:/i.test(trimmed)) return trimmed.replace(/^cookie\s*:/i, '').trim();
  if (trimmed.includes('narada_operator_session=')) return trimmed.split(/\r?\n/).find((line: any) => line.includes('narada_operator_session='))?.trim() ?? '';
  return `narada_operator_session=${trimmed}`;
}

async function runJsonCommand(label: any, command: any): Promise<any> {
  stderr.write(`[cloudflare:operator:check] ${label}\n`);
  const result: any = await spawnCapture(command[0], command.slice(1));
  if (result.code !== 0) {
...
function spawnCapture(command: any, commandArgs: any) {
  return new Promise((resolve: any, reject: any) => {
    ...
    child.on('close', (code: any) => resolve({ code, stdout: childStdout, stderr: childStderr }));
  });
}

function parseJsonObject(output: any) {
...
function tail(value: any, length: any= 1200) {
...
function withTrailingSlash(value: any) {
...
function fail(code: any, detail: any= {}) {
