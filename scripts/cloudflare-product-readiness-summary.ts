...
export function summarizeCloudflareProductReadiness(operatorCheck: any) {
  const requiredFailures: any[] = [];
  const evidence: any = {};
  const knownNonBlocking: any[] = [];
...
function requireValue(condition: any, failures: any, key: any, observed: any) {
  if (condition) return;
  failures.push({ key, observed });
}

function summarizeSchedulerReadback(readback: any, extra: any= {}) {
  return {
...
function requireSchedulerReady(readback: any, failures: any, prefix: any) {
...
function summarizeBoundaries(boundaries: any= [], readyStatus: any) {
  const entries = Array.isArray(boundaries) ? boundaries : [];
  const ready = entries.filter((boundary: any) => boundary?.status === readyStatus).map((boundary: any) => boundary.key).filter(Boolean);
  return {
    ...
    ready_keys: ready,
    missing_keys: REQUIRED_PERSISTENCE_BOUNDARIES.filter((key: any) => !ready.includes(key)),
  };
}

function collectKnownNonBlockingAttention(operatorCheck: any, knownNonBlocking: any) {
