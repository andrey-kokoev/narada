...
const capture: any = await captureOperatorCookie({ workerUrl, cookieFile, host, port, timeoutMs, shouldOpen });
...
async function captureOperatorCookie({ workerUrl, cookieFile, host, port, timeoutMs, shouldOpen }: any) {
  await mkdir(dirname(cookieFile), { recursive: true });
  const server = createServer();
  const captured = new Promise((resolve: any, reject: any) => {
    const timer = setTimeout(() => {
      server.close();
      reject(new Error('operator_cookie_capture_timed_out'));
    }, timeoutMs);
    server.on('request', async (request: any, response: any) => {
      try {
  ...
  if (!address || typeof address === 'string') throw new Error('operator_capture_server_address_unavailable');
...
async function openBrowser(url: any) {
  const command: any = process.platform === 'win32'
    ? ['cmd.exe', ['/c', 'start', '', url]]
...
async function upsertLocalEnv(entries: any) {
  const existing = existsSync(envPath) ? await readFile(envPath, 'utf8') : '';
  const lines = existing.split(/\r?\n/).filter((line: any, index: any, all: any) => index < all.length - 1 || line.length > 0);
  const keys = new Set(Object.keys(entries));
  const next = lines.map((line: any) => {
...
async function runJsonCommand(label: any, command: any) {
