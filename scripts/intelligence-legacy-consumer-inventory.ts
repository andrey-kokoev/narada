...
const SELF_PATH = 'scripts/intelligence-legacy-consumer-inventory.ts';
...
const NEGATIVE_BOUNDARY_PATHS = new Set([
  ...
  'packages/agent-start/bin/verify-registered-site-launchers.ts',
  'packages/nars-provider-runtime/src/canonical-protocol-adapters.ts',
  'packages/nars-capability-gateway/src/mcp-runtime.ts',
]);
...
function isStructuredDocumentationNegative(line: any, symbol: any) {
...
function symbolPattern(symbol: any) {
...
].sort((left: any, right: any) => right.length - left.length));
const SYMBOL_PATTERNS = new Map(SCANNED_SYMBOLS.map((symbol: any) => [symbol, symbolPattern(symbol)]));

function isDirectSymbolDelete(line: any, symbol: any) {
...
function isImmediateSymbolRefusal(line: any, symbol: any) {
...
function isNegativeBoundaryReference({ line, symbol }: any) {
  return isDirectSymbolDelete(line, symbol) || isImmediateSymbolRefusal(line, symbol);
}

function classifyReference({ path, content, line, symbol }: any) {
...
export function scanLegacyIntelligenceEntries(entries: any, { retiredPackagePresent = false }: any= {}) {
  const references: any[] = [];
  const sortedEntries = [...entries]
    .map((entry: any) => ({ path: normalizePath(entry.path), content: String(entry.content ?? '') }))
  ...
  for (const entry of sortedEntries) {
    ...
    for (let index= 0; index < lines.length; index += 1) {
      ...
      for (const symbol of SCANNED_SYMBOLS) {
        if (SYMBOL_PATTERNS.get(symbol)?.test(line) !== true) continue;
  ...
  const violations = references.filter((reference: any) => !reference.admitted);
...
function runCandidateScan(root: any, searchRoots: any, { noIgnore = false }: any= {}) {
...
function distributionRoots(root: any) {
  ...
  return roots.sort((left: any, right: any) => left.localeCompare(right));
}

function candidatePaths(root: any, distributionSearchRoots: any= distributionRoots(root)) {
...
export function inventoryLegacyIntelligence({ root }: any) {
