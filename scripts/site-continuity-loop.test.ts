...
const SCRIPT_PATH = fileURLToPath(new URL('./site-continuity-loop.ts', import.meta.url));

function runLoop(args: any= []) {
...
function createCloudflarePacket({ site_id, generated_at }: any) {
...
test('site continuity loop emits an idempotent offline operator report without token material', async () => {
  ...
  const first: any = await runLoop(args);
  ...
  assert.equal(firstReport.cloudflare_publication.status, 'skipped');
  ...
  const second: any = await runLoop(args);
  assert.equal(second.code, 0, second.stderr);
  const secondReport = JSON.parse(second.stdout);
  assert.equal(secondReport.status, 'ok');
  assert.equal(secondReport.windows_packet_count, 1);
  assert.deepEqual(
    secondReport.windows_packets.map((packet: any) => packet.packet_id),
    firstReport.windows_packets.map((packet: any) => packet.packet_id),
  );
});
