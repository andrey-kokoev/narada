...
function normalizeFocusedCommand(command) {
  ...
  if (
    ...
    return "node scripts/cli-focused-proof.ts task-close";
  }
  ...
  const appendIfMissing = (pattern, text) => {
...
function rejectPreflight(reason) {
...
function validateFocusedCommand(command) {
