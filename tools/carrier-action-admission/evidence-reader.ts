...
function siteEvidenceRoot(siteRoot: any) {
...
function actionAdmissionDir(siteRoot: any) {
  return join(siteEvidenceRoot(siteRoot), 'crew', 'action-admission');
}

function readCarrierActionDecisionFile(path: any) {
  try {
...
function listCarrierActionDecisions(siteRoot: any, options: any= {}) {
  ...
  const decisions = readdirSync(dir)
    ...
    .map((name: any) => readCarrierActionDecisionFile(join(dir, name)))
    .filter((entry: any) => !options.decision || entry.record?.decision === options.decision)
    .sort((a: any, b: any) => entryMtimeMs(b) - entryMtimeMs(a))
    .slice(0, limit)
...
function showCarrierActionDecision(siteRoot: any, requestId: any) {
...
function isSafeRequestId(requestId: any) {
  return /^[A-Za-z0-9_.-]+$/.test(String(requestId ?? ''));
}

function entryMtimeMs(entry: any) {
  try {
...
function summarizeDecisionEntry(entry: any) {
