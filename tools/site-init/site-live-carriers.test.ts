...
} from './site-live-carriers.ts';

function tempSite(name: any= 'narada-proper-site-live-carriers') {
...
function dbOptions(root: any, overrides: any= {}) {
  return {
...
function storageOptions(root: any, overrides: any= {}) {
  return {
    target_site_root: root,
    site_id: path.basename(root).toLowerCase(),
    authority_basis: 'test_receiving_site_authority',
...
function mcpOptions(root: any, overrides: any= {}) {
  return {
    target_site_root: root,
    site_id: path.basename(root).toLowerCase(),
    ...
    mcp_server_descriptors: [{
      ...
      transport: 'stdio',
      command: 'node',
      args: ['tools/site-init/site-doctor.ts'],
      entrypoint: path.join(root, 'tools/site-init/site-doctor.ts'),
    }],
    ...overrides,
  };
}

function profileOptions(root: any, overrides: any= {}) {
  return {
    target_site_root: root,
    site_id: path.basename(root).toLowerCase(),
...
test('local DB carrier plans exact target mutations without writes by default', () => {
  ...
  assert.ok(result.planned_mutations.some((entry: any) => entry.path.endsWith('site-local-db.json')));
});
...
test('MCP registration carrier applies target-local manifest and refuses broader or source authority', () => {
  ...
  const sourceReuse = siteMcpRegistrationCarrier(mcpOptions(root, {
    source_site_root: source,
    mcp_server_descriptors: [{ name: 'source', entrypoint: path.join(source, 'tools/source.ts') }],
  }));
...
test('MCP registration carrier admits policy write roots for bounded local surfaces', () => {
  ...
  assert.equal(server.args.filter((arg: any) => arg === '--allowed-root').length, 2);
...
test('greenfield live path can run all four carriers without source Site state import', () => {
  ...
  assert.deepEqual(auditLines.map((line: any) => JSON.parse(line).carrier_id), [
    LOCAL_DB_CARRIER_ID,
    STORAGE_CARRIER_ID,
    MCP_REGISTRATION_CARRIER_ID,
    WINDOWS_PROFILE_CARRIER_ID,
