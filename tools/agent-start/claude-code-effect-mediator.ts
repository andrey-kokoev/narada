...
function evidencePath(siteRoot: any, id: any) {
  return join(siteRoot, '.narada', 'crew', 'claude-code-effect-requests', `${id}.json`);
}

function inspectPayloadForSecrets(value: any, path: any = []): string[] {
  const findings: string[] = [];
  if (Array.isArray(value)) {
    value.forEach((item: any, index: any) => findings.push(...inspectPayloadForSecrets(item, [...path, String(index)])));
    return findings;
  }
...
function effectRequestEnvelope(request: any) {
...
function mediateEffectRequest(request: any, grants: any= {}) {
...
function writeEffectMediationEvidence(siteRoot: any, decision: any) {
