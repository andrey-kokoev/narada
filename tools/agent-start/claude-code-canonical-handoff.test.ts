...
import { mediateEffectRequest, writeEffectMediationEvidence } from './claude-code-effect-mediator.ts';
import { CANONICAL_SURFACES, canonicalAdmissionCommand, createCanonicalHandoff } from './claude-code-canonical-handoff.ts';
...
test('mediated claude-code request becomes bounded canonical task request artifact', () => {
  ...
  const { handoff, handoff_path: handoffPath, canonical_request_path: requestPath }: any = createCanonicalHandoff(siteRoot, decision, { sourceEvidencePath });
...
test('canonical admission can be delegated to canonical runner without carrier direct mutation', () => {
  ...
  const { handoff, canonical_admission_result: result } = createCanonicalHandoff(siteRoot, decision, {
    ...
    admit: true,
    runCanonicalCommand(command: any, context: any) {
