...
function pathKey(env: any= process.env) {
  return Object.keys(env).find((key: any) => key.toLowerCase() === 'path') ?? 'PATH';
}

function commandNames(command: any, platform: any= process.platform) {
  if (platform !== 'win32') return [command];
  if (/\.[A-Za-z0-9]+$/.test(command)) return [command];
  return WINDOWS_EXTENSIONS.map((extension: any) => `${command}${extension}`);
}
...
function discoverClaudeCodeRuntime({
  ...
  const candidates = unique(envPath
    .split(pathDelimiter)
    .filter(Boolean)
    .flatMap((dir: any) => names.map((name: any) => join(dir, name)))
    .filter((candidate: any) => fileExists(candidate)));
...
function readinessFromDiscovery(discovery: any) {
  return {
...
function launchEvidencePath(siteRoot: any, launchPacket: any) {
  return join(
    siteRoot,
    '.narada',
    'crew',
...
function sanitizeLaunchPacket(launchPacket: any) {
  return {
...
function carrierEnvironment(launchPacket: any) {
  return Object.fromEntries(
    Object.entries(launchPacket.required_environment ?? {})
      .filter(([key, value]: any) => key.startsWith('NARADA_') && typeof value === 'string'),
  );
}

function launchBridgeEvidence({ launchPacket, discovery, phase, runtimeHandle = null, closeout = null, now = new Date().toISOString() }: any) {
...
function writeLiveLaunchEvidence(siteRoot: any, launchPacket: any, evidence: any) {
