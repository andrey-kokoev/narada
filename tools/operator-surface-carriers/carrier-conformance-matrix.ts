...
import { parseLaunchRegistry } from '../mcp-fabric/site-fabric-audit.js';
import {
...
type LaunchRegistrySummary = {
  launch_registry_path: string;
  status: 'missing' | 'loaded';
  agent_count?: number;
  runtime_counts: Record<string, number>;
  native_shell_enabled_counts: Record<string, number>;
};
...
export function currentLaunchRegistrySummary(
  launchRegistryPath = DEFAULT_LAUNCH_REGISTRY,
): LaunchRegistrySummary {
  ...
  const nativeShellEnabledCounts: Record<string, number> = {};
...
function buildCarrierRows(launchRegistrySummary: LaunchRegistrySummary) {
...
export function buildCarrierConformanceMatrix({
  launchRegistryPath = DEFAULT_LAUNCH_REGISTRY,
...
function parseArgs(argv: string[]) {
  const options: { launchRegistryPath: string; help?: boolean } = {
    launchRegistryPath: DEFAULT_LAUNCH_REGISTRY,
  };
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === '--registry' && argv[index + 1]) {
      options.launchRegistryPath = argv[++index];
...
function runCli(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    console.log('Usage: node tools/operator-surface-carriers/carrier-conformance-matrix.ts [--registry <agents.psd1>]');
    return 0;
  }
