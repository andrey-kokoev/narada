...
function runCoherenceGate({ launchRegistryPath = DEFAULT_LAUNCH_REGISTRY }: any= {}) {
...
function parseArgs(argv: any) {
  const options: any = { launchRegistryPath: DEFAULT_LAUNCH_REGISTRY, pretty: false };
...
function runCli(argv: any= process.argv.slice(2)) {
  ...
  if (options.help) {
    console.log('Usage: node tools/mcp-fabric/coherence-gate.ts [--registry <agents.psd1>] [--pretty]');
    return 0;
  }
