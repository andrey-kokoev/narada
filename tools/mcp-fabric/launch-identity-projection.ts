...
import { effectiveSiteRoot, launcherKnownSites, parseLaunchRegistry } from './site-fabric-audit.ts';

const DEFAULT_LAUNCH_REGISTRY = 'C:/Users/Andrey/Narada/config/launch/agents.psd1';

function identityProjectionPath(siteRoot: any) {
  return join(effectiveSiteRoot(normalize(siteRoot)), 'operator-surfaces', 'identities.json');
}

function buildIdentityProjection(siteRoot: any, records: any, options: any= {}) {
  const normalizedSiteRoot = effectiveSiteRoot(normalize(siteRoot));
  const identities = records
    .filter((record: any) => effectiveSiteRoot(normalize(record.narada_root)) === normalizedSiteRoot)
  ...
  return {
    ...
    roles: Object.fromEntries([...new Set(identities.map((identity: any) => identity.role))].map((role: any) => [role, { label: role, affinity_color: '000000' }])),
    updated_at: options.generatedAt ?? new Date().toISOString(),
  };
}

function writeIdentityProjection(siteRoot: any, records: any, options: any= {}) {
  const normalizedSiteRoot = effectiveSiteRoot(normalize(siteRoot));
...
function auditLaunchIdentities(launchRegistryPath: any= DEFAULT_LAUNCH_REGISTRY) {
  const records = parseLaunchRegistry(launchRegistryPath);
  const sites = launcherKnownSites(launchRegistryPath);
  const failures: any[] = [];
  const warnings: any[] = [];
  const siteResults = sites.map((site: any) => {
    ...
    const activeIds = new Set(identities.filter((identity: any) => identity?.deprecated !== true).map((identity: any) => identity.identity_id));
    const activeById = new Map<string, any>(identities.filter((identity: any) => identity?.deprecated !== true).map((identity: any) => [identity.identity_id, identity]));
    const siteRecords = records.filter((record: any) => effectiveSiteRoot(normalize(record.narada_root)) === normalizedSiteRoot);
    const missingAgents = siteRecords.map((record: any) => record.agent).filter((agent: any) => !activeIds.has(agent));
    if (status !== 'loaded') {
...
function writeMissingIdentityProjections(launchRegistryPath: any= DEFAULT_LAUNCH_REGISTRY, options: any= {}) {
  const records = parseLaunchRegistry(launchRegistryPath);
  const sites = launcherKnownSites(launchRegistryPath);
  const results = [];
...
function inferRole(agentId: any) {
...
function parseArgs(argv: any) {
  const options: any = { launchRegistryPath: DEFAULT_LAUNCH_REGISTRY, pretty: false, writeMissing: false, overwrite: false };
...
function runCli(argv: any= process.argv.slice(2)) {
  ...
  if (options.help) {
    console.log('Usage: node tools/mcp-fabric/launch-identity-projection.ts [--registry <agents.psd1>] [--write-missing] [--overwrite] [--pretty]');
    return 0;
  }
