...
} from './launch-identity-projection.ts';
...
assert.equal(after.warnings.some((warning: any) => warning.code === 'identity_projection_is_launch_index_projection'), true);
...
assert.equal(drift.failures.some((failure: any) => failure.code === 'launch_agent_role_mismatch'), true);
