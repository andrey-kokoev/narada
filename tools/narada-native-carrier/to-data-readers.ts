...
import { buildToDataPacketFixture } from './to-data-packet.ts';
import { readRegistration, registrationReadiness } from './adapter-registration.ts';
import { resolveDataReadCapabilityProjection } from './capability-projection.ts';
import { operationalReadiness, reconstruct } from './readiness.ts';
import { supervisorDoctor } from './supervisor.ts';
...
function runNaradaTaskRead(taskNumber: any, { siteRoot }: any): any {
...
function runNaradaWorkNextPeek(agentId: any, { siteRoot }: any): any {
...
function runNaradaInboxList(_request: any, { siteRoot, limit }: any): any {
...
async function readCapabilityGate(readFamily: any, readCapabilityLookup: any, now: any): Promise<any> {
...
function buildCapabilityRefusalPacket(readFamily: any, args: any): any {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
async function readTaskToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
async function readWorkNextToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
async function readInboxSummaryToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
async function readReadinessSnapshotToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
  ...
  readRegistrationReadiness = (root: any) => registrationReadiness(readRegistration(root)),
  readCapabilityLookup,
  now = new Date().toISOString(),
  } = args;
...
async function readEvidenceRefSummaryToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
  ...
  const refs = {
    ...
    ...Object.fromEntries((doctor.supervisor_event_paths ?? []) .map((path: any, index: any) => [`supervisor_${index}`, path])),
  };
...
async function readBoundedFileExcerptToDataPacket(args: any): Promise<any> {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
function buildReadPacket(readFamily: any, args: any): any {
  const {
  siteRoot,
  carrierSessionId,
  agentId,
...
function parseJsonSurface(value: any) {
  return typeof value === 'string' ? JSON.parse(value) : value;
}

function fieldPresence(value: any, fields: any) {
  const record = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  return Object.fromEntries(fields.map((field: any) => [field, record[field] !== undefined && record[field] !== null]));
}

function extractEnvelopeArray(parsed: any) {
...
function summarizeInboxEnvelope(envelope: any) {
...
function scalarOrNull(value: any) {
  return ['string', 'number', 'boolean'].includes(typeof value) ? value : null;
}

function textShape(value: any) {
  return {
...
function safeKeys(record: any) {
  return Object.keys(record).filter((key) => !isSecretLikeKey(key)).sort();
}

function isSecretLikeKey(key: any) {
  return /secret|token|password|credential|authorization|api[_-]?key/i.test(key);
}

function fileExcerptRefusal({ siteRoot, filePath, capabilityRef, maxBytes, maxLines }: any) {
...
function pathAttribution(siteRoot: any, filePath: any) {
...
function summarizeEvidenceFile(name: any, path: any) {
  const summary: any = {
    name,
    path,
