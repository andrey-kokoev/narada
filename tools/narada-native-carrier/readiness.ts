...
function readIfExists(path: any) {
  if (typeof path !== 'string' || path.length === 0) return null;
  return existsSync(path) ? JSON.parse(readFileSync(path, 'utf8')) : null;
}

function reconstruct(siteRoot: any, carrierSessionId: any) {
  const dir = sessionDir(siteRoot, carrierSessionId);
  const files = existsSync(dir) ? readdirSync(dir).filter((name: any) => name.endsWith('.json')) : [];
  const refs = Object.fromEntries(files.map((name: any) => [name.replace(/\.json$/, ''), join(dir, name)]));
  const orderedEvents = orderedSessionEvents(refs);
  return {
    schema: 'narada.narada_native_carrier.reconstruction.v0',
    carrier_session_id: carrierSessionId,
    evidence_refs: refs,
    event_order: orderedEvents.map((event: any) => event.evidence_ref),
    ...
    supervisor_events: orderedEvents.filter((event: any) => event.name.startsWith('supervisor-')),
...
function orderedSessionEvents(refs: any) {
...
function comparableTime(value: any) {
  return typeof value === 'string' && value.length > 0 ? value : '9999-12-31T23:59:59.999Z';
}

function latestPostureSummary(orderedEvents: any) {
  const latest = [...orderedEvents].reverse().find((event: any) => event.recorded_at) ?? orderedEvents[orderedEvents.length - 1] ?? null;
  if (!latest) {
    return {
      status: 'missing',
...
function supervisorControlReconstruction(refs: any) {
...
function wrapperEvidenceRefs(refs: any) {
  return Object.fromEntries(Object.entries(refs).filter(([name]: any) => (
...
function wrapperEvidenceSummaries(refs: any) {
  return Object.entries(wrapperEvidenceRefs(refs)).map(([name, path]: any) => {
    const record = readIfExists(path);
    return {
      name,
      path,
...
function capabilityConsentReconstruction(refs: any) {
  ...
  const projections = [...projectionStatuses, ...adapterProjection].map((projection: any) => ({
...
function adapterEvidenceResiduals(adapterEvidence: any) {
  if (!adapterEvidence) {
...
function operationalReadiness(siteRoot: any, carrierSessionId: any) {
  ...
  return {
    ...
    commands: {
      ...
      inspect: 'node --test tools\\narada-native-carrier\\readiness.test.ts',
...
function reconstructHandoffArtifacts(refs: any) {
  return Object.entries(refs)
    .filter(([name]: any) => name.endsWith('-handoff-payload'))
    .map(([name, path]: any) => {
...
function handoffFamilyFromName(name: any) {
