...
import { readRegistration, writeRegistration, registrationReadiness } from './adapter-registration.ts';
...
test('kimi adapter represents missing capability endpoint model refusal and malformed response', async () => {
  ...
  const missingEndpoint: any = await executeProviderAdapter({
    siteRoot,
...
test('kimi provider refusal failure and timeout are bounded and redacted', async () => {
  const siteRoot = tempSite();
  const providerRefusal: any = normalizeKimiChatCompletionResponse({
