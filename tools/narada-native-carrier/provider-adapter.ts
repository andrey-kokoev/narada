...
function requestSummary(input: any= {}) {
  return {
...
function refusalOutput({ adapterId, reason, diagnostic }: any) {
  return {
...
function normalizeProviderResponse({ adapterId, response }: any) {
...
async function withTimeout(promise: any, timeoutMs: any) {
...
function makeProviderRegistry(entries: any= {}) {
...
async function executeProviderAdapter({
  ...
  const projectionLookup: any = await resolveProviderCapabilityProjection({
    registration,
    capabilityLookup,
    now,
  });
