import { fixtureAdapter, sanitizeAdapterOutput } from './adapter.ts';
...
function selectIntelligenceRoute({ registration, readiness, providerExecutor }: any) {
...
function buildStageResult({ mode, status, route, output, now }: any) {
...
function boundedProviderProblem(kind: any, route: any, now: any, reason: any= null) {
  return {
    schema: 'narada.narada_native_carrier.to_intelligence_stage_result.v0',
    mode: 'provider_backed',
    status: kind,
...
function normalizeProviderOutput(output: any) {
  return {
...
function inertProposalFromOutput(output: any) {
  return {
    status: 'inert_proposal',
...
function promptSummary(packets: any) {
