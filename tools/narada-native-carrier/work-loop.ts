...
import { invokeAdapter } from './adapter.ts';
import { executeProviderAdapter } from './provider-adapter.ts';
...
function loopDir(siteRoot: any, carrierSessionId: any) {
  return join(siteRoot, '.narada', 'crew', 'narada-native-carrier-sessions', carrierSessionId);
}

function writeLoopArtifacts({ siteRoot, carrierSessionId, adapterEvidence, mode, now }: any) {
  const handoff = {
  ...
  const closeout = {
    ...
    authority_refusals: AUTHORITY_REFUSALS.map((refusal: any) => ({
      ...refusal,
...
function runFixtureWorkLoop({ siteRoot, carrierSessionId, startupContext, workPacket, now = new Date().toISOString() }: any) {
