...
import { buildLiveStartEvidence, buildMissingRuntimeHandle } from './live-start.ts';
import { buildLocalProcessRuntimeHandle } from './runtime-handle.ts';

const NOW = '2026-05-16T03:30:00.000Z';

function assertNoEffects(evidence: any) {
...
test('live start evidence blocks missing required local executable and missing runtime handle', async () => {
  ...
  const executableMissing: any = await buildLiveStartEvidence({
