...
import { buildNaradaNativeDoctorCommand } from './doctor-command.ts';
import {
...
test('doctor command returns compact bounded JSON and human output', () => {
  ...
  const json: any = buildNaradaNativeDoctorCommand({ siteRoot, carrierSessionId, format: 'json' });
  const human: any = buildNaradaNativeDoctorCommand({ siteRoot, carrierSessionId, format: 'human' });
  ...
  assert.match(json.result.next_diagnostic_command, /supervisor-cli\.ts doctor/);
...
test('doctor command distinguishes configured blocked fixture provider live failed and stopped states', () => {
  ...
  const configured: any = buildNaradaNativeDoctorCommand({
    siteRoot: configuredSite,
    carrierSessionId: 'carrier_session_configured',
  }).result;
  const blocked: any = buildNaradaNativeDoctorCommand({
    siteRoot: blockedSite,
    carrierSessionId: 'carrier_session_blocked',
  }).result;
  const fixture: any = buildNaradaNativeDoctorCommand({
    siteRoot: fixtureSite,
    carrierSessionId: 'carrier_session_fixture',
  }).result;
  const provider: any = buildNaradaNativeDoctorCommand({
    siteRoot: providerSite,
    carrierSessionId: 'carrier_session_provider',
  }).result;
  const failed: any = buildNaradaNativeDoctorCommand({
    siteRoot: failedSite,
    carrierSessionId: 'carrier_session_failed',
  }).result;
  const stopped: any = buildNaradaNativeDoctorCommand({
    siteRoot: stoppedSite,
    carrierSessionId: 'carrier_session_stopped',
  }).result;
  const degraded: any = buildNaradaNativeDoctorCommand({
    siteRoot: degradedSite,
    carrierSessionId: 'carrier_session_degraded',
  }).result;
...
test('doctor command output omits secrets raw prompts and model output', () => {
  ...
  const json: any = runSupervisorCli([
    'doctor',
    '--site-root', siteRoot,
    '--carrier-session-id', carrierSessionId,
    '--format', 'json',
  ]);
  const human: any = runSupervisorCli([
    'doctor-compact',
    '--site-root', siteRoot,
    '--carrier-session-id', carrierSessionId,
    '--format', 'human',
