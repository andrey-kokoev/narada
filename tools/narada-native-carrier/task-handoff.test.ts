...
import { readTaskPacketViaNaradaCli, runGovernedTaskHandoff } from './task-handoff.ts';
import { reconstruct } from './readiness.ts';
import { writeRegistration } from './adapter-registration.ts';
import { makeProviderRegistry } from './provider-adapter.ts';

function tempSite() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'narada-native-task-handoff-'));
}

function taskPacket(overrides: any= {}) {
  return {
    task_number: 1298,
...
test('native task handoff reads bounded packet and emits report draft without lifecycle mutation', async () => {
  ...
  const { result, draft_path: draftPath }: any = await runGovernedTaskHandoff({
    siteRoot,
    carrierSessionId,
  ...
  const reconstructed: any = reconstruct(siteRoot, carrierSessionId);
  ...
  for (const evidencePath of Object.values(result.evidence_refs) as string[]) {
...
test('native task handoff default reader uses bounded narada task read surface', async () => {
  const siteRoot = tempSite();
  const calls: any[] = [];
  const { result }: any = await runGovernedTaskHandoff({
    siteRoot,
    carrierSessionId: 'carrier_session_cli_reader',
    agentId: 'narada.builder',
    taskNumber: 1298,
    readTaskPacketCommandRunner: async (taskNumber: any, context: any) => {
...
test('native task handoff dispatches configured provider registration through governed work loop', async () => {
  ...
  const { result, draft_path: draftPath }: any = await runGovernedTaskHandoff({
    ...
    readTaskPacket: async () => taskPacket(),
    capabilityGrants: { task_report_draft: true },
    capabilityLookup: async (ref: any) => ({
      granted: true,
      capability_ref: ref,
...
test('bounded narada CLI reader parses task read JSON without raw markdown', async () => {
  const siteRoot = tempSite();
  const packet: any = await readTaskPacketViaNaradaCli(1298, {
    siteRoot,
...
test('native task handoff refuses missing capability before loop execution', async () => {
  ...
  const draft: any = JSON.parse(fs.readFileSync(draftPath, 'utf8'));
