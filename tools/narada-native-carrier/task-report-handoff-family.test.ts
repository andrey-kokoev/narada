...
import { validateCarrierActionPacket } from './carrier-action-packet.ts';
...
test('task-report handoff emits inert reconstructable packet and payload ref', () => {
  ...
  const result = emitTaskReportHandoffPacket({
    ...
    changedFileRefs: ['tools/narada-native-carrier/task-report-handoff-family.ts'],
    verificationRefs: ['node --test task-report-handoff-family.test.ts'],
