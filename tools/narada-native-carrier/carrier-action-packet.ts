...
function validateCarrierActionPacket(packet: any) {
  const errors = [];
...
function boundedSummary(summary: any) {
...
function boundedPayloadSummary(payloadSummary: any) {
  ...
  const keys = Object.keys(payloadSummary)
    .filter((key: any) => !SECRET_KEY_PATTERN.test(key))
    .sort()
    .slice(0, 40);
  return {
    keys,
...
function defaultPacketId({ carrierSessionId, actionFamily, payloadRef }: any) {
