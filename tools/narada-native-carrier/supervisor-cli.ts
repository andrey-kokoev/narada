...
function parseArgs(argv: any) {
  ...
  for (let i= 0; i < rest.length; i += 1) {
    ...
    const name = key.slice(2).replace(/-([a-z])/g, (_: any, letter: any) => letter.toUpperCase());
...
function requireOption(options: any, name: any) {
  const value = options[name];
  if (!value) throw new Error(`--${name.replace(/[A-Z]/g, (letter: any) => `-${letter.toLowerCase()}`)} is required`);
  return value;
}

function supervisedOptions(options: any) {
  return {
...
function runSupervisorCli(argv: any= process.argv.slice(2)) {
