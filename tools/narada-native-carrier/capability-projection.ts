...
export function buildCapabilityProjection(state: any, overrides: any= {}) {
...
export function validateCapabilityProjection(projection: any) {
  const errors = [];
...
export async function resolveDataReadCapabilityProjection({
  ...
  const capabilityKind = (DATA_READ_CAPABILITY_BINDINGS as Record<string, string>)[readFamily] ?? readFamily;
...
function projectionBaseFor(state: any) {
  const base = {
...
function refusedProviderProjection({ reason, capabilityRef, capability = null, now, grantFreshness = null }: any) {
  return {
    schema: 'narada.narada_native_carrier.provider_capability_lookup.v0',
    status: 'refused',
...
function refusedDataReadProjection({ reason, capabilityRef, capabilityKind, capability = null, now, grantFreshness = null }: any) {
  return {
    schema: 'narada.narada_native_carrier.data_read_capability_lookup.v0',
    status: 'refused',
    refusal_reason: reason,
...
function grantStatusFor(capability: any, projectionState: any) {
...
function defaultDataReadCapabilityLookup(capabilityKind: any) {
  return {
    granted: true,
...
function consentRefsFor(capability: any) {
  const refs = [
...
function policyRefsFor(capability: any) {
  const refs = [
...
function boundedGrantFreshness(capability: any, now: any) {
...
function scopeSummaryFor(capability: any) {
...
export function inspectCapabilityMaterialForSecrets(value: any, path: any = []): string[] {
  const findings: string[] = [];
  if (Array.isArray(value)) {
    value.forEach((item: any, index: any) => findings.push(...inspectCapabilityMaterialForSecrets(item, [...path, String(index)])));
    return findings;
  }
  if (isRecord(value)) {
