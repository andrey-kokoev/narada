...
import { materializeAndClose } from './harness.ts';
import { emitCanonicalHandoffDraft } from './handoff-emission-stage.ts';
import {
...
import { emitCommandIntentHandoffPacket } from './command-intent-handoff-family.ts';
import { emitInboxHandoffPacket } from './inbox-handoff-family.ts';
import { emitTaskReportHandoffPacket } from './task-report-handoff-family.ts';
...
test('native carrier reconstruction includes bounded wrapper evidence refs', () => {
  ...
  const reconstructed = reconstruct(siteRoot, carrierSessionId);
  const summaryNames = reconstructed.wrapper_evidence_summaries.map((summary: any) => summary.name).sort();
  ...
  assert.equal(reconstructed.wrapper_evidence_summaries.every((summary: any) => summary.values_omitted), true);
  assert.equal(reconstructed.wrapper_evidence_summaries.every((summary: any) => summary.raw_prompt_recorded === false), true);
  assert.equal(reconstructed.wrapper_evidence_summaries.every((summary: any) => summary.raw_provider_output_recorded === false), true);
  assert.equal(reconstructed.wrapper_evidence_summaries.every((summary: any) => summary.raw_transcript_recorded === false), true);
  assert.equal(reconstructed.wrapper_evidence_summaries.every((summary: any) => summary.raw_secret_values_recorded === false), true);
...
test('native carrier reconstructs live session posture from ordered durable JSON evidence', () => {
  ...
  const reconstructed = reconstruct(siteRoot, carrierSessionId);
  const orderedNames = reconstructed.ordered_events.map((event: any) => event.name);
  const reconstructedText = JSON.stringify(reconstructed);
...
test('readiness and doctor expose bounded handoff artifacts for all families', () => {
  ...
  const families = reconstructed.handoff_artifacts.map((artifact: any) => artifact.family).sort();
  ...
  assert.equal(reconstructed.handoff_artifacts.every((artifact: any) => artifact.requires_canonical_admission), true);
  assert.equal(reconstructed.handoff_artifacts.every((artifact: any) => artifact.direct_mutation_performed === false), true);
  assert.equal(reconstructed.handoff_artifacts.every((artifact: any) => artifact.payload_ref.endsWith('.json')), true);
