...
function commandIntentPayloadPath(siteRoot: any, carrierSessionId: any) {
...
function boundedEnvPolicy(envPolicy: any) {
  const allowed_keys = Array.isArray(envPolicy?.allowed_keys)
    ? envPolicy.allowed_keys.filter((key: any) => typeof key === 'string' && !SECRET_KEY_PATTERN.test(key)).slice(0, 50)
    : [];
  return {
