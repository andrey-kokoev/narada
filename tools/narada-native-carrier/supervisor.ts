...
import { operationalReadiness, reconstruct } from './readiness.ts';
import { readRegistration, registrationReadiness } from './adapter-registration.ts';
import { buildFixtureRuntimeHandle } from './runtime-handle.ts';
import { buildHeartbeatEvidence } from './heartbeat-evidence.ts';

function sessionDir(siteRoot: any, carrierSessionId: any) {
  return join(siteRoot, '.narada', 'crew', 'narada-native-carrier-sessions', carrierSessionId);
}

function supervisorPath(siteRoot: any, carrierSessionId: any, phase: any) {
  return join(sessionDir(siteRoot, carrierSessionId), `supervisor-${phase}.json`);
}

function writeSupervisorEvidence(siteRoot: any, carrierSessionId: any, phase: any, record: any) {
...
function baseEvidence({ siteRoot, carrierSessionId, agentId, phase, state, now = new Date().toISOString(), runtimeHandle = null }: any) {
...
function startSupervisedSession(options: any) {
  ...
  const evidence: any = baseEvidence({
    ...options,
    phase: 'start',
    state: 'running',
    runtimeHandle,
...
function heartbeatSupervisedSession(options: any) {
...
function boundedWrapperStageSummary(stage: any) {
  return {
...
function interruptSupervisedSession(options: any) {
...
function closeSupervisedSession(options: any) {
...
function failSupervisedSession(options: any) {
  const evidence = {
...
function normalizeStatus(value: any, allowed: any, fallback: any) {
  return allowed.includes(value) ? value : fallback;
}

function boundedReasonClass(reason: any) {
...
function boundedDiagnostics(diagnostics: any) {
  ...
  const codes = Array.isArray(diagnostics.codes) ? diagnostics.codes.filter((code: any) => typeof code === 'string').slice(0, 20) : [];
  const classes = Array.isArray(diagnostics.classes) ? diagnostics.classes.filter((entry: any) => typeof entry === 'string').slice(0, 20) : [];
  return {
    codes,
    classes,
...
function boundedEvidenceRefs(refs: any) {
  ...
  return refs.filter((ref: any) => typeof ref === 'string' && ref.length > 0).slice(0, 20);
}

function readSupervisorEvents(siteRoot: any, carrierSessionId: any) {
  const dir = sessionDir(siteRoot, carrierSessionId);
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((name: any) => name.startsWith('supervisor-') && name.endsWith('.json'))
    .map((name: any) => {
      const path = join(dir, name);
      return { path, event: JSON.parse(readFileSync(path, 'utf8')) };
    })
    .sort((a: any, b: any) => a.event.recorded_at.localeCompare(b.event.recorded_at));
}

function supervisorDoctor(siteRoot: any, carrierSessionId: any) {
  ...
  return {
    ...
    next_diagnostic_command: `node tools\\narada-native-carrier\\supervisor-cli.ts doctor --site-root <site-root> --carrier-session-id ${carrierSessionId}`,
    automatic_repair_mutation: false,
    latest_supervisor_event_path: events[events.length - 1]?.path ?? null,
    supervisor_event_paths: events.map((entry: any) => entry.path),
...
function doctorState({ latest, blocked, registration }: any) {
...
function capabilityProjectionPosture({ events, readiness, registration }: any) {
  const statuses = events.flatMap((entry: any) => entry.event.capability_projection_statuses ?? []);
  const refused = statuses.find((status: any) => status.status === 'refused' || status.refusal_reason);
