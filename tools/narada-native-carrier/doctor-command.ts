import { operationalReadiness } from './readiness.ts';
...
function compactDoctorPayload({ doctor, readiness, carrierSessionId }: any) {
...
function consentPosture(consent: any) {
  if (!consent || typeof consent !== 'object') return 'unknown';
  if (consent.projection_count > 0 && consent.projections?.some((projection: any) => projection.refusal_reason)) {
    return 'blocked';
  }
...
function stateMarkers({ doctor, readiness }: any) {
...
function renderNaradaNativeDoctorHuman(payload: any) {
  const refs = payload.latest_evidence_refs.map((ref: any) => `${ref.name}=${ref.ref}`).join(', ') || 'none';
...
function boundedEvidenceRefs(refs: any) {
  ...
  return Object.entries(refs)
    .filter(([name, ref]: any) => typeof name === 'string' && typeof ref === 'string' && !SECRET_VALUE_PATTERN.test(ref))
...
function boundedStringArray(values: any) {
  return Array.isArray(values)
    ? values.filter((value: any) => typeof value === 'string' && !SECRET_VALUE_PATTERN.test(value)).map((value: any) => value.slice(0, 200)).slice(0, 40)
    : [];
}
