...
function openrouterRegistration(record: any= {}) {
  return {
...
function resolveOpenRouterConfig(capability: any= {}) {
  return {
...
function openrouterRefusal(reason: any, diagnostic: any) {
  return {
    status: 'refused',
    reason,
    diagnostic,
...
function openrouterChatCompletionPayload({ prompt, model, routerRef }: any) {
  return {
    model,
    messages: [
...
function normalizeOpenRouterChatCompletionResponse(response: any) {
...
function makeOpenRouterProviderAdapter({ transport }: any= {}) {
  return async function openrouterProviderAdapter({ capability, credential_ref: credentialRef, request }: any) {
