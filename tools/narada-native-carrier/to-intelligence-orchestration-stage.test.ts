...
test('to-intelligence stage selects provider route and preserves output as inert proposal', async () => {
  const result: any = await runToIntelligenceOrchestrationStage({
...
test('to-intelligence stage represents provider timeout failure and malformed output as bounded evidence', async () => {
  const timeout: any = await runToIntelligenceOrchestrationStage({
    registration: REGISTRATION,
    readiness: READY,
    providerExecutor: async () => {
  ...
  const failure: any = await runToIntelligenceOrchestrationStage({
    registration: REGISTRATION,
    readiness: READY,
    providerExecutor: async () => {
  ...
  const malformed: any = await runToIntelligenceOrchestrationStage({
    registration: REGISTRATION,
    readiness: READY,
