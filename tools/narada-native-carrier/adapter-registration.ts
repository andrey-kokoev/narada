...
function registrationPath(siteRoot: any) {
  return join(siteRoot, REGISTRATION_RELATIVE_PATH);
}

function inspectSecretBearingConfig(value: any, path: any = []): string[] {
  const findings: string[] = [];
  if (Array.isArray(value)) {
    value.forEach((item: any, index: any) => findings.push(...inspectSecretBearingConfig(item, [...path, String(index)])));
    return findings;
  }
...
function sanitizeRegistration(record: any) {
  return {
...
function validateRegistration(record: any, grantedCapabilities: any= {}) {
  ...
  const unsafePolicyKeys = UNSAFE_EVIDENCE_POLICY_KEYS.filter((key: any) => record.evidence_policy?.[key] === true);
...
function registrationReadiness(record: any= null, grantedCapabilities: any= {}) {
  if (!record) {
    return {
...
function readRegistration(siteRoot: any) {
...
function writeRegistration(siteRoot: any, record: any, grantedCapabilities: any= {}) {
