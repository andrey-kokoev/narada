...
function projectEvidenceRefs(siteRoot: any, carrierSessionId: any, { now = new Date().toISOString() }: any= {}) {
  const dir = join(siteRoot, '.narada', 'crew', 'narada-native-carrier-sessions', carrierSessionId);
  const refs = existsSync(dir)
    ? readdirSync(dir).filter((name: any) => name.endsWith('.json')).map((name: any) => {
...
function readJsonSafe(path: any) {
  try {
...
function evidenceFamily(name: any, record: any) {
...
function recencyBucket(recordedAt: any, now: any) {
