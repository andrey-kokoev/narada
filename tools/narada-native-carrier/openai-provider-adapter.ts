...
function openaiRegistration(record: any= {}) {
  return {
...
function resolveOpenAIConfig(capability: any= {}) {
  return {
...
function openaiRefusal(reason: any, diagnostic: any) {
  return {
    status: 'refused',
    reason,
    diagnostic,
...
function openaiChatCompletionPayload({ prompt, model }: any) {
  return {
    model,
    messages: [
...
function makeOpenAIProviderAdapter({ transport }: any= {}) {
  return async function openaiProviderAdapter({ capability, credential_ref: credentialRef, request }: any) {
