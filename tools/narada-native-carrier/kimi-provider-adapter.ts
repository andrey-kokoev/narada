...
function kimiRegistration(record: any= {}) {
  return {
...
function resolveKimiConfig(capability: any= {}) {
  return {
...
function kimiRefusal(reason: any, diagnostic: any) {
  return {
    status: 'refused',
    reason,
    diagnostic,
...
function kimiRequestPayload({ prompt, model }: any) {
  return {
    model,
    messages: [
...
function normalizeKimiChatCompletionResponse(response: any) {
...
function makeKimiProviderAdapter({ transport }: any= {}) {
  return async function kimiProviderAdapter({ capability, credential_ref: credentialRef, request }: any) {
