...
test('native work loop runs in no-effect mode and emits governed handoff evidence', () => {
  ...
  assert.deepEqual(closeout.authority_refusals.map((refusal: any) => refusal.surface), [
    'task_report',
    'task_close',
    'task_review',
    'inbox',
  ...
  assert.equal(closeout.authority_refusals.every((refusal: any) => refusal.mutation_performed === false), true);
  assert.equal(closeout.authority_refusals.every((refusal: any) => refusal.canonical_admission_required === true), true);
  assert.equal(closeout.authority_refusals.every((refusal: any) => typeof refusal.refusal_reason === 'string' && refusal.refusal_reason.length > 0), true);
