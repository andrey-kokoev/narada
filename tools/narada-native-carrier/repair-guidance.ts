...
function normalizeBlockedState(state: any) {
  const allowed = [
...
function guidanceFor(state: any, carrierSessionId: any) {
  ...
  return {
    ...
    commands: [
      doctor,
      'node tools\\narada-native-carrier\\supervisor-cli.ts inspect --site-root <site-root> --carrier-session-id <carrier-session-id>',
    ],
  };
}
