...
import { buildCarrierActionPacket } from './carrier-action-packet.ts';
...
function outboxIntentPayloadPath(siteRoot: any, carrierSessionId: any) {
  return join(siteRoot, '.narada', 'crew', 'narada-native-carrier-sessions', carrierSessionId, 'outbox-intent-handoff-payload.json');
}

function repositoryPublicationPayloadPath(siteRoot: any, carrierSessionId: any) {
...
function boundedPayloadSummary(payloadSummary: any) {
  if (!payloadSummary || typeof payloadSummary !== 'object' || Array.isArray(payloadSummary)) {
    return { keys: [], value_count: 0, values_omitted: true };
  }
  return {
    keys: Object.keys(payloadSummary).filter((key: any) => !SECRET_KEY_PATTERN.test(key)).sort().slice(0, 40),
...
function boundedStringArray(values: any) {
  ...
  return values
    .filter((value: any) => typeof value === 'string' && value.length > 0 && !SECRET_VALUE_PATTERN.test(value) && !SECRET_KEY_PATTERN.test(value))
    .map((value: any) => value.slice(0, 300))
    .slice(0, 80);
}

function boundedText(value: any) {
