...
import { readRegistration, registrationReadiness, writeRegistration } from './adapter-registration.ts';
...
} from './openrouter-provider-adapter.ts';
...
test('openrouter provider kind registers as provider configured with capability reference', () => {
  ...
  const { readiness, path: registrationPath }: any = writeRegistration(siteRoot, record, {
...
test('mocked openrouter invocation returns normalized inert adapter output without raw evidence', async () => {
  ...
  const result: any = await executeProviderAdapter({
    siteRoot,
...
test('openrouter adapter represents missing capability model router rate limit and malformed response', async () => {
  const siteRoot = tempSite();
  const missingCapability: any = await executeProviderAdapter({
    siteRoot,
  ...
  const missingModel: any = await executeProviderAdapter({
    siteRoot,
  ...
  const missingRouter: any = await executeProviderAdapter({
    siteRoot,
  ...
  const rateLimit: any = await executeProviderAdapter({
    siteRoot,
  ...
  const malformed: any = await executeProviderAdapter({
    siteRoot,
...
test('openrouter provider refusal failure and timeout are bounded and redacted', async () => {
  const siteRoot = tempSite();
  const providerRefusal: any = normalizeOpenRouterChatCompletionResponse({
    error: { message: 'provider refused raw details that should not persist' },
  });
  const failure: any = await executeProviderAdapter({
    siteRoot,
  ...
  const timeout: any = await executeProviderAdapter({
    siteRoot,
    ...
    providerRegistry: makeProviderRegistry({
      [OPENROUTER_PROVIDER_KIND]: makeOpenRouterProviderAdapter({
        transport: () => new Promise((resolve: any) => setTimeout(() => resolve({ choices: [] }), 50)),
      }),
    }),
  });
