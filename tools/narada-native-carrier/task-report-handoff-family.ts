...
function taskReportPayloadPath(siteRoot: any, carrierSessionId: any) {
...
function boundedStringArray(values: any) {
  return Array.isArray(values)
    ? values.filter((value: any) => typeof value === 'string' && value.length > 0).slice(0, 50)
    : [];
}

function boundedResiduals(residuals: any) {
  return Array.isArray(residuals)
    ? residuals.slice(0, 50).map((residual: any) => ({
