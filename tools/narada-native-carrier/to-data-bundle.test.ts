...
import { TO_DATA_READ_FAMILIES } from './to-data-packet.ts';
import { buildIntegratedToDataBundle, TO_DATA_BUNDLE_SCHEMA } from './to-data-bundle.ts';
import { NO_MUTATION_FLAGS } from './to-data-readers.ts';
...
test('integrated to-data bundle composes all read families as bounded non-mutating packets', async () => {
  ...
  assert.equal(bundle.packets.find((packet: any) => packet.read_family === 'work_next_peek').read_status, 'refused');
  assert.equal(bundle.packets.find((packet: any) => packet.read_family === 'bounded_file_excerpt').bounded_summary.excerpt_present, true);
