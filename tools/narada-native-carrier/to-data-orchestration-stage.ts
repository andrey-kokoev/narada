...
} from './orchestration-session-contract.ts';
import {
...
async function runToDataOrchestrationStage({
  ...
  const packets = [];
  const readerPlan = [
    ['task_packet', readers.task_packet ?? ((args: any) => readTaskToDataPacket({ ...args, taskNumber }))],
...
function refusalResult({ carrierSessionId, agentId, now, packets, missingFamily, reason }: any) {
  return {
...
function isUsableRequiredPacket(packet: any, family: any) {
