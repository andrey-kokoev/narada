...
function buildFixtureRuntimeHandle({ handleId, startedAt = null, heartbeatDueAt = null, evidenceRefs = [] }: any= {}) {
...
function buildMissingRuntimeHandle({ handleId = 'runtime:missing', checkedAt = null, evidenceRefs = [] }: any= {}) {
  return buildRuntimeHandle({
    kind: 'missing',
    handleId,
...
function validateRuntimeHandle(handle: any) {
  const errors = [];
...
function defaultHandleId({ kind, processPid, sessionId }: any) {
