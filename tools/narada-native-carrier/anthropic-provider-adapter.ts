...
function anthropicRegistration(record: any= {}) {
  return {
...
function resolveAnthropicConfig(capability: any= {}) {
  return {
...
function anthropicRefusal(reason: any, diagnostic: any) {
  return {
    status: 'refused',
    reason,
    diagnostic,
...
function anthropicMessagesPayload({ prompt, model }: any) {
  return {
    model,
    max_tokens: 1024,
    messages: [
...
function extractAnthropicText(response: any) {
  const firstText = response.content?.find?.((block: any) => block?.type === 'text' && typeof block.text === 'string');
  return firstText?.text ?? null;
}

function normalizeAnthropicMessagesResponse(response: any) {
...
function makeAnthropicProviderAdapter({ transport }: any= {}) {
  return async function anthropicProviderAdapter({ capability, credential_ref: credentialRef, request }: any) {
