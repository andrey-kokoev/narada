...
import { executeProviderAdapter, makeProviderRegistry } from './provider-adapter.ts';
...
import { runToDataOrchestrationStage } from './to-data-orchestration-stage.ts';
import { runToIntelligenceOrchestrationStage } from './to-intelligence-orchestration-stage.ts';
...
function writeStageEvidence(siteRoot: any, carrierSessionId: any, name: any, record: any) {
...
function packet(carrierSessionId: any, family: any, boundedSummary: any) {
...
test('end-to-end mocked wrapper proof composes bounded evidence without authority mutation', async () => {
  ...
  let providerCalls = 0;
  let providerEvidencePath: any = null;
  let intelligenceStageResult: any = null;
  const registration = {
    adapter_id: 'provider-openai',
    provider_kind: 'openai_compatible',
    capability_ref: 'cap_model_wrapper_ref',
  ...
  const toDataResult = await runToDataOrchestrationStage({
    ...
    invokeIntelligence: async ({ packets, now }: any) => {
