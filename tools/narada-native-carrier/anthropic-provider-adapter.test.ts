...
} from './anthropic-provider-adapter.ts';
...
test('anthropic provider refusal failure and timeout are bounded and redacted', async () => {
  const siteRoot = tempSite();
  const providerRefusal: any = normalizeAnthropicMessagesResponse({
  ...
  const timeout: any = await executeProviderAdapter({
    ...
    providerRegistry: makeProviderRegistry({
      [ANTHROPIC_PROVIDER_KIND]: makeAnthropicProviderAdapter({
        transport: () => new Promise((resolve: any) => setTimeout(() => resolve({ content: [] }), 50)),
      }),
    }),
  });
