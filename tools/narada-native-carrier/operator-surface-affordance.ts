  ...
  return {
    ...
    affordances: [
      affordanceRecord({
        ...
        command: `node tools\\narada-native-carrier\\supervisor-cli.ts start --site-root ${siteRoot} --carrier-session-id ${carrierSessionId} --agent-id ${agentId}`,
      }),
      affordanceRecord({
        name: 'doctor',
        available: doctorAvailable,
        command: `node tools\\narada-native-carrier\\supervisor-cli.ts doctor --site-root ${siteRoot} --carrier-session-id ${carrierSessionId}`,
      }),
      affordanceRecord({
        name: 'focus',
...
function affordanceRecord({ name, available, command }: any) {
  return {
    name,
...
function canonicalTargetFor(name: any) {
