...
import { buildRepairGuidance } from './repair-guidance.ts';
...
test('repair guidance covers required blocked states with bounded diagnostics', () => {
  for (const state of REQUIRED_STATES) {
    ...
    assert.ok(guidance.next_diagnostic_commands.some((command: any) => command.includes('supervisor-cli.ts doctor')));
