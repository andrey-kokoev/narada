...
import { readRegistration } from './adapter-registration.ts';
import { runFixtureWorkLoop, runProviderWorkLoop } from './work-loop.ts';

function taskPacketSummary(packet: any) {
  return {
...
function assertTaskPacketAdmitted(packet: any, { agentId, capabilityGrants = {} }: any) {
  if (!packet) {
...
function handoffDraftPath(siteRoot: any, carrierSessionId: any) {
  return join(siteRoot, '.narada', 'crew', 'narada-native-carrier-sessions', carrierSessionId, 'task-report-draft.json');
}

function runNaradaTaskRead(taskNumber: any, { siteRoot }: any) {
...
async function readTaskPacketViaNaradaCli(taskNumber: any, { siteRoot, runCommand = runNaradaTaskRead }: any= {}) {
...
function createWorkResultReportDraft({ packet, loopResult, carrierSessionId, agentId, now = new Date().toISOString() }: any) {
  return {
