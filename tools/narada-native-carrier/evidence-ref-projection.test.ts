...
import { projectEvidenceRefs } from './evidence-ref-projection.ts';

function tempSite() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'narada-native-evidence-ref-'));
}

function writeEvidence(siteRoot: any, carrierSessionId: any, name: any, record: any) {
...
test('evidence ref projection lists family status path and recency only', () => {
  ...
  assert.ok(projection.refs.some((ref: any) => ref.family === 'supervisor' && ref.status === 'running' && ref.recency === 'fresh'));
  assert.ok(projection.refs.some((ref: any) => ref.family === 'handoff' && ref.status === 'inert_task_report_draft' && ref.recency === 'recent'));
});
