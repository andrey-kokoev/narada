...
- Treat `run startup`, `run startup sequence`, and equivalent startup nudges as requests to use the launch packet's `startup_command_name` / `startup_command` affordance. The current startup command name is `agent_context_startup_sequence` with empty arguments. This is an MCP startup affordance that performs launcher/site identity hydration plus advisory checkpoint continuity; it is not a request to discover repository files, inspect operator surfaces, or run shell commands. If the named MCP tool is unavailable, report the missing startup MCP capability instead of falling back to shell discovery. When a Narada MCP tool returns `reader_tool=mcp_output_show`, call `mcp_output_show` with the returned `output_ref` before deciding next work.
...
| [sqlite-runtime-posture.md](docs/concepts/sqlite-runtime-posture.md) | SQLite Runtime Posture — `node:sqlite` as the canonical runtime binding | Need to evaluate SQLite driver choices, built-in binding behavior, or Node 22+ runtime moves |
...
| [canonical-inbox.md](docs/concepts/canonical-inbox.md) | Canonical Inbox — one typed-envelope intake substrate with optional scale-relative crossing coordinates | Need to handle external intake, internal role handoff, review requests, blockers, CAPA candidates, or Site pub/sub signals without inventing another inbox |
| [intent-interpretation-admission-zones.md](docs/concepts/intent-interpretation-admission-zones.md) | Intent Interpretation and Admission Zones — vague pressure becomes inert candidates before authority-bearing admission | Need to handle "continue", "go on", "process tasks", role activation, inbox intent, MCP calls, or clarification/refusal without magic phrasing |
| [situated-work-discovery-advancement-intent.md](docs/concepts/situated-work-discovery-advancement-intent.md) | Situated Work Discovery and Advancement Intent — governed handling for "where are we", "what next", "continue", "builder is done", and similar nudges | Need to turn ordinary Operator nudges into bounded posture checks and admitted next actions |
| [coverage-audit-zone.md](docs/concepts/coverage-audit-zone.md) | Coverage Audit Zone — inert cataloging of underimplemented or overclaimed surfaces before governed repair admission | Need to detect docs/code/task/test/CLI labels that claim more than proven behavior without auto-repairing |
...
| [canonical-outbox.md](docs/concepts/canonical-outbox.md) | Canonical Outbox — governed outbound effect intents before transport execution | Need to compose, approve, confirm, archive, or export outbound crossings |
...
| [carrier-taxonomy.md](docs/concepts/carrier-taxonomy.md) | Carrier Taxonomy — layered vocabulary for Carrier, CarrierKind, CarrierHost, CarrierTransport, CarrierProtocol, CarrierRuntimeContract, CarrierSurface, and ControlChannel | Need to distinguish semantic carrier behavior from host, transport, surface, protocol, and deployment details |
...
| [delegated-role-taxonomy.md](docs/concepts/delegated-role-taxonomy.md) | Delegated Role Taxonomy — builder-worker, resident-worker, and effect-worker as bounded categories without generic worker collapse | Need to distinguish construction delegation, resident/use delegation, and runtime effect machinery |
...
| [governed-locus-federation.md](docs/concepts/governed-locus-federation.md) | Governed Locus Federation — federated awareness with local admission and singular mutation authority | Need to connect Site lifecycle, pub/sub, lineage, authority routing, and re-instantiation without overbuilding |
| [verifiable-envelope-trust.md](docs/concepts/verifiable-envelope-trust.md) | Verifiable Envelope Trust — authenticity, integrity, confidentiality posture, and provenance for envelopes | Need to reason about signed/encrypted inbox, pub/sub, lineage, or authority-transfer messages |
| [capability-governed-secret-management.md](docs/concepts/capability-governed-secret-management.md) | Capability-governed secret management — secrets as authority-bearing capabilities, not config/knowledge | Need to reason about credential references, stores, agent access, rotation, or Site cloning with secrets |
| [canonical-capability-consent-registry.md](docs/concepts/canonical-capability-consent-registry.md) | Canonical Capability Consent Registry — durable grants without raw secrets | Need to distinguish configured intent, credential reference, and executable authority |
| [canonical-admission-rejection-ledger.md](docs/concepts/canonical-admission-rejection-ledger.md) | Canonical Admission Rejection Ledger — durable admission/rejection decisions | Need to keep rejected, deferred, or superseded candidates from disappearing by silence |
| [observer-role.md](docs/concepts/observer-role.md) | Observer Role — read-only observer of Narada law, telos, authority boundaries, and inhabited-evolution discipline | Need to bootstrap or constrain a non-builder, non-reviewer observer thread |
| [conversation-observer.md](docs/concepts/conversation-observer.md) | Conversation Observer — labeled observer findings and interjections for carrier sessions without hidden injection or authority collapse | Need to let an observer watch an operator/agent conversation and surface bounded notes |
...
| [law-change-propagation.md](docs/concepts/law-change-propagation.md) | Law Change Propagation — durable law changes and agent receipts | Need to propagate AGENTS.md, SEMANTICS.md, role docs, task contracts, or Site law changes to active agents |
...
| [agent-reconstruction-specification.md](docs/product/agent-reconstruction-specification.md) | Agent-readable reconstruction route for environments that cannot run Narada packages | Need to rebuild a compatible local kernel from invariants without importing code |
...
| [site-governance-coordinates.md](docs/product/site-governance-coordinates.md) | Explicit Site law source, authority locus, embodiments, evidence locus, capabilities, readiness, and federation posture | Need to declare Site governance without inferring authority from path, shell, clone, or convenience |
| [site-stabilization-reconciliation.md](docs/product/site-stabilization-reconciliation.md) | Site stabilization and posture reconciliation — read-only comparison of durable Site memory, runtime truth, and operator/session evidence | Need to detect stale readiness memory and propose governed posture updates without silent mutation |
| [site-state-projections.md](docs/product/site-state-projections.md) | Site state projections and transition protocols — current-state read models with explicit authority inputs, volatile observations, freshness, evidence, and escalation | Need to expose runtime posture without letting scripts or private logs become authority |
| [site-posture-work-next.md](docs/product/site-posture-work-next.md) | Site posture and work-next — bounded Operator surface for health, readiness, work, drafts, residuals, publication, and one next Site action | Need to avoid manual choreography across doctor, ops, status, drafts, task, and inbox surfaces |
| [windows-operator-surface-adapter-path.md](docs/product/windows-operator-surface-adapter-path.md) | Windows Operator Surface adapter path — Windows Terminal, Komorebi, YASB/AHK posture without authority collapse | Need to plan stable Windows role windows, focus rules, launch buttons, or adapter materialization evidence |
...
| [inhabited-onboarding.md](docs/product/inhabited-onboarding.md) | Inhabited Onboarding — Site maturation phase between bootstrap and operational steady state | Need to prove a new Site's first real workflows, boundaries, readiness, and residuals |
...
| [visibility-domain-reconciliation.md](docs/concepts/visibility-domain-reconciliation.md) | Visibility Domain Reconciliation — host membership truth as an adapter reconciliation boundary | Need to prevent operator-surface adapter state from overriding Windows desktop, display, browser profile, process, session, or workspace membership |
...
| [operator-surface-action-posture.md](docs/concepts/operator-surface-action-posture.md) | Operator Surface Action Posture — classify primary work, utilities, diagnostics, repair, intrusive mutation, hidden primitives, and contextual projections | Need to decide whether a valid tool belongs on the primary operator work surface |
| [site-local-daemon-sources.md](docs/product/site-local-daemon-sources.md) | Site-local daemon source posture — heartbeat, inbox-drop, and filesystem observation without mailbox projection collapse | Need to equip Project or Site-local daemons with coherent local observation/admission paths |
...
| [site-pubsub-signal-exchange.md](docs/product/site-pubsub-signal-exchange.md) | Site pub/sub signal exchange — typed signals into governed local admission | Need to model cross-Site publication/subscription without replication or authority transfer |
...
- Forward momentum after a prototype/facade/spike transition means preserving the whole capability shape: every typed branch gets implementation work, explicit deferral, or rejection evidence. Continue locally when low-risk and role-authorized; otherwise record the blocker and route the next role.
...
| `known-teardown-noise` | (Deprecated) was used for the legacy `better-sqlite3` cleanup crash; no longer applicable after the migration to `node:sqlite` | Treat post-test crashes as `infrastructure-failure` |

### SQLite Driver Posture (`node:sqlite`)

The project uses `node:sqlite`, the Node.js built-in SQLite binding. This replaces the previous `better-sqlite3` native dependency.

Consequences for tests and runtime:

- **No native teardown race**: `node:sqlite` is a built-in binding, so the `better-sqlite3`-specific V8/ArrayBuffer destructor race (exit code 133 / SIGTRAP after tests complete) no longer applies.
- **Crash classification**: Any post-test exit code 133 or V8 fatal error should now be treated as `infrastructure-failure`, not as `known-teardown-noise`.
- **Lifecycle helper**: `packages/layers/control-plane/test/db-lifecycle.ts` provides `createTestDb()` and `closeAllTestDatabases()` using the `node:sqlite`-backed wrapper. `test/setup.ts` calls `closeAllTestDatabases()` in `afterAll`. New and refactored tests should use `createTestDb()` rather than raw `node:sqlite` handles.
