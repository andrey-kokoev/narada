...
function publicResourceId(value: any): string | null {
...
export function createNarsNativeKernel({
  ...
  let activeTurnCompletion: Promise<void> | null = null;
  let resolveActiveTurnCompletion: (() => void) | null = null;
  let started: AnyRecord | null = null;
  let lastError: any = null;
  let sequence = 0;
  let pendingAbort: AbortController | null = null;
  let currentConfig: AnyRecord = {
  ...
  let closed = false;
  const compactError = (error: any, fallback: string): AnyRecord => ({
  ...
  const emit = async (sink: AnyFunction, event: AnyRecord): Promise<void> => sink({
    ...event,
    sequence: ++sequence,
    timestamp: now(),
  });
  ...
  async function invokeAdmitted(input: AnyRecord = {}): Promise<any> {
  ...
  const api = {
    ...
    async runTurn(turn: AnyRecord, eventSink: AnyFunction, capabilityGateway: AnyRecord) {
      if (closed) throw new NarsKernelContractError('native_kernel_closed', 'Native kernel is closed.');
      ...
      activeTurnCompletion = new Promise<void>((resolve: () => void) => { resolveActiveTurnCompletion = resolve; });
      ...
      try {
        ...
        const undeclaredRequestedTool = normalizedTurn.tools.find((tool: any) => !admittedNames.has(tool.function.name));
      ...
      try {
        ...
        const outcome = await providerAdapter.invoke({
          ...
          invocationEventSink: async (event: any) => emit(sink, { kind: 'kernel_provider_telemetry', source_event: event }),
    ...
    async cancel(request: AnyRecord = {}) {
      if (pendingAbort) {
    ...
    async reconfigure(request: AnyRecord = {}) {
