...
import { createNarsNativeKernel } from './native-kernel.js';

type AnyRecord = Record<string, any>;

const noCapabilityGateway = Object.freeze({
  toolCatalog: async () => [],
  invoke: async ({ toolName }: AnyRecord) => ({ status: 'denied', admission_action: 'deny', execution_outcome: 'not_attempted', tool_name: toolName }),
  close: async () => {},
});
...
test('native kernel satisfies the representation-neutral contract', async () => {
  ...
  const result = await kernel.runTurn({ turn_id: 'turn-1', input_id: 'input-1', messages: [{ role: 'user', content: 'hi' }] }, (event: AnyRecord) => events.push(event), noCapabilityGateway);
  ...
  assert.ok(events.every((event: AnyRecord) => event.kind !== 'assistant_message'));
...
test('native kernel uses its configured execution policy when a turn omits a policy', async () => {
  let providerInput: AnyRecord | null = null;
  ...
  const observedProviderInput = providerInput as unknown as AnyRecord;
  assert.deepEqual(observedProviderInput.executionPolicy, executionPolicy);
  assert.deepEqual(observedProviderInput.execution_policy, executionPolicy);
  assert.equal(observedProviderInput.tool_loop.execution_policy.tool_loop.max_rounds, 12);
