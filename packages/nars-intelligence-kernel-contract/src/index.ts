 ...
 */
export function normalizeNarsExecutionPolicy(value: any = null, {
...
export function assertNarsExecutionPolicy(value: any, options: AnyRecord) {
  return normalizeNarsExecutionPolicy(value, options);
}

export function isIntelligenceKernelKind(value: any) {
  return typeof value === 'string' && INTELLIGENCE_KERNEL_KINDS.includes(value.trim());
}

export function normalizeIntelligenceKernelKind(value: any, { defaultKind = 'narada-native' }: AnyRecord = {}) {
...
export function assertIntelligenceKernelKind(value: any, options: AnyRecord) {
  return normalizeIntelligenceKernelKind(value, options);
}

export function isOperatorSurfaceKind(value: any) {
  return typeof value === 'string' && OPERATOR_SURFACE_KINDS.includes(value.trim());
}

function assertPlainRecord(value: any, code: string, message: string): AnyRecord {
...
function rejectUnknownKeys(value: AnyRecord, allowed: Set<string>, code: string) {
...
function optionalPlainRecord(value: any, code: string): AnyRecord {
  if (value == null) return {};
  return { ...assertPlainRecord(value, code, 'Expected a plain record.') };
}

function assertOptionalString(value: any, field: string, code: string) {
  if (value == null) return;
...
function assertOptionalPositiveInteger(value: any, field: string, code: string) {
  if (value == null) return;
...
function assertCorrelationFields(value: any, codePrefix: string = 'kernel_correlation') {
  for (const field of [
    'turn_id',
    'input_id',
    'input_event_id',
...
function normalizeMessage(value: any, index: number): AnyRecord {
  ...
  return {
    ...
    ...(message.tool_calls == null ? {} : { tool_calls: message.tool_calls.map((call: any) => optionalPlainRecord(call, 'kernel_tool_call_invalid')) }),
...
function normalizeToolDescriptor(value: any, index: number): AnyRecord {
...
export function assertNarsKernelStartContext(context: any): AnyRecord {
...
export function assertNarsAdmittedTurn(turn: any): AnyRecord {
...
export function assertNarsAdmittedInput(input: any): AnyRecord {
...
export function assertNarsKernelCapabilityGateway(gateway: any): AnyRecord {
...
export function assertNarsKernelEventSink(eventSink: any): AnyFunction {
...
export function isKernelTerminalState(value: any) {
