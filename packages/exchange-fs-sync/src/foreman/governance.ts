...
export function governEvaluation(
  ...
  if (evaluation.outcome === "escalation") {
    return {
      outcome: "escalate",
  ...
  if (evaluation.outcome === "clarification_needed") {
    return {
  ...
  for (const action of actionsToGovern) {
    ...
    if (!result.allowed) {
      governanceErrors.push(`${action.action_type}: ${result.reason}`);
      continue;
    }
    if (!result.confidence_sufficient) {
      governanceErrors.push(`${action.action_type}: confidence insufficient for autonomous execution`);
      continue;
    }
    if (!result.payload_valid) {
      governanceErrors.push(`${action.action_type}: invalid payload — ${result.payload_errors.join(", ")}`);
      continue;
    }
  ...
  return {
    ...
    reason: firstAcceptableRequiresApproval
      ? "Action valid but requires human approval"
      : "Action passed governance",
    approval_required: firstAcceptableRequiresApproval,
    governance_errors: [],
  };
}
