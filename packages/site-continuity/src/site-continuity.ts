...
function createSiteContinuityBindingRegistry({ bindings = [], registry_ref = null, generated_at = null } : AnyRecord = {}) {
  return {
    schema: SITE_CONTINUITY_BINDING_REGISTRY_SCHEMA,
    classifier_version: SITE_CONTINUITY_CLASSIFIER_VERSION,
    registry_ref,
    generated_at,
    bindings: bindings.map((binding: AnyRecord) => ({ ...binding, embodiments: [...(binding.embodiments ?? [])] })),
  };
}

function validateSiteContinuityBindingRegistry(registry: any): ValidationResult {
  const errors: string[] = [];
...
function listSiteContinuityBindingSites(registry: any, {
  ...
  return [...new Set<string>(registry.bindings
    .filter((binding: AnyRecord) => required_embodiment_kinds.every((kind: any) => Boolean(findEmbodiment(binding, kind))))
    .map((binding: AnyRecord) => binding.site_id))].sort((left: string, right: string) => left.localeCompare(right));
}
...
function createSiteContinuityExchangePacket({
  ...
  const packet = {
    ...
    binding,
    decisions: [...decisions],
    projections: projections.map((projection: AnyRecord) => ({ ...projection })),
    evidence_refs: evidence_refs.map((evidenceRef: AnyRecord) => ({ ...evidenceRef })),
    executable_mutation_requests: executable_mutation_requests.map((request: AnyRecord) => ({ ...request })),
    generated_at,
  };
  return { packet_id: createSiteContinuityPacketId(packet), ...packet };
}

function createSiteContinuityPacketId(packet: any): string {
  const parts = [
...
function stableIdPart(value: any): string {
  return String(value).trim().replace(/[^A-Za-z0-9_.-]+/g, '-').replace(/^-+|-+$/g, '') || 'unknown';
}

function validateSiteContinuityExchangePacket(packet: any): ValidationResult {
  const errors: string[] = [];
...
function classifySiteContinuityExchangePacket(packet: any): AnyRecord {
...
function validateSiteContinuityBinding(binding: any): ValidationResult {
  const errors: string[] = [];
...
function classifySiteContinuityExchange(binding: any, request: AnyRecord = {}): AnyRecord {
  const validation = validateSiteContinuityBinding(binding);
...
function findEmbodiment(binding: any, embodimentKind: any): AnyRecord | null {
  return binding.embodiments.find((embodiment: AnyRecord) => embodiment.embodiment_kind === embodimentKind) ?? null;
}
...
export type SiteContinuityBinding = ReturnType<typeof createSiteContinuityBinding>;
export type SiteContinuityDecision = ReturnType<typeof classifySiteContinuityExchange>;
export type SiteContinuityExchangePacket = ReturnType<typeof createSiteContinuityExchangePacket>;
