...
import { admitAiProcessInvocation, runAiProcessInvocationSync, spawnAiProcessInvocation } from './ai-process-invocation.js';

type AnyRecord = Record<string, any>;

function invocation(root: string): AnyRecord {
  return {
    adapterKind: 'codex',
...
test('admits first invocation and refuses duplicate live lease before spawn', () => {
  ...
  const first = admitAiProcessInvocation(invocation(root), { ownerPid: 123, isPidAlive: (pid: any) => pid === 123 });
  assert.equal(first.admitted, true);
  const second = admitAiProcessInvocation(invocation(root), { ownerPid: 456, isPidAlive: (pid: any) => pid === 123 });
...
test('codex runtime-session live cap refuses a different command before spawn', () => {
  ...
  const second = admitAiProcessInvocation(secondInvocation, { ownerPid: 456, isPidAlive: (pid: any) => pid === 123 });
  assert.equal(second.admitted, false);
...
test('runtime-session scope allows a live Codex invocation in a different session', () => {
  const root = mkdtempSync(join(tmpdir(), 'narada-ai-invocation-'));
  const first = admitAiProcessInvocation(invocation(root), { ownerPid: 123, isPidAlive: (pid: any) => pid === 123 || pid === 456 });
  assert.equal(first.admitted, true);
  const second = admitAiProcessInvocation({
    ...
    ownerPid: 456,
    isPidAlive: (pid: any) => pid === 123 || pid === 456,
  });
...
test('legacy live leases are reported as unverified and do not block a scoped admission', () => {
  ...
  const admitted = admitAiProcessInvocation(invocation(root), {
    ownerPid: 123,
    isPidAlive: (pid: any) => pid === 123 || pid === 999,
  });
  assert.equal(admitted.admitted, true);
  assert.equal(admitted.admission_diagnostics.unverified_live_invocation_count, 1);
});
...
test('runAiProcessInvocationSync refuses duplicate without calling spawnSync', () => {
  const root = mkdtempSync(join(tmpdir(), 'narada-ai-invocation-'));
  admitAiProcessInvocation(invocation(root), { ownerPid: 123, isPidAlive: (pid: any) => pid === 123 });
  let called = false;
  const result = runAiProcessInvocationSync(invocation(root), {
    ownerPid: 456,
    isPidAlive: (pid: any) => pid === 123,
...
test('spawnAiProcessInvocation writes exit evidence when the child closes', () => {
  const root = mkdtempSync(join(tmpdir(), 'narada-ai-invocation-'));
  let closeHandler: ((code: any, signal: any) => void) | null = null;
  const child = { once(event: string, handler: (code: any, signal: any) => void) { if (event === 'close') closeHandler = handler; } };
  const owner = spawnAiProcessInvocation(invocation(root), {
    ownerPid: 456,
    isPidAlive: (pid: any) => pid === 456,
    spawnProcess: () => ({ child, terminateTree() {} }),
  });
  assert.equal(owner.aiProcessInvocation.event, 'spawn');
  const registeredCloseHandler: any = closeHandler;
  if (!registeredCloseHandler) throw new Error('close_handler_not_registered');
  registeredCloseHandler(0, null);
