...
import { codexCommand } from './codex-subscription-command.js';
import { runAiProcessInvocationSync } from './ai-process-invocation.js';
...
function optionalEnvironmentValue(name: string, processEnv: AnyRecord = process.env) {
  const value = processEnv?.[name];
  return value === undefined || value === '' ? null : String(value);
}

function optionalModel(value: any) {
  if (typeof value === 'string' && value.trim()) return value.trim();
  if (value && typeof value === 'object' && typeof value.id === 'string' && value.id.trim()) return value.id.trim();
  return null;
}

function selectedModel(model: any, processEnv: AnyRecord = process.env) {
...
function selectedReasoningEffort(thinking: any, processEnv: AnyRecord = process.env) {
...
function probeArguments({ command, model, reasoningEffort }: AnyRecord) {
  const args = [
    ...(command.prefixArgs ?? []),
    'exec',
    '--json',
...
function structuredProbeCompletion(stdout: any): AnyRecord {
...
function cacheTtlMs(processEnv: AnyRecord = process.env) {
...
function cliVersion(processEnv: AnyRecord = process.env) {
...
function commandIdentity(command: AnyRecord, processEnv: AnyRecord = process.env) {
  return [
    command.command,
...
export function codexSubscriptionReadinessCachePath({ userSiteRoot }: AnyRecord = {}) {
...
export function deriveUserSiteRootFromRegistryPath(registryDbPath: any) {
...
function sanitizedEnvironment(processEnv: AnyRecord, authHome: any): AnyRecord {
...
function baseEvidence({ session, authorityRef, status, evidenceRef, observedAt, probe }: AnyRecord): AnyRecord {
  return {
...
function cacheRead({ userSiteRoot, cacheKey, now, processEnv, authHome, session, authorityRef }: AnyRecord): AnyRecord | null {
...
function cacheWrite({ userSiteRoot, cacheKey, preflight, now, processEnv, authHome, command, model, reasoningEffort }: AnyRecord): void {
  const ttlMs = cacheTtlMs(processEnv);
  const cachePath = codexSubscriptionReadinessCachePath({ userSiteRoot });
  if (ttlMs <= 0 || !cachePath) return;
  try {
    mkdirSync(dirname(cachePath), { recursive: true });
    let cache: AnyRecord = { schema: 'narada.codex_subscription.preflight_cache.v1', entries: {} };
