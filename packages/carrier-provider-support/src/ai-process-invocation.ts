...
} from './ai-process-invocation-state.js';
...
export class AiProcessInvocationRefusalError extends Error {
  code: string;
  admission: AnyRecord;
  constructor(admission: AnyRecord) {
...
function processIdentityPath(root: any, pid: any) {
  return join(root, 'processes', `${Number(pid)}.json`);
}

function registerProcessIdentity(root: any, pid: any, identity: any) {
...
function processStartIdentityFor(root: any, pid: any, custom: any) {
...
function isLiveLease(lease: AnyRecord, siteRoot: any, options: AnyRecord) {
...
function transitionAndWrite(admission: AnyRecord, nextState: any, evidence: AnyRecord, artifactDir: any) {
...
function leaseRoot(admission: AnyRecord) {
...
function liveCapRefusal({ record, leaseDir, artifactDir, leasePath, options, allowDuplicate }: AnyRecord) {
  ...
  const live = scan.verified.filter((candidate: any) => sameInvocationScope(candidate, record));
...
function scanLiveLeases(leaseDir: any, options: AnyRecord, siteRoot: any) {
  let names;
...
function sameInvocationScope(left: AnyRecord, right: AnyRecord) {
...
function normalizePolicy(invocation: AnyRecord, options: AnyRecord) {
  return {
...
function normalizeInvocationScope(invocation: AnyRecord, siteRoot: any) {
...
function isCanonicalInvocationScope(scope: any) {
  return Boolean(
    scope
...
export function aiProcessInvocationRoot({ siteRoot, cwd }: AnyRecord = {}) {
  return join(resolve(siteRoot ?? cwd ?? process.cwd()), '.ai', 'runtime', 'ai-process-invocation');
}

export function buildAiProcessInvocationRecord(invocation: AnyRecord, options: AnyRecord = {}): AnyRecord {
...
export function admitAiProcessInvocation(invocation: AnyRecord, options: AnyRecord = {}): AnyRecord {
...
export function releaseAiProcessInvocationLease(admission: AnyRecord, result: AnyRecord = {}): AnyRecord {
...
function defaultAiProcessInvocationRunner(command: any, args: any, options: AnyRecord) {
  return runHiddenPostureCommandSync(command, args, { ...options, posture: 'provider_subprocess' });
}

export function runAiProcessInvocationSync(invocation: AnyRecord, { runProcessSync, spawnSync, spawnOptions = {}, ...admissionOptions }: AnyRecord = {}): AnyRecord {
...
export function spawnAiProcessInvocation(invocation: AnyRecord, { spawnProcess, spawnOptions = {}, ...admissionOptions }: AnyRecord = {}): AnyRecord {
  const admission = admitAiProcessInvocation(invocation, admissionOptions);
  ...
  child?.once?.('close', (code: any, signal: any) => releaseAiProcessInvocationLease(admission, { exitCode: code, signal }));
...
function summarizeEnv(env: AnyRecord) {
...
function isPidAlive(pid: any, custom: any) {
  if (!Number.isInteger(Number(pid)) || Number(pid) <= 0) return false;
  if (custom) return Boolean(custom(Number(pid)));
  try { process.kill(Number(pid), 0); return true; } catch { return false; }
}

function writeEvidenceArtifact(artifactDir: any, evidence: AnyRecord): string {
  mkdirSync(artifactDir, { recursive: true });
