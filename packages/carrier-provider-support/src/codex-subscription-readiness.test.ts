...
import { probeCodexSubscriptionService } from './codex-subscription-readiness.js';
...
test('Codex preflight is independent of ambient config and pins the selected route', () => {
  ...
  try {
    let invocation: AnyRecord | null = null;
    const result = probeCodexSubscriptionService({
      ...
      model: 'gpt-5.5',
      thinking: 'medium',
      runProbeSync(request: AnyRecord, options: AnyRecord) {
        invocation = { request, options };
        return { status: 0, signal: null, stdout: '', stderr: '' };
      },
    });
    assert.equal(result.status, 'ready');
    const observedInvocation = invocation as unknown as AnyRecord;
    assert.deepEqual(observedInvocation.request.argv, [
      'exec',
      '--json',
      '--ephemeral',
      '--ignore-user-config',
    ...
    assert.equal(observedInvocation.options.spawnOptions.env.CODEX_HOME, authHome);
    assert.equal(observedInvocation.request.env.OPENAI_API_KEY, undefined);
