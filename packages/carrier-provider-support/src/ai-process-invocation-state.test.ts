...
test('AI process invocation FSM records planned through released lifecycle', () => {
  let record: AnyRecord = { id: 'invocation-1', lifecycle_state: 'planned' };
  ...
  assert.deepEqual(record.lifecycle_history.map((entry: AnyRecord) => entry.state), ['admitted', 'spawned', 'exited', 'released']);
});
