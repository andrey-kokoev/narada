...
- `intelligence_provider_resolution.source_field` remains `cli_argument`, `site_env`, `environment`, `launcher_env`, or `default_for_nars_operator_surface`.
...
- `launcher_contracts` in the launch result bundles the first-class operator-surface, runtime-host, MCP-fabric, selection-session, provider-readiness, terminal-projection, artifact, and failure-rendering projections. Carrier and runtime dimensions are explicit fields only; derived aliases are not emitted.
- `runtime_health_posture` is the compact first-class projection for NARS health and event-stream discovery. It carries the structured health/event endpoint dimensions used by `agent-cli` and `agent-web-ui` so launch banners can summarize health without dumping the full session stream.
- Bootstrap doctor output now exposes `build_freshness_check` and `build_repair_action` alongside the existing `cli_execution_posture` and `repair_plan` fields so stale dist repair is described as policy, not ad hoc text.
