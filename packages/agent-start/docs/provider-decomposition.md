...
- `launcher_contracts` in the launch result bundles the first-class operator-surface, runtime-host, MCP-fabric, selection-session, provider-readiness, terminal-projection, artifact, and failure-rendering projections. Compatibility labels remain present only as derived aliases.
