...
import { buildLauncherContractsFromAgentStartResult } from '../src/launch-result-contracts.js';
import { writeJsonFileAtomically, writeLaunchResultFile } from '../src/carrier-launch-artifacts.js';
import {
  AgentStartResultV0Schema,
  assertAgentStartResultV0,
  evaluateAgentStartHandoff,
  parseAgentStartResultV0,
} from '../src/launch-result-v0-contract.js';
...
test('does not classify canonical launch artifacts as failures', () => {
  for (const status of ['materialized', 'dry_run'] as const) {
    const contracts: any = buildLauncherContractsFromAgentStartResult({
...
test('treats top-level session aliases as canonical declarations', () => {
  ...
  const conflicting: any = { ...coherent, session_id: 'runtime_other' };
...
test('rejects non-canonical launch failures before broader projection', () => {
  ...
  } as any), /agent_start_result_contract_invalid/);
});
...
test('accepts canonical materialized results and preserves additive fields', () => {
  ...
  const parsed: any = parseAgentStartResultV0(result);
...
test('publishes only validated launch results through an atomic final path', () => {
  const siteRoot: any = mkdtempSync('narada-launch-result-');
  const resultDir: any = join(siteRoot, '.ai', 'runtime', 'agent-start-results');
  try {
    const result: any = {
      schema: 'narada.agent_start.result.v0',
      status: 'materialized',
    ...
    const resultPath: any = writeLaunchResultFile(result, { siteRoot });
...
test('publishes command result JSON through an atomic replacement', () => {
  const siteRoot: any = mkdtempSync('narada-json-output-');
  const resultPath: any = join(siteRoot, 'result.json');
  try {
...
test('keeps the checked-in JSON Schema generated from the Zod contract', async () => {
  const generated: any = zodToJsonSchema(AgentStartResultV0Schema, {
    target: 'jsonSchema7',
    $refStrategy: 'none',
  });
  const expected: any = {
  ...
  const checkedIn: any = JSON.parse(await readFile(resolve(__dirname, '..', 'contracts', 'agent-start.result.v0.schema.json'), 'utf8'));
...
test('rejects unknown statuses and legacy result schemas before correlation', () => {
  for (const result of [
    ...
    const handoff: any = evaluateAgentStartHandoff(result);
