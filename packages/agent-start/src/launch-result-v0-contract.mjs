...
import { z } from 'zod';
export const AGENT_START_RESULT_SCHEMA = 'narada.agent_start.result.v0';
export const AGENT_START_RESULT_STATUSES = ['materialized', 'dry_run'];
export const AGENT_START_SESSION_REF_KINDS = ['runtime', 'nars', 'carrier'];
...
export class AgentStartResultContractError extends Error {
    code = 'agent_start_result_contract_invalid';
    issues;
    constructor(issues) {
...
export function parseAgentStartResultV0(value) {
    return AgentStartResultV0Schema.safeParse(value);
}
export function assertAgentStartResultV0(value) {
    const parsed = parseAgentStartResultV0(value);
    if (!parsed.success)
        throw new AgentStartResultContractError(parsed.error.issues);
    return parsed.data;
}
...
function coherentSessionId(values) {
    const ids = [...new Set(values.map(nonEmptyValue).filter((value) => Boolean(value)))];
    return { value: ids[0] ?? null, conflict: ids.length > 1 };
}
export function resolveAgentStartSessionProjection(value) {
    const parsed = parseAgentStartResultV0(value);
    if (!parsed.success)
        return null;
    ...
    if (runtimeSession.conflict || narsSession.conflict || carrierSessionId.conflict)
        return null;
    const componentId = sessionRefValue?.kind === 'runtime'
        ...
        : sessionRefValue?.kind === 'nars'
            ...
            : sessionRefValue?.kind === 'carrier'
                ...
                : null;
    if (sessionRefValue && componentId !== sessionRefValue.id)
        return null;
    return {
...
export function evaluateAgentStartHandoff(value) {
    const parsed = parseAgentStartResultV0(value);
    if (!parsed.success) {
        return {
            eligible: false,
