...
if (options.help) {
  process.stdout.write(`Usage: node tools/typed-mcp/inbox-mcp-server.js --site-root <path> --narada-cli <path>\n`);
  process.exit(0);
}
...
function dispatchMethod(method: any, params: any, site: any, serverOptions: any): any {
  switch (method) {
    case 'initialize':
      return {
...
function callTool(params: any, site: any, serverOptions: any): any {
  ...
  switch (canonicalName) {
    case 'inbox_doctor':
      return jsonToolResult({
        ...
        allowed_tools: tools().map((tool: any) => tool.name),
        conceptual_role: {
    ...
    case 'mcp_command_submit':
      return jsonToolResult(commandSubmit({
        ...
        args,
        admitters: {
          'narada.command.inbox.submit.v1': (command: any) => admitInboxSubmitCommand(command, site, serverOptions),
          'narada.command.inbox.submit_typed_envelope.v1': (command: any) => admitInboxSubmitTypedEnvelopeCommand(command, site, serverOptions),
          'narada.command.inbox.acknowledge.v1': (command: any) => admitInboxAcknowledgeCommand(command, site, serverOptions),
          'narada.command.inbox.dismiss.v1': (command: any) => admitInboxDismissCommand(command, site, serverOptions),
          'narada.command.inbox.promote_capa.v1': (command: any) => admitInboxPromoteCapaCommand(command, site, serverOptions),
          'narada.command.inbox.capa_related.v1': (command: any) => admitInboxCapaRelatedCommand(command, site, serverOptions),
          'narada.command.inbox.capability_review_complete.v1': (command: any) => admitCapabilityReviewCompleteCommand(command, site, serverOptions),
          'narada.command.inbox.export_disposition_ledger.v1': (command: any) => admitInboxExportDispositionLedgerCommand(command, site, serverOptions),
        },
      }));
...
function stageSubmissionWorkflow(args: any, site: any, serverOptions: any): any {
...
function submitObservation(args: any, site: any, serverOptions: any): any {
  const targetSiteRoot = resolveTargetSiteRoot(args, site, serverOptions);
...
function submitTypedEnvelope(args: any, site: any, serverOptions: any): any {
  const targetSiteRoot = resolveTargetSiteRoot(args, site, serverOptions);
  const naradaCli = resolveNaradaCli(serverOptions, true);
  const dryRun = booleanField(args, 'dry_run') === true;
  const sourceKind = stringField(args, 'source_kind') ?? 'agent_report';
...
function runCanonicalInboxCommand({ site, targetSiteRoot, naradaCli, commandArgs, dryRun, operation, identityVerification = null }: AnyRecord): any {
  ...
  if (result.status === 0 && envelopeId && parsed?.canonical_result?.envelope) {
    try {
      ...
      stderr += `\n[admission_log_warning: ${logErr instanceof Error ? logErr.message : String(logErr)}]`;
    }
  }
...
function deriveSeverity(envelope: any): any {
...
function resolveAgentRole(siteRoot: any, agentId: any): any {
  try {
    ...
    const agent = doc?.agents?.find((a: any) => a.agent_id === agentId);
...
function kindsForRole(role: any): any {
...
function readValidEnvelopeFiles(envelopeDir: any): any {
...
function effectiveEnvelopeStatus(envelope: any, latestLogEvents: any): any {
...
function postalProjection(envelope: any): any {
...
function isPostalEnvelope(envelope: any): any {
  return Boolean(postalProjection(envelope));
}

function readDispositionLedgerEvents(siteRoot: any): any {
  ...
  return events
    .filter((event: any) => event?.envelope_id && event?.event_kind)
    .map((event: any) => ({
...
function readDurableDispositionEvents(siteRoot: any): any {
...
function getLatestDurableEventsByEnvelope(siteRoot: any): any {
  const map = new Map();
...
function exportDispositionLedgerSnapshot(siteRoot: any): any {
  return exportDispositionLedger(siteRoot, { output_path: DEFAULT_DISPOSITION_LEDGER_PATH });
}

function inboxList(args: any, site: any): any {
  ...
  let envelopes = files.map((e: any) => ({
    ...e,
    _effective_status: effectiveEnvelopeStatus(e, latestLogEvents),
  }));

  if (statusFilter !== 'all') {
    envelopes = envelopes.filter((e: any) => e._effective_status === statusFilter);
  }
  if (kindFilter) {
    envelopes = envelopes.filter((e: any) => e.kind === kindFilter);
  }
  if (severityFilter) {
    envelopes = envelopes.filter((e: any) => deriveSeverity(e) === severityFilter);
  }
  if (principalFilter) {
    const p = principalFilter.toLowerCase();
    envelopes = envelopes.filter((e: any) => (e.authority?.principal ?? '').toLowerCase().includes(p));
  }
  if (postalFilter) {
    const wantsPostal = ['true', '1', 'yes', 'postal'].includes(postalFilter.toLowerCase());
    envelopes = envelopes.filter((e: any) => isPostalEnvelope(e) === wantsPostal);
  }
  if (sourceSiteIdFilter) {
    envelopes = envelopes.filter((e: any) => postalProjection(e)?.source_site_id === sourceSiteIdFilter);
  }
  if (mailIdFilter) {
    envelopes = envelopes.filter((e: any) => postalProjection(e)?.mail_id === mailIdFilter);
  }
  if (targetSiteIdFilter) {
    envelopes = envelopes.filter((e: any) => postalProjection(e)?.target_site_id === targetSiteIdFilter);
  }
  if (since) {
    const sinceDate = new Date(since).getTime();
    envelopes = envelopes.filter((e: any) => new Date(e.received_at ?? 0).getTime() >= sinceDate);
  }
  if (until) {
    const untilDate = new Date(until).getTime();
    envelopes = envelopes.filter((e: any) => new Date(e.received_at ?? 0).getTime() <= untilDate);
  }

  envelopes.sort((a: any, b: any) => new Date(b.received_at ?? 0).getTime() - new Date(a.received_at ?? 0).getTime());
...
function inboxNext(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  ...
  const unprocessed = files
    ...
    .filter((e: any) => e._effective_status === 'received');
  if (unprocessed.length === 0) {
    return { has_work: false, envelope: null, priority_score: 0, schema: 'narada.inbox.next.v0', agent_role: agentRole ?? undefined };
  }

  const authorityScores: AnyRecord = { operator_directed: 100, system_detected: 70, agent_reported: 40 };
  const kindScores: AnyRecord = { incident: 100, observation: 60, proposal: 40, command_request: 50, task_candidate: 45 };
  ...
  const scored = unprocessed.map((e: any) => {
  ...
  const roleFiltered = scored.filter((s: any) => s.role_relevance);
  const candidates = roleKindSet.size > 0 && roleFiltered.length > 0 ? roleFiltered : scored;

  candidates.sort((a: any, b: any) => b.priority_score - a.priority_score);
...
function inboxShow(args: any, site: any): any {
  ...
  if (!envelope) {
    ...
    const files = readdirSync(envelopeDir).filter((f: any) => f.endsWith('.json'));
...
function resolveEnvelopePath(siteRoot: any, envelopeId: any): any {
  const envelopeDir = join(siteRoot, '.ai', 'inbox-envelopes');
...
function readEnvelope(siteRoot: any, envelopeId: any): any {
...
function updateEnvelopeStatus(siteRoot: any, envelopeId: any, status: any, resolution: any): any {
  const path = resolveEnvelopePath(siteRoot, envelopeId);
...
function inboxAcknowledge(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  const envelopeId = requiredString(args, 'envelope_id');
...
function inboxDismiss(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  const envelopeId = requiredString(args, 'envelope_id');
...
function findEnvelopeById(siteRoot: any, envelopeId: any): any {
  const envelopeDir = join(siteRoot, '.ai', 'inbox-envelopes');
  if (!existsSync(envelopeDir)) return null;
  return readValidEnvelopeFiles(envelopeDir).find((e: any) => e.envelope_id === envelopeId) ?? null;
}

function inboxCreateCapa(args: any, site: any, serverOptions: any): any {
...
function inboxAmendCapa(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  ...
  const activeCapa = queue.capas.find((capa: any) => capa.capa_id === capaId);
...
function inboxPromoteCapa(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  const envelopeId = requiredString(args, 'envelope_id');
...
function inboxExportDispositionLedger(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
...
function capabilityNext(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  ...
  scored.sort((a: any, b: any) => b.severity - a.severity || a.days_until_due - b.days_until_due);
...
function capabilityReviewComplete(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  const capPath = join(siteRoot, 'operator-surfaces', 'capability-announcements.json');
  ...
  const index = capabilities.findIndex((cap: any) => cap?.capability_id === capabilityId);
...
function capaQueue(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  ...
  if (existsSync(capPath)) {
    ...
    capaItems = capabilities
      .filter((cap: any) => (cap.review_status ?? 'pending') !== 'completed')
      .map((cap: any) => {
        const reviewDue = cap.review_due ? new Date(cap.review_due).getTime() : 0;
        const daysUntilDue = reviewDue ? Math.floor((reviewDue - now) / msPerDay) : Infinity;
        const status = cap.review_status ?? 'pending';
  ...
  const envelopeDir = join(siteRoot, '.ai', 'inbox-envelopes');
  let inboxCapas: AnyRecord[] = [];
  if (existsSync(envelopeDir)) {
    const files = readValidEnvelopeFiles(envelopeDir);
    const latestLogEvents = getLatestDurableEventsByEnvelope(siteRoot);
    const terminalStates = new Set(['acknowledged', 'dismissed']);
    const envelopeById = new Map(files.map((e: any) => [e.envelope_id, e]));
    ...
    inboxCapas = [...promotionsByEnvelope.entries()]
      .map(([envelopeId, event]) => {
        ...
        const effectiveStatus = envelope
          ...
          : resolveEnvelopeStatus(events.filter((e: any) => e.envelope_id === envelopeId));
        return { envelope, envelopeId, event, promotion, effectiveStatus };
      })
      .filter((item: any) => !terminalStates.has(item.effectiveStatus))
      .map((item: any) => ({
  ...
  if (existsSync(envelopeDir)) {
    ...
    for (const capa of allCapas) {
      ...
      const related = files.filter((e: any) => {
...
function capaRelated(args: any, site: any): any {
  const siteRoot = resolveTargetSiteRoot(args, site, {});
  ...
  const candidates = queue.capas
    .filter((capa: any) => capa.capa_id !== excludeCapaId)
    .map((capa: any) => scoreRelatedCapa(capa, queryTokens, conceptName))
    .filter((candidate: any) => candidate.score > 0)
    .sort((a: any, b: any) => b.score - a.score || b.shared_terms.length - a.shared_terms.length || b.severity - a.severity)
    .slice(0, limit);
...
function scoreRelatedCapa(capa: any, queryTokens: any, conceptName: any): any {
  ...
  const sharedTerms = [...querySet].filter((token: any) => candidateSet.has(token)).sort();
...
function tokenizeRelatedText(value: any): any {
  const stopWords = new Set(['about', 'after', 'again', 'before', 'into', 'from', 'that', 'this', 'with', 'without', 'when', 'where', 'while', 'and', 'the', 'for']);
  return [...new Set(String(value ?? '')
    .toLowerCase()
    .replace(/[^a-z0-9_\-\s]/g, ' ')
    .split(/\s+/)
    .map((token: any) => token.replace(/^[-_]+|[-_]+$/g, ''))
    .filter((token: any) => token.length >= 3 && !stopWords.has(token)))];
}

function normalizeRelatedPhrase(value: any): any {
  return tokenizeRelatedText(value).join(' ');
}

function writePayloadFixture(targetSiteRoot: any, payload: any): any {
...
function resolveNaradaCli(serverOptions: any, required: any): any {
...
function cliExists(value: any): any {
  return Boolean(value && existsSync(value));
}

function resolveTargetSiteRoot(args: any, site: any, serverOptions: any): any {
...
function buildTargetLocusGuardStatus(args: any, site: any, serverOptions: any): any {
  ...
  const operatorMismatch = mismatches.some((item: any) => item.kind === 'operator_stated_locus_default_site_root_mismatch'
...
function guardMutationTargetLocus(args: any, site: any, serverOptions: any, toolName: any): any {
...
function collectTargetLocusMismatches({ serverDefaultRoot, cwdRoot, operatorStatedRoot, explicitTargetRoot = null }: AnyRecord): any {
  const mismatches = [];
...
function resolveOperatorStatedRoot(): any {
...
function sameResolvedPath(left: any, right: any): any {
...
function admitInboxSubmitCommand(command: any, site: any, serverOptions: any): any {
  return admitInboxSubmitTypedEnvelopeCommand(command, site, serverOptions, 'narada.command.inbox.submit.result.v1');
}

function admitInboxSubmitTypedEnvelopeCommand(command: any, site: any, serverOptions: any, resultSchema: any = 'narada.command.inbox.submit_typed_envelope.result.v1'): any {
...
function admitInboxAcknowledgeCommand(command: any, site: any, serverOptions: any): any {
...
function admitInboxDismissCommand(command: any, site: any, serverOptions: any): any {
  return admitInboxDispositionCommand(command, site, serverOptions, {
    action: 'dismiss',
...
function admitInboxDispositionCommand(command: any, site: any, serverOptions: any, { action, toolName, resultSchema, run }: AnyRecord): any {
  const domainArgs = asRecord(command.domain_args);
  const merged = {
    ...domainArgs,
    target_locus: domainArgs.target_locus ?? command.target_locus,
...
function admitInboxPromoteCapaCommand(command: any, site: any, serverOptions: any): any {
  const domainArgs = asRecord(command.domain_args);
  const payloadRefs = Array.isArray(command.payload_refs) ? command.payload_refs : [];
  const payloadRefEntry = findCommandPayloadRefEntry(payloadRefs, 'domain_payload');
  const payloadRef = payloadRefFromEntry(payloadRefEntry);
...
function findCommandPayloadRefEntry(payloadRefs: any, preferredRole: any): any {
  const roleMatch = payloadRefs.find((entry: any) => payloadRefMetadataFromEntry(entry).role === preferredRole);
  return roleMatch ?? payloadRefs[0];
}

function enrichPayloadSourceFromEntry(payloadSource: any, entry: any): any {
...
function validateCapaPromotionPayloadForCommand(payload: any, payloadRefEntry: any = null): any {
  ...
  if (recurrenceEvidence !== undefined && (!Array.isArray(recurrenceEvidence) || recurrenceEvidence.some((item: any) => typeof item !== 'string' || item.trim().length === 0))) {
...
function admitInboxCapaRelatedCommand(command: any, site: any, serverOptions: any): any {
  const domainArgs = asRecord(command.domain_args);
  const merged = {
    ...domainArgs,
    target_locus: domainArgs.target_locus ?? command.target_locus,
...
function admitCapabilityReviewCompleteCommand(command: any, site: any, serverOptions: any): any {
  const domainArgs = {
...
function admitInboxExportDispositionLedgerCommand(command: any, site: any, serverOptions: any): any {
  const domainArgs = {
    ...asRecord(command.domain_args),
    target_locus: asRecord(command.domain_args).target_locus ?? command.target_locus,
    target_site_root: asRecord(command.domain_args).target_site_root ?? command.target_site_root,
...
function verifyInboxPrincipal(args: any, { sourceKind = 'agent_report', action, allowOperatorOverride = false }: AnyRecord = {}): any {
...
function parseJsonInput(input: any): any {
...
function parseContentLengthMessages(input: any): any {
...
function arrayStringSchema(description: any): any {
...
function stringArrayField(record: any, key: any): any {
  ...
  return value.filter((item: any) => typeof item === 'string' && item.trim().length > 0).map((item: any) => item.trim());
}
