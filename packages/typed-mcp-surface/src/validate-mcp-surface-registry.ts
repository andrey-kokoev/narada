...
let errors: any[] = [];
let warnings: any[] = [];
let localProjectionNotes: any[] = [];

function validateRegistry(): any {
  // Check schema
...
function validateSurface(surface: any): any {
  ...
  if (surface.tool_contract) {
    ...
    if (contract.tool_arguments) {
      for (const [tool, args] of Object.entries(contract.tool_arguments as AnyRecord)) {
    ...
    if (!surface.client_config.generated_path) {
      ...
      if (!existsSync(configPath)) {
        ...
        try {
          ...
          errors.push(`Surface ${surfaceId}: generated config is not valid JSON: ${e instanceof Error ? e.message : String(e)}`);
        }
      }
    }
  }
