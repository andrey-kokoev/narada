...
import { loadOperatorSurfaceLaunchMatrixContract, loadRuntimeSubstrateKindsContract } from './operator-surface-runtime-contract.js';
import {
...
const isCarrierConformanceProfile = (profile: any): boolean => profile
  ...
  && profile.known_gaps.every((gap: any) => isNonEmptyString(gap));
const isCarrierLaunchMatrixRow = (row: any): boolean => row
  && typeof row === 'object'
  ...
  && row.projection_capabilities.every((capability: any) => isNonEmptyString(capability))
  ...
  && row.expected_tools.every((tool: any) => isNonEmptyString(tool))
  ...
  && Array.isArray(row.states)
  && row.states.length > 0
  && row.states.every((state: any) => isNonEmptyString(state))
  ...
  && (row.intelligence_kernel_kinds === undefined
    || (Array.isArray(row.intelligence_kernel_kinds)
      ...
      && row.intelligence_kernel_kinds.every((kind: any) => INTELLIGENCE_KERNEL_KINDS.includes(kind))));
if (
...
export function operatorSurfaceLaunchMatrixRow(launchSelectionKind: any): any {
  return carrierLaunchMatrixByLaunchSelection.get(String(launchSelectionKind ?? '')) ?? null;
}

export function operatorSurfaceKindsForRuntimeHost(runtimeHostKind: any): readonly string[] {
  return Object.freeze(carrierLaunchMatrixRows
    .filter((row: any) => row.runtime_host_kind === runtimeHostKind)
    .map((row: any) => row.operator_surface_kind));
}

export function operatorSurfaceKindsForProjectionCapability(capability: any): readonly string[] {
  return Object.freeze((carrierLaunchMatrixByProjectionCapability.get(String(capability ?? '')) ?? [])
    .map((row: any) => row.operator_surface_kind));
}

export function defaultRuntimeForOperatorSurface(launchSelectionKind: any): any {
...
export function operatorSurfaceRefusal(candidate: any, {
...
export function normalizeRuntimeAlias(value: any): string {
...
export function carrierRefusal(candidate: any, {
...
export function runtimeRefusal(candidate: any, {
...
export function normalizeRuntimeSubstrateKind(value: any, {
  admittedRuntimeSubstrateKinds,
  runtimeContractSchema,
}: any = {}): JsonRecord {
