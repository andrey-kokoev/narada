...
type NarsSurfaceOrigin = 'local' | 'cloudflare';
type NarsRuntimeSurfaceQuadrantId = 'local/local' | 'local/cloudflare' | 'cloudflare/local' | 'cloudflare/cloudflare';

export type NarsCapabilityState = 'absent' | 'declared' | 'present';
export type NarsCapabilityEvidence = JsonRecord;

export interface NarsRuntimeSurfaceContract extends JsonRecord {
  schema: string;
...
interface NarsRuntimeSurfaceContractViolation {
  path: string;
  code: string;
  detail: string;
}

interface NarsRuntimeSurfaceContractValidation {
  ok: boolean;
...
export const NARS_RUNTIME_ORIGINS = Object.freeze(['local', 'cloudflare'] as const);
export const NARS_SURFACE_ORIGINS = Object.freeze(['local', 'cloudflare'] as const);
...
export const NARS_RUNTIME_EVIDENCE_CLASSES = Object.freeze(['genuine', 'projected', 'synthetic'] as const);
export const NARS_AUTHORITY_CANONICITY = Object.freeze(['canonical', 'synthetic_canonical'] as const);
export const NARS_PROJECTION_POSTURES = Object.freeze(['non_canonical_projection', 'synthetic_authority'] as const);
export const NARS_PROJECTION_ROUTE_KINDS = Object.freeze(['projection_edge', 'intent_route'] as const);
export const NARS_CAPABILITY_STATES = Object.freeze(['absent', 'declared', 'present'] as const);
export const NARS_CLOUDFLARE_NATIVE_MCP_STATES = Object.freeze(['absent', 'fabric_summary'] as const);
...
export const NARS_CLOUDFLARE_GRADUABLE_CAPABILITIES = Object.freeze(['provider_execution'] as const);
export const NARS_CROSSING_AUTHORITY_OWNERS = Object.freeze(['local', 'cloudflare-host'] as const);
...
export function runtimeOriginFromAuthorityHost(authorityRuntimeHost: any): NarsRuntimeOrigin | null {
  return HOST_TO_RUNTIME_ORIGIN[String(authorityRuntimeHost) as keyof typeof HOST_TO_RUNTIME_ORIGIN] ?? null;
}

export function deriveNarsRuntimeQuadrant(runtimeOrigin: any, surfaceOrigin: any): NarsRuntimeSurfaceQuadrantId | null {
  if (!NARS_RUNTIME_ORIGINS.includes(runtimeOrigin)) return null;
  if (!NARS_SURFACE_ORIGINS.includes(surfaceOrigin)) return null;
  return `${runtimeOrigin}/${surfaceOrigin}` as NarsRuntimeSurfaceQuadrantId;
}

export function buildNarsCapabilityProfile(runtimeOrigin: any, overrides: JsonRecord = {}): JsonRecord {
...
export function buildNarsRuntimeSurfaceContract({
  ...
  crossing = null,
  generated_at = null,
}: any = {}): NarsRuntimeSurfaceContract {
...
export function validateNarsRuntimeSurfaceContract(candidate: any): NarsRuntimeSurfaceContractValidation {
  const violations: NarsRuntimeSurfaceContractViolation[] = [];
  const push = (path: string, code: string, detail: string): void => { violations.push({ path, code, detail }); };
  ...
  if (!crossing || typeof crossing !== 'object' || Array.isArray(crossing)) {
    ...
    if (expectation && expectedQuadrant && JSON.stringify(crossing) !== JSON.stringify(NARS_RUNTIME_SURFACE_CROSSINGS[expectedQuadrant])) {
    ...
    if (evidence != null) {
      if (typeof evidence !== 'object' || Array.isArray(evidence)) {
        ...
        for (const [dimension, entry] of Object.entries(evidence)) {
          ...
          const evidenceEntry = entry as JsonRecord;
          if (!NARS_CAPABILITY_STATES.includes(evidenceEntry.state)) {
            push(`$.capability_evidence.${dimension}.state`, 'invalid_capability_evidence_state', 'Expected absent, declared, or present.');
          }
          if (typeof evidenceEntry.evidence_ref !== 'string' || !evidenceEntry.evidence_ref.trim()) {
          ...
          if (evidenceEntry.state === 'present' && (typeof evidenceEntry.graduated_at !== 'string' || !evidenceEntry.graduated_at.trim())) {
            push(`$.capability_evidence.${dimension}.graduated_at`, 'capability_graduated_at_required', 'present state requires graduated_at.');
          }
          if (evidenceEntry.state && capability[dimension] && evidenceEntry.state !== capability[dimension]) {
            push(`$.capability_evidence.${dimension}.state`, 'capability_evidence_state_mismatch', `Evidence state ${evidenceEntry.state} does not match capability_profile ${capability[dimension]}.`);
          }
        }
      }
    }
