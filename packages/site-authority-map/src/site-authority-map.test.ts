...
} from './site-authority-map.js';

interface FixtureCase {
  name: string;
  request: Record<string, unknown>;
  expected: { action: string; reason: string; authority_locus_kind: string | null };
}
...
const fixtureCases = JSON.parse(
  readFileSync(new URL('../fixtures/site-authority-map-cases.json', import.meta.url), 'utf8'),
) as FixtureCases;
