...
import {
  ...
  renderOperatorEvent,
  styleInlineCode,
} from './projected-terminal.js';
import { countReadlineSubmissionsForPaste, createBracketedPasteComposer, projectedHelpText } from './projected-input.js';
import type { JsonRecord } from './projected-input.js';
import type { ProjectedTerminalBridge, TerminalComposer } from './projected-terminal.js';

type RenderedLine = ReturnType<typeof renderOperatorEvent>[number];

function renderedText(line: RenderedLine): string {
  return typeof line === 'string' ? line : line.raw;
}
...
function renderedRaw(line: RenderedLine): string {
  if (typeof line === 'string') throw new Error('expected raw terminal projection item');
  return line.raw;
}

function slashFrame(command: string): { method: string; params: JsonRecord } {
  const action = createProjectedSlashCommandAction(command);
  if (!action || action.kind !== 'frame') throw new Error(`expected frame for ${command}`);
  return action.frame;
}

function jsonFrame(command: string): JsonRecord {
  const result = createExplicitJsonControlFrame(command);
  if (!result || !('frame' in result)) throw new Error(`expected JSON frame for ${command}`);
  return result.frame;
}

function jsonError(command: string): string {
  const result = createExplicitJsonControlFrame(command);
  if (!result || !('error' in result)) throw new Error(`expected JSON error for ${command}`);
  return result.error;
}

function requiredComposer(bridge: ProjectedTerminalBridge): TerminalComposer {
  if (!bridge.composer) throw new Error('expected interactive terminal composer');
  return bridge.composer;
}

interface TestInput extends PassThrough {
  isTTY?: boolean;
  setRawMode?: (mode: boolean) => void;
}

interface TestOutput extends PassThrough {
  isTTY?: boolean;
  columns?: number;
}
...
test('projected slash commands produce NARS protocol frames', () => {
  ...
  assert.equal(slashFrame('/status').method, 'session.health');
  assert.equal(slashFrame('/health').method, 'session.health');
  assert.equal(slashFrame('/events').method, 'session.events.subscribe');
  assert.equal(slashFrame('/interrupt').method, 'session.cancel');
  assert.equal(slashFrame('/exit').method, 'session.close');
  assert.equal(slashFrame('/quit').method, 'session.close');
  assert.equal(slashFrame('exit').method, 'session.close');
  assert.equal(slashFrame('/tool').method, 'session.command.execute');
  assert.deepEqual(slashFrame('/tool').params, {
    command: '/tools',
    value: '',
  });
  assert.deepEqual(slashFrame('/tool-outputs on').params, {
    command: '/tool-output',
    value: 'on',
  });
  assert.deepEqual(slashFrame('/queue clear').params, {
    command: '/queue clear',
    value: '',
  });
  assert.equal(jsonFrame('/json {"id":"health-1","method":"session.health","params":{}}').method, 'session.health');
  assert.equal(jsonFrame('/json {"id":"command-1","method":"session.command.execute","params":{"command":"/tool","value":""}}').method, 'session.command.execute');
  assert.equal(jsonError('/json {"id":"bad-1","method":"bad.method","params":{}}'), '/json unsupported session-core method: bad.method');
  ...
  if (!steering) throw new Error('expected steering frame');
...
test('startup event renders operator-facing runtime summary rows', () => {
  ...
  assert.equal(rendered.some((line) => renderedText(line).includes('agent-cli')), true);
  assert.equal(rendered.some((line) => renderedText(line).includes('narada.test')), true);
  assert.equal(rendered.some((line) => renderedText(line).includes('cloudflare-host epoch 4')), true);
  assert.equal(rendered.some((line) => renderedText(line).includes('target_active')), true);
  assert.equal(rendered.some((line) => renderedText(line).includes('authority_locator:cloudflare-host/site/cf_session')), true);
  assert.equal(rendered.some((line) => renderedText(line).includes('narada-test-agent-context')), true);
});
...
test('operator event rendering accepts terminalColumns alias for wrapping', () => {
  ...
  assert.equal(rendered.slice(1).every((line) => renderedText(line).startsWith('  ')), true);
});
...
test('projected stream renders triple-backtick code blocks as code', () => {
  const style = createOperatorStyle({ enabled: true });
  const state = { streamedTurns: new Set<string>(), timestamps: false, style };
  ...
  assert.equal(rendered.length, 1);
  assert.equal(renderedRaw(rendered[0]).includes('```'), false);
  assert.equal(renderedRaw(rendered[0]).includes('\x1b[90m  do everything atomically or roll everything back\x1b[0m'), true);
});
...
test('projected terminal bridge inserts bracketed multiline paste and submits on enter', async () => {
  ...
  childStdin.on('data', (chunk: Buffer) => {
    for (const line of chunk.toString('utf8').trim().split(/\n/).filter(Boolean)) frames.push(JSON.parse(line) as TestFrame);
  });
  ...
  assert.equal(requiredComposer(bridge).getDraft(), pasted);
...
test('projected terminal bridge handles raw navigation escape sequences in draft', async () => {
  ...
  assert.equal(requiredComposer(bridge).getDraft(), 'SXabcZY');
  assert.equal(requiredComposer(bridge).getDraft().includes('[H'), false);
});
