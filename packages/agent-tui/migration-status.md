...
- MCP runtime config is explicit: Site MCP fabric remains disabled unless `NARADA_AGENT_TUI_ENABLE_MCP_FABRIC` is enabled; process launch MCP config/fabric env reaches `configured` posture only when the MCP config path is lexically inside the declared Site MCP fabric root and readable, while production exposure remains controlled by launch admission gates.
