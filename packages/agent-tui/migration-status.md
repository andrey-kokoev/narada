...
- MCP fabric transport parses carrier MCP config, normalizes nonblank server names, derives policy-bound visibility from configured nonblank tool names, resolves tools to nonblank stdio server launch commands, and prepares evidenced JSON-RPC `tools/call` requests for supervised stdio execution.
