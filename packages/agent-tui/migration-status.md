...
- MCP fabric transport parses carrier MCP config, normalizes nonblank server names and optional server metadata, validates nonblank stdio transport values, derives policy-bound visibility from configured nonblank tool names, resolves tools to nonblank stdio server launch commands and args, and prepares evidenced JSON-RPC `tools/call` requests for supervised stdio execution.
