...
- MCP fabric transport parses carrier MCP config, derives policy-bound visibility from configured nonblank tool names, resolves tools to stdio server launch specs, and prepares evidenced JSON-RPC `tools/call` requests for supervised stdio execution.
