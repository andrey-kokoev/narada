...
- Turn coordinator with provider dispatch boundary recording and carrier-validated, schema-marked explicit `completed_without_provider` terminal evidence.
- Provider dispatch trait plus schema-marked provider request/output payload contracts, centralized adapter factory, stub, and scripted admitted adapters; real provider execution remains disabled outside explicit adapter admission.
