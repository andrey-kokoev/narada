...
- Layout, status, composer, transcript, and aggregate app view models, including one runtime posture bundle for disabled/refused/configured-not-implemented provider posture plus disabled/refused/configured MCP and terminal-rendering posture in status output; process env is snapshotted through the library runtime config snapshot boundary before deriving the combined posture.
