...
- `provider-adapters.json`: provider execution and adapter-kind environment gates, provider adapter kind names, and production implementation posture used by Rust provider runtime/admission and `agent-start` provider launch metadata.
