...
function findExactLinkedTask(store: any, envelope: any) : any {
  ...
  if (store.getTaskByEnvelopeId) {
    ...
    if (mapping) {
      const lifecycle: any = store.db.prepare('SELECT status FROM task_lifecycle WHERE task_id = ?').get(mapping.task_id);
      return {
  ...
  `).all(envelopeId).find((candidate: any) : any => hasEnvelopeCoverageEvidence(candidate, envelopeId));
...
export function buildInboxWorkboard(siteRoot: any, { store }: any = {}) : any {
  const index: any = readIndexedInboxBacklog(siteRoot, { evaluateEnvelopeSeverity });
  const rows: any = index.rows;
  const backlog: any = [];
  const linkedTaskSuppressed: any = [];
  let highSeverity: any = 0;

  for (const row of rows) {
    const envelope: any = row.envelope;
    const linkedTask: any = findExactLinkedTask(store, envelope);
    const item: any = {
  ...
  return {
    ...
    counts: {
      ...
      incidents: backlog.filter((e: any) : any => e.kind === 'incident').length,
      capa_requests: backlog.filter((e: any) : any => e.action === 'review_capa_request').length,
      observations: backlog.filter((e: any) : any => e.kind === 'observation').length,
      proposals: backlog.filter((e: any) : any => e.kind === 'proposal').length,
