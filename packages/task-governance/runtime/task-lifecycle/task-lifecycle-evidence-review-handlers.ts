export const TASK_LIFECYCLE_EVIDENCE_REVIEW_TOOL_NAMES: any = Object.freeze([
...
export function createTaskLifecycleEvidenceReviewHandlers(context: any) : any {
  const {
  ...
  async function dispatchEvidenceReviewTool(canonicalName: any, args: any, dispatchContext: any = {}) : Promise<any> {
    ...
    case 'task_lifecycle_finish': {
      ...
      if (!taskNumber) throw new Error('task_number_required');
      if (!agentId) throw new Error('agent_id_required');
      const validReviewVerdicts: any = ['accepted', 'accepted_with_notes', 'rejected'];
      ...
      const isBlocked: any = payload.close_action === 'blocked';
      if (isBlocked) {
    ...
    case 'task_lifecycle_close': {
      ...
      const result: any = await withAuthoredRosterJsonPreserved(siteRoot, () : any => closeTaskService({ cwd: siteRoot, taskNumber, agent: agentId, mode, noContinuationNeeded }));
      const payload: any = result.result || result;
    ...
    case 'task_lifecycle_review': {
      ...
      const payload: any = result.result || result;
      const isBlocked: any = payload.evidence_blocked === true || payload.close_action === 'blocked';
      if (isBlocked) {
        payload.close_blocked = true;
      }
