...
function normalizedNonEmptyString(value: any) : any {
  return typeof value === 'string' && value.trim().length > 0 ? value.trim() : null;
}

function looksLikeAgentId(value: any) : any {
  return Boolean(normalizedNonEmptyString(value)?.includes('.'));
}

function firstPayloadString(payload: any, fields: any, { requireAgentId = false }: any = {}) : any {
...
function resolveAgentRoleFromStore(store: any, agentId: any) : any {
...
function ensureTaskRolePreferencesTable(store: any) : any {
  store.db.exec(`
...
export function deriveRoutingFromEnvelopePayload(envelope: any, severityResult: any = {}, store: any = null) : any {
  const payload: any = envelope?.payload ?? {};
  const ownership: any = firstPayloadString(payload, OWNERSHIP_FIELD_PRECEDENCE, { requireAgentId: true });
  const explicitRole: any = firstPayloadString(payload, ROLE_FIELD_PRECEDENCE);
 ...
 */
export function checkDuplicateTask(store: any, envelope: any) : any {
 ...
 */
export function buildTaskSpecFromEnvelope(envelope: any, severityResult: any, options: any = {}) : any {
  const payload: any = envelope.payload ?? {};
  const title: any = `[From Inbox] ${payload.title ?? envelope.title ?? 'Untitled'}`;
  const goal: any = payload.summary ?? payload.description ?? '';
  const routing: any = options.routing ?? deriveRoutingFromEnvelopePayload(envelope, severityResult, options.store ?? null);
  ...
  const requiredWork: any = workItems.join('\n');
 ...
 */
export function decideEnvelopeBridgeOutcome({ store, envelope, severityResult, dryRun = false }: any) : any {
...
export function summarizeBridgeOutcome(outcome: any) : any {
  return {
 ...
 */
export async function materializeEnvelopeAsTask(cwd: any, envelope: any) : Promise<any> {
  ...
  const slug: any = slugify(spec.title);
  ...
  if (proposals.length > 0 || evidence.length > 0) {
    const parts: any = [];
    if (evidence.length > 0) {
      parts.push('Evidence:', ...evidence.map((e: any) : any => `- ${e}`));
    }
    if (proposals.length > 0) {
      parts.push('Proposals:', ...proposals.map((p: any) : any => `- ${p}`));
    }
 ...
 */
export function markEnvelopeMaterialized(cwd: any, envelope: any, taskNumber: any, taskId: any) : any {
...
export async function applyMaterializableBridgeOutcome({ cwd, store, envelope, outcome }: any) : Promise<any> {
...
function buildBridgeCommittablePathSet(cwd: any, materializeResult: any, markResult: any) : any {
...
function relativeSitePath(cwd: any, path: any) : any {
 ...
 */
function readEnvelopeFiles(cwd: any) : any {
  const envelopeDir: any = join(resolve(cwd), INBOX_DIR);
...
export function readEnvelopeById(cwd: any, envelopeId: any) : any {
  return readEnvelopeFiles(cwd).find((entry: any)  => entry.envelope?.envelope_id === envelopeId) ?? null;
}

function updateEnvelopeDispositionFile(entry: any, status: any, resolution: any) : any {
...
export function readUnprocessedEnvelopes(cwd: any) : any {
  const fileEnvelopes: any = readEnvelopeFiles(cwd).map((entry: any) : any => entry.envelope);

  // Try admission log first
  try {
    const latestEvents: any = getLatestEventsByEnvelope(cwd);
    const processedKinds: any = new Set(['envelope_promoted', 'envelope_dismissed', 'envelope_acknowledged']);
...
export async function targetInboxEnvelope(cwd: any, options: any = {}) : Promise<any> {
  const envelopeId: any = options.envelopeId ?? options.envelope_id;
  if (!envelopeId) throw new Error('envelope_id_required');

  const dryRun: any = options.dryRun ?? options.dry_run ?? false;
 ...
 */
export async function pollInboxBridge(cwd: any, options: any = {}) : Promise<any> {
  const dryRun: any = options.dryRun ?? false;
  const threshold: any = options.threshold ?? AUTO_MATERIALIZE_THRESHOLD;
  const limit: any = options.limit ?? 20;

  let envelopes: any = readUnprocessedEnvelopes(cwd);
  // Sort by severity descending so highest-priority items are processed first
  envelopes = envelopes
    .map((e: any) : any => ({ envelope: e, severityResult: evaluateEnvelopeSeverity(e) }))
    .sort((a: any, b: any) : any => b.severityResult.severity - a.severityResult.severity)
    .map((item: any) : any => item.envelope);
  ...
  try {
    ...
    return {
      status: 'error',
      error: `failed_to_open_store: ${e instanceof Error ? e.message : String(e)}`,
      evaluated,
      materialized,
      skipped,
      duplicates,
