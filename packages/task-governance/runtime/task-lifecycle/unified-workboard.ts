...
export function buildNextWorkContract(board: any, recommendation: any = null) : any {
  const executableCounts: any = {
    ...
    high_severity_inbox: (board.inbox_backlog || []).filter((item: any) : any => item.severity >= 70).length,
  };
  const executableTotal: any = Object.values(executableCounts).reduce((sum: any, count) : any => sum + count, 0);
  const roleWidePresent: any = executableCounts.role_wide_followups > 0;
...
export function deriveNextRecommendation(board: any, agentId: any) : any {
  ...
  const highSeverityInbox: any = board.inbox_backlog.filter((b: any) : any => b.severity >= 70);
  ...
  if (board.local_followups.length > 0) {
    const claimable: any = board.local_followups.filter((t: any) : any => !t.assigned_agent);
  ...
  if ((board.role_wide_followups || []).length > 0) {
    const claimable: any = board.role_wide_followups.filter((t: any) : any => !t.assigned_agent);
    if (claimable.length > 0) {
      const task: any = claimable[0];
      return {
...
export function buildUnifiedWorkboard({ store, siteRoot, agentId, agentRole, allTasks, limit = 8 }: any) : any {
  // Build task workboard
  const taskBoard: any = buildWorkboard({ store, siteRoot, agentId, agentRole, allTasks });

  // Build inbox workboard
  const inboxBoard: any = buildInboxWorkboard(siteRoot, { store });

  // Build obligations
  let obligations: any = [];
  if (agentId) {
    const rawObligations: any = store.listDirectedObligationsForTarget(agentId, agentRole, 'open');
    obligations = rawObligations.map((o: any) : any => ({
  ...
  // 2. Review obligations
  for (const item of obligations.filter((o: any)  => o.kind === 'review_request')) {
    recommendations.push({
  ...
  const oneHourAgo: any = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  const recentlyMaterialized: any = [
  ...
  return {
    ...
    my_review_obligations: obligations.filter((o: any) : any => o.kind === 'review_request').slice(0, limit),
    ...
    counts: {
      ...
      my_review_obligations: obligations.filter((o: any) : any => o.kind === 'review_request').length,
