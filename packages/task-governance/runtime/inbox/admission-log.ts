...
const DEFAULT_DISPOSITION_EXPORT_PATH: any = 'kb/operations/inbox-disposition-ledger.json';
const DISPOSITION_EVENT_KINDS: any = new Set(['envelope_acknowledged', 'envelope_dismissed', 'envelope_promoted']);
const ROTATION_THRESHOLD_BYTES: any = 10 * 1024 * 1024; // 10 MB

function logPath(cwd: any) : any {
  return join(resolve(cwd), LOG_DIR, LOG_FILE);
}

function ensureDir(cwd: any) : any {
  const dir: any = join(resolve(cwd), LOG_DIR);
  if (!existsSync(dir)) {
    mkdirSync(dir, { recursive: true });
  }
}

function hashPayload(payload: any) : any {
  return 'sha256:' + createHash('sha256').update(JSON.stringify(payload)).digest('hex');
}

function getNextSequence(cwd: any) : any {
  const path: any = logPath(cwd);
...
function rotateIfNeeded(cwd: any) : any {
  const path: any = logPath(cwd);
  ...
  const rotatedPath: any = join(resolve(cwd), LOG_DIR, rotatedName);
 ...
 */
export function appendAdmissionEvent(cwd: any, event: any) : any {
  ensureDir(cwd);
  rotateIfNeeded(cwd);

  const eventId: any = 'evt_' + randomUUID().replace(/-/g, '');
  const eventSequence: any = getNextSequence(cwd);
  ...
  const line: any = JSON.stringify(fullEvent) + '\n';
 ...
 */
export function admitEnvelope(cwd: any, envelope: any) : any {
  const envelopesPath: any = join(resolve(cwd), ENVELOPES_DIR);
  if (!existsSync(envelopesPath)) {
    mkdirSync(envelopesPath, { recursive: true });
  }

  const envelopeId: any = envelope.envelope_id ?? ('env_' + randomUUID());
  const receivedAt: any = envelope.received_at ?? new Date().toISOString();
  const fullEnvelope: any = {
    ...envelope,
    envelope_id: envelopeId,
    received_at: receivedAt,
  };

  const payloadJson: any = JSON.stringify(fullEnvelope, null, 2);
  ...
  const event: any = emitEnvelopeAdmitted(cwd, fullEnvelope, {
...
export function emitEnvelopeAdmitted(cwd: any, envelope: any, meta: any = {}) : any {
  const payloadHash: any = hashPayload(envelope);
  const payloadUri: any = meta.payload_uri ?? `.ai/inbox-envelopes/${envelope.envelope_id}.json`;
  ...
  const admittedEvent: any = appendAdmissionEvent(cwd, {
    envelope_id: envelope.envelope_id,
 ...
 */
export function acknowledgeEnvelope(cwd: any, envelopeId: any, principal: any, reason: any) : any {
 ...
 */
export function dismissEnvelope(cwd: any, envelopeId: any, principal: any, reason: any) : any {
  return appendAdmissionEvent(cwd, {
    envelope_id: envelopeId,
 ...
 */
export function readAdmissionLog(cwd: any) : any {
  const path: any = logPath(cwd);
  if (!existsSync(path)) return [];
...
export function exportDispositionLedger(cwd: any, options: any = {}) : any {
  const outputPath: any = options.output_path ?? DEFAULT_DISPOSITION_EXPORT_PATH;
  const siteId: any = options.site_id ?? 'Narada';
  const events: any = readAdmissionLog(cwd)
    .filter((event: any) : any => DISPOSITION_EVENT_KINDS.has(event.event_kind))
    .map((event: any) : any => ({
  ...
  const exportPath: any = resolve(cwd, outputPath);
  const payload: any = {
 ...
 */
export function getLatestEventsByEnvelope(cwd: any) : any {
  const events: any = readAdmissionLog(cwd);
  const map: any = new Map();
  for (const evt of events) {
    if (!evt.envelope_id) continue;
    const existing: any = map.get(evt.envelope_id);
 ...
 */
export function resolveEnvelopeStatus(events: any) : any {
  const kinds: any = new Set(events.map((e: any) : any => e.event_kind));
 ...
 */
export function recordPromotion(cwd: any, envelopeId: any, promotion: any) : any {
  return appendAdmissionEvent(cwd, {
    envelope_id: envelopeId,
