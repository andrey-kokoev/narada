...
export function isValidEnvelopeId(envelopeId: any) : any {
  return typeof envelopeId === 'string' && ENVELOPE_ID_PATTERN.test(envelopeId);
}

function indexPath(siteRoot: any) : any {
  return join(resolve(siteRoot), INDEX_PATH);
}

function openInboxIndex(siteRoot: any) : any {
...
function envelopeFiles(siteRoot: any) : any {
...
function readEnvelopeFile(filePath: any) : any {
  try {
...
function effectiveStatus(envelope: any, latestEvents: any) : any {
...
export function refreshInboxIndex(siteRoot: any, { evaluateEnvelopeSeverity }: any = {}) : any {
  const { db, dbPath }: any = openInboxIndex(siteRoot);
  const now: any = new Date().toISOString();
  ...
  const transaction: any = db.transaction(() : any => {
    for (const filePath of files) {
      ...
      const { envelope, text }: any = raw;
      const severity: any = evaluateEnvelopeSeverity ? evaluateEnvelopeSeverity(envelope) : {};
  ...
  const total: any = db.prepare('SELECT COUNT(*) AS count FROM inbox_envelopes').get().count;
  return {
    status: 'ok',
    db,
    db_path: dbPath,
...
export function readIndexedInboxBacklog(siteRoot: any, { evaluateEnvelopeSeverity }: any = {}) : any {
...
export function readInboxIndexCounts(siteRoot: any, { evaluateEnvelopeSeverity }: any = {}) : any {
  const index: any = refreshInboxIndex(siteRoot, { evaluateEnvelopeSeverity });
  try {
    const receivedStatus: any = { status: 'received' };
    const count: any = (where: any, params: any = {}) : any => index.db.prepare(`SELECT COUNT(*) AS count FROM inbox_envelopes ${where}`).get(params).count;
    return {
      ...index,
      counts: {
