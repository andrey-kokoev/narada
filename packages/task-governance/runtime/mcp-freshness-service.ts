...
export function buildMcpFreshnessStatus({
  ...
  const missingExpectedTools: any = expectedTools.filter((name: any) : any => !registeredTools.includes(name));
...
function inheritedNarsSessionEnvironment(env: any = process.env) : any {
...
export function writeMcpRuntimeInstanceObservation({
  ...
  const index: any = instances.findIndex((candidate: any) : any =>
...
function sameMcpSiteRoot(candidate: any, siteRoot: any) : any {
...
function canonicalMcpSiteRoot(value: any) : any {
...
function reconcileMcpRuntimeRegistryAfterAcknowledgement({
  ...
  const registry: any = readJsonFile(registryPath);
  const instances: any = Array.isArray(registry?.instances) ? registry.instances : [];
  const index: any = instances.findIndex((entry: any) : any => entry?.surface_id === targetSurface
...
export function validateMcpRestartAcknowledgement({
  ...
  const registry: any = readJsonFile(registryPath);
  const instances: any = Array.isArray(registry?.instances) ? registry.instances : [];
  const instance: any = instances.find((entry: any) : any => entry?.surface_id === targetSurface
    || entry?.server_entrypoint === targetEntrypoint
    || entry?.server_entry_point === targetEntrypoint);
  if (!instance) {
    return {
...
export function buildMcpRestartPressure(freshnessEntries: any = []) : any {
  const staleSurfaces: any = freshnessEntries.filter((entry: any) : any =>
  ...
  return {
    ...
    surfaces: staleSurfaces.map((entry: any) : any => {
...
export function deriveMcpRestartPressureRecommendation(mcpRestartPressure: any) : any {
  ...
  return {
    action: mcpRestartPressure.surfaces?.some((surface: any) : any => surface.startup_disposition === 'operator_restart_required_with_handle')
...
export function buildStaleLiveNavigationDegradation(mcpRestartPressure: any) : any {
  ...
  return {
    ...
    field_quality: Object.fromEntries(fields.map((field: any) : any => [
      field,
      {
