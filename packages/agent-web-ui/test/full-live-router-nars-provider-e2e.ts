...
if (cleanupErrors.length > 0) {
  process.stderr.write('full_live_cleanup_errors:\n' + cleanupErrors.map((error: any) => String(error?.stack ?? error)).join('\n') + '\n');
  process.exitCode = 1;
}
...
async function runFullLiveE2e() {
  ...
  assert.ok(routesBeforeBrowser.every((route: any) => route.state === 'healthy'), JSON.stringify(routesBeforeBrowser));
  ...
  assert.ok(
    routesBeforeBrowser.some((route: any) => normalizePathname(route.public_path) === publicWebUiPath),
    JSON.stringify({ expected: publicWebUiPath, routes: routesBeforeBrowser }),
  );
  assert.ok(
    routesBeforeBrowser.some((route: any) => normalizePathname(route.public_path) === publicWebSocketPath),
  ...
  await waitFor(
    () => page.network.websocketCreated.some((url: any) => sameUrl(url, publicWebSocketUrl)),
    'browser_public_websocket_created',
  );
  assert.equal(page.network.websocketCreated.some((url: any) => sameUrl(url, record.event_endpoint)), false);
  await page.evaluate(
    '(() => {'
  ...
  const interruptedQueued = await waitForEvent(
    ...
    (event: any) => event.event === 'input_event_queued' && event.idempotency_key === interrupted.idempotencyKey,
    'interrupted_input_queued',
  );
  await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'input_event_started' && event.request_id === interruptedQueued.request_id,
    'interrupted_input_started',
  );
  const interruptedTurnStarted = await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'carrier_turn_started' && event.turn_id === interruptedQueued.event_id,
  ...
  const interruptedAssistant = await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'assistant_message' && event.turn_id === interruptedTurnStarted.turn_id,
    'interrupted_assistant_durable_completion',
  );
  const interruptedCompleted = await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'carrier_turn_completed' && event.turn_id === interruptedTurnStarted.turn_id,
    'interrupted_turn_durable_completion',
  );
  const interruptedInputCompleted = await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'input_event_completed' && event.request_id === interruptedQueued.request_id,
    'interrupted_input_durable_completion',
  );
  const interruptedResponse = await waitForEvent(
    record.events_path,
    interruptedFromIndex,
    (event: any) => event.event === 'session_control_response' && event.request_id === interruptedQueued.request_id,
  ...
  await waitFor(
    async () => {
      ...
      return routes.length >= 2 && routes.every((route: any) => route.state === 'healthy');
    },
  ...
  assert.equal(readProviderRecords().filter((entry: any) => entry.content === 'full-live interrupted sentinel').length, 1);
  ...
  const retryQueued = await waitForEvent(
    record.events_path,
    retryFromIndex,
    (event: any) => event.event === 'input_event_queued' && event.idempotency_key === retry.idempotencyKey,
    'retry_input_queued',
  );
  const retryTurnStarted = await waitForEvent(
    record.events_path,
    retryFromIndex,
    (event: any) => event.event === 'carrier_turn_started' && event.turn_id === retryQueued.event_id,
    'retry_turn_started',
  );
  ...
  const deduplicated = await waitForEvent(
    record.events_path,
    retryFromIndex,
    (event: any) => event.event === 'input_event_deduplicated'
  ...
  await new Promise((resolve: any) => setTimeout(resolve, 350));
  assert.equal(readProviderRecords().filter((entry: any) => entry.content === 'full-live idempotent retry sentinel').length, 1);
  await writeControl({ hold: false, release: true });
...
function startRouter(stateRoot: any) {
...
async function submitUiInput(currentPage: any, content: any) {
...
async function sendPublicProtocolRetry(currentPage: any, publicWebSocketUrl: any, original: any) {
  const frame = {
...
async function waitForCompletedOperation(eventsPath: any, fromIndex: any, submitted: any, label: any, knownTurnStarted: any= null) {
  const queued = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'input_event_queued' && event.idempotency_key === submitted.idempotencyKey,
    label + '_queued',
  );
  const started = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'input_event_started' && event.request_id === queued.request_id,
    label + '_started',
  );
  const turnStarted = knownTurnStarted ?? await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'carrier_turn_started' && event.turn_id === queued.event_id,
    label + '_turn_started',
  );
  const assistant = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'assistant_message' && event.turn_id === turnStarted.turn_id,
    label + '_assistant',
  );
  const turnCompleted = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'carrier_turn_completed' && event.turn_id === turnStarted.turn_id,
    label + '_turn_completed',
  );
  const inputCompleted = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'input_event_completed' && event.request_id === queued.request_id,
    label + '_input_completed',
  );
  const response = await waitForEvent(
    eventsPath,
    fromIndex,
    (event: any) => event.event === 'session_control_response' && event.request_id === queued.request_id,
    label + '_control_response',
  );
  const sequence = [queued, started, turnStarted, assistant, turnCompleted, inputCompleted, response]
    .map((event: any) => Number(event.event_sequence ?? event.sequence));
...
async function assertUiCompleted(currentPage: any, requestId: any) {
...
async function waitForProviderContent(content: any, count: any) {
  return waitFor(
    () => {
      const matches = readProviderRecords().filter((entry: any) => entry.content === content);
...
async function writeControl(value: any) {
  await writeFile(providerControlFile, JSON.stringify(value) + '\n', 'utf8');
}

async function waitForSessionRecord(root: any, agentId: any) {
  try {
    ...
    const indexes = Object.fromEntries(indexPaths.map((path: any) => [path, readJsonFileIfPresent(path)]));
    ...
    const runtimeProcessFiles = Object.fromEntries(runtimeProcessEntries
      .filter((entry: any) => /\.(log|json)$/i.test(String(entry)))
      .map((entry: any) => {
...
function findSessionRecord(root: any, agentId: any) {
  const records = [];
  const recordsRoots = [
  ...
  records.sort((left: any, right: any) => Date.parse(right.started_at ?? '') - Date.parse(left.started_at ?? ''));
  return records[0] ?? false;
}

async function waitForEvent(eventsPath: any, fromIndex: any, predicate: any, label: any) {
  try {
    return await waitFor(
...
async function waitForRouterHealthy(url: any, label: any) {
...
async function waitForHealthy(url: any, label: any) {
  return waitFor(async () => {
    try {
...
async function waitForEndpointClosed(url: any, label: any) {
  return waitFor(async () => {
    try {
...
async function waitForProcessNotAlive(pid: any, label: any) {
  return waitFor(() => {
    try {
      process.kill(pid, 0);
      return false;
...
async function waitForSessionRoutes(url: any, sessionId: any, minimum: any) {
  return waitFor(async () => {
...
async function readSessionRoutes(url: any, sessionId: any) {
  try {
    const response = await fetch(url + '/routes', { cache: 'no-store' });
    if (!response.ok) return [];
    const body = await response.json();
    return (body.routes ?? []).filter((route: any) => route.session_id === sessionId);
  } catch {
    return [];
  }
}
...
async function stopProcess(child: any, label: any, gracefulSignal: any= 'SIGTERM', { signalRelay = false }: any= {}) {
...
async function waitForExit(child: any, timeoutMs: any) {
...
function readJsonFileIfPresent(path: any) {
  try {
...
function readTextFileIfPresent(path: any) {
  try {
...
function normalizePathname(value: any) {
  const normalized = String(value ?? '').replace(/\/+$/, '');
  return normalized || '/';
}

function sameUrl(left: any, right: any) {
...
async function openCdpPage({ browserPath, url, workDir }: any) {
  ...
  const pages = await fetch('http://' + browserHttpUrl.host + '/json/list').then((response: any) => response.json());
  const target = pages.find((entry: any) => entry.type === 'page') ?? pages[0];
  ...
  const send = (method: any, params: any= {}) => new Promise((resolve: any, reject: any) => {
    const id = ++commandId;
