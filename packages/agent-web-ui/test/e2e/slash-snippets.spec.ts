...
async function visibleCommandPaletteSlashes(page: any) {
  return await page.locator('#agent-web-ui-command-palette code').evaluateAll((nodes: any) => nodes.map((node: any) => node.textContent ?? ''));
}
...
test.describe('agent-web-ui slash and snippet palette', () => {
  ...
  test('browser and attach-client slash health commands project the same session snapshot without raw CDP', async ({ page }: any) => {
    ...
    attachOutput.on('data', (chunk: any) => { attachText += String(chunk); });
    ...
    try {
      ...
      const browserHealth = await waitFor(
        () => runtime.events.slice(browserFromIndex).find((event: any) => event.event === 'session_health'),
        5_000,
        () => ({ browser_events: runtime.events.slice(browserFromIndex).map((event: any) => ({ event: event.event, request_id: event.request_id, session_id: event.session_id, status: event.status })) }),
      );
      expect(runtime.outboundFrames.find((frame: any) => frame.method === 'session.health')).toMatchObject({
      ...
      const cliHealth = await waitFor(
        () => runtime.events.slice(cliFromIndex).find((event: any) => event.event === 'session_health'),
        5_000,
        () => ({ cli_events: runtime.events.slice(cliFromIndex).map((event: any) => ({ event: event.event, request_id: event.request_id, session_id: event.session_id, status: event.status })) }),
      );

      expect(healthSnapshot(browserHealth)).toEqual(healthSnapshot(cliHealth));
    } finally {
      attachInput.end();
      await Promise.race([attachPromise, new Promise((resolve: any) => setTimeout(resolve, 1_000))]);
      await page.close().catch(() => {});
      await runtime.close();
    }
  });
  test('supports command autocomplete and snippet submenu without raw CDP', async ({ page }: any) => {
    const runtime = await startSharedRuntime();
    try {
      await page.goto(runtime.localWeb.url);
      await expect(page.locator('#operator-input')).toBeVisible();
      ...
      const submittedEvent = await waitFor(
        () => runtime.events.slice(healthFromIndex).find((event: any) => event.event === 'session_health'),
        5_000,
        () => ({ events: runtime.events.slice(healthFromIndex).map((event: any) => ({ event: event.event, request_id: event.request_id, status: event.status })) }),
      );
      ...
      await page.keyboard.press('Enter');
      await waitFor(
        () => runtime.events.slice(runFromIndex).some((event: any) => event.event === 'assistant_message' && event.content === 'web-ui playwright test response'),
        5_000,
        () => ({ events: runtime.events.slice(runFromIndex).map((event: any) => ({ event: event.event, content: event.content, message: event.message, method: event.method })) }),
      );
      ...
      await waitFor(
        () => runtime.events.slice(clickRunFromIndex).some((event: any) => event.event === 'assistant_message' && event.content === 'web-ui playwright test response'),
        5_000,
        () => ({ events: runtime.events.slice(clickRunFromIndex).map((event: any) => ({ event: event.event, content: event.content, message: event.message, method: event.method })) }),
      );
      expect(runtime.inputFrameAttempts.at(-1)?.frame).toMatchObject({
        method: 'session.submit',
        params: { content: 'Run from snippet submenu', source: 'manual_operator' },
      ...
      await page.locator('.command-option-active').click();
      await waitFor(
        () => runtime.events.slice(clickQueueFromIndex).some((event: any) => event.event === 'input_event_completed')
          && runtime.events.slice(clickQueueFromIndex).some((event: any) => event.event === 'assistant_message' && event.content === 'web-ui playwright test response'),
        5_000,
        () => ({ events: runtime.events.slice(clickQueueFromIndex).map((event: any) => ({ event: event.event, content: event.content, message: event.message, method: event.method })) }),
      );
      expect(runtime.inputFrameAttempts.at(-1)?.frame).toMatchObject({
        method: 'session.submit',
  ...
  test('snippet slash grammar is registry-driven across actions and aliases', async ({ page }: any) => {
    const runtime = await startSharedRuntime();
    try {
      await page.goto(runtime.localWeb.url);
      await expect(page.locator('#operator-input')).toBeVisible();
      ...
      const searchAction = AGENT_WEB_UI_SNIPPET_ACTIONS.find((action: any) => action.id === 'search');
      ...
      const editAction = AGENT_WEB_UI_SNIPPET_ACTIONS.find((action: any) => action.id === 'edit');
      for (const verb of editAction.verbs) {
        ...
        await expect.poll(() => page.evaluate(() => JSON.parse(window.localStorage.getItem('narada:agent-web-ui:operator-snippets.v1') ?? '[]').find((entry: any) => entry.name === 'registry-run')?.body)).toBe(`Registry run body edited by ${verb}`);
      }

      const runAction = AGENT_WEB_UI_SNIPPET_ACTIONS.find((action: any) => action.id === 'run');
      for (const verb of runAction.verbs) {
        const fromIndex = runtime.events.length;
        await submitOperatorInputText(page, `/snippet ${verb} registry-run`);
        await waitFor(
          () => runtime.events.slice(fromIndex).some((event: any) => event.event === 'assistant_message' && event.content === 'web-ui playwright test response'),
          5_000,
          () => ({ verb, events: runtime.events.slice(fromIndex).map((event: any) => ({ event: event.event, content: event.content, message: event.message, method: event.method })) }),
        );
        await expect(page.getByText(/Ran snippet: registry-run/).last()).toBeVisible();
      }

      const enqueueAction = AGENT_WEB_UI_SNIPPET_ACTIONS.find((action: any) => action.id === 'enqueue');
      for (const verb of enqueueAction.verbs) {
        const fromIndex = runtime.events.length;
        await submitOperatorInputText(page, `/snippet ${verb} registry-run`);
        await waitFor(
          () => runtime.events.slice(fromIndex).some((event: any) => event.event === 'input_event_completed')
            && runtime.events.slice(fromIndex).some((event: any) => event.event === 'assistant_message' && event.content === 'web-ui playwright test response'),
          5_000,
          () => ({ verb, events: runtime.events.slice(fromIndex).map((event: any) => ({ event: event.event, content: event.content, message: event.message, method: event.method })) }),
        );
      ...
      const deleteAction = AGENT_WEB_UI_SNIPPET_ACTIONS.find((action: any) => action.id === 'delete');
      for (const [index, verb] of deleteAction.verbs.entries()) {
        ...
        await expect.poll(() => page.evaluate((snippetName: any) => JSON.parse(window.localStorage.getItem('narada:agent-web-ui:operator-snippets.v1') ?? '[]').some((entry: any) => entry.name === snippetName), name)).toBe(false);
      }
  ...
  test('command palette keeps mobile layout and listbox semantics coherent for long snippet rows', async ({ page }: any) => {
    const runtime = await startSharedRuntime();
    try {
