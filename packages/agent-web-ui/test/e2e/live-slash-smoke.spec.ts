...
async function renderedEventRows(page: any, kind: any= null) {
  return page.evaluate((eventKind: any) => {
    const rows = Array.from(document.querySelectorAll('[data-event-kind]'));
    const filtered = eventKind ? rows.filter((row: any) => row.dataset.eventKind === eventKind) : rows;
...
async function waitForRenderedEventCount(page: any, kind: any, count: any, label: any, timeoutMs: any= 5_000) {
  await waitFor(
...
async function waitForRuntimeEvent(runtime: any, fromIndex: any, predicate: any, label: any, timeoutMs: any= 5_000) {
  return waitFor(
    () => runtime.events.slice(fromIndex).find(predicate),
    timeoutMs,
    () => ({
      label,
      recent_events: runtime.events.slice(fromIndex).map((entry: any) => ({
        event: entry.event,
...
test.describe('agent-web-ui session-core runtime slash commands', () => {
  test('projects local UI slash actions and runtime control events through Playwright', async ({ page }: any) => {
    ...
    try {
      ...
      for (const step of runtimeCases) {
        ...
        const event = await waitForRuntimeEvent(
          runtime,
          fromIndex,
          (candidate: any) => candidate.event === step.kind,
  ...
  test('projects operator input delivery through NARS acknowledgment and turn completion', async ({ page }: any) => {
    ...
    try {
      ...
      const queued = await waitForRuntimeEvent(
        runtime,
        fromIndex,
        (event: any) => event.event === 'input_event_queued' && Boolean(event.request_id),
        'operator_input_queued_timeout',
      );
      const submittedFrame = runtime.outboundFrames.find((frame: any) => frame.id === queued.request_id);
  ...
  test('keeps a durably completed turn visible when live terminal events are lost', async ({ page }: any) => {
    ...
    runtime.eventHub.publish = (event: any) => {
  ...
  test('replays terminal input outcome after the event stream reconnects', async ({ page }: any) => {
    const runtime = await startSharedRuntime({
    ...
    try {
      ...
      expect(runtime.outboundFrames.filter((frame: any) => frame.id === queued.request_id)).toHaveLength(1);
  ...
  test('keeps an unacknowledged input recoverable across reload without automatic resend', async ({ page }: any) => {
    const runtime = await startSharedRuntime({
  ...
  test('shows queued delivery while an earlier NARS turn is active', async ({ page }: any) => {
    ...
    try {
      ...
      const firstTurn = await waitForRuntimeEvent(
        runtime,
        firstFromIndex,
        (event: any) => event.event === 'carrier_turn_started',
      ...
      const secondQueued = await waitForRuntimeEvent(
        runtime,
        firstFromIndex,
        (event: any) => event.event === 'input_event_queued' && event.event_id !== firstTurn.turn_id && Boolean(event.request_id),
      ...
      expect(runtime.outboundFrames.find((frame: any) => frame.id === secondQueued.request_id)).toMatchObject({
        method: 'session.submit',
      ...
      await waitForRuntimeEvent(
        runtime,
        firstFromIndex,
        (event: any) => event.event === 'input_event_completed' && event.request_id === secondQueued.request_id,
  ...
  test('projects provider failure as failed input delivery after NARS rejection', async ({ page }: any) => {
    ...
    try {
      ...
      const failedTurn = await waitForRuntimeEvent(
        runtime,
        fromIndex,
        (event: any) => event.event === 'carrier_turn_failed' && event.turn_id === queued.event_id,
        'failed_turn_timeout',
      );
      const rejected = await waitForRuntimeEvent(
        runtime,
        fromIndex,
        (event: any) => event.event === 'session_control_rejected' && event.request_id === queued.request_id,
      ...
      expect(runtime.outboundFrames.find((frame: any) => frame.id === queued.request_id)).toMatchObject({
        method: 'session.submit',
  ...
  test('correlates missing request IDs and converges duplicate out-of-order delivery events', async ({ page }: any) => {
    const runtime = await startSharedRuntime({
  ...
  test('keeps pending input state unchanged when an event comes from another session', async ({ page }: any) => {
    const runtime = await startSharedRuntime({
