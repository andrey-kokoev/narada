...
async function withHealthServer(fn: any, options: any= {}) {
...
function publishScenarioEvents(eventHub: any, scenario: any, sessionId: any= `ux_${scenario}`) {
...
async function withScenarioServer(scenario: any, fn: any, options: any= {}) {
  if (scenario=== 'disconnected') {
    return withHealthServer(async (healthUrl: any) => {
...
async function scrollTranscriptToTop(page: any) {
  const scroller = page.locator('.events-scroll');
  await scroller.hover();
  await page.mouse.wheel(0, -100_000);
  await expect.poll(() => scroller.evaluate((element: any) => element.scrollTop), { timeout: 5_000 }).toBe(0);
}

async function runScenarioViewport({ page, scenario, viewport }: any) {
  return withScenarioServer(scenario.name, async (url: any, publishEvents: any= () => {}, inputFrames: any= [], eventHub: any= null) => {
    await page.setViewportSize({ width: viewport.width, height: viewport.height });
    await page.addInitScript(({ view }: any) => {
    ...
    try {
      ...
      if (scenario.name=== 'snippets') {
        ...
        await page.waitForFunction(() => {
          ...
          return Array.isArray(stored) && stored.some((snippet: any) => snippet.name === 'ux-drawer');
        });
...
test('agent-web-ui projection UX matrix has no obvious layout regressions', async ({ page }: any) => {
  const scenarios = [
    { name: 'normal' },
    { name: 'snippets' },
    { name: 'thinking' },
...
test('agent-web-ui User Site onboarding makes the first-use path explicit', async ({ page }: any) => {
  await withScenarioServer('normal', async (url: any, publishEvents: any) => {
    ...
    assert.equal(await page.locator('#operator-input').evaluate((element: any) => document.activeElement === element), true);
...
test('agent-web-ui transcript scroll authority preserves operator-controlled history until explicit follow', async ({ page }: any) => {
  await withScenarioServer('normal', async (url: any, _publishEvents: any, _inputFrames: any, eventHub: any) => {
    const base = { agent_id: 'ux.agent', session_id: 'ux_scroll', timestamp: new Date().toISOString(), provider: 'codex-subscription' };
    const publishAssistant = (index: any, content: any) => eventHub.publish({ ...base, event: 'assistant_message', request_id: `scroll_${index}`, content });
    await page.setViewportSize({ width: 900, height: 560 });
    await page.goto(url);
    await page.waitForLoadState('domcontentloaded');
...
test('agent-web-ui operator footer selector hides and restores target and input boxes', async ({ page }: any) => {
  await withScenarioServer('normal', async (url: any, publishEvents: any) => {
    await page.setViewportSize({ width: 900, height: 560 });
    await page.goto(url);
    await page.waitForLoadState('domcontentloaded');
    ...
    const setFooterSelectorItem = async (label: any, visible: any) => {
