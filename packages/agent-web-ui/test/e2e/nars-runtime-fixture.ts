...
async function closeServer(server: any) {
  ...
  await Promise.race([
    ...
    new Promise((resolve: any) => setTimeout(resolve, 1_000)),
  ]);
}
  ...
  try {
    ...
    const service = createSessionCoreRuntimeService({
      ...
      invokeIntelligenceFn: async (messages: any, tools: any) => {
        ...
        if (providerDelayMs > 0) await new Promise((resolve: any) => setTimeout(resolve, providerDelayMs));
    ...
    await waitFor(() => events.some((event: any) => event.event === 'session_started'), 5_000, () => ({ events: events.map((event: any) => event.event) }));
    if (startWeb) {
    ...
    return {
      ...
      sessionId,
      eventsPath: runtimeContext.eventsPath,
      setSwallowInputFrames(value: any) {
      ...
      async close() {
        ...
        runtimeInput.end();
        await Promise.race([runtimePromise, new Promise((resolve: any) => setTimeout(resolve, 1_000))]);
