...
function replaceCanonicalReference(value: any, replacements: any) {
  if (typeof value !== 'string') return value;
  return [...replacements.entries()].reduce(
    (current: any, [from, to]: any) => current.replaceAll(from, to),
    value,
  );
}

function rewriteCanonicalSeed(value: any, replacements: any, providerId: any) {
  if (typeof value === 'string') return replaceCanonicalReference(value, replacements);
  if (Array.isArray(value)) return value.map((entry: any) => rewriteCanonicalSeed(entry, replacements, providerId));
  if (!value || typeof value !== 'object') return value;

  const rewritten = Object.fromEntries(
    Object.entries(value).map(([key, entry]: any) => [key, rewriteCanonicalSeed(entry, replacements, providerId)]),
  );
...
function normalizeSeedRecord(record: any, sourceReference: any) {
...
export async function seedLiveIntelligenceRegistry(
  siteRoot: any,
  { siteId, providerId = 'kimi-code-api', endpointBaseUrl }: any,
) {
  ...
  const secondaryRecords = secondarySeed.records
    .filter((record: any) => secondaryDocumentIds.has(record.document.id))
    .map((record: any, index: any) => normalizeSeedRecord({
      ...record,
