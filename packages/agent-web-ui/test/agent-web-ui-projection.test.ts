...
function createProjectionDocument({ withNarsConfig = false }: any= {}) {
  ...
  const documentRef = {
    getElementById(id: any) { return elements.get(id) ?? null; },
...
function descendantsWithClass(node: any, className: any, matches: any= []) {
...
test('classifies input acknowledgment evidence as operations without adding it to conversation rows', () => {
  ...
  assert.deepEqual(projection.rows.map((row: any) => row.kind), ['operator_input_submitted']);
});
...
test('conversation projection shows canonical lifecycle assistant message and hides provider assistant telemetry', () => {
  ...
  const assistantRows = projection.rows.filter((row: any) => row.kind === 'assistant_message');
  ...
  assert.equal(projection.rows.some((row: any) => row.kind === 'assistant_message_stream'), false);
...
test('conversation projection keeps identical assistant text from distinct turns', () => {
  ...
  assert.deepEqual(assistantRows.map((row: any) => row.event.request_id), ['input_first', 'input_second']);
});
...
test('conversation projection collapses duplicate tool results by render identity', () => {
  ...
  const toolRows = projection.rows.filter((row: any) => row.kind === 'tool_result');
...
test('conversation projection preserves audio artifact references for browser playback', () => {
  ...
  const projection = createSessionProjection(events, { verbosity: 'conversation' });
  assert.deepEqual(projection.rows.map((row: any) => row.kind), ['assistant_message']);
...
test('DOM renderer projects audio artifact references as explicit audio controls', () => {
  ...
  const row = elements.get('events').children.find((child: any) => child?.dataset?.eventKind === 'assistant_message');
  const card = row.children[1].children[0].children.find((child: any) => child?.className === 'artifact-card');
  const audio = card.children.find((child: any) => child?.tagName === 'AUDIO');
...
test('DOM renderer keeps artifact presentation and lifecycle message while hiding provider notes', () => {
  ...
  const rows = elements.get('events').children.filter((child: any) => child?.dataset?.eventKind === 'assistant_message');
  ...
  assert.equal(elements.get('events').children.some((child: any) => child?.dataset?.eventKind === 'provider_agent_message'), false);
});
...
test('replayed user message does not duplicate operator row or reopen queued activity after assistant completion', () => {
  ...
  const operatorRows = projection.rows.filter((row: any) => row.kind === 'operator_input_submitted' || row.kind === 'user_message');
...
test('DOM renderer replaces local operator submit echo with canonical user message', () => {
  ...
  const rows = elements.get('events').children.filter((child: any) => child?.dataset?.eventKind === 'operator_input_submitted' || child?.dataset?.eventKind === 'user_message');
...
test('conversation projection keeps chat compact while activity summarizes top-level tool progress', () => {
  ...
  const projection = createSessionProjection(events, { verbosity: 'conversation', nowMs: Date.parse('2026-07-08T20:50:12.000Z') });
  assert.equal(projection.rows.some((row: any) => row.kind === 'tool_call' || row.kind === 'tool_result'), false);
  assert.equal(projection.activity.active, true);
  assert.equal(projection.activity.state, 'thinking');
  assert.match(projection.activity.label, /resident is thinking/);
