...
async function connectWebSocket(url: any) {
  ...
  return {
    sendJson(payload: any) { socket.send(JSON.stringify(payload)); },
    async nextJson() {
...
function rawCustomPropertyReferenceViolations(root: any) {
  ...
  return [...referenced].filter((token: any) => !declared.has(token)).sort();
}
...
function createFakeAgentWebUiElements() {
  class FakeElement {
    ...
    addEventListener(name: any, listener: any) { this.listeners.set(name, listener); }
    submit() { this.listeners.get('submit')?.({ preventDefault() {} }); }
    change() { this.listeners.get('change')?.({}); }
    setAttribute(name: any, value: any) { this[name] = value; }
  }
  const byId = new Map();
  ...
  const documentRef = {
    getElementById(id: any) { return byId.get(id) ?? null; },
    createElement(name: any) { return new FakeElement(name); },
    createTextNode(text: any) { return { tagName: '#TEXT', textContent: String(text ?? ''), children: [], dataset: {} }; },
  };
  return { byId, documentRef };
}

function textOfNode(node: any) {
  if (!node) return '';
...
function createFakeDocument() {
  ...
  class FakeLink {
    ...
    setAttribute(name: any, value: any) { this.attributes.set(name, String(value)); }
    removeAttribute(name: any) { this.attributes.delete(name); }
    getAttribute(name: any) { return this.attributes.get(name) ?? null; }
  }
  return {
    links,
    head: { appendChild(node: any) { links.push(node); } },
    ...
    querySelector(selector: any) {
      ...
      return links.find((link: any) => link.getAttribute('data-narada-managed-favicon') === 'true') ?? null;
    },
  };
}
...
const AGENT_WEB_UI_CSS_MODULES = AGENT_WEB_UI_CSS_IMPORTS
  .map(([modulePath]: any) => modulePath)
  .filter((modulePath: any) => modulePath.startsWith('styles/'));
...
async function readAgentWebUiCss() {
  ...
  const modules = await Promise.all([
    ...
    ...AGENT_WEB_UI_CSS_MODULES.map((modulePath: any) => readFile(new URL(modulePath, sourceRoot), 'utf8')),
  ]);
  return [entry, ...modules].join('\n');
}

async function waitFor(predicate: any, { timeoutMs = 2000 }: any= {}) {
...
async function withRealNarsWebServer(fn: any) {
  ...
  try {
    ...
    const service = createSessionCoreRuntimeService({
      ...
      intelligenceRuntime: {
        ...
        callIntelligence: async (messages: any, tools: any) => {
    ...
    await waitFor(() => events.some((event: any) => event.event === 'session_started'), { timeoutMs: 2000 });
    ...
    runtimeInput.end();
    if (runtimePromise) await Promise.race([runtimePromise, new Promise((resolve: any) => setTimeout(resolve, 1000))]);
...
function readInjectedBrowserConfig(html: any) {
...
test('default package test script remains non-browser and non-e2e', async () => {
  ...
  const testFiles = defaultTest.match(/test\/\S+\.test\.ts/g) ?? [];
  const forbidden = testFiles.filter((file: any) => (
    /-e2e\.test\.ts$/.test(file)
    || /(?:browser|ux-smoke)\.test\.ts$/.test(file)
  ));
...
test('browser preferences hydrate through qualified feature-owned storage without session state', () => {
  ...
  const storage = {
    getItem(key: any) { return values.get(key) ?? null; },
    setItem(key: any, value: any) { values.set(key, String(value)); },
  };
  const keys = Object.values(AGENT_WEB_UI_PREFERENCE_KEYS);

  assert.equal(keys.every((key: any) => key.startsWith('narada:agent-web-ui:')), true);
...
test('ordinary Agent Web UI browser UX tests stay Playwright-owned', async () => {
  ...
  const legacyRawBrowserTests = [
    'agent-web-ui-live-slash-cross-surface-e2e.test.ts',
    'agent-web-ui-panels-e2e.test.ts',
    'agent-web-ui-ux-smoke.test.ts',
  ];
...
test('Vue operator components expose composer without hidden privileged controls', async () => {
  ...
  const slashSnippetE2e = await readFile(new URL('../test/e2e/slash-snippets.spec.ts', import.meta.url), 'utf8');
...
test('agent-web-ui CSS entry imports logical modules in cascade order', async () => {
  ...
  const expectedImports = [
    ...AGENT_WEB_UI_CSS_IMPORTS.map(([modulePath, layer]: any) => layer
...
test('Vue layout smoke covers shell, status, event list, composer, and event tone styles', async () => {
  ...
  for (const marker of ['id="events"', 'id="projection-verbosity"', 'class="composer"', 'id="operator-input"']) {
    assert.equal([transcript, status, selectorComponent, composer].some((source: any) => source.includes(marker)), true, marker);
  }
...
test('agent-web-ui CSS enforces theme-token discipline for new color declarations', async () => {
  ...
  const missingDarkTokens = [...lightTokens].filter((token: any) => !darkTokens.has(token));
  assert.deepEqual(missingDarkTokens, []);
});

function rawColorDeclarationViolations(root: any) {
  const violations = [];
  root.walkDecls((declaration: any) => {
...
function rawColorTokensInRoot(root: any, { dark }: any) {
...
function isInsideDarkMedia(node: any) {
  let current = node.parent;
  while (current) {
...
function hasRawColor(value: any) {
  return /#[0-9a-fA-F]{3,8}|rgba?\(/.test(value);
}

function locationLabel(node: any) {
...
test('package server injects operator-capable config and proxies health with GET only', async () => {
  const upstream = createServer((request: any, response: any) => {
  ...
  try {
    const index = await fetch(web.url).then((response: any) => response.text());
    ...
    const health = await fetch(new URL('/api/health', web.url)).then((response: any) => response.json());
    assert.equal(health.status, 'healthy');
    assert.equal(health.session_id, 'carrier_test');
    const artifact = await fetch(new URL('/api/nars/sessions/carrier_test/artifacts/art_html', web.url)).then((response: any) => response.json());
...
test('package server serves browser-loadable modules without workspace bare imports', async () => {
  ...
  try {
    ...
    const appModule = await fetch(new URL('/agent-web-ui.js', root)).then((response: any) => response.text());
...
test('package server serves an explicit artifact directory before compatibility fallback', async () => {
  ...
  try {
    const html = await fetch(web.url).then((response: any) => response.text());
    assert.match(html, /data-test="built-artifact"/);
    assert.match(html, /id="nars-config"/);
    assert.doesNotMatch(html, /id="events"/);
  } finally {
    await new Promise((resolve: any) => web.server.close(resolve));
    rmSync(artifactRoot, { recursive: true, force: true });
  }
});

test('served web UI config attaches to live NARS health and event projections', async () => {
  await withRealNarsWebServer(async ({ web, eventProjection, healthProjection, events, providerCalls }: any) => {
    ...
    try {
      ...
      const fullHealth = await fetch(new URL('/api/health?detail=full', web.url)).then((response: any) => response.json());
      ...
      assert.equal(providerCalls[0].messages.some((message: any) => message.role === 'user' && /run startup sequence/.test(message.content)), true);
      await waitFor(() => events.some((event: any) => event.event === 'assistant_message' && event.content === 'Real NARS fixture response.'), { timeoutMs: 2000 });
    } finally {
      client.close();
    }
  });
...
test('local projection control refuses browser-supplied session authority and starts from server context', async () => {
  ...
  const web = await startAgentWebUiServer({
    ...
    startCloudflareProjection: async (input: any) => {
      captured = input;
      return {
