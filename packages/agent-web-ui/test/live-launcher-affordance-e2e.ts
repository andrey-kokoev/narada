...
async function startJsonlMcpServer({ entrypoint, siteRoot, siteId, timeoutMs }: any) {
  ...
  child.on('error', (error: any) => rejectPending(error));
  child.on('exit', (code: any, signal: any) => {
    if (!closing) rejectPending(new Error(`nars_session_mcp_exited:${code ?? 'null'}:${signal ?? 'null'}:${stderr.slice(0, 2000)}`));
  });

  const request = (method: any, params: any= {}) => new Promise((resolvePromise: any, rejectPromise: any) => {
  ...
  const close = async () => {
    ...
    if (child.exitCode=== null && child.signalCode === null) {
      ...
      await Promise.race([
        once(child, 'exit'),
        new Promise((resolvePromise: any) => setTimeout(resolvePromise, 3000)),
      ]);
    }
  ...
  try {
    ...
    const toolNames = new Set((toolList.result?.tools ?? []).map((tool: any) => tool.name));
    ...
    return {
      async callTool(name: any, argumentsValue: any) {
...
async function startFixtureProvider({ responseDelayMs = 0, responseDelayForRequest = null }: any= {}) {
  const requests = [];
  const server = createServer(async (request: any, response: any) => {
    ...
    const requestDelayMs = typeof responseDelayForRequest === 'function'
      ...
      : responseDelayMs;
    if (requestDelayMs > 0) await new Promise((resolvePromise: any) => setTimeout(resolvePromise, requestDelayMs));
...
async function listenHttpServer(server: any) {
  await new Promise((resolvePromise: any, rejectPromise: any) => {
    server.once('error', rejectPromise);
    server.listen(0, '127.0.0.1', () => {
      server.off('error', rejectPromise);
...
async function closeHttpServer(server: any) {
...
try {
  ...
  const startupEvent = await waitFor(() => readJsonlFile(record.events_path).find((event: any) => event.event === 'session_started'), {
    timeoutMs,
...
function parseArgs(args: any) {
  ...
  for (let index= 0; index < args.length; index += 1) {
    ...
    const key = arg.slice(2).replace(/-([a-z])/g, (_match: any, char: any) => char.toUpperCase());
...
function siteRuntimeRoot(root: any) {
  return basename(root).toLowerCase() === '.narada' ? root : join(root, '.narada');
}

function sessionRoots(root: any) {
  return [
...
async function waitForSessionRecord({ siteRoot, agentId, timeoutMs, runtimeProcess, runtimeOutput }: any) {
  try {
...
function findLatestSessionRecord(siteRoot: any, agentId: any) {
  ...
  for (const root of sessionRoots(siteRoot)) {
    ...
    const entries = readdirSync(root, { withFileTypes: true })
      ...
      .sort((left: any, right: any) => right.mtimeMs - left.mtimeMs)
      .slice(0, 128);
...
async function waitForHealthy(endpoint: any, timeoutMs: any) {
...
function findHeadlessBrowser() {
  ...
  ].find((path: any) => existsSync(path)) ?? null;
}

async function openCdpPage({ browserPath, url, workDir }: any) {
  ...
  const browserWsUrl = await new Promise((resolvePromise: any, reject: any) => {
    let stderr = '';
  ...
  const pages = await fetch(`http://${browserUrl.host}/json/list`).then((response: any) => response.json());
  const page = pages.find((entry: any) => entry.type === 'page') ?? pages[0];
  ...
  const send = (method: any, params: any= {}) => new Promise((resolvePromise: any, reject: any) => {
    const nextId = ++id;
  ...
  await new Promise((resolvePromise: any) => setTimeout(resolvePromise, 900));
  return {
    ...
    async fill(selector: any, value: any) {
    ...
    async selectOption(selector: any, value: any) {
      ...
      await this.click(selector);
      const pressKey = async ({ key, code, windowsVirtualKeyCode }: any) => {
    ...
    async clickText(containerSelector: any, text: any, { textSelector = '*', clickSelector = null }: any= {}) {
      const point = await this.evaluate(`(() => {
    ...
    async clickInputForText(containerSelector: any, text: any, { textSelector = '*' }: any= {}) {
    ...
    async scrollToTop(selector: any) {
    ...
    async waitForExpression(expression: any, timeoutMs: any= 10_000) {
      return waitFor(async () => this.evaluate(expression), { timeoutMs, label: 'cdp_expression' });
    },
    async close() {
      try { await send('Browser.close'); } catch {}
      try { ws.close(); } catch {}
      await new Promise((resolvePromise: any) => {
...
function collectProcessOutput(child: any) {
  let stdout = '';
  let stderr = '';
  child.stdout?.setEncoding('utf8');
  child.stderr?.setEncoding('utf8');
...
async function waitForTextMatch(readText: any, regex: any, { timeoutMs = 5000, label = 'text_match' }: any= {}) {
  let match = null;
  await waitFor(() => {
...
async function waitFor(check: any, { timeoutMs, label }: any) {
...
async function requestSessionHealth(eventEndpoint: any, requestId: any, timeoutMs: any) {
  ...
  try {
    return await new Promise((resolvePromise: any, rejectPromise: any) => {
      ...
      const onMessage = (message: any) => {
        ...
        const candidates = [parsed, parsed?.payload].filter((value: any) => value && typeof value === 'object');
        const health = candidates.find((value: any) => value.event === 'session_health' && value.request_id === requestId);
        if (!health) return;
        clearTimeout(timer);
...
async function closeNarsSession(eventEndpoint: any) {
  const socket = new WebSocket(eventEndpoint);
  await once(socket, 'open');
...
async function runIntelligenceRegistryScenario({ record, page, provider, timeoutMs, intelligenceFixture }: any) {
...
async function runExternalInputProjectionScenario({
  ...
  for (const content of inputs) {
    assert.ok(provider.requests.some((request: any) => JSON.stringify(request).includes(content)), `provider did not receive ${content}`);
  }
  ...
  assert.equal(chatRows.filter((row: any) => row.kind === 'user_message' && row.text.includes(inputs[0])).length, 1);
  assert.equal(chatRows.filter((row: any) => row.kind === 'user_message' && row.text.includes(inputs[1])).length, 1);
  assert.equal(chatRows.filter((row: any) => row.kind === 'assistant_message' && row.text.includes(response)).length, 3);
  assert.ok(chatRows.some((row: any) => row.kind === 'user_message'), `Chat must show conversation input rows: ${JSON.stringify(chatRows)}`);
  assert.ok(chatRows.some((row: any) => row.kind === 'assistant_message'), `Chat must show assistant conversation rows: ${JSON.stringify(chatRows)}`);
  assert.ok(
    chatRows.every((row: any) => row.disposition === 'conversation_fact'),
    `Chat must exclude operations, diagnostics, and raw records: ${JSON.stringify(chatRows)}`,
  );
  assert.ok(
    !chatRows.some((row: any) => ['session_started', 'turn_started', 'turn_complete', 'tool_call', 'tool_result'].includes(row.kind)),
  ...
  assert.equal(operationRows.filter((row: any) => row.kind === 'user_message' && row.text.includes(inputs[0])).length, 1);
  assert.equal(operationRows.filter((row: any) => row.kind === 'user_message' && row.text.includes(inputs[1])).length, 1);
  assert.equal(operationRows.filter((row: any) => row.kind === 'assistant_message' && row.text.includes(response)).length, 3);
  assert.ok(operationRows.some((row: any) => row.disposition === 'operation_fact'), `Operations must show operation rows: ${JSON.stringify(operationRows)}`);
  assert.ok(operationRows.some((row: any) => row.disposition === 'conversation_fact'), `Operations must retain conversation context: ${JSON.stringify(operationRows)}`);
  assert.ok(
    operationRows.every((row: any) => ['conversation_fact', 'operation_fact'].includes(row.disposition)),
...
async function runParallelSurfaceScenario({
  ...
  const latestSequence = () => Math.max(
    0,
    ...readJsonlFile(record.events_path).map((event: any) => Number(event.event_sequence ?? event.sequence ?? 0)),
  );
  const waitForTurn = async (content: any, baselineSequence: any, label: any) => waitFor(() => {
    const events = readJsonlFile(record.events_path);
    const userMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > baselineSequence
      && event.event === 'user_message'
      && event.content === content);
    const assistantMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > baselineSequence
  ...
  for (const content of [browserInput, agentCliInput]) {
    const userMessage = persistedEvents.find((event: any) => event.event === 'user_message' && event.content === content);
    ...
    const assistantMessage = persistedEvents.find((event: any) => event.event === 'assistant_message'
      && event.content === response
  ...
  assert.ok(
    diagnosticRows.some((row: any) => row.kind === 'session_health' && row.disposition === 'diagnostic_signal'),
    'Diagnostics must show the live session health event: ' + JSON.stringify(diagnosticRows),
  );
  assert.ok(
    !diagnosticRows.some((row: any) => ['user_message', 'assistant_message', 'tool_call', 'tool_result'].includes(row.kind)),
    'Diagnostics must suppress conversation and tool-operation rows: ' + JSON.stringify(diagnosticRows),
  );
}

async function runReplayReconnectScenario({ record, page, narsSessionMcp, provider, timeoutMs }: any) {
  ...
  assert.equal(rows.filter((row: any) => row.kind === 'user_message' && row.text.includes(replayInput)).length, 1);
  assert.equal(rows.filter((row: any) => row.kind === 'assistant_message' && row.text.includes(replayResponse)).length, 1);
  assert.ok(rows.every((row: any) => row.disposition === 'conversation_fact'), JSON.stringify(rows));
  ...
  assert.equal(rows.filter((row: any) => row.kind === 'user_message' && row.text.includes(replayInput)).length, 1, JSON.stringify(rows));
  assert.equal(rows.filter((row: any) => row.kind === 'assistant_message' && row.text.includes(replayResponse)).length, 1, JSON.stringify(rows));
  assert.ok(rows.every((row: any) => row.disposition === 'conversation_fact'), JSON.stringify(rows));
  assert.ok(!rows.some((row: any) => ['session_started', 'turn_started', 'turn_complete', 'tool_call', 'tool_result'].includes(row.kind)), JSON.stringify(rows));
  await selectProjectionView({ page, view: 'operations', timeoutMs });
  await page.waitForExpression('document.querySelector("[data-event-kind=\\"session_started\\"]") !== null', timeoutMs);
  const operationRows = await readConversationProjectionRows(page);
  assert.ok(operationRows.some((row: any) => row.disposition === 'operation_fact'), JSON.stringify(operationRows));
  assert.ok(operationRows.some((row: any) => row.kind === 'user_message' && row.text.includes(replayInput)), JSON.stringify(operationRows));
  assert.ok(operationRows.every((row: any) => ['conversation_fact', 'operation_fact'].includes(row.disposition)), JSON.stringify(operationRows));
  ...
  assert.ok(diagnosticRows.some((row: any) => ['session_events_subscription_started', 'websocket_connected', 'session_health'].includes(row.kind)), JSON.stringify(diagnosticRows));
  assert.ok(!diagnosticRows.some((row: any) => ['user_message', 'assistant_message', 'tool_call', 'tool_result'].includes(row.kind)), JSON.stringify(diagnosticRows));
  await selectProjectionView({ page, view: 'conversation', timeoutMs });
  ...
  assert.ok(initialPayloads.some((event: any) => event?.event === 'user_message' && event.content === replayInput), JSON.stringify(initialReplay));
  assert.ok(initialPayloads.some((event: any) => event?.event === 'assistant_message' && event.content === replayResponse), JSON.stringify(initialReplay));
  assert.ok(initialSequences.every((sequence: any) => Number.isFinite(sequence)), JSON.stringify(initialReplay));
  ...
  assert.ok(reconnectPayloads.some((event: any) => event?.event === 'user_message' && event.content === reconnectInput), JSON.stringify(reconnectReplay));
  assert.ok(reconnectPayloads.some((event: any) => event?.event === 'assistant_message' && event.content === reconnectResponse), JSON.stringify(reconnectReplay));
  assert.ok(reconnectPayloads.every((event: any) => event?.content !== replayInput && event?.content !== replayResponse), JSON.stringify(reconnectReplay));
  assert.ok(reconnectSequences.every((sequence: any) => Number.isFinite(sequence) && sequence > reconnectCursor), JSON.stringify(reconnectReplay));
  ...
  rows = await readConversationProjectionRows(page);
  assert.equal(rows.filter((row: any) => row.kind === 'user_message' && row.text.includes(reconnectInput)).length, 1, JSON.stringify(rows));
  assert.equal(rows.filter((row: any) => row.kind === 'assistant_message' && row.text.includes(reconnectResponse)).length, 1, JSON.stringify(rows));

  const scrollInputs = Array.from({ length: 8 }, (_value: any, index: any) => 'long-session input ' + (index + 1) + ' ' + Date.now());
  ...
  const followBaseline = Math.max(0, ...readJsonlFile(record.events_path).map((event: any) => Number(event.event_sequence ?? event.sequence ?? 0)));
  ...
  await waitFor(() => {
    const events = readJsonlFile(record.events_path);
    const userMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > followBaseline
      && event.event === 'user_message'
      && event.content === followInput);
    const assistantMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > followBaseline
      && event.event === 'assistant_message'
...
async function readConversationProjectionRows(page: any) {
...
async function openJsonSocket(endpoint: any) {
...
async function readSessionReplay(endpoint: any, { sinceSequence = null, maxReplay = 100, timeoutMs }: any) {
...
function unwrapSessionReplayMessage(message: any) {
...
function sequenceFromSessionReplayMessage(message: any) {
...
async function publishCloudflareProjectionFromBox({
  ...
  assert.ok(
    cloudflareResponses.some((entry: any) => entry.url.includes('/api/nars/projections/register') && entry.status < 400),
  ...
  await waitFor(() => cloudflareResponses.some((entry: any) => (
  ...
  const bridgeAck = await waitFor(() => cloudflareResponses.find((entry: any) => (
    entry.url.includes(`/api/nars/projections/${encodeURIComponent(publication.projection_id)}/input/`)
  ...
  await waitFor(() => {
    const events = readJsonlFile(record.events_path);
    remoteUserMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > baselineSequence
      && event.event === 'user_message'
      && event.content === remoteContent);
    remoteAssistantMessage = events.find((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > baselineSequence
      && event.event === 'assistant_message'
  ...
  await waitFor(() => provider.requests.some((request: any) => JSON.stringify(request).includes(remoteContent)), {
    timeoutMs,
  ...
  const submitProjectionInput = async (method: any, payload: any= {}) => {
  ...
  const waitForProjectionAck = (inputId: any, label: any) => waitFor(() => cloudflareResponses.find((entry: any) => (
    entry.url.includes(`/api/nars/projections/${encodeURIComponent(publication.projection_id)}/input/`)
    && entry.url.endsWith('/ack')
    && entry.status < 400
    && String(entry.body ?? '').includes('"status":"acknowledged"')
  ...
  await waitFor(() => {
    const events = readJsonlFile(record.events_path);
    return events.some((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > steerBaselineSequence
      && event.event === 'user_message'
      && event.content === steerContent)
      && events.some((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > steerBaselineSequence
        && event.event === 'assistant_message'
        && event.content === 'Live launcher fixture response');
  }, { timeoutMs, label: 'cloudflare_direct_steer_nars_response' });
  assert.ok(provider.requests.some((request: any) => JSON.stringify(request).includes(steerContent)));
  ...
  await waitFor(() => provider.requests.some((request: any) => JSON.stringify(request).includes(interruptContent)), {
    timeoutMs,
  ...
  await waitFor(() => {
    const events = readJsonlFile(record.events_path);
    return events.some((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > interruptBaselineSequence
      && event.event === 'interrupt_requested')
      && events.some((event: any) => Number(event.event_sequence ?? event.sequence ?? 0) > interruptBaselineSequence
  ...
  assert.ok(
    cloudflareResponses.some((entry: any) => entry.url.includes('/events') && entry.body.includes(remoteContent)),
    `Cloudflare Worker must serve the remote input event: ${JSON.stringify(cloudflareResponses)}`,
  );
  assert.ok(
    cloudflareResponses.some((entry: any) => entry.url.includes('/events') && entry.body.includes(remoteResponse)),
    `Cloudflare Worker must serve the remote assistant response event: ${JSON.stringify(cloudflareResponses)}`,
  );
  assert.ok(
    cloudflareResponses.some((entry: any) => entry.url.includes(`/api/nars/projections/${encodeURIComponent(publication.projection_id)}/events`)),
    `Cloudflare Worker must serve published session events: ${JSON.stringify(cloudflareResponses)}`,
  );
}

async function selectProjectionView({ page, view, timeoutMs }: any) {
...
async function submitExternalSessionInput({ record, narsSessionMcp, content, requestId, response, timeoutMs }: any) {
  const baselineSequence = Math.max(0, ...readJsonlFile(record.events_path).map((event: any) => Number(event.event_sequence ?? event.sequence ?? 0)));
...
async function stopProcess(child: any) {
  if (!child) return;
...
async function stopProcessByPid(pid: any) {
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    await new Promise((resolvePromise: any) => setTimeout(resolvePromise, 100));
  }
  try {
...
async function removeEphemeralSiteRoot(path: any) {
  let lastError = null;
