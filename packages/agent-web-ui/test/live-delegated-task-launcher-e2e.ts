...
const runtimeServerPath = join(NARADA_ROOT, 'packages', 'agent-runtime-server', 'bin', 'narada-agent-runtime-server.ts');
...
try {
  const missing = [cliPath, runtimeServerPath, delegatedTaskServerPath, narsSessionServerPath].find((path: any) => !existsSync(path));
  if (missing) {
    status = 'not_run';
    ...
    const startupEvent = await waitFor(() => readJsonl(sessionRecord.events_path).find((event: any) => event.event === 'session_started'), 'carrier_start_event');
    ...
    assert.equal(listedSessions.some((entry: any) => asRecord(entry).session_id === sessionRecord.session_id), true, JSON.stringify(listed));
    ...
    await waitFor(() => {
      ...
      return events.some((event: any) => event.event === 'carrier_tool_completed' && String(event.tool_name ?? event.name ?? '') === 'delegated_task_run')
        && events.some((event: any) => event.event === 'carrier_turn_completed');
    ...
    assert.equal(carrierEvents.some((event: any) => event.event === 'carrier_tool_requested' && String(event.tool_name ?? event.name ?? '') === 'delegated_task_run'), true, JSON.stringify(carrierEvents));
    assert.equal(carrierEvents.some((event: any) => event.event === 'carrier_tool_completed' && String(event.tool_name ?? event.name ?? '') === 'delegated_task_run'), true, JSON.stringify(carrierEvents));
    assert.equal(carrierEvents.some((event: any) => event.event === 'mcp_startup_status' && event.mcp_operational_state !== 'healthy'), false, JSON.stringify(carrierEvents));
    ...
    const workerRunDirs = workerRefs
      .map((ref: any) => typeof ref.run_dir === 'string' ? ref.run_dir : null)
      .filter((runDir: any) => runDir !== null);
    ...
    assert.equal(provider.requests.some((request: any) => JSON.stringify(request.messages ?? '').includes('L5_WORKER_MARKER')), true, 'worker carrier request was not observed');
    status = 'passed';
...
async function startProviderFixture() {
  ...
  const server = createServer(async (request: any, response: any) => {
    ...
    const hasToolResult = messages.some((message: any) => message && typeof message === 'object' && message.role === 'tool');
  ...
  await new Promise((resolvePromise: any, reject: any) => {
...
function createJsonlClient(child: any, responseTimeoutMs: any) {
  let buffer = '';
  ...
  child.on('close', (code: any) => { if (code !== 0) rejectAll(new Error(`nars-session MCP exited with code ${code}`)); });
  return {
    request(id: any, method: any, params: any) {
    ...
    async close() {
      ...
      await Promise.race([once(child, 'close'), new Promise((resolvePromise: any) => setTimeout(resolvePromise, 3000))]);
    },
  };
}

function findSessionRecord(root: any, identity: any) {
...
function collectOutput(child: any) {
  let output = '';
  child.stdout.on('data', (chunk: any) => { output = `${output}${String(chunk)}`.slice(-8000); });
  child.stderr.on('data', (chunk: any) => { output = `${output}${String(chunk)}`.slice(-8000); });
  return { all: () => output };
}

async function waitFor(check: any, label: any) {
...
function readJsonl(path: any) {
  if (!existsSync(path)) return [];
  return readFileSync(path, 'utf8').split(/\r?\n/).filter(Boolean).map((line: any) => JSON.parse(line));
}

function boundedDirectories(path: any, limit: any) {
  if (!existsSync(path)) return [];
  return readdirSync(path, { withFileTypes: true }).filter((entry: any) => entry.isDirectory()).slice(0, limit).map((entry: any) => join(path, entry.name));
}
...
function readRequestBody(request: any) {
  return new Promise((resolvePromise: any, reject: any) => {
    const chunks = [];
    request.on('data', (chunk: any) => chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk))));
    request.on('end', () => resolvePromise(Buffer.concat(chunks).toString('utf8')));
    request.on('error', reject);
  });
}

function closeServer(server: any) { return new Promise((resolvePromise: any) => server.close(() => resolvePromise())); }
function tomlPath(path: any) { return path.replaceAll('\\', '/').replaceAll('"', '\\"'); }
function asRecord(value: any) { return value && typeof value === 'object' && !Array.isArray(value) ? value : {}; }
function removeRoot(path: any) { try { rmSync(path, { recursive: true, force: true }); return true; } catch { return false; } }
...
async function stopProcess(child: any) {
  ...
  child.kill();
  await Promise.race([once(child, 'exit'), new Promise((resolvePromise: any) => setTimeout(resolvePromise, 3000))]);
