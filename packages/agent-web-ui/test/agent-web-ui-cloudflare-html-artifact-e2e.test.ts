...
async function createRealNarsSiteWithHtmlArtifact() {
  ...
  try {
    ...
    async function registerArtifact(sourcePath: any, kind: any, title: any, contentType: any= undefined) {
...
async function callLiveSpeechMcp({ siteRoot, outputPath, text }: any) {
...
test('[fixture-boundary] local Worker emulation renders an explicitly admitted local NARS HTML artifact', async () => {
  ...
  try {
    ...
    const iframeNetwork = await page.waitForNetworkResponse(
      (entry: any) => String(entry.url ?? '').includes(`/api/nars/projections/proj_html_artifact_e2e/artifacts/${encodeURIComponent(artifactId)}/content`),
      5000,
    );
    ...
    const servedIframe = servedResponses.find((entry: any) => entry.url.includes(`/api/nars/projections/proj_html_artifact_e2e/artifacts/${encodeURIComponent(artifactId)}/content`));
