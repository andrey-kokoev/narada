...
type AgentWebUiOptions = {
  ...
  authorityTransition?: UnknownRecord | null;
  siteId?: string | null;
...
type ProjectionStartInput = {
  ...
  inputPolicy?: string[];
  cachePolicy?: unknown;
  artifactPolicy?: UnknownRecord;
};

type ServerDependencies = {
  startCloudflareProjection?: (input: ProjectionStartInput) => Promise<UnknownRecord>;
};
...
export function parseAgentWebUiArgs(args: readonly string[] = []): AgentWebUiOptions & { help?: boolean } {
  const options: AgentWebUiOptions & { help?: boolean } = { host: '127.0.0.1', port: 0, eventEndpoint: null, healthEndpoint: null, onboarding: false };
...
function normalizePublicBasePath(value: unknown): string | null {
...
function publicPath(basePath: string | null | undefined, suffix: string): string {
...
async function readJsonBody(request: IncomingMessage): Promise<UnknownRecord> {
  const chunks: Buffer[] = [];
  let size = 0;
...
function assertNoProjectionAuthorityOverride(body: UnknownRecord): void {
...
function buildRemoteWebUrl(args: { projectionId: string; cloudflareApiBaseUrl: string; browserTokenFingerprint?: string | null }): string {
...
async function defaultStartCloudflareProjection(args: ProjectionStartInput): Promise<UnknownRecord> {
  ...
  } as Parameters<typeof registerProjectionRemotely>[0]);
  const registrationRecord = registration as unknown as UnknownRecord;
  if (registrationRecord.status !== 'registered_remotely') {
    return {
      schema: 'narada.agent_web_ui.cloudflare_projection_start.v1',
      status: 'refused',
      reason: registrationRecord.status,
      projection_id: registrationRecord.projection_id ?? args.projectionId ?? null,
      registration: registrationRecord,
    };
  }
  const projectionId = String(registrationRecord.projection_id ?? args.projectionId ?? '');
  const remoteAccess = isRecord(registrationRecord.remote_access) ? registrationRecord.remote_access : {};
  const tokens = Array.isArray(remoteAccess.browser_access_tokens) ? remoteAccess.browser_access_tokens : [];
  const firstToken = isRecord(tokens[0]) ? tokens[0] : {};
  const browserTokenFingerprint = typeof firstToken.token_fingerprint === 'string' ? firstToken.token_fingerprint : null;
  ...
  return {
    ...
    cloudflare_api_base_url: args.cloudflareApiBaseUrl,
    registration_status: registrationRecord.status,
    bridge_once_status: (bridgeOnce as unknown as UnknownRecord).status,
    bridge_run_status: (bridgeRun as unknown as UnknownRecord).status,
    bridge_state: (bridgeOnce as unknown as UnknownRecord).bridge_state ?? null,
    bridge_process: bridgeRun as unknown as UnknownRecord,
    state_path: registrationRecord.bridge_state_path ?? null,
  };
}

function buildProjectionStartInput(options: AgentWebUiOptions, body: UnknownRecord): ProjectionStartInput {
  ...
  return {
    ...
    projectionId: typeof (body.projection_id ?? body.projectionId) === 'string' ? String(body.projection_id ?? body.projectionId) : undefined,
...
function buildProjectionStatus(options: AgentWebUiOptions, projectionId: string | null): UnknownRecord {
...
function hasLocalProjectionAuthority(options: AgentWebUiOptions): boolean {
  return Boolean(options.sessionId && options.siteRoot && options.siteId);
}

function onboardingConfig(options: AgentWebUiOptions): UnknownRecord {
...
export function buildClientConfig(options: AgentWebUiOptions): ClientConfig {
  ...
  const localPublicPath = (suffix: string): string => publicPath(publicBasePath, suffix);
  return {
...
function rewritePublicBaseAssetPaths(content: string, publicBasePath: string): string {
...
async function tryReadStaticRoot(root: URL, relativePath: string): Promise<{ content: string } | null> {
...
function resolveDistRoot(artifactRoot: unknown): URL {
...
async function readStaticFile(pathname: string, clientConfig: ClientConfig, distRoot: URL): Promise<{ content: string; contentType: string } | null> {
  ...
  let sourceFile = await tryReadStaticRoot(STATIC_ROOT, sourceRelativePath);
  let sourceIsTranspiledTypeScript = false;
  if (!sourceFile && sourceRelativePath.endsWith('.js')) {
    sourceFile = await tryReadStaticRoot(STATIC_ROOT, `${sourceRelativePath.slice(0, -3)}.ts`);
    sourceIsTranspiledTypeScript = Boolean(sourceFile);
  }
  ...
  } else if (sourceIsTranspiledTypeScript) {
    content = transpileBrowserModule(content);
...
export function createAgentWebUiServer(options: AgentWebUiOptions, deps: ServerDependencies = {}) {
  ...
  const server = createServer(async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (url.pathname.startsWith('/api/nars/')) {
      ...
      try {
        ...
        const artifactId = upstream.headers.get('x-narada-artifact-id');
        const artifactKind = upstream.headers.get('x-narada-artifact-kind');
        if (contentSecurityPolicy) headers['content-security-policy'] = contentSecurityPolicy;
        if (artifactId) headers['x-narada-artifact-id'] = artifactId;
        if (artifactKind) headers['x-narada-artifact-kind'] = artifactKind;
