...
export function useOperatorCommandPalette(options: OperatorCommandPaletteOptions) {
  ...
  watch(commandResults, (commands: any, previousCommands: any) => {
    const commandIds = commands.map((entry: any) => entry.id).join('\n');
    const previousCommandIds = previousCommands?.map((entry: any) => entry.id).join('\n');
  ...
  function exactTypedCommandEntry(): OperatorCommandPaletteEntry | undefined {
    ...
    return commandResults.value.find((entry: any) => entry.kind === 'command' && (entry.command.slash === token || entry.command.aliases.includes(token as `/${string}`)));
  }

  function acceptPaletteEntry(entry: OperatorCommandPaletteEntry, submitWhenComplete: any= false) {
