...
for (const mode of ['exit', 'timeout', 'malformed']) {
  test(`real MCP transport reports ${mode} failure after bounded retry`, async () => {
    ...
    const fixture = fileURLToPath(new URL('../dist/fixtures/mcp-failure-fixture.js', import.meta.url));
    ...
    const gateway = createNarsCapabilityGateway({ siteRoot, ownershipContext: { request_timeout_ms: 40 }, recordEvidence: (event: AnyRecord) => { evidence.push(event); } });
    try {
      await gateway.start();
      const result = await gateway.invoke({ toolName: 'fixture_tool', arguments: {} });
      assert.equal(result.status, 'failed');
      assert.ok(evidence.some((event: AnyRecord) => event.kind === 'tool_execution_failed'));
      assert.deepEqual(
        evidence.filter((event: AnyRecord) => event.kind === 'tool_execution_state_transition').map((event) => event.execution_state),
