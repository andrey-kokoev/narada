...
export type AgentIdentityScopeV2 =
  | { kind: 'narada_site'; site_id: string }
  | { kind: 'unscoped' };
...
type OperatorRenderOptions = { mode?: 'inline' | 'block'; limit?: number; depth?: number };
type IdentityInputSource = { status: 'ok'; value: unknown };

export const AGENT_IDENTITY_REF_SCHEMA = 'narada.agent_identity_ref.v1' as const;
export const AGENT_IDENTITY_REF_V2_SCHEMA = 'narada.agent_identity_ref.v2' as const;

function isRecord(value: unknown): value is IdentityRecord {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export function buildAgentIdentityRef(
  identityValue: unknown,
  roleValue: unknown = null,
  explicitSiteId: unknown = null,
): AgentIdentityRef {
...
export function buildAgentIdentityRefV2(input: unknown = {}): AgentIdentityRefV2 {
  if (!isRecord(input)) throw new TypeError('agent_identity_ref_v2_input_required');
...
export function resolveAgentIdentityRef(input: unknown, context: IdentityResolutionContext = {}): AgentIdentityResolution {
  ...
  if (isRecord(value) && value.schema === AGENT_IDENTITY_REF_V2_SCHEMA) {
    try {
      return resolvedIdentityResolution(buildAgentIdentityRefV2(value), [{ kind: 'copied_current_shape', field: 'agent_identity_ref' }]);
    } catch (error) {
      return refusedIdentityResolution(
        'invalid_current_shape',
        error instanceof Error ? error.message : 'Invalid current identity shape',
      );
    }
  }
...
export function agentIdentityDisplay(identityRef: unknown, fallback: unknown = null): string | null {
  if (isRecord(identityRef)) {
...
export function normalizeAgentIdentityRef(value: unknown): AgentIdentityRef | null {
...
export function normalizeAgentIdentityRefV2(value: unknown, context: IdentityRecord = {}): AgentIdentityRefV2 | null {
  const resolved = resolveAgentIdentityRef(value, context);
  return resolved.status === 'resolved' ? resolved.value : null;
}

export function agentIdentityRefMatchesRequest(identityRef: unknown, requestedAgentId: unknown): boolean {
...
export function agentIdentityGroupKey(
  identityRef: unknown,
  fallbackAgentId: unknown = null,
  fallbackSiteId: unknown = null,
): string {
  const ref = normalizeAgentIdentityRef(identityRef)
...
export function renderOperatorValue(value: unknown, options: OperatorRenderOptions = {}): string {
  ...
  if (Array.isArray(value)) {
    const rendered: string[] = value.map((entry: unknown) => renderOperatorValue(entry, { ...options, depth: depth + 1 })).filter(Boolean);
...
export function renderOperatorObjectSummary(value: unknown, options: OperatorRenderOptions = {}): string {
  ...
  const pushEntry = (key: string, rawValue: unknown): void => {
...
export function roleSegment(agentId: unknown): string | null {
...
export function siteSegment(agentId: unknown): string | null {
  const value = normalizeOptionalString(agentId);
  if (!value || !value.includes('.')) return null;
  return value.split('.')[0] ?? null;
}

export function normalizeSiteToken(value: unknown): string {
...
function normalizeIdentityScope(value: unknown): AgentIdentityScopeV2 {
  if (!isRecord(value)) throw new TypeError('identity_scope_required');
...
function extractIdentityInput(input: unknown, fallbackAgentId: string | null): IdentityInputSource | AgentIdentityResolution {
...
function provenanceForObject(value: IdentityRecord, usedContextSite: boolean): AgentIdentityResolutionProvenance[] {
  const provenance: AgentIdentityResolutionProvenance[] = [{ kind: 'legacy_object_consumed', field: value.schema === AGENT_IDENTITY_REF_SCHEMA ? 'agent_identity_ref.v1' : 'agent_identity_ref' }];
  if (usedContextSite) provenance.push({ kind: 'context_consumed', field: 'site_id' });
  if (value.source_agent_id || value.agent_id) provenance.push({
    kind: 'legacy_scalar_consumed',
    field: value.source_agent_id ? 'source_agent_id' : 'agent_id',
    value: normalizeOptionalString(value.source_agent_id ?? value.agent_id) ?? undefined,
  });
  return provenance;
}

function resolvedIdentityResolution(
  value: AgentIdentityRefV2,
  provenance: AgentIdentityResolutionProvenance[],
): Extract<AgentIdentityResolution, { status: 'resolved' }> {
  return { status: 'resolved', value, provenance };
}

function refusedIdentityResolution(
  code: string,
  message: string,
): Extract<AgentIdentityResolution, { status: 'refused' }> {
...
function renderOperatorJson(value: unknown, mode: 'inline' | 'block', limit: number): string {
  try {
