...
async function run() {
    ...
    try {
        ...
        const passed = serviceHealth.body?.status === 'degraded'
...
function phase(message) {
    if (!args.quiet && args.format !== 'json')
        process.stderr.write(`[cloudflare-nars-projection:boundary] ${message}\n`);
}
function parseArgs(values) {
    ...
    for (let index = 0; index < values.length; index += 1) {
        ...
        if (value === '--live')
            ...
            parsed.format = values[++index] ?? 'human';
        else if (value === '--cloudflare-api-base-url')
            parsed.cloudflareApiBaseUrl = values[++index] ?? null;
        else if (value === '--evidence-path')
            parsed.evidencePath = values[++index] ?? null;
        else if (value === '--session-id')
            parsed.sessionId = values[++index] ?? null;
        else if (value === '--site-id')
            parsed.siteId = values[++index] ?? null;
        else if (value === '--agent-id')
            parsed.agentId = values[++index] ?? null;
        else
            throw new Error(`unknown_option:${value}`);
    }
    return parsed;
}
//# sourceMappingURL=cloudflare-nars-authority-live-smoke.js.map