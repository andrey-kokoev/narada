...
import { deliverRemoteProjectionInputsOnce, preflightCloudflareProjectionRegistration, registerProjectionRemotely, startLocalProjectionBridgeOnce, } from '../dist/node.js';
import { findHeadlessBrowser, openCdpPage, waitForPageText, waitForPageTextOccurrence, waitForPageTextWithAction, } from './lib/browser-smoke.js';
...
async function run() {
    ...
    if (artifactId)
        artifactContent = await getJson(`${base}/artifacts/${encodeURIComponent(artifactId)}/content`, { 'x-narada-browser-token-fingerprint': browserToken });
...
async function verifyHostedBrowserProjection(args) {
    ...
    if (!browserPath)
        return { status: 'failed', code: 'headless_browser_not_found' };
...
async function waitForRemoteProjectionCacheText(args) {
    ...
    while (Date.now() - started < args.timeoutMs) {
        ...
        if (text.includes(args.text))
            return { found: true, waited_ms: Date.now() - started, since_sequence: args.sinceSequence ?? 0, event_count: last.event_count ?? last.events?.length ?? null };
...
async function submitLiveLocalNarsInput(args) {
    ...
    if (!session?.event_endpoint)
        throw new Error('local_nars_event_endpoint_not_found');
...
async function waitForLocalEventLogMessage(args) {
    ...
    if (!eventsPath)
        return { found: false, reason: 'local_nars_events_path_not_found' };
    const started = Date.now();
    while (Date.now() - started < args.timeoutMs) {
        const text = existsSync(eventsPath) ? readFileSync(eventsPath, 'utf8') : '';
        const found = text.includes(args.message);
        if (found)
            return { found: true, events_path: eventsPath, waited_ms: Date.now() - started };
        await new Promise((resolve) => setTimeout(resolve, 250));
    }
...
function compareAssetManifests(expected, deployed) {
    ...
    for (const field of ['schema', 'target', 'source_hash', 'asset_tree_hash']) {
        if (expected?.[field] !== deployed?.[field])
            mismatches.push({ field, expected: expected?.[field] ?? null, deployed: deployed?.[field] ?? null });
    }
    ...
    for (const scope of ['console', 'sessions']) {
        ...
        for (const field of ['source_hash', 'recipe_hash', 'output_tree_hash']) {
            if (expectedArtifact[field] !== deployedArtifact[field])
                mismatches.push({ field: `source_artifacts.${scope}.${field}`, expected: expectedArtifact[field] ?? null, deployed: deployedArtifact[field] ?? null });
        }
    }
    return {
...
function strongestHostedWebUiEvidence({ hostedShell, hostedBrowser, assetFingerprint }) {
    if (assetFingerprint?.status === 'passed' && hostedBrowser?.status === 'passed')
        return 'browser_level_deployed_artifact_fingerprint_verified';
    if (hostedBrowser?.status === 'passed')
        return 'browser_level_local_origin_projection_e2e';
    if (hostedShell?.ok === true)
        return 'http_html_shell_only';
    return 'none';
}
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--live')
            options.live = true;
        else if (arg === '--require-operator-session')
            options.requireOperatorSession = true;
        else if (arg.startsWith('--')) {
            const equalsIndex = arg.indexOf('=');
            if (equalsIndex > 0)
                options[optionKey(arg.slice(0, equalsIndex))] = arg.slice(equalsIndex + 1);
            else
                options[optionKey(arg)] = argv[index + 1], index += 1;
        }
    }
    return options;
}
//# sourceMappingURL=cloudflare-nars-web-projection-live-smoke.js.map