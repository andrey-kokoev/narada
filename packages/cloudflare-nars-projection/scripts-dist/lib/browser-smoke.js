...
export async function openCdpPage({ browserPath, url, userDataPrefix = 'narada-browser-smoke-', viewport = { width: 1100, height: 800 } }) {
    ...
    ws.addEventListener('message', (message) => {
        ...
        if (payload.method === 'Network.responseReceived') {
            ...
            for (const waiter of [...networkWaiters]) {
                if (networkResponses.length - 1 < waiter.afterCount)
                    continue;
    ...
    return {
        ...
        return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
            ...
            if (!point)
                throw new Error(`cdp_click_target_not_found:${selector}`);
        ...
        return Array.from(select.options).findIndex((option) => option.value === ${JSON.stringify(String(value))});
            ...
            if (optionIndex < 0)
                throw new Error(`cdp_select_option_not_found:${selector}:${value}`);
            await this.click(selector);
            ...
            if (selectedValue !== String(value))
                throw new Error(`cdp_select_option_failed:${selector}:${value}:${selectedValue}`);
        ...
        return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
            ...
            if (!point)
                throw new Error(`cdp_click_text_target_not_found:${containerSelector}:${text}`);
            await send('Input.dispatchMouseEvent', { type: 'mousePressed', button: 'left', clickCount: 1, ...point });
            await send('Input.dispatchMouseEvent', { type: 'mouseReleased', button: 'left', clickCount: 1, ...point });
        },
        async evaluateInFrameByUrl(urlPart, expression) {
            ...
            if (!frame)
                throw new Error(`frame_not_found:${urlPart}:${JSON.stringify(frames.map((candidate) => candidate.url ?? ''))}`);
            const contextId = executionContextsByFrame.get(frame.id);
            if (!contextId)
                throw new Error(`frame_execution_context_not_found:${urlPart}`);
        ...
        async waitForNetworkResponse(predicate, timeoutMs, afterCount = 0) {
            ...
            if (existing)
                return { found: true, ...existing, waited_ms: 0 };
        ...
        async getNetworkResponseBody(requestId) {
            ...
            try {
                ...
                return { body, base64_encoded: Boolean(result?.base64Encoded) };
            }
        },
        async reload() {
...
export async function waitForPageText(page, text, timeoutMs) {
    ...
    while (Date.now() - started < timeoutMs) {
        ...
        if (result)
            return { found: true, waited_ms: Date.now() - started };
        await sleep(250);
    }
...
export async function waitForPageTextOccurrence(page, text, minimumCount, timeoutMs) {
    ...
    while (Date.now() - started < timeoutMs) {
        ...
        if (count >= minimumCount)
            return { found: true, count, minimum_count: minimumCount, waited_ms: Date.now() - started };
        await sleep(250);
    }
    const bodyText = await page.evaluate('document.body?.innerText?.slice(0, 1000) ?? ""').catch(() => '');
...
//# sourceMappingURL=browser-smoke.js.map