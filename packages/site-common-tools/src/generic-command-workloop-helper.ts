import { genericCommandSchemaMetadata } from './generic-command-registry.js';

const LOCAL_SITE: any = 'local_site';

const INTENTS: any = Object.freeze({
  'task.claim': {
...
export function genericWorkloopMutationIntents() : any{
  return Object.entries(INTENTS).map(([intent, spec]: any) => ({
    intent,
    ...clone(spec),
...
export function buildGenericWorkloopMutation(intent: any, options: any = {}) : any{
  const spec: any = INTENTS[intent];
  if (!spec) {
  ...
  const targetSiteRoot: any = stringOrNull(options.target_site_root ?? options.targetSiteRoot);
  if (!targetSiteRoot) {
  ...
  const authorityBasis: any = asObject(options.authority_basis ?? options.authorityBasis);
  ...
  const domainArgs: any = {
    ...asObject(options.domain_args ?? options.domainArgs),
  };
  const payload: any = maybeSchemaPayload(spec, options.payload);
  const payloadRef: any = stringOrNull(options.payload_ref ?? options.payloadRef);
  ...
  if (Array.isArray(spec.required_any_domain_args) && !spec.required_any_domain_args.some((field: any) => domainArgs[field] !== undefined && domainArgs[field] !== null && domainArgs[field] !== '')) {
  ...
  const createdBy: any = stringOrNull(options.created_by ?? options.createdBy);
  const payloadId: any = stringOrNull(options.payload_id ?? options.payloadId);
  const commandId: any = stringOrNull(options.command_id ?? options.commandId);
...
export function summarizeGenericCommandSubmission(result: any = {}) : any{
  const status: any = stringOrNull(result.status) ?? 'unknown';
  return {
...
export function buildGenericCommandAdoptionEvidence(input: any = {}) : any{
  const commandSchema: any = stringOrNull(input.command_schema ?? input.commandSchema);
  const equivalentCommandSchema: any = stringOrNull(input.equivalent_command_schema ?? input.equivalentCommandSchema);
  const registeredSchema: any = commandSchema ?? equivalentCommandSchema;
  const registryEntry: any = registeredSchema ? genericCommandSchemaMetadata(registeredSchema) : null;
  const surfaceKind: any = stringOrNull(input.surface_kind ?? input.surfaceKind);
  const fallbackReason: any = stringOrNull(input.fallback_reason ?? input.fallbackReason);
  const payloadRef: any = stringOrNull(input.payload_ref ?? input.payloadRef);
  const commandRef: any = stringOrNull(input.command_ref ?? input.commandRef);
  const resultRef: any = stringOrNull(input.result_ref ?? input.resultRef);
...
function expectedSelectors(spec: any, domainArgs: any) : any{
...
function adoptionEvidence(input: any, fields: any) : any{
  return {
...
function maybeSchemaPayload(spec: any, payload: any) : any{
  if (payload === undefined || payload === null) return null;
  const object: any = asObject(payload);
...
function narrowFallback(spec: any) : any{
  const registryEntry: any = genericCommandSchemaMetadata(spec.command_schema);
  const facade: any = registryEntry?.compatibility_facades?.[0] ?? spec.generic_command_unavailable?.narrow_fallback_tool ?? null;
  return facade
    ? {
        status: 'available',
...
function refusal(reasonCode: any, fields: any = {}) : any{
  return {
    schema: 'narada.generic_command.workloop_helper.v1',
    status: 'refused',
