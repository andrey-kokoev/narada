...
} from '../site-locus-shim.js';

export const MCP_RUNTIME_INSTANCE_REGISTRY_SCHEMA: any = 'narada.pc_runtime.mcp_runtime_instance_registry.v0';
export const MCP_RUNTIME_INSTANCE_STATUS_SCHEMA: any = 'narada.pc_runtime.mcp_runtime_instance_status.v0';
export const MCP_SURFACE_CARRIER_SUPERVISOR_STATUS_SCHEMA: any = 'narada.mcp_surface.carrier_supervisor.status.v0';
...
export function buildMcpRuntimeRegistryStatus({
  ...
  const root: any = resolve(siteRoot ?? process.cwd());
  const pcRoot: any = resolve(pcSiteRoot ?? 'C:/ProgramData/Narada/sites/pc/desktop-sunroom-2');
  const declarationPath: any = join(siteControlRoot(root), 'capabilities', 'mcp-surfaces.json');
  const registryPath: any = join(pcRoot, 'runtime', 'mcp-runtime-instances.json');
  const declarations: any = readJson(declarationPath);
  const runtimeRegistry: any = readOptionalJson(registryPath);
  const declarationSiteId: any = declarations?.site_id ?? null;
  const surfaces: any = Array.isArray(declarations?.surfaces) ? declarations.surfaces : [];
  const knownSurfaces: any = surfaces.map((surface: any) => observeDeclaredSurface({
    surface,
    declarationSiteId,
    siteRoot: root,
    runtimeRegistry,
  ...
  if (target) {
    const resolved: any = resolveRuntimeTarget(target, knownSurfaces);
    if (!resolved) {
  ...
  return {
    ...
    pending_restart_requests: knownSurfaces.flatMap((surface: any) => surface.restart_request?.state === 'restart_requested' ? [surface.restart_request] : []),
    notes: registryNotes({ runtimeRegistry, knownSurfaces }),
  };
}

export function resolveRuntimeTarget(target: any, knownSurfaces: any) : any{
  const needle: any = String(target ?? '').trim();
  if (!needle) return null;
  return knownSurfaces.find((surface: any) => surface.surface_id === needle || surface.server_name === needle) ?? null;
}
...
export function coordinateMcpRuntimeRestartRequest({
  ...
  const registryStatus: any = buildMcpRuntimeRegistryStatus({ siteRoot, pcSiteRoot, target, now, maxEvidenceAgeMs });
  ...
  const knownSurfaces: any = registryStatus.surface ? [registryStatus.surface] : (registryStatus.known_surfaces ?? []);
  const affected: any = knownSurfaces.filter((surface: any) => isAffectedByRestartRequest(surface, staleEpoch));
  const requestedAt: any = now instanceof Date ? now.toISOString() : new Date(now).toISOString();
  const entries: any = affected.map((surface: any) => coordinateSurfaceRestart({
    surface,
    dryRun,
    mutatingAuthorized,
    requestedAt,
  ...
  const counts: any = countCoordinationStatuses(entries);
  return {
    schema: 'narada.pc_runtime.mcp_restart_coordination.v0',
    status: 'completed',
    ...
    counts,
    entries,
    terminal_evidence: entries.map((entry: any) => entry.terminal_evidence),
    registry_status: {
...
function observeDeclaredSurface({ surface, declarationSiteId, siteRoot, runtimeRegistry, pcRoot, registryPath, now, maxEvidenceAgeMs }: any) : any{
  const instance: any = findRuntimeInstance(runtimeRegistry, surface.surface_id);
  const transport: any = surface.runtime_binding?.transport ?? {};
  const transportType: any = transport.type ?? null;
  const runtimeKind: any = surface.runtime_binding?.runtime_kind ?? null;
  const serverName: any = deriveServerName(surface);
  const evidencePath: any = instance?.freshness_evidence_path ?? surface.evidence?.path ?? null;
  const restartRequest: any = instance?.restart_request && typeof instance.restart_request === 'object'
    ? instance.restart_request
    : readRestartRequestForSurface(surface, siteRoot);
  const lastObservedAt: any = instance?.last_observed_at ?? null;
  const evidenceAge: any = ageEvidence(lastObservedAt, now);
  const staleEvidence: any = !lastObservedAt || evidenceAge > maxEvidenceAgeMs;
  const sourceFreshness: any = normalizeSourceFreshness({ instance, surface, staleEvidence, restartRequest });
  const supervisorOwned: any = instance?.supervisor_owned === true;
  const reloadSupport: any = normalizeReloadSupport(instance?.reload_support, transportType, supervisorOwned);
  const ownership: any = classifyOwnership({ transportType, runtimeKind, supervisorOwned, reloadSupport });
  const registryAuthority: any = buildSurfaceRegistryAuthority({ surface, declarationSiteId, registryPath });
  const entrypoint: any = surface.runtime_binding?.entrypoint ?? transport.args?.[0] ?? null;
  const runtimeAuthorityConformance: any = buildRuntimeAuthorityConformance({
    instance,
    registryAuthority,
    siteRoot,
    staleEvidence,
  });
  const carrierStatus: any = buildCarrierSupervisorStatus({
    surface,
    instance,
    serverName,
    entrypoint,
  ...
  const carrierSessionBinding: any = classifyCarrierSessionBinding({
    instance,
    restartSemantics: ownership.restart_semantics,
    surface,
  });
  const startupDisposition: any = classifyStartupDisposition({
    carrierStatus,
  ...
  const criticalNonconformance: any = buildCriticalRestartNonconformance({
    carrierStatus,
...
function buildSiteRegistryAuthority({ declarationSiteId, declarationPath, registryPath }: any) : any{
  return {
...
function buildSurfaceRegistryAuthority({ surface, declarationSiteId, registryPath }: any) : any{
  const owningSite: any = surface.runtime_binding?.owner_site_id ?? declarationSiteId ?? null;
  const owningSiteLocus: any = owningSite ?? null;
  const declarationSiteLocus: any = declarationSiteId ?? null;
  const localAuthority: any = Boolean(declarationSiteLocus) && owningSiteLocus === declarationSiteLocus;
  return {
...
function buildCarrierSupervisorStatus({
  ...
  const processEvidence: any = instance?.process_identity_evidence ?? {};
  const runtime: any = {
  ...
  const restartState: any = normalizeRestartRequestState(restartRequest);
  const verification: any = normalizeVerification(instance?.verification, restartState, staleEvidence);
  const observedState: any = classifyObservedState({ restartState, staleEvidence, verification, runtime });
  const missingCarrierAction: any = classifyMissingCarrierAction({ restartState, ownership, verification });
  const carrierSessionBinding: any = classifyCarrierSessionBinding({
    instance,
    restartSemantics: ownership.restart_semantics,
    surface,
  });
  const selfRestartSupported: any = reloadSupport.carrier_reload_supported === true || reloadSupport.reconnect_supported === true;
...
function buildRuntimeAuthorityConformance({ instance, registryAuthority, siteRoot, staleEvidence }: any) : any{
  const processEvidence: any = instance?.process_identity_evidence ?? {};
  const evidenceSiteRoot: any = processEvidence.site_root ?? null;
  const carrierSessionOwner: any = instance?.carrier_session_owner ?? processEvidence.agent_id ?? null;
  const expectedSiteIds: any = expectedAuthoritySiteIds(registryAuthority);
  const siteRootStatus: any = classifyEvidenceSiteRoot({
    evidenceSiteRoot,
    expectedSiteRoot: siteRoot,
  });
  const carrierOwnerStatus: any = classifyCarrierSessionOwner({
    carrierSessionOwner,
    expectedSiteIds,
  });
  ...
  const status: any = mismatches.length === 0
...
function expectedAuthoritySiteIds(registryAuthority: any) : any{
  return [
  ...
  ].filter((value: any, index: any, values: any) => typeof value === 'string' && value.length > 0 && values.indexOf(value) === index);
}

function classifyEvidenceSiteRoot({ evidenceSiteRoot, expectedSiteRoot }: any) : any{
  if (!evidenceSiteRoot) {
    return {
      status: 'missing',
  ...
  const normalizedEvidence: any = normalizePathForComparison(evidenceSiteRoot);
  const normalizedExpected: any = normalizePathForComparison(expectedSiteRoot);
  return {
...
function classifyCarrierSessionOwner({ carrierSessionOwner, expectedSiteIds }: any) : any{
  ...
  const declaredOwnerSite: any = carrierSessionOwner.split('.')[0];
  const matches: any = expectedSiteIds.some((siteId: any) => carrierSessionOwner === siteId || carrierSessionOwner.startsWith(`${siteId}.`));
  return {
...
function normalizePathForComparison(path: any) : any{
  return resolve(path).replace(/\\/g, '/').replace(/\/+$/g, '').toLowerCase();
}

function classifyStartupDisposition({ carrierStatus, carrierSessionBinding = null, restartSemantics, supervisorOwned = false, reloadSupport = {} }: any) : any{
  const pendingRestart: any = carrierStatus?.restart_request?.state === 'restart_requested'
    || carrierStatus?.source_freshness?.pending_restart === true;
  const liveVerified: any = carrierStatus?.verification?.live_surface_verified === true;
  const carrierRestarted: any = carrierStatus?.restart_request?.state === 'carrier_restarted';
  ...
  const restartHandle: any = carrierSessionBinding?.record_summary?.restart_handle;
...
function buildCriticalRestartNonconformance({ carrierStatus, carrierSessionOwner, carrierSessionBinding = null, restartSemantics, startupDisposition = null }: any) : any{
  const pendingRestart: any = carrierStatus?.restart_request?.state === 'restart_requested'
    || carrierStatus?.source_freshness?.pending_restart === true;
  ...
  const carrierUnknown: any = !carrierSessionOwner || carrierSessionOwner === 'carrier_session_unknown';
  const missingCarrierSession: any = carrierSessionBinding?.status === 'terminal_missing_embodiment_authority'
    || carrierSessionBinding?.status === 'legacy_unbound';
  const restartUnproven: any = carrierStatus?.self_restart_supported !== true
...
function isNoRequestFreshnessMarkerContradicted({ carrierStatus, carrierSessionBinding = null }: any) : any{
...
function isRestartRequestMarkerContradictedByLiveBoot({ carrierStatus, carrierSessionBinding = null }: any) : any{
  ...
  const requestedAt: any = Date.parse(firstString(
  ...
  const bootedAt: any = Date.parse(firstString(
...
function hasParentCarrierSessionEvidence(carrierSessionBinding: any) : any{
...
function classifyCarrierSessionBinding({ instance, restartSemantics, surface }: any) : any{
  const inheritedId: any = instance?.carrier_session_id ?? instance?.process_identity_evidence?.carrier_session_id ?? null;
  const parentRef: any = instance?.parent_carrier_session_ref ?? instance?.process_identity_evidence?.parent_carrier_session_ref ?? null;
  const recordedBinding: any = instance?.carrier_session_binding && typeof instance.carrier_session_binding === 'object'
    ? instance.carrier_session_binding
    : null;
  const criticalSurface: any = restartSemantics === 'carrier_session_restart_required'
  ...
  const status: any = criticalSurface ? 'terminal_missing_embodiment_authority' : 'legacy_unbound';
  return {
    schema: 'narada.pc_runtime.mcp_child_carrier_session_binding.v0',
    status,
...
function buildCarrierSessionMigrationGuidance({ status, criticalSurface }: any) : any{
  return {
...
function normalizeRestartRequestState(restartRequest: any) : any{
  if (restartRequest?.state) return restartRequest.state;
  return 'no_restart_request';
}

function normalizeVerification(value: any, restartState: any, staleEvidence: any) : any{
  const freshOneShotVerified: any = value?.fresh_one_shot_verified;
  const liveSurfaceVerified: any = value?.live_surface_verified;
  return {
...
function classifyObservedState({ restartState, staleEvidence, verification, runtime }: any) : any{
  ...
  const sourceChanged: any = runtime.source_digest_changed
...
function classifyMissingCarrierAction({ restartState, ownership, verification }: any) : any{
...
function buildSupervisorRefusals({ transportType, runtimeKind, ownership, restartState }: any) : any{
  const refusals: any = ['direct_native_shell_path_refused'];
...
function normalizeSourceFreshness({ instance, surface, staleEvidence, restartRequest }: any) : any{
  const source: any = instance?.source ?? {};
  const baseline: any = instance?.source_baseline ?? instance?.baseline ?? source.baseline ?? {};
  const currentMaxMtime: any = firstFiniteNumber(
  ...
  const baselineMtime: any = firstFiniteNumber(
    instance?.baseline_mtime,
    baseline.baseline_mtime,
    baseline.source_epoch,
    source.baseline_mtime,
  );
  const currentDigest: any = firstString(
    instance?.source_digest,
    source.source_digest,
    instance?.source_freshness?.source_digest,
  );
  const baselineDigest: any = firstString(
    instance?.baseline_source_digest,
    baseline.source_digest,
    instance?.source_freshness?.baseline_source_digest,
  );
  const sourceDigestChanged: any = sourceDigestChangedSinceBaseline({ instance, currentDigest, baselineDigest });
  const sourceNewer: any = sourceNewerThanBaseline(instance, currentMaxMtime, baselineMtime);
  const restartState: any = normalizeRestartRequestState(restartRequest);
  const authoritativeSourceChanged: any = sourceDigestChanged ?? sourceNewer;
  return {
...
function sourceDigestChangedSinceBaseline({ instance, currentDigest = null, baselineDigest = null }: any) : any{
...
function sourceNewerThanBaseline(instance: any, currentMaxMtime: any = null, baselineMtime: any = null) : any{
...
function firstString(...values: any[]): any {
  return values.find((value: any) => typeof value === 'string' && value.length > 0) ?? null;
}

function isAffectedByRestartRequest(surface: any, staleEpoch: any) : any{
  ...
  const epoch: any = surface.source_freshness?.source_epoch;
  const threshold: any = firstFiniteNumber(staleEpoch);
  return Number.isFinite(epoch) && Number.isFinite(threshold) && epoch >= threshold;
}

function coordinateSurfaceRestart({ surface, dryRun, mutatingAuthorized, requestedAt, requestedBy, staleEpoch }: any) : any{
...
function countCoordinationStatuses(entries: any) : any{
  const counts: any = {
    restarted: 0,
    skipped: 0,
    failed: 0,
...
function buildHostFreshnessProjection(knownSurfaces: any) : any{
  const counts: any = { fresh: 0, stale: 0, missing: 0, pending_restart: 0, restart_capable: 0, runtime_authority_mismatch: 0 };
  const instances: any = knownSurfaces.map((surface: any) => {
...
function firstFiniteNumber(...values: any[]): any {
  for (const value of values) {
    const number: any = typeof value === 'number' ? value : Number(value);
    if (Number.isFinite(number)) return number;
  }
  return null;
}

function refusedTarget({ target, declarationPath, registryPath, knownSurfaces }: any) : any{
  const looksLikeCommand: any = /[\\/\s]|\.ts$|\.ps1$|\.exe$/i.test(String(target));
  return {
    schema: MCP_RUNTIME_INSTANCE_STATUS_SCHEMA,
    status: 'refused',
    ...
    declaration_path: declarationPath,
    registry_path: registryPath,
    known_surface_ids: knownSurfaces.map((surface: any) => surface.surface_id),
  };
}

function findRuntimeInstance(runtimeRegistry: any, surfaceId: any) : any{
  const instances: any = Array.isArray(runtimeRegistry?.instances) ? runtimeRegistry.instances : [];
  return instances.find((entry: any) => entry?.surface_id === surfaceId) ?? null;
}

function deriveServerName(surface: any) : any{
  if (surface.server_name) return surface.server_name;
  const generatedPath: any = surface.client_config?.generated_path;
  if (generatedPath) return generatedPath.split(/[\\/]/).pop()?.replace(/\.json$/i, '') ?? surface.surface_id;
  return surface.surface_id;
}

function normalizeReloadSupport(value: any, transportType: any, supervisorOwned: any) : any{
...
function classifyOwnership({ transportType, runtimeKind, supervisorOwned, reloadSupport }: any) : any{
...
function readRestartRequestForSurface(surface: any, siteRoot: any) : any{
  const surfaceId: any = surface.surface_id ?? '';
  ...
  const requests: any = candidates.map((path: any) => ({ path, payload: readOptionalJson(join(siteRoot, path)) })).filter((entry: any) => entry.payload);
  if (requests.length === 0) return { state: 'no_restart_request', paths_checked: candidates };
  const latest: any = requests[requests.length - 1];
  return {
...
function registryNotes({ runtimeRegistry, knownSurfaces }: any) : any{
  const notes: any = [];
  ...
  if (knownSurfaces.some((surface: any) => surface.restart_semantics === 'carrier_session_restart_required')) {
  ...
  if (knownSurfaces.some((surface: any) => surface.observation_freshness !== 'fresh')) {
    notes.push('One or more surfaces have stale or missing PC runtime observation evidence.');
  }
  if (knownSurfaces.some((surface: any) => surface.runtime_authority_conformance?.guard?.startup_readiness_blocking === true)) {
...
function ageEvidence(lastObservedAt: any, now: any) : any{
  if (!lastObservedAt) return null;
  const observed: any = Date.parse(lastObservedAt);
  const current: any = now instanceof Date ? now.getTime() : Date.parse(now);
...
function readOptionalJson(path: any) : any{
  const resolved: any = resolve(path);
