...
const dbDir: any = resolve(siteRoot, '.ai', 'db');
if (!existsSync(dbDir)) mkdirSync(dbDir, { recursive: true });

const dbPath: any = resolve(dbDir, 'operator-surface.db');
...
const identitiesPath: any = resolve(siteRoot, 'operator-surfaces', 'identities.json');
...
const insertLog: any = db.prepare(`
...
for (const [siteId, siteData] of Object.entries(sites) as Array<[string, any]>) {
  insertSite.run(siteId, siteData.affinity_color || '000000', now);
}

const roles: any = data.roles || {};
for (const [role, roleData] of Object.entries(roles) as Array<[string, any]>) {
  insertRole.run(role, roleData.label || role, roleData.affinity_color || '000000', now);
}

const identities: any = data.identities || [];
...
for (const id of identities) {
  const admittedAt: any = id.admitted_at || data.updated_at || now;
  const updatedAt: any = id.updated_at || admittedAt;
  const admittedBy: any = id.admitted_by || 'operator';
...
// Verify
const count: any = db.prepare('SELECT COUNT(*) as c FROM operator_surface_identities').get();
