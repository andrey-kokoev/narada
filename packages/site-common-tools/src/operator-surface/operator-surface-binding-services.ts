...
export function bindingRecommendedAction(status: any) : any{
...
export function bindingDiagnosisEvaluation(counts: any) : any{
...
export function classifyBindingLiveness(liveness: any) : any{
  const drift: any = [...(liveness.guard_drift ?? [])];
  const hardFailures: any = [...(liveness.hard_failures ?? [])];
  const currentClass: any = liveness.current?.window_class ?? null;
  const softTerminalDrift: any = currentClass === 'CASCADIA_HOSTING_WINDOW_CLASS'
    && drift.length > 0
    && drift.every((reason: any) => reason === 'pid_mismatch' || reason === 'title_mismatch');
...
export function assembleBindingDiagnosis({ rows, livenessBySurfaceId, dbPath }: any) : any{
  const bindings: any = rows.map((row: any) => {
    const fallbackLiveness: any = {
      live: false,
      window_live: false,
    ...
    const liveness: any = row.hwnd ? (livenessBySurfaceId.get(row.surface_id) ?? fallbackLiveness) : fallbackLiveness;
    const classification: any = classifyBindingLiveness(liveness);
    return {
  ...
  const counts: any = Object.fromEntries(['healthy', 'evidence_drift', 'guard_drift', 'dead'].map((status: any) => [
    status,
    bindings.filter((binding: any) => binding.status === status).length,
  ]));
  const volatileEvidenceDrift: any = bindings.filter((binding: any) => (binding.volatile_evidence_drift ?? []).length > 0).length;
  return {
...
export function assembleRuntimeBindingProjection({
  ...
  const projected: any = rows.map((row: any) => {
    const evidence: any = evidenceBySurfaceId.get(row.surface_id) ?? {
      hwnd: row.hwnd,
      pid: row.pid,
