export function levenshteinDistance(a: any, b: any) : any{
  const m: any = a.length;
  const n: any = b.length;
  if (m === 0) return n;
  if (n === 0) return m;

  const prev: any = new Array(n + 1);
  const curr: any = new Array(n + 1);
...
export function normalizeTitle(title: any) : any{
  return String(title ?? '').toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
}

export function classifyEnvelope(envelope: any) : any{
  const title: any = normalizeTitle(envelope.payload?.title ?? envelope.title ?? '');
  const summary: any = normalizeTitle(envelope.payload?.summary ?? '');
  const text: any = `${title} ${summary}`;
  const kind: any = envelope.kind ?? 'unknown';
  const recommendation: any = String(envelope.payload?.recommendation ?? '').toLowerCase();
  const hasCapaRequest: any = envelope.payload?.capa_request && typeof envelope.payload.capa_request === 'object';

  const categories: any = [];
  const recurrenceEvidencePattern: any = /\b(recurrence|recurring|regression|incident|failure|failed|broken|blocked|missing|risk|error|violation|drift|gap|cannot|can't|stale)\b/;
  ...
  for (const [category, pattern] of Object.entries(keywordMap) as Array<[string, RegExp]>) {
...
export function determineTargetRole(envelope: any, categories: any) : any{
  const explicit: any = envelope.payload?.target_role ?? envelope.target_role ?? null;
...
export function determineAction(envelope: any, categories: any, recommendation: any, ageHours: any, duplicateInfo: any) : any{
  if (duplicateInfo.isDuplicate) return 'acknowledge_duplicate';

  const kind: any = envelope.kind ?? 'unknown';
  const title: any = normalizeTitle(envelope.payload?.title ?? envelope.title ?? '');

  const statusReportPatterns: any = /\bbuilder session complete\b|\bbuilder idle\b|\binbox backlog check\b|\bworkboard check\b|\bno tasks available\b|\bno claimable\b|\bchecking for materializable\b/;
  ...
  if (kind === 'observation') {
    const hasProposals: any = Array.isArray(envelope.payload?.proposal) && envelope.payload.proposal.length > 0;
    if (hasProposals) {
...
export function evaluateEnvelopeSeverity(envelope: any) : any{
  if (envelope.target_role) {
    const explicitSeverity: any = envelope.severity ?? 50;
    return {
  ...
  const kind: any = envelope.kind ?? 'observation';
  const authority: any = envelope.authority?.level ?? 'agent_reported';
  const payload: any = envelope.payload ?? {};
  const recommendation: any = String(payload.recommendation ?? '');
  ...
  if (payload.capa_request && typeof payload.capa_request === 'object') {
    const severity: any = authority === 'operator_confirmed' || authority === 'operator_directed' ? 75 : 60;
    return {
      severity,
...
export function matchNormalizedTitles(a: any, b: any, options: any = {}) : any{
  const absoluteThreshold: any = options.absoluteThreshold ?? 10;
  const normalizedThreshold: any = options.normalizedThreshold ?? 0.25;
  const distance: any = levenshteinDistance(a, b);
  const maxLen: any = Math.max(a.length, b.length);
  const normalized: any = maxLen > 0 ? distance / maxLen : 0;
...
export function findDuplicateInTitleIndex(titleIndex: any, title: any) : any{
  ...
  for (const existing of titleIndex) {
    const result: any = matchNormalizedTitles(normTitle, existing.normTitle, {
      absoluteThreshold: 5,
...
export function findDuplicateTaskRows(taskRows: any, envelope: any) : any{
  const envelopeId: any = envelope.envelope_id;
  const title: any = String(envelope.payload?.title ?? envelope.title ?? '').trim();
  ...
  if (normTitle.length > 0) {
    for (const row of taskRows) {
      const taskTitle: any = String(row.title ?? '').trim();
      const normTaskTitle: any = normalizeTitle(taskTitle);
      if (normTaskTitle.length === 0) continue;
      const result: any = matchNormalizedTitles(normTitle, normTaskTitle);
      if (result.matched) {
        return {
          isDuplicate: true,
          duplicateTaskId: row.task_id,
...
export function hasEnvelopeCoverageEvidence(row: any, envelopeId: any) : any{
  if (!envelopeId || String(envelopeId).length < 8) return false;

  const context: any = String(row?.context_markdown ?? '');
  const envelopePattern: any = escapeRegExp(envelopeId);
  const hasCanonicalEnvelopeMarker: any = new RegExp(
    `(^|\\n)\\s*\\*\\*Envelope ID:\\*\\*\\s*${envelopePattern}(\\s|$)`,
    'i',
  ).test(context);
  const hasLegacyCreatedFromMarker: any = new RegExp(
...
export function hasPreservedResidualEvidence(row: any, envelopeId: any) : any{
  const fields: any = [
