 ...
 *
 * Usage:
 *   node tools/inbox/batch-triage.ts <site-root> --action acknowledge|archive|materialize \
 *     [--filter-kind <kind>] [--older-than <hours>] [--limit <n>] [--dry-run]
 *
 * Examples:
 *   node tools/inbox/batch-triage.ts . --action acknowledge --filter-kind observation --older-than 48 --dry-run
 *   node tools/inbox/batch-triage.ts . --action archive --older-than 168 --limit 50 --dry-run
 *   node tools/inbox/batch-triage.ts . --action materialize --filter-kind capa --dry-run
 */
...
function normalizeTitle(title: any) : any{
  return String(title ?? '').toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
}

function levenshteinDistance(a: any, b: any) : any{
...
function classifyForAction(envelope: any) : any{
  ...
  for (const [cat, pattern] of Object.entries(keywordMap) as Array<[string, RegExp]>) {
...
function determineAction(envelope: any, categories: any, recommendation: any, ageHours: any) : any{
  const kind: any = envelope.kind ?? 'unknown';
  const title: any = normalizeTitle(envelope.payload?.title ?? envelope.title ?? '');
...
async function main() : Promise<any>{
  ...
  if (!args.action) {
    console.error('Usage: node tools/inbox/batch-triage.ts <site-root> --action <acknowledge|archive|materialize> [options]');
    process.exit(1);
  }

  const inboxDir: any = join(args.siteRoot, '.ai', 'inbox-envelopes');
  const archiveDir: any = join(args.siteRoot, '.ai', 'inbox-archive');
  ...
  for (const envelope of envelopes) {
    const { categories, recommendation, title }: any = classifyForAction(envelope);
    const ageHours: any = envelope.received_at ? Math.round((now - new Date(envelope.received_at).getTime()) / 3600000) : -1;
    const action: any = determineAction(envelope, categories, recommendation, ageHours);
  ...
  candidates.sort((a: any, b: any) => a.ageHours - b.ageHours);
  const selected: any = candidates.slice(0, args.limit);
  ...
  for (const item of selected) {
    const envelope: any = item.envelope;
    const filePath: any = join(inboxDir, envelope.file);

    try {
      if (args.action === 'acknowledge') {
        // Update envelope status
        const updated: any = { ...envelope, status: 'acknowledged', acknowledgement: { acknowledged_at: new Date().toISOString(), acknowledged_by: 'batch-triage' } };
        ...
        const destPath: any = join(archiveDir, envelope.file);
        ...
        try {
          ...
          const dup: any = checkDuplicateTask(
            { db: { prepare: () => ({ all: () => [] }) } }, // lightweight store for duplicate check
            envelope
          );

          // Actually materialize
          const result: any = await materializeEnvelopeAsTask(args.siteRoot, envelope);
