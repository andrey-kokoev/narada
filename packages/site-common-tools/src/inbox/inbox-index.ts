...
const repoRoot: any = resolve(__dirname, '..', '..');
...
export function readInboxIndexCounts(siteRoot: any, { evaluateEnvelopeSeverity }: any = {}) : any{
  ...
  try {
    ...
    const count: any = (where: any, params: any = {}) => index.db.prepare(`SELECT COUNT(*) AS count FROM inbox_envelopes ${where}`).get(params).count;
    return {
      ...index,
      counts: {
