...
async function withSite(fn: any) : Promise<any>{
  const siteRoot: any = await mkdtemp(join(tmpdir(), 'narada-task-lifecycle-resolution-'));
  try {
    await fn(siteRoot);
  } finally {
...
test('task lifecycle MCP resolution prefers explicit env server', async () => {
  await withSite(async (siteRoot: any) => {
    const configuredServer: any = join(siteRoot, 'configured-task-mcp-server.js');
...
test('task lifecycle MCP resolution falls back through package bin, MCP config, then local tools', async () => {
  await withSite(async (siteRoot: any) => {
    ...
    const packageBin: any = join(siteRoot, 'node_modules', '.bin', packageBinName);
    ...
    const configuredServer: any = join(siteRoot, 'projected', 'task-mcp-server.js');
