...
import { payloadCreate, payloadShow } from '../mcp-payload-file.js';
import {
  createSiteLiftLifecycle,
  transitionSiteLiftLifecycle,
} from './site-lift-lifecycle.js';

const SEND_PAYLOAD_SCHEMA: any = 'narada.payload.site_lift.send.v1';
const SEND_RECORD_SCHEMA: any = 'narada.site_lift.send_record.v0';
const RESULT_SCHEMA: any = 'narada.site_lift.send_result.v0';
const DEFAULT_SEND_RECORD_DIR: any = 'site-lift/sends';
const INLINE_PAYLOAD_THRESHOLD: any = 200;
...
export function usage() : any{
  return [
    'Usage: node tools/site-lift/send-lift-package.ts --payload-ref mcp_payload:<id>@v1 --target-site-root <path> [--dry-run]',
    '',
    'Builds a compact target inbox envelope and durable send record for a site lift package.',
  ].join('\n');
}

export function sendLiftPackageFromPayloadRef(options: any = {}) : any{
...
export function sendLiftPackage({
  ...
  const prepared: any = prepareSendPayload({ siteRoot: root, payloadRef, payload });
  const sendPayload: any = prepared.payload;
  const validation: any = validateSendPayload(sendPayload);
  const resolvedTargetSiteRoot: any = resolveTargetSiteRoot(targetSiteRoot ?? sendPayload.target_site_root);
  const createdAt: any = new Date().toISOString();
  const envelopeId: any = `env_${randomUUID()}`;
  const envelope: any = buildEnvelope({ payload: sendPayload, payloadRef: prepared.payloadRef, envelopeId, createdAt, targetSiteRoot: resolvedTargetSiteRoot });
  const recordRelPath: any = normalizePath(join(sendRecordDir, `${sendPayload.package_id}-${envelopeId}.json`));
  const recordPath: any = resolveInside(root, recordRelPath);
  ...
  const plannedRecord: any = buildSendRecord({
    payload: sendPayload,
  ...
  try {
    ...
    const record: any = buildSendRecord({
      payload: sendPayload,
      payloadRef: prepared.payloadRef,
      payloadStagedFromInline: prepared.payloadStagedFromInline,
      envelope,
...
function prepareSendPayload({ siteRoot, payloadRef, payload }: any) : any{
  ...
  const payloadText: any = JSON.stringify(payload);
...
function validateSendPayload(payload: any) : any{
...
function buildEnvelope({ payload, payloadRef, envelopeId, createdAt, targetSiteRoot }: any) : any{
  const principal: any = payload.requested_by ?? 'site-lift-send-workflow';
  return {
...
function buildSendRecord({ payload, payloadRef, payloadStagedFromInline, envelope, targetSiteRoot, recordRelPath, createdAt, admission, lifecycle }: any) : any{
  return {
...
function resolveTargetSiteRoot(value: any) : any{
  const text: any = requireString(value, 'target_site_root');
  return resolve(text);
}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result: any = sendLiftPackageFromPayloadRef(args);
