...
import { payloadCreate } from '../mcp-payload-file.js';
import { createLiftPackageFromPayloadRef } from './create-lift-package.js';
import { sendLiftPackage } from './send-lift-package.js';

test('site lift records package creation and full receiving admission path', () => {
  const sourceRoot: any = mkdtempSync(join(tmpdir(), 'narada-site-lift-source-'));
  const targetRoot: any = mkdtempSync(join(tmpdir(), 'narada-site-lift-target-'));
  try {
    const packagePayload: any = payloadCreate({
      siteRoot: sourceRoot,
      args: {
        payload: {
    ...
    const created: any = createLiftPackageFromPayloadRef({ siteRoot: sourceRoot, payloadRef: packagePayload.ref });
    ...
    const metadata: any = JSON.parse(readFileSync(join(sourceRoot, created.paths.metadata_sidecar), 'utf8'));
    assert.equal(metadata.lifecycle_state, 'created');

    const sendPayload: any = payloadCreate({
      siteRoot: sourceRoot,
      args: {
        payload: {
    ...
    const sent: any = sendLiftPackage({ siteRoot: sourceRoot, payloadRef: sendPayload.ref, targetSiteRoot: targetRoot });
