const UPDATE_KINDS: any = new Set(['semantics_change', 'tooling_change', 'policy_change', 'capability_retirement']);
const AUTHORITY_LEVELS: any = new Set(['operator_direct', 'architect_admitted', 'agent_reported']);

export function compileCapabilityUpdateBroadcast(input: any = {}) : any{
  const event: any = normalizeCapabilityUpdateEvent(input.event ?? {});
  const publisher: any = normalizePublisher(input.publisher ?? {});
  const subscriptions: any = normalizeSubscriptions(input.subscriptions ?? []);
  ...
  const envelopes: any = subscriptions.map((subscription: any) => buildUpdateEnvelope({ event, publisher, subscription }));
  return {
    schema: 'narada.capability_update.broadcast_plan.v0',
...
export function normalizeCapabilityUpdateEvent(event: any) : any{
  return {
...
export function normalizePublisher(publisher: any) : any{
  return {
...
export function normalizeSubscriptions(subscriptions: any) : any{
...
function validateEvent(event: any) : any{
  const errors: any = [];
...
function validatePublisher(publisher: any) : any{
  const errors: any = [];
...
function validateSubscriptions(subscriptions: any) : any{
  const errors: any = [];
...
function buildUpdateEnvelope({ event, publisher, subscription }: any) : any{
  const subscribed: any = subscription.subscribed_capabilities.includes(event.capability_id) || subscription.subscribed_capabilities.includes('*');
  return {
...
function requireText(errors: any, field: any, value: any) : any{
...
function stringArray(value: any) : any{
  ...
  return value.map((item: any) => text(item)).filter(Boolean);
}
