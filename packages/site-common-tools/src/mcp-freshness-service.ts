...
export function readJsonFile(filePath: any) : any{
...
export function buildMcpFreshnessStatus({
  ...
  const pendingRestart: any = Boolean(restartRequest) || sourceChangedSinceBaseline === true || missingExpectedTools.length > 0;
...
export function writeMcpRuntimeInstanceObservation({
  ...
  const observedAt: any = now instanceof Date ? now.toISOString() : new Date(now).toISOString();
  const pcRoot: any = resolve(pcSiteRoot);
  ...
  const sourceEvidence: any = collectSourceEvidence({ siteRoot, watchedPaths });
  const baseline: any = readJsonFile(baselinePath);
  const restartRequestPayload: any = readJsonFile(restartRequestPath);
  const baselineMtime: any = Number.isFinite(baseline?.baseline_mtime) ? baseline.baseline_mtime : null;
  const sourceDigestComparison: any = compareSourceDigest({ sourceEvidence, baseline });
  ...
  const index: any = instances.findIndex((candidate: any) => candidate?.surface_id === surfaceId);
...
function isBoundCarrierObservation(entry: any) : any{
...
function isUnboundCarrierObservation(entry: any) : any{
  if (!entry || typeof entry !== 'object') return false;
  const carrierSessionId: any = entry.carrier_session_id ?? entry.process_identity_evidence?.carrier_session_id ?? null;
...
function preserveBoundCarrierObservation(existing: any, rejected: any) : any{
...
function summarizeSuppressedUnboundObservation(entry: any) : any{
  return {
...
function readInheritedCarrierSession({ pcRoot }: any) : any{
...
export function reconcileNoRequestMcpFreshnessMarker({
  ...
  const reconciledAt: any = new Date().toISOString();
  const restartRequest: any = readJsonFile(restartRequestPath);
  ...
  const sourceEvidence: any = collectSourceEvidence({ siteRoot, watchedPaths });
  const gate: any = validateNoRequestFreshnessReconciliation({
    pcSiteRoot,
    targetSurface,
    targetEntrypoint,
    sourceEvidence,
  ...
  const registry_reconciliation: any = reconcileMcpRuntimeRegistryAfterNoRequestFreshness({
    pcSiteRoot,
    targetSurface,
    targetEntrypoint,
    baselinePayload: payload,
...
function validateNoRequestFreshnessReconciliation({
  ...
  const registry: any = readJsonFile(registryPath);
  const instances: any = Array.isArray(registry?.instances) ? registry.instances : [];
  const instance: any = instances.find((entry: any) => entry?.surface_id === targetSurface
    || entry?.server_entrypoint === targetEntrypoint
    || entry?.server_entry_point === targetEntrypoint);
  if (!instance) {
    return {
  ...
  const restartState: any = instance.restart_request?.state ?? 'no_restart_request';
  ...
  const bootedAtRaw: any = processEvidence.booted_at ?? instance.booted_at ?? instance.runtime?.booted_at ?? null;
  const bootedAt: any = Date.parse(bootedAtRaw ?? '');
  ...
  const binding: any = instance.carrier_session_binding ?? {};
  const carrierSessionId: any = instance.carrier_session_id ?? processEvidence.carrier_session_id ?? null;
  const bindingStatus: any = binding.status ?? (carrierSessionId ? 'bound_to_parent_carrier_session' : 'legacy_unbound');
  ...
  const recordedSourceChanged: any = instance.source_freshness?.source_digest_changed
    ?? instance.source?.source_digest_changed
    ?? instance.runtime?.source_digest_changed;
  const sourceChanged: any = recordedSourceChanged === true || sourceDigestComparison.source_digest_changed === true
    ? true
    : (recordedSourceChanged ?? sourceDigestComparison.source_digest_changed);
  const recordedPendingRestart: any = instance.source_freshness?.pending_restart ?? instance.pending_restart;
  const pendingRestart: any = recordedPendingRestart === true || sourceChanged === true;
...
function reconcileMcpRuntimeRegistryAfterNoRequestFreshness({
  ...
  const instance: any = instances[index];
  const reconciledAt: any = baselinePayload.reconciled_at;
  const reconciliation: any = {
    schema: 'narada.mcp.no_request_freshness_registry_reconciliation.v0',
...
export function validateMcpRestartAcknowledgement({
  ...
  const requestedAt: any = Date.parse(restartRequest.requested_at ?? '');
  ...
  const sourceFreshness: any = instance.source_freshness ?? {};
  const sourceDigestComparison: any = compareSourceDigest({
    sourceEvidence,
...
export function buildMcpRestartPressure(freshnessEntries: any = []) : any{
...
export function deriveMcpRestartPressureRecommendation(mcpRestartPressure: any) : any{
...
export function buildStaleLiveNavigationDegradation(mcpRestartPressure: any) : any{
...
function classifyFreshnessStartupDisposition(entry: any) : any{
...
function buildHostRuntimeReference({ serverName, serverEntryPoint, restartRequest }: any) : any{
...
function inferSurfaceIdFromServerName(serverName: any) : any{
...
function buildRestartRemediation({ restartToolName, testToolName, serverEntryPoint, selfRestartSupported }: any) : any{
  const steps: any = [];
  if (restartToolName) {
...
function collectSourceEvidence({ siteRoot, watchedPaths }: any) : any{
  const files: any = [];
...
function compareSourceDigest({ sourceEvidence, baseline }: any) : any{
...
function normalizeRelativePath(siteRoot: any, file: any) : any{
  return relative(resolve(siteRoot), file).split(sep).join('/');
}

function collectFiles(absPath: any, out: any) : any{
  if (!existsSync(absPath)) return;
  const stats: any = statSync(absPath);
  if (stats.isFile()) {
    if (absPath.endsWith('.ts') || absPath.endsWith('.js') || absPath.endsWith('.json')) out.push(absPath);
    return;
  }
