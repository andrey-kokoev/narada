...
export function resolveTaskLifecycleMcpServer(siteRoot: any, env: any = process.env) : any{
  const root: any = resolve(siteRoot);
  const configuredServerPath: any = env.NARADA_TASK_LIFECYCLE_MCP_SERVER;
  ...
  const packageBinPath: any = join(root, 'node_modules', '.bin', packageBinName);
  ...
  for (const configPath of taskLifecycleMcpConfigPaths(root)) {
    const server: any = taskLifecycleServerFromMcpConfig(configPath, root);
    if (server) return server;
  }

  const localServerPath: any = join(root, 'tools', 'task-lifecycle', 'task-mcp-server.ts');
...
export function taskLifecycleReadinessPaths(siteRoot: any, env: any = process.env) : any{
  const root: any = resolve(siteRoot);
  const packageBinName: any = process.platform === 'win32' ? 'task-lifecycle-mcp.cmd' : 'task-lifecycle-mcp';
  const configuredServerPath: any = env.NARADA_TASK_LIFECYCLE_MCP_SERVER ?? null;
  const packageBinPath: any = join(root, 'node_modules', '.bin', packageBinName);
  const localServerPath: any = join(root, 'tools', 'task-lifecycle', 'task-mcp-server.ts');
  return {
...
function taskLifecycleMcpConfigPaths(siteRoot: any) : any{
  const mcpDir: any = join(siteRoot, '.ai', 'mcp');
  if (!existsSync(mcpDir)) return [];
  try {
    return readdirSync(mcpDir, { withFileTypes: true })
      .filter((entry: any) => entry.isFile() && /task-lifecycle.*mcp\.json$/i.test(entry.name))
      .map((entry: any) => join(mcpDir, entry.name))
      .sort();
  } catch {
    return [];
  }
...
function taskLifecycleServerFromMcpConfig(configPath: any, siteRoot: any) : any{
  let config: any = null;
  try {
  ...
  for (const candidate of taskLifecycleMcpConfigCandidates(config)) {
    const args: any = Array.isArray(candidate?.args) ? candidate.args.map((arg: any) => String(arg).replaceAll('{site_root}', siteRoot)) : [];
    const command: any = typeof candidate.command === 'string' && candidate.command.trim()
      ? candidate.command
      : process.execPath;
    const serverArg: any = args.find((arg: any) => /(^|[\\/])task-mcp-server\.(mjs|js)$/i.test(arg.replace(/\\/g, '/')));
    if (serverArg) {
      const serverPath: any = isAbsolute(serverArg) ? serverArg : resolve(siteRoot, serverArg);
...
function taskLifecycleMcpConfigCandidates(config: any) : any{
  ...
  for (const collection of [config.mcpServers, config.mcp_servers, config.servers]) {
    ...
    if (Array.isArray(collection)) {
      candidates.push(...collection.filter((entry: any) => entry && typeof entry === 'object'));
    } else {
      for (const [key, entry] of Object.entries(collection) as Array<[string, any]>) {
  ...
  return candidates.filter((entry: any) => {
    const name: any = String(entry.name ?? entry.server_name ?? basename(String(entry.id ?? ''))).toLowerCase();
    const normalizedName: any = name.replace(/[^a-z0-9]/g, '');
    const args: any = Array.isArray(entry.args) ? entry.args.join(' ').toLowerCase() : '';
