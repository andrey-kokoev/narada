#!/usr/bin/env node
/**
 * verify-agent-context-schema.ts
 *
...
function tableColumns(db: any, tableName: any) : any{
  const stmt: any = db.prepare(`PRAGMA table_info(${tableName})`);
  return stmt.all().map((row: any) => row.name);
}

function tableExists(db: any, tableName: any) : any{
  const stmt: any = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?");
...
function main() : any{
  ...
  const icCols: any = tableColumns(db, 'intelligence_context_materializations');
  const icRequired: any = ['event_id', 'schema_id', 'payload_json', 'created_at', 'expires_at'];
  ...
  const ecCols: any = tableColumns(db, 'execution_context_materializations');
  const ecRequired: any = ['event_id', 'runtime', 'payload_json', 'created_at', 'expires_at'];
  ...
  const prCols: any = tableColumns(db, 'proposal_records');
  const prRequired: any = ['proposal_type', 'verdict', 'verdict_at', 'verdict_by'];
  ...
  const rrCols: any = tableColumns(db, 'residual_records');
  const rrRequired: any = ['status', 'promoted_task_id'];
  ...
  try {
    const insertEvent: any = db.prepare(`INSERT INTO agent_start_events (event_id, identity_id, runtime, created_at, status) VALUES (?, ?, ?, ?, ?)`);
    insertEvent.run('evt-test-001', 'andrey-user.architect', 'kimi-cli', '2026-05-04T19:00:00Z', 'active');

    const insertIC: any = db.prepare(`INSERT INTO intelligence_context_materializations (materialization_id, event_id, schema_id, payload_json, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?)`);
    insertIC.run('ic-test-001', 'evt-test-001', 'narada.intelligence_context.v0', '{}', '2026-05-04T19:00:00Z', '2026-05-04T20:00:00Z');

    const insertEC: any = db.prepare(`INSERT INTO execution_context_materializations (materialization_id, event_id, runtime, payload_json, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?)`);
    insertEC.run('ec-test-001', 'evt-test-001', 'kimi-cli', '{}', '2026-05-04T19:00:00Z', '2026-05-04T20:00:00Z');

    const deleteEvent: any = db.prepare(`DELETE FROM agent_start_events WHERE event_id = ?`);
    deleteEvent.run('evt-test-001');

    const icAfter: any = db.prepare(`SELECT COUNT(*) as count FROM intelligence_context_materializations WHERE event_id = ?`).get('evt-test-001');
    const ecAfter: any = db.prepare(`SELECT COUNT(*) as count FROM execution_context_materializations WHERE event_id = ?`).get('evt-test-001');
  ...
  try {
    const insertEvent2: any = db.prepare(`INSERT INTO agent_start_events (event_id, identity_id, runtime, created_at, status) VALUES (?, ?, ?, ?, ?)`);
    insertEvent2.run('evt-test-002', 'andrey-user.architect', 'kimi-cli', '2026-05-04T19:00:00Z', 'active');

    const insertIC2: any = db.prepare(`INSERT INTO intelligence_context_materializations (materialization_id, event_id, schema_id, payload_json, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?)`);
    insertIC2.run('ic-test-002', 'evt-test-002', 'narada.intelligence_context.v0', '{}', '2026-05-04T19:00:00Z', '2026-05-04T20:00:00Z');

    const insertProposal: any = db.prepare(`INSERT INTO proposal_records (proposal_id, event_id, materialization_id, proposal_type, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?)`);
    insertProposal.run('prop-test-001', 'evt-test-002', 'ic-test-002', 'decision_request', '{}', '2026-05-04T19:00:00Z');

    const insertResidual: any = db.prepare(`INSERT INTO residual_records (residual_id, event_id, materialization_id, label, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?)`);
    insertResidual.run('res-test-001', 'evt-test-002', 'ic-test-002', 'session_resume_boundary', '{}', '2026-05-04T19:00:00Z');

    const deleteIC: any = db.prepare(`DELETE FROM intelligence_context_materializations WHERE materialization_id = ?`);
    deleteIC.run('ic-test-002');

    const proposalAfter: any = db.prepare(`SELECT materialization_id FROM proposal_records WHERE proposal_id = ?`).get('prop-test-001');
    const residualAfter: any = db.prepare(`SELECT materialization_id FROM residual_records WHERE residual_id = ?`).get('res-test-001');
...
function printResults(findings: any) : any{
  ...
  const status: any = failed === 0 ? 'PASS' : 'FAIL';
  console.log(`\n---`);
