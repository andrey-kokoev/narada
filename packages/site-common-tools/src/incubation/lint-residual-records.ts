 ...
 *
 * Usage:
 *   node tools/incubation/lint-residual-records.ts [path/to/agent-context.sqlite]
 *
...
const DEFAULT_DB_PATH: any = join(rootDir, '.ai', 'state', 'agent-context.sqlite');

const dbPath: any = process.argv[2] ?? DEFAULT_DB_PATH;

function isTokenLike(label: any) : any{
...
function hasHumanReadableDescription(payload: any) : any{
  if (!payload || typeof payload !== 'object') return false;
  const desc: any = payload.description;
  if (typeof desc !== 'string') return false;
  if (desc.trim().length < 10) return false;
  return true;
}

function lintResidual(row: any) : any{
  const { residual_id, label, payload_json, status, event_id, materialization_id }: any = row;
  const findings: any = [];
  ...
  if (!payload.source_ref || typeof payload.source_ref !== 'object') {
    ...
    const sr: any = payload.source_ref;
  ...
  if (!payload.expected_next_boundary || typeof payload.expected_next_boundary !== 'object') {
    ...
    const enb: any = payload.expected_next_boundary;
...
function main() : any{
  ...
  const rows: any = db.prepare('SELECT residual_id, label, payload_json, status, event_id, materialization_id FROM residual_records').all();

  let hardCount: any = 0;
  let softCount: any = 0;
  let cleanCount: any = 0;

  for (const row of rows) {
    const findings: any = lintResidual(row);
