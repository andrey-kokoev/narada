...
const INTENT_SCHEMA: any = 'narada.onboarding.intent.v0';
const PROPOSAL_SCHEMA: any = 'narada.site_config.proposal.v0';
const ERROR_SCHEMA: any = 'narada.onboarding.intent_projection.error.v0';

const REQUIRED_INTENT_FIELDS: any = [
  'schema',
  'intent_id',
  'operator_words',
  'source_context',
...
function site(kind: any, role: any, root: any, authorityRequired: any) : any{
  return {
    kind,
    role,
    root,
...
function projectionForKind(intent: any) : any{
  const roots: any = targetRoots(intent);
  const projectRoot: any = roots.project_site ?? roots.receiving_site ?? null;
...
function validateIntent(intent: any) : any{
  const errors: any = [];
...
function projectIntent(intent: any) : any{
  const errors: any = validateIntent(intent);
  if (errors.length > 0) {
  ...
  const projected: any = projectionForKind(intent);
  return {
    ...
    continuation: {
      lifecycle_route: intent.lifecycle_route,
      next_surface: 'tools/lifecycle/continue-first-time-operator-onboarding.ts'
    }
  };
}
...
function runCli(argv: any = process.argv.slice(2), stdout: any = process.stdout, stderr: any = process.stderr) : any{
  try {
    ...
    const intent: any = options.intentPath ? readJson(options.intentPath) : options.intentJson;
    const result: any = projectIntent(intent);
