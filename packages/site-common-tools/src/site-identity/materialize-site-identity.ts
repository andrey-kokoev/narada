...
const DEFAULT_PURPOSES: any = ['site_identity', 'sign_declarations', 'sign_crossings', 'sign_probe_reports'];

export function materializeSiteIdentity(rawOptions: any) : any{
  const options: any = normalizeOptions(rawOptions);
  const siteRoot: any = resolve(options.siteRoot);
  const secretRoot: any = resolveSecretRoot(options.secretRoot);
  const keyId: any = options.keyId ?? defaultKeyId(new Date());
  const secretDir: any = join(secretRoot, options.siteId);
  const secretPath: any = join(secretDir, `${keyId}.private.jwk.json`);
  const identityPath: any = resolveIdentityDocumentPath(siteRoot);
  ...
  const keyMaterial: any = loadOrCreatePrivateKey({ secretPath, siteId: options.siteId, keyId, createdAt: options.createdAt });
  const privateKey: any = createPrivateKey({ key: keyMaterial.private_key_jwk, format: 'jwk' });
  if (privateKey.asymmetricKeyType !== 'ed25519') throw new Error('private_key_not_ed25519');
  const publicKeyObject: any = createPublicKey(privateKey);
  const publicKeyDer: any = publicKeyObject.export({ type: 'spki', format: 'der' });
  const fingerprint: any = createHash('sha256').update(publicKeyDer).digest('hex');
  const createdAt: any = keyMaterial.created_at;
  ...
  const identity: any = {
    ...
    evidence_refs: [
      'task:#502',
      'tool:tools/site-identity/materialize-site-identity.ts',
    ],
  };
...
function normalizeOptions(rawOptions: any) : any{
  const options: any = {
...
function parseArgs(argv: any) : any{
  const options: any = { siteRoot: process.cwd(), locusType: 'user', purposes: DEFAULT_PURPOSES, force: false, json: false };
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === '--purpose') { options.purposes = argv[++i].split(',').map((item: any) => item.trim()).filter(Boolean); continue; }
...
function usage() : any{
  return [
    'Usage: node tools/site-identity/materialize-site-identity.ts --site-id <site-id> [options]',
    '',
...
function loadOrCreatePrivateKey({ secretPath, siteId, keyId, createdAt }: any) : any{
  if (existsSync(secretPath)) {
    const parsed: any = JSON.parse(readFileSync(secretPath, 'utf8'));
  ...
  const { privateKey }: any = generateKeyPairSync('ed25519');
  const privateJwk: any = privateKey.export({ format: 'jwk' });
  const materializedAt: any = createdAt ?? new Date().toISOString();
  const payload: any = {
...
function relativeSiteRootForIdentity(siteRoot: any) : any{
  const normalized: any = siteRoot.split('\\').join('/').toLowerCase();
  return normalized.endsWith('/.narada') ? '.narada' : '.';
}

function defaultKeyId(date: any) : any{
  const year: any = date.getUTCFullYear();
  const month: any = String(date.getUTCMonth() + 1).padStart(2, '0');
  return `site-ed25519-${year}-${month}`;
}

function base64url(buffer: any) : any{
  return Buffer.from(buffer).toString('base64url');
}

function isPathInside(child: any, parent: any) : any{
  const childResolved: any = resolve(child).toLowerCase();
  const parentResolved: any = resolve(parent).toLowerCase();
  return childResolved === parentResolved || childResolved.startsWith(`${parentResolved}\\`);
}

function humanResult(result: any) : any{
  return [
...
if (isMain()) {
  try {
    ...
    if (options.help) {
      ...
      const result: any = materializeSiteIdentity(options);
