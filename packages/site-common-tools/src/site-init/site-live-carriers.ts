...
} from './site-live-carrier-state.js';

const CARRIER_SCHEMA: any = 'narada.site_live_carrier.result.v0';
const LOCAL_DB_CARRIER_ID: any = 'site_local_db_init';
const STORAGE_CARRIER_ID: any = 'site_local_storage_hydration';
const MCP_REGISTRATION_CARRIER_ID: any = 'site_mcp_registration_transport';
const WINDOWS_PROFILE_CARRIER_ID: any = 'windows_profile_site_binding';
const LOCAL_DB_SCHEMA_VERSION: any = 'site-local-db.v0';

const LOCAL_DB_TABLES: any = [
  'admissions',
  'checkpoints',
  'inbox_index',
  'capability_state',
...
const NON_PORTABLE_CLASSES: any = [
  '.ai',
...
function siteLocalDbCarrier(options: any = {}) : any{
  return runCarrier({
...
function siteLocalStorageHydrationCarrier(options: any = {}) : any{
  return runCarrier({
...
function siteMcpRegistrationCarrier(options: any = {}) : any{
  return runCarrier({
...
function windowsProfileSiteBindingCarrier(options: any = {}) : any{
  return runCarrier({
...
function siteLivePathDoctor(options: any = {}) : any{
  const targetSiteRoot: any = normalizePath(requiredOption(options, 'target_site_root'));
  const paths: any = carrierPaths(targetSiteRoot);
  const siteDoc: any = inspectJsonFile(path.join(targetSiteRoot, '.narada/site.json'));
  const siteId: any = stringOption(options.site_id) ?? stringOption(siteDoc.value?.site_id) ?? path.basename(targetSiteRoot).toLowerCase();
  const context: any = {
  ...
  const nextGate: any = gates.find((gate: any) => gate.status !== 'verified') ?? null;
  return {
...
function runCarrier({ carrierId, mode, options, planBuilder, applier, verifier, recoverer }: any) : any{
  const context: any = buildContext(carrierId, mode, options);
  let lifecycle: any = createSiteLiveCarrierLifecycle();
  lifecycle = transitionSiteLiveCarrierLifecycle(lifecycle, 'planning');
  const refusals: any = validateSharedContext(context, options);
  const plan: any = planBuilder(context, options);
  lifecycle = transitionSiteLiveCarrierLifecycle(lifecycle, 'planned');
  const result: any = baseResult(context, plan, refusals);
  const withLifecycle: any = (value: any) => ({
    ...value,
  ...
  if (mode === 'verify') {
    ...
    const verified: any = verifier(context, plan, result);
  ...
  if (mode === 'recover') {
    ...
    const recovered: any = recoverer(context, plan, result);
  ...
  const applied: any = applier(context, plan, result);
...
function buildContext(carrierId: any, mode: any, options: any) : any{
  const targetSiteRoot: any = normalizePath(requiredOption(options, 'target_site_root'));
  const siteId: any = requiredOption(options, 'site_id');
  return {
    schema: CARRIER_SCHEMA,
    carrierId,
    mode,
...
function buildLocalDbPlan(context: any) : any{
  const dbState: any = inspectJsonFile(context.paths.localDb);
  const compatible: any = dbState.status === 'missing'
  ...
  const refusals: any = compatible ? [] : ['existing_db_incompatible_or_belongs_to_another_site'];
  return {
...
function applyLocalDbPlan(context: any, plan: any, result: any) : any{
  ...
  const before: any = inspectJsonFile(context.paths.localDb);
  const dbDoc: any = {
    ...
    tables: Object.fromEntries(LOCAL_DB_TABLES.map((name: any) => [name, []])),
  ...
  const verify: any = verifyLocalDbPlan(context, plan, result);
  const changed: any = before.status === 'missing'
...
function verifyLocalDbPlan(context: any, plan: any, result: any) : any{
  const db: any = inspectJsonFile(context.paths.localDb);
  const ok: any = db.status === 'json'
    ...
    && LOCAL_DB_TABLES.every((name: any) => Array.isArray(db.value?.tables?.[name]));
  return {
    ...result,
...
function recoverLocalDbPlan(context: any, plan: any, result: any) : any{
  const db: any = inspectJsonFile(context.paths.localDb);
  let recoveryStatus: any = 'absent';
...
function buildStoragePlan(context: any, options: any) : any{
  const manifest: any = inspectJsonFile(context.paths.hydrationManifest);
  const handoffDisposition: any = options.handoff_material
  ...
  return {
    planned_mutations: [
      ...STORAGE_DIRS.map((item: any) => mutationRecord(path.join(context.targetSiteRoot, item), 'ensure_directory')),
      ...STORAGE_FILES.map((item: any) => mutationRecord(path.join(context.targetSiteRoot, item), 'ensure_file')),
      mutationRecord(context.paths.auditLog, 'append_audit_event'),
    ],
    created_or_changed: [],
    audit_evidence: {
...
function applyStoragePlan(context: any, plan: any, result: any) : any{
  if (plan.refusals.length > 0) return refusedFromPlan(result, plan.refusals);
  const changed: any = [];
  for (const dir of STORAGE_DIRS) {
    const absolute: any = path.join(context.targetSiteRoot, dir);
    if (!fs.existsSync(absolute)) changed.push(dir);
    fs.mkdirSync(absolute, { recursive: true });
  }
  const files: any = storageFileContents(context, plan);
  for (const [relative, content] of Object.entries(files) as Array<[string, string]>) {
    const absolute: any = path.join(context.targetSiteRoot, relative);
  ...
  const verify: any = verifyStoragePlan(context, plan, result);
  return {
    ...verify,
    status: 'applied',
    created_or_changed: changed,
...
function verifyStoragePlan(context: any, plan: any, result: any) : any{
  const manifest: any = inspectJsonFile(context.paths.hydrationManifest);
  const missingDirs: any = STORAGE_DIRS.filter((item: any) => !isDirectory(path.join(context.targetSiteRoot, item)));
  const ok: any = manifest.status === 'json'
...
function recoverStoragePlan(context: any, plan: any, result: any) : any{
  const presentDirs: any = STORAGE_DIRS.filter((item: any) => isDirectory(path.join(context.targetSiteRoot, item)));
  const manifest: any = inspectJsonFile(context.paths.hydrationManifest);
  const classification: any = presentDirs.length === 0 && manifest.status === 'missing'
    ? 'absent'
...
function buildMcpRegistrationPlan(context: any, options: any) : any{
  const manifest: any = inspectJsonFile(context.paths.mcpRegistrationManifest);
  const descriptors: any = normalizeMcpServerDescriptors(options.mcp_server_descriptors);
  const refusals: any = [];
  ...
  if (descriptors.some((descriptor: any) => descriptor.grants_broader_authority === true)) {
    refusals.push('mcp_registration_broader_authority_refused');
  }
  if (context.sourceSiteRoot && descriptors.some((descriptor: any) => descriptor.entrypoint && sameOrParent(context.sourceSiteRoot, normalizePath(descriptor.entrypoint)))) {
  ...
  return {
    ...
    audit_evidence: {
      ...
      restart_required: descriptors.some((descriptor: any) => (descriptor.transport ?? 'stdio') === 'stdio'),
...
function applyMcpRegistrationPlan(context: any, plan: any, result: any) : any{
  const manifest: any = mcpRegistrationManifest(context, context.options, plan);
  const before: any = inspectJsonFile(context.paths.mcpRegistrationManifest);
  const changed: any = [];
  ...
  const verify: any = verifyMcpRegistrationPlan(context, plan, result);
  return {
    ...verify,
    status: 'applied',
    created_or_changed: changed,
...
function verifyMcpRegistrationPlan(context: any, plan: any, result: any) : any{
  const manifest: any = inspectJsonFile(context.paths.mcpRegistrationManifest);
  const ok: any = manifest.status === 'json'
    && manifest.value?.site_id === context.siteId
    && manifest.value?.target_site_root === context.targetSiteRoot
...
function recoverMcpRegistrationPlan(context: any, plan: any, result: any) : any{
  const manifest: any = inspectJsonFile(context.paths.mcpRegistrationManifest);
  const classification: any = manifest.status === 'missing'
    ? 'absent'
    : manifest.status === 'json' && manifest.value?.site_id === context.siteId
      ? 'compatible_seed'
...
function buildWindowsProfilePlan(context: any, options: any) : any{
  const artifactPath: any = resolveProfileArtifactPath(context, options);
  const existing: any = inspectJsonFile(artifactPath);
  const refusals: any = [];
...
function applyWindowsProfilePlan(context: any, plan: any, result: any) : any{
  const artifactPath: any = path.join(context.targetSiteRoot, plan.audit_evidence.profile_artifact_path);
  const before: any = fs.existsSync(artifactPath) ? fs.readFileSync(artifactPath, 'utf8') : null;
  const binding: any = windowsProfileBinding(context, context.options);
  const next: any = `${JSON.stringify(binding, null, 2)}\n`;
  const changed: any = [];
  if (before !== next) {
  ...
  const verify: any = verifyWindowsProfilePlan(context, plan, result);
  return {
    ...verify,
    status: 'applied',
    created_or_changed: changed,
...
function verifyWindowsProfilePlan(context: any, plan: any, result: any) : any{
  const artifactPath: any = path.join(context.targetSiteRoot, plan.audit_evidence.profile_artifact_path);
  const binding: any = inspectJsonFile(artifactPath);
  const ok: any = binding.status === 'json'
...
function recoverWindowsProfilePlan(context: any, plan: any, result: any) : any{
  const artifactPath: any = path.join(context.targetSiteRoot, plan.audit_evidence.profile_artifact_path);
  const binding: any = inspectJsonFile(artifactPath);
  const preimage: any = inspectJsonFile(context.paths.profilePreimage);
  const classification: any = binding.status === 'missing'
    ? 'absent'
...
function validateSharedContext(context: any, options: any) : any{
  const refusals: any = [];
...
function baseResult(context: any, plan: any, sharedRefusals: any) : any{
  return {
...
function storageFileContents(context: any, plan: any) : any{
  const manifest: any = {
...
function carrierPaths(targetSiteRoot: any) : any{
  return {
...
function doctorGateLocalDb(context: any) : any{
  const db: any = inspectJsonFile(context.paths.localDb);
  const verified: any = db.status === 'json'
    && db.value?.site_id === context.siteId
    && db.value?.target_site_root === context.targetSiteRoot
    && db.value?.schema_version === LOCAL_DB_SCHEMA_VERSION;
  return {
...
function doctorGateStorage(context: any) : any{
  const manifest: any = inspectJsonFile(context.paths.hydrationManifest);
  const missingDirs: any = STORAGE_DIRS.filter((item: any) => !isDirectory(path.join(context.targetSiteRoot, item)));
  const verified: any = manifest.status === 'json'
    && manifest.value?.site_id === context.siteId
    && manifest.value?.target_site_root === context.targetSiteRoot
    && missingDirs.length === 0;
  return {
...
function doctorGateMcp(context: any) : any{
  const manifest: any = inspectJsonFile(context.paths.mcpRegistrationManifest);
  const verified: any = manifest.status === 'json'
    && manifest.value?.site_id === context.siteId
    && manifest.value?.target_site_root === context.targetSiteRoot
...
function doctorGateProfile(context: any) : any{
  const binding: any = inspectJsonFile(context.paths.windowsProfileBinding);
  const verified: any = binding.status === 'json'
    && binding.value?.site_id === context.siteId
    && binding.value?.target_site_root === context.targetSiteRoot
    && binding.value?.carrier_id === WINDOWS_PROFILE_CARRIER_ID;
  return {
...
function liveCarrierCommand(context: any, carrierId: any, mode: any, extraArgs: any = []) : any{
  return [
    'node tools/site-init/site-live-carriers.ts',
    '--carrier', carrierId,
    '--mode', mode,
...
function mcpRegistrationManifest(context: any, options: any, plan: any) : any{
  const descriptors: any = normalizeMcpServerDescriptors(options.mcp_server_descriptors)
    .map((descriptor: any) => withSiteAccessPolicyAllowedRoots(descriptor, context));
  return {
...
function normalizeMcpServerDescriptors(value: any) : any{
...
function withSiteAccessPolicyAllowedRoots(descriptor: any, context: any) : any{
  if (!descriptorNeedsAllowedRoots(descriptor)) return descriptor;
  const policyRoots: any = siteAccessPolicyAllowedRoots(context);
  if (policyRoots.length === 0) return descriptor;
  const args: any = [...(descriptor.args ?? [])];
  const existingRoots: any = allowedRootsFromArgs(args);
  const roots: any = uniqueStrings([...existingRoots, ...policyRoots]);
...
function descriptorNeedsAllowedRoots(descriptor: any) : any{
  const name: any = String(descriptor?.name ?? '').toLowerCase();
  const entrypoint: any = String(descriptor?.entrypoint ?? '').toLowerCase();
  const args: any = descriptor?.args ?? [];
  ...
  return [
    ...
    'delegated-task',
    'git',
  ].some((needle: any) => name.includes(needle) || entrypoint.includes(needle));
}

function allowedRootsFromArgs(args: any) : any{
  const roots: any = [];
...
function siteAccessPolicyAllowedRoots(context: any) : any{
  const policy: any = readSiteAccessPolicy(context);
  if (!policy) return [];
  const entries: any = Array.isArray(policy?.allowed_roots) ? policy.allowed_roots : [];
  const roots: any = entries
    .filter((entry: any) => entry && typeof entry === 'object')
    .filter((entry: any) => Array.isArray(entry.access) && entry.access.includes('write'))
    .map((entry: any) => stringOption(entry.path))
    .filter(Boolean)
    .map((root: any) => normalizePath(root))
    .filter((root: any) => !looksLikeSecretFile(root));
  return uniqueStrings([context.targetSiteRoot, ...roots]);
}

function readSiteAccessPolicy(context: any) : any{
  for (const policyPath of siteAccessPolicyCandidatePaths(context)) {
    const policy: any = inspectJsonFile(policyPath);
    if (policy.status === 'json') return policy.value;
  }
  return null;
}

function siteAccessPolicyCandidatePaths(context: any) : any{
  const siteSlug: any = String(context.siteId ?? '').toLowerCase();
...
function looksLikeSecretFile(root: any) : any{
...
function resolveProfileArtifactPath(context: any, options: any) : any{
  const requested: any = stringOption(options.profile_artifact_path);
  return requested ? normalizePath(requested) : context.paths.windowsProfileBinding;
}

function profileBindings(context: any, options: any) : any{
  return {
...
function windowsProfileBinding(context: any, options: any) : any{
  return {
...
function migrationEvent(context: any, status: any) : any{
  return {
...
function appendAudit(context: any, event: any) : any{
  fs.mkdirSync(path.dirname(context.paths.auditLog), { recursive: true });
...
function appendJsonl(filePath: any, value: any) : any{
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.appendFileSync(filePath, `${JSON.stringify(value)}\n`, 'utf8');
}

function writeJson(filePath: any, value: any) : any{
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, `${JSON.stringify(value, null, 2)}\n`, { flag: 'wx' });
}

function inspectJsonFile(filePath: any) : any{
...
function readJsonl(filePath: any) : any{
...
function mutationRecord(absolutePath: any, action: any) : any{
  return { action, path: absolutePath };
}

function refusedFromPlan(result: any, refusals: any) : any{
  return { ...result, status: 'refused', refusals, recovery_hint: 'Resolve refusal conditions and rerun plan.' };
}

function recoveryHint(carrierId: any, refusals: any) : any{
...
function stablePlanId(context: any, label: any) : any{
  return `${context.siteId}:${label}:${relativeToTarget(context, context.targetSiteRoot) || '.'}`;
}

function relativeToTarget(context: any, absolutePath: any) : any{
  return path.relative(context.targetSiteRoot, absolutePath).replace(/\\/g, '/');
}

function sameOrParent(parent: any, child: any) : any{
  const rel: any = path.relative(parent, child);
...
function isSuspiciousRoot(projectRoot: any) : any{
  const resolved: any = normalizePath(projectRoot);
  const parsed: any = path.parse(resolved);
  const lower: any = resolved.toLowerCase();
...
function readMcpServerDescriptorsFile(filePath: any) : any{
...
function runCli(argv: any = process.argv.slice(2), stdout: any = process.stdout, stderr: any = process.stderr) : any{
  try {
    ...
    const result: any = options.doctor || options.carrier === 'site_live_path_doctor'
