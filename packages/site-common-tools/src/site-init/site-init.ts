...
} from './site-init-lifecycle.js';

const SCHEMA: any = 'narada.site_init.result.v0';

const SEED_FILES: any = [
  '.narada/site.json',
  '.narada/README.md',
...
const NON_PORTABLE_REFUSALS: any = [
  '.ai',
  'SQLite databases',
...
function seedFileContent(relativePath: any, result: any) : any{
...
function sanitizeProjectId(name: any) : any{
  const sanitized: any = String(name || 'project')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/^-+|-+$/g, '');
...
function isSuspiciousRoot(projectRoot: any) : any{
  ...
  const home: any = normalizePath(os.homedir()).toLowerCase();
  const temp: any = normalizePath(os.tmpdir()).toLowerCase();
  ...
  const systemSegments: any = ['windows', 'program files', 'program files (x86)', 'programdata'];
  const basename: any = path.basename(resolved).toLowerCase();
...
function projectionTargetsProjectMemory(siteConfigProposal: any) : any{
  if (!siteConfigProposal) return true;
  const kinds: any = new Set(siteConfigProposal.required_sites.map((site: any) => site.kind));
  return kinds.has('project_site')
    || kinds.has('client_receiving_site')
    || kinds.has('handoff_adoption_site');
}

function baseResult({ command, projectRoot, siteConfigProposal = null }: any) : any{
  const resolvedRoot: any = normalizePath(projectRoot);
  const projectMemoryRoot: any = siteControlRoot(resolvedRoot);
  return {
    schema: SCHEMA,
    command,
    status: 'blocked',
...
function createSeed(result: any) : any{
  const createdFiles: any = [];
  for (const relativePath of SEED_FILES) {
    try {
      const targetPath: any = path.join(result.project_root, relativePath);
...
function inspectProject({
  ...
  const siteConfigProposal: any = intent ? projectIntent(intent) : null;
  const result: any = baseResult({ command, projectRoot, siteConfigProposal });
  let lifecycle: any = createSiteInitLifecycle();
  lifecycle = transitionSiteInitLifecycle(lifecycle, 'inspecting');
  const finish: any = (value: any, state: any) => {
  ...
  const rootStat: any = statMaybe(result.project_root);
  ...
  const suspicious: any = isSuspiciousRoot(result.project_root);
  ...
  const siteStat: any = statMaybe(result.project_memory_root);
  const hasNarada: any = Boolean(siteStat && siteStat.isDirectory());
  const incompatibleNarada: any = Boolean(siteStat && !siteStat.isDirectory());
  ...
  if (yes) {
    ...
    try {
      const createdFiles: any = createSeed(result);
      ...
      const createdFiles: any = Array.isArray(error.created_files) ? error.created_files : [];
      return finish({
        ...result,
        status: 'partial',
        authority_classification: 'write_authorized',
...
function parseArgs(argv: any) : any{
  const args: any = [...argv];
  let command: any = 'init';
  let projectRoot: any = process.cwd();
  let apply: any = false;
  let yes: any = false;
  let pretty: any = false;
  let intentPath: any = null;
  let intent: any = null;
...
function runCli(argv: any = process.argv.slice(2), stdout: any = process.stdout, stderr: any = process.stderr) : any{
  try {
    ...
    const result: any = inspectProject(options);
