...
const TARGET_CONFIG: any = 'C:\\Users\\Andrey\\OneDrive - Global Maxima LLC\\!Business\\!Clients\\Staccato\\.narada\\config.json';
const TARGET_ROOT: any = dirname(TARGET_CONFIG);
const WORKSPACE_ROOT: any = 'C:\\Users\\Andrey\\OneDrive - Global Maxima LLC\\!Business\\!Clients\\Staccato';
const TASK_NUMBER: any = 458;
const APPROVAL_ENVELOPE: any = 'env_e13a0490f8e94e73a3b3b6b4bd847297';
let activeOutputToolName: any = null;

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  process.stdout.write('Usage: node tools/site-config/staccato-config-crossing-mcp-server.ts --site-root <path>\n');
  process.exit(0);
}
...
async function runStdioServer() : Promise<any>{
  ...
  for await (const chunk of process.stdin) {
    ...
    buffer = lines.pop() ?? '';
    for (const request of lines.filter(Boolean).map((line: any) => JSON.parse(line))) {
...
function dispatch(method: any, params: any) : any{
...
function doctor() : any{
  const loaded: any = existsSync(TARGET_CONFIG) ? loadConfig() : null;
  return {
...
function applyCrossing(args: any) : any{
  ...
  const before: any = loadConfig();
  const now: any = new Date().toISOString();
  const updated: any = buildUpdatedConfig(before.config, now, args.authority_basis);
  const validation: any = validateConfig(updated);
  if (validation.status !== 'valid') {
    return { schema: 'narada.staccato_config_crossing.apply.v0', status: 'blocked', validation };
  }
  const content: any = `${JSON.stringify(updated, null, 2)}\n`;
  const afterSha256: any = sha256(content);
...
function buildUpdatedConfig(original: any, now: any, authorityBasis: any) : any{
  const targetSiteId: any = original.site_id ?? original.static_config?.site_id ?? 'staccato-client-service';
...
function knownUserSite(targetSiteId: any, now: any) : any{
  return {
...
function claim(from: any, to: any, capability: any, status: any, basis: any) : any{
  return { from, to, capability, status, basis, evidence_refs: [`task:${TASK_NUMBER}`, `inbox:${APPROVAL_ENVELOPE}`] };
}

function denial(from: any, to: any, capability: any) : any{
...
function runtimeParam(currentValue: any, defaultValue: any, authority: any, mutableAtRuntime: any) : any{
...
function validateConfig(config: any) : any{
  ...
  const site: any = config.structural_config.site_awareness.known_sites['andrey-user'];
  ...
  if (!site.authority_boundaries.not_granted_by_awareness.some((entry: any) => entry.includes('mutate'))) {
...
function assertAuthority(authorityBasis: any) : any{
  ...
  const text: any = JSON.stringify(authorityBasis);
...
function loadConfig() : any{
  ...
  const content: any = readFileSync(TARGET_CONFIG, 'utf8');
  return { content, sha256: sha256(content), config: JSON.parse(content) };
}

function sha256(content: any) : any{
  return createHash('sha256').update(content, 'utf8').digest('hex');
}

function jsonResult(value: any) : any{
