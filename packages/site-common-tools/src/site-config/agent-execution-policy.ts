...
export function validateAgentExecutionPolicy(siteRoot: any, config: any = null, registry: any = null) : any{
  const root: any = resolve(siteRoot);
  const loadedConfig: any = config ?? readJson(join(root, 'config.json'));
  const registryPath: any = join(siteControlRoot(root), 'capabilities', 'mcp-surfaces.json');
  const loadedRegistry: any = registry ?? readJson(registryPath);
  const policy: any = loadedConfig?.structural_config?.agent_execution_policy;
  const errors: any = [];
  const residuals: any = [];
  ...
  const nonGrants: any = new Set(Array.isArray(policy.non_grants) ? policy.non_grants : []);
  ...
  const registryEntrypoints: any = declaredMcpEntrypoints(loadedRegistry);
  const allowlist: any = Array.isArray(policy.allowed_mcp_entrypoints) ? policy.allowed_mcp_entrypoints : [];
  if (!Array.isArray(policy.allowed_mcp_entrypoints)) errors.push('agent_execution_policy_allowed_mcp_entrypoints_must_be_array');
  const allowlistPaths: any = new Set(allowlist.map((entry: any) => entry?.path).filter(Boolean));
  ...
  for (const entry of allowlist) {
    ...
    if (entry.path && !registryEntrypoints.some((candidate: any) => candidate.path === entry.path && candidate.surface_id === entry.surface_id)) {
...
export function declaredMcpEntrypoints(registry: any) : any{
  ...
  return surfaces
    ...
    .filter((entry: any) => entry.surface_id && entry.path);
}

function result({ root, policy, registry, errors, residuals, registryEntrypoints = declaredMcpEntrypoints(registry) }: any) : any{
  return {
