const COMMAND_REGISTRY_ENTRIES: any = Object.freeze([
...
const COMMAND_REGISTRY: any = new Map(COMMAND_REGISTRY_ENTRIES.map((item: any) => [item.command_schema, item]));

export function genericCommandRegistryEntries() : any{
  return COMMAND_REGISTRY_ENTRIES.map(cloneEntry);
}

export function genericCommandSchemaMetadata(commandSchema: any) : any{
  const entry: any = COMMAND_REGISTRY.get(commandSchema);
  return entry ? cloneEntry(entry) : null;
}

export function supportedGenericCommandSchemas() : any{
  return new Set(COMMAND_REGISTRY.keys());
}

export function genericCommandRegistrySummary() : any{
  const entries: any = genericCommandRegistryEntries();
  return {
    schema: 'narada.generic_command.registry.v1',
    count: entries.length,
    supported_command_schemas: entries.map((item: any) => item.command_schema),
    domains: [...new Set(entries.map((item: any) => item.domain))].sort(),
    entries,
  };
}

function entry(commandSchema: any, resultSchema: any, domain: any, mutationClass: any, compatibilityFacades: any, options: any = {}) : any{
...
function cloneEntry(item: any) : any{
  return {
    ...item,
