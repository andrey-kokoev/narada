...
import { runMailboxSync } from './graph-sync.js';
...
export function parseDaemonArgs(argv: any) : any{
  const args: any = {
    mode: 'dry-run',
...
export function usage() : any{
  return [
    'Usage: node tools/mailbox-sync/mailbox-sync-daemon.ts --scope-config <path> --scope-id <id> --runtime-root <path> --read-side-only [--dry-run|--live]',
    '',
    'Runs the bounded Microsoft Graph read-side mailbox sync and writes scheduler status evidence under runtime/.',
  ].join('\n');
}

export function runMailboxSyncDaemon(options: any = {}) : any{
  if (options.readSideOnly !== true) throw new Error('read_side_only_required');
  const runtimeRoot: any = resolve(options.runtimeRoot ?? `runtime/mailboxes/${options.scopeId ?? DEFAULT_SCOPE_ID}`);
  const statusPath: any = resolve(runtimeRoot, 'scheduler-status.json');
  const startedAt: any = new Date().toISOString();
  try {
    const report: any = runMailboxSync({
...
function writeSchedulerStatus(path: any, status: any) : any{
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, `${JSON.stringify(status, null, 2)}\n`, 'utf8');
}

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const args: any = parseDaemonArgs(process.argv.slice(2));
  if (args.help) {
    console.log(usage());
    process.exit(0);
  }
  const result: any = runMailboxSyncDaemon(args);
