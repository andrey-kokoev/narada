...
const DEFAULT_COMMAND_DIR = '.ai/tmp/mcp-commands';
const DEFAULT_RESULT_DIR = '.ai/tmp/mcp-results';
...
const COMMAND_REF_PATTERN = /^mcp_command:([A-Za-z0-9][A-Za-z0-9_-]{2,63})@v([1-9][0-9]*)$/;
const RESULT_REF_PATTERN = /^mcp_result:([A-Za-z0-9][A-Za-z0-9_-]{2,63})@v([1-9][0-9]*)$/;
...
const SUBSTRATE_CREATED_BY_VALUES = new Set(['codex', 'kimi', 'claude', 'gpt', 'openai', 'shell', 'powershell', 'node']);

function normalizeCreatedBy(input) {
  const suppliedCreatedBy = stringOrNull(input.created_by);
  ...
  const boundAgentId = stringOrNull(process.env.NARADA_AGENT_ID);
...
export function payloadRefFromEntry(entry) {
  if (typeof entry === 'string' && entry.trim().length > 0) return entry.trim();
  if (isPlainObject(entry)) {
    const ref = stringOrNull(entry.ref);
    if (ref) return ref;
  }
  return null;
}

export function payloadRefMetadataFromEntry(entry) {
  if (!isPlainObject(entry)) return {};
  const metadata = {};
  const role = stringOrNull(entry.role);
  const payloadSchema = stringOrNull(entry.payload_schema);
...
export function payloadCreate({ siteRoot, args, maxBytes = DEFAULT_MAX_BYTES, payloadDir = DEFAULT_PAYLOAD_DIR }) {
  ...
  const createdBy = normalizeCreatedBy(input);
...
function publicOutputShowRecord(record, { outputLimit = DEFAULT_OUTPUT_SHOW_CHAR_LIMIT, compact = false } = {}) {
...
function buildCompactOutputSummary(record) {
  const value = asRecord(record.full_output);
  const taskLike = summarizeTaskLikeOutput(value);
  const workboardLike = summarizeWorkboardLikeOutput(value);
  const chapterLike = summarizeChapterLikeOutput(value);
  return {
...
function summarizeTaskLikeOutput(value) {
  if (!value || typeof value !== 'object') return null;
  const taskNumber = value.task_number ?? value.spec?.task_number ?? value.lifecycle?.task_number ?? null;
  const taskId = value.task_id ?? value.spec?.task_id ?? value.lifecycle?.task_id ?? null;
  const lifecycle = value.lifecycle ?? null;
...
function summarizeWorkboardLikeOutput(value) {
  if (!value || typeof value !== 'object') return null;
...
function summarizeChapterLikeOutput(value) {
  if (!value || typeof value !== 'object') return null;
  const chapter = value.chapter ?? value;
...
function summarizeCriteria(value) {
  const rawCriteria = parseJsonMaybe(value.spec?.acceptance_criteria_json);
  const body = typeof value.body === 'string' ? value.body : '';
  const checked = (body.match(/^- \[x\]/gim) || []).length;
  const unchecked = (body.match(/^- \[ \]/gim) || []).length;
  const totalFromBody = checked + unchecked;
  return {
...
function collectResiduals(value) {
  const residuals = [];
  const projectionResiduals = value?.status_projection?.residuals ?? value?.chapter?.status_projection?.residuals;
  ...
  if (Array.isArray(value?.follow_up_policy?.candidates)) {
    residuals.push(...value.follow_up_policy.candidates.map((candidate) => candidate.reason ?? candidate.kind ?? 'follow_up_candidate'));
  }
  return residuals;
}

function summarizeOmittedSections(value) {
  const output = [];
...
export function listCommandTools() {
  return [
    {
...
export function commandCreate({
  ...
  const input = asRecord(args);
  const command = input.payload_ref
  ...
  const commandId = input.command_id ? validatePayloadId(String(input.command_id)) : randomCommandId();
  const createdAt = new Date().toISOString();
  const createdBy = normalizeCreatedBy(input);
...
export function commandShow({ siteRoot, args, maxBytes = DEFAULT_MAX_BYTES, commandDir = DEFAULT_COMMAND_DIR }) {
  const revision = readCommandRevision({ siteRoot, ref: requireRef(args, 'command_show_requires_ref'), maxBytes, commandDir });
...
export function commandValidate({
  ...
  const revision = readCommandRevision({ siteRoot, ref: requireRef(args, 'command_validate_requires_ref'), maxBytes, commandDir });
  const validation = validateCommandPacket({ siteRoot, command: revision.command, maxBytes, payloadDir });
  return {
...
export function commandSubmit({
  ...
  const input = asRecord(args);
  const revision = readCommandRevision({ siteRoot, ref: requireRef(input, 'command_submit_requires_ref'), maxBytes, commandDir });
  const validation = validateCommandPacket({ siteRoot, command: revision.command, maxBytes, payloadDir });
  const planned = buildCommandSubmitResult({
  ...
  const record = writeResultRevision({
    siteRoot,
    resultDir,
    result: planned,
    createdBy: process.env.NARADA_AGENT_ID || null,
...
export async function commandSubmitAsync({
  ...
  const planned = await buildCommandSubmitResultAsync({
    commandRef: revision.ref,
    command: revision.command,
    validation,
    admitter: admitters[revision.command.command_schema],
...
export async function commandAuthorAndSubmitAsync({
  ...
  const input = asRecord(args);
  const createdBy = normalizeCreatedBy(input);
  let payloadRef = stringOrNull(input.payload_ref);
  let payloadCreateResult = null;
  ...
  const payloadRefs = payloadRef ? [{ ref: payloadRef, role: 'domain_payload' }] : [];
  const command = {
  ...
  const commandCreateResult = commandCreate({
    siteRoot,
    args: {
  ...
  const submitResult = await commandSubmitAsync({
    siteRoot,
...
export function resultShow({ siteRoot, args, maxBytes = DEFAULT_MAX_BYTES, resultDir = DEFAULT_RESULT_DIR }) {
  const revision = readResultRevision({ siteRoot, ref: requireResultRef(args, 'result_show_requires_ref'), maxBytes, resultDir });
...
function buildCommandRecord({ commandId, revision, command, createdAt, createdBy, source, maxBytes }) {
  const commandJson = stableJson(command);
  const byteSize = Buffer.byteLength(commandJson, 'utf8');
...
function writeCommandRevision({ siteRoot, commandDir, record, overwrite }) {
  const path = commandPath({ siteRoot, commandDir, commandId: record.command_id, revision: record.revision });
  if (!overwrite && existsSync(path)) throw new Error(`command_revision_already_exists: ${record.ref}`);
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, `${stableJson(record)}\n`, 'utf8');
}

function readCommandRevision({ siteRoot, ref, maxBytes = DEFAULT_MAX_BYTES, commandDir = DEFAULT_COMMAND_DIR }) {
  const parsed = parseCommandRef(ref);
  const path = commandPath({ siteRoot, commandDir, commandId: parsed.commandId, revision: parsed.revision });
  let stat;
  try {
    stat = statSync(path);
  } catch {
...
function validateCommandRevisionRecord(record, parsed, statSize, maxBytes) {
...
function validateCommandPacket({ siteRoot, command, maxBytes, payloadDir }) {
  const errors = [];
  const warnings = [];
  const schema = stringOrNull(command.schema);
  const commandSchema = stringOrNull(command.command_schema);
  ...
  for (const [index, entry] of payloadRefs.entries()) {
    const ref = payloadRefFromEntry(entry);
    if (!ref) {
      errors.push({
  ...
  const supportedSchemas = supportedCommandSchemas();
  const supported = Boolean(commandSchema && supportedSchemas.has(commandSchema));
  if (commandSchema && !supported) errors.push({ code: 'unsupported_command_schema', field: 'command_schema', value: commandSchema });
  const submitPosture = supported ? 'validator_present_admitter_not_implemented' : 'refuse_without_domain_mutation';
...
function buildCommandSubmitResult({ commandRef, command, validation, admitter }) {
  ...
  if (typeof admitter === 'function') {
    try {
      const admitted = admitter(command);
      const domainStatus = admitted?.status ?? 'completed';
      const resultStatus = ['dry_run', 'planned'].includes(domainStatus)
        ? 'validated'
...
async function buildCommandSubmitResultAsync({ commandRef, command, validation, admitter }) {
  if (validation.errors.length > 0) {
    return {
      schema: 'narada.command_result.v1',
      command_ref: commandRef,
  ...
  if (typeof admitter === 'function') {
    try {
      const admitted = await admitter(command);
      const domainStatus = admitted?.status ?? 'completed';
      const resultStatus = ['dry_run', 'planned'].includes(domainStatus)
        ? 'validated'
        : (['success', 'submitted', 'promoted', 'acknowledged', 'dismissed', 'completed', 'exported', 'created', 'sent'].includes(domainStatus) ? 'admitted' : domainStatus);
...
function commandRefRemediation(error) {
...
function supportedCommandSchemas() {
  return supportedGenericCommandSchemas();
}

function writeResultRevision({ siteRoot, resultDir, result, createdBy, maxBytes }) {
  const resultId = randomResultId();
  const revision = 1;
  const ref = buildResultRef(resultId, revision);
  const createdAt = new Date().toISOString();
  const resultWithId = { ...result, result_id: resultId, created_at: createdAt };
  const resultJson = stableJson(resultWithId);
  const byteSize = Buffer.byteLength(resultJson, 'utf8');
  ...
  const path = resultPath({ siteRoot, resultDir, resultId, revision });
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, `${stableJson(record)}\n`, 'utf8');
  return record;
}

function readResultRevision({ siteRoot, ref, maxBytes = DEFAULT_MAX_BYTES, resultDir = DEFAULT_RESULT_DIR }) {
  const parsed = parseResultRef(ref);
  const path = resultPath({ siteRoot, resultDir, resultId: parsed.resultId, revision: parsed.revision });
  let stat;
  try {
    stat = statSync(path);
  } catch {
...
function validateResultRevisionRecord(record, parsed, statSize, maxBytes) {
...
function publicCommandResult({ status, record, includeCommand = false }) {
  return {
    status,
    ref: record.ref,
...
function publicResultRecord({ status, record, includeResult = false }) {
  return {
    status,
    ref: record.ref,
...
function parseCommandRef(ref) {
  ...
  const match = value.match(COMMAND_REF_PATTERN);
...
function requireResultRef(args, message, field = 'ref') {
  const value = asRecord(args)[field];
  if (typeof value !== 'string' || value.trim().length === 0) throw new Error(message);
  return value.trim();
}

function commandPath({ siteRoot, commandDir, commandId, revision }) {
  return resolve(siteRoot, commandDir, DEFAULT_WORKSPACE_DIR, commandId, `v${revision}.json`);
}

function resultPath({ siteRoot, resultDir, resultId, revision }) {
  return resolve(siteRoot, resultDir, DEFAULT_WORKSPACE_DIR, resultId, `v${revision}.json`);
}

function buildCommandRef(commandId, revision) {
  return `mcp_command:${commandId}@v${revision}`;
}

function buildResultRef(resultId, revision) {
