...
import { localExecutionEvidence } from '../src/server-wrapper.js';

test('launcher handoff remains ready when its detached parent is unavailable', () => {
  const session: any = 'launcher-handoff-test';
  const launcher: any = localExecutionEvidence({
    lifecycleBinding: { session_id: session },
    launchProcessContext: { createdByPid: null },
  }).find(({ component_kind }: any) => component_kind === 'launcher');
