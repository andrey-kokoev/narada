...
import {
  ...
  waitForEvent,
  recordLiveEvidence,
} from '../../agent-pi-tui/test/live-test-harness.js';
...
const rpcFixture: any = join(REPO_ROOT, 'packages', 'nars-intelligence-runtime-pi', 'test', 'fixtures', 'pi-rpc-fixture.ts');

const canonicalKeys: any = new Set(['event', 'event_kind', 'turn_state', 'terminal_state', 'terminal_status', 'content', 'status', 'source', 'source_kind', 'transport', 'delivery_mode', 'turn_attempt', 'attempt', 'tool_name', 'request_outcome']);
function canonicalEvent(event: any) {
  return Object.fromEntries([...canonicalKeys]
    .filter((key: any) => event[key] !== undefined && event[key] !== null)
    .sort()
    .map((key: any) => [key, event[key]]));
}

async function runKernel(kernelKind: any, kernelEnv: any = {}) {
  let site: any = null;
  let runtime: any = null;
  let pi: any = null;
  const providerRequestCountBefore: any = provider.requests.filter((request: any) => request.prompt === 'GAP_KERNEL_INPUT').length;
  try {
    ...
    const startup: any = readEvents(site.eventsPath).find((event: any) => event.event === 'session_started');
    const health: any = await fetch(runtime.healthEndpoint).then((response: any) => response.json());
    const observedKernelKind: any = health.intelligence_kernel_kind
      ?? health.intelligence?.intelligence_kernel_kind
      ?? health.kernel?.kernel_kind
      ?? health.intelligence?.kernel?.kernel_kind;
    const startEvidence: any = health.kernel_start_evidence ?? health.intelligence?.kernel_start_evidence;
    ...
    await waitForEvent(site.eventsPath, (event: any) => event.event === 'user_message' && event.content === 'GAP_KERNEL_INPUT', `${kernelKind}_user`);
    await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'rpc-ok', `${kernelKind}_assistant`);
    ...
    return {
      kernelKind,
      events: events
        .filter((event: any) => ['user_message', 'assistant_message', 'carrier_turn_started', 'carrier_turn_completed', 'turn_lifecycle_transition', 'turn_complete'].includes(event.event))
        .map(canonicalEvent),
      providerRequestCount: provider.requests.filter((request: any) => request.prompt === 'GAP_KERNEL_INPUT').length - providerRequestCountBefore,
...
try {
  const native: any = await runKernel('narada-native');
  const rpc: any = await runKernel('pi-rpc', {
  ...
  assert.equal(native.events.some((event: any) => event.event === 'pi_event_observed'), false);
  assert.equal(rpc.events.some((event: any) => event.event === 'pi_event_observed'), false);
  assert.equal(rpc.events.some((event: any) => event.kernel_kind || event.pi_event_kind), false, 'Pi diagnostics must not enter the canonical conversation projection');
  result = {
