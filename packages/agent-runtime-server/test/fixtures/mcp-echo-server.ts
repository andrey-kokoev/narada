...
const disconnectMarker: any = process.argv[2] ?? null;
const deniedSideEffectMarker: any = process.env.NARADA_MCP_FIXTURE_DENIED_MARKER ?? null;
const toolDelayMs: any = Math.max(
  0,
  Number(process.argv[3] ?? process.env.NARADA_MCP_FIXTURE_TOOL_DELAY_MS ?? 0) || 0,
);
const malformedResponse: any = process.env.NARADA_MCP_FIXTURE_MALFORMED === '1';
const malformedMarker: any = process.env.NARADA_MCP_FIXTURE_MALFORMED_MARKER ?? null;
...
process.stdin.on('data', (chunk: any) => {
  ...
  while (true) {
    ...
    if (newline === -1) return;
    const line: any = buffer.slice(0, newline);
