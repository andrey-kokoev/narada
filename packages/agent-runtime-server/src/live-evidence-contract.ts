export const LIVE_EVIDENCE_SCHEMA: any = 'narada.agent.live_evidence.v2';
 ...
 * posture name.
 */
export const LIVE_EVIDENCE_POSTURES: any = Object.freeze({
  FIXTURE_BOUNDARY: 'fixture-boundary',
  PARTIAL_PRODUCTION_LAUNCH: 'partial-production-launch',
  PRODUCTION_LAUNCH: 'production-launch',
});

function fail(message: any, details: any = {}) {
  const error: any = new Error(`live_evidence_contract:${message}`);
  error.code = 'live_evidence_contract_invalid';
  error.details = details;
  throw error;
}

function nonEmpty(value: any, field: any) {
  if (typeof value !== 'string' || !value.trim()) fail(`${field}_required`);
  return value.trim();
}

function arrayField(value: any, field: any) {
  if (!Array.isArray(value)) fail(`${field}_must_be_array`);
  return value;
}

function assertProductionBinding(binding: any) {
...
export function assertLiveEvidenceContract(evidence: any) {
  ...
  const postureValues: any = Object.values(LIVE_EVIDENCE_POSTURES);
  ...
  const isFixture: any = evidence.posture === LIVE_EVIDENCE_POSTURES.FIXTURE_BOUNDARY;
