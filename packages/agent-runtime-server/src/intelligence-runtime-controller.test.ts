...
import { createNarsIntelligenceRuntimeController, NarsIntelligenceInvocationError } from './intelligence-runtime-controller.js';
...
function planResult(overrides: any = {}) {
  return {
    kind: 'plan',
    intent: { id: 'intent:1' },
    plan: PLAN,
...
function createFixture({ busy = false, result = planResult() }: any = {}) {
  const calls: any = [];
  const transitions: any = [];
  let runtimeBusy: any = busy;
  const gateway: any = { async invoke(request: any) { calls.push(request); return result; } };
  const controller: any = createNarsIntelligenceRuntimeController({
    ...
    gateway,
    isBusy: () => runtimeBusy,
    onTransition: (record: any) => transitions.push(record),
  });
  return { controller, calls, transitions, setBusy: (value: any) => { runtimeBusy = value; } };
}
...
test('controller invokes the canonical gateway with stable delivery identity and exposes exact plan coordinates', async () => {
  ...
  const events: any = [];
  const response: any = await fixture.controller.callIntelligence(
    [{ role: 'user', content: 'hello' }],
    [{ name: 'search' }],
    { turnId: 'turn-1', inputEventId: 'input-1', invocationEventSink: async (event: any) => events.push(event) },
  );
...
test('equivalent reordered tool catalogs retain one canonical payload identity', async () => {
  ...
  const reorderedTools: any = [firstTools[1], firstTools[0]];
  await fixture.controller.callIntelligence(
    [{ role: 'user', content: 'same payload' }],
    firstTools,
...
test('controller preserves typed canonical refusal and does not manufacture an invocation result', async () => {
  ...
  const fixture: any = createFixture({ result: refusal });
  await assert.rejects(
    fixture.controller.callIntelligence([], [], { inputEventId: 'input-refused' }),
    (error: any) => error instanceof NarsIntelligenceInvocationError
...
test('controller refuses legacy provider/model/thinking reconfiguration inputs', async () => {
  const fixture: any = createFixture();
  const result: any = await fixture.controller.reconfigure({ request_id: 'legacy-1', provider: 'openai-api' });
  ...
  assert.deepEqual(fixture.transitions.map((record: any) => record.reconfiguration_state), [
...
test('controller activates an execution policy only at a clean turn boundary', async () => {
  ...
  const controller: any = createNarsIntelligenceRuntimeController({
    ...
    isBusy: () => busy,
    reconfigureExecutionPolicyFn: async (policy: any) => {
      received = policy;
  ...
  const refused: any = await controller.reconfigureExecutionPolicy(policy);
  assert.equal(refused.accepted, false);
  assert.equal(refused.reason, 'runtime_not_at_clean_turn_boundary');
  assert.equal(received, null);
  busy = false;
  const active: any = await controller.reconfigureExecutionPolicy(policy);
...
test('controller passes the privately admitted plan to the kernel switch', async () => {
  ...
  const controller: any = createNarsIntelligenceRuntimeController({
    ...
    reconfigureKernel: async (target: any, admittedPlan: any) => {
