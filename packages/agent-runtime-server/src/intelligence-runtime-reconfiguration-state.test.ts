...
} from './intelligence-runtime-reconfiguration-state.js';

test('intelligence runtime reconfiguration FSM admits only the clean lifecycle', () => {
  const machine: any = createNarsIntelligenceRuntimeReconfigurationStateMachine({ requestId: 'reconfigure-1' });
  assert.deepEqual([
...
test('intelligence runtime reconfiguration FSM refuses invalid terminal re-entry', () => {
  const machine: any = createNarsIntelligenceRuntimeReconfigurationStateMachine({ requestId: 'reconfigure-2' });
