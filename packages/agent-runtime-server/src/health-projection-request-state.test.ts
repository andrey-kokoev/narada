...
} from './health-projection-request-state.js';
...
test('health projection request FSM records successful child-runtime resolution', () => {
  ...
  assert.deepEqual(machine.history().map((record: any) => record.request_state), [
...
test('health projection request FSM distinguishes timeout from transport failure', () => {
  const timedOut: any = createMachine();
  ...
  const failed: any = createMachine();
  failed.transition('requested');
  failed.transition('failed', { error: 'child_stdin_unavailable' });
  assert.deepEqual(failed.history().map((record: any) => record.request_state), ['requested', 'failed']);
