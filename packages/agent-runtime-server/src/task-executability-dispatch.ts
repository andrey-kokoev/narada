const TASK_EXECUTABILITY_FOLLOW_UP_SCHEMA: any = 'narada.task.executability.follow_up.v1';
...
function boundedString(value: any, maxLength: any) {
  return typeof value === 'string' && value.trim() && value.length <= maxLength ? value.trim() : null;
}

function validDigest(value: any) {
...
function successfulToolResultFromEvent(event: any) {
...
function validateFollowUp(candidate: any) {
  ...
  const requestId: any = boundedString(candidate.request_id, MAX_ID_LENGTH);
  const taskId: any = boundedString(candidate.task_id, MAX_ID_LENGTH);
  const taskSpecDigest: any = boundedString(candidate.task_spec_digest, MAX_DIGEST_LENGTH);
  const environmentDigest: any = boundedString(candidate.environment_digest, MAX_DIGEST_LENGTH);
  const evaluatorProfile: any = boundedString(candidate.evaluator_profile, MAX_ID_LENGTH);
  const evaluatorProfileVersion: any = boundedString(candidate.evaluator_profile_version, MAX_ID_LENGTH);
...
export function createNarsTaskExecutabilityDispatchHook({
  dispatch,
  emit = () => {},
  schedule = (callback: any) => queueMicrotask(callback),
  ...
  const tracked: any = new Set();
  const pending: any = new Map();
  const deferred: any = new Set();
  let closed: any = false;

  const emitPosture: any = (event: any, followUp: any, extra: any = {}) => {
    emit({
  ...
  const run: any = async (key: any, followUp: any, lifecyclePayload: any) => {
    pending.delete(key);
  ...
  return {
    onToolResult(payload: any) {
      const sourceEvent: any = payload?.source_event;
      const followUp: any = validateFollowUp(followUpFromResult(successfulToolResultFromEvent(sourceEvent)));
      if (!followUp) return { status: 'ignored', reason: 'structured_follow_up_not_admitted' };
      const key: any = dispatchKey(followUp);
      ...
      const scheduled: any = () => { void run(key, followUp, payload); };
      try {
    ...
    async close({ reason = 'runtime_shutdown' }: any = {}) {
