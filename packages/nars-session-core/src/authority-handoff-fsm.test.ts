...
} from './authority-handoff-fsm.js';
import type { NaradaAuthorityHandoffState } from './authority-handoff-fsm.js';

test('authority handoff follows the explicit orchestration path', () => {
  let lifecycle = createNarsAuthorityHandoffLifecycle();
  for (const state of ['validating', 'preparing', 'draining', 'source_sealed', 'target_activating', 'committed'] as NaradaAuthorityHandoffState[]) {
