...
function startedEvent(sessionsRoot: string, sessionId: string, startedAt: string): Record<string, unknown> {
...
test('session index projects canonical agent id from identity when legacy field is absent', () => {
  const root = mkdtempSync(join(tmpdir(), 'nars-session-index-identity-'));
  const sessionsRoot = join(root, 'sessions');
  try {
    const event = startedEvent(sessionsRoot, 'session-identity', '2026-07-11T00:00:00.000Z');
    ...
    event.agent_identity_ref = {
      ...
      role: 'builder',
      canonical_agent_id: 'sonar.builder',
      display: 'sonar.builder',
    ...
    assert.ok(result);
    assert.equal(result.record.agent_id, 'sonar.builder');
    assert.equal(result.index.sessions[0]?.agent_id, 'sonar.builder');
  } finally {
...
test('session index records derive runtime_origin from authority_runtime_host', () => {
  ...
  try {
    ...
    assert.equal((localEntry.runtime_surface_contract as Record<string, unknown>).quadrant, 'local/local');
