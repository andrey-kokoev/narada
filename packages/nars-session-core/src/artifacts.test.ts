...
interface ArtifactError {
  code?: string;
  details?: { lifecycle_state?: unknown };
}
...
test('artifact registry persists revoke, expire, and archive transitions', () => {
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => {
        const typed = error as ArtifactError;
        return typed.code === 'artifact_not_active' && typed.details?.lifecycle_state === 'revoked';
      },
    );
    ...
    const expiredEntry = readNarsArtifactIndex({ sessionPath }).artifacts.find((entry) => entry.artifact_id === expired.record.artifact_id);
    assert.ok(expiredEntry);
    assert.equal(expiredEntry.lifecycle.state, 'expired');
  } finally {
...
test('artifact content reader refuses forged paths outside admitted roots', () => {
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => (error as ArtifactError).code === 'artifact_path_outside_admitted_roots',
    );
  } finally {
    rmSync(siteRoot, { recursive: true, force: true });
  }
