...
} from './authority-transition-fsm.js';
...
test('authority runtime host FSM permits the governed handoff path and terminal retirement', () => {
  ...
  for (const state of ['proposed', 'preparing_target', 'source_draining', 'source_sealed', 'target_activating', 'target_active', 'source_retired'] as NaradaAuthorityRuntimeHostTransitionState[]) {
...
test('authority runtime host FSM rejects reversal and missing boundary order', () => {
  ...
  assert.throws(
    () => createNarsAuthorityRuntimeHostTransitionStateMachine('unknown' as NaradaAuthorityRuntimeHostTransitionState),
