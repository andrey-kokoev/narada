export interface NarsSessionStatusState {
  requestOutcomeCounts?: Record<string, unknown>;
  requestIssueCounts?: Record<string, unknown>;
  sessionEventCount?: unknown;
  lastEventKind?: unknown;
  lastEventAt?: unknown;
  lastTerminalState?: unknown;
  activeTurnId?: unknown;
  activeTurnState?: unknown;
  lastTurnId?: unknown;
...
export interface NarsMcpServerSummaryInput {
  tools?: readonly NarsMcpToolInput[];
  process_ownership?: unknown;
}

export interface NarsMcpToolInput {
  name?: unknown;
  description?: unknown;
  inputSchema?: unknown;
}

export function createSessionActivitySnapshot(state: NarsSessionStatusState = {}): Record<string, unknown> {
  ...
  return {
    ...
    active_turn_id: state.activeTurnId ?? null,
    active_turn_state: state.activeTurnState ?? null,
    last_turn_id: state.lastTurnId ?? null,
    last_turn_state: state.lastTurnState ?? null,
    ...requestPosture,
...
export function createOperationalPostureSnapshot({ state = {}, mcpOperationalState = 'unknown' }: { state?: NarsSessionStatusState; mcpOperationalState?: string } = {}): Record<string, unknown> {
  ...
  else if (state.closed === true) posture = 'closed';
  return {
    operational_posture: posture,
    operational_posture_display: posture === 'healthy' ? 'healthy' : `${posture} [mcp=${mcpOperationalState}; request=${requestPosture}; lifecycle=${state.closed === true ? 'closed' : 'none'}]`,
    recommended_action: requestPosture === 'invalid_control_traffic' ? 'review_invalid_control_traffic' : state.closed === true ? 'session_closed' : 'review_session_summary',
    recommended_action_display: requestPosture === 'invalid_control_traffic' ? 'review invalid control traffic' : state.closed === true ? 'session closed' : 'review session summary',
  };
}

export function classifyRequestIssueOutcome(issueCode: unknown): 'invalid_request' | 'rejected_closed' | 'dispatch_failure' | 'request_error' {
...
export function summarizeRequestPosture(requestOutcomeCounts: Record<string, unknown> = {}): { request_outcome_total: number; request_posture: string; request_posture_display: string } {
  ...
  const requestPosture = order
    .map((key) => [key, counts[key]] as const)
    .sort((left, right) => right[1] - left[1] || order.indexOf(left[0]) - order.indexOf(right[0]))[0]?.[0] ?? 'runtime_failures';
  return { request_outcome_total: total, request_posture: requestPosture, request_posture_display: `${requestPosture} (${total})` };
}

export function sessionHandoffs({ identity, session, eventCount = 20 }: { identity?: unknown; session?: unknown; eventCount?: number } = {}): Record<string, string> {
...
export function summarizeCounts(counts: Record<string, unknown> = {}): string {
  const entries = Object.entries(counts).filter(([, value]) => Number(value ?? 0) > 0);
  if (entries.length === 0) return '0';
  return entries.map(([key, value]) => `${key}:${value}`).join(', ');
}

export function mcpServerSummaryEntries(mcpServers: Record<string, NarsMcpServerSummaryInput> = {}): Array<Record<string, unknown>> {
...
export function mcpToolCatalogEntries(mcpServers: Record<string, NarsMcpServerSummaryInput> = {}): Array<Record<string, unknown>> {
