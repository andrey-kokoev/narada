...
test('session core owns journal sequencing, queue persistence, artifacts, health, and recovery', async () => {
  ...
  try {
    ...
    assert.equal((artifact.public_record as unknown as Record<string, unknown>).source_path, undefined);
  ...
  try {
    ...
    const queue = first.createQueue({
      ...
      appendSessionFn: (event) => {
        ...
        if (eventRecord.event === 'input_event_completed') throw new Error('crash_before_queue_completion_evidence');
        return first.appendEvent(eventRecord);
      },
    });
...
test('turn FSM persists metadata, rejects illegal transitions, and rehydrates its terminal state', async () => {
  ...
  try {
    ...
    const firstTurn = first.turn('input_turn_fsm_1');
    assert.ok(firstTurn);
    assert.equal(firstTurn.turn_state, 'accepted');
    assert.equal(firstTurn.authority_posture, 'read_only');
    ...
    const secondTurn = second.turn('input_turn_fsm_1');
    assert.ok(secondTurn);
    assert.equal(secondTurn.turn_state, 'completed');
    assert.equal(secondTurn.attempt, 1);
