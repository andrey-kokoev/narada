...
} from './support/mcp-payload-file.mjs';

function validateIdentityAgainstRoster(root, agentId) {
  const role = String(agentId ?? '').split('.').pop();
  if (!agentId || !role) return { valid: false, error: 'missing_or_malformed_agent_id' };
  let siteConfig = {};
  try {
    siteConfig = JSON.parse(readFileSync(resolve(root, 'config.json'), 'utf8'));
  } catch (error) {
    return { valid: false, error: `site_config_unreadable: ${error instanceof Error ? error.message : String(error)}` };
  }
  const rolePolicy = siteConfig?.mcp?.role_policies?.[role];
  const shellPolicy = rolePolicy?.servers?.['narada-shell'];
  if (!shellPolicy) return { valid: false, error: `role_policy_missing_narada_shell: ${role}` };
  return {
    valid: true,
    role,
    capability_policy: {
      ...
      rules: ['Shell execution is mediated by this MCP server and scoped by Site config.'],
    },
  };
}
...
function gitPushCurrent(args, root) {
  ...
  if (!dryRun) {
    const result = runGitSync(work.repoRoot, ['push']);
    if (result.status !== 0) throw new Error(`git_push_current_failed: ${result.stderrText || result.stdoutText}`);
    postPush = readGitPushPlan(work.repoRoot);
  }

  const reportedPush = dryRun ? prePush : postPush;
  const payload = {
    status: 'ok',
    ...
    push_state: dryRun
      ? { planned: prePush }
      : { pre_push: prePush, post_push: postPush },
    residual_dirty_files: readGitDirtyFiles(work.repoRoot),
  };
  logGitOperation(root, payload, agentPolicy);
  return payload;
...
function gitCommitAndPushIncrement(args, root) {
  ...
  const push = gitPushCurrent(args, root);
  const residualsAfter = push.residual_dirty_files ?? readGitDirtyFiles(work.repoRoot);
  const payload = {
    status: 'ok',
    operation: 'git_commit_and_push_increment',
    agent_id: agentPolicy.agent_id,
...
function gitTaskCloseoutCommitAndPush(args, root) {
  ...
  if (pushResult.status !== 0) throw new Error(`git_task_closeout_commit_and_push_push_failed: ${pushResult.stderrText || pushResult.stdoutText}`);
  const pushAfter = readGitPushPlan(work.repoRoot);
...
function readGitPushPlan(repoRoot) {
  ...
  if (behind > 0) throw new Error(`git_push_current_refuses_behind_upstream: behind=${behind} ahead=${ahead}`);
