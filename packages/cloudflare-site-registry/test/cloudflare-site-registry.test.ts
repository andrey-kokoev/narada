...
type FakeState = { sites: AnyRecord[]; memberships: AnyRecord[]; settings: AnyRecord[]; operations: AnyRecord[]; carrierSessions: AnyRecord[]; authorityEvents: AnyRecord[] };
...
} from '../src/cloudflare-site-registry.js';
import { readCloudflareSiteRegistryLocalProjection } from '../src/cloudflare-site-registry-local-projection.js';
...
test('creates reads and lists site operations behind site authority', async () => {
  ...
  const registry: any = createD1CloudflareSiteRegistry(db, { now: fixedNow });
  ...
  assert.deepEqual(listed.operations.map((operation: AnyRecord) => operation.operation_id), ['operation_control']);

  const siteRead = await registry.readSite({ site_id: 'site_operations', principal: owner });
  assert.deepEqual(siteRead.operations.map((operation: AnyRecord) => operation.operation_id), ['operation_control']);
  assert.equal(siteRead.authority_events.some((event: AnyRecord) => event.event_kind === 'site_operation_updated'), true);
});
...
test('updates operation status behind owner or maintainer authority', async () => {
  ...
  assert.equal(readPaused.authority_events.some((event: AnyRecord) => event.event_kind === 'site_operation_status_updated'), true);
  const pauseEvent = readPaused.authority_events.find((event: AnyRecord) => event.event_kind === 'site_operation_status_updated');
  ...
  const refusedEvent = readClosed.authority_events.find(
    (event: AnyRecord) => event.event_kind === 'site_operation_status_update_rejected'
...
test('creates lists and reads authority-bearing sites', async () => {
  const registry: any = createD1CloudflareSiteRegistry(fakeD1SiteRegistryDatabase(), { now: fixedNow });
  ...
  assert.deepEqual(listed.sites.map((site: AnyRecord) => site.site_id), ['site_alpha']);
...
test('puts site membership behind owner or maintainer authority', async () => {
  ...
  assert.deepEqual(read.memberships.map((membership: AnyRecord) => [membership.principal_id, membership.role]), [
    ['user:owner', 'owner'],
    ['microsoft:tenant:operator', 'operator'],
  ]);
  assert.equal(read.authority_events.some((event: AnyRecord) => event.event_kind === 'site_membership_updated'), true);
...
test('rejects invalid or unauthorized site membership updates', async () => {
  ...
  assert.equal(denied.ok, false);
  assert.equal(denied.code, 'site_authority_denied');
  assert.equal(db.dump().authorityEvents.some((event: AnyRecord) => event.event_kind === 'site_membership_update_rejected'), true);
});
...
test('validates and records carrier session binding to registered site', async () => {
  ...
  assert.equal(read.authority_events.some((event: AnyRecord) => event.event_kind === 'carrier_site_binding_admitted'), true);
  ...
  assert.equal(db.dump().authorityEvents.some((event: AnyRecord) => event.event_kind === 'carrier_site_binding_admitted'), true);
});
...
test('rejects carrier binding for missing site mismatched site and missing authority', async () => {
  ...
  assert.equal(db.dump().authorityEvents.filter((event: AnyRecord) => event.action === 'deny').length, 3);
});

test('creates adapter only when a D1 binding is present', async () => {
  assert.equal(createCloudflareSiteRegistryAdapter({}), null);
  const adapter: any = createCloudflareSiteRegistryAdapter({ CLOUDFLARE_SITE_REGISTRY_DB: fakeD1SiteRegistryDatabase() }, { now: fixedNow });
...
test('reads secret-free local site registry projection', async () => {
  ...
  try {
    ...
    assert.deepEqual(projection.site_records.map((site: AnyRecord) => site.site_id), ['site_alpha', 'site_beta']);
...
function fakeD1SiteRegistryDatabase(): AnyRecord {
...
function fakeD1Statement(state: FakeState, sql: string): AnyRecord {
  let bound: any[] = [];
  return {
    bind(...values: any[]) {
      bound = values;
    ...
    async run() {
      if (/^INSERT INTO cloudflare_sites/i.test(sql)) {
        ...
        if (!state.sites.some((site: AnyRecord) => site.site_id === siteId)) {
        ...
        const existing = state.memberships.find((membership: AnyRecord) => membership.site_id === siteId && membership.principal_id === principalId);
        if (existing) {
        ...
        const existing = state.settings.find((setting: AnyRecord) => setting.site_id === siteId && setting.setting_key === settingKey);
        ...
        const existing = state.operations.find((operation: AnyRecord) => operation.operation_id === operationId);
        ...
        const existing = state.carrierSessions.find((binding: AnyRecord) => binding.carrier_session_id === carrierSessionId);
        ...
        if (!state.carrierSessions.some((binding: AnyRecord) => binding.carrier_session_id === carrierSessionId)) {
    ...
    async first() {
      if (/FROM cloudflare_sites WHERE site_id = \?/i.test(sql)) {
        const [siteId] = bound;
        return clone(state.sites.find((site: AnyRecord) => site.site_id === siteId));
      }
      if (/FROM cloudflare_site_memberships WHERE site_id = \? AND principal_id = \?/i.test(sql)) {
        const [siteId, principalId] = bound;
        return clone(state.memberships.find((membership: AnyRecord) => membership.site_id === siteId && membership.principal_id === principalId));
      }
      if (/FROM cloudflare_site_carrier_sessions WHERE carrier_session_id = \?/i.test(sql)) {
        const [carrierSessionId] = bound;
        return clone(state.carrierSessions.find((binding: AnyRecord) => binding.carrier_session_id === carrierSessionId));
      }
      if (/FROM cloudflare_site_operations WHERE operation_id = \?/i.test(sql)) {
        const [operationId] = bound;
        return clone(state.operations.find((operation: AnyRecord) => operation.operation_id === operationId));
      }
      return null;
    },
    async all() {
      if (/FROM cloudflare_sites s\s+JOIN cloudflare_site_memberships m/i.test(sql)) {
        ...
        return {
          results: state.memberships
            .filter((membership: AnyRecord) => membership.principal_id === principalId && membership.status === 'active')
            .map((membership: AnyRecord) => state.sites.find((site: AnyRecord) => site.site_id === membership.site_id && site.status === 'active'))
            .filter(Boolean)
      ...
      if (/FROM cloudflare_site_settings WHERE site_id = \?/i.test(sql)) {
        ...
        return {
          results: state.settings
            .filter((setting: AnyRecord) => setting.site_id === siteId)
            .sort((left: any, right: any) => left.setting_key.localeCompare(right.setting_key))
            .map(({ setting_key, value_json }: AnyRecord) => ({ setting_key, value_json })),
        };
      }
      if (/FROM cloudflare_site_memberships\s+WHERE site_id = \?/i.test(sql)) {
        ...
        return {
          results: state.memberships
            .filter((membership: AnyRecord) => membership.site_id === siteId)
            .sort((left: any, right: any) => left.created_at.localeCompare(right.created_at))
      ...
      if (/FROM cloudflare_site_operations/i.test(sql)) {
        ...
        return {
          results: state.operations
            .filter((operation: AnyRecord) => operation.site_id === siteId)
            .sort((left: any, right: any) => left.created_at.localeCompare(right.created_at))
            .slice(0, Number(limit))
            .map(clone),
        };
      ...
      if (/FROM cloudflare_site_carrier_sessions/i.test(sql)) {
        ...
        if (/WHERE operation_id = \?/i.test(sql)) {
          return {
            results: state.carrierSessions
              .filter((binding: AnyRecord) => binding.operation_id === siteId)
        ...
        return {
          results: state.carrierSessions
            .filter((binding: AnyRecord) => binding.site_id === siteId)
            .sort((left: any, right: any) => right.created_at.localeCompare(left.created_at))
            .slice(0, Number(limit))
            .map(clone),
        };
      ...
      if (/FROM cloudflare_site_authority_events/i.test(sql)) {
        ...
        return {
          results: state.authorityEvents
            .filter((event: AnyRecord) => event.site_id === siteId)
