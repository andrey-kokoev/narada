...
type DoctorOptions = AnyRecord;

export async function runMcpFabricDoctor(siteRoot: string, options: DoctorOptions = {}): Promise<AnyRecord> {
  const timeoutMs = Math.max(1, Number(options.timeoutMs ?? 5000));
  let fabric: AnyRecord;
...
export function renderMcpFabricDoctorTable(report: AnyRecord): string {
...
async function probeMcpServer({ siteRoot, serverName, server, sourceFile, configDiagnostic, timeoutMs, env }: AnyRecord): Promise<AnyRecord> {
  ...
  try {
    ...
    proc.stdout.on('data', (chunk: any) => {
      ...
      for (const line of lines) {
        ...
        try {
          const message: AnyRecord = JSON.parse(line);
...
function sendMcpDoctorRequest(proc: any, pending: Map<any, AnyRecord>, request: AnyRecord, timeoutMs: number, timeoutCode: string): Promise<any> {
  return new Promise<any>((resolve, reject) => {
    const timeout = setTimeout(() => {
      pending.delete(request.id);
      const error = new Error(`${timeoutCode}: ${request.method} timed out after ${timeoutMs}ms`);
      (error as any).code = timeoutCode;
      reject(error);
    }, timeoutMs);
...
function resolveServerEntrypoint(server: AnyRecord, siteRoot: string): AnyRecord | null {
...
function serverPathNormalization(server: AnyRecord): string {
...
function summarizeCommand(server: AnyRecord): string {
  return [basename(String(server.command ?? '')), ...(server.args ?? []).map(summarizeCommandArg)].join(' ');
}

function summarizeCommandArg(arg: any): string {
...
function doctorDiagnostic(error: any, phase: string): AnyRecord {
  return {
...
function entrypointRepairPlan(entrypoint: AnyRecord, configDiagnostic: AnyRecord | null): AnyRecord | null {
...
function buildGeneratedConfigDiagnostics(siteRoot: string, fabric: AnyRecord): AnyRecord {
  const registry = loadMcpSurfaceRegistry(siteRoot);
  const generatedConfigs = [];
  const serversByName: AnyRecord = {};
  ...
  for (const [serverName, serverValue] of Object.entries((fabric.servers ?? {}) as AnyRecord)) {
...
function surfaceProvenance(surface: AnyRecord, registryPath: string | null): AnyRecord {
  return {
...
function isIgnoredLocalProjection(siteRoot: string, path: string): boolean {
...
function shouldSuppressMcpDoctorStderr(_line: string): boolean {
  return false;
}

function stopDoctorProcess(proc: any): Promise<void> {
