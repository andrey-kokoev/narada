...
export function mcpFabricRepairPlan(code: string, details: AnyRecord = {}): AnyRecord | null {
  ...
  if (code === 'mcp_fabric_registry_mismatch') {
    return {
      ...
      missing: (details.missing ?? []).map((item: any) => ({
      ...
      server_name_mismatches: (details.serverNameMismatches ?? details.server_name_mismatches ?? []).map((item: any) => ({
        generated_file: item.generated_file,
