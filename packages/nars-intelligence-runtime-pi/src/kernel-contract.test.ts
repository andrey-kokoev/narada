...
function createRpcFixtureHost() {
  ...
  return Object.freeze({
    ...
    async runTurn(input: any, eventSink: any) {
...
async function runContractScenario(kernel: any) {
  const events: any = [];
  const methods: any = ['start', 'runTurn', 'steer', 'cancel', 'reconfigure', 'inspect', 'close'];
  for (const method of methods) assert.equal(typeof kernel[method], 'function', `${method} is required`);
  const started: any = await kernel.start({ session_id: 'contract-session', agent_id: 'contract-agent' });
  const result: any = await kernel.runTurn({
    turn_id: 'contract-turn',
    input_id: 'contract-input',
    messages: [{ role: 'user', content: 'hello' }],
    tools: [],
  }, (event: any) => events.push(event), noCapabilityGateway);
  const healthBeforeClose: any = await kernel.inspect();
  const cancel: any = await kernel.cancel({ reason: 'idle_cancel' });
  const reconfigured: any = await kernel.reconfigure({
    admitted_plan: {
      selected: {
  ...
  const closed: any = await kernel.close({ reason: 'contract_close' });
  ...
  assert.equal(events.some((event: any) => ['assistant_message', 'user_message', 'turn_complete'].includes(event.kind)), false);
  return {
    session_id: started.session_id,
    terminal_state: result.terminal_state,
    assistant: result.response.choices[0].message.content,
    canonical_event_kinds: events.filter((event: any) => typeof event.kind === 'string' && !event.kind.startsWith('pi_') && !event.kind.startsWith('kernel_')).map((event: any) => event.kind),
    health_fields: ['kernel_kind', 'kernel_version', 'kernel_state', 'active_turn_id', 'capability_profile'].every((field: any) => field in healthBeforeClose),
...
test('narada-native, Pi SDK, and Pi RPC satisfy one representation-neutral contract suite', async () => {
  const providerAdapter: any = { invoke: async () => providerOutcome() };
