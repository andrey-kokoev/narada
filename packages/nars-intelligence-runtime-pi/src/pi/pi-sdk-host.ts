...
function resourceOrFallback(value: any, fallback: any = null) {
...
function isPiModel(value: any) {
  return Boolean(value && typeof value === 'object' && nonEmpty(value.id) && nonEmpty(value.provider) && value.api);
}

function isRawCredentialKey(key: any) {
  const normalized: any = String(key).toLowerCase().replace(/[^a-z0-9]/g, '');
...
function assertNoRawCredentialMaterial(value: any, path: any = '$', seen: any = new Set()) {
  if (value == null || typeof value !== 'object' || seen.has(value)) return;
  seen.add(value);
  if (Array.isArray(value)) {
    value.forEach((item: any, index: any) => assertNoRawCredentialMaterial(item, `${path}[${index}]`, seen));
  } else {
...
function assertPinnedSdkVersion(actualVersion: any, expectedVersion: any) {
  const actual: any = nonEmpty(actualVersion);
  const expected: any = nonEmpty(expectedVersion);
...
function assertAdmittedModelBinding(model: any, config: any = {}) {
  if (!model || typeof model !== 'object') return model;
  const expectedProvider: any = resourceOrFallback(config.provider);
  const expectedModel: any = resourceOrFallback(config.model);
  const actualProvider: any = resourceOrFallback(model.provider);
  const actualModel: any = resourceOrFallback(model.id);
...
function assertExternalSessionIsolation(session: any) {
  if (!session || typeof session !== 'object') return session;
  const agentState: any = session.agent?.state;
  ...
  for (const candidateTools of [agentState?.tools, session.tools]) {
    ...
    const unsafeTool: any = candidateTools.find((tool: any) => tool && tool.nars_gateway_proxy !== true);
 ...
 */
async function disableExternalPiAutomaticRetry(session: any) {
  const retryOwners: any = [session, session?.agent];
  for (const owner of retryOwners) {
    if (typeof owner?.setAutoRetryEnabled === 'function') await owner.setAutoRetryEnabled(false);
  }
  const settingsManager: any = session?.settingsManager ?? session?.agent?.settingsManager;
  if (typeof settingsManager?.setRetryEnabled === 'function') await settingsManager.setRetryEnabled(false);

  const observedEnabled: any = retryOwners
    .map((owner: any) => owner?.autoRetryEnabled)
    .find((value: any) => value === true);
  const settingsRetryEnabled: any = typeof settingsManager?.getRetryEnabled === 'function'
...
async function createSdkSession(sdk: any, options: any) {
  ...
  if (typeof options.sessionFactory === 'function') {
    const result: any = await options.sessionFactory(options);
  ...
  if (typeof sdk.createAgentSession === 'function') {
    ...
    const hasModelRuntimeOptions: any = Boolean(
    ...
    const modelRuntime: any = options.modelRuntime
    ...
    let model: any = isPiModel(options.piModel)
      ? options.piModel
    ...
    const result: any = await sdk.createAgentSession({
      ...
      tools: Array.isArray(options.customTools)
        ? options.customTools.map((tool: any) => tool?.name).filter(Boolean)
        : [],
...
export function createPiSdkHost({
  ...
  let sessionUsesSdkModelRuntime: any = false;
  let currentTurnInput: any = null;
  let projectedModelRuntime: any = null;
  const explicitSdkHost: any = Boolean(
    sdk
    || sessionFactory
  ...
  const useBundledPiSdk: any = runtimeConfig.useBundledPiSdk === true;
  let resolvedSdk: any = sdk;
  const eventListeners: any = new Set();
  const emit: any = async (event: any) => {
    for (const listener of eventListeners) await listener(event);
  };
  async function buildSession(options: any) {
    ...
    const rawSession: any = await createSdkSession(options.sdk ?? resolvedSdk, options)
    ...
    const allowFallback: any = fallbackToCompatibilityHost === true;
  ...
  async function reconfigureSession(config: any = {}) {
    ...
    let modelObject: any = isPiModel(config?.modelObject) ? config.modelObject : null;
    ...
    const result: any = session?.reconfigure?.({
      ...config,
      ...nextConfig,
  ...
  return Object.freeze({
    ...
    async start(context: any = {}) {
      ...
      const modelRuntime: any = runtimeConfig.modelRuntime ?? projectedModelRuntime;
      modelRuntime?.setAdmittedBinding?.(activeConfig);
      const effectivePiVersion: any = piVersion
      ...
      const built: any = await buildSession(sessionOptions);
    ...
    async steer(input: any) { return session?.steer?.(input) ?? { accepted: false, reason: 'pi_host_not_started' }; },
    async cancel(reason: any) { return session?.cancel?.(reason) ?? { requested: false, reason: 'pi_host_not_started' }; },
    async reconfigure(config: any) { return reconfigureSession(config); },
    async recover({ context = null }: any = {}) {
      if (!started) return { continuation_state_discarded: false, reason: 'pi_host_not_started' };
      if (closed) throw new NarsKernelContractError('pi_host_closed', 'Pi SDK host is closed.');
      const previous: any = session;
      session = null;
