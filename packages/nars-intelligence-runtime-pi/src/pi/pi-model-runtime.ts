...
function modelForBinding(binding: any) {
  const provider: any = resourceId(binding?.provider);
  const model: any = resourceId(binding?.model);
...
function responseMessage(outcome: any, model: any) {
  const response: any = outcome?.response ?? outcome;
  const choice: any = Array.isArray(response?.choices) ? response.choices[0] : null;
  const source: any = choice?.message ?? (response?.role === 'assistant' ? response : null) ?? {};
  const content: any = [];
  ...
  const stopReason: any = outcome?.error
...
function createEventStream(outcomePromise: any, model: any) {
  const messagePromise: any = outcomePromise.then((outcome: any) => responseMessage(outcome, model));
  return {
    async *[Symbol.asyncIterator]() {
      const message: any = await messagePromise;
      let partial: any = { ...message, content: [] };
...
export function createNarsProjectedPiModelRuntime({ providerInvoker, getCurrentInput = () => null }: any = {}) {
  if (typeof providerInvoker !== 'function') throw new Error('pi_projected_provider_invoker_required');
  let binding: any = null;
  const runtime: any = {
    setAdmittedBinding(nextBinding: any = {}) {
      binding = {
    ...
    getModel(provider: any, model: any) {
      const requested: any = { provider: resourceId(provider), model: resourceId(model) };
    ...
    getModels(provider: any) {
      const model: any = runtime.getModel(binding?.provider, binding?.model);
      return model && (!provider || resourceId(provider) === model.provider) ? [model] : [];
    },
    getProviders() {
      const model: any = modelForBinding(binding);
    ...
    hasConfiguredAuth(provider: any) {
      return Boolean(binding?.provider && resourceId(provider) === binding.provider);
    },
    checkAuth(provider: any) {
      return runtime.hasConfiguredAuth(provider);
    },
    getAuth: async () => undefined,
    isUsingOAuth: () => false,
    streamSimple(model: any, context: any, options: any = {}) {
      const input: any = getCurrentInput() ?? {};
      const turnInvoker: any = typeof input.providerInvoker === 'function' ? input.providerInvoker : providerInvoker;
      ...
      const outcomePromise: any = Promise.resolve().then(() => turnInvoker(providerInput));
      return createEventStream(outcomePromise, model);
    },
    completeSimple: async (model: any, context: any, options: any = {}) => {
      const stream: any = runtime.streamSimple(model, context, options);
