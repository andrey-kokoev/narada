...
const forbiddenKeys: any = new Set([
  'apikey',
  'accesstoken',
  'clientsecret',
  'refreshtoken',
...
function isCredentialKey(key: any) {
...
function hasCredentialMaterial(value: any, path: any = '$') {
  if (!value || typeof value !== 'object') return null;
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      const found: any = hasCredentialMaterial(value[index], `${path}[${index}]`);
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    const found: any = hasCredentialMaterial(nested, `${path}.${key}`);
    if (found) return found;
  }
  return null;
}
...
export function createPiRuntimeIsolationConfig({
  ...
  const credentialPath: any = hasCredentialMaterial(extra);
  if (credentialPath) {
  ...
  if (tools.some((tool: any) => tool?.native === true || tool?.source === 'ambient')) {
    throw new NarsKernelContractError('pi_native_tool_forbidden', 'Pi native or ambient tools cannot be admitted.');
  }
  if (tools.some((tool: any) => tool?.nars_gateway_proxy !== true)) {
  ...
  return Object.freeze({
    ...
    mode,
    credential_refs: Object.freeze([...credentialRefs].map((ref: any) => String(ref))),
    tools: Object.freeze([...tools]),
  });
}

export function assertPiRuntimeIsolation(config: any) {
  ...
  const expectedBoundary: any = config.mode === 'rpc' ? 'filtered-child-process' : 'in-process-adapter';
  const expectedEnforcement: any = config.mode === 'rpc'
    ? 'filtered-environment-disposable-cwd'
    : 'configuration-and-adapter-checks';
