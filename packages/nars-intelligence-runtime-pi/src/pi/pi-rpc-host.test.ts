...
import { createPiRpcHost } from './pi-rpc-host.js';
...
test('Pi RPC host correlates JSONL requests and normalizes events', async () => {
  const events: any = [];
  const host: any = createPiRpcHost({ command: process.execPath, args: [fixture], piVersion: 'fixture-1.0.0' });
  const started: any = await host.start({ session_id: 'session-rpc', agent_id: 'agent-rpc' });
  assert.equal(started.negotiation.mode, 'rpc');
  const response: any = await host.runTurn({ turn_id: 'turn-rpc', messages: [{ role: 'user', content: 'hi' }] }, (event: any) => events.push(event));
...
test('Pi RPC preserves an authenticated provider refusal without treating it as success', async () => {
  const host: any = createPiRpcHost({
  ...
  await assert.rejects(host.runTurn({ turn_id: 'auth-failure-turn', messages: [] }), (error: any) => {
...
test('Pi RPC host restarts a crashed process without resending the uncertain turn', async () => {
  const root: any = mkdtempSync(join(tmpdir(), 'narada-pi-rpc-recovery-'));
  ...
  const recovery: any = await host.recover();
  assert.equal(recovery.process_restarted, true);
  const result: any = await host.runTurn({ turn_id: 'fresh-turn', messages: [] });
  ...
  const turnRequests: any = readFileSync(requestLogFile, 'utf8')
    ...
    .map((request: any) => request.params.turn_id);
...
test('Pi RPC recovery restarts with the exact admitted provider binding', async () => {
  const root: any = mkdtempSync(join(tmpdir(), 'narada-pi-rpc-reconfigure-recovery-'));
  const crashOnceFile: any = join(root, 'crash-once');
  const requestLogFile: any = join(root, 'requests.jsonl');
  const host: any = createPiRpcHost({
    command: process.execPath,
  ...
  const starts: any = readFileSync(requestLogFile, 'utf8')
    .trim()
    .split('\n')
    .map((line: any) => JSON.parse(line))
    .filter((request: any) => request.method === 'start');
