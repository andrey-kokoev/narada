...
export const PI_TOOL_POSTURE_VERSION: any = 'nars-gateway-only.v1';
export const SUPPORTED_PI_EVENT_KINDS: any = Object.freeze([
  'assistant_token',
  'assistant_message',
  'provider_telemetry',
  'usage',
...
export const SUPPORTED_PI_CAPABILITIES: any = Object.freeze([
  'provider-cognition',
...
export function negotiatePiCapabilities({
  ...
  if (capabilities.some((capability: any) => typeof capability !== 'string' || !capability.trim())
    || eventKinds.some((kind: any) => typeof kind !== 'string' || !kind.trim())) {
  ...
  const missing: any = required.filter((capability: any) => !capabilities.includes(capability));
  const unsupportedCapabilities: any = capabilities.filter((capability: any) => !SUPPORTED_PI_CAPABILITIES.includes(capability));
  const unsupportedEvents: any = eventKinds.filter((kind: any) => !SUPPORTED_PI_EVENT_KINDS.includes(kind));
