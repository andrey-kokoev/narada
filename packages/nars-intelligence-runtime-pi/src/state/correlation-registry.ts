...
export function createCorrelationRegistry({ maxEntries = 2048 }: any = {}) {
  ...
  const limit: any = Math.max(1, Math.trunc(Number(maxEntries) || 2048));
  const records: any = new Map();
  let nextSequence: any = 0;
  function keyFor(record: any) {
  ...
  function register(record: any = {}) {
    ...
    const key: any = keyFor(normalized);
    const existing: any = records.get(key);
    const sameInvocation: any = existing
      && ['runtime_request_id', 'input_id', 'idempotency_key', 'turn_id', 'pi_session_id']
        .every((field: any) => existing[field] === normalized[field]);
    ...
    if (existing && sameInvocation) {
      const refreshed: any = Object.freeze({
        ...existing,
        ...normalized,
        correlation_key: key,
  ...
  return Object.freeze({
    register,
    get: (key: any) => records.get(String(key)) ?? null,
