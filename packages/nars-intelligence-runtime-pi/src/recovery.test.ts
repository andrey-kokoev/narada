...
const rpcFixture: any = fileURLToPath(new URL('../test/fixtures/pi-rpc-fixture.ts', import.meta.url));
...
test('Pi host failure leaves a failed kernel that recovers from NARS records without redispatch', async () => {
  ...
  const host: any = Object.freeze({
    ...
    async reconfigure(config: any) { return { active: config }; },
    async close() {},
  ...
  const recovery: any = await kernel.recover({
...
test('an injected SDK session factory is used as the Pi cognition host', async () => {
  ...
  const result: any = await host.runTurn({ turn_id: 'factory-turn' });
...
test('an SDK-shaped session factory envelope is unwrapped behind the host boundary', async () => {
  ...
  const result: any = await host.runTurn({ turn_id: 'factory-envelope-turn' });
...
test('a prompt-based SDK session receives the admitted context and normalizes SDK events', async () => {
  ...
  let gatewayInvocation: any = null;
  let unsubscribeCalls: any = 0;
  const host: any = createPiSdkHost({
    piVersion: 'fixture-sdk-1.0.0',
    sessionFactory: async () => ({
      session: {
        ...
        async prompt(prompt: any, options: any) {
          const toolResult: any = await options.customTools[0].execute('sdk-call-1', { note: 'x' });
  ...
  const result: any = await host.runTurn({
    turn_id: 'prompt-turn',
...
test('prompt-based SDK completion is projected back as provider output without making SDK events canonical', async () => {
  ...
  const result: any = await host.runTurn({ turn_id: 'event-turn', messages: [{ role: 'user', content: 'hello' }] }, (event: any) => events.push(event));
...
test('Pi recovery is refused while a turn is still active', async () => {
  ...
  const host: any = Object.freeze({
    ...
    async runTurn() {
      await new Promise((resolve: any) => { release = resolve; });
  ...
  const turn: any = kernel.runTurn({ turn_id: 'active-recovery-turn', input_id: 'active-recovery-input' }, async () => {}, noCapabilityGateway);
...
test('Pi RPC kernel recovery reconstructs the host without redispatching an uncertain turn', async () => {
  const root: any = mkdtempSync(join(tmpdir(), 'narada-pi-rpc-kernel-recovery-'));
  const crashOnceFile: any = join(root, 'crash-once');
  const requestLogFile: any = join(root, 'requests.jsonl');
  ...
  try {
    ...
    const evidence: any = await kernel.recover({
    ...
    assert.equal((await kernel.inspect()).kernel_state, 'ready');
    const result: any = await kernel.runTurn({ turn_id: 'fresh-turn', input_id: 'fresh-input', messages: [] }, async () => {}, noCapabilityGateway);
    assert.equal(result.response.choices[0].message.content, 'rpc-ok');

    const turnRequests: any = readFileSync(requestLogFile, 'utf8')
      .trim()
      .split('\n')
      .map((line: any) => JSON.parse(line))
      .filter((request: any) => request.method === 'turn')
      .map((request: any) => request.params.turn_id ?? request.params.turnId);
