...
function gatewayFailure(name: any, reason: any, error: any, context: any = {}) {
  return {
...
function redactErrorMessage(error: any) {
...
function refusal(name: any, reason: any, context: any = {}) {
  return {
...
export function isNativePiToolName(name: any) {
  const normalized: any = String(name ?? '').trim().toLowerCase();
...
export function normalizeNarsGatewayTool(tool: any) {
  const name: any = toolName(tool);
  if (!name) throw new NarsKernelContractError('pi_gateway_tool_name_required', 'NARS gateway tools require a name.');
  const sourceShape: any = tool && typeof tool === 'object' ? tool : {};
  const functionShape: any = sourceShape.function && typeof sourceShape.function === 'object'
  ...
  const parameters: any = functionShape.parameters ?? functionShape.input_schema ?? { type: 'object', properties: {} };
  ...
  const capabilityIdentity: any = functionShape.capability_identity ?? functionShape.capabilityIdentity ?? tool.capability_identity ?? tool.capabilityIdentity;
  const authorityPosture: any = functionShape.authority_posture ?? functionShape.authorityPosture ?? tool.authority_posture ?? tool.authorityPosture;
...
export function createNarsPiCapabilityGateway({
  ...
  const invokeCapability: any = typeof gateway?.invoke === 'function'
  ...
  const contextSnapshot: any = Object.freeze({
  ...
  let admittedCatalogNames: any = null;
  const api: any = {
    async listTools() {
      const catalog: any = typeof gateway.toolCatalog === 'function' ? await gateway.toolCatalog() : [];
      ...
      const normalizedTools: any = catalog.map(normalizeNarsGatewayTool);
      ...
      const normalized: any = Object.freeze(normalizedTools);
      admittedCatalogNames = new Set(normalized.map((tool: any) => tool.function.name));
      return normalized;
    },
    async execute(request: any = {}) {
      const name: any = toolName(request);
      const toolCallId: any = request.tool_call_id ?? request.toolCallId ?? null;
      const invocationContext: any = {
        ...contextSnapshot,
      ...
      if (!name || isNativePiToolName(name)) {
        const denied: any = refusal(name, 'native_pi_tool_not_admitted', invocationContext);
        await eventSink({
      ...
      if (admittedCatalogNames == null) {
        try {
          ...
          const unknown: any = gatewayFailure(name, 'gateway_catalog_unavailable', error, invocationContext);
      ...
      if (!admittedCatalogNames.has(name)) {
        const denied: any = refusal(name, 'tool_not_in_admitted_catalog', invocationContext);
...
export function createPiGatewayToolProxies({ tools = [], gateway = null, context = {}, eventSink }: any = {}) {
  if (gateway) return createNarsPiCapabilityGateway({ gateway, context, eventSink });
  const normalized: any = tools.map(normalizeNarsGatewayTool);
  return Object.freeze({
    async listTools() { return normalized; },
    async execute(request: any = {}) {
      const name: any = toolName(request);
      if (!normalized.some((tool: any) => tool.function.name === name)) return refusal(name, 'tool_not_in_admitted_catalog', context);
