...
import { createPiEventAdapter, normalizePiEvent } from './event-adapter.js';

test('Pi event normalization classifies fixtures without emitting canonical NARS events', async () => {
  const events: any = [];
  const adapter: any = createPiEventAdapter({ eventSink: async (event: any) => events.push(event), now: () => '2026-07-21T00:00:00.000Z' });
  const token: any = await adapter.observe({ type: 'assistant_token', id: 'token-1', sequence: 1, content: 'a' }, { turnId: 'turn-1' });
  const duplicate: any = await adapter.observe({ type: 'assistant_token', id: 'token-1', sequence: 1, content: 'a' }, { turnId: 'turn-1' });
  const malformed: any = normalizePiEvent({ id: 'bad' }, { turnId: 'turn-1' });
  const outOfOrder: any = normalizePiEvent({ type: 'retry', sequence: 1 }, { turnId: 'turn-1', sequence: 4 });
...
test('Pi event fixtures cover every normalized observation class and redact secrets', async () => {
  const events: any = [];
  const adapter: any = createPiEventAdapter({ eventSink: async (event: any) => events.push(event) });
  const fixtures: any = [
  ...
  for (const [kind, classification] of fixtures) {
    const normalized: any = await adapter.observe({
      kind,
      id: `fixture-${kind}`,
  ...
  assert.equal(events.some((event: any) => ['user_message', 'assistant_message', 'tool_requested', 'turn_complete'].includes(event.kind)), false);
...
test('Pi SDK event vocabulary is normalized before NARS observes it', async () => {
  ...
  assert.equal(events.some((event: any) => event.event === 'assistant_message'), false);
  assert.equal(events.some((event: any) => event.kind === 'pi_event_unsupported'), false);
});
...
test('Pi agent-settled lifecycle evidence remains diagnostic provider telemetry', async () => {
  const events: any = [];
  const adapter: any = createPiEventAdapter({ eventSink: async (event: any) => events.push(event) });
  const normalized: any = await adapter.observe({ type: 'agent_settled', id: 'settled-1' }, { turnId: 'settled-turn' });
  assert.equal(normalized.classification, 'provider_telemetry');
  assert.equal(events[0].kind, 'pi_event_observed');
...
test('Pi observations are emitted in adapter order when callbacks overlap', async () => {
  ...
  const firstSink: any = new Promise((resolve: any) => { releaseFirst = resolve; });
  let sinkCalls: any = 0;
  const adapter: any = createPiEventAdapter({
  ...
  const first: any = adapter.observe({ kind: 'assistant_token', id: 'ordered-1', sequence: 1, content: 'a' });
  const second: any = adapter.observe({ kind: 'assistant_token', id: 'ordered-2', sequence: 2, content: 'b' });
  await new Promise((resolve: any) => setTimeout(resolve, 5));
  assert.equal(events.length, 1);
  assert.equal(events[0].pi_event_id, 'ordered-1');
  releaseFirst();
  await Promise.all([first, second]);
  assert.deepEqual(events.map((event: any) => event.pi_event_id), ['ordered-1', 'ordered-2']);
  assert.deepEqual(events.map((event: any) => event.kernel_observation_sequence), [1, 2]);
});
