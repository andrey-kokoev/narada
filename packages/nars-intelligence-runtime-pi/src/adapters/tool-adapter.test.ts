...
import { createNarsPiCapabilityGateway, isNativePiToolName, normalizeNarsGatewayTool } from './tool-adapter.js';
...
test('gateway proxy carries correlation and never confirms an external effect', async () => {
  const calls: any = [];
  const gateway: any = createNarsPiCapabilityGateway({
    ...
    gateway: {
      ...
      invoke: async (request: any) => { calls.push(request); return { status: 'completed', result: { text: 'ok' } }; },
    },
  });
  assert.equal((await gateway.listTools())[0].nars_gateway_proxy, true);
  const result: any = await gateway.execute({ tool_name: 'read_note', tool_call_id: 'call-1', arguments: { path: 'x' } });
...
test('refused native tool is structured', async () => {
  const events: any = [];
  const gateway: any = createNarsPiCapabilityGateway({
    eventSink: async (event: any) => events.push(event),
    gateway: { invoke: async () => { throw new Error('must not run'); } },
  });
  const result: any = await gateway.execute({ tool_name: 'bash', arguments: { command: 'echo unsafe' } });
...
test('execute-only capability gateways remain supported without widening Pi authority', async () => {
  ...
  const result: any = await gateway.execute({ tool_name: 'read_note', tool_call_id: 'call-execute-only' });
