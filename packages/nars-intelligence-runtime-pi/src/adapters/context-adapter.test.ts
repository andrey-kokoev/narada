...
import { buildPiContextFromNarsRecords } from './context-adapter.js';

test('Pi context is rebuilt from NARS records and remains reconstructable after Pi state loss', () => {
  const context: any = buildPiContextFromNarsRecords({
  ...
  assert.deepEqual(context.messages.map((message: any) => message.content), ['old input', 'old response', 'new input']);
...
test('context reconstruction includes explicitly recorded admitted tool results', () => {
  ...
  assert.deepEqual(context.messages.map((message: any) => message.role), ['user', 'tool']);
