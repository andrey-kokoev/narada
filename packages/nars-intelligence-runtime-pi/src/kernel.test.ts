...
import { createIntelligenceKernel, createNarsPiSdkKernel } from './kernel.js';

function providerResult(content: any = 'ok') {
...
test('Pi SDK kernel runs a turn through the admitted provider and normalizes observations', async () => {
  const events: any = [];
  let calls: any = 0;
  const kernel: any = createNarsPiSdkKernel({
    providerAdapter: { invoke: async (input: any) => { calls += 1; assert.equal(input.tools[0].function.name, 'read_note'); return providerResult('pi-ok'); } },
    now: () => '2026-07-21T00:00:00.000Z',
  });
  const started: any = await kernel.start({ session_id: 'session-1', agent_id: 'agent-1' });
  assert.equal(started.kernel_kind, 'pi-sdk');
  const result: any = await kernel.runTurn({
  ...
  assert.ok(events.some((event: any) => event.kind === 'pi_event_observed'));
  assert.equal(events.some((event: any) => event.kind === 'assistant_message'), false);
  const health: any = await kernel.inspect();
...
test('kernel refuses native tools before provider execution', async () => {
  let calls: any = 0;
  const kernel: any = createNarsPiSdkKernel({ providerAdapter: { invoke: async () => { calls += 1; return providerResult(); } } });
...
test('Pi SDK kernel requires a complete NARS capability gateway before reserving a turn', async () => {
  const kernel: any = createNarsPiSdkKernel({ providerAdapter: { invoke: async () => providerResult() } });
...
test('admitted provider stream metadata becomes normalized NARS stream evidence before Pi reduces it', async () => {
  ...
  await kernel.runTurn({ turn_id: 'stream-turn', input_id: 'stream-input' }, (event: any) => events.push(event), noCapabilityGateway);
  assert.deepEqual(events.filter((event: any) => event.kind === 'assistant_message_stream').map((event: any) => ({
...
test('Pi SDK provider errors remain failed NARS outcomes instead of assistant success', async () => {
  ...
  const result: any = await kernel.runTurn({ turn_id: 'failure-turn', input_id: 'failure-input' }, async () => {}, noCapabilityGateway);
...
test('Pi compaction observations become NARS-owned evidence without deleting canonical history', async () => {
  ...
  const host: any = Object.freeze({
    ...
    async runTurn(_input: any, eventSink: any) {
      await eventSink({
        type: 'compaction_end',
        id: 'compaction-1',
        sequence: 1,
  ...
  const kernel: any = createNarsPiSdkKernel({ providerAdapter: { invoke: async () => providerResult() }, host });
  await kernel.start({ session_id: 'compaction-session', agent_id: 'compaction-agent' });
  await kernel.runTurn({ turn_id: 'compaction-turn', input_id: 'compaction-input' }, (event: any) => events.push(event), noCapabilityGateway);
  const evidence: any = events.find((event: any) => event.kind === 'pi_compaction_evidence');
  assert.ok(evidence);
...
test('cancellation is evidence rather than an immediate completion claim', async () => {
  ...
  const kernel: any = createNarsPiSdkKernel({
    providerAdapter: { invoke: async ({ abortSignal }: any) => new Promise((resolve: any) => {
  ...
  const turn: any = kernel.runTurn({ turn_id: 'turn-3', input_id: 'input-3', messages: [{ role: 'user', content: 'wait' }] }, async () => {}, noCapabilityGateway);
  await new Promise((resolve: any) => setTimeout(resolve, 10));
  const cancellation: any = await kernel.cancel({ reason: 'operator_cancel' });
...
test('reconfiguration is admitted only at a clean turn boundary and health is redacted', async () => {
  const kernel: any = createIntelligenceKernel({ kind: 'pi-sdk', providerAdapter: { invoke: async () => providerResult() } });
  await kernel.start({ session_id: 'session-4', agent_id: 'agent-4' });
  const result: any = await kernel.reconfigure({
    admitted_plan: {
      selected: {
...
test('kernel reconfiguration consumes the exact admitted plan binding', async () => {
  ...
  const host: any = Object.freeze({
    ...
    async reconfigure(config: any) { reconfigurations.push(config); return { active: config }; },
    async close() {},
