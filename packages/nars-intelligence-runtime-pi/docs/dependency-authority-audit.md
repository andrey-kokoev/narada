...
| --- | --- | --- |
| Agent and NARS session identity | `@narada-core/nars-session-core` / `@narada-core/agent-runtime-server` | Carries the admitted identifiers for correlation only |
...
| Provider cognition mechanics | `@narada-core/nars-pi-kernel` | Runs the isolated Pi SDK/RPC substrate inside one admitted turn |

The native kernel and this Pi kernel are selected behind the same
`@narada-core/nars-intelligence-kernel-contract`. The operator surface is a
...
| --- | --- | --- |
| `@narada-core/nars-intelligence-kernel-contract` | Representation-neutral public contract and native conformance adapter | Contains no Pi SDK types and owns no durable session state |
