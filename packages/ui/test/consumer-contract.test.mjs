...
test('shared UI package exports and consumer direction are explicit', async () => {
  ...
  const agentWebUiPackage = await readJson(resolve(agentWebUiRoot, 'package.json'));
  ...
  assert.equal(agentWebUiPackage.dependencies['@narada2/ui'], 'workspace:*');
  assert.equal(agentWebUiPackage.dependencies['@narada2/ui-vue'], 'workspace:*');
});
...
test('consumers import shared foundation before app-specific layers', async () => {
  ...
  assert.match(agentStyles, /@import "@narada2\/ui\/styles\.css";/);
  assert.match(agentStyles, /@import "@narada2\/ui-vue\/components\.css"/);
