...
class CloseAfterMarkerWebSocket {
  ...
  addEventListener(type: any, listener: any, options: any) {
    this.inner.addEventListener(type, (event: any) => {
      listener(event);
  ...
  send(data: any) {
    ...
    const frame: any = JSON.parse(String(data));
...
try {
  ...
  const ambiguous: any = await attachClient(runtime, {
  ...
  const ambiguousResult: any = await ambiguous.client.submit('GAP_AMBIGUOUS', { idempotencyKey: 'gap-ambiguous-key' });
  assert.equal(ambiguousResult.transport, 'ambiguous');
  assert.equal(ambiguousResult.retryAllowed, false);
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'GAP_AMBIGUOUS_ASSISTANT', 'ambiguous_forwarded_completion');
  const ambiguousProviderCount: any = provider.requests.filter((request: any) => request.prompt.includes('GAP_AMBIGUOUS')).length;
  await new Promise((resolvePromise: any) => setTimeout(resolvePromise, 800));
  assert.equal(provider.requests.filter((request: any) => request.prompt.includes('GAP_AMBIGUOUS')).length, ambiguousProviderCount, 'ambiguous transport must not auto-resend');
  const stable: any = await attachClient(runtime, { reconnect: false, subscriptionId: `gap-stable-${Date.now()}` });
  clients.push(stable);
  const explicitRetry: any = await stable.client.submit('GAP_AMBIGUOUS', { idempotencyKey: 'gap-ambiguous-key' });
  assert.equal(explicitRetry.transport, 'written');
  await new Promise((resolvePromise: any) => setTimeout(resolvePromise, 400));
  assert.equal(provider.requests.filter((request: any) => request.prompt.includes('GAP_AMBIGUOUS')).length, ambiguousProviderCount, 'same-key explicit retry must reuse the durable operation');

  const first: any = await attachClient(runtime, {
    reconnect: true,
  ...
  const writer: any = await attachClient(runtime, { reconnect: false, subscriptionId: `gap-writer-${Date.now()}` });
  clients.push(writer);
  await writer.client.submit('GAP_OVERLAP_FIRST', { idempotencyKey: 'gap-overlap-first-key' });
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'GAP_OVERLAP_FIRST_ASSISTANT', 'overlap_first_durable');
  ...
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'GAP_OVERLAP_DURING_ASSISTANT', 'overlap_during_durable');
  await waitFor(() => first.client.getState().phase === 'live', 'overlap_reconnected');
  await waitFor(() => first.events.filter((event: any) => event.content === 'GAP_OVERLAP_DURING_ASSISTANT').length === 1, 'overlap_event_replayed_once');
  assert.equal(first.events.filter((event: any) => event.content === 'GAP_OVERLAP_FIRST_ASSISTANT').length, 1);
  assert.equal(first.events.filter((event: any) => event.content === 'GAP_OVERLAP_DURING_ASSISTANT').length, 1);

  const raceA: any = await attachClient(runtime, { reconnect: false, subscriptionId: `gap-race-a-${Date.now()}` });
  const raceB: any = await attachClient(runtime, { reconnect: false, subscriptionId: `gap-race-b-${Date.now()}` });
  clients.push(raceA, raceB);
  const [raceResultA, raceResultB]: any = await Promise.all([
  ...
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'GAP_IDEMPOTENT_ASSISTANT', 'idempotency_assistant');
  await new Promise((resolvePromise: any) => setTimeout(resolvePromise, 500));
  const durableRaceInputs: any = readEvents(site.eventsPath).filter((event: any) => event.event === 'user_message' && event.content === 'GAP_IDEMPOTENT');
  assert.equal(durableRaceInputs.length, 1, 'same-key concurrent submissions must admit one durable input');
  assert.equal(provider.requests.filter((request: any) => request.prompt.includes('GAP_IDEMPOTENT')).length, 1, 'same-key concurrent submissions must invoke provider once');
  assert.equal(readEvents(site.eventsPath).filter((event: any) => event.event === 'assistant_message' && event.content === 'GAP_IDEMPOTENT_ASSISTANT').length, 1);
  ...
  result = {
    ...
    evidence: await recordLiveEvidence({
      ...
      site,
      runtime,
      clients: clients.map((entry: any) => entry.client ?? entry),
