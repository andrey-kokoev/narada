...
try {
  ...
  const bindingPath: any = join(site.siteRoot, '.ai', 'runtime', 'valid-pi-binding.json');
  ...
  const beforeControls: any = readEvents(site.eventsPath).length;
  ...
  const afterReadOnlyControls: any = readEvents(site.eventsPath).slice(beforeControls);
  assert.equal(afterReadOnlyControls.some((event: any) => event.event === 'user_message'), false, 'read-only controls must not become conversation input');

  await pi.submit('/model model:kimi-k2-thinking');
  const modelReconfiguration: any = await waitForEvent(
    site.eventsPath,
    (event: any) => event.event === 'runtime_intelligence_reconfiguration'
  ...
  const thinkingReconfiguration: any = await waitForEvent(
    site.eventsPath,
    (event: any) => event.event === 'runtime_intelligence_reconfiguration'
  ...
  const providerRefusal: any = await waitForEvent(
    site.eventsPath,
    (event: any) => event.event === 'runtime_intelligence_reconfiguration'
  ...
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'user_message' && event.content === 'GAP_CONTROL_TURN', 'control_turn_user');
  await waitForEvent(site.eventsPath, (event: any) => event.event === 'assistant_message' && event.content === 'GAP_CONTROL_TURN_ASSISTANT', 'control_turn_assistant');
  ...
  const artifactBefore: any = readEvents(site.eventsPath).filter((event: any) => event.event === 'session_artifact_registered').length;
  const outsideResponse: any = await fetch(new URL(`/sessions/${site.sessionId}/artifacts`, runtime.healthEndpoint), {
    method: 'POST',
  ...
  const outsideBody: any = await outsideResponse.json();
  ...
  assert.equal(readEvents(site.eventsPath).filter((event: any) => event.event === 'session_artifact_registered').length, artifactBefore);
