...
const ptyModule: any = await import('node-pty');
...
try {
  ...
  const targetSiteId: any = CANONICAL_LOCAL_TEST_IDS.targetSite;
  const agentId: any = `${siteId}.resident`;
  const sessionWorkspace: any = join(siteRoot, '.ai');
  ...
  const generatedArtifactPath: any = join(siteRoot, 'pi-live-generated.html');
  const piRpcRequestLogPath: any = join(sessionWorkspace, 'runtime', 'pi-rpc-requests.jsonl');
  const piRpcHoldReleasePath: any = join(sessionWorkspace, 'runtime', 'pi-rpc-hold-release');
  await writeFile(
  ...
  const intelligence: any = await seedIntelligenceRegistry(siteRoot, {
  ...
  const intelligenceContextPath: any = join(sessionWorkspace, 'intelligence-launch-context.json');
  ...
  const startup: any = await waitForEvent(record.events_path, (event: any) => event.event === 'session_started', 'session_started');
  ...
  const health: any = await waitForHealthy(record.health_endpoint);
  ...
  const clientRoot: any = join(siteRoot, '.ai', 'runtime', 'four-surface-clients');
  await mkdir(clientRoot, { recursive: true });
  const cursorPath: any = join(clientRoot, 'agent-pi-tui-cursors.json');

  const launchBinding: any = await waitFor(() => {
    const binding: any = readJson(launchBindingPath);
  ...
  const agentTui: any = spawnPtySurface('agent-tui', AGENT_TUI_ENTRYPOINT, ['--launch-binding', launchBindingPath, '--identity', `${agentId}.agent-tui`], {
    cwd: clientRoot,
    env: processEnv(providerEnv),
  });
  ...
  const observedControlEvents: any = [];
  ...
  controlClient.onEvent(({ event }: any) => observedControlEvents.push(event));
  ...
  // follow-up row.
  const agentTuiToolDiagnosticBaseline: any = countOccurrences(agentTui.text(), 'carrier diagnostic');
  await submitPty(pi, 'PI_LIVE_TOOL');
  await waitForEvent(record.events_path, (event: any) => event.event === 'carrier_tool_requested' && event.tool_name === 'fixture_echo', 'tool_requested');
  await waitForEvent(record.events_path, (event: any) => (
  ...
  await waitForEvent(record.events_path, (event: any) => event.event === 'carrier_tool_completed' && event.tool_name === 'fixture_denied' && event.status === 'refused', 'tool_refused');
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_TOOL_ASSISTANT', 'tool_assistant_message');
  ...
  if (piRpcMode) {
    ...
    const artifactRegistration: any = await waitForEvent(
      record.events_path,
      (event: any) => event.event === 'session_artifact_registered' && event.artifact?.title === 'PI_LIVE_ARTIFACT',
      'artifact_registered',
    );
    const artifactId: any = artifactRegistration.artifact_id;
    assert.match(String(artifactId), /^art_/);
    const artifactContentResponse: any = await fetch(new URL(`/sessions/${record.session_id}/artifacts/${encodeURIComponent(artifactId)}/content`, record.health_endpoint));
  ...
  await waitForEvent(record.events_path, (event: any) => event.event === 'user_message' && event.content === 'PI_LIVE_ORDINARY', 'ordinary_user_message');
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_ORDINARY_ASSISTANT', 'ordinary_assistant_message');
  ...
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message_stream' && event.content === 'PI_LIVE_STREAM_PARTIAL' && event.done === false, 'stream_partial');
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message_stream' && event.content === 'PI_LIVE_STREAM_FINAL' && event.done === true, 'stream_final_chunk');
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_STREAM_FINAL', 'stream_assistant_message');
  await waitForAllSurfaces([pi, agentCli, agentTui], webPage, 'PI_LIVE_STREAM_FINAL', 'stream_projection');

  const readRequest: any = await controlClient.readEvents({ limit: 40, view: 'conversation' });
  ...
  const holdUser: any = await waitForEvent(record.events_path, (event: any) => event.event === 'user_message' && event.content === 'PI_LIVE_HOLD', 'hold_user_message');
  if (piRpcMode) {
  ...
  const holdTurn: any = await waitForEvent(
    record.events_path,
    (event: any) => event.event === 'turn_started' && event.input_event_id === holdUser.input_event_id,
    'hold_turn_started',
  );
  await waitForPtySurface(agentCli, ['thinking...'], 'agent_cli_active_hold');
  await submitPty(agentCli, 'PI_LIVE_STEER');
  const queued: any = await waitForEvent(
    record.events_path,
    (event: any) => event.event === 'input_event_queued'
      && event.source === 'operator_steering'
      && event.source_kind === 'operator',
    'steering_queued',
  );
  const steeringUser: any = await waitForEvent(
    record.events_path,
    (event: any) => event.event === 'user_message'
  ...
  await waitForEvent(
    record.events_path,
    (event: any) => eventKind(event) === 'input_queued_for_turn_boundary'
  ...
  await waitForEvent(
    record.events_path,
    (event: any) => eventKind(event) === 'input_admitted_to_turn'
      && (event.input_event_id === queued.event_id || event.payload?.input_event_id === queued.event_id),
    'steering_admitted',
  );
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_HOLD_ASSISTANT', 'hold_assistant_message');
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_STEER_ASSISTANT', 'steer_assistant_message');
  ...
  const agentTuiFailureBaseline: any = countOccurrences(agentTui.text(), 'failed');
  await submitPty(agentCli, 'PI_LIVE_FAILURE');
  const failure: any = await waitForEvent(record.events_path, (event: any) => event.event === 'carrier_turn_failed' && event.error, 'provider_failure');
  await waitFor(
    () => countOccurrences(agentTui.text(), 'failed') > agentTuiFailureBaseline,
    'agent_tui_failure_projection',
  );
  await submitPty(pi, '/recovery');
  await waitFor(() => observedControlEvents.some((event: any) => event.event === 'session_recovery'), 'recovery_response');
  const recovery: any = observedControlEvents.find((event: any) => event.event === 'session_recovery');
  ...
  const degradedAfterFailure: any = await waitForRuntimeHealth(record.health_endpoint);
  ...
  const beforeDetachEvents: any = readEvents(record.events_path);
  const beforeDetachSequence: any = Math.max(...beforeDetachEvents.map(eventSequence), 0);
  const beforeDetachCursor: any = await waitFor(() => {
    if (!existsSync(cursorPath)) return false;
    const cursor: any = readCursor(cursorPath, record.session_id);
  ...
  await pi.kill();
  const afterDetachEvents: any = readEvents(record.events_path);
  assert.equal(afterDetachEvents.some((event: any) => event.event === 'session_closed'), false, 'local Pi detach must not send session.close');
  assert.equal((await (await fetch(record.health_endpoint)).json()).status, 'degraded');

  await submitPty(agentCli, 'PI_LIVE_RECONNECT');
  const reconnectUser: any = await waitForEvent(record.events_path, (event: any) => event.event === 'user_message' && event.content === 'PI_LIVE_RECONNECT', 'reconnect_user_message');
  const agentTuiRecoveryCompletionBaseline: any = countOccurrences(agentTui.text(), 'completed');
  ...
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_FAILURE_RECOVERED', 'failed_turn_provider_retry_completion');
  const failureAssistantEvents: any = readEvents(record.events_path).filter((event: any) => (
  ...
  assert.equal(readEvents(record.events_path).some((event: any) => (
  ...
  await waitForEvent(record.events_path, (event: any) => event.event === 'assistant_message' && event.content === 'PI_LIVE_RECONNECT_ASSISTANT', 'reconnect_assistant_message');
  await waitFor(
  ...
  const afterReattachCursor: any = await waitFor(() => {
    const cursor: any = readCursor(cursorPath, record.session_id);
  ...
  const reconnectEvents: any = readEvents(record.events_path).filter((event: any) => event.content === 'PI_LIVE_RECONNECT_ASSISTANT');
  ...
  for (const [surface, textValue] of Object.entries(evidence)) {
  ...
  const finalEventsBeforeClose: any = readEvents(record.events_path);
  const requiredEventKinds: any = [
    'session_started',
    'user_message',
    'assistant_message',
  ...
  for (const kind of requiredEventKinds) assert.ok(finalEventsBeforeClose.some((event: any) => eventKind(event) === kind), `missing durable live scenario event ${kind}`);
  assert.ok(finalEventsBeforeClose.some((event: any) => (
    (event.event === 'tool_execution_completed' && event.tool_name === 'fixture_echo')
      || (event.event === 'carrier_tool_completed' && event.tool_name === 'fixture_echo' && event.status === 'completed')
  ...
  await setProjectionView(webPage, 'operations');
  const agentTuiCloseProjectionBaseline: any = agentTui.text();
  piReattached.write('\u000f');
  await submitPty(agentCli, '/exit');
  await waitForEvent(record.events_path, (event: any) => event.event === 'session_closed', 'admitted_session_close');
  await Promise.all([
  ...
  const closeEvents: any = readEvents(record.events_path).filter((event: any) => event.event === 'session_closed');
  assert.equal(closeEvents.length, 1, 'admitted session close must produce one durable session_closed event');

  const liveEvidence: any = await recordLiveEvidence({
...
function processEnv(overrides: any = {}) {
...
async function cleanupStep(label: any, action: any, timeoutMs: any = 5_000) {
  await Promise.race([
...
async function waitFor(check: any, label: any, timeoutMs: any = TIMEOUT_MS) {
...
function readEvents(eventsPath: any) {
...
function eventSequence(event: any) {
...
async function waitForEvent(eventsPath: any, predicate: any, label: any) {
  return waitFor(() => readEvents(eventsPath).find(predicate), label);
}

async function waitForSessionRecord({ siteRoot, agentId, runtimeProcess: child, runtimeOutput }: any) {
  return waitFor(() => {
    ...
    for (const sessionsRoot of [
      ...
      for (const entry of aggregate?.sessions ?? []) {
        const path: any = entry.record_path ?? (entry.session_dir ? join(entry.session_dir, 'session-index-record.json') : null);
        const record: any = readJson(path);
      ...
      for (const entry of entries) {
        ...
        const record: any = readJson(join(sessionsRoot, entry.name, 'session-index-record.json'));
        if (record?.agent_id === agentId && record.event_endpoint && record.events_path) return record;
      }
    }
    return null;
...
async function waitForRuntimeHealth(endpoint: any) {
  return waitFor(async () => {
    const response: any = await fetch(endpoint);
    if (!response.ok) return null;
    const body: any = await response.json();
...
function captureProcessOutput(child: any) {
  let stdout: any = '';
  let stderr: any = '';
...
async function terminateProcess(child: any) {
...
function countOccurrences(value: any, needle: any) {
  const text: any = String(value ?? '');
  if (!needle) return 0;
  return text.split(needle).length - 1;
}

function spawnPtySurface(name: any, command: any, args: any, options: any) {
  const terminal: any = pty.spawn(command, args, {
    name: 'xterm-256color',
    cols: 140,
    rows: 42,
  ...
  let resolveExit: any;
  const exitPromise: any = new Promise((resolvePromise: any) => { resolveExit = resolvePromise; });
...
async function waitForPtySurface(surface: any, needles: any, label: any) {
  return waitFor(() => {
    if (surface.exited()) throw new Error(`${surface.name}_exited:${surface.text().slice(-4000)}`);
    const text: any = surface.text();
    return needles.some((needle: any) => text.includes(needle));
  }, label);
}
...
async function waitForAllSurfaces(ptyClients: any, page: any, marker: any, label: any) {
  await Promise.all([
    ...ptyClients.map((surface: any) => waitForPtySurface(surface, [marker], `${label}:${surface.name}`)),
...
function findHeadlessBrowser() {
  ...
  ].filter(Boolean).find((candidate: any) => existsSync(candidate)) ?? null;
}

async function openCdpPage({ browserPath: executable, url, workDir }: any) {
  const userDataDir: any = join(workDir, 'runtime', `pi-tui-live-browser-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  await mkdir(userDataDir, { recursive: true });
  const browser: any = spawnTestChild(executable, [
    '--headless=new',
    '--disable-gpu',
    '--no-sandbox',
  ...
  const browserWsUrl: any = await new Promise((resolvePromise: any, rejectPromise: any) => {
    const timer: any = setTimeout(() => rejectPromise(new Error(`browser_cdp_timeout:${stderr.slice(-1000)}`)), 15_000);
    browser.stderr.on('data', (chunk: any) => {
      stderr += String(chunk);
      const match: any = stderr.match(/DevTools listening on (ws:\/\/[^\s]+)/);
      if (match) { clearTimeout(timer); resolvePromise(match[1]); }
    });
    browser.on('error', (error: any) => { clearTimeout(timer); rejectPromise(error); });
    browser.on('exit', (code: any) => { clearTimeout(timer); rejectPromise(new Error(`browser_exited:${code}:${stderr.slice(-1000)}`)); });
  });
  const debugUrl: any = new URL(browserWsUrl);
  const pages: any = await fetch(`http://${debugUrl.host}/json/list`).then((response: any) => response.json());
  const target: any = pages.find((entry: any) => entry.type === 'page') ?? pages[0];
  const socket: any = new WebSocket(target.webSocketDebuggerUrl);
  await once(socket, 'open');
  let nextId: any = 0;
  const pending: any = new Map();
  socket.addEventListener('message', (message: any) => {
    const payload: any = JSON.parse(String(message.data));
    const waiter: any = pending.get(payload.id);
    if (!waiter) return;
    pending.delete(payload.id);
    if (payload.error) waiter.reject(new Error(JSON.stringify(payload.error)));
    else waiter.resolve(payload.result);
  });
  const send: any = (method: any, params: any = {}) => new Promise((resolvePromise: any, rejectPromise: any) => {
    const id: any = ++nextId;
  ...
  return {
    async evaluate(expression: any) {
      const result: any = await send('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true });
    ...
    async waitForText(text: any, timeoutMs: any = TIMEOUT_MS, label: any = 'web_text') {
      return waitFor(async () => (await this.bodyText()).includes(text), label, timeoutMs);
    },
    async waitForExpression(expression: any, timeoutMs: any = TIMEOUT_MS) {
      return waitFor(() => this.evaluate(expression), 'web_expression', timeoutMs);
    },
    async click(selector: any) {
      const point: any = await this.evaluate(`(() => { const e = document.querySelector(${JSON.stringify(selector)}); if (!(e instanceof HTMLElement)) return null; const r = e.getBoundingClientRect(); return r.width && r.height ? { x: r.left + r.width / 2, y: r.top + r.height / 2 } : null; })()`);
      if (!point) throw new Error(`cdp_click_target_missing:${selector}`);
      await send('Input.dispatchMouseEvent', { type: 'mousePressed', button: 'left', clickCount: 1, ...point });
      await send('Input.dispatchMouseEvent', { type: 'mouseReleased', button: 'left', clickCount: 1, ...point });
    },
    async selectProjectionView(value: any) {
...
async function waitForTextMatch(readText: any, regex: any, label: any) {
  let match: any = null;
  await waitFor(() => {
...
function readCursor(path: any, sessionId: any) {
  const parsed: any = readJson(path) ?? {};
  const key: any = `${sessionId}::agent-pi-tui`;
...
async function seedIntelligenceRegistry(siteRoot: any, {
  ...
  const endpointUrl: any = `${endpointBaseUrl.endsWith('/') ? endpointBaseUrl.slice(0, -1) : endpointBaseUrl}/v1/chat/completions`;
  ...
  for (const record of seed.records) {
    ...
    if (disableTopologyRequirements
      ...
      record.document.topology.nodes = record.document.topology.nodes.map((node: any) => ({
        ...node,
        required_feasibility: [],
      }));
      record.document.topology.edges = record.document.topology.edges.map((edge: any) => ({
        ...edge,
        required_feasibility: [],
      }));
      record.source.digest = canonicalSha256(record.document);
...
async function startFixtureProvider({ generatedArtifactPath }: any) {
  const requests: any = [];
  let toolResponseIssued: any = false;
  let failureResponseIssued: any = false;
  let holdObserved: any = false;
  let holdReleased: any = false;
  let holdPromise: any = null;
  let releaseHoldPromise: any = null;
  const server: any = createServer(async (request: any, response: any) => {
    ...
    requests.push(body);
    const messagesText: any = (body.messages ?? []).map((message: any) => typeof message?.content === 'string' ? message.content : JSON.stringify(message?.content ?? '')).join('\n');
    const latestUserMessage: any = [...(body.messages ?? [])].reverse().find((message: any) => message?.role === 'user');
    const latestUserContent: any = typeof latestUserMessage?.content === 'string'
    ...
    const promptText: any = latestUserContent || messagesText;
    if (promptText.includes('PI_LIVE_HOLD') && !holdReleased) {
      holdObserved = true;
      holdPromise ??= new Promise((resolvePromise: any) => { releaseHoldPromise = resolvePromise; });
      await holdPromise;
    }
