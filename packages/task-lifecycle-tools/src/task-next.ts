...
import { taskLifecycleTools } from './task-mcp-tool-registry.js';
import {
...
function parseArgs(argv: any) : any {
  ...
  if (positional.length === 1) {
    const p: any = positional[0];
    const isPath: any = p.includes('\\') || p.includes('/') || p === '.' || p === '..';
    if (isPath) {
      args.cwd = p;
    } else {
      args.agentId = p;
...
function buildNextPayload({ store, cwd, agentId, limit, lastWorkboardCheckAt }: any) : any {
  const roleResolution: any = resolveAgentRoleWithDiagnostics(store, cwd, agentId);
  const agentRole: any = roleResolution.role;

  const all: any = store.getAllLifecycle();
  const board: any = buildUnifiedWorkboard({ store, siteRoot: cwd, agentId, agentRole, allTasks: all, limit });

  const recommendation: any = deriveNextRecommendation(board, agentId);
  const correctiveDebtReadiness: any = buildCorrectiveDebtReadiness({ siteRoot: cwd, store, allTasks: all });
  ...
  const mcpFreshness: any = buildMcpFreshnessStatus({
    siteRoot: cwd,
...
function renderHuman(result: any) : any {
  const lines: any = [
...
const cwd: any = args.cwd || process.cwd();
const agentId: any = args.agentId;

if (!agentId) {
  console.error(
    'Usage: node task-next.ts [<cwd>] <agent_id> [--json] [--limit <n>] [--last-workboard-check-at <iso>]\n\n' +
...
try {
  const result: any = buildNextPayload({
    store,
    cwd,
    agentId,
    limit: args.limit,
