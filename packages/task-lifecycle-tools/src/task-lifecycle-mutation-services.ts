...
import { findTaskFile, readTaskFile, writeTaskProjection, isValidTransition } from '@narada-core/task-governance/task-governance';
...
export async function tombstoneLifecycleTask({ siteRoot, store, taskNumber, agentId, reason, authorityBasis = null, disposition = 'tombstoned', metadata = {} }: any) : Promise<any> {
  const lifecycle: any = store.getLifecycleByNumber(taskNumber);
  if (!lifecycle) throw new Error(`task_not_found: ${taskNumber}`);
  ...
  const tombstoneDisposition: any = normalizeTombstoneDisposition(disposition);
  const tombstoneMetadata: any = normalizeTombstoneMetadata(metadata);
  ...
  const activeAssignment: any = store.getActiveAssignment(lifecycle.task_id) ?? null;
  if (activeAssignment) {
...
const TOMBSTONE_DISPOSITIONS: any = new Set([
  'tombstoned',
  'wrong_site',
  'handoff_created',
...
function normalizeTombstoneDisposition(value: any) : any {
  const disposition: any = typeof value === 'string' && value.trim().length > 0 ? value.trim() : 'tombstoned';
...
function normalizeTombstoneMetadata(value: any) : any {
  const input: any = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const allowed: any = ['source_task_ref', 'destination_site', 'handoff_ref', 'evidence_ref', 'authority_ref', 'residual_status'];
  const result: any = {};
...
async function writeTaskTombstoneProjection({ siteRoot, taskNumber, status, closedAt, closedBy, closureMode, reason, authorityBasis, disposition, metadata }: any) : Promise<any> {
  try {
    ...
    const metadataLines: any = Object.entries(metadata ?? {})
      .filter(([, value]) : any => value !== null && value !== undefined && String(value).trim().length > 0)
