...
export function checkTaskRoleEligibilityLocal({ store, siteRoot, taskId, taskNumber = null, agentId }: any) : any {
  ...
  const rolePolicy: any = resolveTaskRolePolicy({ siteRoot, taskSpec: routing.taskSpec });
  if (targetRole && roleResolution.role !== targetRole) {
    const severity: any = roleMismatchSeverity(rolePolicy);
    const warning: any = `Task${taskNumber ? ` ${taskNumber}` : ''} targets role '${targetRole}'. Agent '${agentId}' has role '${roleResolution.role ?? 'null'}'.`;
    return {
  ...
  if (routing.preferredAgentId && routing.preferredAgentId !== agentId) {
    const warning: any = `Task${taskNumber ? ` ${taskNumber}` : ''} prefers agent '${routing.preferredAgentId}'. Claiming as '${agentId}'.`;
    return {
      eligible: true,
      warning,
