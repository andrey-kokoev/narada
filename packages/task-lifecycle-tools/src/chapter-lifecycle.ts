...
const OPEN_STATUSES: any = new Set(['opened', 'claimed', 'needs_continuation', 'in_review']);
const TERMINAL_STATUSES: any = new Set(['closed', 'confirmed']);
const BLOCKED_STATUSES: any = new Set(['blocked', 'deferred', 'needs_continuation', 'evidence_repair']);

export function ensureChapterLifecycleTables(store: any) : any {
  store.db.exec(`
...
export function upsertChapterDefinition(store: any, args: any) : any {
  ensureChapterLifecycleTables(store);
  const now: any = new Date().toISOString();
  const chapterId: any = requiredString(args.chapter_id, 'chapter_id');
  const title: any = requiredString(args.title, 'title');
  const actor: any = requiredString(args.actor_agent_id, 'actor_agent_id');
  const current: any = store.db.prepare('SELECT * FROM chapter_definitions WHERE chapter_id = ?').get(chapterId);
  const status: any = optionalString(args.status) ?? current?.status ?? 'active';
...
export function addChapterTask(store: any, args: any) : any {
  ensureChapterLifecycleTables(store);
  const now: any = new Date().toISOString();
  const chapterId: any = requiredString(args.chapter_id, 'chapter_id');
  const actor: any = requiredString(args.actor_agent_id, 'actor_agent_id');
  const taskNumber: any = requiredNumber(args.task_number, 'task_number');
  ...
  const current: any = store.db.prepare('SELECT * FROM chapter_memberships WHERE chapter_id = ? AND task_id = ?').get(chapterId, lifecycle.task_id);
  const orderIndex: any = Number.isFinite(Number(args.order_index))
...
export function listChapters(store: any, { limit = 50 }: any = {}) : any {
  ensureChapterLifecycleTables(store);
  const bounded: any = Math.max(1, Math.min(Number(limit) || 50, 100));
  ...
  return {
    ...
    status: 'ok',
    count: rows.length,
    chapters: rows.map((row: any) : any => {
      const chapter: any = readChapter(store, row.chapter_id);
      const { members, ...summary }: any = chapter;
      return summary;
    }),
  };
}

export function showChapter(store: any, args: any) : any {
  ensureChapterLifecycleTables(store);
  const chapterId: any = requiredString(args.chapter_id, 'chapter_id');
  const chapter: any = readChapter(store, chapterId, { includeSources: Boolean(args.include_sources) });
  if (!chapter) throw new Error(`chapter_not_found: ${chapterId}`);
  return { schema: 'narada.task.chapter.show.v0', status: 'ok', chapter };
}

export function importChapterMarkdownIndex(store: any, args: any) : any {
  ensureChapterLifecycleTables(store);
  const chapterId: any = requiredString(args.chapter_id, 'chapter_id');
  const actor: any = requiredString(args.actor_agent_id, 'actor_agent_id');
  const path: any = requiredString(args.path, 'path');
  ...
  const taskNumbers: any = extractTaskNumbers(text);
  const imported: any = [];
  ...
  const sourceRecordId: any = `chapter-source-${randomUUID()}`;
  store.db.prepare(`
...
function readChapter(store: any, chapterId: any, { includeMembers = true, includeSources = false }: any = {}) : any {
  const definition: any = readChapterDefinition(store, chapterId);
  if (!definition) return null;
  const members: any = includeMembers ? readChapterMembers(store, chapterId) : [];
  const statusProjection: any = computeChapterStatus(members);
  return {
    ...definition,
...
function readChapterDefinition(store: any, chapterId: any) : any {
  const row: any = store.db.prepare('SELECT * FROM chapter_definitions WHERE chapter_id = ?').get(chapterId);
  return row ? rowToChapter(row) : null;
}

function readChapterMembers(store: any, chapterId: any) : any {
  const rows: any = store.db.prepare(`
    SELECT
      m.*,
...
function computeChapterStatus(members: any) : any {
  const counts: any = { total: members.length, open: 0, terminal: 0, blocked: 0, deferred: 0, other: 0 };
  ...
  const residuals: any = collectChapterResiduals(members);
  const status: any = counts.total === 0
    ? 'empty'
    : counts.blocked > 0
      ? 'blocked'
      : counts.open > 0
        ...
        : counts.terminal === counts.total && residuals.length > 0
          ...
          : counts.terminal === counts.total
            ? 'terminal_complete'
            : 'mixed';
  const legacyStatus: any = counts.total === 0
    ? 'empty'
    : counts.terminal === counts.total
      ? 'complete'
      : counts.open > 0
...
function buildBoundedCompletionAffordance({ status, counts, residuals }: any) : any {
  const residualList: any = Array.isArray(residuals) ? residuals : [];
  ...
  const blockerCount: any = Number(counts?.blocked ?? 0);
  return {
    schema: 'narada.task.chapter.bounded_completion.v0',
...
function collectChapterResiduals(members: any) : any {
  const residuals: any = [];
  for (const member of members) {
    const text: any = [
...
function residualKindsFromText(value: any) : any {
  const kinds: any = [];
...
function residualSourceForKind(kind: any) : any {
...
function truncateEvidence(value: any) : any {
  const text: any = String(value ?? '').replace(/\s+/g, ' ').trim();
  return text.length > 240 ? `${text.slice(0, 237)}...` : text;
}

function nextOrderIndex(store: any, chapterId: any) : any {
  const row: any = store.db.prepare('SELECT COALESCE(MAX(order_index), 0) + 1 AS next_order FROM chapter_memberships WHERE chapter_id = ?').get(chapterId);
  return Number(row?.next_order ?? 1);
}

function extractTaskNumbers(text: any) : any {
  const seen: any = new Set();
  const numbers: any = [];
  for (const match of String(text).matchAll(/(?:#|task\s+)(\d{1,7})\b/gi)) {
    const value: any = Number(match[1]);
...
function rowToChapter(row: any) : any {
  return {
...
function rowToMember(row: any) : any {
  return {
    chapter_id: row.chapter_id,
    task_id: row.task_id,
...
function requiredString(value: any, name: any) : any {
  const text: any = optionalString(value);
  if (!text) throw new Error(`${name}_required`);
  return text;
}

function optionalString(value: any) : any {
  return typeof value === 'string' && value.trim().length > 0 ? value.trim() : null;
}

function requiredNumber(value: any, name: any) : any {
