...
const tasksDir: any = join(cwd, '.ai', 'do-not-open', 'tasks');
...
try {
  const files: any = readdirSync(tasksDir).filter((f: any )=> f.endsWith('.md'));
  for (const file of files) {
    const content: any = readFileSync(join(tasksDir, file), 'utf8');
    const { frontMatter }: any = parseFrontMatter(content);
    const preferredRole: any = frontMatter?.preferred_role || null;

    // target_role is authoritative; fall back to preferred_role for backward compatibility
    const targetRole: any = frontMatter?.target_role || preferredRole || null;

    // preferred_agent_id can be top-level or nested in continuation_affinity
    const preferredAgentId: any = frontMatter?.preferred_agent_id
      || frontMatter?.continuation_affinity?.preferred_agent_id
      || null;

    // Extract task number from filename (e.g., 20260501-89-title.md -> 89)
    const match: any = file.match(/\d{8}-(\d+)-/);
    if (!match) continue;
    const taskNumber: any = parseInt(match[1], 10);
    ...
    const relativePriority: any = frontMatter?.relative_priority ?? null;
    const priorityReason: any = frontMatter?.priority_reason ?? null;
