...
export function buildCorrectiveDebtReadiness({ siteRoot, store, allTasks = [] }: any = {}) : any {
  const inboxCapas: any = readActiveInboxCapaItems(siteRoot);
  const capabilityCapas: any = readPendingCapabilityCapaItems(siteRoot);
  ...
  const missingCoverageItems: any = highSeverityItems.filter((item: any) : any => CORRECTIVE_COVERAGE_PRESSURE_STATUSES.has(item.corrective_task_coverage.status));
...
export function deriveCorrectiveDebtRecommendation({ correctiveDebtReadiness, siteRoot, store, agentRole }: any) : any {
  if (agentRole !== 'architect') return null;
  const items: any = (correctiveDebtReadiness?.active_items ?? [])
    .filter((item: any) : any => item.severity >= 75)
    .filter((item: any) : any => CORRECTIVE_COVERAGE_PRESSURE_STATUSES.has(item.corrective_task_coverage?.status))
    .filter((item: any) : any => capaBelongsToRole({ item, siteRoot, store, role: agentRole }));
...
export function selectWorkboardRecommendation({ baseRecommendation, correctiveDebtRecommendation }: any) : any {
...
export function correctiveDebtRecommendationListItem(recommendation: any) : any {
  if (recommendation?.action !== 'corrective_debt_coverage') return null;
  const capa: any = recommendation.capa ?? {};
  return {
...
function capaBelongsToRole({ item, siteRoot, store, role }: any) : any {
  const responsibleAgentId: any = item.responsible_agent_id || '';
  if (responsibleAgentId) {
    const resolved: any = resolveAgentRoleWithDiagnostics(store, siteRoot, responsibleAgentId);
    return resolved.role === role;
  }
  const linkedTasks: any = item.corrective_task_coverage?.tasks ?? [];
  return linkedTasks.some((task: any)  => {
    const lifecycle: any = task.task_number ? store.getLifecycleByNumber(task.task_number) : null;
    const routing: any = lifecycle?.task_id ? readTaskRouting(store, lifecycle.task_id) : null;
    return routing?.target_role === role || routing?.preferred_role === role || lifecycle?.governed_by === role;
  });
}

function readActiveInboxCapaItems(siteRoot: any) : any {
  let events: any = [];
  try {
...
function readPendingCapabilityCapaItems(siteRoot: any) : any {
...
function detectCorrectiveTaskCoverage({ siteRoot, store, item, allTasks }: any) : any {
  const matches: any = [];
  ...
  for (const row of allTasks ?? []) {
    ...
    const projectedTaskBody: any = readProjectedTaskBody(siteRoot, row.task_id);
...
function isClosedCorrectiveImplementationTask({ siteRoot, row, spec, item, projectedTaskBody = null }: any) : any {
...
function readProjectedTaskBody(siteRoot: any, taskId: any) : any {
  if (!taskId) return '';
...
function readTaskRouting(store: any, taskId: any) : any {
  try {
