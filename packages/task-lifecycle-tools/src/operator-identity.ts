...
export function getReviewIndependenceMeta(store: any, reviewRow: any) : any {
  const singleOperatorMeta: any = getSingleOperatorReviewMeta(reviewRow);
  const reviewerAgent: any = reviewRow?.reviewer_agent_id ?? null;
  let finisherAgent: any = null;
  try {
  ...
  const reviewerOperatorIdentity: any = reviewerAgent ? getOperatorIdentity(store, reviewerAgent) : null;
  const finisherOperatorIdentity: any = finisherAgent ? getOperatorIdentity(store, finisherAgent) : null;
  const reviewerEqualsWorker: any = Boolean(reviewerAgent && finisherAgent && reviewerAgent === finisherAgent);
  const sameOperatorPrincipal: any = Boolean(
    reviewerAgent &&
    finisherAgent &&
  ...
  const limitationKind: any = singleOperatorMeta?.kind
...
export const REVIEW_REPLAY_STATUSES: any = ['not_replayed', 'passed', 'failed', 'not_applicable'];

function parseReviewFindings(reviewRow: any) : any {
...
export function normalizeReviewReplayStatus(status: any) : any {
  if (!status) return 'not_replayed';
  return REVIEW_REPLAY_STATUSES.includes(status) ? status : null;
}

export function buildReviewAcceptanceProvenanceAnnotation({ verdict, reviewReplayStatus = 'not_replayed' }: any) : any {
  const replayStatus: any = normalizeReviewReplayStatus(reviewReplayStatus);
  if (!replayStatus) {
    throw new Error(`review_replay_status must be one of: ${REVIEW_REPLAY_STATUSES.join(', ')}`);
  }
  const accepted: any = ['accepted', 'accepted_with_notes'].includes(verdict);
...
export function getReviewAcceptanceProvenance(store: any, reviewRow: any) : any {
  const verdict: any = reviewRow?.verdict ?? null;
  const accepted: any = ['accepted', 'accepted_with_notes'].includes(verdict);
  const findings: any = parseReviewFindings(reviewRow);
  const annotation: any = findings.find((f: any) : any => f?.location === 'review_acceptance_provenance');
  const annotated: any = annotation?.acceptance_provenance && typeof annotation.acceptance_provenance === 'object'
    ? annotation.acceptance_provenance
    : null;
  const requestedReplayStatus: any = normalizeReviewReplayStatus(annotated?.requested_replay_status)
    ?? normalizeReviewReplayStatus(annotated?.replay_status)
    ?? 'not_replayed';
  const reviewIndependence: any = store ? getReviewIndependenceMeta(store, reviewRow) : null;
  const independentReview: any = Boolean(reviewIndependence?.independent_review);
  const replayStatus: any = requestedReplayStatus === 'passed' && independentReview ? 'passed' : requestedReplayStatus;
  const acceptanceProvenance: any = !accepted
    ? 'not_accepted'
