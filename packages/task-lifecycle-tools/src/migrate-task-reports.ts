...
const dbPath: any = `${cwd}/.ai/task-lifecycle.db`;
...
try {
  ...
  const sourceCount: any = db.prepare('SELECT COUNT(*) as c FROM task_report_records').get().c;
  const targetCount: any = db.prepare('SELECT COUNT(*) as c FROM task_reports').get().c;
  ...
  const rows: any = db.prepare('SELECT * FROM task_report_records ORDER BY reported_at').all();
  ...
  const migrateOne: any = db.transaction((row: any) : any => {
    let summary: any = '';
    let changedFilesJson: any = null;
    let verificationJson: any = null;

    try {
      const parsed: any = JSON.parse(row.report_json);
  ...
  const finalTargetCount: any = dryRun ? 0 : db.prepare('SELECT COUNT(*) as c FROM task_reports').get().c;
