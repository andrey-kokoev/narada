#!/usr/bin/env node
/**
 * migrate-add-relative-priority.ts
 *
 ...
 *
 * Usage:
 *   node tools/task-lifecycle/migrate-add-relative-priority.ts [<cwd>] [--dry-run]
 */
...
const dbPath: any = resolve(cwd, '.ai', 'task-lifecycle.db');
...
try {
  const columns: any = db.prepare("pragma table_info(task_lifecycle)").all();
  const hasRelativePriority: any = columns.some((c: any) : any => c.name === 'relative_priority');
  const hasPriorityReason: any = columns.some((c: any) : any => c.name === 'priority_reason');
