...
const reviewerAgent: any = parsed.positional[2];
const verdict: any = parsed.verdict || parsed.positional[3] || null;
let findings: any = parsed.positional[4] || null;
...
if (isNaN(taskNumber) || !reviewerAgent) {
  console.error('Usage: node task-review.ts <cwd> <task-number> <reviewer> [verdict] [findings-json] [--findings-file <path>] [--report <report-id>] [--verdict <accepted|accepted_with_notes|rejected>] [--single-operator-review]');
  process.exit(1);
}
...
const reviewReplayStatus: any = normalizeReviewReplayStatus(parsed.reviewReplayStatus);
...
const result: any = await reviewTaskService({ cwd, taskNumber, agent: reviewerAgent, verdict, findings, report: parsed.report });
const output: any = result.result || result;
if (output.review_id) {
  try {
    const store: any = openTaskLifecycleStore(cwd);
    try {
      const reviewRow: any = store.db.prepare('SELECT * FROM task_reviews WHERE review_id = ?').get(output.review_id);
