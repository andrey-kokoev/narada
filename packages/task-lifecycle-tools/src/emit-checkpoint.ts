...
function openAgentContextDb(cwd: any) : any {
  const siteRoot: any = resolve(cwd);
  const dbDir: any = join(siteRoot, '.ai', 'state');
  const dbPath: any = join(dbDir, 'agent-context.sqlite');
  try {
    if (!existsSync(dbDir)) {
...
export function emitCheckpoint(options: any) : any {
  const {
    cwd,
    agentId,
    sessionId,
  ...
  const db: any = openAgentContextDb(cwd);
  if (!db) {
    return { status: 'skipped', reason: 'agent_context_db_not_found' };
  }

  try {
    const hasTable: any = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'agent_events'").get();
    if (!hasTable) {
      db.close();
