...
export function deriveNextRecommendation(board: any, agentId: any) : any {
  ...
  const myNeedsContinuation: any = board.needs_continuation.filter((t: any) : any => !t.assigned_agent || t.assigned_agent === agentId);
...
export function buildUnifiedWorkboard({ store, siteRoot, agentId, agentRole, allTasks, limit = 8 }: any) : any {
  ...
  if (agentId) {
    ...
    obligations = rawObligations.map((o: any) : any => {
      const lifecycle: any = o.task_number ? store.getLifecycleByNumber(o.task_number) : null;
      const taskLifecycleStatus: any = lifecycle?.status ?? null;
      const reviewExecutable: any = o.kind !== 'review_request' || taskLifecycleStatus === 'in_review';
      return {
  ...
  const reviewObligations: any = obligations.filter((o: any) : any => o.kind === 'review_request');
  const executableReviewObligations: any = reviewObligations.filter((o: any) : any => o.review_executable === true);
  const blockedReviewObligations: any = reviewObligations.filter((o: any) : any => o.review_executable !== true);
