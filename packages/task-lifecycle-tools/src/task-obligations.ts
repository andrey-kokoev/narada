...
function buildSourceAgentIdentityRef(sourceAgentId: any) : any {
  const resolved: any = resolveAgentIdentityRef(sourceAgentId, { role: sourceAgentId });
  if (resolved.status === 'resolved') return resolved.value;
  const localAgentId: any = String(sourceAgentId || 'unknown').split('.').filter(Boolean).at(-1) || 'unknown';
...
MAIN: try {
  if (args.list) {
    const agent: any = args.agent || null;
    const status: any = args.status || 'open';
    if (!agent) {
    ...
    const obligations: any = store.listDirectedObligationsForTarget(agent, agentRole, status);
    const result: any = obligations.map((o: any) : any => {
  ...
  if (args.create) {
    ...
    const obligationId: any = `obl_manual_${kind}_${taskNumber}_${Date.now()}`;
  ...
  if (args.sweep_stale) {
    ...
    const stmt: any = store.db.prepare(`
  ...
  if (args.task) {
    ...
    const status: any = args.status || null;
    const obligations: any = store.listDirectedObligationsForTask(lifecycle.task_id, status);
    const result: any = obligations.map((o: any) : any => ({
      obligation_id: o.obligation_id,
      kind: o.kind,
      status: o.status,
