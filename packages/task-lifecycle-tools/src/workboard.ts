/**
 * Shared workboard categorization logic used by generate-workboard.ts and task_mcp_next.
 */

import { existsSync, readFileSync } from 'node:fs';
import { join } from 'node:path';
import { detectSameOperatorReview, detectSelfReview, getReviewAcceptanceProvenance } from './operator-identity.js';
...
export function buildWorkboard({ store, siteRoot = null, agentId, agentRole, allTasks }: any) : any {
  const all_in_review: any = [];
  const reviewed_closeouts: any = [];
  const in_progress: any = [];
  const needs_continuation: any = [];
  const local_followups: any = [];
  const role_wide_followups: any = [];
  const non_actionable_parent_followups: any = [];
  const closure_authority_conflicts: any = [];
  const downstream_role_followups: any = [];
  const claimed_terminal_blocked: any = [];
  const deferred: any = [];
  const actionable_deferred: any = [];
  const lifecycleByNumber: any = new Map((allTasks || []).map((t: any) : any => [t.task_number, t]));

  // Preload operator identities for agents in the workboard
  const operatorIdentities: any = new Map();
  try {
    const rows: any = store.db.prepare("SELECT agent_id, operator_identity FROM agent_roster WHERE operator_identity IS NOT NULL").all();
  ...
  for (const task of allTasks) {
    const spec: any = store.getTaskSpecByNumber(task.task_number);
    const assignment: any = store.getActiveAssignment(task.task_id);
    ...
    if (task.status === 'in_review') {
      ...
      if (agentId) {
        let structuralInfo: any = detectSameOperatorReview(store, agentId, task.task_number);
      ...
      const acceptedReview: any = findLatestAcceptedReview(store, task.task_id);
      if (acceptedReview) {
      ...
      const terminalBlocker: any = deriveClaimedTerminalBlocker({ siteRoot, store, task, spec });
      if (terminalBlocker) {
      ...
      const observationText: any = loadObservationArtifactText({ store, task });
      const actionability: any = deriveDeferredActionability({ siteRoot, task, spec, observationText, lifecycleByNumber });
      ...
      deferred.push(item);
      const assignmentMatches: any = !assignment?.agent_id || assignment.agent_id === agentId;
      const roleMatches: any = !targetRole || agentRole === targetRole;
  ...
  const sortByPriorityThenPreferred: any = (a: any, b: any) : any => {
    const prioDiff: any = (b.relative_priority ?? 0) - (a.relative_priority ?? 0);
    if (prioDiff !== 0) return prioDiff;
    const aPref: any = a.preferred_agent_id === agentId ? 0 : 1;
    const bPref: any = b.preferred_agent_id === agentId ? 0 : 1;
    return aPref - bPref;
  };
...
function findLatestAcceptedReview(store: any, taskId: any) : any {
  const acceptedVerdicts: any = new Set(['accepted', 'accepted_with_notes']);
  try {
    const reviews: any = typeof store.listReviews === 'function'
    ...
    return reviews.find((review: any)  => acceptedVerdicts.has(review.verdict)) || null;
  } catch {
    return null;
  }
}

const ACTIVE_CHILD_STATUSES: any = new Set(['opened', 'claimed', 'needs_continuation', 'in_review', 'deferred']);

function deriveParentCoordinatorActionability({ task, spec, lifecycleByNumber }: any) : any {
  ...
  const childTaskNumbers: any = extractChildTaskNumbers(text, task.task_number);
  ...
  const activeChildTaskNumbers: any = childTaskNumbers.filter((number: any) : any => {
    const child: any = lifecycleByNumber.get(number);
  ...
  return {
    ...
    reason: `Parent/coordinator task has active child task(s): ${activeChildTaskNumbers.map((n: any) : any => `#${n}`).join(', ')}.`,
    child_task_numbers: childTaskNumbers,
    active_child_task_numbers: activeChildTaskNumbers,
  };
}

function extractChildTaskNumbers(text: any, parentTaskNumber: any) : any {
  const refs: any = new Set();
  const childRefPattern: any = /(?:^|\n)\s*(?:[-*]\s*)?`?#(\d+)`?/g;
  let match: any;
  while ((match = childRefPattern.exec(text)) !== null) {
    const number: any = Number(match[1]);
...
function buildPreClaimWarnings({ item, assignment, agentId }: any) : any {
  const warnings: any = [];
...
function preferredAgentRelation(preferredAgentId: any, agentId: any) : any {
  if (!preferredAgentId) return 'none';
  if (agentId && preferredAgentId === agentId) return 'self';
  return 'other';
}

function deriveClaimedTerminalBlocker({ siteRoot, store, task, spec }: any) : any {
  const bodyText: any = loadTaskText({ siteRoot, task, spec });
  const bodyValidation: any = validateRecoveryTruthfulnessBody({ body: bodyText, summary: '', context: `task:${task.task_number}` });
  const normalizedBodyState: any = bodyValidation.evaluation?.normalized_state ?? null;
  const latestReport: any = findLatestTaskReport(store, task.task_id);
  const reportText: any = [latestReport?.summary, latestReport?.verification_json, latestReport?.changed_files_json].filter(Boolean).join('\n');
  const normalizedReport: any = reportText.toLowerCase();
  const bodyHasTerminalBlocked: any = normalizedBodyState === 'terminal_blocked' || /\brecovery truthfulness[\s\S]{0,400}\bstate\s*:\s*terminal_blocked\b/i.test(bodyText);
  const reportHasTerminalBlocked: any = /\bterminal_blocked\b/.test(normalizedReport);
  const externalAuthorityBlocked: any = detectExternalAuthorityBlocker(`${bodyText}\n${reportText}`);
  const checkedBlockedCriteria: any = detectCheckedBlockedCriteria(bodyText);
...
function findLatestTaskReport(store: any, taskId: any) : any {
...
function detectExternalAuthorityBlocker(text: any) : any {
  const normalized: any = text.toLowerCase();
  const hasTerminalOrDeferred: any = /\b(terminal_blocked|deferred|blocked|needs_external_authority|external authority|other locus|pc-locus|staccato)\b/.test(normalized);
  const hasExternalBoundary: any = /\b(external authority|needs_external_authority|other locus|different locus|pc-locus|staccato|outside (?:this )?(?:workspace|user site|authority)|missing .*capability)\b/.test(normalized);
  const hasNoLocalPath: any = /\b(no local executable path|cannot honestly|cannot .* from .*locus|outside .*authority|awaiting .*authority|requires .*authority|deferred to)\b/.test(normalized);
  return hasTerminalOrDeferred && hasExternalBoundary && hasNoLocalPath;
}

function detectCheckedBlockedCriteria(body: any) : any {
  const matches: any = [];
  const section: any = extractAcceptanceCriteriaSection(body);
...
function extractAcceptanceCriteriaSection(body: any) : any {
  const match: any = body.match(/^##\s+Acceptance Criteria\s*$/mi);
  if (!match) return '';
...
function deriveDeferredActionability({ siteRoot, task, spec, observationText = '', lifecycleByNumber = new Map() }: any) : any {
  const text: any = loadDeferredTaskText({ siteRoot, task, spec, observationText });
  const normalized: any = text.toLowerCase();
  const coordinatorBlocker: any = detectDeferredCoordinatorChildBlocker({ text, task, lifecycleByNumber });
  ...
  const explicitBlocker: any = detectExplicitDeferredBlockerSection(text);
  if (explicitBlocker) {
    return {
      status: 'blocked',
  ...
  const admittedBlocker: any = detectAdmittedDeferredBlockerObservation(normalized);
  if (admittedBlocker) {
    return {
      status: 'blocked',
  ...
  const reviewAcceptanceBlocker: any = detectReviewAcceptanceDeferredBlocker(normalized);
  ...
  const externalBlocker: any = detectExternalDeferredBlocker(normalized);
  if (externalBlocker) {
    return {
      status: 'blocked',
  ...
  const hasClearedBlocker: any = /\b(blocker|blocked|external|upstream)\b[\s\S]{0,80}\b(cleared|resolved|accepted|available|unblocked)\b/i.test(text)
...
function detectExplicitDeferredBlockerSection(text: any) : any {
  const match: any = text.match(/^##\s+Blocker\s*\n([\s\S]*?)(?=^##\s|$)/im);
  if (!match) return null;
  const section: any = match[1].trim();
...
function detectAdmittedDeferredBlockerObservation(normalizedText: any) : any {
  const hasBlockerObservation: any = /\b(finish_blocked|review_blocked|mcp_finish_blocked|mcp_review_blocked|guard[_-]blocked|self_cert_false_treated_missing|task_file_resolution_failed)\b/.test(normalizedText);
...
function detectReviewAcceptanceDeferredBlocker(normalizedText: any) : any {
  const hasReviewAcceptanceGate: any = /\b(independent review|operator acceptance|explicit operator acceptance|review\/operator acceptance|review or operator acceptance|self-certification|same-subject|same subject)\b/.test(normalizedText);
  const hasRequirementLanguage: any = /\b(needs|requires|required|until|dependent on|pending|deferred|blocked)\b/.test(normalizedText);
  const hasTerminalOrClosureLanguage: any = /\b(terminal|closure|close|finish|acceptance|corrected|correction)\b/.test(normalizedText);
...
function detectDeferredCoordinatorChildBlocker({ text, task, lifecycleByNumber }: any) : any {
  if (!/\b(child tasks?|children|parent\/coordinator|coordinator|blocked on|blocked until|blocked by)\b/i.test(text)) {
    return null;
  }
  const childTaskNumbers: any = extractTaskRefsAndRanges(text, task.task_number);
  ...
  return {
    ...
    reason: `deferred parent/coordinator has unresolved child task blocker evidence: ${activeChildTaskNumbers.map((n: any) : any => `#${n}`).join(', ')}`,
    child_task_numbers: childTaskNumbers,
    active_child_task_numbers: activeChildTaskNumbers,
...
function extractTaskRefsAndRanges(text: any, parentTaskNumber: any) : any {
  const refs: any = new Set();
  const rangePattern: any = /#(\d+)\s*-\s*#?(\d+)/g;
  let range: any;
  while ((range = rangePattern.exec(text)) !== null) {
    const start: any = Number(range[1]);
    const end: any = Number(range[2]);
    if (!Number.isInteger(start) || !Number.isInteger(end)) continue;
    const low: any = Math.min(start, end);
    const high: any = Math.max(start, end);
    if (high - low > 50) continue;
    for (let number = low; number <= high; number += 1) {
      if (number !== parentTaskNumber) refs.add(number);
    }
  }
  const refPattern: any = /#(\d+)/g;
  let match: any;
...
function loadDeferredTaskText({ siteRoot, task, spec, observationText = '' }: any) : any {
  const chunks: any = [loadTaskText({ siteRoot, task, spec })];
  if (observationText) chunks.push(observationText);
  return chunks.join('\n\n');
}

function loadTaskText({ siteRoot, task, spec }: any) : any {
  const chunks: any = [
    spec?.title,
    spec?.goal_markdown,
    spec?.context_markdown,
  ...
  if (siteRoot && task?.task_id) {
    const taskPath: any = join(siteRoot, '.ai', 'do-not-open', 'tasks', `${task.task_id}.md`);
...
function loadObservationArtifactText({ store, task }: any) : any {
