...
MAIN: try {
  if (args.admit) {
    const taskNumber: any = args.task ? Number(args.task) : null;
    const admittedBy: any = args.by || null;
    const methodsRaw: any = args.methods || 'admission';
    const methods: any = methodsRaw.split(',').map((m: any) : any => m.trim()).filter(Boolean);
    ...
    try {
      const { bundle, result, blockers }: any = await admitTaskEvidence({
        cwd,
        taskNumber,
        admittedBy,
        methods,
  ...
  if (args.inspect) {
    ...
    const store: any = openTaskLifecycleStore(cwd);
    try {
      const evidence: any = await inspectTaskEvidence(cwd, String(taskNumber), store);
      console.log(JSON.stringify({
