...
function combinedText(packet: any) : any {
  return [
    packet.title,
    packet.summary,
    packet.context,
...
function inferTargetCategory(packet: any) : any {
...
function hasTerminalClaim(packet: any) : any {
...
function hasIndependentReview(packet: any) : any {
...
function hasOperatorAcceptance(packet: any) : any {
  return text(packet.operator_acceptance_ref).trim().length > 0;
}

function parseSelfCertificationSection(section: any) : any {
  if (!section) return {};
...
function hasRequiredFieldValue(packet: any, field: any) : any {
  if (!Object.prototype.hasOwnProperty.call(packet, field)) return false;
  const value: any = packet[field];
  if (value == null) return false;
  if (field === 'requires_independent_review') return coerceBoolean(value) !== null;
  return text(value).trim().length > 0;
}

export function selfCertificationGuardContract() : any {
  return {
    ...
    non_target_low_risk_path: 'Use target_category: non_target_low_risk only for routine work outside architect-failure, deception/trust, misleading completion, and same-subject terminal correction claims.',
...
export function evaluateSelfCertificationGuard(packet: any = {}) : any {
  ...
  return {
    ...
    non_target_reason: target ? null : 'Routine low-risk or non-target material is outside architect-failure, deception/trust, misleading completion, and same-subject terminal correction enforcement.',
    non_target_remediation: target ? null : 'If this was meant to be guarded material, set target_category to architect_failure, deception_trust, misleading_completion, or self_certification; otherwise non_target_low_risk is the explicit passing state.',
  };
}

export function validateSelfCertificationPacket(packet: any = {}) : any {
  ...
  if (!evaluation.target) {
    return {
      ...
      evaluation,
      errors,
      remediation: evaluation.non_target_remediation,
    };
  }

  const missingFields: any = REQUIRED_TARGET_FIELDS.filter((field: any) : any => !hasRequiredFieldValue(packet, field));
...
export function validateSelfCertificationBody({ body = '', summary = '', actor_principal = '' }: any = {}) : any {
  const section: any = extractMarkdownSection(body, 'Self-Certification Guard');
  const parsed: any = parseSelfCertificationSection(section);
