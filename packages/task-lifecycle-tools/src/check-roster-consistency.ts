#!/usr/bin/env node
/**
 * check-roster-consistency.ts — Verify roster.json matches task lifecycle assignments.
 *
 * Checks:
...
function readRoster(cwd: any) : any {
  const path: any = join(resolve(cwd), '.ai', 'agents', 'roster.json');
...
try {
  const roster: any = readRoster(cwd);

  for (const agent of roster.agents) {
    const agentId: any = agent.agent_id;
    const rosterTask: any = agent.task ?? null;
    const rosterStatus: any = agent.status ?? null;
    const rosterLastDone: any = agent.last_done ?? null;

    // Find active assignment for this agent
    const activeAssignment: any = store.db.prepare(
    ...
    const activeTaskNumber: any = activeAssignment ? activeAssignment.task_number : null;
    ...
    const lastFinished: any = store.db.prepare(
    ...
    const sqlLastDone: any = lastFinished ? lastFinished.task_number : null;
...
const ok: any = divergences.length === 0;
