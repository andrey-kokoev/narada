...
import { claimLifecycleTask, proveTaskCriteria, transitionLifecycleTask, tombstoneLifecycleTask, unclaimLifecycleTask, unDeferLifecycleTask } from './task-lifecycle-mutation-services.js';
import { TASK_LIFECYCLE_TOOL_ALIASES, taskLifecycleTools } from './task-mcp-tool-registry.js';
import { addChapterTask, importChapterMarkdownIndex, listChapters, showChapter, upsertChapterDefinition } from './chapter-lifecycle.js';
import { applyBulkTaskRouting } from './bulk-routing.js';
...
const SESSION_IDENTITY: any = process.env.NARADA_AGENT_ID || null;
...
if (options.help) {
  process.stdout.write('Usage: node task-mcp-server.ts --site-root <path>\n');
  process.exit(0);
}
...
function recordTaskLifecycleRuntimeObservation() : any {
  try {
    writeMcpRuntimeInstanceObservation({
      siteRoot,
      surfaceId: 'task-lifecycle-mcp.local',
      serverName: SERVER_NAME,
      serverEntryPoint: 'tools/task-lifecycle/task-mcp-server.ts',
      serverBootedAt: SERVER_BOOTED_AT,
      watchedPaths: ['tools/task-lifecycle', 'tools/mcp-freshness-service.ts'],
...
function buildCorrectiveDebtReadiness({ allTasks = [] }: any = {}) : any {
  ...
  const terminalCoverageItems: any = activeItems.filter((item: any) : any => item.corrective_task_coverage.status === 'closed_corrective_implementation_coverage');
  const highSeverityItems: any = activeItems.filter((item: any) : any => item.severity >= 75);
  ...
  const missingCoverageItems: any = highSeverityItems.filter((item: any) : any => correctiveCoveragePressureStatuses.has(item.corrective_task_coverage.status));
  const state: any = activeItems.length === 0
    ? 'terminal_corrected'
  ...
  return {
    ...
    counts: {
      ...
      linked_or_covered: activeItems.filter((item: any) : any => ['covered_by_open_task', 'closed_corrective_implementation_coverage'].includes(item.corrective_task_coverage.status)).length,
    ...
    missing_corrective_task_coverage: missingCoverageItems.slice(0, 25).map((item: any) : any => ({
...
function readActiveInboxCapaItems() : any {
  let events: any = [];
  try {
...
function readPendingCapabilityCapaItems() : any {
  const capPath: any = join(siteRoot, 'operator-surfaces', 'capability-announcements.json');
  ...
  for (const cap of capabilities) {
    ...
    const severity: any = Number.isFinite(cap.severity)
      ? cap.severity
      : daysUntilDue === null
        ? 0
        : daysUntilDue < 0
...
function detectCorrectiveTaskCoverage({ item, allTasks }: any) : any {
  const matches: any = [];
  const normalizedConcept: any = normalizeLinkText(item.concept_name);
  for (const row of allTasks ?? []) {
    const spec: any = row.task_number ? store.getTaskSpecByNumber(row.task_number) : null;
    const projectedTaskBody: any = readProjectedTaskBody(row.task_id);
    const text: any = normalizeLinkText([
      spec?.title,
      spec?.goal_markdown,
  ...
  const openStatuses: any = new Set(['opened', 'claimed', 'needs_continuation', 'in_review']);
  const openMatches: any = matches.filter((match: any) : any => openStatuses.has(match.status));
  ...
  const terminalMatches: any = matches.filter((match: any) : any => match.closed_corrective_implementation === true);
...
function correctiveCoverageNextStates() : any {
  return [
...
function isClosedCorrectiveImplementationTask({ row, spec, item, projectedTaskBody = null }: any) : any {
...
function readProjectedTaskBody(taskId: any) : any {
  if (!taskId) return '';
  const path: any = join(siteRoot, '.ai', 'do-not-open', 'tasks', `${taskId}.md`);
...
function classifyProjectedTaskEvidence(body: any) : any {
  const acceptanceMatches: any = [...String(body).matchAll(/^\s*-\s+\[(x|X| )\]\s+\S.*$/gm)];
  const checkedCount: any = acceptanceMatches.filter((match: any) : any => match[1].toLowerCase() === 'x').length;
  return {
...
function sectionHasSubstantiveContent(body: any, heading: any) : any {
  const escaped: any = heading.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const match: any = String(body).match(new RegExp(`(?:^|\\n)## ${escaped}\\s*\\n([\\s\\S]*?)(?=\\n## |$)`, 'i'));
  if (!match) return false;
  const normalized: any = match[1]
    .replace(/<!--[\s\S]*?-->/g, '')
    .replace(/^\s*[-*]\s*/gm, '')
    .trim();
...
function normalizeLinkText(value: any) : any {
  return String(value ?? '').toLowerCase().replace(/[^a-z0-9_\-]+/g, ' ').replace(/\s+/g, ' ').trim();
}

function finiteNumber(value: any, fallback: any) : any {
...
function buildPostCloseoutContinuation({ agentId, result }: any) : any {
  refreshStore();
  ...
  const correctiveDebtReadiness: any = buildSharedCorrectiveDebtReadiness({ siteRoot, store, allTasks: all });
  const correctiveDebtRecommendation: any = deriveCorrectiveDebtRecommendation({
    correctiveDebtReadiness,
    siteRoot,
    store,
    agentRole,
  });
  const finalRecommendation: any = selectWorkboardRecommendation({
...
function buildAgentRelationshipSemantics() : any {
  return {
...
function classifyTaskAgentRelationship({ agentId, agentRole, taskId, assignment, routing, reviewTaskIds }: any) : any {
  const claimedBy: any = Boolean(agentId && assignment?.agent_id === agentId);
  const preferredFor: any = Boolean(agentId && routing?.preferred_agent_id === agentId);
  const targetRoleMatch: any = Boolean(agentRole && routing?.target_role === agentRole);
  const reviewObligation: any = Boolean(taskId && reviewTaskIds?.has(taskId));
  return {
...
function isStoreError(error: any) : any {
  const msg: any = error instanceof Error ? error.message : String(error);
 ...
 */
function verifySessionIdentity(agentId: any) : any {
 ...
 */
function enforceSessionIdentity(agentId: any) : any {
...
async function callTool(params: any) : Promise<any> {
  ...
  const payloadResolution: any = !canonicalName.startsWith('mcp_command_') && (stringField(args, 'payload_ref') || stringField(args, 'payload_path'))
    ? resolveToolPayloadArgs({
      siteRoot,
      toolName: canonicalName,
      args,
      allowedTools: taskLifecycleTools().map((tool: any) : any => tool.name),
    })
    : { args, payloadSource: null };
  const resolvedArgs: any = payloadResolution.args;
  if (canonicalName === 'task_lifecycle_create') {
    const createArgs: any = resolveTaskCreatePayloadArgs(args);
    const locusGuard: any = guardLifecycleTargetLocus(canonicalName, createArgs.args);
  ...
  if (canonicalName === 'task_lifecycle_create_task_batch') {
    const createArgs: any = resolveTaskBatchCreatePayloadArgs(args);
    const locusGuard: any = guardLifecycleTargetLocus(canonicalName, createArgs.args);
    if (locusGuard.status === 'refused') return jsonToolResult(locusGuard, true);
    return await dispatchTool(canonicalName, createArgs.args, { payloadSource: createArgs.payloadSource });
  }

  const toolDef: any = taskLifecycleTools().find((t: any) : any => t.name === canonicalName);
  if (toolDef?.inputSchema) {
    const validationErrors: any = validateArgs(canonicalName, resolvedArgs, toolDef.inputSchema);
    if (validationErrors) {
  ...
  const locusGuard: any = guardLifecycleTargetLocus(canonicalName, resolvedArgs);
  if (locusGuard.status === 'refused') return jsonToolResult(locusGuard, true);
  ...
  try {
    ...
    if (isStoreError(error)) {
      const refreshed: any = refreshStore();
      if (refreshed) {
        try {
          return await dispatchTool(canonicalName, resolvedArgs, { payloadSource: payloadResolution.payloadSource });
...
function resolveTaskCreatePayloadArgs(args: any) : any {
  const input: any = asRecord(args);
  const inlineTaskFields: any = [
    'title',
    'goal',
    'context',
    'required_work',
  ...
  const inlineFields: any = inlineTaskFields.filter((field: any) : any => Object.prototype.hasOwnProperty.call(input, field));
...
function validateTaskCreatePayload(args: any) : any {
  const title: any = stringField(args, 'title');
  ...
  const executionWindow: any = normalizeExecutionWindow(args);
...
function resolveTaskBatchCreatePayloadArgs(args: any) : any {
  const input: any = asRecord(args);
  const inlineBatchFields: any = ['chapter', 'shared', 'tasks', 'dry_run'];
  const inlineFields: any = inlineBatchFields.filter((field: any) : any => Object.prototype.hasOwnProperty.call(input, field));
  if (inlineFields.length > 0) {
...
function validateTaskBatchCreatePayload(args: any) : any {
  const tasks: any = Array.isArray(args.tasks) ? args.tasks : null;
  ...
  const chapter: any = asRecord(args.chapter);
  ...
  const shared: any = asRecord(args.shared);
  validateBatchSpecRecord(shared, 'shared', { allowMissingTitle: true });
  for (const [index, task] of tasks.entries()) {
    const record: any = asRecord(task);
    validateBatchSpecRecord(record, `tasks_${index}`, { allowMissingTitle: false });
  }
}

function validateBatchSpecRecord(record: any, label: any, { allowMissingTitle }: any) : any {
  void allowMissingTitle;
  if (record.acceptance_criteria !== undefined && (!Array.isArray(record.acceptance_criteria) || record.acceptance_criteria.some((item: any)  => typeof item !== 'string'))) {
    throw new Error(`task_lifecycle_create_task_batch_${label}_acceptance_criteria_must_be_string_array`);
  }
  if (record.dependencies !== undefined && (!Array.isArray(record.dependencies) || record.dependencies.some((item: any)  => typeof item !== 'number' && typeof item !== 'string'))) {
...
function buildLifecycleTargetLocusStatus() : any {
  const operatorStatedRoot: any = process.env.NARADA_OPERATOR_STATED_SITE_ROOT
    ...
    || null;
  const resolvedOperatorRoot: any = operatorStatedRoot ? resolve(String(operatorStatedRoot)) : null;
  const mismatch: any = resolvedOperatorRoot && resolve(String(resolvedOperatorRoot)).toLowerCase() !== resolve(siteRoot).toLowerCase();
  return {
...
function guardLifecycleTargetLocus(canonicalName: any, args: any) : any {
  ...
  const status: any = buildLifecycleTargetLocusStatus();
...
function buildTaskLifecycleDoctorSummary({ registeredTools }: any) : any {
  const registry: any = genericCommandRegistrySummary();
  const freshness: any = buildTaskLifecycleFreshness({ registeredTools });
  return {
    status: 'ok',
    site_root: siteRoot,
...
async function dispatchTool(canonicalName: any, args: any, dispatchContext: any = {}) : Promise<any> {
  switch (canonicalName) {
    case 'task_lifecycle_doctor': {
      const registeredTools: any = taskLifecycleTools().map((t: any) : any => t.name);
    ...
    case 'task_lifecycle_list': {
      ...
      const queryMode: any = stringField(args, 'query_mode') ?? (agentFilter ? 'claimed_by' : 'all');
      const allowedModes: any = new Set(['all', 'claimed_by', 'preferred_for', 'target_role', 'review_obligations', 'all_agent_related']);
      ...
      const rows: any = store.db.prepare('SELECT * FROM task_lifecycle ORDER BY task_number DESC').all();
      const roleResolution: any = agentFilter ? resolveAgentRoleWithDiagnostics(store, siteRoot, agentFilter) : null;
      const reviewObligations: any = agentFilter
        ? store.listDirectedObligationsForTarget(agentFilter, roleResolution?.role, 'open').filter((o: any) : any => o.kind === 'review_request')
        : [];
      const reviewTaskIds: any = new Set(reviewObligations.map((o: any) : any => o.task_id).filter(Boolean));
      const tasks: any = rows.map((row: any) : any => {
        ...
        const routing: any = getTaskRouting(store, row.task_id);
        const executionWindow: any = readTaskExecutionWindow(store, row.task_id);
        const executionWindowState: any = classifyExecutionWindow(executionWindow);
        const relationship: any = classifyTaskAgentRelationship({
          agentId: agentFilter,
      ...
      const filtered: any = tasks.filter((t: any) : any => {
    ...
    case 'task_lifecycle_diagnose_task_ref': {
      const inputTaskId: any = stringField(args, 'task_id');
      const inputTaskNumber: any = numberField(args, 'task_number');
      if (!inputTaskId && !inputTaskNumber) throw new Error('task_ref_required');
      const lifecycleById: any = inputTaskId ? store.getLifecycle(inputTaskId) : undefined;
      const lifecycleByNumber: any = inputTaskNumber ? store.getLifecycleByNumber(inputTaskNumber) : undefined;
      const collision: any = Boolean(lifecycleById && lifecycleByNumber && lifecycleById.task_id !== lifecycleByNumber.task_id);
      const lifecycle: any = lifecycleById ?? lifecycleByNumber;
      const taskId: any = lifecycle?.task_id ?? inputTaskId ?? null;
      const taskNumber: any = lifecycle?.task_number ?? inputTaskNumber ?? null;
      const spec: any = taskId ? store.getTaskSpec(taskId) : undefined;
      ...
      const refIds: any = [...new Set([inputTaskId, inputTaskNumber === undefined ? null : String(inputTaskNumber)].filter(Boolean))];
      const directiveTables: any = store.db.prepare(`
        SELECT name
        FROM sqlite_master
        WHERE type = 'table' AND name IN ('directive_records', 'directive_refs')
      `).all().map((row: any) : any => row.name);
      const directiveReferenceStorePresent: any = directiveTables.includes('directive_records') && directiveTables.includes('directive_refs');
      const directiveRefs: any = !directiveReferenceStorePresent || refIds.length === 0
        ? []
        : store.db.prepare(`
          ...
          WHERE dr.ref_kind = 'task' AND dr.ref_id IN (${refIds.map(() : any => '?').join(', ')})
          ORDER BY dr.directive_id, dr.ref_id
        `).all(...refIds);
      const unsafeDirectiveRefs: any = directiveRefs.filter((ref: any) : any => !ref.directive_present || (taskId && ref.ref_id !== taskId && ref.ref_id !== String(taskNumber)));
      return jsonToolResult({
    ...
    case 'task_lifecycle_show': {
      ...
      const spec: any = store.getTaskSpec(lifecycle.task_id);
      const routing: any = getTaskRouting(store, lifecycle.task_id);
      const rolePolicy: any = resolveTaskRolePolicy({ siteRoot, taskSpec: spec });
      const executionWindow: any = readTaskExecutionWindow(store, lifecycle.task_id);
      const executionWindowState: any = classifyExecutionWindow(executionWindow);
      const assignment: any = store.db.prepare("SELECT * FROM task_assignments WHERE task_id = ? AND released_at IS NULL ORDER BY claimed_at DESC LIMIT 1").get(lifecycle.task_id);
      const observations: any = store.db.prepare("SELECT * FROM observation_artifacts WHERE task_id = ? ORDER BY created_at DESC").all(lifecycle.task_id);
      const reviewRows: any = store.db.prepare("SELECT * FROM task_reviews WHERE task_id = ? ORDER BY reviewed_at DESC").all(lifecycle.task_id);
      const assignmentIntents: any = store.listAssignmentIntentsForTask ? store.listAssignmentIntentsForTask(lifecycle.task_id) : [];
      const reviews: any = reviewRows.map((r: any) : any => ({
      ...
      let body: any = null;
      try {
        const taskFile: any = await findTaskFile(siteRoot, String(taskNumber));
        if (taskFile) {
    ...
    case 'task_lifecycle_claim': {
      const outcome: any = await claimTaskLifecycleWithGuards(args);
    ...
    case 'task_lifecycle_continue': {
      ...
      const lifecycle: any = store.getLifecycleByNumber(taskNumber);
      if (!lifecycle) throw new Error(`task_not_found: ${taskNumber}`);
      const executionWindowGate: any = evaluateExecutionWindowMutationGate({ store, lifecycle, args, mutation: 'continue' });
      if (executionWindowGate.status === 'blocked') return jsonToolResult(executionWindowGate, true);

      // Role eligibility check
      const eligibility: any = checkTaskRoleEligibilityLocal({ store, siteRoot, taskId: lifecycle.task_id, taskNumber, agentId });
      ...
      const result: any = await withAuthoredRosterJsonPreserved(siteRoot, () : any => continueTaskService({ cwd: siteRoot, taskNumber, agent: agentId, reason }));
    ...
    case 'task_lifecycle_dependency_disposition_record': {
      ...
      const requiredOutcomeId: any = stringField(args, 'required_outcome_id') ?? latestOutcome?.outcome_id;
      if (!requiredOutcomeId) throw new Error('required_outcome_id_required_without_outcome');
      const authorityBasis: any = args.authority_basis ?? null;
      ...
      const targetTaskId: any = stringField(args, 'target_task_id') ?? null;
      if (targetTaskId && !store.getLifecycle(targetTaskId)) throw new Error(`target_task_not_found: ${targetTaskId}`);
      const routedObligationId: any = stringField(args, 'routed_obligation_id') ?? null;
      ...
      const requestedStatus: any = stringField(args, 'status');
      const status: any = requestedStatus ?? ((kind === 'operator_deferred' || kind === 'out_of_scope_or_rejected') ? 'deferred' : 'open');
    ...
    case 'task_lifecycle_unclaim': {
      const taskNumber: any = numberField(args, 'task_number');
      const agentId: any = stringField(args, 'agent_id');
      const reason: any = stringField(args, 'reason') ?? 'mcp_unclaim';
      if (!taskNumber) throw new Error('task_number_required');
      if (agentId) enforceSessionIdentity(agentId);
      const serviceResult: any = await unclaimLifecycleTask({ siteRoot, store, taskNumber, agentId, reason });
    ...
    case 'task_lifecycle_next': {
      ...
      const all: any = store.getAllLifecycle();
      const executionWindowSummary: any = summarizeExecutionWindowGates(store, all);
      const allForWorkboard: any = all.filter((task: any) : any => !executionWindowSummary.hidden_task_ids.has(task.task_id));
      const board: any = buildUnifiedWorkboard({ store, siteRoot, agentId, agentRole, allTasks: allForWorkboard, limit });
      const correctiveDebtReadiness: any = buildSharedCorrectiveDebtReadiness({ siteRoot, store, allTasks: all });
      ...
      const mcpFreshness: any = buildTaskLifecycleFreshness({ registeredTools: taskLifecycleTools().map((t: any) : any => t.name) });
      ...
      const myInProgress: any = board.in_progress.filter((t: any) : any => t.assigned_agent === agentId);
      const myNeedsContinuation: any = board.needs_continuation.filter((t: any) : any => t.assigned_agent === agentId);

      // pending_reviews = only in_review tasks that the agent has an open review obligation for
      const obligatedTaskIds: any = new Set(board.my_review_obligations.map((o: any) : any => o.task_id));
      const pending_reviews: any = board.pending_reviews.filter((t: any) : any => obligatedTaskIds.has(t.task_id));
      ...
      const workloopGuidance: any = buildWorkloopGuidance({
        agentId,
        agentRole,
        recommendation: finalRecommendation,
    ...
    case 'task_lifecycle_workboard_snapshot': {
      ...
      const myNeedsContinuation: any = board.needs_continuation.filter((t: any) : any => t.assigned_agent === agentId);
      const obligatedTaskIds: any = new Set(board.my_review_obligations.map((o: any) : any => o.task_id));
      const pendingReviews: any = board.pending_reviews.filter((t: any) : any => obligatedTaskIds.has(t.task_id));
    ...
    case 'task_lifecycle_obligations': {
      ...
      const obligations: any = store.listDirectedObligationsForTarget(agentId, agentRole, status)
        .map((o: any) : any => {
          const spec: any = o.task_number ? store.getTaskSpecByNumber(o.task_number) : null;
          return {
    ...
    case 'task_lifecycle_inspect': {
      ...
      const routing: any = readTaskRouting(store, lifecycle.task_id, spec);
      ...
      return jsonToolResult({
        ...
        reports: reports || [],
        reviews: reviews || [],
        obligations: obligations.map((o: any) : any => ({ obligation_id: o.obligation_id, kind: o.kind, status: o.status })),
    ...
    case 'task_lifecycle_self_certification_preflight': {
      const packet: any = objectField(args, 'self_certification');
      if (!packet) throw new Error('self_certification_required');
      const validation: any = validateSelfCertificationPacket({
        ...packet,
    ...
    case 'task_lifecycle_admit_evidence': {
      ...
      const admission: any = await admitTaskEvidence({ cwd: siteRoot, taskNumber, admittedBy: agentId, methods: ['admission'] });
      const evidencePreflight: any = await buildTaskEvidencePreflight({ siteRoot, store, taskNumber });
    ...
    case 'task_lifecycle_disposition_closeout': {
      ...
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const result: any = await taskLifecycleDispositionCloseout({
        siteRoot,
        store,
        taskNumber,
        agentId,
    ...
    case 'task_lifecycle_audit': {
      const since: any = stringField(args, 'since');
      const until: any = stringField(args, 'until');
      ...
      `;
      const rows: any = store.db.prepare(sql).all(sinceVal, untilVal, sinceVal, untilVal, sinceVal, untilVal, sinceVal, untilVal, sinceVal, untilVal);
      return jsonToolResult({
        status: 'ok',
    ...
    case 'task_lifecycle_finish': {
      ...
      const agentId: any = stringField(args, 'agent_id');
      const summary: any = stringField(args, 'summary');
      const verdict: any = stringField(args, 'verdict');
      ...
      const selfCertification: any = objectField(args, 'self_certification');
      const payloadSource: any = dispatchContext.payloadSource ?? null;
      if (!taskNumber) throw new Error('task_number_required');
      if (!agentId) throw new Error('agent_id_required');
      ...
      enforceSessionIdentity(agentId);
      const identityWarning: any = verifySessionIdentity(agentId);
      const truthfulnessGate: any = validateTaskFinishRecoveryTruthfulness({
        taskNumber,
        summary,
        changedFiles,
        noFilesChanged,
      ...
      if (truthfulnessGate.evaluation?.normalized_state === 'terminal_blocked') {
        const payload: any = buildTerminalBlockedFinishBlock({ taskNumber, agentId, source: 'recovery_truthfulness_argument' });
        if (identityWarning) {
          payload.identity_warning = identityWarning;
        }
        return jsonToolResult(attachPayloadSource(payload, payloadSource), true);
      ...
      const lifecycle: any = store.getLifecycleByNumber(taskNumber);
      const testGate: any = lifecycle ? testResultArtifactGate(store, lifecycle.task_id) : { failed_test_artifacts: [], latest_passing_artifacts: [] };
      ...
      if (taskFile) {
        const { body }: any = await readTaskFile(taskFile.path);
        const selfCertificationValidation: any = selfCertification
        ...
        const recoveryTruthfulnessValidation: any = recoveryTruthfulness
          ? { ok: true }
        ...
        if (recoveryTruthfulnessValidation.evaluation?.normalized_state === 'terminal_blocked') {
          const payload: any = buildTerminalBlockedFinishBlock({ taskNumber, agentId, source: 'task_body_recovery_truthfulness' });
          if (identityWarning) {
            payload.identity_warning = identityWarning;
          }
          return jsonToolResult(attachPayloadSource(payload, payloadSource), true);
      ...
      const autoDetectedChangedFiles: any = !changedFiles && !noFilesChanged ? detectGitChangedFiles(siteRoot) : [];
      const finishOptions: any = { cwd: siteRoot, taskNumber, agent: agentId, summary, verdict, close: true };
      ...
      const result: any = await withAuthoredRosterJsonPreserved(siteRoot, () : any => finishTaskService(finishOptions));
      const payload: any = result.result || result;
      let isBlocked: any = payload.close_action === 'blocked';
      if (isBlocked) {
        const reconciled: any = await reconcileStructuredPreflightClose({
          siteRoot,
          store,
          taskNumber,
          agentId,
      ...
      if (isBlocked) {
        ...
        const remediation: any = buildStateAwareFinishBlockerRemediation({ taskNumber, agentId, lifecycle, payload });
    ...
    case 'task_lifecycle_close': {
      const taskNumber: any = numberField(args, 'task_number');
      const agentId: any = stringField(args, 'agent_id');
      const mode: any = stringField(args, 'mode') || 'agent_finish';
      const noContinuationNeeded: any = stringField(args, 'no_continuation_needed');
      if (!taskNumber) throw new Error('task_number_required');
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const selfCertification: any = objectField(args, 'self_certification');
      ...
      const result: any = await withAuthoredRosterJsonPreserved(siteRoot, () : any => closeTaskService({ cwd: siteRoot, taskNumber, by: agentId, mode, noContinuationNeeded }));
      const payload: any = result.result || result;
      const isBlocked: any = result.exitCode !== 0 || payload.close_action === 'blocked';
      if (!isBlocked) {
        payload.post_closeout_continuation = buildPostCloseoutContinuation({ agentId, result: payload });
      }
    ...
    case 'task_lifecycle_tombstone': {
      ...
      const disposition: any = stringField(args, 'disposition') ?? 'tombstoned';
      const metadata: any = objectField(args, 'metadata');
      if (!taskNumber) throw new Error('task_number_required');
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const serviceResult: any = await tombstoneLifecycleTask({ siteRoot, store, taskNumber, agentId, reason, authorityBasis, disposition, metadata });
      return jsonToolResult(serviceResult, serviceResult.status === 'error');
    }

    case 'task_lifecycle_search': {
      const query: any = stringField(args, 'query');
      const statusFilter: any = stringField(args, 'status');
      const limit: any = numberField(args, 'limit') ?? 20;
      if (!query) throw new Error('query_required');
      const result: any = await searchTasksService({ cwd: siteRoot, query, maxSnippets: 3 });
      const output: any = result.result || result;
      if (statusFilter && output.results) {
        output.results = output.results.filter((r: any) : any => r.status === statusFilter);
    ...
    case 'task_lifecycle_related': {
      ...
      const limit: any = numberField(args, 'limit') ?? 8;
      if (!taskNumber) throw new Error('task_number_required');
      const result: any = findRelatedTasks({ tasksDir: join(siteRoot, '.ai', 'do-not-open', 'tasks'), targetTaskNumber: taskNumber, limit });
    ...
    case 'task_lifecycle_defer': {
      ...
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const serviceResult: any = await transitionLifecycleTask({ siteRoot, store, taskNumber, agentId, reason, toStatus: 'deferred', resultStatus: 'deferred' });
      return jsonToolResult(serviceResult, serviceResult.status === 'error');
    }
    ...
    case 'task_lifecycle_un_defer': {
      ...
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const serviceResult: any = await unDeferLifecycleTask({ siteRoot, store, taskNumber, agentId, reason, authorityBasis });
      return jsonToolResult(serviceResult, serviceResult.status === 'error');
    }
    ...
    case 'task_lifecycle_reopen': {
      ...
      if (!agentId) throw new Error('agent_id_required');
      enforceSessionIdentity(agentId);
      const serviceResult: any = await transitionLifecycleTask({ siteRoot, store, taskNumber, agentId, reason, toStatus: 'opened', resultStatus: 'reopened' });
      return jsonToolResult(serviceResult, serviceResult.status === 'error');
    }
    ...
    case 'task_lifecycle_review': {
      ...
      const verdict: any = stringField(args, 'verdict');
      const payloadSource: any = dispatchContext.payloadSource ?? null;
      let findings: any = args.findings;
      ...
      const reviewReplayStatus: any = normalizeReviewReplayStatus(stringField(args, 'review_replay_status'));
      ...
      let structuralReviewInfo: any = null;
      try {
      ...
      const isStructuralReview: any = structuralReviewInfo?.sameOperator || structuralReviewInfo?.selfReview;
      ...
      const provenanceAnnotation: any = buildReviewAcceptanceProvenanceAnnotation({ verdict, reviewReplayStatus });
      if (Array.isArray(parsedFindings)) {
      ...
      const result: any = await withAuthoredRosterJsonPreserved(siteRoot, () : any => reviewTaskService({ cwd: siteRoot, taskNumber, agent: agentId, verdict, findings }));
      const payload: any = result.result || result;
      let isBlocked: any = payload.evidence_blocked === true || payload.close_action === 'blocked';
      ...
      if (payload.review_id) {
        try {
          const reviewRow: any = store.db.prepare('SELECT * FROM task_reviews WHERE review_id = ?').get(payload.review_id);
    ...
    case 'task_lifecycle_route_task_set': {
      ...
      const result: any = await applyBulkTaskRouting({ store, siteRoot, args, actorAgentId, actorRole });
    ...
    case 'mcp_command_submit':
      return jsonToolResult(await commandSubmitAsync({
        ...
        args,
        admitters: {
          'narada.command.task.create.v1': (command: any) : any => admitTaskPayloadRefFacadeCommand({
            command,
            toolName: 'task_lifecycle_create',
            expectedKinds: ['task_lifecycle_create', 'task_lifecycle_chapter_create'],
            resultSchema: 'narada.command.task.create.result.v1',
          }),
          'narada.command.task.chapter_upsert.v1': (command: any) : any => admitTaskFacadeCommand({
            command,
          ...
          'narada.command.task.claim.v1': (command: any) : any => admitTaskClaimCommand(command),
          'narada.command.task.finish.v1': (command: any) : any => admitTaskFacadeCommand({
            command,
          ...
          'narada.command.task.disposition_closeout.v1': (command: any) : any => admitTaskFacadeCommand({
            command,
          ...
          'narada.command.task.inbox_target.v1': (command: any) : any => admitTaskFacadeCommand({
            command,
          ...
          'narada.command.task.test_replay.v1': (command: any) : any => admitTaskTestReplayCommand(command),
        },
      }));
    ...
    case 'task_lifecycle_replay_test_evidence': {
      ...
      enforceSessionIdentity(agentId);
      const taskNumber: any = numberField(args, 'task_number');
      const evidenceRef: any = stringField(args, 'evidence_ref');
      ...
      const replayPayload: any = normalizeTestReplayPayload(args);
      const authorityBasis: any = normalizeReplayAuthorityBasis(args.authority_basis)
        ?? { kind: 'operator_direct_instruction', summary: 'Governed test replay requested through task lifecycle MCP.' };
      const result: any = await commandAuthorAndSubmitAsync({
        siteRoot,
        args: {
    ...
    case 'task_lifecycle_create': {
      ...
      const goal: any = stringField(args, 'goal') || title;
      const context: any = stringField(args, 'context') || null;
      const requiredWork: any = stringField(args, 'required_work') || '1. TBD';
      const nonGoals: any = stringField(args, 'non_goals') || null;
      const preferredRole: any = stringField(args, 'preferred_role') || null;
      const targetRole: any = stringField(args, 'target_role') || null;
      const acceptanceCriteria: any = Array.isArray(args.acceptance_criteria) && args.acceptance_criteria.length > 0
        ? args.acceptance_criteria
        : ['TBD'];
      const executionWindow: any = normalizeExecutionWindow(args).window;
    ...
    case 'task_lifecycle_recurring_create': {
      const title: any = stringField(args, 'title');
      const actorAgentId: any = stringField(args, 'actor_agent_id');
      const authorityBasis: any = normalizeRecurringAuthorityBasis(args.authority_basis);
      if (!title) throw new Error('title_required');
      if (!actorAgentId) throw new Error('actor_agent_id_required');
      ...
      const initialStatus: any = stringField(args, 'initial_status') || 'active';
      if (!['draft', 'active'].includes(initialStatus)) throw new Error('invalid_initial_status');
      const targetRole: any = stringField(args, 'target_role') || null;
      const preferredRole: any = stringField(args, 'preferred_role') || targetRole;
      if (targetRole && !roleExistsInRoster(store, siteRoot, targetRole)) {
        return jsonToolResult({ status: 'blocked', reason: 'target_role_not_in_roster', target_role: targetRole }, true);
      }
      ...
      const triggerMode: any = stringField(args, 'trigger_mode') || 'manual';
      if (!['manual', 'schedule'].includes(triggerMode)) throw new Error('invalid_trigger_mode');
      const scheduleKind: any = stringField(args, 'schedule_kind') || (triggerMode === 'schedule' ? 'daily' : null);
      ...
      const scheduleTimezone: any = stringField(args, 'schedule_timezone') || (triggerMode === 'schedule' ? 'UTC' : null);
      if (scheduleTimezone && scheduleTimezone !== 'UTC') throw new Error('unsupported_schedule_timezone');
      const recurrenceId: any = `rtask_${randomUUID()}`;
      const now: any = new Date().toISOString();
    ...
    case 'task_lifecycle_recurring_show': {
      const recurrenceId: any = stringField(args, 'recurrence_id');
      ...
      const includeRuns: any = booleanField(args, 'include_runs') ?? true;
      return jsonToolResult({
    ...
    case 'task_lifecycle_recurring_trigger': {
      ...
      const actorAgentId: any = stringField(args, 'actor_agent_id');
      const authorityBasis: any = normalizeRecurringAuthorityBasis(args.authority_basis);
      const runReason: any = stringField(args, 'run_reason');
      if (!recurrenceId) throw new Error('recurrence_id_required');
      if (!actorAgentId) throw new Error('actor_agent_id_required');
      if (!authorityBasis) throw new Error('valid_authority_basis_required');
      ...
      const now: any = new Date().toISOString();
      const taskNumber: any = (await allocateTaskNumbers(siteRoot, 1))[0];
      const taskTitle: any = `${definition.title} (${now.slice(0, 10)})`;
    ...
    case 'task_lifecycle_recurring_run_due': {
      ...
      enforceSessionIdentity(actorAgentId);
      const actorRole: any = requireRecurringAuthorityActor({ store, siteRoot, actorAgentId });
      const now: any = parseIsoOrNow(stringField(args, 'current_time'));
      const limit: any = Math.max(1, Math.min(numberField(args, 'limit') ?? 20, 100));
      const dueDefinitions: any = listDueRecurringDefinitions(store, { now, limit });
      const created: any = [];
      const skipped: any = [];
      for (const definition of dueDefinitions) {
        const dueKey: any = recurringDueKey(definition, now);
        ...
        const result: any = await createRecurringTaskInstance({
          store,
          siteRoot,
          definition,
          actorAgentId,
...
function buildTaskLifecycleFreshness({ registeredTools }: any) : any {
  return buildMcpFreshnessStatus({
    ...
    serverBootedAt: SERVER_BOOTED_AT,
    watchedPaths: ['tools/task-lifecycle', 'tools/mcp-freshness-service.ts'],
    expectedTools: taskLifecycleTools().map((tool: any) : any => tool.name),
    registeredTools,
    restartRequestPath: join(siteRoot, '.ai', 'tmp', 'task-lifecycle-restart-request.json'),
    baselinePath: join(siteRoot, '.ai', 'tmp', 'task-lifecycle-mcp-baseline.json'),
...
function taskLifecycleRestart(args: any) : any {
  ...
  const baselinePath: any = join(siteRoot, '.ai', 'tmp', 'task-lifecycle-mcp-baseline.json');
  const watchedPaths: any = ['tools/task-lifecycle', 'tools/mcp-freshness-service.ts'];
  ...
  if (mode === 'acknowledge' || mode === 'clear') {
    return acknowledgeMcpRestartRequest({
      ...
      targetEntrypoint: 'tools/task-lifecycle/task-mcp-server.ts',
      restartRequestPath: requestPath,
      baselinePath,
      watchedPaths,
      expectedTools: taskLifecycleTools().map((tool: any) : any => tool.name),
      registeredTools: taskLifecycleTools().map((tool: any) : any => tool.name),
  ...
  if (mode === 'status') {
    return {
      ...
      mcp_freshness: buildTaskLifecycleFreshness({ registeredTools: taskLifecycleTools().map((tool: any) : any => tool.name) }),
...
async function buildTaskEvidencePreflight({ siteRoot, store, taskNumber }: any) : Promise<any> {
  const lifecycle: any = store.getLifecycleByNumber(taskNumber);
  if (!lifecycle) throw new Error(`task_not_found: ${taskNumber}`);
  const evidence: any = await inspectTaskEvidence(siteRoot, String(taskNumber), store);
  const taskFile: any = await findTaskFile(siteRoot, taskNumber);
  ...
  const structuredReportEvidence: any = collectStructuredReportEvidence(reports, sqliteReports);
  ...
  const recoveryTruthfulnessTriggered: any = recoveryTruthfulnessValidation.evaluation?.triggered === true;
  const hasCanonicalFollowUpLedger: any = /^##\s+Follow-Up Ledger\s*$/mi.test(body);
  const mentionsFollowUpLedger: any = /Follow-Up Ledger/i.test(body);
  ...
  addRequirement(requirements, {
    ...
    observed: {
      ...
      passed_verification_runs: verificationRuns.filter((run: any) : any => run.status === 'passed').map((run: any) : any => run.run_id),
  ...
  const blockers: any = requirements.filter((item: any) : any => item.required_for_finish && item.satisfied !== true).map((item: any) : any => ({
    id: item.id,
    label: item.label,
    remediation: item.remediation,
    next_action: item.next_action,
  }));
  const closeReadiness: any = buildTaskCloseReadiness({
    taskNumber,
...
async function reconcileStructuredPreflightClose({ siteRoot, store, taskNumber, agentId, payload, closeMode }: any) : Promise<any> {
  const evidencePreflight: any = await buildTaskEvidencePreflight({ siteRoot, store, taskNumber });
  ...
  const admission: any = await admitTaskEvidence({ cwd: siteRoot, taskNumber, admittedBy: agentId, methods: ['admission'], store });
  ...
  const closeResult: any = await closeTaskService({ cwd: siteRoot, taskNumber, by: agentId, mode: closeMode, store });
  const closePayload: any = closeResult.result || closeResult;
...
function buildTaskCloseReadiness({ taskNumber, taskId, lifecycleStatus, verdict, blockers = [], requirements = [] }: any) : any {
  const requiredRequirements: any = requirements.filter((item: any) : any => item.required_for_finish !== false);
  const blockerIds: any = new Set(blockers.map((item: any) : any => item.id));
  const evidence: any = Object.fromEntries(requiredRequirements.map((item: any) : any => [item.id, {
  ...
  return {
    ...
    requirements: requiredRequirements.map((item: any) : any => ({
      id: item.id,
      label: item.label,
      satisfied: item.satisfied === true,
      required_for_finish: item.required_for_finish !== false,
...
function closeReadinessFromPreflight(evidencePreflight: any) : any {
  return evidencePreflight?.close_readiness ?? null;
}

function addRequirement(requirements: any, item: any) : any {
  requirements.push({
...
function buildEvidenceNextAction(id: any, taskNumber: any) : any {
  const actions: any = {
    execution_notes: {
...
function hasMaterialTaskSection(body: any, heading: any) : any {
...
function extractTaskSection(body: any, heading: any) : any {
...
function collectChangedFileEvidenceFromReports(reportRecords: any, sqliteReports: any) : any {
  const files: any = [];
  ...
  return {
    changedFiles: [...new Set(files.filter((file: any) : any => typeof file === 'string' && file.trim().length > 0 && file !== NO_FILES_CHANGED_MARKER))],
    noFilesChangedDeclarations: uniqueDeclarations,
  };
}

function collectStructuredReportEvidence(reportRecords: any, sqliteReports: any) : any {
  const executionReportIds: any = [];
  const verificationReportIds: any = [];
  for (const report of reportRecords) {
    try {
      const parsed: any = JSON.parse(report.report_json);
...
function collectStructuredReportEvidenceFromEntry(entry: any, executionReportIds: any, verificationReportIds: any) : any {
  const reportId: any = entry.reportId ?? 'unknown_report';
  const summary: any = typeof entry.summary === 'string' ? entry.summary.trim() : '';
  const changedFiles: any = Array.isArray(entry.changedFiles) ? entry.changedFiles.filter((file: any) : any => typeof file === 'string' && file !== NO_FILES_CHANGED_MARKER) : [];
  const verification: any = Array.isArray(entry.verification) ? entry.verification : [];
  if (summary.length >= 80 && changedFiles.length > 0) {
    executionReportIds.push(reportId);
  }
  const verificationText: any = `${summary} ${JSON.stringify(verification)}`.toLowerCase();
  const namesVerification: any = /\b(verif(?:y|ied|ication)|test(?:ed|s)?|passed|checked|evidence)\b/.test(verificationText);
...
function safeJsonObject(value: any) : any {
  try {
    const parsed: any = JSON.parse(value ?? '{}');
...
function countReportVerificationEntries(reportRecords: any, sqliteReports: any) : any {
  let count: any = 0;
  for (const report of reportRecords) {
    try {
      const parsed: any = JSON.parse(report.report_json);
...
function legacyTaskLifecycleToolsSnapshot() : any {
  return [
    {
    ...
    },
    {
      ...
      inputSchema: objectSchema({
        server_path: stringSchema('Path to the MCP server script relative to site root (e.g., "tools/task-lifecycle/task-mcp-server.ts").'),
...
function testResultArtifactGate(store: any, taskId: any) : any {
  ...
  const latestBySelector: any = new Map();
  const latestPassingBySelector: any = new Map();
...
function parseTestResultArtifact(row: any) : any {
  try {
...
function failedTestResultArtifacts(store: any, taskId: any) : any {
...
function testTargetsForSelector(selector: any) : any {
  switch (selector) {
    case 'task-lifecycle':
      return [
        { test_id: 'task_next_cli' },
        { path: 'tools/task-lifecycle/tests/Test-TaskRoutingMcp.ts' },
        { path: 'tools/task-lifecycle/tests/Test-PreferredAgentMismatchClaimAuthority.ts' },
        { path: 'tools/task-lifecycle/tests/Test-RecoveryTruthfulnessGuard.ts' },
        { path: 'tools/task-lifecycle/tests/Test-McpFinishChangedFileEvidence.ts' },
        { path: 'tools/task-lifecycle/tests/Test-SelfCertificationGuard.ts' },
        { path: 'tools/task-lifecycle/tests/Test-SelfCertificationMcpFinishGuard.ts' },
        { path: 'tools/task-lifecycle/tests/Test-AgentsPostureAudit.ts' },
        { path: 'tools/task-lifecycle/tests/Test-RosterSqlVolatileState.ts' },
        { path: 'tools/task-lifecycle/tests/Test-CapaDispositionCorrectiveCoverage.ts' },
        { path: 'tools/task-lifecycle/tests/Test-TaskBatchCreationMcp.ts' },
        { path: 'tools/task-lifecycle/tests/Test-TaskExecutionWindowMcp.ts' },
      ];
    case 'typed-mcp':
      return [{ test_id: 'mcp_surface_registry_validation' }];
    case 'operator-surface':
      return [
        { path: 'tools/operator-surface-carriers/Test-AcceptanceCriteriaBodyEnforcement.test.ts' },
        { path: 'tools/operator-surface-carriers/agent-desktop-shortcuts-authority.test.ts' },
        { path: 'tools/operator-surface/osm-send-permission-policy.test.ts' },
        { path: 'tools/operator-surface/operator-surface-shutdown-paths.test.ts' },
      ];
    case 'all':
      return [
        { test_id: 'task_next_cli' },
...
function drainJsonRpcFrames(input: any) : any {
  ...
  while (cursor < input.length) {
    ...
    const body: any = input.slice(bodyStart, bodyEnd);
    try {
...
function validateArgs(toolName: any, args: any, schema: any) : any {
  ...
  for (const [key, value] of Object.entries(args)) {
    ...
    const expectedType: any = propSchema.type;
...
function suggestCanonicalToolForUnexpectedField(toolName: any, field: any) : any {
...
function buildInlinePayloadRemediation(toolName: any, error: any) : any {
  const message: any = String(error?.message ?? '');
  const match: any = message.match(/field=([^ ]+) length=([0-9]+) threshold=([0-9]+)/);
  const helper: any = payloadRefHelperForTool(toolName);
  return {
    status: 'error',
...
function payloadRefHelperForTool(toolName: any) : any {
  const helpers: any = {
...
function authorityBasisSchema(description: any) : any {
  return {
    type: 'object',
    description,
    properties: {
...
function nullableStringSchema(description: any) : any {
...
function computeStateFreshness(lastWorkboardCheckAt: any, generatedAt: any) : any {
  const now: any = new Date();
  ...
  const secondsSinceCheck: any = Math.floor((generated - lastCheck) / 1000);
...
function summarizeWorkboardTask(task: any) : any {
  if (!task) return null;
  return {
...
function buildWorkloopGuidance({ agentId, agentRole, recommendation, executableWorkAvailable }: any) : any {
  const task: any = recommendation?.task ?? null;
  const capa: any = recommendation?.capa ?? null;
  const summary: any = {
...
function buildStateAwareFinishBlockerRemediation({ taskNumber, agentId, lifecycle, payload }: any) : any {
  const closeBlockers: any = Array.isArray(payload.close_blockers) ? payload.close_blockers : [];
  const blockerText: any = closeBlockers.join('\n');
  const currentStatus: any = lifecycle?.status ?? payload.current_status ?? null;
  ...
  if (/Latest Evidence Admission result is rejected|evidence repair/i.test(blockerText)) {
    const continueAllowed: any = currentStatus === 'needs_continuation' || currentStatus === 'in_review';
    return {
      next_action: {
...
function readTaskRouting(store: any, taskId: any, spec: any = null) : any {
  let rolePref: any = null;
  try {
...
function buildRoutingAssignmentDivergence({ lifecycle, routing, assignment, reports }: any) : any {
  const preferredAgentId: any = routing?.preferred_agent_id ?? null;
  const activeAgentId: any = assignment?.agent_id ?? null;
  const reportAgentIds: any = [...new Set((reports || []).map((report: any) : any => report.agent_id).filter(Boolean))];
  const finishedBy: any = lifecycle.closed_by ?? (reportAgentIds.length === 1 ? reportAgentIds[0] : null);
  const activeMismatch: any = Boolean(preferredAgentId && activeAgentId && preferredAgentId !== activeAgentId);
  const finishedMismatch: any = Boolean(preferredAgentId && finishedBy && preferredAgentId !== finishedBy);
  return {
...
async function admitTaskClaimCommand(command: any) : Promise<any> {
  const commandTargetRoot: any = stringField(command, 'target_site_root');
  ...
  const domainArgs: any = asRecord(command.domain_args);
  const authorityBasis: any = isPlainObject(domainArgs.authority_basis)
  ...
  const expected: any = asRecord(command.expected_consequence);
  const expectedKind: any = stringField(expected, 'kind');
  ...
  const taskNumber: any = numberField(args, 'task_number');
  const agentId: any = stringField(args, 'agent_id');
  const expectedTaskNumber: any = numberField(expected, 'task_number');
  const expectedAgentId: any = stringField(expected, 'assignment_agent_id') ?? stringField(expected, 'agent_id');
...
async function admitTaskFacadeCommand({ command, toolName, expectedKinds, resultSchema }: any) : Promise<any> {
  const commandTargetRoot: any = stringField(command, 'target_site_root');
  if (!commandTargetRoot) {
    return refusedCommandResult('target_locus_mismatch', {
      status: 'target_locus_mismatch',
  ...
  const mergedPayload: any = readCommandPayloadArgs(command);
  const args: any = {
    ...mergedPayload,
  ...
  const expectedAgentId: any = stringField(expected, 'agent_id')
  ...
  const expectedVerdict: any = stringField(expected, 'verdict');
  const verdict: any = stringField(args, 'verdict');
  ...
  const directResult: any = await callLifecycleFacadeForCommand(toolName, args);
  const directPayload: any = parseLifecycleFacadeCommandResult(directResult);
...
async function admitTaskPayloadRefFacadeCommand({ command, toolName, expectedKinds, resultSchema }: any) : Promise<any> {
  const commandTargetRoot: any = stringField(command, 'target_site_root');
  if (!commandTargetRoot) {
    return refusedCommandResult('target_locus_mismatch', {
      status: 'target_locus_mismatch',
  ...
  const payloadRef: any = firstCommandPayloadRef(command);
  if (!payloadRef) {
  ...
  const facadeArgs: any = toolName === 'task_lifecycle_create'
    ? resolveTaskCreatePayloadArgs(args)
    : { args, payloadSource: null };
  const directResult: any = await callLifecycleFacadeForCommand(toolName, facadeArgs.args, { payloadSource: facadeArgs.payloadSource });
  const directPayload: any = parseLifecycleFacadeCommandResult(directResult);
  if (directResult.isError) {
    return refusedCommandResult(mapTaskFacadeRefusalCode(directPayload), {
      direct_tool: toolName,
...
async function admitTaskTestReplayCommand(command: any) : Promise<any> {
  const commandTargetRoot: any = stringField(command, 'target_site_root');
  if (!commandTargetRoot) {
    return refusedCommandResult('target_locus_mismatch', {
      status: 'target_locus_mismatch',
  ...
  const domainArgs: any = asRecord(command.domain_args);
  const agentId: any = stringField(domainArgs, 'agent_id');
  if (!agentId) return refusedCommandResult('agent_id_required', { status: 'agent_id_required' });
  enforceSessionIdentity(agentId);
  const taskNumber: any = numberField(domainArgs, 'task_number');
  const evidenceRef: any = stringField(domainArgs, 'evidence_ref');
  ...
  const targets: any = testReplayTargets(payload);
  const timeoutSeconds: any = payload.timeout_seconds ?? 120;
  const results: any = [];
  for (const target of targets) {
    results.push(await testMcpTool(siteRoot, 'tools/mcp-servers/test/test-mcp-server.ts', 'run_test', target, { timeoutSeconds }));
  }
  const failed: any = results.filter((result: any) : any => result.status !== 'passed');
  const replayResult: any = {
...
function normalizeReplayAuthorityBasis(value: any) : any {
  ...
  const allowed: any = new Set(['operator_direct_instruction', 'directed_obligation', 'task_owner_handoff']);
  if (!allowed.has(kind) || !summary) throw new Error('authority_basis_invalid_for_test_replay');
  return { kind, summary };
}

function normalizeTestReplayPayload(value: any) : any {
  const input: any = asRecord(value);
  const forbidden: any = ['command', 'args', 'cwd', 'working_directory', 'network', 'shell', 'script', 'destructive'];
  const presentForbidden: any = forbidden.filter((field: any) : any => Object.prototype.hasOwnProperty.call(input, field));
  if (presentForbidden.length > 0) throw new Error(`test_replay_payload_forbidden_fields: ${presentForbidden.join(',')}`);
  const selector: any = stringField(input, 'selector');
  const testId: any = stringField(input, 'test_id');
  const path: any = stringField(input, 'path');
  const selected: any = [selector, testId, path].filter(Boolean);
  ...
  const timeoutSeconds: any = Math.min(300, Math.max(1, numberField(input, 'timeout_seconds') ?? 120));
  return {
    ...(selector ? { selector } : {}),
...
function testReplayTargets(payload: any) : any {
...
async function callLifecycleFacadeForCommand(toolName: any, args: any, dispatchContext: any = {}) : Promise<any> {
  const previousToolName: any = activeOutputToolName;
...
function parseLifecycleFacadeCommandResult(toolResult: any) : any {
  const text: any = toolResult?.content?.[0]?.text;
  if (typeof text !== 'string') return {};
  try {
    const parsed: any = JSON.parse(text);
    if (typeof parsed?.output_ref === 'string') {
      const shown: any = outputShow({ siteRoot, args: { ref: parsed.output_ref, output_limit: 1_000_000 } });
...
function mapTaskFacadeRefusalCode(result: any) : any {
  const status: any = stringField(result, 'status');
  const error: any = stringField(result, 'error');
  const reasonCode: any = stringField(result, 'reason_code');
...
function refusedCommandResult(reasonCode: any, result: any) : any {
  return {
    status: 'refused',
...
function withTaskClaimFacadeCompatibility(result: any, args: any) : any {
  return {
    ...result,
...
function mapClaimRefusalCode(result: any) : any {
  const status: any = stringField(result, 'status');
...
async function claimTaskLifecycleWithGuards(args: any) : Promise<any> {
  const taskNumber: any = numberField(args, 'task_number');
  const agentId: any = stringField(args, 'agent_id');
  if (!taskNumber) throw new Error('task_number_required');
  if (!agentId) throw new Error('agent_id_required');
  ...
  const lifecycle: any = store.getLifecycleByNumber(taskNumber);
  if (!lifecycle) throw new Error(`task_not_found: ${taskNumber}`);
  const executionWindowGate: any = evaluateExecutionWindowMutationGate({ store, lifecycle, args, mutation: 'claim' });
  ...
  const mismatchAuthority: any = validatePreferredAgentMismatchAuthority({ args, eligibility, lifecycle, taskNumber, agentId });
  ...
  const serviceResult: any = await claimLifecycleTask({ siteRoot, store, taskNumber, agentId });
  ...
  const result: any = { status: 'claimed', assignment_id: serviceResult.assignment_id, task_number: taskNumber };
  const preClaimWarnings: any = [eligibility.roleMismatchWarning, eligibility.preferredAgentWarning].filter(Boolean);
...
function validatePreferredAgentMismatchAuthority({ args, eligibility, lifecycle, taskNumber, agentId }: any) : any {
  ...
  const preferredAgentWarning: any = eligibility.preferredAgentWarning;
  const authorityBasis: any = normalizeClaimAuthorityBasis(args.authority_basis);
  if (!authorityBasis) {
    return {
      status: 'blocked',
...
function normalizeClaimAuthorityBasis(value: any) : any {
  const record: any = asRecord(value);
  const kind: any = stringField(record, 'kind');
  const summary: any = stringField(record, 'summary');
...
function recordClaimIntent({ store, lifecycle, taskNumber, agentId, status, assignmentId = null, rejectionReason = null, authorityBasis = null, preferredAgentWarning = null, preClaimWarnings = null }: any) : any {
...
function buildTerminalBlockedFinishBlock({ taskNumber, agentId, source }: any) : any {
  return {
    status: 'blocked',
...
function validateTaskFinishRecoveryTruthfulness({ taskNumber, summary, changedFiles, noFilesChanged, recoveryTruthfulness }: any) : any {
  const packet: any = {
    ...(recoveryTruthfulness ?? {}),
    task_number: taskNumber,
    surface: 'task_lifecycle_finish',
...
function nullableStringField(record: any, key: any) : any {
...
function objectField(record: any, key: any) : any {
  const value: any = record?.[key];
...
function stringArrayField(record: any, key: any) : any {
  const value: any = record?.[key];
  if (!Array.isArray(value)) return undefined;
  const strings: any = value.map((entry) : any => typeof entry === 'string' ? entry.trim() : '').filter((entry) : any => entry.length > 0);
  return strings.length > 0 && strings.length === value.length ? strings : undefined;
}

function detectGitChangedFiles(cwd: any) : any {
  const result: any = runGovernedCommandSync('git', ['diff', '--name-only', 'HEAD'], { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
function ensureTaskExecutionWindowTables(taskStore: any) : any {
  taskStore.db.exec(`
...
function normalizeExecutionWindow(args: any) : any {
  const source: any = asRecord(args.execution_window);
  const notBefore: any = stringField(source, 'not_before') || stringField(args, 'execution_not_before') || null;
  const notAfter: any = stringField(source, 'not_after') || stringField(args, 'execution_not_after') || null;
  const timezone: any = stringField(source, 'timezone') || 'UTC';
  const basis: any = stringField(source, 'basis') || null;
  const expiredDisposition: any = stringField(source, 'expired_disposition') || 'requires_reassessment';
  const errors: any = [];
...
function writeTaskExecutionWindow(taskStore: any, taskId: any, window: any, { source_kind, source_ref }: any = {}) : any {
...
function readTaskExecutionWindow(taskStore: any, taskId: any) : any {
  ensureTaskExecutionWindowTables(taskStore);
  const row: any = taskStore.db.prepare('SELECT * FROM task_execution_windows WHERE task_id = ?').get(taskId);
  if (!row) return null;
  return {
...
function classifyExecutionWindow(window: any, now: any = new Date()) : any {
  if (!window) return { status: 'none', actionable: true };
  const nowMs: any = now.getTime();
  const notBeforeMs: any = window.not_before ? Date.parse(window.not_before) : null;
  const notAfterMs: any = window.not_after ? Date.parse(window.not_after) : null;
...
function evaluateExecutionWindowMutationGate({ store: taskStore, lifecycle, args, mutation }: any) : any {
  const executionWindow: any = readTaskExecutionWindow(taskStore, lifecycle.task_id);
  const state: any = classifyExecutionWindow(executionWindow);
  if (state.actionable) return { status: 'clear' };
  const authority: any = asRecord(args.authority_basis);
  const authorityKind: any = stringField(authority, 'kind');
  const authoritySummary: any = stringField(authority, 'summary');
...
function summarizeExecutionWindowGates(taskStore: any, tasks: any) : any {
  const hidden: any = [];
  for (const task of tasks) {
    const executionWindow: any = readTaskExecutionWindow(taskStore, task.task_id);
    const state: any = classifyExecutionWindow(executionWindow);
  ...
  return {
    hidden_task_ids: new Set(hidden.map((task: any) : any => task.task_id)),
    public: {
...
async function createTaskBatch(args: any, { payloadSource }: any) : Promise<any> {
  const dryRun: any = booleanField(args, 'dry_run') ?? false;
  const actor: any = stringField(args, 'actor_agent_id') || SESSION_IDENTITY || 'unknown';
  if (SESSION_IDENTITY && actor !== SESSION_IDENTITY) enforceSessionIdentity(actor);
  const chapter: any = asRecord(args.chapter);
  const shared: any = asRecord(args.shared);
  const tasks: any = rawArrayField(args, 'tasks').map((task: any) : any => asRecord(task));
  const duplicateWarnings: any = findLikelyBatchDuplicates(tasks);
  const now: any = new Date().toISOString();
  const chapterTitle: any = stringField(chapter, 'title') || stringField(shared, 'chapter_title') || null;
  const chapterId: any = stringField(chapter, 'chapter_id') || (chapterTitle ? `chapter-${slugify(chapterTitle)}` : null);
  const chapterIndexPath: any = chapterId ? join(siteRoot, '.ai', 'do-not-open', 'chapters', `${chapterId}.md`) : null;
  const chapterIndexRelativePath: any = chapterIndexPath ? relativeSitePath(siteRoot, chapterIndexPath) : null;
  const planned: any = tasks.map((task: any, index: any) : any => buildBatchTaskPlan({ task, shared, index, taskNumber: null, payloadSource }));
  const validation: any = validateBatchTaskPlans(planned);
  ...
  const taskNumbers: any = await allocateTaskNumbers(siteRoot, tasks.length);
  const tasksDir: any = join(siteRoot, '.ai', 'do-not-open', 'tasks');
  ...
  for (const [index, task] of tasks.entries()) {
    const taskNumber: any = taskNumbers[index];
    const plan: any = buildBatchTaskPlan({ task, shared, index, taskNumber, payloadSource });
    const taskId: any = `${todayYmd()}-${taskNumber}-${slugify(plan.title)}`;
    const filePath: any = join(tasksDir, `${taskId}.md`);
    const body: any = renderTaskBodyFromSpec({ spec: plan.spec, executionNotes: null, verification: null });
    const frontMatterLines: any = [
      '---',
      `number: ${taskNumber}`,
    ...
    frontMatterLines.push('---');
    writeFileSync(filePath, `${frontMatterLines.join('\n')}\n${body}`, 'utf8');
    const dependencies: any = resolveBatchDependencies(plan.dependencies, created);
    store.upsertLifecycle({
      task_id: taskId,
      task_number: taskNumber,
      status: 'opened',
  ...
  return {
    ...
    tasks: created,
    created_task_numbers: created.map((task: any) : any => task.task_number),
    created_paths: created.map((task: any) : any => task.path),
    commit_ready: buildBatchCommitReady(created.map((task: any) : any => task.path), chapterIndexRelativePath),
    payload_ref: payloadSource?.ref ?? null,
    payload_sha256: payloadSource?.sha256 ?? null,
  };
}

function buildBatchTaskPlan({ task, shared, index, taskNumber, payloadSource }: any) : any {
  const title: any = stringField(task, 'title') || '';
  const sharedContext: any = stringField(shared, 'context');
  const taskContext: any = stringField(task, 'context');
  const context: any = [sharedContext, taskContext].filter(Boolean).join('\n\n') || null;
  return {
    index,
    task_number: taskNumber,
    title,
...
function validateBatchTaskPlans(plans: any) : any {
  const errors: any = [];
  for (const plan of plans) {
    if (!plan.title) errors.push({ index: plan.index, field: 'title', message: 'title_required' });
    if (!Array.isArray(plan.spec.acceptance_criteria) || plan.spec.acceptance_criteria.length === 0 || plan.spec.acceptance_criteria.some((item: any)  => typeof item !== 'string' || !item.trim())) {
...
function findLikelyBatchDuplicates(tasks: any) : any {
  const seen: any = new Map();
  const duplicates: any = [];
  for (const [index, task] of tasks.entries()) {
    const title: any = stringField(asRecord(task), 'title');
    if (!title) continue;
    const key: any = slugify(title);
...
function resolveBatchDependencies(dependencies: any, created: any) : any {
  return dependencies.map((dependency: any)  => {
    if (typeof dependency === 'number') return created[dependency]?.task_number ?? dependency;
    const key: any = slugify(String(dependency));
    return created.find((task: any)  => slugify(task.title) === key)?.task_number ?? dependency;
  });
}

function writeBatchChapterIndex({ path, chapterTitle, chapterId, created, summary }: any) : any {
  mkdirSync(resolve(path, '..'), { recursive: true });
  const lines: any = [`# ${chapterTitle}`, '', summary || `Chapter index for ${chapterId}.`, '', '## Tasks'];
  for (const task of created) lines.push(`- #${task.task_number} ${task.title}`);
  lines.push('');
  writeFileSync(path, `${lines.join('\n')}\n`, 'utf8');
}

function buildBatchCommitReady(taskPaths: any, chapterIndexPath: any) : any {
  const stagePaths: any = [...taskPaths];
...
async function taskLifecycleDispositionCloseout({ siteRoot, store, taskNumber, agentId, envelopeId, disposition, summary, dryRun, proveCriteria, finish, changedFiles: finishChangedFiles, noFilesChanged }: any) : Promise<any> {
  const lifecycle: any = store.getLifecycleByNumber(taskNumber);
  if (!lifecycle) throw new Error(`task_not_found: ${taskNumber}`);
  const taskFile: any = await findTaskFile(siteRoot, taskNumber);
  if (!taskFile) {
  ...
  const resolvedEnvelopeId: any = envelopeId ?? extractEnvelopeId(original);
  const envelope: any = resolvedEnvelopeId ? readIndexedEnvelope(siteRoot, resolvedEnvelopeId) : null;
  const envelopeStatus: any = envelope?.status ?? null;
  const inferredDisposition: any = disposition ?? inferDisposition(envelopeStatus);
  const changedFiles: any = [relativeSitePath(siteRoot, taskFile.path)];
  const now: any = new Date().toISOString();
  const executionNotes: any = [
  ...
  const plannedContent: any = replaceTaskSection(replaceTaskSection(original, 'Execution Notes', executionNotes), 'Verification', verificationNotes);
  const capaCoverageValidation: any = validateCapaDispositionCorrectiveCoverage({ envelope, body: plannedContent, store });
  ...
  if (!dryRun) {
    ...
    if (proveCriteria) {
      const afterNotes: any = readFileSync(taskFile.path, 'utf8');
      const proved: any = afterNotes.replace(/^(\s*)- \[ \](.*)$/gm, '$1- [x]$2');
      if (proved !== afterNotes) writeFileSync(taskFile.path, proved, 'utf8');
      const admission: any = await admitTaskEvidence({ cwd: siteRoot, taskNumber, admittedBy: agentId, methods: ['criteria_proof', 'disposition_closeout' as any] });
      criteriaResult = {
    ...
    if (finish) {
      ...
      const autoDetectedChangedFiles: any = !finishChangedFiles && !noFilesChanged ? detectGitChangedFiles(siteRoot) : [];
      const finishOptions: any = { cwd: siteRoot, taskNumber, agent: agentId, summary: summary ?? `Disposition close-out: ${inferredDisposition}`, close: true };
  ...
  const dispositionEvidencePreflight: any = finishResult?.evidence_preflight ?? await buildTaskEvidencePreflight({ siteRoot, store, taskNumber });
  const dispositionCloseReadiness: any = closeReadinessFromPreflight(dispositionEvidencePreflight);
  return {
...
function validateCapaDispositionCorrectiveCoverage({ envelope, body, store }: any) : any {
  const payload: any = envelope?.envelope?.payload ?? envelope?.envelope ?? envelope?.payload ?? {};
  const envelopeId: any = envelope?.envelope_id ?? envelope?.envelope?.envelope_id ?? null;
  const classification: any = String(payload.classification ?? envelope?.kind ?? '').toLowerCase();
  const correctiveAction: any = typeof payload.corrective_action === 'string' ? payload.corrective_action.trim() : '';
  const capaLike: any = Boolean(correctiveAction)
  ...
  const ledger: any = extractTaskSection(body, 'Follow-Up Ledger');
  if (!ledger) {
    return {
      ok: false,
      required: true,
  ...
  const lines: any = ledger.split(/\r?\n/).map((line: any) : any => line.trim().replace(/^[-*]\s+/, '')).filter(Boolean);
  const activeStatuses: any = new Set(['opened', 'claimed', 'needs_continuation', 'in_review']);
  const taskLinks: any = [];
  for (const line of lines) {
    const taskMatches: any = [...line.matchAll(/\b(?:created|covered by)\s+#(\d+)\b/gi)];
    for (const match of taskMatches) {
      const taskNumber: any = Number(match[1]);
      const lifecycle: any = Number.isFinite(taskNumber) ? store.getLifecycleByNumber(taskNumber) : null;
      taskLinks.push({
        task_number: taskNumber,
  ...
  const activeTaskLinks: any = taskLinks.filter((link: any) : any => link.active_implementation_coverage);
  const deferredOrBlocked: any = lines.find((line: any) : any => /\b(?:deferred|blocked|blocker)\s*:/i.test(line));
  const noAction: any = lines.find((line: any) : any => /\bno follow-?up needed\s*:/i.test(line) || /\bno[- ]action rationale\s*:/i.test(line));
...
function extractEnvelopeId(text: any) : any {
  const match: any = text.match(/env_[A-Za-z0-9_-]+/);
  return match ? match[0] : null;
}

function readIndexedEnvelope(siteRoot: any, envelopeId: any) : any {
  const index: any = refreshInboxIndex(siteRoot, { evaluateEnvelopeSeverity });
  try {
    const row: any = index.db.prepare('SELECT * FROM inbox_envelopes WHERE envelope_id = ?').get(envelopeId);
    if (!row) return null;
...
function inferDisposition(envelopeStatus: any) : any {
...
function replaceTaskSection(markdown: any, heading: any, replacement: any) : any {
  const pattern: any = new RegExp(`(## ${heading}\\r?\\n\\r?\\n)[\\s\\S]*?(?=\\r?\\n## )`);
  if (!pattern.test(markdown)) return `${markdown.trimEnd()}\n\n## ${heading}\n\n${replacement}\n`;
  return markdown.replace(pattern, `$1${replacement}\n`);
}

function relativeSitePath(siteRoot: any, filePath: any) : any {
  return relative(resolve(siteRoot), resolve(filePath)).replace(/\\/g, '/');
}

function ensureRecurringTaskTables(taskStore: any) : any {
  taskStore.db.exec(`
...
function ensureRecurringColumn(taskStore: any, columnName: any, columnType: any) : any {
...
function insertRecurringDefinition(taskStore: any, definition: any) : any {
...
function insertRecurringEvent(taskStore: any, { recurrenceId, eventType, stateAfter, actorAgentId, authorityBasis, event, now }: any) : any {
  ensureRecurringTaskTables(taskStore);
  taskStore.db.prepare(`
...
function insertRecurringRun(taskStore: any, run: any) : any {
  ensureRecurringTaskTables(taskStore);
  taskStore.db.prepare(`
...
function getRecurringDefinition(taskStore: any, recurrenceId: any) : any {
  ensureRecurringTaskTables(taskStore);
...
function listRecurringDefinitions(taskStore: any, { status = null, limit = 50 }: any = {}) : any {
  ensureRecurringTaskTables(taskStore);
  const boundedLimit: any = Math.max(1, Math.min(Number(limit) || 50, 200));
...
function listDueRecurringDefinitions(taskStore: any, { now, limit = 20 }: any = {}) : any {
  ensureRecurringTaskTables(taskStore);
  const boundedLimit: any = Math.max(1, Math.min(Number(limit) || 20, 100));
  const rows: any = taskStore.db.prepare(`
    ...
    LIMIT ?
  `).all(boundedLimit * 2);
  return rows.map(hydrateRecurringDefinition).filter((definition: any)  => {
    const dueKey: any = recurringDueKey(definition, now);
    return Boolean(dueKey && definition.last_due_key !== dueKey);
  }).slice(0, boundedLimit);
}

function listRecurringRuns(taskStore: any, recurrenceId: any, limit: any = 20) : any {
  ensureRecurringTaskTables(taskStore);
  const boundedLimit: any = Math.max(1, Math.min(Number(limit) || 20, 200));
  return taskStore.db.prepare(`
    ...
    LIMIT ?
  `).all(recurrenceId, boundedLimit).map((row: any) : any => ({
    run_id: row.run_id,
...
function hydrateRecurringDefinition(row: any) : any {
  return {
    recurrence_id: row.recurrence_id,
    title: row.title,
    status: row.status,
...
async function createRecurringTaskInstance({ store, siteRoot, definition, actorAgentId, actorRole, authorityBasis, triggerMode, runReason, eventType, now, dueKey = null }: any) : Promise<any> {
...
function updateRecurringDefinitionStatus({ store, siteRoot, recurrenceId, actorAgentId, authorityBasis, nextStatus, eventType, reason }: any) : any {
  if (!recurrenceId) throw new Error('recurrence_id_required');
  if (!actorAgentId) throw new Error('actor_agent_id_required');
  if (!authorityBasis) throw new Error('valid_authority_basis_required');
  if (!reason) throw new Error('reason_required');
...
function requireRecurringAuthorityActor({ store, siteRoot, actorAgentId }: any) : any {
  const actorRoleResolution: any = resolveAgentRoleWithDiagnostics(store, siteRoot, actorAgentId);
  const actorRole: any = actorRoleResolution.role;
  if (!['architect', 'operator'].includes(actorRole)) {
...
function normalizeRecurringAuthorityBasis(value: any) : any {
  const record: any = asRecord(value);
  const kind: any = stringField(record, 'kind');
  const summary: any = stringField(record, 'summary');
...
function parseIsoOrNow(value: any) : any {
...
function recurringDueKey(definition: any, now: any) : any {
  if (definition.trigger_mode !== 'schedule') return null;
  if (definition.schedule_kind !== 'daily') return null;
  return now.toISOString().slice(0, 10);
}

function arrayOfStrings(value: any, fallback: any = []) : any {
...
function ensureTaskRoutingTables(taskStore: any) : any {
  taskStore.db.exec(`
...
function getTaskRouting(taskStore: any, taskId: any) : any {
...
function ensureColumn(taskStore: any, tableName: any, columnName: any, columnType: any) : any {
...
function ensureAgentRosterEventsTable(taskStore: any) : any {
  taskStore.db.exec(`
...
function normalizeRosterAuthorityBasis(value: any) : any {
  const record: any = asRecord(value);
  const kind: any = stringField(record, 'kind');
  const summary: any = stringField(record, 'summary');
  const allowedKinds: any = new Set(['operator_direct_instruction', 'directed_obligation', 'task_owner_handoff']);
...
function validateRosterIdentifier(value: any, fieldName: any) : any {
...
function admitRosterIdentity(args: any) : any {
  const agentId: any = stringField(args, 'agent_id');
  ...
  const operatorIdentityCol: any = store.db.prepare("PRAGMA table_info(agent_roster)").all().some((column: any) : any => column.name === 'operator_identity');
  const event: any = {
...
function ensureStaticRosterAgentInSql(taskStore: any, root: any, agentId: any) : any {
  if (!agentId) return;
  try {
    const existing: any = taskStore.db.prepare('SELECT agent_id FROM agent_roster WHERE agent_id = ?').get(agentId);
    if (existing) return;
  } catch {
    return;
  }

  const rosterPath: any = join(root, '.ai', 'agents', 'roster.json');
  let staticAgent: any = null;
  try {
    const roster: any = JSON.parse(readFileSync(rosterPath, 'utf8'));
    staticAgent = Array.isArray(roster.agents) ? roster.agents.find((agent: any) : any => agent?.agent_id === agentId) : null;
  } catch {
    return;
  }
...
async function withAuthoredRosterJsonPreserved(root: any, fn: any) : Promise<any> {
  const rosterPath: any = join(root, '.ai', 'agents', 'roster.json');
  let before: any = null;
  try {
 ...
 */
async function testMcpTool(cwd: any, serverPath: any, toolName: any, toolArgs: any, options: any = {}) : Promise<any> {
  const siteServerPath: any = resolve(cwd, serverPath);
  const fullServerPath: any = existsSync(siteServerPath) ? siteServerPath : resolve(process.cwd(), serverPath);
  ...
  const timeoutSeconds: any = Math.min(300, Math.max(1, Number.isFinite(options.timeoutSeconds) ? options.timeoutSeconds : 10));
  const timeoutMs: any = timeoutSeconds * 1000;

  return new Promise((res, rej)  => {
    const proc: any = spawnMcpServer(process.execPath, [fullServerPath, '--site-root', cwd], { cwd: existsSync(siteServerPath) ? cwd : process.cwd() });
    let out: any = '';
    let err: any = '';
    ...
    proc.on('close', (code: any) : any => {
      ...
      const results: any = out.split(/\r?\n/).filter((line: any) : any => line.trim().length > 0).map((line: any) : any => {
