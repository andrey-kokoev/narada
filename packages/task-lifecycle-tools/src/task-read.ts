...
if (isNaN(taskNumber)) {
  console.error('Usage: node task-read.ts <cwd> <task-number>');
  process.exit(1);
}
...
try {
  ...
  const reports: any = store.db.prepare('SELECT agent_id FROM task_reports WHERE task_id = ?').all(lifecycle.task_id);
  ...
  const preferredAgentId: any = rolePref?.preferred_agent_id || null;
  const rolePolicy: any = resolveTaskRolePolicy({ siteRoot: cwd, taskSpec: spec });
