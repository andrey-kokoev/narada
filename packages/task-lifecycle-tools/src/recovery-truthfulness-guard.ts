...
function textMatches(packet: any, pattern: any) : any {
  const haystack: any = [
    packet.title,
    packet.summary,
    packet.context,
...
function boolOrText(packet: any, key: any, pattern: any) : any {
...
function normalizeRecoveryState(value: any) : any {
...
function extractMarkdownSection(body: any, heading: any) : any {
...
function parseRecoveryTruthfulnessSection(section: any) : any {
  if (!section) return {};
  const packet: any = {};
...
export function recoveryTruthfulnessTriggerContract() : any {
  return {
...
export function evaluateRecoveryTruthfulnessTrigger(packet: any = {}) : any {
  const triggers: any = [];
  const recoveryState: any = normalizeRecoveryState(packet.state);
...
export function validateRecoveryTruthfulnessBody({ body = '', summary = '', context = '' }: any = {}) : any {
  ...
  return {
    ...
    errors: validation.errors.map((error: any) : any => `${error} Add or repair a ## Recovery Truthfulness section with labels: Known facts, Inferences, Uncertainty, Changed, Not changed, Remaining work, Evidence limits, CAPA open status, State. For terminal_corrected, also include Repository durability / commit-push state.`),
  };
}

function normalizedText(value: any) : any {
  return text(value).toLowerCase();
}

function claimsRemediationEvidenceAsAuthorization(packet: any) : any {
  const haystack: any = [
    packet.summary,
    packet.operator_summary,
    packet.closeout_text,
  ...
  const citesRemediationEvidence: any = /\b(?:rollback|rolled\s+back|remediation|remediated|closure\s+evidence|repair\s+evidence|bg_\d+[A-Za-z0-9_]*)\b/i.test(haystack);
  const claimsOriginalAuthorization: any = /\b(?:original|initial|startup|prior)\b.{0,80}\b(?:authori[sz]ed|authority|allowed|approved)\b/i.test(haystack)
...
function hasSeparateOriginalAuthorizationEvidence(packet: any) : any {
  const evidence: any = text(packet.original_authorization_evidence ?? packet.authorization_evidence).trim();
...
function claimsNoRemainingWork(value: any) : any {
...
function claimsCapaClosed(value: any) : any {
  const content: any = normalizedText(value);
  if (!content.trim()) return false;
...
function claimsRepositoryDurable(value: any) : any {
  const content: any = normalizedText(value);
  if (!content.trim()) return false;
...
function hasDurableChangeEvidence(packet: any) : any {
...
export function validateRecoveryTruthfulnessPacket(packet: any = {}) : any {
  const evaluation: any = evaluateRecoveryTruthfulnessTrigger(packet);
  if (!evaluation.triggered) return { ok: true, evaluation, errors: [] };

  const errors: any = [];
  const recoveryState: any = evaluation.normalized_state;
  const missingFields: any = REQUIRED_TRUTHFULNESS_FIELDS.filter((field: any) : any => {
