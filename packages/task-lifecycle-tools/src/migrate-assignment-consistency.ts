...
MAIN: try {
  ...
  const tableExists: any = store.db.prepare(
  ...
  const records: any = store.db.prepare("SELECT task_id, record_json FROM task_assignment_records").all();
  let inserted: any = 0;
  let released: any = 0;
  let skipped: any = 0;
  let lifecyclesBackfilled: any = 0;

  for (const record of records) {
    let jsonAssignments: any;
    try {
    ...
    const lifecycle: any = store.db.prepare("SELECT task_id FROM task_lifecycle WHERE task_id = ?").get(record.task_id);
    if (!lifecycle) {
      const numMatch: any = record.task_id.match(/-(\d+)-/);
      let taskNum: any = numMatch ? Number(numMatch[1]) : null;
      // If number is null or already taken, allocate a unique fallback
      if (taskNum == null) {
        const maxRow: any = store.db.prepare("SELECT COALESCE(MAX(task_number), 0) AS max_num FROM task_lifecycle").get();
        taskNum = maxRow.max_num + 1;
      } else {
        const existing: any = store.db.prepare("SELECT task_id FROM task_lifecycle WHERE task_number = ?").get(taskNum);
        if (existing) {
          const maxRow: any = store.db.prepare("SELECT COALESCE(MAX(task_number), 0) AS max_num FROM task_lifecycle").get();
          taskNum = maxRow.max_num + 1;
        }
    ...
    const sqlAssignments: any = store.db.prepare(
