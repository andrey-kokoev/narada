export const TASK_LIFECYCLE_TOOL_ALIASES: any = {
...
import { listCommandTools, listPayloadTools } from '@narada-core/site-common-tools/compat/mcp-payload-file.legacy-site';

export function taskLifecycleTools() : any {
  return [
...
function tool(name: any, description: any, inputSchema: any) : any {
...
function enumStringSchema(values: any, description: any) : any {
...
function dispositionCloseoutSchema() : any {
  return objectSchema({
    task_number: numberSchema('Task number to close out.'),
    agent_id: stringSchema('Agent id performing the close-out.'),
    envelope_id: stringSchema('Optional envelope id. If omitted, the task body is scanned for env_<id>.'),
...
function arraySchema(items: any, description: any) : any {
  return { type: 'array', items, description };
}

function submitObservationSchema() : any {
  return objectSchema({
    task_number: numberSchema('Optional task number to attach to.'),
    artifact_uri: { type: 'string' },
    content: { type: 'object', additionalProperties: true },
