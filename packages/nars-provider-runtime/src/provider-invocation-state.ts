...
export class NarsProviderInvocationRefusalError extends Error {
  ...
  constructor(message: any, details: AnyRecord = {}) {
    super(message);
...
export function createNarsProviderInvocationId(idFn: any = randomUUID): any {
  const raw = String(idFn()).replace(/[^0-9A-Za-z_-]+/g, '-').replace(/^-+|-+$/g, '');
  return `prov_inv_${raw || randomUUID()}`;
}

export function isNarsProviderInvocationState(state: any): any {
  return NARS_PROVIDER_INVOCATION_STATES.includes(state);
}

export function isNarsProviderInvocationTerminalState(state: any): any {
  return TERMINAL_STATES.has(state);
}

export function canTransitionNarsProviderInvocation(previousState: any, nextState: any): any {
...
export function assertNarsProviderInvocationTransition(previousState: any, nextState: any): any {
...
export function terminalStateForNarsProviderInvocation(state: any): any {
  return isNarsProviderInvocationTerminalState(state) ? state : null;
}

export function normalizeNarsProviderInvocationRecord(record: AnyRecord = {}): any {
