...
// Canonical OpenAI-compatible chat-completions request shape.
// Pure module: no node builtins, no process.env, no mutable module context.
// Invocation coordinates are supplied only by the resolved canonical plan.

export function reasoningEffort(thinking: any): any {
...
export function isKimiProvider(provider: any): any {
  return provider === 'kimi-api' || provider === 'kimi-code-api';
}

export function normalizeKimiJsonSchema(schema: any): any {
  if (Array.isArray(schema)) return schema.map((item: any) => normalizeKimiJsonSchema(item));
  ...
  if (Array.isArray(normalized.anyOf) && normalized.type !== undefined) {
    ...
    normalized.anyOf = normalized.anyOf.map((item: any) => {
...
export function flattenKimiRootAnyOf(schema: any): any {
...
export function normalizeKimiToolParameters(schema: any): any {
...
export function normalizeOpenAiCompatibleTools(tools: any[] = [], { provider }: AnyRecord = {}): any {
...
export function cleanOpenAiMessages(messages: any, { provider }: AnyRecord = {}): any {
  return messages.map((m: any) => {
    const clean: AnyRecord = { role: m.role };
    if (m.role === 'tool') {
...
export function buildOpenAiChatRequest(messages: any, tools: any, options: AnyRecord = {}): any {
...
export function extractOpenAiChatReply(body: any): any {
  ...
  const tool_calls = toolCallsRaw.flatMap((entry: any) => {
