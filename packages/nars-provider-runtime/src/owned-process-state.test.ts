...
} from './owned-process-state.js';
import { createOwnedProcess } from './process-supervisor.js';
...
test('owned process FSM separates termination from release', () => {
  ...
  assert.deepEqual(machine.history.map((entry: any) => entry.state), ['created', 'running', 'terminating', 'exited', 'released']);
});
...
test('process supervisor exposes owned child termination and registry release states', () => {
  ...
  const child = {
    ...
    signalCode: null,
    kill() { killCount += 1; },
    once(event: any, handler: any) { listeners.set(event, handler); },
  };
  ...
  assert.deepEqual(owner.stateHistory.map((entry: any) => entry.state), ['created', 'running', 'terminating', 'exited', 'released']);
});
