...
import { CredentialLocatorResolutionError, resolveCredentialLocator } from './credential-locator.js';
import { assertNarsProviderInvocationTransition, isNarsProviderInvocationTerminalState } from './provider-invocation-state.js';

function protocolKey(protocol: any): any {
  return `${protocol?.family ?? ''}/${protocol?.operation ?? ''}/${protocol?.version ?? ''}`;
}

function providerSlug(inferenceProvider: any): any {
  return String(inferenceProvider.id).replace(/^inference-provider:/, '');
}

function thinkingOption(plan: any): any {
  return typeof plan.options?.thinking === 'string' ? plan.options.thinking : undefined;
}

function normalizedUsage(response: any, elapsedMs: any): any {
...
function refusal(code: any, message: any): any {
  return {
...
function transportFailure(error: any): any {
  return {
    admission: (error as AnyRecord)?.admission ?? 'uncertain',
    transportSubmitted: error?.transportSubmitted !== false,
    error: {
      code: (error as AnyRecord)?.code ?? 'provider-transport-failed',
      message: error instanceof Error ? error.message : String(error),
      retryable: (error as AnyRecord)?.admission !== 'acknowledged',
    },
  };
}
...
export function createCanonicalInvocationAdapter({
  runtimeContext = {},
  env = process.env,
  credentialResolver = (locator: any) => resolveCredentialLocator(locator, { env }),
  ...
  return Object.freeze({
    async invoke(input: any) {
      ...
      const transition = async (nextState: any, evidence: AnyRecord = {}) => {
      ...
      const refuse = async (code: any, message: any) => {
      ...
      try {
        ...
        if (error instanceof CredentialLocatorResolutionError || (error as AnyRecord)?.code) {
          return refuse((error as AnyRecord).code ?? 'credential-unavailable', error instanceof Error ? error.message : String(error));
        }
