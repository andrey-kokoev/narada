...
import { createCanonicalInvocationAdapter } from './canonical-invocation-adapter.js';
...
test('canonical adapter uses exact plan-bound endpoint and invocation model key while ignoring selection env', async () => {
  ...
  const adapter = createCanonicalInvocationAdapter({
    ...
    httpTransport: async (request: any) => {
      submitted = request;
  ...
  const outcome = await adapter.invoke(invocation({ invocationEventSink: (event: any) => events.push(event) }));
