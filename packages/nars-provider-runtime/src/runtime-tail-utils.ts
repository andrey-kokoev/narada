...
export function extractOutputRef(content: any): any {
...
export function stringifySummary(value: any): any {
  try { return JSON.stringify(value); } catch { return String(value); }
}

export function parseJson(text: any): any {
  try { return JSON.parse(text); } catch { return {}; }
}

export function isAbortError(error: any): any {
...
export function codexCliSpawnError(error: any, command: any): any {
  const message = error instanceof Error ? error.message : String(error);
...
export function terminateChildProcess(child: any): any {
  if (!child || child.killed) return;
  child.kill();
}

export function codexCommand({ processEnv = process.env, platform = process.platform, exists = existsSync }: AnyRecord = {}): any {
