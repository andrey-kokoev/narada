...
import { spawnOwnedProcess } from './process-supervisor.js';
import { codexCliSpawnError, codexCommand } from './runtime-tail-utils.js';

function transportError(message: any, {
  admission = 'uncertain',
...
export function sendHttp({ url, body, headers }: AnyRecord, settings: AnyRecord): any {
...
export async function sendCodex(request: any, settings: any, onAdmitted: any = null): Promise<any> {
  if (settings.abortSignal?.aborted) {
  ...
  return new Promise((resolveRequest, rejectRequest) => {
    ...
    owner.child.stdout.on('data', (chunk: any) => { stdout += chunk; });
    owner.child.stderr.on('data', (chunk: any) => { stderr += chunk; });
    owner.child.on('error', (error: any) => rejectRequest(codexCliSpawnError(error, command)));
    owner.child.on('exit', (code: any) => {
