...
function stripAnsi(value: any): any {
...
function defaultChildProcessEnv(extra: AnyRecord = {}, baseEnv: AnyRecord = process.env): any {
  return { ...baseEnv, ...extra, FORCE_COLOR: '0', NO_COLOR: '1' };
}

function requiredOption(options: any, name: any, code: any): any {
  const value = options?.[name];
  if (value === undefined || value === null || value === '') throw new Error(code);
  return value;
}

function buildCodexMcpRequest(messages: any, tools: any[] = [], options: AnyRecord = {}): any {
  const model = requiredOption(options, 'model', 'canonical_invocation_model_required');
  const siteRoot = requiredOption(options, 'siteRoot', 'canonical_invocation_site_root_required');
  const { thinking, nativeMcpTools = true, mcpServers = {}, codexSessionState = null } = options;
...
function findLastMessageIndex(messages: any, role: any): any {
...
function codexToolProtocolInstructions(tools: any[] = [], { nativeMcpTools = false }: AnyRecord = {}): any {
...
function parseCodexMcpResponse(response: any): any {
...
function isPotentialNaradaToolCallText(content: any): any {
...
function buildOpenAiChatRequest(messages: any, tools: any, options: AnyRecord = {}): any {
  requiredOption(options, 'baseUrl', 'canonical_endpoint_url_required');
  requiredOption(options, 'model', 'canonical_invocation_model_required');
  requiredOption(options, 'apiKey', 'canonical_invocation_credential_required');
  requiredOption(options, 'provider', 'canonical_inference_provider_required');
  return coreBuildOpenAiChatRequest(messages, tools, options);
}

function cleanOpenAiMessages(messages: any, { provider }: AnyRecord = {}): any {
  return coreCleanOpenAiMessages(messages, { provider });
}

function buildAnthropicMessagesRequest(messages: any, tools: any, options: AnyRecord = {}): any {
  const baseUrl = requiredOption(options, 'baseUrl', 'canonical_endpoint_url_required');
  const model = requiredOption(options, 'model', 'canonical_invocation_model_required');
  const apiKey = requiredOption(options, 'apiKey', 'canonical_invocation_credential_required');
...
function cleanAnthropicMessages(messages: any): any {
  const systemParts = [];
...
function toAnthropicTool(tool: any): any {
  const fn = tool.function ?? {};
  return {
    name: fn.name,
...
function parseAnthropicMessagesResponse(response: any): any {
  const content = Array.isArray(response.content) ? response.content : [];
  const text = joinAssistantTextParts(content.filter((item: any) => item?.type === 'text').map((item: any) => item.text ?? ''));
  ...
  const message: AnyRecord = { role: 'assistant', content: text || null };
...
function joinAssistantTextParts(parts: any): any {
  return parts.map((part: any) => String(part ?? '')).filter((part: any) => part.trim()).reduce((content: any, part: any) => appendAssistantTextPart(content, part), '');
}

function appendAssistantTextPart(content: any, text: any): any {
...
function assistantAppend(content: any, text: any, { forceBoundary = false }: AnyRecord = {}): any {
  const prior = String(content ?? '');
  const next = String(text ?? '');
...
function codexExecEventTextPart(event: any): any {
...
function codexExecEventItemKey(event: any): any {
  const item = event?.item;
  return item?.id ?? event?.item_id ?? event?.id ?? null;
}

function createCodexExecTextAccumulator(content: any = ''): any {
  return {
...
function accumulateCodexExecEvent(state: any, event: any): any {
...
function codexExecAccumulationResult(state: any, appendText: any): any {
  return {
    state,
...
function buildCodexExecArgs(request: any, options: AnyRecord = {}): any {
  const { model, thinking } = options;
  const siteRoot = requiredOption(options, 'siteRoot', 'canonical_invocation_site_root_required');
  const effort = reasoningEffort(thinking);
  const requestedModel = request.arguments?.model ?? model;
  if (!requestedModel) throw new Error('canonical_invocation_model_required');
  const common = [
    '--json',
...
function codexExecPrompt(request: any): any {
...
function parseCodexExecJsonLine(line: any): any {
  try {
...
function codexExecMcpToolEventSummary(event: any): any {
  const item = event?.item;
...
function codexExecEventText(event: any): any {
  return codexExecEventTextPart(event)?.text ?? '';
}

function accumulateCodexExecText(content: any, text: any): any {
...
function defaultCodexAuthHome(): any {
...
function writeCodexExecHome(mcpServers: any, sessionDir: any, { sourceHome = defaultCodexAuthHome(), writeDurableTextFile = writeFileSync }: AnyRecord = {}): any {
  if (!sessionDir) throw new Error('canonical_invocation_session_dir_required');
...
function codexRequestMcpServers(request: any, settings: AnyRecord = {}): any {
  return request.arguments?.mcpServers ?? settings.mcpServers ?? {};
}

function buildCodexSubprocessEnv(mcpServers: any, settings: AnyRecord = {}): any {
  const codexHome = writeCodexExecHome(mcpServers, settings.sessionDir, {
    sourceHome: settings.codexAuthHome ?? defaultCodexAuthHome(),
    writeDurableTextFile: settings.writeDurableTextFile ?? writeFileSync,
  });
  const buildChildProcessEnv = settings.buildChildProcessEnv ?? defaultChildProcessEnv;
...
function buildCodexMcpServerArgs(): any {
  return ['mcp-server'];
}

function codexExecMcpConfigArgs(mcpServers: any): any {
  const args = [];
  for (const [name, serverValue] of Object.entries(mcpServers as AnyRecord)) {
    ...
    args.push('-c', `mcp_servers."${tomlKey(name)}".args=${JSON.stringify((config.args ?? []).map((arg: any) => String(arg).replaceAll('\\', '/')))}`);
...
function codexMcpServerEnvVars(config: AnyRecord = {}): any {
  return [...new Set([...(config.env_vars ?? []).map(String), ...codexMcpEnvVarNames()])];
}

function codexExecConfigToml(mcpServers: any): any {
  const lines = [
    '# Generated by packages/nars-provider-runtime/src/canonical-protocol-adapters.js for nested Codex subprocesses.',
  ...
  for (const [name, serverValue] of Object.entries(mcpServers as AnyRecord)) {
    ...
    lines.push(`args = ${JSON.stringify((config.args ?? []).map((arg: any) => String(arg).replaceAll('\\', '/')))}`);
...
function tomlString(value: any): any {
  return JSON.stringify(String(value).replaceAll('\\', '/'));
}

function tomlKey(value: any): any {
  return String(value).replaceAll('\\', '\\\\').replaceAll('"', '\\"');
}

function stringifyContent(value: any): any {
...
function stableStringify(value: any): any {
  ...
  if (Array.isArray(value)) return `[${value.map((item: any) => stableStringify(item)).join(',')}]`;
  const record = value;
  return `{${Object.keys(record)
    .filter((key: any) => record[key] !== undefined)
    .sort()
    .map((key: any) => `${JSON.stringify(key)}:${stableStringify(record[key])}`)
    .join(',')}}`;
}

function formatCompactJsonSchema(schema: any, { limit = 1200 }: AnyRecord = {}): any {
