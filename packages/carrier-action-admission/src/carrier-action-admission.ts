...
interface DelegatedAuthorityHandoff extends JsonRecord {
  schema?: string;
  crossing_regime?: string;
  parse_status?: string;
  authority_ref?: string;
  authority_mode?: string;
  authority?: string;
  mode?: string;
  delegated_mutation_authority?: boolean;
  permissions?: unknown;
  capabilities?: unknown;
  allowed_action_families?: unknown;
  mutating_families?: unknown;
  allowed_tools?: unknown;
  mutating_tools?: unknown;
}

interface Classification extends JsonRecord {
  family: string;
  authority_owner: string | null;
  decision?: string;
  reason: string;
  secret_findings: string[];
  secret_diagnostics?: JsonRecord[];
  remediation?: string | null;
  classifier_source?: string;
  registry_diagnostics?: JsonRecord | null;
  surface_registry?: JsonRecord | null;
  delegated_authority?: JsonRecord | null;
...
interface ClassificationOptions {
  ...
  delegatedAuthorityHandoff?: DelegatedAuthorityHandoff | null;
}

interface CarrierActionRequest extends JsonRecord {
  schema: string;
  request_id: string;
  ...
  source: JsonRecord;
  target_locus: JsonRecord;
  requested_action: JsonRecord;
}

interface CarrierActionDecision extends JsonRecord {
  schema: string;
  request_id: string;
  created_at: string;
  classifier_version: string;
  ...
  request: CarrierActionRequest;
}

interface CarrierActionCandidate extends JsonRecord {
  schema: string;
  request_id: string;
  created_at: string;
  status: string;
...
interface CarrierActionContext {
  agentId: string;
...
function candidateDir(siteRoot: string): string {
  return join(actionAdmissionDir(siteRoot), 'candidates');
}

function handoffDir(siteRoot: string): string {
...
function inspectPayloadForSecrets(value: unknown, path: string[] = []): string[] {
  return [...new Set(inspectPayloadForSecretFindings(value, path).map((finding: SecretFinding) => finding.path))];
}

function inspectPayloadForSecretFindings(value: unknown, path: string[] = []): SecretFinding[] {
  const findings: SecretFinding[] = [];
  if (Array.isArray(value)) {
    value.forEach((item: unknown, index: number) => findings.push(...inspectPayloadForSecretFindings(item, [...path, String(index)])));
    return findings;
  }
...
function secretFinding(path: string[], matchKind: string, trigger: string): SecretFinding {
  return {
...
function registryToolNotDeclaredDiagnostics(toolName: string, metadata: ToolMetadata | null | undefined): JsonRecord | null {
...
function candidateRegistryPatchClassification(toolName: string, metadata: ToolMetadata): JsonRecord {
...
function registryPatchRecommendedAction(): string {
...
function secretDiagnosticSummary(findings: SecretFinding[]): JsonRecord[] {
  return findings.map((finding: SecretFinding) => ({
    path: finding.path,
...
function safeRetryForSecretFinding(finding: SecretFinding): string {
...
function argumentSummary(args: unknown = {}): JsonRecord {
...
function classifyCarrierActionRequest(toolName: string, args: unknown = {}, options: ClassificationOptions = {}): Classification {
  const secretFindingDetails = inspectPayloadForSecretFindings(args);
  const secretFindings = [...new Set(secretFindingDetails.map((finding: SecretFinding) => finding.path))];
  ...
  const withDelegatedAuthority = (classification: Classification): Classification => applyDelegatedAuthorityAdmission(
    classification,
    toolName,
    options,
  );
  ...
  if (metadata?.read_only === false) {
    ...
    if (metadataFamily) {
      return withDelegatedAuthority({
        ...
        authority_owner: metadata.authority_owner ?? familyClassification(metadataFamily, metadata.reason ?? 'tool_catalog_metadata_non_read_only').authority_owner,
        classifier_source: metadata.source ?? 'tool_metadata',
      });
    }
  }
...
function normalizeToolMetadata(metadata: unknown): ToolMetadata | null {
  if (!metadata || typeof metadata !== 'object') return null;
  return metadata;
}

function readOnlyClassification(reason: string): Classification {
  return {
...
function familyClassification(family: string, reason: string): Classification {
...
function normalizeActionFamily(value: unknown): string | null {
...
function refusedClassification(reason: string, authorityOwner: string | null = null, family = 'unknown_action_family'): Classification {
  return {
    family,
...
function applyDelegatedAuthorityAdmission(classification: Classification, toolName: string, options: ClassificationOptions = {}): Classification {
...
function normalizeDelegatedAuthorityHandoff(handoff: unknown): DelegatedAuthorityHandoff | null {
...
function delegatedAuthorityDecision({ handoff, family, toolName }: {
  handoff: DelegatedAuthorityHandoff | null;
  family: string;
  toolName: string;
}): { admitted: boolean; diagnostic: JsonRecord } {
  const base = {
...
function delegatedHandoffGrantsWrite(handoff: DelegatedAuthorityHandoff, family: string, toolName: string): boolean {
...
function delegatedHandoffScopeAllows(handoff: DelegatedAuthorityHandoff, family: string, toolName: string): boolean {
...
}: CarrierActionContext): CarrierActionRequest {
...
function classifierMetadataSummary(toolMetadata: ToolMetadata | null | undefined): JsonRecord | null {
...
function decideCarrierActionRequest(request: CarrierActionRequest): CarrierActionDecision {
...
function decisionForClassification(classification: Classification): JsonRecord & {
  decision: string;
  reason: string;
  authority_owner: string | null;
  carrier_mutation_admitted?: boolean;
...
function writeCarrierActionAdmissionEvidence(siteRoot: string, decision: CarrierActionDecision): {
  path: string;
  decision: CarrierActionDecision & { evidence_path: string };
} {
...
function createAndWriteCarrierActionAdmission(context: CarrierActionContext): {
  path: string;
  decision: CarrierActionDecision & { evidence_path: string };
...
function createCarrierActionCandidate(decision: CarrierActionDecision, evidencePath: string): CarrierActionCandidate {
...
function candidateSchemaForFamily(family: string): string {
...
function candidateExtensionForFamily(family: string): string {
...
function writeCarrierActionCandidate(siteRoot: string, candidate: CarrierActionCandidate): { path: string; candidate: CarrierActionCandidate } {
...
function carrierActionCandidatePath(siteRoot: string, candidate: CarrierActionCandidate): string {
