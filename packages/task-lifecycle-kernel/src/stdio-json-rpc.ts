...
export function drainBufferedRequests(buffer: string, parseJsonRpcInput: AnyFunction): { requests: any[]; remaining: string } {
...
export function drainJsonRpcFrames(buffer: string): { requests: any[]; remaining: string } {
  const requests = [];
...
function writeJsonLine(stdout: AnyRecord, response: any): void {
