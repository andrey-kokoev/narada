...
export function createTaskLifecycleToolCaller({
  ...
  return async function callTaskLifecycleTool(params: any): Promise<any> {
    ...
    const registeredToolNames = tools.map((tool: AnyRecord) => tool.name);
    ...
    const toolDef = tools.find((tool: AnyRecord) => tool.name === canonicalName);
...
export function isStoreError(error: unknown): boolean {
  const msg = error instanceof Error ? error.message : String(error);
  return /database|sqlite|SQLITE|disk I\/O|malformed|not a database/i.test(msg);
}

function detectReviewSurfaceMismatch(canonicalName: string, args: AnyRecord): AnyRecord | null {
  if (canonicalName !== 'task_lifecycle_finish') return null;
  const verdict = stringField(args, 'verdict');
  if (verdict === undefined || !['accepted', 'accepted_with_notes', 'rejected'].includes(verdict)) return null;
...
export function buildLifecycleTargetLocusStatus({ siteRoot, env = process.env }: AnyRecord): AnyRecord {
...
export function guardLifecycleTargetLocus({ canonicalName, args, siteRoot, env = process.env, locusGuardedMutationTools }: AnyRecord): AnyRecord {
...
export function resolveTaskCreatePayloadArgs({ args, siteRoot, resolveToolPayloadArgs }: AnyRecord): AnyRecord {
...
export function validateTaskCreatePayload(args: AnyRecord): void {
...
function stringField(record: any, key: string): string | undefined {
  const value = asRecord(record)[key];
  return typeof value === 'string' && value.trim().length > 0 ? value.trim() : undefined;
}

function booleanField(record: any, key: string): boolean | undefined {
  const value = asRecord(record)[key];
