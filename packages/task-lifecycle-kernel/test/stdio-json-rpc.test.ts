...
import { drainJsonRpcFrames, runJsonRpcStdioServer } from '../src/stdio-json-rpc.js';
...
await runJsonRpcStdioServer({
  stdin,
  stdout,
  parseJsonRpcInput: (input: string) => input.split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line)),
  handleRequest: async (item: any) => ({ jsonrpc: '2.0', id: item.id, result: { ok: true } }),
});
