...
} from '../src/tool-call-pipeline.js';

function makeCaller(overrides: AnyRecord = {}) {
  const calls: string[] = [];
  const dispatchTool = async (name: string, args: AnyRecord): Promise<AnyRecord> => {
    calls.push(name);
  ...
  const caller = createTaskLifecycleToolCaller({
    ...
    resolveToolPayloadArgs: ({ args }: AnyRecord) => ({ args }),
...
test('tool caller requests payload merge mode for non-create tools', async () => {
  ...
  const { caller } = makeCaller({
    resolveToolPayloadArgs: ({ args, payloadRefMode }: AnyRecord) => {
