...
// Worker-safe provider runtime binding core. No node:fs in this module: the
// provider registry must be injected as parsed metadata (Workers bundle the
// registry JSON via the @narada2/carrier-provider-contract/provider-registry
// export). node:crypto is used only for the credential fingerprint and is
// available both in Node and in Workers under nodejs_compat.
...
export function firstEnvValue(names = [], env = process.env) {
...
export function resolveProviderMetadata(provider, metadata) {
...
export function providerEnvironment(provider, metadata, env = process.env) {
  const providerDefault = resolveProviderMetadata(provider, metadata);
