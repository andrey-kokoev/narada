...
A generated JSON Schema is also available at `packages/layers/control-plane/config.schema.json` for editor validation and external tooling.
...
If no OS keychain is available, credentials are encrypted using AES-256-GCM with a key derived from your user account. Encryption keys are stored with 0600 permissions in `~/.narada/credentials/`.
