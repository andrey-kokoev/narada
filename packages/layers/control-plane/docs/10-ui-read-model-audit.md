...
**UI Gap**: No persisted context index exists outside the mail-vertical `context_records` table. Timer/webhook/fs contexts rely on the same `context_records` table but may lack vertical-specific hydration.
