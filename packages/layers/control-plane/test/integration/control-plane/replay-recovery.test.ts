...
describe("Replay and Recovery Tests", () => {
  ...
  describe("Work Item Replay (W1-W4)", () => {
    ...
    it("W2: replay after partial execution recovers stale lease to failed_retryable", () => {
      ...
      // Mechanical recovery (scheduler), then semantic failure (foreman)
      const recovered = h.scheduler.recoverStaleLeases(past);
      expect(recovered).toHaveLength(1);
      h.foreman.failWorkItem(wi.work_item_id, "Lease abandoned due to stale recovery", true, "immediate");
    ...
    it("W3: replay after process restart recovers stale lease deterministically", async () => {
      ...
      const recovered = restartedScheduler.recoverStaleLeases();
      expect(recovered).toHaveLength(1);

      // Mechanical recovery leaves work item in executing state;
      // foreman owns the semantic transition to failed_retryable.
      const wiAfterMechanical = h.coordinatorStore.getWorkItem(wi.work_item_id)!;
      expect(wiAfterMechanical.status).toBe("executing");

      h.foreman.failWorkItem(wi.work_item_id, "Lease abandoned due to stale recovery", true, "immediate");
      const wiAfterForeman = h.coordinatorStore.getWorkItem(wi.work_item_id)!;
      expect(wiAfterForeman.status).toBe("failed_retryable");
  ...
  describe("Revision Supersession (R1-R4)", () => {
    ...
    it("R2: new revision during execution aborts commit and superseded old work item", async () => {
      ...
      const evaluation = makeEvaluation(wiA.work_item_id, exId);      insertEvaluation(h, evaluation);
      const resolveResult = await h.foreman.resolveWorkItem({ work_item_id: wiA.work_item_id, execution_id: exId, evaluation_id: evaluation.evaluation_id });
  ...
  describe("Outbound Idempotency (O1-O4)", () => {
    it("O1: duplicate command creation attempts abort silently with exactly one command", async () => {
      ...
      const evaluation = makeEvaluation(wi.work_item_id, exId);      insertEvaluation(h, evaluation);
      const r1 = await h.foreman.resolveWorkItem({ work_item_id: wi.work_item_id, execution_id: exId, evaluation_id: evaluation.evaluation_id });
      ...
      const r2 = await h.foreman.resolveWorkItem({ work_item_id: wi.work_item_id, execution_id: exId, evaluation_id: evaluation.evaluation_id });
    ...
    it("O2: repeated evaluation with identical payload does not duplicate outbound commands", async () => {
      ...
      const r1 = await h.foreman.resolveWorkItem({ work_item_id: wiA.work_item_id, execution_id: exId1, evaluation_id: eval1.evaluation_id });
      expect(r1.success).toBe(true);
      ...
      insertEvaluation(h, eval2);
      const r2 = await h.foreman.resolveWorkItem({ work_item_id: wiA.work_item_id, execution_id: exId2, evaluation_id: eval2.evaluation_id });
      expect(r2.success).toBe(true);
  ...
  describe("Tool/Runtime Failure (T1-T5)", () => {
    ...
    it("T2: tool denial records permission_denied and execution continues", async () => {
      ...
      const evaluation = makeEvaluation(wi.work_item_id, exId);      insertEvaluation(h, evaluation);
      const result = await h.foreman.resolveWorkItem({ work_item_id: wi.work_item_id, execution_id: exId, evaluation_id: evaluation.evaluation_id });
    ...
    it("T5: transient runtime crash marks attempt crashed and work item retryable", () => {
      ...
      // Simulate runtime crash — scheduler cleans up execution/lease, foreman decides retryable/terminal
      h.scheduler.failExecution(attempt.execution_id, "Uncaught exception", true);
      h.foreman.failWorkItem(wi.work_item_id, "Uncaught exception", true);
  ...
  describe("Commentary Separation (C1-C4)", () => {
    it("C1: traces deleted before work resolution does not block decision commit", async () => {
      ...
      const traces = h.db.prepare("select count(*) as c from agent_traces where context_id = ?").get("conv-1") as { c: number };
    ...
    it("C2: traces deleted before outbound dedupe does not block confirmation", async () => {
      ...
      await h.foreman.resolveWorkItem({ work_item_id: wi.work_item_id, execution_id: exId, evaluation_id: evaluation.evaluation_id });
  ...
  describe("Daemon/Wake Duplication (D1-D4)", () => {
    ...
    it("D3: wake after crash recovery runs recovery scanner before lease acquisition", async () => {
      ...
      // Mechanical recovery releases the stale lease; foreman must be called separately
      // for the semantic transition. The key assertion is that no stale expired lease remains.
