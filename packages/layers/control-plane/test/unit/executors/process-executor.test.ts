...
describe("ProcessExecutor", () => {
  ...
  it("recovery resets intent to admitted (not failed_retryable)", async () => {
    // This test documents the intentional behavioral difference from scheduler
    // stale-lease recovery, which transitions work items to failed_retryable.
    // Process executor recovery resets the intent to admitted so the worker
    // registry can re-claim it. There is no exponential backoff.
    admitProcessIntent({ intent_id: "int-contract", idempotency_key: "key-contract" });
    executionStore.create({
      execution_id: "pe-contract",
    ...
    intentStore.updateStatus("int-contract", "executing", { target_id: "pe-contract" });

    const recovered = executor.recoverStaleExecutions("2024-01-01T01:00:00.000Z");
    expect(recovered).toHaveLength(1);

    const intent = intentStore.getById("int-contract")!;
    expect(intent.status).toBe("admitted");
    // terminal_reason is preserved for audit even when resetting to admitted
    expect(intent.terminal_reason).toContain("Recovered stale execution");
  });
});
