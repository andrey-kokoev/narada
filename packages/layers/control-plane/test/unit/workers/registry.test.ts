...
describe("DefaultWorkerRegistry", () => {
  ...
  it("unified worker model: process and outbound workers coexist in one registry", async () => {
    const registry = new DefaultWorkerRegistry();
    ...
    registry.register({
      ...
      fn: async () => ({ processed: true, execution_id: "proc-1" }),
    });
    ...
    registry.register({
      ...
      fn: async () => ({ processed: true, execution_id: "out-1" }),
    });
    ...
    registry.register({
      ...
      fn: async () => ({ processed: true, execution_id: "rec-1" }),
    });

    const workers = registry.listWorkers();
    expect(workers).toHaveLength(4);
    expect(workers.map((w) => w.worker_id)).toContain("process_executor");
    expect(workers.map((w) => w.worker_id)).toContain("send_reply");
    expect(workers.map((w) => w.worker_id)).toContain("non_send_actions");
    expect(workers.map((w) => w.worker_id)).toContain("outbound_reconciler");

    const processResult = await drainWorker(registry, "process_executor");
    expect(processResult.totalProcessed).toBe(1);

    const sendResult = await drainWorker(registry, "send_reply");
    expect(sendResult.totalProcessed).toBe(1);

    const nonSendResult = await drainWorker(registry, "non_send_actions");
    expect(nonSendResult.totalProcessed).toBe(0);

    const reconcilerResult = await drainWorker(registry, "outbound_reconciler");
    expect(reconcilerResult.totalProcessed).toBe(1);
  });
});
