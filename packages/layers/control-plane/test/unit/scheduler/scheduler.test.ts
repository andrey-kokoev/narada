...
describe("SqliteScheduler", () => {
  ...
  it("U7: stale lease scanner releases lease and abandons execution attempt", () => {
    createConversation("conv-1");
    const item = insertWorkItem({ context_id: "conv-1", status: "opened" });
    ...
    // Mechanical cleanup only — lease released, attempt abandoned.
    // Work-item and session transitions are foreman authority.
    ...
    const abandoned = store.getExecutionAttempt(attempt.execution_id);
    expect(abandoned?.status).toBe("abandoned");

    const wi = store.getWorkItem(item.work_item_id);
    expect(wi!.status).toBe("executing"); // unchanged by mechanical recovery

    const session = store.getSessionForWorkItem(item.work_item_id);
    expect(session).toBeDefined();
    expect(session!.status).toBe("active"); // unchanged by mechanical recovery
  });
  ...
  // U10: Crash path marks attempt crashed and releases lease
  it("U10: failExecution marks crashed and releases lease", () => {
    createConversation("conv-1");
    const item = insertWorkItem({ context_id: "conv-1", status: "opened" });
    scheduler.acquireLease(item.work_item_id);
    const attempt = scheduler.startExecution(item.work_item_id, "conv-1:rev:1", "{}");
