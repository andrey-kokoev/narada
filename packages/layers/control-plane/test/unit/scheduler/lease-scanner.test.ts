...
describe("LeaseScanner", () => {
  ...
  it("recovers stale leases mechanically (lease release + attempt abandon)", () => {
    ...
    // Mechanical cleanup only — work-item transition is foreman authority.
    const item = store.getWorkItem(wi);
    expect(item!.status).toBe("leased"); // unchanged
    expect(item!.retry_count).toBe(0);   // unchanged
