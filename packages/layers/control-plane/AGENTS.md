# AGENTS.md — control-plane package
...
> **Monolithic Control Plane Note**: The foreman, scheduler, charter runtime integration, outbound pipeline, and coordinator store all live inside this single `control-plane` package. They are separated by directory structure and interface discipline, not by physical packages. See [`docs/02-architecture.md`](docs/02-architecture.md) for the rationale and future extraction criteria.
...
| [`src/executors/process-executor.ts`](src/executors/process-executor.ts) | `ProcessExecutor` — subprocess intent execution, distinct lease/recovery model |
| [`src/foreman/facade.ts`](src/foreman/facade.ts) | `DefaultForemanFacade` — work opening, resolution, outbound handoff orchestration |
| [`src/foreman/types.ts`](src/foreman/types.ts) | `SyncCompletionSignal`, `ChangedContext`, `EvaluationEnvelope`, `ResolveWorkItemRequest` (`evaluation_id`-based, foreman loads from store) |
...
Key principles:
- The compiler (`narada`) determines source truth; the control plane decides what to do about it.
...
The control plane operates **two intentionally distinct lease/recovery substrates**:

1. **Scheduler leases** (`work_item_leases`) — Govern charter execution work items.
   - Recovery: `SqliteScheduler.recoverStaleLeases()` (mechanical: lease release + attempt abandon)
   - Outcome: Foreman `failWorkItem()` → `failed_retryable` or `failed_terminal`, session idled/abandoned
   - Retry authority: Scheduler `scanForRunnableWork()` (includes `failed_retryable` whose backoff has elapsed)

2. **Process execution leases** (`process_executions.lease_expires_at`) — Govern subprocess intents (`process.run`).
   - Recovery: `ProcessExecutor.recoverStaleExecutions()`
   - Outcome: Execution → `failed`, intent → `admitted`
   - Retry authority: `WorkerRegistry` (re-claims admitted intent)

These are **not accidental duplication**. They reflect genuinely different lifecycle semantics:
- Work items are long-lived, stateful, and retryable with backoff.
- Process intents are short-lived subprocesses; retry is handled by the worker registry loop.

The daemon dispatch loop calls both recovery paths explicitly. Do not merge or silently omit either.

See `docs/02-architecture.md` § "Dual Recovery Model" for the full rationale.
...
- **Schema authority**: The coordinator schema is defined inline in `SqliteCoordinatorStore.initSchema()` (`src/coordinator/store.ts`). Do not treat any standalone `.sql` file as authoritative.
...
- Secondary references (`context_id`, `work_item_id`, `session_id`, `reference_outbound_id`) exist only for navigation and diagnostics.
...
pnpm test             # Vitest (package-scoped; all tests in this package)
