...
export class SqliteCoordinatorStore implements CoordinatorStore {
  ...
  initSchema(): void {
    this.db.exec(`
      ...
      create index if not exists idx_context_records_scope
        on context_records(scope_id, status, updated_at);

      -- v2 context_revisions (monotone ordinal tracking)
      ...
      -- reactor_outputs (durable reactor evaluation output)
  ...
  insertEvaluation(evaluation: Evaluation): void {
    this.db.prepare(`
      insert into evaluations (
        ...
        output_version, analyzed_at, outcome, confidence_json, summary,
        classifications_json, facts_json, escalations_json, proposed_actions_json,
        tool_requests_json, recommended_action_class, created_at
      ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      evaluation.recommended_action_class,
  ...
  getReactorOutputsByReactor(reactorId: string, limit: number = 100): ReactorOutputRow[] {
    const rows = this.db.prepare(`
      ...
      order by evaluated_at desc
      limit ?
    `).all(reactorId, limit) as Record<string, unknown>[];
    return rows.map(rowToReactorOutput);
  }
