...
import type { CoordinatorStore, WorkItem, Evaluation } from "../coordinator/types.js";
...
export function buildEvaluationRecord(
  ...
  return {
    ...
    charter_id: output.charter_id as import("../foreman/types.js").CharterId,
    role: output.role,
...
/**
 * Persist an evaluation envelope to the coordinator store.
 *
 * Responsibility: runtime (daemon dispatch). The foreman only consumes
 * already-persisted evaluations during resolveWorkItem.
 */
export function persistEvaluation(
  ...
  const existing = store.getEvaluationByExecutionId(evaluation.execution_id);
  if (existing) {
    return;
  }
  ...
  store.insertEvaluation(evalRow);
}
