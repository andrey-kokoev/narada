 ...
 * ## Recovery Model
 *
 * This scheduler owns recovery for **charter execution work items** via
 * `recoverStaleLeases()` on the `work_item_leases` table.
 *
 * It does NOT own recovery for subprocess intents (`process.run`).
 * `ProcessExecutor` has an intentionally distinct lease/recovery model
 * for process executions. See `src/executors/process-executor.ts` for
 * the rationale and the dual-model documentation.
 */
...
export class SqliteScheduler implements Scheduler {
  ...
  scanForRunnableWork(scopeId?: string, limit = 10): WorkItem[] {
    // Caller is responsible for recoverStaleLeases() + foreman.failWorkItem()
    // before scanning so that recovered work items become runnable.
  ...
  failExecution(executionId: string, errorMessage: string, _retryable: boolean): void {
    const now = new Date().toISOString();
    const attempt = this.store.getExecutionAttempt(executionId);
    if (!attempt) {
      throw new Error("Execution attempt not found");
  ...
  isQuiescent(scopeId?: string): boolean {
    // Caller is responsible for recoverStaleLeases() + foreman.failWorkItem()
    // before checking quiescence so that recovered work items are accounted for.
    const now = new Date().toISOString();
