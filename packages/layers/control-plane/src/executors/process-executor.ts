 ...
 *
 * ## Recovery Model (Intentionally Distinct from Scheduler)
 *
 * The ProcessExecutor maintains its own `lease_expires_at` / `lease_runner_id`
 * fields on `process_executions` and its own `recoverStaleExecutions()`.
 * This is **intentionally distinct** from the scheduler's `work_item_leases`
 * recovery model, not an accidental parallel substrate.
 *
 * | Aspect | Scheduler (`work_item_leases`) | ProcessExecutor (`process_executions`) |
 * |--------|-------------------------------|----------------------------------------|
 * | Scope | Charter execution work items | Subprocess intents (`process.run`) |
 * | Retry model | `failed_retryable` with exponential backoff | Reset intent to `admitted`; WorkerRegistry retry loop |
 * | Recovery outcome | Work item retryable, session idled | Intent reset to admitted, execution marked failed |
 * | Lease table | `work_item_leases` | `process_executions.lease_expires_at` |
 * | Caller | `recoverStaleLeases()` → `foreman.failWorkItem()` | Daemon dispatch loop → `recoverStaleExecutions()` |
 *
 * Unification would require making process intents first-class work items
 * under scheduler authority. That is a future architectural refactor, not
 * a bug fix. Until then, the daemon calls both recovery paths explicitly.
 *
