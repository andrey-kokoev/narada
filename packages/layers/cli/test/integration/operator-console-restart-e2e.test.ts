...
async function stopDetachedProjection(port, env) {
  const stopper = launchNode(cliEntrypoint, [
    'console', 'stop',
    '--host', '127.0.0.1',
    '--port', String(port),
  ], env);
  try {
    await waitForChildExit(stopper.child, 15_000);
  } catch {
    terminateProcessTree(stopper.child.pid);
  }
}
...
async function waitForOperatorConsoleRoute(url, predicate, processInfo, timeoutMs = 20_000, { failOnProcessExit = true } = {}) {
  ...
  let lastRoutes = null;
  while (Date.now() < deadline) {
    if (failOnProcessExit && processInfo.child.exitCode !== null) {
...
test('console restart authenticates across real process boundaries and replaces the projection', {
  ...
  try {
    ...
    const replacementRoute = await waitForOperatorConsoleRoute(
      ...
      restartedProjection,
      20_000,
      { failOnProcessExit: false },
    );

    assert.notEqual(replacementRoute.owner_id, initialRoute.owner_id);
    await waitForChildExit(restartedProjection.child);
    assert.equal(restartedProjection.child.exitCode, 0);
    ...
    // `console restart` intentionally leaves the replacement runtime detached
    // after its short-lived CLI command exits. Stop it through the same
    // lifecycle command before deleting the isolated profile; otherwise its
    // backend can keep the registry SQLite file open and make test cleanup
    // hang or fail with EBUSY.
    if (restartedProjection?.child.exitCode === null && restartedProjection.child.signalCode === null) {
      try {
        await waitForChildExit(restartedProjection.child, 5_000);
      } catch {
        terminateProcessTree(restartedProjection.child.pid);
      }
    }
    await stopDetachedProjection(port, env);
