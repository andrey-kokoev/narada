...
describe('task report operator', () => {
  ...
  it('refuses report instead of creating an unrouted review obligation when no reviewer resolves', async () => {
    ...
    expect((result.result as { error: string }).error).toContain('Unrouted directed review obligations are not admitted');
  ...
  it('is idempotent: repeated report on same assignment does not create duplicate', async () => {
    ...
    const resetStore = openTaskLifecycleStore(tempDir);
    try {
      resetStore.updateStatus('20260420-999-test-task', 'claimed', 'test-agent');
    } finally {
      resetStore.db.close();
    }
    writeFileSync(
  ...
  it('creates a new report after re-claim with a different assignment', async () => {
    ...
    // Reset SQLite lifecycle status and insert a new active assignment for re-claim simulation
    const newClaimedAt = new Date().toISOString();
    const resetStore2 = openTaskLifecycleStore(tempDir);
    try {
      resetStore2.updateStatus('20260420-999-test-task', 'claimed', 'test-agent');
      resetStore2.insertAssignment({
      ...
      resetStore2.db.close();
    }

    // Keep assignment record projection in sync
