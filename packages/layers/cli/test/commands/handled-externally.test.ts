...
import { handledExternallyCommand } from '../../src/commands/handled-externally.js';
import { ExitCode } from '../../src/lib/exit-codes.js';
import type { CommandContext } from '../../src/lib/command-wrapper.js';
import { mkdtempSync, writeFileSync, mkdirSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { Database, SqliteOutboundStore, SqliteIntentStore, SqliteCoordinatorStore } from '@narada-core/control-plane';
...
describe('handled-externally command', () => {
  let tempDir: string;
  let configPath: string;
  let dbPath: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'narada-handled-externally-test-'));
  ...
  it('records handled externally and transitions to cancelled', async () => {
    seedDraftReadyOutbound('out-1');

    const context = createMockContext({ configPath });
    const result = await handledExternallyCommand(
      { outboundId: 'out-1', ref: 'ticket-123' },
      context,
    );
    ...
    expect(cmd.terminal_reason).toBe('handled_externally');
    expect(cmd.external_reference).toBe('ticket-123');

    const intentStore = new SqliteIntentStore({ db });
    const intent = intentStore.getByTargetId('out-1')!;
    expect(intent.status).toBe('cancelled');
    expect(intent.terminal_reason).toBe('handled_externally');

    // Verify audit record
    const coordinatorStore = new SqliteCoordinatorStore({ db });
    const allActions = db.prepare("select * from operator_action_requests where action_type = 'handled_externally'").all() as Array<{
      request_id: string;
      scope_id: string;
      action_type: string;
      target_id: string;
    ...
    expect(allActions[0]!.action_type).toBe('handled_externally');
    expect(allActions[0]!.target_id).toBe('out-1');
    expect(allActions[0]!.status).toBe('executed');
    expect(JSON.parse(allActions[0]!.payload_json)).toEqual({ external_reference: 'ticket-123' });
  ...
  it('returns error when outbound command is not draft_ready and audits rejection', async () => {
    ...
    const context = createMockContext({ configPath });
    const result = await handledExternallyCommand({ outboundId: 'out-2', ref: 'ticket-123' }, context);
    ...
    const allActions = db2.prepare("select * from operator_action_requests where action_type = 'handled_externally'").all() as Array<{
      request_id: string;
      scope_id: string;
      action_type: string;
      target_id: string;
  ...
  it('returns error when outbound command is not found', async () => {
    const context = createMockContext({ configPath });
    const result = await handledExternallyCommand({ outboundId: 'missing', ref: 'ticket-123' }, context);
