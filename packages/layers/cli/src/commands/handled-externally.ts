...
export interface HandledExternallyOptions {
  config?: string;
  verbose?: boolean;
...
export async function handledExternallyCommand(
  options: HandledExternallyOptions,
  ...
  for (let i = 0; i < scopeIds.length; i++) {
    ...
    try {
      ...
      const result = await executeOperatorAction(
        ...
        {
          action_type: 'handled_externally',
          target_id: options.outboundId,
          payload_json: JSON.stringify({ external_reference: options.ref }),
        },
      );

      if (result.status === 'executed') {
        fmt.message(`Recorded draft ${options.outboundId} as handled externally (${options.ref})`, 'success');
        return {
          exitCode: ExitCode.SUCCESS,
          result: { status: 'success', outbound_id: options.outboundId, action: 'handled_externally' },
        };
      }
