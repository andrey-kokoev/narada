...
export async function acknowledgeAlertCommand(
  ...
  for (let i = 0; i < scopeIds.length; i++) {
    ...
    try {
      ...
      const workItem = coordinatorStore.getWorkItem(options.workItemId);
      ...
      const result = await executeOperatorAction(
        ...
        },
        {
          ...
          target_id: options.workItemId,
        },
      );

      if (result.status === 'executed') {
        fmt.message(`Acknowledged alert for work item ${options.workItemId}`, 'success');
        return {
          exitCode: ExitCode.SUCCESS,
          result: { status: 'success', work_item_id: options.workItemId, action: 'acknowledge_alert' },
        };
      }
  ...
  return {
    exitCode: ExitCode.GENERAL_ERROR,
    result: { status: 'error', error: `Work item ${options.workItemId} not found in any operation` },
  };
}
