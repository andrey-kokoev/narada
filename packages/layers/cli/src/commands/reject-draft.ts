...
export async function rejectDraftCommand(
  options: RejectDraftOptions,
  ...
  for (let i = 0; i < scopeIds.length; i++) {
    ...
    try {
      ...
      // Confirm the command exists in this scope before attempting action
      ...
      const result = await executeOperatorAction(
        ...
        },
        {
          ...
          payload_json: options.rationale ? JSON.stringify({ rationale: options.rationale }) : undefined,
        },
      );

      if (result.status === 'executed') {
        fmt.message(`Rejected draft ${options.outboundId}${options.rationale ? ` (${options.rationale})` : ''}`, 'success');
        return {
          exitCode: ExitCode.SUCCESS,
          result: { status: 'success', outbound_id: options.outboundId, action: 'reject_draft' },
        };
      }

      // If rejected (validation failure), surface the reason
      return {
