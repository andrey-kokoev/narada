    ...
    try {
      ...
      const result = await executeOperatorAction(
        ...
        },
        {
          ...
          payload_json: options.notes ? JSON.stringify({ reviewer_notes: options.notes }) : undefined,
        },
      );

      if (result.status === 'executed') {
        fmt.message(`Marked draft ${options.outboundId} as reviewed`, 'success');
        return {
          exitCode: ExitCode.SUCCESS,
          result: { status: 'success', outbound_id: options.outboundId, action: 'mark_reviewed' },
        };
      }
