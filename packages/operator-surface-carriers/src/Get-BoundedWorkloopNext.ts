#!/usr/bin/env node
/**
 * Get-BoundedWorkloopNext.ts — Node.js prototype of the bounded workloop selector.
 *
 * Reads workboard + obligations + git status, applies policy rules, emits JSON.
 *
 * Usage:
 *   node Get-BoundedWorkloopNext.ts [--agent <name>] [--limit <n>] [--user-site-root <path>]
...
type WorkloopArgs = {
  ...
  intendedTaskNumber: number;
  beforeMutation: string;
  mutationPath: string[];
  userSiteRoot: string;
  naradaCli: string | null;
  workboardFixturePath: string | null;
  gitStatusFixturePath: string | null;
  obligationsPath: string | null;
};


function parseArgs(argv: string[]): WorkloopArgs {
  const args: WorkloopArgs = {
...
function getPropertyValue(obj: AnyRecord | null | undefined, names: string[]): any {
...
function getTaskNumber(obj: AnyRecord): number | null {
...
function readWorkboard(args: WorkloopArgs): AnyRecord {
  ...
  if (existsSync(scriptPath)) {
    ...
    if (result.status === 0) {
      try { return JSON.parse(String(result.stdout)); } catch { /* fall through */ }
    }
  }
...
function readDirectedObligations(args: WorkloopArgs, workboard: AnyRecord): AnyRecord {
...
function convertDirectedObligationFact(obligation: AnyRecord | null | undefined): AnyRecord | null {
...
function getWorkboardFacts(workboard: AnyRecord): AnyRecord {
  if (workboard.tasks) {
...
function convertTaskFact(task: AnyRecord | null | undefined): AnyRecord | null {
...
function readGitStatusLines(args: WorkloopArgs): string[] {
...
function convertGitStatusLine(line: string): AnyRecord | null {
...
function findTaskFile(userSiteRoot: string, taskNumber: number): string | null {
  const patterns = [
...
function getTaskNumberFromPath(path: string): number | null {
  const m = path.match(/^\.ai\/(?:do-not-open\/)?tasks\/\d{8}-(\d+)-/);
  return m ? parseInt(m[1], 10) : null;
}

function testTaskMentionsPath(taskPath: string | null, dirtyPath: string): boolean {
...
function getDirtyFileFacts(args: WorkloopArgs, activeTasks: AnyRecord[]): AnyRecord[] {
  const taskByNumber: Record<number, AnyRecord> = {};
...
function checkCollaboratorOverlap(nextAction: AnyRecord, dirtyFiles: AnyRecord[], currentAgent: string): AnyRecord {
  if (!nextAction.task_number) return { has_overlap: false, overlaps: [] };
  const overlaps: AnyRecord[] = [];
  for (const file of dirtyFiles) {
    const relatedToNext = file.likely_tasks.some((t: AnyRecord) => t.task_number === nextAction.task_number);
    if (!relatedToNext) continue;
    const otherOwners = file.likely_tasks.filter((t: AnyRecord) =>
      t.task_number !== nextAction.task_number && t.owner && t.owner !== currentAgent
    );
    if (otherOwners.length > 0) {
      overlaps.push({
        path: file.path,
        other_owners: [...new Set(otherOwners.map((t: AnyRecord) => t.owner))],
        other_tasks: [...new Set(otherOwners.map((t: AnyRecord) => t.task_number))],
      });
    }
  }
...
function getMutationPreflight(args: WorkloopArgs, dirtyFiles: AnyRecord[]): AnyRecord {
  const warnings = [];
  const blocking = [];
...
function newHumanSummary(args: WorkloopArgs, facts: AnyRecord, preflight: AnyRecord, warnings: string[]): AnyRecord {
  const lines = [];
  ...
  for (const file of facts.dirty_files.slice(0, 5)) {
    const owners = file.likely_tasks.map((t: AnyRecord) => `#${t.task_number}:${t.reason}`).join(',');
...
function main(): void {
  ...
  const pendingReviews = toArray(workboardFacts.pending_reviews).map(convertTaskFact).filter((t): t is AnyRecord => t !== null && t.task_number != null)
    .sort((a, b) => (a.preferred_agent_id === args.agent ? 0 : 1) - (b.preferred_agent_id === args.agent ? 0 : 1))
    .slice(0, args.limit);
  const inProgress = toArray(workboardFacts.in_progress).map(convertTaskFact).filter((t): t is AnyRecord => t !== null && t.task_number != null)
    .sort((a, b) => (a.preferred_agent_id === args.agent ? 0 : 1) - (b.preferred_agent_id === args.agent ? 0 : 1))
    .slice(0, args.limit);
  const localFollowups = toArray(workboardFacts.local_followups).map(convertTaskFact).filter((t): t is AnyRecord => t !== null && t.task_number != null)
    .sort((a, b) => (a.preferred_agent_id === args.agent ? 0 : 1) - (b.preferred_agent_id === args.agent ? 0 : 1))
    .slice(0, args.limit);
