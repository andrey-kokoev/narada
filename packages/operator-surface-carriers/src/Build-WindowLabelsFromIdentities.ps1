...
$result = [ordered]@{
    ...
    projection_authority = if ($identityRegistry.projection_authority -eq "sqlite") { "sqlite" } else { "identity_json" }
    projection_source = if ($identityRegistry.projection_source) { [string]$identityRegistry.projection_source } else { $identityPath }
    projection_note = if ($identityRegistry.projection_authority -eq "sqlite") { "Generated projection for the current OSL renderer. Do not edit as authority." } else { "Generated projection from identities JSON." }
