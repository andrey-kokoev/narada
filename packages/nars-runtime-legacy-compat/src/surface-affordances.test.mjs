...
test('NARS surface affordance projection advertises runtime intelligence controls', () => {
  const projection = buildNarsSurfaceAffordanceProjection({
    ...
    intelligence: {
      ...
      model: 'gpt-5.5',
      available_models: ['gpt-5.5', 'gpt-5.6'],
  ...
  assert.deepEqual(item.actions.configure, ['set_provider', 'set_model', 'set_thinking']);
  assert.deepEqual(item.controls.provider.choices.map((choice) => choice.value), ['codex-subscription', 'openai-api', 'deepseek-api']);
  ...
  assert.deepEqual(item.affordance_document.actions.map((action) => ({ id: action.id, target: action.target })), [
    { id: 'set_provider', target: { kind: 'runtime', operation: 'set_provider' } },
