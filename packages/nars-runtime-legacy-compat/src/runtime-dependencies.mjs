...
/**
 * Provider-only compatibility adapter. Unlike createCarrierRuntimeDependencies,
 * it allocates no session persistence, MCP lifecycle, request interpreter, or
 * legacy control state.
 */
export function createLegacyProviderCall({ runtimeContext = {}, env = process.env } = {}) {
  const intelligenceProvider = runtimeContext.intelligenceProvider ?? env.NARADA_INTELLIGENCE_PROVIDER ?? 'codex-subscription';
  const defaults = resolveProviderRuntimeDefaults(intelligenceProvider, env);
  const siteRoot = resolve(runtimeContext.siteRoot ?? env.NARADA_SITE_ROOT ?? process.cwd());
  const providerSettings = {
    provider: intelligenceProvider,
    model: runtimeContext.providerSettings?.model ?? defaults.model,
    thinking: runtimeContext.providerSettings?.thinking ?? defaults.thinking,
  ...
  return (messages, tools, settings = {}) => callChatApi(messages, tools, {
    ...providerSettings,
    ...settings,
    provider: intelligenceProvider,
    apiKey: defaults.apiKey,
...
async function handleServerRequestLine(line, context) {
  ...
  if (request?.method === 'session.affordance.action.request') {
    const requestId = request?.id ?? null;
...
async function handleServerRequest(request, context) {
  ...
  try {
    ...
    if (controlRequest.method_kind === 'carrier_input_deliver') {
      const deliveryConstructor = request?.params?.delivery_constructor
        ?? request?.params?.input?.metadata?.nars_session_input?.delivery_constructor
        ?? 'send';
      let input = normalizeServerControlInputRequest(request, requestId);
      if (deliveryConstructor === 'steer') {
        if (!state.activeTurn) {
          emit('error', { request_id: requestId, code: 'no_active_turn', message: 'steer delivery requires an active turn.' });
          return;
        }
        const activeTurn = state.activeTurn;
        context.appendSessionRecord?.({
          event: 'carrier_input_steer_requested',
          request_id: requestId,
          input_event_id: input.event_id,
          active_turn_id: activeTurn.turnId,
        ...
        emit('carrier_input_steer_requested', {
          request_id: requestId,
          input_event_id: input.event_id,
          active_turn_id: activeTurn.turnId,
          source_kind: input.source_kind,
        ...
        requestTurnInterrupt(activeTurn);
        emit('turn_interrupted', { request_id: requestId, turn_id: activeTurn.turnId, terminal_state: 'interrupted_requested', reason: 'carrier_input_steering' });
        input = {
          ...input,
          delivery_mode: 'admit_after_active_turn',
          metadata: {
            ...(input.metadata ?? {}),
            nars_session_input: {
              ...(input.metadata?.nars_session_input ?? {}),
      ...
      await state.inputQueue.enqueue(input, { drain: true, state });
      return;
    }
