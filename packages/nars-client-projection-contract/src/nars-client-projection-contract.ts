...
export function normalizeNarsSessionEventView(view: any = NARS_SESSION_EVENT_DEFAULT_VIEW) {
...
export const NARS_CLIENT_PROJECTION_VERBOSITY_RANK: Record<string, number> = Object.freeze({
  conversation: 0,
  operations: 1,
  diagnostics: 2,
  raw: 3,
...
export const NARS_CLIENT_EVENT_LABELS: Record<string, string> = Object.freeze({
...
export function isAgentWebUiNarsMethod(method: any) {
  return AGENT_WEB_UI_NARS_METHODS.has(method);
}

export function isAgentWebUiLegacyNarsMethod(method: any) {
  return AGENT_WEB_UI_LEGACY_METHODS.has(method);
}

export function isAgentWebUiCloudflareMethod(method: any) {
  return AGENT_WEB_UI_CLOUDFLARE_METHODS.has(method);
}

function authorityTransitionSummary(event: AnyRecord, kind: any): string {
  ...
  if (kind === 'authority_target_activation_refused') {
    const refusals = Array.isArray(event.refusals) ? event.refusals.map((entry: AnyRecord) => entry?.reason_code).filter(Boolean).join(', ') : '';
...
function authorityLocatorSummary(locator: any, fallbackRef: any = null): string | null {
...
export function buildNarsArtifactRefPart({ artifactId, artifact_id, kind = null, title = null, renderHint = null, render_hint = null } : AnyRecord = {}) {
...
export function buildAgentWebUiConversationEnqueueFrame(text: any, options: AnyRecord = {}): AnyRecord | null {
...
export function buildAgentWebUiEventsReadFrame(options: AnyRecord = {}) {
...
export function buildAgentWebUiSopSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiMailboxSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiInboxSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiDelegationSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiGitSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiArtifactsSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiSurfaceFeedbackSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiSchedulerSummaryFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiTaskLifecycleSummaryFrame(options: AnyRecord = {}) {
  const params: AnyRecord = {
...
export function buildAgentWebUiSurfaceAffordancesFrame(options: AnyRecord = {}) {
  return {
...
export function buildAgentWebUiAffordanceActionConfirmFrame({ confirmationId, confirmation_id } : AnyRecord = {}, options: AnyRecord = {}) {
...
export function buildAgentWebUiAffordanceActionCancelFrame({ confirmationId, confirmation_id, reason = 'operator_cancelled' } : AnyRecord = {}, options: AnyRecord = {}) {
  const normalizedConfirmationId = String(confirmation_id ?? confirmationId ?? '').trim();
  if (!normalizedConfirmationId) return null;
  return {
...
export function buildNarsAffordanceActionRequestedEvent({ requestId, request_id, surfaceId, surface_id, actionId, action_id, clientCorrelationId = null, client_correlation_id = null } : AnyRecord = {}) {
  return {
...
export function buildNarsAffordanceActionResultEvent({ requestId, request_id, surfaceId, surface_id, actionId, action_id, serverName, server_name, toolName, tool_name, clientCorrelationId = null, client_correlation_id = null, result = null, status = 'ok', terminalState, terminal_state } : AnyRecord = {}) {
  return {
...
export function buildNarsAffordanceActionFailureEvent({ requestId, request_id, surfaceId, surface_id, actionId, action_id, serverName, server_name, toolName, tool_name, clientCorrelationId = null, client_correlation_id = null, error } : AnyRecord = {}) {
  const event: AnyRecord = buildNarsAffordanceActionResultEvent({
    request_id: request_id ?? requestId ?? null,
    surface_id: surface_id ?? surfaceId ?? null,
    action_id: action_id ?? actionId ?? null,
    server_name: server_name ?? serverName ?? null,
...
export function buildNarsAffordanceActionRefusalEvent({ requestId, request_id, surfaceId, surface_id, actionId, action_id, clientCorrelationId = null, client_correlation_id = null, code, message, serverName = null, server_name = null, toolName = null, tool_name = null, posture = null } : AnyRecord = {}) {
  return {
...
export function buildNarsAffordanceActionConfirmationRequiredEvent(options: AnyRecord = {}) {
...
export function buildNarsAffordanceActionConfirmedEvent({ requestId, request_id, confirmationId, confirmation_id, surfaceId = null, surface_id = null, actionId = null, action_id = null } : AnyRecord = {}) {
  return {
...
export function buildNarsAffordanceActionCancelledEvent({ requestId, request_id, confirmationId, confirmation_id, surfaceId = null, surface_id = null, actionId = null, action_id = null, reason = null } : AnyRecord = {}) {
  return {
...
function requestIdForCommand(command: any): string {
  return `agent-web-ui-${command}-${Date.now()}`;
}

function commandFrame(command: any, value: any = ''): AnyRecord {
  return {
...
export function buildAgentWebUiConversationSendFrame(text: any, options: AnyRecord = {}): AnyRecord | null {
  const message = String(text ?? '').trim();
  if (!message) return null;
  return {
...
export function buildAgentWebUiConversationSteerFrame(text: any, options: AnyRecord = {}): AnyRecord | null {
  const message = String(text ?? '').trim();
  if (!message) return null;
  return {
...
export function buildAgentWebUiSubscribeFrame(options: AnyRecord = {}) {
  const params: AnyRecord = {
...
export function isNarsSessionCoreProtocolFrame(frame: any): boolean {
  return Boolean(frame && typeof frame === 'object' && isNarsSessionCoreMethod(frame.method));
}

export function isAgentWebUiCloudflareProtocolFrame(frame: any): boolean {
...
export function isAgentWebUiProtocolFrame(frame: any): boolean {
...
export function translateAgentWebUiFrameForCloudflare(frame: any): AnyRecord | null {
...
function localCommand(id: any, slash: any, options: AnyRecord = {}): AnyRecord {
...
function frameCommand(id: any, slash: any, method: any, options: AnyRecord = {}): AnyRecord {
  return localCommand(id, slash, {
    ...options,
    protocolMethods: [method],
    kind: options.kind ?? 'nars_protocol',
    group: options.group ?? 'session',
    buildAction: (input: AnyRecord, context: AnyRecord = {}) => ({
      kind: 'frame',
...
function sessionCommand(id: any, slash: any, options: AnyRecord = {}): AnyRecord {
  return localCommand(id, slash, {
    ...options,
    protocolMethods: [NARS_COMMAND_METHOD],
    kind: 'nars_session_command',
    group: options.group ?? 'session',
    buildAction: (input: AnyRecord) => ({ kind: 'frame', frame: commandFrame(slash, input.value) }),
  });
}
...
export function findAgentWebUiSnippetAction(rawVerb: string = '') {
...
export function filterAgentWebUiSnippetActions(query: string = '') {
  ...
  return [...actions].sort((left: AnyRecord, right: AnyRecord) => left.rank - right.rank);
}

export function isAgentWebUiSnippetSelectionAction(rawVerb: string = '') {
  return findAgentWebUiSnippetAction(rawVerb)?.mode === 'select';
}

export function isAgentWebUiSnippetManagementAction(rawVerb: string = '') {
  const action = findAgentWebUiSnippetAction(rawVerb);
  return Boolean(action && (action.mode === 'write' || action.mode === 'delete'));
}

export function parseAgentWebUiSnippetCommand(value: string = '') {
...
export const AGENT_WEB_UI_COMMANDS = Object.freeze([
  ...
  localCommand('events', '/events', {
    ...
    rank: 120,
    buildAction: (input: AnyRecord, context: AnyRecord = {}) => ({ kind: 'frame', frame: buildAgentWebUiSubscribeFrame({ id: context.id ?? requestIdForCommand('events'), maxReplay: 20, includeReplay: true }) }),
  }),
  ...
  localCommand('snippet', '/snippet', {
    ...
    rank: 370,
    buildAction: (input: AnyRecord) => ({ kind: 'snippet_command', value: input.value, raw: input.raw }),
  }),
  localCommand('snippets', '/snippets', {
    ...
    rank: 371,
    buildAction: (input: AnyRecord) => ({ kind: 'snippet_panel_command', value: input.value, raw: input.raw }),
  }),
]);

export const AGENT_WEB_UI_COMMAND_GROUP_LABELS: Record<string, string> = Object.freeze({
...
export function buildAgentWebUiHelpText(options: AnyRecord = {}) {
...
export function findAgentWebUiCommand(rawCommand: any) {
  const command = String(rawCommand ?? '').trim().toLowerCase();
  if (!command) return null;
  return AGENT_WEB_UI_COMMANDS.find((entry: AnyRecord) => entry.slash === command || entry.aliases.includes(command)) ?? null;
}

export function filterAgentWebUiCommands(query: string = '', options: AnyRecord = {}) {
  const normalized = String(query ?? '').trim().toLowerCase().replace(/^\//, '');
  const commands = AGENT_WEB_UI_COMMANDS.filter((entry: AnyRecord) => (
    entry.palette.visible !== false
    && (typeof options.supportsProtocolMethod !== 'function'
      || entry.protocolMethods.length === 0
      || entry.protocolMethods.some((method: any) => options.supportsProtocolMethod(method)))
  ));
  if (!normalized) return [...commands].sort(compareAgentWebUiCommands);
  return commands
    .map((entry: AnyRecord) => ({ entry, score: agentWebUiCommandMatchScore(entry, normalized) }))
    .filter((candidate) => candidate.score >= 0)
    .sort((left: AnyRecord, right: AnyRecord) => left.score - right.score || compareAgentWebUiCommands(left.entry, right.entry))
    .map((candidate) => candidate.entry);
}

function compareAgentWebUiCommands(left: AnyRecord, right: AnyRecord) {
  return (left.palette.rank ?? 500) - (right.palette.rank ?? 500) || left.slash.localeCompare(right.slash);
}

function agentWebUiCommandMatchScore(entry: AnyRecord, query: string): number {
...
export function buildAgentWebUiOperatorInputAction(text: any, options: AnyRecord = {}): AnyRecord | null {
...
export function unwrapNarsClientEvent(message: any): AnyRecord | null {
...
export function normalizeNarsClientProjectionVerbosity(verbosity: any = NARS_CLIENT_PROJECTION_DEFAULT_VERBOSITY): string {
  const normalized = String(verbosity ?? NARS_CLIENT_PROJECTION_DEFAULT_VERBOSITY).trim().toLowerCase();
  return NARS_CLIENT_PROJECTION_VERBOSITY_LEVELS.includes(normalized as any) ? normalized : NARS_CLIENT_PROJECTION_DEFAULT_VERBOSITY;
}

export function isRoutineHealthyNarsSessionHealth(event: AnyRecord): boolean {
...
export function classifyNarsClientEventProjection(projection: AnyRecord): string {
...
export function shouldProjectNarsClientEvent(message: any, options: AnyRecord = {}): boolean {
...
function toolName(event: AnyRecord, fallback: any) {
  return event?.tool_name ?? event?.tool ?? event?.name ?? fallback;
}

function errorSummary(event: AnyRecord): string | null {
...
function diagnosticSummary(event: AnyRecord, kind: any): string {
...
function runtimeProjectionFailureSummary(event: AnyRecord): string {
...
function runtimeControlInputBridgeErrorSummary(event: AnyRecord): string {
...
function runtimeIntelligenceReconfigurationSummary(event: AnyRecord): string {
...
export function projectNarsClientEvent(message: any): AnyRecord | null {
  const event: AnyRecord = unwrapNarsClientEvent(message) ?? {};
...
function eventRenderKey(event: AnyRecord, kind: any): string | null {
  if (!event || typeof event !== 'object') return null;
...
function genericRenderKey(event: AnyRecord, kind: any): string | null {
...
function projectNestedProviderNarsClientEvent(event: AnyRecord): AnyRecord | null {
...
function projectNestedProviderItemEvent(type: any, item: AnyRecord, event: AnyRecord) {
...
function projection({ kind, class: eventClass = null, label, tone, summary, event, renderKey = null }: AnyRecord): AnyRecord {
  return {
    kind,
...
function providerItemRenderKey(event: AnyRecord, item: AnyRecord, prefix: string): string | null {
...
function eventAgentDisplay(event: AnyRecord): string | null {
...
function eventAgentGroupKey(event: AnyRecord): string | null {
...
export function buildNarsAttachCommands({ eventEndpoint = '<session_started.event_endpoint>', healthEndpoint = '<session_started.health_endpoint>' } : AnyRecord = {}): AnyRecord {
