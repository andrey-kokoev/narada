...
`product:resident-dispatch:windows-fallback:execute` is the adjacent governed local executor surface. It reads the pending fallback request for the site or operation, requires explicit `--execute-windows-fallback` acknowledgement, then records `resident_dispatch.windows_fallback_evidence.put` with the Windows-local execution and resident-session refs so the operator path advances through an actual fallback execution lane instead of stopping at queue inspection.

`product:resident-dispatch:windows-fallback-evidence:review` is the adjacent focused review surface once that execution evidence exists. It reads `operation.read`, selects the focused fallback-evidence record, shows the local execution and session-start admissions that were recorded, and emits the exact `product:operation:focus-review` acknowledgement command so the operator can move the route beyond `review_windows_fallback_resident_dispatch_evidence` without dropping into raw JSON or generic evidence spelunking.

`product:resident-dispatch:windows-fallback-evidence` is the governed return lane after the Windows resident loop has actually executed that fallback. In put mode it records the Windows session-start evidence into Cloudflare with `local_session_start_admission=admitted_by_windows_resident_loop` and `direct_cloudflare_session_start_admission=not_admitted`. In list mode it reads back the recorded evidence posture for review without pretending that Cloudflare started the session itself.

`product:resident-dispatch:local-resident-carrier-bridge` is the governed replay-binding lane after that fallback evidence has been reviewed. In put mode it calls `resident_dispatch.local_resident_carrier_bridge.put`, resolves the latest fallback evidence for the operation when explicit refs are omitted, records the Cloudflare-side bridge from the Windows local resident session into a replayable Cloudflare carrier session/evidence posture, and keeps direct Cloudflare runtime session start explicitly `not_admitted`. In list mode it reads back the bridge posture for the site or operation.
