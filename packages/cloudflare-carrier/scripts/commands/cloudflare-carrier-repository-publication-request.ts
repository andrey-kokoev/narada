...
export function parseRepositoryPublicationRequestArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function createCloudflareRepositoryPublicationRequest(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_request_failed:${code}`);
    error.code = code;
...
export function summarizeRepositoryPublicationRequest(body: any= {}, params: any= {}) {
...
export function formatRepositoryPublicationRequestText(result: any) {
...
function redactRepositoryPublicationRequestParams(params: any= {}) {
  return { ...params };
}
