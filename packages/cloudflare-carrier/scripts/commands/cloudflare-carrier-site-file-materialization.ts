...
export function parseSiteFileMaterializationArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function admitCloudflareSiteFileMaterialization(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`site_file_materialization_request_failed:${code}`);
    error.code = code;
...
export function summarizeSiteFileMaterialization(body: any= {}, params: any= {}) {
...
export function formatSiteFileMaterializationText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.materialization_id) {
    lines.push(`Materialization Review: pnpm --filter @narada-core/cloudflare-carrier product:site-file:materialization:review:text -- --url ${workerUrl} --site ${summary.site_id} --site-file-materialization-id ${summary.materialization_id} --operator-session-file <operator-session-file>`);
  }
...
function redactSiteFileMaterializationParams(params: any= {}) {
  return { ...params };
}
