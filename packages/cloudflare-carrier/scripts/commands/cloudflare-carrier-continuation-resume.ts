...
import { putCloudflareOperationStatus } from './cloudflare-carrier-operation-status-put.ts';
...
export function parseContinuationResumeArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function resumeCloudflareContinuation(config: any, fetchImpl: any= fetch) {
  ...
  if (route && route.workflow_next_action !== CONTINUATION_RESUME_ACTION) {
    const error: any = new Error(`continuation_resume_route_refused:${route.workflow_next_action ?? 'missing_route'}`);
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`continuation_resume_session_start_failed:${code}`);
    error.code = code;
...
export async function readContinuationRoute(config: any, fetchImpl: any= fetch) {
  const response = await fetchImpl(new URL('/api/carrier', withTrailingSlash(config.workerUrl)), {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
  ...
  if (response.status < 200 || response.status >= 300) {
    const code = body?.code ?? body?.error ?? `http_${response.status}`;
    const error: any = new Error(`continuation_resume_route_read_failed:${code}`);
    error.code = code;
    error.http_status = response.status;
    error.response = body;
    error.summary = summarizeContinuationResumeFailure(body, config.params);
...
export function summarizeContinuationRoute(body: any= {}, params: any= {}) {
...
export function summarizeContinuationResume(result: any= {}, params: any= {}) {
...
export function summarizeContinuationRouteRefusal(route: any= {}, params: any= {}) {
  return {
    ok: false,
...
export function summarizeContinuationResumeFailure(body: any= {}, params: any= {}) {
  return {
    ok: body.ok ?? false,
...
export function formatContinuationResumeText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.carrier_session_id) {
    lines.push(`Session Evidence: pnpm --filter @narada-core/cloudflare-carrier product:session:evidence:text -- --url ${workerUrl} --site ${summary.site_id} --carrier-session-id ${summary.carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${summary.site_id} --carrier-session-id ${summary.carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --carrier-session-id ${summary.carrier_session_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
...
function generatedSessionId(operationId: any, now: any) {
