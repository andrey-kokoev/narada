...
import { createCloudflareTaskLifecycleTask } from './cloudflare-carrier-task-lifecycle-create.ts';
import { readProductSurface } from '../read-models/cloudflare-carrier-product-read.ts';

export function parseTaskLifecycleCreateFromDirectiveIntentArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function createTaskFromDirectiveIntent(config: any, fetchImpl: any= fetch, now: any= () => Date.now()) {
...
export function directiveIntentTaskTitle(record: any= {}) {
...
export function directiveIntentTaskPredicate(record: any= {}) {
  ...
  return (task: any= {}) => {
    ...
    return tokens.some((token: any) => taskText.includes(token));
  };
}

export function taskForDirectiveIntent(record: any= {}, operationReadBody: any= {}) {
...
export function selectDirectiveIntentWithoutTask(operationReadBody: any= {}) {
  ...
  return records.find((record: any) => !taskForDirectiveIntent(record, operationReadBody)) ?? records[0] ?? null;
}

export function formatTaskLifecycleCreateFromDirectiveIntentText(result: any) {
  ...
  if (workerUrl && result?.site_id && taskId) {
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${result.site_id} --task-id ${taskId} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${result.site_id} --task-id ${taskId} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
