...
export function parseSiteMembershipPutArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function putCloudflareSiteMembership(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`site_membership_put_request_failed:${code}`);
    error.code = code;
...
export function summarizeSiteMembershipPut(body: any= {}, params: any= {}) {
...
export function formatSiteMembershipPutText(result: any) {
