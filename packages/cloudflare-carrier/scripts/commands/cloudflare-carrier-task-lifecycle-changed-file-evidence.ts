...
export function parseTaskLifecycleChangedFileEvidenceArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function admitCloudflareTaskLifecycleChangedFileEvidence(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`changed_file_evidence_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleChangedFileEvidence(body: any= {}, params: any= {}) {
  ...
  return {
    ...
    operation_id: task.operation_id ?? evidence.operation_id ?? body.operation_id ?? params.operation_id ?? null,
    carrier_session_id: task.carrier_session_id ?? evidence.carrier_session_id ?? body.carrier_session_id ?? params.carrier_session_id ?? null,
...
export function formatTaskLifecycleChangedFileEvidenceText(result: any) {
...
function redactTaskLifecycleChangedFileEvidenceParams(params: any= {}) {
  return { ...params };
}
