...
export function parseRepositoryPublicationCloudflareExecutionArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function executeCloudflareRepositoryPublication(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_cloudflare_execution_failed:${code}`);
    error.code = code;
...
export function summarizeRepositoryPublicationCloudflareExecution(body: any= {}, params: any= {}) {
...
export function formatRepositoryPublicationCloudflareExecutionText(result: any) {
