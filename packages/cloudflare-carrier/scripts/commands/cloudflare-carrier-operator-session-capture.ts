...
export function parseOperatorSessionCaptureArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function captureOperatorSession({ workerUrl, host, port, timeoutMs }: any, fetchImpl: any= fetch) {
  let server: any;
  const received = new Promise((resolve: any, reject: any) => {
    server = createServer((request: any, response: any) => {
  ...
  await new Promise((resolve: any) => server.listen(port, host, resolve));
  ...
  try {
    const captured: any = await Promise.race([received, timeout]);
...
export async function verifyOperatorSession(workerUrl: any, cookie: any, fetchImpl: any= fetch) {
  ...
  if (response.status !== 200 || body?.ok === false) {
    ...
    const error: any = new Error(`operator_session_verify_failed:${code}`);
    error.code = code;
...
export function summarizeOperatorSessionVerifyFailure(body: any= {}) {
  return {
    ok: body?.ok ?? false,
...
export function formatOperatorSessionCaptureError(error: any) {
