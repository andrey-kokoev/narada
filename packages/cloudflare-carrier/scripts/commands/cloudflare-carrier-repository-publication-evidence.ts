...
export function parseRepositoryPublicationEvidenceArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function putCloudflareRepositoryPublicationEvidence(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_evidence_failed:${code}`);
    error.code = code;
...
export function summarizeRepositoryPublicationEvidence(body: any= {}, params: any= {}) {
...
export function formatRepositoryPublicationEvidenceText(result: any) {
...
function redactRepositoryPublicationEvidenceParams(params: any= {}) {
  return { ...params };
}
