...
const previousMaterializationEnvelopeIds = new Map<string, string>([
  [
...
function replace(value: any, replacements: any): any {
  if (typeof value=== 'string') {
    return [...replacements.entries()]
      .sort(([left]: any, [right]: any) => right.length - left.length)
      .reduce((output: any, [source, destination]: any) => output.replaceAll(source, destination), value);
  }
  if (Array.isArray(value)) return value.map((item: any) => replace(item, replacements));
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(Object.entries(value).map(([key, item]: any) => [key, replace(item, replacements)]));
}

function catalogAuthority(document: any) {
...
function productionEvidence(value: any): any {
  if (Array.isArray(value)) return value.map(productionEvidence);
  if (!value || typeof value !== 'object') return value;
  const output: any = Object.fromEntries(Object.entries(value).map(([key, item]: any) => [key, productionEvidence(item)]));
...
let seed: any = buildCanonicalCloudflareTestSeed({
...
seed.records = seed.records.filter(({ document }: any) => ![
...
seed.records = seed.records.filter((record: any, index: any, records: any) =>
  records.findIndex(({ document }: any) => document.id === record.document.id) === index
);
const servicePrincipalAuthority = {
  owner_kind: 'principal' as const,
...
for (const record of seed.records) {
  const document: any = record.document;
...
seed.records.forEach((record: any, index: any) => {
...
const materializations = seed.records
  ...
  .map((statementRecord: any) => {
    const statement = statementRecord.document;
    const payloadRecord = seed.records.find(({ record_id }: any) => record_id === statement.payload_ref);
