...
const intelligenceTargetSite = intelligenceSeed.records.find(({ document }) => (
...
const intelligencePrincipal = intelligenceSeed.records.find(({ document }) => (
...
assert.match(configText, /^main = "src\/worker-entry\.mjs"$/m);
...
assert.deepEqual(kvDurableEnv.AI.calls[0].request.tools.map((tool) => tool.function.name), ['cloudflare_carrier_kv_get']);
...
assert.deepEqual(kvWriteDurableEnv.AI.calls[0].request.tools.map((tool) => tool.function.name), ['cloudflare_carrier_kv_put']);
