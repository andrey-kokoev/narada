...
export function parseLocalIngressRequestArgs(argv: any = [], env: any = process.env, now: any = () => Date.now()) {
  const args = [...argv];
...
export async function createCloudflareLocalIngressRequest(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`local_ingress_request_failed:${code}`);
    error.code = code;
...
export function summarizeLocalIngressRequest(body: any= {}, params: any= {}) {
...
export function formatLocalIngressRequestText(result: any) {
  ...
  if (workerUrl && summary.site_id) {
    lines.push(
      `Site Read: pnpm --filter @narada-core/cloudflare-carrier product:site:read:text -- --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file>`,
    );
    lines.push(
      `Site Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:site:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file> --execute-site-next`,
    );
    lines.push(
      `Posture Coherence Review: pnpm --filter @narada-core/cloudflare-carrier product:posture:coherence:live:text -- --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file>`,
    );
    lines.push(
      `Durability Coherence Review: pnpm --filter @narada-core/cloudflare-carrier product:durability:coherence:live:text -- --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file>`,
    );
  }
  if (workerUrl && summary.site_id && summary.local_ingress_request_id) {
    lines.push(
      `Evidence Read: pnpm --filter @narada-core/cloudflare-carrier product:local-ingress:evidence:review:text -- --url ${workerUrl} --site ${summary.site_id} --local-ingress-request-id ${summary.local_ingress_request_id} --operator-session-file <operator-session-file>`,
    );
  }
  if (workerUrl && summary.site_id && summary.operation_id) {
    lines.push(
      `Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${workerUrl} --site ${summary.site_id} --operation-id ${summary.operation_id} --operator-session-file <operator-session-file>`,
    );
    lines.push(
      `Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --operation-id ${summary.operation_id} --operator-session-file <operator-session-file> --execute-operation-next`,
    );
  }
  if (workerUrl && summary.site_id && summary.task_id) {
    lines.push(
      `Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${summary.site_id} --task-id ${summary.task_id} --operator-session-file <operator-session-file>`,
    );
    lines.push(
      `Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --task-id ${summary.task_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`,
    );
  }
  return lines.join('\n') + '\n';
}

function redactLocalIngressRequestParams(params: any= {}) {
  return { ...params };
}
