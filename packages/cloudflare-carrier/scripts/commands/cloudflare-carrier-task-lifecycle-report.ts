...
export function parseTaskLifecycleReportArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function reportCloudflareTaskLifecycleTask(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`task_lifecycle_report_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleReport(body: any= {}, params: any= {}) {
...
export function formatTaskLifecycleReportText(result: any) {
...
function redactTaskLifecycleReportParams(params: any= {}) {
  return { ...params };
}
...
function optionAll(args: any, name: any) {
  const values = [];
...
function booleanFlag(args: any, name: any) {
...
function parseStringList(repeatedValues: any= [], singleValue: any= null) {
  const values = [];
  ...
  if (single) {
    ...
    if (Array.isArray(parsed)) {
      values.push(...parsed.map((entry: any) => String(entry).trim()).filter(Boolean));
    } else {
      values.push(...single.split(',').map((entry: any) => entry.trim()).filter(Boolean));
    }
  }
  return values;
}

function parseVerification(repeatedValues: any= [], jsonValue: any= null) {
  const values = [];
  for (const value of repeatedValues) {
