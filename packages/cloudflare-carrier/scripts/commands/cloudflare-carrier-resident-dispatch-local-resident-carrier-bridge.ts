...
export function parseResidentDispatchLocalResidentCarrierBridgeArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function runResidentDispatchLocalResidentCarrierBridge(config: any, fetchImpl: any= fetch) {
...
async function resolveBridgePutConfig(config: any, fetchImpl: any) {
...
export function summarizeResidentDispatchLocalResidentCarrierBridge(operation: any, body: any= {}, params: any= {}) {
...
export function formatResidentDispatchLocalResidentCarrierBridgeText(result: any= {}) {
  ...
  if (workerUrl && result.site_id && summary.cloudflare_carrier_session_id) {
    lines.push(`Session Evidence: pnpm --filter @narada-core/cloudflare-carrier product:session:evidence:text -- --url ${workerUrl} --site ${result.site_id} --carrier-session-id ${summary.cloudflare_carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${result.site_id} --carrier-session-id ${summary.cloudflare_carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${result.site_id} --carrier-session-id ${summary.cloudflare_carrier_session_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
  if (workerUrl && result.site_id && operationId) {
    lines.push(`Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${workerUrl} --site ${result.site_id} --operation-id ${operationId} --operator-session-file <operator-session-file>`);
    lines.push(`Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${workerUrl} --site ${result.site_id} --operation-id ${operationId} --operator-session-file <operator-session-file> --execute-operation-next`);
  }
  if (workerUrl && result.site_id && operationId && summary.fallback_evidence_id) {
    lines.push(`Resident Dispatch Windows Fallback Evidence Review: pnpm --filter @narada-core/cloudflare-carrier product:resident-dispatch:windows-fallback-evidence:review:text -- --url ${workerUrl} --site ${result.site_id} --operation-id ${operationId} --operator-session-file <operator-session-file>`);
  }
  return lines.join('\n');
}
