...
export function parseRepositoryPublicationAdmissionArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function classifyCloudflareRepositoryPublicationAdmission(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_admission_failed:${code}`);
    error.code = code;
...
export function summarizeRepositoryPublicationAdmission(body: any= {}, params: any= {}) {
...
export function formatRepositoryPublicationAdmissionText(result: any) {
...
function redactRepositoryPublicationAdmissionParams(params: any= {}) {
  return { ...params };
}
