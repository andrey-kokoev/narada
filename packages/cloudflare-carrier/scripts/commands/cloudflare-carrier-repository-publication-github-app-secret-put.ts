...
export function parseGithubAppSecretPutArgs(argv: any= [], env: any= process.env, roots: any= defaultRoots()) {
  const args = [...argv];
...
export function formatGithubAppSecretPutText(result: any) {
  const lines = [
...
export async function installGithubAppSecrets(config: any, secretWriter: any= putSecret) {
  const installed = [];
  const redactions = config.secrets.map((entry: any) => entry.value);
  ...
  for (const secret of config.secrets) {
    ...
    if (result.exitCode !== 0) {
      const error: any = new Error(`repository_publication_github_app_secret_put_failed:${secret.secretName}:${result.exitCode}`);
...
export function putSecret({ secretName, value, configPath, packageRoot: cwd = packageRoot, redactions = [] }: any) {
  return new Promise((resolvePromise: any) => {
    const wrangler = wranglerInvocation();
    const child: any = runGovernedCommand(wrangler.command, [...wrangler.args, 'secret', 'put', secretName, '--config', configPath], {
      cwd,
      windowsHide: true,
...
export function formatGithubAppSecretPutError(error: any) {
...
function readSecretFile(secretFilePath: any, root: any= repoRoot) {
...
function normalizePrivateKey(value: any) {
  return String(value ?? '').trim().replace(/\\n/g, '\n');
}

function redactSecrets(value: any, redactions: any) {
