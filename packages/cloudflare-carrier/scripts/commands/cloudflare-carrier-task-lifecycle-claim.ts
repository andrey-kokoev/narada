...
export function parseTaskLifecycleClaimArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function claimCloudflareTaskLifecycleTask(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`task_lifecycle_claim_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleClaim(body: any= {}, params: any= {}) {
...
export function formatTaskLifecycleClaimText(result: any) {
...
function redactTaskLifecycleClaimParams(params: any= {}) {
  return { ...params };
}
