...
export function parseOperationCreateArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function createCloudflareOperation(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`operation_create_request_failed:${code}`);
    error.code = code;
...
export function summarizeOperationCreate(body: any= {}) {
...
export function summarizeOperationCreateFailure(body: any= {}, params: any= {}) {
  return {
    ok: body?.ok ?? false,
...
export function formatOperationCreateText(result: any) {
