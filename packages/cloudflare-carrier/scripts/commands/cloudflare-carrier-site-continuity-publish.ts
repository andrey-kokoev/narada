...
export function parseSiteContinuityPublishArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function publishCloudflareSiteContinuityPacket(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`site_continuity_publish_request_failed:${code}`);
    error.code = code;
...
export function summarizeSiteContinuityPublish(body: any= {}, params: any= {}) {
...
export function formatSiteContinuityPublishText(result: any) {
