...
export function parseLocalIngressEvidenceArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function putCloudflareLocalIngressEvidence(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`local_ingress_evidence_failed:${code}`);
    error.code = code;
...
export function summarizeLocalIngressEvidence(body: any= {}, params: any= {}) {
...
export function formatLocalIngressEvidenceText(result: any) {
  ...
  const lines = [
    ...
    ...summary.changed_files.map((filePath: any) => `Changed File: ${filePath}`),
  ...
  if (workerUrl && summary.site_id && summary.local_ingress_request_id) {
    lines.push(`Request Review: pnpm --filter @narada-core/cloudflare-carrier product:local-ingress:request:review:text -- --url ${workerUrl} --site ${summary.site_id} --local-ingress-request-id ${summary.local_ingress_request_id} --operator-session-file <operator-session-file>`);
  }
  return `${lines.join('\n')}\n`;
}

function collectChangedFiles(args: any, env: any) {
  ...
  if (!envValue) return [];
  return envValue.split(',').map((value: any) => normalizeOptionalString(value)).filter(Boolean);
}

function collectRepeatedOption(args: any, name: any) {
  const values = [];
...
function redactLocalIngressEvidenceParams(params: any= {}) {
  return { ...params };
}
