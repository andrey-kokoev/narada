...
export function parseTaskLifecycleFinishArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function finishCloudflareTaskLifecycleTask(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`task_lifecycle_finish_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleFinish(body: any= {}, params: any= {}) {
...
export function formatTaskLifecycleFinishText(result: any) {
...
function redactTaskLifecycleFinishParams(params: any= {}) {
  return { ...params };
}
