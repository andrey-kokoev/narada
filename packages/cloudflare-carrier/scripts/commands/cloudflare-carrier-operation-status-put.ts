...
export function parseOperationStatusPutArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function putCloudflareOperationStatus(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`operation_status_put_request_failed:${code}`);
    error.code = code;
...
export function summarizeOperationStatusPut(body: any= {}, params: any= {}) {
...
export function formatOperationStatusPutText(result: any) {
...
export async function main(argv: any= process.argv.slice(2), env: any= process.env) {
  try {
    const config = parseOperationStatusPutArgs(argv, env);
