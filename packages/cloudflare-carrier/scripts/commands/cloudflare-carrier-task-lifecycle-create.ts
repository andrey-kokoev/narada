...
export function parseTaskLifecycleCreateArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function createCloudflareTaskLifecycleTask(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`task_lifecycle_create_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleCreate(body: any= {}, params: any= {}) {
...
export function formatTaskLifecycleCreateText(result: any) {
...
function redactTaskLifecycleCreateParams(params: any= {}) {
  return { ...params };
}
