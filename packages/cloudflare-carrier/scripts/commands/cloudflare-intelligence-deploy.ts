...
function manifestFile(directory: any, reference: any, label: any) {
  if (
    !reference
...
export function loadIntelligenceDeploymentBundle(manifestPath: any= DEFAULT_MANIFEST) {
...
export async function deployIntelligence(args: any= process.argv.slice(2), env: any= process.env, fetchImpl: any= fetch) {
  ...
  if (!response.ok || result?.ok !== true) {
    const error: any = new Error(`cloudflare_intelligence_deployment_refused:${response.status}:${result?.result?.error?.code ?? result?.code ?? 'unknown'}`);
    error.result = result;
    throw error;
  }
  return result;
