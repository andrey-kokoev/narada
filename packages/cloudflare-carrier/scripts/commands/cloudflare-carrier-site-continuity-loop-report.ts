...
export async function parseSiteContinuityLoopReportArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function loadSiteContinuityLoopReport(reportFile: any) {
...
export async function putSiteContinuityLoopReport(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`site_continuity_loop_report_request_failed:${code}`);
    error.code = code;
...
export function summarizeSiteContinuityLoopReport(body: any= {}, params: any= {}) {
...
export function formatSiteContinuityLoopReportText(result: any) {
