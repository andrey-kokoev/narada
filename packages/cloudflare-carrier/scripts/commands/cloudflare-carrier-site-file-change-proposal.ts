...
export function parseSiteFileChangeProposalArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export async function recordCloudflareSiteFileChangeProposal(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`site_file_change_proposal_request_failed:${code}`);
    error.code = code;
...
export function summarizeSiteFileChangeProposal(body: any= {}, params: any= {}) {
...
export function formatSiteFileChangeProposalText(result: any) {
  ...
  const lines = [
    ...
    ...summary.files.slice(0, 5).map((file: any) => `File: ${file.file_path} kind=${file.change_kind}${file.material_source_ref ? ` material=${file.material_source_ref}` : ''}`),
  ...
  if (workerUrl && summary.site_id && summary.proposal_id) {
    lines.push(`Proposal Review: pnpm --filter @narada-core/cloudflare-carrier product:site-file-change:proposal:review:text -- --url ${workerUrl} --site ${summary.site_id} --focus-ref ${summary.proposal_id} --operator-session-file <operator-session-file>`);
  }
  if (workerUrl && summary.site_id && summary.task_id) {
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${summary.site_id} --task-id ${summary.task_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --task-id ${summary.task_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
...
function parseProposalFiles(args: any, env: any) {
  ...
  if (filesJson) {
    ...
    return parsed.map((entry: any, index: any) => normalizeProposalFileEntry(entry, index));
  }
...
function normalizeProposalFileEntry(entry: any, index: any) {
...
function redactSiteFileChangeProposalParams(params: any= {}) {
  return { ...params };
}
