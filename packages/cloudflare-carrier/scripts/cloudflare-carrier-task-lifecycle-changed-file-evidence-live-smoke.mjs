...
if (!bearerToken) throw new Error('changed_file_evidence_live_smoke_requires_--token_or_--token-file_or_CLOUDFLARE_CARRIER_TOKEN');
...
const title = option('--title') ?? `Cloudflare changed-file evidence ${suffix}`;
...
const refusedEvidence = await postCarrier({
...
const admittedEvidence = await postCarrier({
  operation: 'task_lifecycle.changed_file_evidence.admit',
...
]).has(operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission), `unexpected changed-file evidence smoke Cloudflare write admission: ${operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission}`);
