...
} from '../workflows/cloudflare-site-continuity-scheduler.mjs';

const SCRIPT_PATH = fileURLToPath(new URL('../workflows/cloudflare-site-continuity-scheduler.mjs', import.meta.url));
...
test('site continuity scheduler install plan is bounded and secret-free', async () => {
  ...
  const syncEntrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.mjs');
  const scheduledTaskEntrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-scheduled-task.mjs');
...
test('site continuity scheduler text summary surfaces local and inbound continuity posture', async () => {
  ...
  try {
    ...
    assert.match(text, /Reconciliation Execution Record: pnpm --filter @narada2\/cloudflare-carrier exec node scripts\/workflows\/cloudflare-site-continuity-sync\.mjs reconciliation-execution-put --site site_missing --execution D:\\code\\narada\\\.narada\\site-continuity\\reconcile-last\.json --url https:\/\/carrier\.example --operator-session-file D:\\code\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
...
test('site continuity reconciliation plan reports configuration blockers instead of synthesizing commands', () => {
  const plan = buildSiteContinuityReconciliationPlan({
    ...
    syncEntryPoint: 'D:/repo/packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.mjs',
