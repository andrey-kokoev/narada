...
} from '../read-models/cloudflare-carrier-mailbox-draft-reply-proposal-read.ts';

test('parseMailboxDraftReplyProposalReadArgs extends operation.read params with mailbox limits', () => {
  const parsed: any = parseMailboxDraftReplyProposalReadArgs([
...
test('readMailboxDraftReplyProposal summarizes focused proposal and linked draft creates', async () => {
  ...
  const result: any = await readMailboxDraftReplyProposal({
...
test('formatMailboxDraftReplyProposalReadText omits synthetic operation ids from review ack', () => {
  const text = formatMailboxDraftReplyProposalReadText({
    worker_url: 'https://carrier.example',
    auth_source: 'operator-session-file',
    summary: {
