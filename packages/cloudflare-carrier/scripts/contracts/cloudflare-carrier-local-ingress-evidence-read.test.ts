...
} from '../read-models/cloudflare-carrier-local-ingress-evidence-read.ts';

test('parseLocalIngressEvidenceReadArgs reuses direct local ingress evidence list parsing', () => {
  const parsed: any = parseLocalIngressEvidenceReadArgs([
...
test('formatLocalIngressEvidenceReadText prints local ingress evidence summary', () => {
  ...
  assert.match(text, /Request Read: pnpm --filter @narada-core\/cloudflare-carrier product:local-ingress:request:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --local-ingress-request-id local_ingress_request_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation_site_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Operation Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:next:workflow:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation_site_alpha --operator-session-file <operator-session-file> --execute-operation-next/);
...
test('formatLocalIngressEvidenceReadText suppresses worker-scoped handoff without a real worker url', () => {
  const text = formatLocalIngressEvidenceReadText({
    auth_source: 'operator-session-file',
    summary: {
      site_id: 'site_alpha',
