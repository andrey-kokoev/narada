...
} from '../read-models/cloudflare-carrier-repository-publication-readiness.ts';
...
test('summaries and text output preserve readiness refusal evidence', () => {
  ...
  assert.match(text, /Repository Publication Provider Liveness: pnpm --filter @narada-core\/cloudflare-carrier product:repository-publication:provider-liveness:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file>/);
});

test('formatRepositoryPublicationReadinessText suppresses worker-scoped handoffs without worker url', () => {
  const text = formatRepositoryPublicationReadinessText({
    status: 'refused',
    auth_source: 'flag:--token',
    params: { site_id: 'site_alpha' },
  ...
  assert.doesNotMatch(text, /Repository Publication Provider Liveness:/);
});
