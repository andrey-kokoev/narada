...
} from '../read-models/cloudflare-carrier-mailbox-send-confirmation-read.ts';

test('parseMailboxSendConfirmationReadArgs reuses mailbox send confirmation list parsing', () => {
  const parsed: any = parseMailboxSendConfirmationReadArgs([
...
test('formatMailboxSendConfirmationReadText prints mailbox send confirmation summary', () => {
  ...
  assert.match(text, /Accepted Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-accepted:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref mailbox_send_accepted_alpha --operator-session-file <operator-session-file>/);
