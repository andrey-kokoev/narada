...
} from '../workflows/cloudflare-carrier-repository-publication-readback-live-smoke.ts';
...
test('runRepositoryPublicationReadbackLiveSmoke verifies cloudflare lane through readback surfaces', async () => {
  const calls: any[] = [];
  const result: any = await runRepositoryPublicationReadbackLiveSmoke({
...
function responseFor(operation: any, params: any) {
  switch (operation) {
