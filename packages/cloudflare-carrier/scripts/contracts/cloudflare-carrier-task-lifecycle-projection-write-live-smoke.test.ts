...
import {
  formatTaskLifecycleProjectionWriteLiveSmokeText,
  parseTaskLifecycleProjectionWriteLiveSmokeArgs,
  runTaskLifecycleProjectionWriteLiveSmoke,
} from '../workflows/cloudflare-carrier-task-lifecycle-projection-write-live-smoke.ts';

test('parseTaskLifecycleProjectionWriteLiveSmokeArgs supports operator session auth and text format', () => {
  const parsed = parseTaskLifecycleProjectionWriteLiveSmokeArgs([
...
test('formatTaskLifecycleProjectionWriteLiveSmokeText emits downstream reads', () => {
  const text = formatTaskLifecycleProjectionWriteLiveSmokeText({
    status: 'ok',
  ...
  assert.match(text, /Task Lifecycle Projection Write Smoke: ok/);
...
test('runTaskLifecycleProjectionWriteLiveSmoke returns summarized projection write state', async () => {
  const result = await runTaskLifecycleProjectionWriteLiveSmoke({
    ...
    fetchImpl: async (_url: any, init: any) => {
      ...
      if (body.operation=== 'task_lifecycle.projection_write.admit') {
        return responseJson(200, {
          ...
          task: { task_lifecycle_projection_write_count: 1 },
        });
      }
      if (body.operation=== 'operation.read') {
        return responseJson(200, {
          task_lifecycle_tasks: [{ task_id: 'task_alpha', status: 'finished', task_lifecycle_projection_write_count: 1 }],
          operation_product_surface: {
            task_lifecycle_projection_write_authority: 'cloudflare_task_lifecycle_d1',
            task_lifecycle_projection_write_count: 1,
            task_lifecycle_authority_partition: 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_cloudflare_remaining_windows',
            task_lifecycle_write_admission_posture: 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_admitted_remaining_writes_not_admitted',
            task_lifecycle_cloudflare_write_admission: 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_admitted',
  ...
  assert.equal(result.write_effect, 'task_lifecycle_projection_write');
});
