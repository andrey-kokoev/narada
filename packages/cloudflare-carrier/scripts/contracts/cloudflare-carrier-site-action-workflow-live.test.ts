...
} from '../workflows/cloudflare-carrier-site-action-workflow-live.ts';
...
test('runSiteActionWorkflowLive delegates operation focus to operation.next workflow', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-next-workflow-live.ts') {
        return JSON.stringify({ schema: 'narada.cloudflare_carrier.operation_next_workflow_live.v1', status: 'ok' });
      }
      if (scriptName=== 'cloudflare-carrier-product-read.ts' && invocations.length === 3) {
...
test('runSiteActionWorkflowLive retries operation focus once when site stays focused on operations', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-next-workflow-live.ts' && invocations.length === 2) {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-next-workflow-live.ts' && invocations.length === 4) {
...
test('runSiteActionWorkflowLive delegates continuity publish', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-continuity-publish.ts') {
...
test('runSiteActionWorkflowLive delegates next-site binding preparation', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-site-continuity-bindings.ts') {
...
test('runSiteActionWorkflowLive delegates membership creation to site membership put', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-membership-put.ts') {
  ...
  assert.equal(invocations[1][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-site-membership-put.ts');
  assert.deepEqual(invocations[1].slice(1, 9), [
    '--url', 'https://carrier.example',
    '--site', 'site_alpha',
...
test('runSiteActionWorkflowLive delegates site authority read', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-authority-read.ts') {
  ...
  assert.equal(invocations[1][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-site-authority-read.ts');
});
...
test('runSiteActionWorkflowLive delegates authority transfer continuation to authority transfer read', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-authority-transfer-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[1][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-authority-transfer-read.ts');
...
test('runSiteActionWorkflowLive delegates site scope read', async () => {
  ...
  const result = await runSiteActionWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-scope-read.ts') {
  ...
  assert.equal(invocations[1][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-site-scope-read.ts');
});
