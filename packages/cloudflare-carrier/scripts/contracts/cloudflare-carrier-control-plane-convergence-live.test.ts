...
} from '../workflows/cloudflare-carrier-control-plane-convergence-live.ts';
...
test('control plane convergence executes focused site pass then proves posture and durability', async () => {
  ...
  assert.ok(invocations.some((args: any) => args[0].split(/[\\/]/).pop() === 'cloudflare-carrier-site-next-workflow-live.ts'));
...
function classifyArgs(args: any) {
  ...
  if (scriptName === 'cloudflare-carrier-site-next-workflow-live.ts') return 'site.next';
