...
} from '../commands/cloudflare-carrier-repository-publication-admission.ts';
...
test('summaries and text output preserve refusal evidence', () => {
  ...
  assert.match(text, /Request Review: pnpm --filter @narada-core\/cloudflare-carrier product:repository-publication:request:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --repository-publication-request-id repository-publication-request-missing --operator-session-file <operator-session-file>/);
});
