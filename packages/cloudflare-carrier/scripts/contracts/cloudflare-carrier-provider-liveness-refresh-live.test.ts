...
} from '../workflows/cloudflare-carrier-provider-liveness-refresh.ts';
...
test('formatProviderLivenessRefreshText emits downstream reads', () => {
  ...
  assert.match(text, /Local Ingress Provider Liveness: pnpm --filter @narada-core\/cloudflare-carrier product:local-ingress:provider-liveness:text/);
  assert.match(text, /Repository Publication Provider Liveness: pnpm --filter @narada-core\/cloudflare-carrier product:repository-publication:provider-liveness:text/);
});
...
test('formatProviderLivenessRefreshText suppresses downstream reads without site id', () => {
  ...
  assert.doesNotMatch(text, /Local Ingress Provider Liveness: pnpm --filter @narada-core\/cloudflare-carrier product:local-ingress:provider-liveness:text/);
  assert.doesNotMatch(text, /Repository Publication Provider Liveness: pnpm --filter @narada-core\/cloudflare-carrier product:repository-publication:provider-liveness:text/);
});
