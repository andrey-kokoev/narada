...
} from '../commands/cloudflare-carrier-resident-dispatch-windows-fallback-execute.ts';
...
test('formatResidentDispatchWindowsFallbackExecuteText renders the execution summary', () => {
  ...
  assert.match(text, /Resident Dispatch Windows Fallback Evidence Review: pnpm --filter @narada-core\/cloudflare-carrier product:resident-dispatch:windows-fallback-evidence:review:text -- --url https:\/\/carrier\.example --site site_alpha --operation-id operation_alpha --operator-session-file <operator-session-file>/);
});
...
function response(status: any, body: any) {
  return {
