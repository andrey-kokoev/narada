...
} from '../workflows/cloudflare-carrier-mailbox-send-live-smoke.ts';
...
test('formatMailboxSendLiveSmokeText emits downstream reads', () => {
  ...
  assert.match(text, /Proposal Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:draft-reply-proposal:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref proposal_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Draft Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:outlook-draft:text/);
  assert.match(text, /Send Accepted Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-accepted:text/);
  assert.match(text, /Send Confirmation Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-confirmation:text/);
});

test('formatMailboxSendLiveSmokeText suppresses worker-scoped handoffs without worker url', () => {
  const text = formatMailboxSendLiveSmokeText({
    status: 'ok',
    worker_url: '',
    site_id: 'site_alpha',
...
function extractId(requestId: any, prefix: any, idPrefix: any) {
