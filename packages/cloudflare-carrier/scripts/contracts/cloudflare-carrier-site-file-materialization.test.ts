...
} from '../commands/cloudflare-carrier-site-file-materialization.ts';
...
test('formatSiteFileMaterializationText renders admitted and refused summaries without auth material', () => {
  ...
  assert.match(admitted, /Materialization Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file:materialization:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --site-file-materialization-id materialization-1 --operator-session-file <operator-session-file>/);
  assert.match(admitted, /Proposal Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file-change:proposal:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref proposal-9 --operator-session-file <operator-session-file>/);
