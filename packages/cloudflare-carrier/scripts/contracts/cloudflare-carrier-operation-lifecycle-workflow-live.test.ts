...
} from '../workflows/cloudflare-carrier-operation-lifecycle-workflow-live.ts';
...
test('runOperationLifecycleWorkflowLive orchestrates lifecycle create, continuation, resume, and close', async () => {
  ...
  const result = await runOperationLifecycleWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-create.ts') {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-status-put.ts') {
      ...
      if (scriptName=== 'cloudflare-carrier-continuation-resume.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts') {
        const readsSoFar = invocations.filter((call: any) => call[0].endsWith('cloudflare-carrier-product-read.ts')).length;
        if (readsSoFar=== 1) {
          return JSON.stringify({
  ...
  assert.equal(invocations[0][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-create.ts');
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-status-put.ts');
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[4][0].split(/[\\/]/).pop(), 'cloudflare-carrier-continuation-resume.ts');
  assert.equal(invocations[5][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[6][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-status-put.ts');
  assert.equal(invocations[7][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
...
test('formatOperationLifecycleWorkflowLiveText surfaces direct follow-on workflows and reads', () => {
  ...
  assert.match(text, /Create Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:session:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file> --execute-operation-session/);
  assert.match(text, /Continuation Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:continuation:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file> --execute-operation-continuation-resume/);
  assert.match(text, /Resume Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:continuity:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --expected-pre-action refresh_site_continuity_loop --operator-session-file <operator-session-file> --execute-operation-continuity/);
  assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Operation Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:next:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file> --execute-operation-next/);
  assert.match(text, /Site Read: pnpm --filter @narada-core\/cloudflare-carrier product:site:read:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file <operator-session-file>/);
  assert.match(text, /Site Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:site:next:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file <operator-session-file> --execute-site-next/);
  assert.match(text, /Session Evidence: pnpm --filter @narada-core\/cloudflare-carrier product:session:evidence:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --carrier-session-id carrier_session_operation_live_alpha_1 --operator-session-file <operator-session-file>/);
  assert.match(text, /Task Review: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:review:text -- --url https:\/\/carrier\.example --site site_live_smoke --carrier-session-id carrier_session_operation_live_alpha_1 --operator-session-file <operator-session-file>/);
  assert.match(text, /Task Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --carrier-session-id carrier_session_operation_live_alpha_1 --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next/);
});
