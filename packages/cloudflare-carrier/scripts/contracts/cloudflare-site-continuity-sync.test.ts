...
} from '../../../site-continuity/src/site-continuity.ts';

const SCRIPT_PATH = fileURLToPath(new URL('../workflows/cloudflare-site-continuity-sync.ts', import.meta.url));
const SCRIPT_CWD = fileURLToPath(new URL('../..', import.meta.url));

function runSync(args: any= [], { input = '', env = {} }: any= {}) {
  return new Promise((resolve: any, reject: any) => {
    const child: any = spawnTestChild(process.execPath, [SCRIPT_PATH, ...args], {
      cwd: SCRIPT_CWD,
      env: {
        ...process.env,
...
function startCarrierMock(handler: any) {
  const requests: any[] = [];
  ...
  return new Promise((resolve: any, reject: any) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      resolve({
        ...
        requests,
        requestHeaders,
        close: () => new Promise((closeResolve: any, closeReject: any) => server.close((error: any) => (error ? closeReject(error) : closeResolve()))),
      });
    });
  });
}
...
test('site continuity sync help describes supported transports', async () => {
  const result: any = await runSync(['help']);
...
test('site continuity sync accepts operator session auth for Cloudflare transport', async () => {
  ...
  const mock: any = await startCarrierMock((body: any) => {
  ...
  try {
    const result: any = await runSync(['pull-cloudflare', '--site', 'site_fixture', '--url', mock.url], {
      env: {
...
test('site continuity sync refuses malformed packets before push', async () => {
  const result: any = await runSync(['push-cloudflare'], {
...
test('site continuity sync refuses packet site mismatch before network', async () => {
  ...
  const pushResult: any = await runSync(['push-cloudflare', '--site', 'site_other'], {
    input: JSON.stringify({ packet }),
  });
  ...
  const syncResult: any = await runSync(['sync-once', '--site', 'site_other'], {
    input: JSON.stringify({ packet }),
  });
...
test('site continuity sync pulls Cloudflare exchange packet before local import', async () => {
  ...
  try {
    ...
    assert.ok(body.packet.decisions.some((decision: any) => (
...
test('site continuity sync cycle pushes local packet and returns Cloudflare packet', async () => {
  ...
  try {
    const result: any = await runSync(['sync-once', '--site', 'site_fixture', '--url', mock.url, '--out', outputPath, '--local-inbound-dir', inboundDirectory], {
...
test('site continuity read-cloudflare emits operator text handoff when using operator session file', async () => {
  ...
  try {
    ...
    const result: any = await runSync([
      'read-cloudflare',
      '--site', 'site_fixture',
      '--url', mock.url,
...
test('site continuity sync records reconciliation execution evidence in Cloudflare', async () => {
  ...
  try {
    const result: any = await runSync(['reconciliation-execution-put', '--site', 'site_fixture', '--execution', executionPath, '--url', mock.url, '--out', outputPath]);
...
test('site continuity sync refuses direct Cloudflare repository publication evidence before network', async () => {
  const result: any = await runSync(['repository-publication-evidence-put', '--site', 'site_fixture'], {
...
test('site continuity sync executes pending repository publication requests by returning local refusal evidence without implicit push', async () => {
  ...
  try {
    const result: any = await runSync(['repository-publication-execute-pending', '--site', 'site_fixture', '--repo', SCRIPT_CWD, '--url', mock.url]);
...
test('site continuity sync executes pending repository publication requests with explicit Windows push evidence', async () => {
  ...
  try {
    ...
    assert.match(String(remoteHead.stdout), new RegExp(`^${fixture.head}\\trefs/heads/main`));
  } finally {
    await mock.close();
...
async function createPublishableGitFixture() {
  ...
  const head = String((await git(repo, ['rev-parse', 'HEAD'])).stdout).trim();
  return { root, remote, repo, head };
}

async function git(cwd: any, args: any) {
