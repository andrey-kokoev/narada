...
const SCRIPT_PATH = fileURLToPath(new URL('../workflows/cloudflare-carrier-provider-liveness-refresh.ts', import.meta.url));
const SCRIPT_CWD = fileURLToPath(new URL('../..', import.meta.url));

function runRefresh(args: any= [], { env = {} }: any= {}) {
...
test('provider liveness refresh records local and repository heartbeats without claiming mutation authority', async () => {
  ...
  try {
    const result: any = await runRefresh(['--url', mock.url, '--site', 'site_fixture', '--local-root', localRoot]);
...
test('provider liveness refresh can refresh only local ingress', async () => {
  ...
  try {
    const result: any = await runRefresh(['--url', mock.url, '--site', 'site_fixture', '--local-root', localRoot, '--skip-repository']);
