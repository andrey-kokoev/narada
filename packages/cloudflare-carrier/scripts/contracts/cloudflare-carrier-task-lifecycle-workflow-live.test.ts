...
} from '../workflows/cloudflare-carrier-task-lifecycle-workflow-live.ts';
...
test('formatTaskLifecycleWorkflowLiveText renders direct task read', () => {
  ...
  assert.match(text, /Task Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url https:\/\/carrier\.example --site site_alpha --task-id task_alpha --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next/);
...
test('runTaskLifecycleWorkflowLive drives create claim report finish and read', async () => {
  ...
  const result = await runTaskLifecycleWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-create.ts') {
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-claim.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-report.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-finish.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[0][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-create.ts');
  assert.ok(invocations[0].includes('--admit-cloudflare-task-create'));
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-claim.ts');
  assert.ok(invocations[1].includes('--admit-cloudflare-task-claim'));
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-report.ts');
  assert.ok(invocations[2].includes('--admit-cloudflare-task-report'));
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-finish.ts');
  assert.ok(invocations[3].includes('--admit-cloudflare-task-finish'));
  assert.equal(invocations[4][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-read.ts');
});
