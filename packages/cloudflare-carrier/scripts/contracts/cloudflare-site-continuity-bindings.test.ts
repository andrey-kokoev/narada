...
} from '../workflows/cloudflare-site-continuity-bindings.ts';

function scheduledHealthSnapshot(overrides: any= {}) {
  return {
...
test('site continuity binding materializer writes registry from packet directory', async () => {
  ...
  assert.deepEqual(result.packet_reads.map((read: any) => read.site_id), ['site_alpha', 'site_beta']);
  const registry = JSON.parse(await readFile(registryPath, 'utf8'));
...
test('site continuity binding packet preparation writes packet consumable by materializer', async () => {
  ...
  const result: any = await runSiteContinuityBindingWorkflow(buildBindingMaterializationPlan({
    cwd: tmp,
    argv: [
      '--action', 'prepare-next-binding-packet',
      '--health', healthPath,
  ...
  assert.deepEqual(packet.binding.embodiments.map((embodiment: any) => embodiment.embodiment_kind).sort(), [
    'cloudflare_carrier',
    'local_windows',
  ]);
...
test('site continuity binding admission plans without execute and preserves existing bindings', async () => {
  ...
  assert.deepEqual(unchangedRegistry.bindings.map((binding: any) => binding.site_id), ['site_alpha']);
});
...
test('site continuity binding workflow lists operator-readable binding details', async () => {
  ...
  assert.deepEqual(result.sites[0].embodiments.map((embodiment: any) => embodiment.embodiment_kind), [
    'cloudflare_carrier',
    'local_windows',
  ]);
});
