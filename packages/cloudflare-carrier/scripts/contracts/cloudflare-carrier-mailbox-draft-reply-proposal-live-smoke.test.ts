...
} from '../workflows/cloudflare-carrier-mailbox-draft-reply-proposal-live-smoke.ts';
...
test('formatMailboxDraftReplyProposalLiveSmokeText emits downstream reads', () => {
  ...
  assert.match(text, /Proposal Review: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:draft-reply-proposal:text/);
  ...
  assert.match(text, /Accepted Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-accepted:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref accepted_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Confirmation Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-confirmation:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref confirmation_alpha --operator-session-file <operator-session-file>/);
...
function extractProposalId(requestId: any) {
  return requestId
