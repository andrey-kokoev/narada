...
} from '../read-models/cloudflare-carrier-operation-focus-review.ts';

test('parseOperationFocusReviewArgs builds acknowledge payload', () => {
  const parsed: any = parseOperationFocusReviewArgs([
...
test('formatOperationFocusReviewText emits direct follow-on operation commands', () => {
  ...
  assert.match(text, /Review List: pnpm --filter @narada-core\/cloudflare-carrier product:operation:focus-review:text -- --operation operation_focus_review\.list --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file>/);
