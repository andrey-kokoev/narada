...
} from '../workflows/cloudflare-carrier-operation-continuity-workflow-live.ts';
...
test('runOperationContinuityWorkflowLive orchestrates continuity refresh through existing live surfaces', async () => {
  ...
  const result = await runOperationContinuityWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-site-continuity-scheduler.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[1][0].split(/[\\\\/]/).pop(), 'cloudflare-site-continuity-scheduler.ts');
