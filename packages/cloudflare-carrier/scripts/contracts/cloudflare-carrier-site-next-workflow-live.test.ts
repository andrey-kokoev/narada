...
import {
  formatSiteNextWorkflowLiveText,
  parseSiteNextWorkflowLiveArgs,
  runSiteNextWorkflowLive,
} from '../workflows/cloudflare-carrier-site-next-workflow-live.ts';
...
test('parseSiteNextWorkflowLiveArgs supports text format', () => {
  const parsed = parseSiteNextWorkflowLiveArgs([
    '--url', 'https://carrier.example',
    '--focused-site-id', 'site_alpha',
    '--operator-session-cookie', 'operator-session-cookie',
...
test('formatSiteNextWorkflowLiveText renders direct reads', () => {
  const text = formatSiteNextWorkflowLiveText({
    status: 'ok',
    worker_url: 'https://carrier.example',
    delegated_route_action: 'focus_next_site',
    delegated_workflow: 'focus_next_operation',
    delegated_site_action: 'focus_next_operation',
    delegated_operation_id: 'operation_alpha',
    delegated_operation_action: 'refresh_site_continuity_loop',
    delegated_operation_reason: 'operation_lifecycle_continuity_loop_stale',
    delegated_operation_focus_kind: 'site_continuity_reconciliation_execution',
    delegated_operation_focus_ref: 'focus-ref',
  ...
  assert.match(text, /^Site Next Workflow: ok/m);
  ...
  assert.match(text, /Site Action Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:site:action:workflow:live:text -- --url https:\/\/carrier\.example --site site_alpha --operation-id operation_alpha --operator-session-file <operator-session-file> --execute-site-action/);
...
test('formatSiteNextWorkflowLiveText suppresses scoped handoffs without worker url', () => {
  const text = formatSiteNextWorkflowLiveText({
    status: 'ok',
    worker_url: null,
    delegated_route_action: 'focus_next_site',
  ...
  assert.doesNotMatch(text, /Site List: pnpm --filter @narada-core\/cloudflare-carrier product:site:list:text/);
  assert.doesNotMatch(text, /Site Read: pnpm --filter @narada-core\/cloudflare-carrier product:site:read:text/);
  assert.doesNotMatch(text, /Site Action Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:site:action:workflow:live:text/);
...
test('runSiteNextWorkflowLive delegates current site action when route monitors but overview candidate is actionable', async () => {
  ...
  const result = await runSiteNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-action-workflow-live.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-site-action-workflow-live.ts');
});
...
test('runSiteNextWorkflowLive delegates focus_next_site to site focus workflow', async () => {
  ...
  const result = await runSiteNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-focus-workflow-live.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts' && invocations.length === 4) {
        return JSON.stringify({
          schema: 'narada.cloudflare_carrier.product_read.v1',
          summary: {
            site_count: 2,
  ...
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-site-focus-workflow-live.ts');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-site-action-workflow-live.ts');
});
...
test('runSiteNextWorkflowLive retries site list once when top-level readback stays on the just-cleared site', async () => {
  ...
  const result = await runSiteNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      invocations.push(args);
      const scriptName = args[0].split(/[\\/]/).pop();
      if (scriptName=== 'cloudflare-carrier-product-read.ts' && invocations.length === 1) {
        return JSON.stringify({
          schema: 'narada.cloudflare_carrier.product_read.v1',
          summary: {
            site_count: 2,
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts' && invocations.length === 5) {
        return JSON.stringify({
          schema: 'narada.cloudflare_carrier.product_read.v1',
          summary: {
            site_count: 2,
...
test('runSiteNextWorkflowLive performs one delayed re-read when the immediate follow-up is still stale', async () => {
  ...
  const result = await runSiteNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts' && invocations.length === 6) {
        return JSON.stringify({
          schema: 'narada.cloudflare_carrier.product_read.v1',
          summary: {
            site_count: 2,
