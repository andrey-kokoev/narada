...
} from '../workflows/cloudflare-carrier-task-lifecycle-next-workflow-live.ts';
...
test('runTaskLifecycleNextWorkflowLive reports a claimed task', async () => {
  ...
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-report.ts');
});
...
test('runTaskLifecycleNextWorkflowLive finishes a reported task', async () => {
  ...
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-finish.ts');
});
