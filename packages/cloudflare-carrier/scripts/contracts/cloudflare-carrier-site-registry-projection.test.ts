...
import { materializeCloudflareSiteRegistryProjection } from '../read-models/cloudflare-carrier-site-registry-projection.ts';
...
test('site registry projection materializes active site list without credentials', async () => {
  ...
  try {
    const result: any = await materializeCloudflareSiteRegistryProjection({
    ...
    assert.deepEqual(result.projection.sites.map((site: any) => site.site_id), ['site_alpha', 'site_beta']);
    ...
    assert.deepEqual(written.sites.map((site: any) => site.site_id), ['site_alpha', 'site_beta']);
