...
} from '../read-models/cloudflare-carrier-site-file-materialization-read.ts';

test('parseSiteFileMaterializationReadArgs uses the site_file_materialization.list operation', () => {
  const parsed: any = parseSiteFileMaterializationReadArgs([
...
test('formatSiteFileMaterializationReadText prints the key review facts', () => {
  ...
  assert.match(text, /Materialization Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file:materialization:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --site-file-materialization-id materialization-1 --operator-session-file <operator-session-file>/);
  assert.match(text, /Proposal Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file-change:proposal:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref proposal-9 --operator-session-file <operator-session-file>/);
  ...
  assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation-9 --operator-session-file <operator-session-file>/);
  assert.match(text, /Operation Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:next:workflow:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation-9 --operator-session-file <operator-session-file> --execute-operation-next/);
});
...
test('formatSiteFileMaterializationReadText prints focused wording for direct historical reads', () => {
  ...
  assert.match(text, /Materialization Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file:materialization:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --site-file-materialization-id materialization-9 --operator-session-file <operator-session-file>/);
  assert.match(text, /Proposal Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file-change:proposal:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref proposal-9 --operator-session-file <operator-session-file>/);
  assert.match(text, /Site Read: pnpm --filter @narada-core\/cloudflare-carrier product:site:read:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Site Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:site:next:workflow:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file> --execute-site-next/);
  assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation-9 --operator-session-file <operator-session-file>/);
...
test('formatSiteFileMaterializationReadText suppresses worker-scoped handoffs without worker url', () => {
  const text = formatSiteFileMaterializationReadText({
    auth_source: 'operator-session-file',
    summary: {
      site_id: 'site_alpha',
