...
} from '../workflows/cloudflare-carrier-operation-next-workflow-live.ts';
...
test('runOperationNextWorkflowLive delegates local ingress request route to local ingress request read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts') {
  ...
  assert.ok(invocations.some((args: any) => /cloudflare-carrier-local-ingress-request-read\.ts$/.test(args[0])));
});
...
test('runOperationNextWorkflowLive delegates local ingress evidence route to local ingress evidence read', async () => {
  ...
  assert.ok(invocations.some((args: any) => /cloudflare-carrier-local-ingress-evidence-read\.ts$/.test(args[0])));
});

test('runOperationNextWorkflowLive delegates authority evidence routes to site authority read', async () => {
  const invocations: any[] = [];
  const result = await runOperationNextWorkflowLive({
    workerUrl: 'https://carrier.example.test',
    siteId: 'site_alpha',
  ...
  assert.equal(result.delegated_route_action, 'focus_authority_evidence');
  assert.ok(invocations.some((args: any) => /cloudflare-carrier-site-authority-read\.ts$/.test(args[0])));
});
...
test('runOperationNextWorkflowLive advances open-task routes through task lifecycle next workflow', async () => {
  ...
  assert.ok(invocations.some((args: any) => /cloudflare-carrier-task-lifecycle-next-workflow-live\.ts$/.test(args[0])));
  const workflowArgs = invocations.find((args: any) => /cloudflare-carrier-task-lifecycle-next-workflow-live\.ts$/.test(args[0]));
...
test('runOperationNextWorkflowLive selects the next operation and delegates to session workflow', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts') {
        const operation = args[args.indexOf('--operation') + 1];
        const readCount = invocations.filter((call: any) => call[0].endsWith('cloudflare-carrier-product-read.ts')).length;
        if (operation=== 'operation.list') {
          return JSON.stringify({
            schema: 'narada.cloudflare_carrier.product_read.v1',
            summary: {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-session-workflow-live.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[0][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-session-workflow-live.ts');
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
});
...
test('runOperationNextWorkflowLive forwards continuation inputs and executes continuation workflow', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-continuation-workflow-live.ts') {
        return JSON.stringify({
...
test('runOperationNextWorkflowLive delegates inspect_operation_evidence into focus review when a reviewable focus is pending', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-evidence-read.ts') {
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-operation-focus-review.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-evidence-read.ts');
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-focus-review.ts');
});
...
test('runOperationNextWorkflowLive delegates continuity reconciliation review route into focus review', async () => {
  ...
  assert.equal(result.delegated_result.schema, 'narada.cloudflare_carrier.operation_focus_review.v1');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-focus-review.ts');
...
test('runOperationNextWorkflowLive retargets to posture target when the initially selected operation only reports focused-operation posture', async () => {
  ...
  assert.equal(result.delegated_workflow, 'session');
  assert.equal(result.delegated_route_action, 'start_or_select_session');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[2][invocations[2].indexOf('--operation-id') + 1], 'operation_focus');
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-session-workflow-live.ts');
...
test('runOperationNextWorkflowLive does not retarget away from an already actionable focused operation', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-continuity-workflow-live.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-continuity-workflow-live.ts');
  assert.equal(invocations[2][invocations[2].indexOf('--operation-id') + 1], 'operation_focus');
});
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-product-read.ts') {
        ...
        if (operation=== 'operation.read' && operationId === 'operation_alpha') {
          if (invocations.filter((entry: any) => entry[0].split(/[\\/]/).pop() === 'cloudflare-carrier-product-read.ts' && entry.includes('--operation') && entry[entry.indexOf('--operation') + 1] === 'operation.read').length=== 1) {
            return JSON.stringify({
              schema: 'narada.cloudflare_carrier.product_read.v1',
              summary: {
                operation_id: 'operation_alpha',
...
test('runOperationNextWorkflowLive delegates focus_evidence to operation evidence read', async () => {
  const invocations: any[] = [];
  const result = await runOperationNextWorkflowLive({
    workerUrl: 'https://carrier.example',
    siteId: 'site_live_smoke',
  ...
  assert.equal(result.delegated_workflow, 'evidence');
  assert.equal(result.delegated_route_action, 'focus_evidence');
  assert.equal(result.delegated_result.schema, 'narada.cloudflare_carrier.operation_evidence_read.v1');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-evidence-read.ts');
});
...
test('runOperationNextWorkflowLive delegates focus_session_path_evidence to session evidence read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
        ...
        if (operation=== 'operation.list') {
          return JSON.stringify({
            ...
            summary: {
              next_operation_id: 'operation_alpha',
              route_next_action: 'focus_next_operation',
              next_action: 'focus_session_path_evidence',
            },
          });
        }
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-session-evidence-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-session-evidence-read.ts');
...
test('runOperationNextWorkflowLive advances focus_session_path_task through task lifecycle next workflow', async () => {
  const invocations: any[] = [];
  const result = await runOperationNextWorkflowLive({
    workerUrl: 'https://carrier.example',
    siteId: 'site_live_smoke',
    ...
    runNodeScript: async (args: any) => {
        ...
        if (operation=== 'operation.list') {
          return JSON.stringify({
            ...
            summary: {
              next_operation_id: 'operation_alpha',
              route_next_action: 'focus_next_operation',
              next_action: 'focus_session_path_task',
            },
          });
        }
        return JSON.stringify({
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-next-workflow-live.ts') {
        return JSON.stringify({
          schema: 'narada.cloudflare_carrier.task_lifecycle_next_workflow_live.v1',
          status: 'ok',
          selected_step: 'claim',
          read_after_next: { task_id: 'task_alpha', task_status: 'claimed' },
        });
      }
      throw new Error(`unexpected_script:${scriptName}`);
    },
  ...
  assert.equal(result.delegated_workflow, 'task_lifecycle_next');
  assert.equal(result.delegated_route_action, 'focus_session_path_task');
  assert.equal(result.delegated_result.schema, 'narada.cloudflare_carrier.task_lifecycle_next_workflow_live.v1');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-next-workflow-live.ts');
  assert.equal(invocations[2][invocations[2].indexOf('--carrier-session-id') + 1], 'session_alpha');
  assert.equal(invocations[2][invocations[2].indexOf('--agent-id') + 1], 'agent.operator');
        ...
        if (operation=== 'operation.list') {
          return JSON.stringify({
            ...
            summary: {
              next_operation_id: 'operation_alpha',
              route_next_action: 'focus_next_operation',
              next_action: 'focus_operation_path_task',
            },
          });
        }
        return JSON.stringify({
...
test('runOperationNextWorkflowLive delegates start_resident_dispatch to resident dispatch workflow', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-resident-dispatch-live-smoke.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-resident-dispatch-live-smoke.ts');
});
...
test('runOperationNextWorkflowLive delegates site file change proposal review route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-file-change-proposal-review.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-site-file-change-proposal-review.ts');
});
...
test('runOperationNextWorkflowLive delegates review_outlook_draft_create_evidence route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-mailbox-outlook-draft-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-mailbox-outlook-draft-read.ts');
});
...
test('runOperationNextWorkflowLive delegates review_site_file_materialization route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-site-file-materialization-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-site-file-materialization-read.ts');
});
...
test('runOperationNextWorkflowLive delegates Windows fallback resident dispatch request route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-resident-dispatch-windows-fallback-request.ts') {
...
test('runOperationNextWorkflowLive delegates Windows fallback resident dispatch evidence route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-resident-dispatch-windows-fallback-evidence-review.ts') {
        assert.deepEqual(args.slice(1), [
          '--url', 'https://carrier.example',
          '--site', 'site_live_smoke',
          '--operation-id', 'operation_alpha',
...
test('runOperationNextWorkflowLive delegates local ingress provider liveness review route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-local-ingress-provider-liveness-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-local-ingress-provider-liveness-read.ts');
});
...
test('runOperationNextWorkflowLive delegates create_task_from_directive_intent to directive intent task create', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-task-lifecycle-create-from-directive-intent.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-task-lifecycle-create-from-directive-intent.ts');
});
...
test('runOperationNextWorkflowLive delegates repository publication provider liveness review route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-repository-publication-provider-liveness-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-repository-publication-provider-liveness-read.ts');
});
...
test('runOperationNextWorkflowLive delegates mailbox send confirmation review route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-mailbox-send-confirmation-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-mailbox-send-confirmation-read.ts');
});
...
test('runOperationNextWorkflowLive delegates mailbox send acceptance review route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-mailbox-send-accepted-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-mailbox-send-accepted-read.ts');
});
...
test('runOperationNextWorkflowLive delegates local resident carrier bridge route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-resident-dispatch-local-resident-carrier-bridge.ts') {
        assert.deepEqual(args.slice(1), [
          '--url', 'https://carrier.example',
          '--site', 'site_live_smoke',
          '--operation-id', 'operation_alpha',
...
test('runOperationNextWorkflowLive delegates Windows fallback resident dispatch execution route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-resident-dispatch-windows-fallback-execute.ts') {
        assert.deepEqual(args.slice(1), [
          '--url', 'https://carrier.example',
          '--site', 'site_live_smoke',
          '--operation-id', 'operation_alpha',
...
test('runOperationNextWorkflowLive continues past evidence when the focus review is already acknowledged', async () => {
  ...
  assert.equal(result.read_after_next.workflow_next_action, 'monitor_operation');
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-evidence-read.ts');
  assert.equal(invocations[4][0].split(/[\\/]/).pop(), 'cloudflare-carrier-operation-continuity-workflow-live.ts');
});
...
test('runOperationNextWorkflowLive prefers direct workflow when operation read has a fresher actionable route than stale evidence posture', async () => {
  ...
  assert.equal(result.delegated_result.schema, 'narada.cloudflare_carrier.operation_continuity_workflow_live.v1');
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-continuity-workflow-live.ts');
  assert.equal(invocations.some((call: any) => call[0].split(/[\\\\/]/).pop() === 'cloudflare-carrier-operation-evidence-read.ts'), false);
});
...
test('runOperationNextWorkflowLive continues continuity into immediate continuity reconciliation focus review', async () => {
  ...
  assert.equal(result.read_after_next.workflow_next_action, 'monitor_operation');
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-continuity-workflow-live.ts');
  assert.equal(invocations[4][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-focus-review.ts');
...
test('runOperationNextWorkflowLive delegates mailbox draft reply proposal review route to mailbox proposal read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-mailbox-draft-reply-proposal-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-mailbox-draft-reply-proposal-read.ts');
});
...
test('runOperationNextWorkflowLive delegates repository publication request review route to repository publication review read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-repository-publication-request-review.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-repository-publication-request-review.ts');
});
...
test('runOperationNextWorkflowLive delegates directive delivery review route to directive delivery review read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-directive-delivery-review.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-directive-delivery-review.ts');
});
...
test('runOperationNextWorkflowLive delegates webhook delay shadow read focus route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-webhook-delay-shadow-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-webhook-delay-shadow-read.ts');
});
...
test('runOperationNextWorkflowLive delegates repository publication evidence route to repository publication evidence read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-repository-publication-read.ts') {
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-repository-publication-read.ts');
});
...
test('runOperationNextWorkflowLive delegates recovery review route to operation recovery read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-recovery-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-recovery-read.ts');
});
...
test('runOperationNextWorkflowLive delegates persistence review route to operation persistence read', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-persistence-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-persistence-read.ts');
});
...
test('runOperationNextWorkflowLive advances recovery review into local resident carrier bridge when recovery names that admitted next action', async () => {
  ...
  assert.equal(result.delegated_result.schema, 'narada.cloudflare_carrier.local_resident_carrier_bridge.v1');
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-recovery-read.ts');
  assert.equal(invocations[3][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-evidence-read.ts');
  assert.equal(invocations[4][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-resident-dispatch-local-resident-carrier-bridge.ts');
});
test('runOperationNextWorkflowLive delegates read_operation_scope route', async () => {
  ...
  const result = await runOperationNextWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-operation-scope-read.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[2][0].split(/[\\\\/]/).pop(), 'cloudflare-carrier-operation-scope-read.ts');
});
