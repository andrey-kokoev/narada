...
} from '../read-models/cloudflare-carrier-operation-persistence-read.ts';
...
test('formatOperationPersistenceReadText includes recovery follow-on command', () => {
  ...
  assert.match(text, /Recovery Read: pnpm --filter @narada-core\/cloudflare-carrier product:operation:recovery:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation_alpha --operator-session-file <operator-session-file>/);
});
...
test('formatOperationPersistenceReadText suppresses operator handoff without a real worker url', () => {
  const text = formatOperationPersistenceReadText({
    auth_source: 'operator-session-file',
    summary: {
      site_id: 'site_alpha',
