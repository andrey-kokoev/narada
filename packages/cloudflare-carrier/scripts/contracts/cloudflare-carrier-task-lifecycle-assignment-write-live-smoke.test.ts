...
import {
  formatTaskLifecycleAssignmentWriteLiveSmokeText,
  parseTaskLifecycleAssignmentWriteLiveSmokeArgs,
  runTaskLifecycleAssignmentWriteLiveSmoke,
} from '../workflows/cloudflare-carrier-task-lifecycle-assignment-write-live-smoke.ts';

test('parseTaskLifecycleAssignmentWriteLiveSmokeArgs supports operator session auth and text format', () => {
  const parsed = parseTaskLifecycleAssignmentWriteLiveSmokeArgs([
...
test('formatTaskLifecycleAssignmentWriteLiveSmokeText emits downstream reads', () => {
  const text = formatTaskLifecycleAssignmentWriteLiveSmokeText({
    status: 'ok',
  ...
  assert.match(text, /Task Lifecycle Assignment Write Smoke: ok/);
...
test('runTaskLifecycleAssignmentWriteLiveSmoke returns summarized assignment write state', async () => {
  const result = await runTaskLifecycleAssignmentWriteLiveSmoke({
    workerUrl: 'https://carrier.example.test',
    auth: { kind: 'operator_session', value: 'operator-cookie', source: 'operator-session-cookie' },
    siteId: 'site_alpha',
    operationId: 'operation_alpha',
    agentId: 'agent_alpha',
    filePath: 'packages/cloudflare-carrier/src/cloudflare-worker.ts',
  }, {
    fetchImpl: async (_url: any, init: any) => {
      ...
      if (body.operation=== 'task_lifecycle.source_state_write.admit') {
        return responseJson(200, { write_effect: 'task_lifecycle_source_state_write' });
      }
      ...
      if (body.operation=== 'task_lifecycle.assignment_write.admit') {
        return responseJson(200, {
          ...
          task: { task_lifecycle_assignment_write_count: 1 },
        });
      }
      if (body.operation=== 'operation.read') {
        return responseJson(200, {
          task_lifecycle_tasks: [{ task_id: 'task_alpha', status: 'finished', task_lifecycle_assignment_write_count: 1 }],
          operation_product_surface: {
            task_lifecycle_assignment_authority: 'cloudflare_task_lifecycle_d1',
            task_lifecycle_assignment_write_count: 1,
            task_lifecycle_roster_mutation_admission: 'not_admitted',
            task_lifecycle_role_resolution_authority_admission: 'not_admitted',
            task_lifecycle_authority_partition: 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_cloudflare_remaining_windows_effects',
            task_lifecycle_write_admission_posture: 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted_remaining_external_effects_not_admitted',
            task_lifecycle_cloudflare_write_admission: 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted',
            task_lifecycle_write_admission_count: 7,
          },
        });
      }
  ...
  assert.equal(result.write_effect, 'task_lifecycle_assignment_write');
});
