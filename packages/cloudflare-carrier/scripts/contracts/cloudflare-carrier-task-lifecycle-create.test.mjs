...
} from '../commands/cloudflare-carrier-task-lifecycle-create.mjs';
...
test('formatTaskLifecycleCreateText renders admitted and refused summaries without auth material', () => {
  ...
  assert.match(admitted, /Posture Coherence Review: pnpm --filter @narada2\/cloudflare-carrier product:posture:coherence:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file>/);
  assert.match(admitted, /Durability Coherence Review: pnpm --filter @narada2\/cloudflare-carrier product:durability:coherence:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operator-session-file <operator-session-file>/);
