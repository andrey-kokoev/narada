...
} from '../read-models/cloudflare-carrier-local-ingress-request-read.ts';

test('parseLocalIngressRequestReadArgs reuses direct local ingress request list parsing', () => {
  const parsed: any = parseLocalIngressRequestReadArgs([
...
test('formatLocalIngressRequestReadText prints local ingress request summary', () => {
  ...
  assert.match(text, /Evidence Read: pnpm --filter @narada-core\/cloudflare-carrier product:local-ingress:evidence:review:text -- --url https:\/\/carrier\.example\.test --site site_alpha --local-ingress-evidence-id local_ingress_evidence_alpha --operator-session-file <operator-session-file>/);
  ...
  assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation_site_read --operator-session-file <operator-session-file>/);
  assert.match(text, /Operation Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:next:workflow:live:text -- --url https:\/\/carrier\.example\.test --site site_alpha --operation-id operation_site_read --operator-session-file <operator-session-file> --execute-operation-next/);
});
