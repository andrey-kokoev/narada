...
} from '../workflows/cloudflare-carrier-repository-publication-cloudflare-workflow-live.ts';
...
test('runRepositoryPublicationCloudflareWorkflowLive runs execution then readback with shared ids', async () => {
  ...
  const result = await runRepositoryPublicationCloudflareWorkflowLive({
    ...
    runNodeScript: async (args: any) => {
      ...
      if (scriptName=== 'cloudflare-carrier-repository-publication-cloudflare-github-live-smoke.ts') {
      ...
      if (scriptName=== 'cloudflare-carrier-repository-publication-readback-live-smoke.ts') {
        return JSON.stringify({
  ...
  assert.equal(invocations[0][0].split(/[\\/]/).pop(), 'cloudflare-carrier-repository-publication-cloudflare-github-live-smoke.ts');
  assert.equal(invocations[1][0].split(/[\\/]/).pop(), 'cloudflare-carrier-repository-publication-readback-live-smoke.ts');
