...
} from '../workflows/cloudflare-site-continuity-scheduler.ts';

const SCRIPT_PATH = fileURLToPath(new URL('../workflows/cloudflare-site-continuity-scheduler.ts', import.meta.url));

async function writeInboundPacketArtifact(artifactDirectory: any, siteId: any, {
...
async function writePlannedSiteContinuityHiddenWrapper(options: any) {
...
test('site continuity scheduler install plan is bounded and secret-free', async () => {
  ...
  const syncEntrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.ts');
  const scheduledTaskEntrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-scheduled-task.ts');
  ...
  await mkdir(join(root, 'packages/cloudflare-carrier/scripts/workflows'), { recursive: true });
  ...
  try {
    ...
    assert.match(plan.direct_task_command, /cloudflare-site-continuity-scheduled-task\.ts/);
...
test('site continuity scheduler product read auth prefers explicit operator session file over bearer env', async () => {
  ...
  try {
    const auth: any = resolveSiteContinuitySchedulerProductReadAuth({
...
test('site continuity scheduler parseArgs accepts --url as projection worker alias', () => {
  ...
  assert.equal(args.operatorSessionFile, 'C:\\workspace\\narada\\.narada\\auth\\cloudflare-operator-session.json');
});
...
test('site continuity scheduled health reports binding preparation readiness without synthesizing refs', () => {
  const missingRefs = summarizeCloudflareProductBindingPreparation({
    configuredSites: {
      site_records: [{ site_id: 'site_alpha', local_site_ref: 'file:///C:/workspace/narada', cloudflare_site_ref: 'cloudflare://site-alpha' }],
  ...
  const ready = summarizeCloudflareProductBindingPreparation({
    configuredSites: {
      site_records: [{ site_id: 'site_beta', local_site_ref: 'file:///C:/workspace/narada', cloudflare_site_ref: 'cloudflare://site-beta' }],
    },
    cloudflareProductPosture: { summary: { next_site_id: 'site_beta' } },
    cloudflareProductBindingAlignment: {
      state: 'unbound_remote_next_site',
  ...
  assert.equal(ready.local_site_ref, 'file:///C:/workspace/narada');
  ...
  assert.equal(ready.prepare_command, 'pnpm --filter @narada-core/cloudflare-carrier continuity:bindings:prepare-next -- --local-site-ref file:///C:/workspace/narada --cloudflare-site-ref cloudflare://site-beta');
});
...
test('site continuity scheduler text emits concrete binding preparation handoff when refs are ready', () => {
  const text = formatSiteContinuitySchedulerResultForText({
    ...
    cloudflare_product_binding_preparation: {
      ...
      target_site_id: 'site_beta',
      required_inputs: [],
      local_site_ref: 'file:///C:/workspace/narada',
      cloudflare_site_ref: 'cloudflare://site-beta',
      prepare_command: 'pnpm --filter @narada-core/cloudflare-carrier continuity:bindings:prepare-next -- --local-site-ref file:///C:/workspace/narada --cloudflare-site-ref cloudflare://site-beta',
    },
  });
...
test('site continuity reconciliation plan resolves one packet per configured site from packet directory', async () => {
  ...
  try {
    const plan: any = buildSiteContinuitySchedulerPlan({
      action: 'reconcile',
      repoRoot: root,
      syncEntrypoint,
      packetDirectory,
    ...
    assert.deepEqual(plan.reconciliation_plan.selected_sites.map((site: any) => [
      site.site_id,
      site.packet_path,
...
test('site continuity scheduler live install executes bounded schtasks command', async () => {
  ...
  try {
    ...
    const result = await runSiteContinuitySchedulerActionWithOptionalRefresh({
      ...
      execFileImpl: async (command: any, args: any, options: any) => {
    ...
    assert.match(result.direct_task_command, /cloudflare-site-continuity-scheduled-task\.ts/);
...
test('site continuity scheduler reads local inbound packet inventory per configured site', async () => {
  ...
  try {
    ...
    assert.deepEqual(inventory.configured_site_inbound_statuses.map((site: any) => [site.site_id, site.status, site.reason]), [
...
test('site continuity scheduler text summary surfaces local and inbound continuity posture', async () => {
  ...
  try {
    ...
    plan.last_sync = {
      ...
      continuity_loop_report_artifact_path: 'C:\\workspace\\narada\\.narada\\site-continuity\\site_missing-loop-report.json',
    };
    plan.last_reconciliation_execution = {
    ...
    assert.match(text, /Site List: pnpm --filter @narada-core\/cloudflare-carrier product:site:list:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
    assert.match(text, /Site Read: pnpm --filter @narada-core\/cloudflare-carrier product:site:read:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing/);
    assert.match(text, /Posture Coherence Review: pnpm --filter @narada-core\/cloudflare-carrier product:posture:coherence:live:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing/);
    assert.match(text, /Durability Coherence Review: pnpm --filter @narada-core\/cloudflare-carrier product:durability:coherence:live:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing/);
    assert.match(text, /Operation Review: pnpm --filter @narada-core\/cloudflare-carrier product:operation:read:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing --operation-id operation_site_missing_control/);
    assert.match(text, /Operation Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:next:workflow:live:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing --operation-id operation_site_missing_control --execute-operation-next/);
    assert.match(text, /Site Next Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:site:next:workflow:live:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing --execute-site-next/);
    assert.match(text, /Loop Report: site-continuity-loop:site_missing:2026-06-11T10:30:00\.000Z/);
    assert.match(text, /Loop Report Artifact Path: C:\\workspace\\narada\\\.narada\\site-continuity\\site_missing-loop-report\.json/);
    assert.match(text, /Loop Report Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-continuity:loop-report:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json --site site_missing --report-file C:\\workspace\\narada\\\.narada\\site-continuity\\site_missing-loop-report\.json/);
    assert.match(text, /Reconciliation Execution: status=completed plan=ready selected=1 completed=1 failed=0/);
    assert.match(text, /Reconciliation Artifact Path: C:\\workspace\\narada\\\.narada\\site-continuity\\reconcile-last\.json/);
    assert.match(text, /Reconciliation Execution Record: pnpm --filter @narada-core\/cloudflare-carrier exec node scripts\/workflows\/cloudflare-site-continuity-sync\.ts reconciliation-execution-put --site site_missing --execution C:\\workspace\\narada\\\.narada\\site-continuity\\reconcile-last\.json --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
...
test('site continuity scheduler live health passes with synced artifacts and healthy scheduler readback', async () => {
  ...
  try {
    const result = await runSiteContinuitySchedulerActionWithOptionalRefresh({
      ...
      productReadSurface: async ({ operation, auth, params }: any) => {
        assert.equal(auth.kind, 'operator_session');
        assert.equal(auth.source, 'operator-session-file');
        assert.equal(auth.value, 'session-fixture');
...
test('site continuity scheduler status-all distinguishes configured sites missing local artifacts', async () => {
  ...
  try {
    ...
    assert.deepEqual(configured.site_records.find((site: any) => site.site_id === 'site_registry'), {
    ...
    assert.deepEqual(plan.local_sync_artifacts.configured_site_sync_statuses.map((site: any) => [site.site_id, site.status, site.reason]), [
    ...
    assert.deepEqual(plan.local_sync_artifacts.configured_site_sync_statuses.find((site: any) => site.site_id === 'site_registry'), {
      site_id: 'site_registry',
      display_name: 'Registry Site',
      site_ref: null,
      site_status: 'active',
    ...
    assert.deepEqual(plan.local_inbound_packets.configured_site_inbound_statuses.map((site: any) => [site.site_id, site.status, site.reason]), [
      ['site_missing', 'needs_attention', 'configured_site_inbound_packet_missing'],
...
test('site continuity scheduler uses binding registry as managed site set before discovery registry', async () => {
  ...
  try {
    ...
    assert.deepEqual(plan.local_sync_artifacts.configured_site_sync_statuses.map((site: any) => site.site_id), ['site_bound']);
...
test('site continuity scheduler status-all inventories local sync artifacts', async () => {
  ...
  try {
    const inventory: any = readLocalSyncArtifactInventory(artifactDirectory, {
    ...
    assert.deepEqual(inventory.artifacts.map((artifact: any) => artifact.site_id), ['site_alpha', 'site_beta']);
    assert.deepEqual(inventory.artifacts.map((artifact: any) => artifact.status), ['synced', 'needs_attention']);
...
test('site continuity scheduler selects loop-stale sites before broad sync artifact expiry', async () => {
  ...
  try {
    ...
    assert.deepEqual(plan.selected_sites.map((site: any) => [site.site_id, site.reason]), [
...
test('site continuity scheduler reconcile plans stale and missing configured site sync commands only', async () => {
  ...
  try {
    ...
    assert.deepEqual(plan.reconciliation_plan.selected_sites.map((site: any) => [site.site_id, site.reason]), [
      ['site_missing', 'configured_site_sync_artifact_missing'],
      ['site_stale', 'configured_site_continuity_loop_report_stale'],
    ]);
    assert.equal(plan.reconciliation_plan.selected_sites.some((site: any) => site.site_id === 'site_synced'), false);
    const staleSite = plan.reconciliation_plan.selected_sites.find((site: any) => site.site_id === 'site_stale');
    const missingSite = plan.reconciliation_plan.selected_sites.find((site: any) => site.site_id === 'site_missing');
    assert.equal(staleSite.command_status, 'ready');
    assert.deepEqual(staleSite.command_blockers, []);
    assert.match(staleSite.sync_command, /cloudflare-site-continuity-sync\.ts/);
...
test('site continuity reconciliation plan reports configuration blockers instead of synthesizing commands', () => {
  const plan = buildSiteContinuityReconciliationPlan({
    ...
    syncEntryPoint: 'D:/repo/packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.ts',
...
test('site continuity reconciliation plan refuses packet site mismatch before sync command synthesis', async () => {
  ...
  try {
    ...
    const packetSite = plan.reconciliation_plan.selected_sites.find((site: any) => site.site_id === 'site_packet');
    const otherSite = plan.reconciliation_plan.selected_sites.find((site: any) => site.site_id === 'site_other');
...
test('site continuity reconcile-execute defaults to dry-run refusal without executing sync', async () => {
  ...
  try {
    ...
    const result = await runSiteContinuitySchedulerActionWithOptionalRefresh({
      ...
      execFileImpl: async (command: any) => { if (command === process.execPath) executedSync = true; },
    });
...
test('site continuity scheduled health reports remote product next-site outside local binding set', async () => {
  ...
  try {
    ...
    const result = await runSiteContinuitySchedulerActionWithOptionalRefresh({
      ...
      execFileImpl: async (command: any) => {
        if (command=== 'schtasks') {
          return {
            stdout: [
              'TaskName: \\Narada\\CloudflareSiteContinuitySync',
      ...
      productReadSurface: async (config: any) => {
...
test('site continuity reconcile-execute uses per-site packet paths from packet directory', async () => {
  ...
  try {
    ...
    const syncCalls = calls.filter((call: any) => call.command === process.execPath && call.args[1] === 'sync-once');
    assert.equal(result.status, 'completed');
    ...
    assert.deepEqual(syncCalls.map((call: any) => [
...
test('site continuity reconcile-execute runs ready sites through sync-once argv and records artifacts', async () => {
  ...
  try {
    ...
    const statusPlan: any = buildSiteContinuitySchedulerPlan({
      action: 'status-all',
      repoRoot: root,
...
test('site continuity scheduler can refresh site registry projection before status-all', async () => {
  ...
  try {
    const plan: any = await buildSiteContinuitySchedulerPlanWithOptionalRefresh({
      action: 'status-all',
      repoRoot: root,
      artifactDirectory,
      siteRegistryProjectionPath,
      ...
      env: {},
      materializeSiteRegistryProjection: async (inputs: any) => {
    ...
    assert.deepEqual(plan.local_sync_artifacts.configured_site_sync_statuses.map((site: any) => [site.site_id, site.reason]), [
...
test('site continuity scheduler CLI emits operator text status when requested', async () => {
  ...
  assert.match(String(result.stdout), /Site List: pnpm --filter @narada-core\/cloudflare-carrier product:site:list:text -- --url https:\/\/carrier\.example --operator-session-file C:\\workspace\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
...
test('site continuity scheduler read-last summarizes local sync artifact', async () => {
  ...
  await writeFile(reconciliationExecutionOutputPath, `${JSON.stringify({
    schema: 'narada.cloudflare_carrier.site_continuity_reconciliation_execution.v1',
    status: 'completed',
    generated_at: '2026-06-11T09:00:30.000Z',
    persisted_at: '2026-06-11T09:00:31.000Z',
    reconciliation_plan_status: 'ready',
    selected_site_count: 1,
    executed_site_count: 1,
    completed_site_count: 1,
    ...
    results: [
      { site_id: 'site_fixture', status: 'completed' },
    ],
  }, null, 2)}\n`, 'utf8');
  try {
    ...
    assert.equal(plan.last_reconciliation_execution.status, 'completed');
    assert.deepEqual(plan.last_reconciliation_execution.site_ids, ['site_fixture']);
    const text = formatSiteContinuitySchedulerResultForText(plan);
