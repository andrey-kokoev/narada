...
} from '../commands/cloudflare-carrier-resident-dispatch-local-resident-carrier-bridge.ts';

test('parseResidentDispatchLocalResidentCarrierBridgeArgs builds put payload with defaults', () => {
  const parsed: any = parseResidentDispatchLocalResidentCarrierBridgeArgs([
...
test('formatResidentDispatchLocalResidentCarrierBridgeText prints key posture', () => {
  ...
  assert.match(text, /Posture Coherence Review: pnpm --filter @narada-core\/cloudflare-carrier product:posture:coherence:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file <operator-session-file>/);
  assert.match(text, /Durability Coherence Review: pnpm --filter @narada-core\/cloudflare-carrier product:durability:coherence:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file <operator-session-file>/);
  assert.match(text, /Session Evidence: pnpm --filter @narada-core\/cloudflare-carrier product:session:evidence:text -- --url https:\/\/carrier\.example --site site_live_smoke --carrier-session-id cloudflare-bridged:site_live_smoke:operation_live_alpha:1 --operator-session-file <operator-session-file>/);
  assert.match(text, /Task Review: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:review:text -- --url https:\/\/carrier\.example --site site_live_smoke --carrier-session-id cloudflare-bridged:site_live_smoke:operation_live_alpha:1 --operator-session-file <operator-session-file>/);
  ...
  assert.match(text, /Resident Dispatch Windows Fallback Evidence Review: pnpm --filter @narada-core\/cloudflare-carrier product:resident-dispatch:windows-fallback-evidence:review:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file>/);
});

test('formatResidentDispatchLocalResidentCarrierBridgeText suppresses worker-scoped handoffs without worker url', () => {
  const text = formatResidentDispatchLocalResidentCarrierBridgeText({
    site_id: 'site_live_smoke',
    operation_id: 'operation_live_alpha',
    status: 'ok',
