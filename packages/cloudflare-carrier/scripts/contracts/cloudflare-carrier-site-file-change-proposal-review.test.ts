...
} from '../read-models/cloudflare-carrier-site-file-change-proposal-review.ts';

test('parseSiteFileChangeProposalReviewArgs extends operation.read params with site file limits', () => {
  const parsed: any = parseSiteFileChangeProposalReviewArgs([
...
test('readSiteFileChangeProposalReview summarizes focused proposal and linked materialization', async () => {
  ...
  const result: any = await readSiteFileChangeProposalReview({
...
test('formatSiteFileChangeProposalReviewText omits synthetic operation ids from review ack', () => {
  ...
  assert.match(text, /Review Ack: pnpm --filter @narada-core\/cloudflare-carrier product:operation:focus-review:text -- --url https:\/\/carrier\.example --site site_narada_cloudflare --focus-kind site_file_change_proposal --focus-ref site_file_change_proposal_live_1 --operator-session-file <operator-session-file>/);
