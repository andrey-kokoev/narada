...
} from '../read-models/cloudflare-carrier-resident-dispatch-windows-fallback-evidence-review.ts';

test('parseResidentDispatchWindowsFallbackEvidenceReviewArgs adds focused evidence limits', () => {
  const parsed: any = parseResidentDispatchWindowsFallbackEvidenceReviewArgs([
...
test('readResidentDispatchWindowsFallbackEvidenceReview summarizes focused evidence and review state', async () => {
  const result: any = await readResidentDispatchWindowsFallbackEvidenceReview({
...
test('readResidentDispatchWindowsFallbackEvidenceReview supports direct focused review without operation read', async () => {
  ...
  assert.deepEqual(calls.map((entry: any) => entry.operation), [
...
test('formatResidentDispatchWindowsFallbackEvidenceReviewText prints review ack command', () => {
  ...
  assert.match(text, /Session Evidence: pnpm --filter @narada-core\/cloudflare-carrier product:session:evidence:text -- --url https:\/\/carrier\.example --site site_alpha --carrier-session-id carrier_session_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Task Review: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:review:text -- --url https:\/\/carrier\.example --site site_alpha --carrier-session-id carrier_session_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Task Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url https:\/\/carrier\.example --site site_alpha --carrier-session-id carrier_session_alpha --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next/);
...
test('formatResidentDispatchWindowsFallbackEvidenceReviewText omits synthetic operation ids from review ack', () => {
  ...
  assert.match(text, /Review Ack: pnpm --filter @narada-core\/cloudflare-carrier product:operation:focus-review:text -- --url https:\/\/carrier\.example --site site_alpha --focus-kind resident_dispatch_windows_fallback_evidence --focus-ref resident_dispatch_windows_fallback_evidence_alpha --operator-session-file <operator-session-file>/);
  assert.doesNotMatch(text, /Review Ack:.*<operation-id>/);
});

test('formatResidentDispatchWindowsFallbackEvidenceReviewText suppresses review ack without site id', () => {
  const text = formatResidentDispatchWindowsFallbackEvidenceReviewText({
    worker_url: 'https://carrier.example',
    auth_source: 'operator-session-file',
    summary: {
