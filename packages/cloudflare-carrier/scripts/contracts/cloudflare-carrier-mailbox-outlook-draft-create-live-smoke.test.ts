...
} from '../workflows/cloudflare-carrier-mailbox-outlook-draft-create-live-smoke.ts';
...
test('formatMailboxOutlookDraftCreateLiveSmokeText suppresses worker-scoped handoffs without worker url', () => {
  const text = formatMailboxOutlookDraftCreateLiveSmokeText({
    status: 'ok',
    worker_url: '',
    site_id: 'site_alpha',
...
function extractDraftCreateId(requestId: any) {
  return requestId
