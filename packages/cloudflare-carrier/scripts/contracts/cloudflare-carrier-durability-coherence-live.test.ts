...
} from '../workflows/cloudflare-carrier-durability-coherence-live.ts';
...
test('durability coherence preserves operator-session-file auth for child scripts', async () => {
  ...
  assert.ok(seen.every((args: any) => args.includes('--operator-session-file')));
});
...
test('durability coherence fails on recovery mismatch and site persistence drift', async () => {
  ...
  assert.deepEqual(
    result.issues.map((current: any) => current.code),
    [
...
test('durability coherence text surfaces direct workflow and durability reads', () => {
...
function classifyArgs(args: any) {
  if (args.some((arg: any) => String(arg).endsWith('cloudflare-carrier-operation-recovery-read.ts'))) {
