...
} from '../workflows/cloudflare-carrier-site-file-change-proposal-live-smoke.ts';
...
test('formatSiteFileChangeProposalLiveSmokeText emits downstream reads', () => {
  ...
  assert.match(text, /Proposal Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file-change:proposal:review:text/);
...
test('runSiteFileChangeProposalLiveSmoke returns summarized proposal state', async () => {
  let createdProposalId: any = null;
