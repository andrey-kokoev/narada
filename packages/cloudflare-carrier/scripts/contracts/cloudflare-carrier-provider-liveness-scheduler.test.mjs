...
} from '../workflows/cloudflare-carrier-provider-liveness-scheduler.mjs';

const SCRIPT_PATH = fileURLToPath(new URL('../workflows/cloudflare-carrier-provider-liveness-scheduler.mjs', import.meta.url));
...
test('provider liveness scheduler install plan is bounded and secret-free', async () => {
  ...
  const entrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-carrier-provider-liveness-refresh.mjs');
  const scheduledTaskEntrypoint = join(root, 'packages/cloudflare-carrier/scripts/workflows/cloudflare-carrier-provider-liveness-scheduled-task.mjs');
...
test('provider liveness scheduler CLI emits operator text status', async () => {
  ...
  assert.match(result.stdout, /Posture Coherence Review: pnpm --filter @narada2\/cloudflare-carrier product:posture:coherence:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file D:\\code\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
  assert.match(result.stdout, /Durability Coherence Review: pnpm --filter @narada2\/cloudflare-carrier product:durability:coherence:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operator-session-file D:\\code\\narada\\\.narada\\auth\\cloudflare-operator-session\.json/);
});
