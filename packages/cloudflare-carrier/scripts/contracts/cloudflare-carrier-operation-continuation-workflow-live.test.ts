...
} from '../workflows/cloudflare-carrier-operation-continuation-workflow-live.ts';
...
test('runOperationContinuationWorkflowLive selects continuation from operation.list then resumes it', async () => {
  ...
  assert.equal(invocations[2][0].split(/[\\/]/).pop(), 'cloudflare-carrier-continuation-resume.ts');
  assert.equal(invocations[3][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
  assert.equal(invocations[4][0].split(/[\\/]/).pop(), 'cloudflare-carrier-product-read.ts');
...
test('formatOperationContinuationWorkflowLiveText surfaces direct follow-on workflows and reads', () => {
  ...
  assert.match(text, /Resume Workflow: pnpm --filter @narada-core\/cloudflare-carrier product:operation:session:workflow:live:text -- --url https:\/\/carrier\.example --site site_live_smoke --operation-id operation_live_alpha --operator-session-file <operator-session-file> --execute-operation-session/);
