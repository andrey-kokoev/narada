...
} from '../workflows/cloudflare-carrier-site-file-materialization-live-smoke.ts';
...
test('formatSiteFileMaterializationLiveSmokeText emits downstream reads', () => {
  ...
  assert.match(text, /Materialization Review: pnpm --filter @narada-core\/cloudflare-carrier product:site-file:materialization:review:text/);
...
test('runSiteFileMaterializationLiveSmoke returns summarized materialization state', async () => {
  let createdMaterializationId: any = null;
