...
} from '../read-models/cloudflare-carrier-mailbox-send-accepted-read.ts';

test('parseMailboxSendAcceptedReadArgs reuses mailbox send accepted list parsing', () => {
  const parsed: any = parseMailboxSendAcceptedReadArgs([
...
test('formatMailboxSendAcceptedReadText prints mailbox send acceptance summary', () => {
  ...
  assert.match(text, /Proposal Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:draft-reply-proposal:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref mailbox_send_proposal_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Draft Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:outlook-draft:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref mailbox_outlook_draft_create_alpha --operator-session-file <operator-session-file>/);
  assert.match(text, /Confirmation Read: pnpm --filter @narada-core\/cloudflare-carrier product:mailbox:send-confirmation:text -- --url https:\/\/carrier\.example\.test --site site_alpha --focus-ref mailbox_send_confirmation_alpha --operator-session-file <operator-session-file>/);
...
test('formatMailboxSendAcceptedReadText suppresses mailbox handoff without a real site id', () => {
  const text = formatMailboxSendAcceptedReadText({
    worker_url: 'https://carrier.example.test',
    auth_source: 'operator-session-file',
    summary: {
