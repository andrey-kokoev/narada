...
} from '../commands/cloudflare-carrier-operator-session-capture.ts';
import {
  formatOperatorSessionStatus,
  parseOperatorSessionStatusArgs,
  readOperatorSessionStatus,
} from '../read-models/cloudflare-carrier-operator-session-status.ts';
...
test('verifyOperatorSession preserves structured Worker refusal evidence', async () => {
  ...
  await assert.rejects(
    async () => verifyOperatorSession('https://carrier.example.test', 'secret-cookie', async (url: any, init: any) => {
...
test('formatOperatorSessionCaptureError renders verification evidence without session cookie', () => {
  const error: any = new Error('operator_session_verify_failed:operator_session_expired');
