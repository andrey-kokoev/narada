...
} from '../commands/cloudflare-carrier-repository-publication-github-app-secret-put.ts';
...
test('parseGithubAppSecretPutArgs normalizes complete CLI configuration', () => {
  ...
  assert.deepEqual(config.secrets.map((secret: any) => secret.secretName), [
...
test('installGithubAppSecrets writes all secrets without exposing values', async () => {
  ...
  const result = await installGithubAppSecrets(config, async (request: any) => {
    calls.push({
  ...
  assert.deepEqual(calls.map((call: any) => call.secretName), [
    REPOSITORY_PUBLICATION_GITHUB_APP_SECRET_NAMES.appId,
    REPOSITORY_PUBLICATION_GITHUB_APP_SECRET_NAMES.installationId,
    REPOSITORY_PUBLICATION_GITHUB_APP_SECRET_NAMES.privateKey,
  ]);
  assert.equal(calls.every((call: any) => call.redactions.includes(call.value)), true);
});
...
test('installGithubAppSecrets preserves sanitized writer failure evidence', async () => {
  ...
  await assert.rejects(
    installGithubAppSecrets(config, async ({ secretName }: any) => ({
...
test('formatGithubAppSecretPutError renders structured command evidence', () => {
  const error: any = new Error('repository_publication_github_app_secret_put_failed:secret:1');
