...
const scriptPath = join(packageRoot, 'scripts', 'cloudflare-carrier-repository-publication-github-app-secret-preflight.ts');
...
test('github app secret preflight accepts complete redacted source values', async () => {
  ...
  assert.equal(body.checks.every((check: any) => check.present), true);
  assert.doesNotMatch(String(stdout), /12345|67890|redacted-test-key/);
});
