...
} from '../workflows/cloudflare-carrier-operation-convergence-live.ts';
...
test('operation convergence returns ok immediately when operation routes already monitor', async () => {
  const result = await runOperationConvergenceLive({
    ...
    runNodeScript: async (args: any) => JSON.stringify(mockResponse(args, {
      'site.list': {
...
test('operation convergence text surfaces direct workflow and read handoffs', () => {
  ...
  assert.match(text, /Site List: pnpm --filter @narada-core\/cloudflare-carrier product:site:list:text -- --url https:\/\/carrier\.example --operator-session-file <operator-session-file>/);
  ...
  assert.match(text, /Operation List: pnpm --filter @narada-core\/cloudflare-carrier product:operation:list:text -- --url https:\/\/carrier\.example --site site_alpha --operator-session-file <operator-session-file>/);
...
function mockResponse(args: any, responses: any) {
...
function classifyArgs(args: any) {
  const scriptName = args[0].split(/[\\/]/).pop();
  if (scriptName === 'cloudflare-carrier-posture-coherence-live.ts') return 'posture';
  if (scriptName === 'cloudflare-carrier-durability-coherence-live.ts') return 'durability';
  if (scriptName === 'cloudflare-carrier-operation-next-workflow-live.ts') return `operation.next:${valueAfter(args, '--site')}`;
  const operation = valueAfter(args, '--operation');
  if (scriptName === 'cloudflare-carrier-product-read.ts' && operation === 'site.list') return 'site.list';
  if (scriptName === 'cloudflare-carrier-product-read.ts' && operation === 'operation.list') return `operation.list:${valueAfter(args, '--site')}`;
  if (scriptName=== 'cloudflare-carrier-product-read.ts' && operation === 'operation.read') {
