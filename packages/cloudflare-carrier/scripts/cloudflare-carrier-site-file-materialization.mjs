...
export function formatSiteFileMaterializationText(result) {
  ...
  if (summary.site_id && summary.materialization_id) {
    lines.push(`Materialization Review: pnpm --filter @narada2/cloudflare-carrier product:site-file:materialization:review:text -- --url ${result?.worker_url ?? '<worker-url>'} --site ${summary.site_id} --site-file-materialization-id ${summary.materialization_id} --operator-session-file <operator-session-file>`);
  }
