...
import { runTaskLifecycleRoleResolutionWriteLiveSmoke } from './cloudflare-carrier-task-lifecycle-role-resolution-write-live-smoke.ts';

const scriptPath = fileURLToPath(import.meta.url);
const scriptDir = dirname(scriptPath);
const repoRoot = resolve(scriptDir, '../../../..');

export function parseTaskLifecycleRosterMutationWriteLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleRosterMutationWriteLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleRosterMutationWriteLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === roleResolution.task_id && entry.task_lifecycle_roster_mutation_write_count === 1));
