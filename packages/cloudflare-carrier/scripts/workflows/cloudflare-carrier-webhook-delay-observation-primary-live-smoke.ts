...
export function formatWebhookDelayObservationPrimaryLiveSmokeText(result: any) {
  const lines = [
...
export async function runWebhookDelayObservationPrimaryLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.webhook_delay_observation_primary_reads.some((entry: any) => entry.observation_id === observationId));
