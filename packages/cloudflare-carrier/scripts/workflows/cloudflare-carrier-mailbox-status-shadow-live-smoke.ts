...
export function formatMailboxStatusShadowLiveSmokeText(result: any) {
...
export async function runMailboxStatusShadowLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedShadowRead = listed.body.reads.find((entry: any) => entry.read_id === readId);
  ...
  assert.ok(operationRead.body.mailbox_status_shadow_reads.some((entry: any) => entry.read_id === readId));
