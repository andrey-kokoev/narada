...
const siteFocusWorkflowScript = resolve(scriptDir, './cloudflare-carrier-site-focus-workflow-live.ts');
const siteActionWorkflowScript = resolve(scriptDir, './cloudflare-carrier-site-action-workflow-live.ts');
const CHILD_STDIO_MAX_BUFFER = 64 * 1024 * 1024;
const SITE_LIST_STALE_RECHECK_DELAY_MS = 20_000;

export function parseSiteNextWorkflowLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export function formatSiteNextWorkflowLiveText(result: any) {
  const lines = [
...
export async function runSiteNextWorkflowLive(
  config: any,
  { runNodeScript = defaultRunNodeScript, wait = defaultWait }: any= {},
) {
...
async function defaultWait(ms: any) {
  await new Promise((resolveDelay: any) => setTimeout(resolveDelay, ms));
}
...
function buildSiteActionArgs(config: any, siteId: any) {
  const args = [
...
function buildSiteFocusArgs(config: any, siteId: any) {
  const args = [
...
function shouldRetryListAfterNext({ selectedSiteId, delegatedResult, listAfterSummary }: any) {
