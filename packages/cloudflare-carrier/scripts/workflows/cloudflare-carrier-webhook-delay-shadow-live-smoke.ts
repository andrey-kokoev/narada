...
export function formatWebhookDelayShadowLiveSmokeText(result: any) {
  const lines = [
...
export async function runWebhookDelayShadowLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedObservation = listed.body.observations.find((entry: any) => entry.observation_id === observationId);
  ...
  assert.ok(siteRead.body.webhook_delay_shadow_observations.some((entry: any) => entry.observation_id === observationId));
  ...
  assert.ok(operationRead.body.webhook_delay_shadow_observations.some((entry: any) => entry.observation_id === observationId));
