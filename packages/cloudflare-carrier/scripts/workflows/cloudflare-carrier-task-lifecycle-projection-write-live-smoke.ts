...
export function parseTaskLifecycleProjectionWriteLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
  ...
  const format = option(args, '--format') ?? env.CLOUDFLARE_TASK_LIFECYCLE_PROJECTION_WRITE_LIVE_SMOKE_FORMAT ?? 'json';
  ...
  if (!['json', 'text'].includes(format)) throw new Error(`projection_write_live_smoke_unknown_format:${format}`);
  if (!auth) throw new Error('projection_write_live_smoke_requires_bearer_token_or_operator_session');
...
export function formatTaskLifecycleProjectionWriteLiveSmokeText(result: any) {
  const lines = [
    `Task Lifecycle Projection Write Smoke: ${result.status}`,
    `Worker: ${result.worker_url}`,
    `Site: ${result.site_id}`,
    `Operation: ${result.operation_id}`,
    `Task: ${result.task_id} #${result.task_number}`,
    `Admissions: create=${result.create_admission_id} claim=${result.claim_admission_id} report=${result.report_admission_id} changed_file_evidence=${result.changed_file_evidence_admission_id} finish=${result.finish_admission_id} projection_write=${result.projection_write_admission_id}`,
    ...
    `Projection Admissions: sqlite=${result.sqlite_mutation_admission ?? 'unknown'} filesystem=${result.filesystem_mutation_admission ?? 'unknown'} repository_publication=${result.repository_publication_admission ?? 'unknown'}`,
    `Partition: ${result.authority_partition ?? 'unknown'}`,
    `Counts: projection_write=${result.task_lifecycle_projection_write_count ?? 0} write_admissions=${result.task_lifecycle_write_admission_count ?? 0}`,
...
export async function runTaskLifecycleProjectionWriteLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const title = config.title ?? `Cloudflare projection write ${suffix}`;
  ...
  const refusedProjection = await postCarrier(config, {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'finished' && entry.task_lifecycle_projection_write_count === 1));
...
if (process.argv[1]=== scriptPath) {
  const config = parseTaskLifecycleProjectionWriteLiveSmokeArgs(process.argv.slice(2));
  const result = await runTaskLifecycleProjectionWriteLiveSmoke(config);
  if (config.format=== 'text') {
    process.stdout.write(formatTaskLifecycleProjectionWriteLiveSmokeText(result));
  } else {
