...
export function formatWebhookDelayRemoteSourceLiveSmokeText(result: any) {
  const lines = [
...
export async function runWebhookDelayRemoteSourceLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.webhook_delay_observation_primary_reads.some((entry: any) => entry.observation_id === observationId && entry.record?.source_material_locus === 'cloudflare_remote_source_adapter'));
