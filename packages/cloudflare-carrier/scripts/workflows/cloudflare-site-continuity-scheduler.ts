...
import { readProductSurface } from '../read-models/cloudflare-carrier-product-read.ts';
...
export function buildSiteContinuitySchedulerPlan({
  ...
  syncEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.ts',
  scheduledTaskEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-scheduled-task.ts',
  localRoot = null,
...
async function executeSchedulerSettingsPlan(plan: any, { execFileImpl, timeout }: any) {
...
function isContinuitySyncArtifactSummary(artifact: any, lastOutputPath: any) {
...
function summarizeSiteContinuityHealth(plan: any, schedulerTaskReadback: any= plan?.scheduler_task_readback ?? null) {
...
function onlyConfiguredSiteStaleness(statuses: any= [], staleReason: any) {
  if (!Array.isArray(statuses) || statuses.length === 0) return false;
  return statuses.every((site: any) => (
    site?.artifact_present === true
    && (site.status === 'synced' || (site.status === 'needs_attention' && site.reason === staleReason))
  ));
}

function hasConfiguredSiteStaleness(statuses: any= [], staleReason: any) {
  return Array.isArray(statuses) && statuses.some((site: any) => (
    site?.artifact_present === true
...
export function formatSiteContinuitySchedulerResultForText(result: any) {
  ...
  const derivedLocalStatus = localPostureStatuses.length === 0
    ? null
    : localPostureStatuses.every((statusValue: any) => statusValue === 'synced') ? 'synced' : 'needs_attention';
  ...
  const syncBySite = new Map((localSyncArtifacts?.configured_site_sync_statuses ?? []).map((site: any) => [site.site_id, site]));
  const syncArtifactsBySite = new Map((localSyncArtifacts?.artifacts ?? []).map((artifact: any) => [artifact.site_id, artifact]));
  const inboundBySite = new Map((localInboundPackets?.configured_site_inbound_statuses ?? []).map((site: any) => [site.site_id, site]));
  ...
  if (siteIds.length > 0) {
    ...
    for (const siteId of siteIds) {
      const sync: any = syncBySite.get(siteId);
      const syncArtifact: any = syncArtifactsBySite.get(siteId);
      const inbound: any = inboundBySite.get(siteId);
...
function summarizeSchedulerTaskReadback({ state, command, args, stdout, stderr, timeout_ms: timeoutMs, parsed, expectedIntervalMinutes, expectedTaskCommand, expectedTaskEntrypoint, hiddenWrapperReadback = null }: any) {
...
function isSchedulerLastResultHealthy(lastResult: any, statusText: any) {
...
function summarizeSchedulerPowerManagementStatus(powerManagement: any) {
...
function parseSchedulerTaskListOutput(output: any) {
  const parsed: any = {};
...
function parseSchedulerRepeatMinutes(value: any) {
...
function normalizeCommandForComparison(value: any) {
  return String(value ?? '').replaceAll('"', '').replace(/\s+/g, ' ').trim().toLowerCase();
}

function readHiddenWrapperFileReadback({ wrapperPath, expectedContent }: any) {
  if (!wrapperPath) {
    return {
      status: 'unknown',
      path: null,
...
function normalizeWrapperContentForComparison(value: any) {
...
function countResultStatuses(results: any) {
  const counts: any = {};
...
export function resolveSiteContinuitySchedulerProductReadAuth(options: any= {}, env: any= process.env) {
...
export function summarizeCloudflareProductBindingAlignment({
  ...
  const localSiteIds = normalizeConfiguredSiteList([
    ...
    ...(Array.isArray(configuredSites?.site_records) ? configuredSites.site_records.map((site: any) => site?.site_id) : []),
  ]);
...
export function summarizeCloudflareProductBindingPreparation({
  ...
  const localRecord = localSiteRecords.find((site: any) => site.site_id === targetSiteId) ?? null;
  const projectedRecord = projectedSiteRecords.find((site: any) => site.site_id === targetSiteId) ?? null;
...
function sanitizeProductPostureError(error: any) {
...
function persistSiteContinuityHealthSnapshot(snapshot: any, { dryRun = true, now = () => new Date().toISOString() }: any= {}) {
...
function summarizeReconciliationExecutionForHealthSnapshot(result: any) {
  return {
...
async function executeSchedulerTaskPlan(plan: any, { execFileImpl = execFileGoverned, executionTimeoutMs = DEFAULT_RECONCILE_EXECUTION_TIMEOUT_MS }: any= {}) {
  const scheduledTaskCommand = plan?.scheduled_task_command ?? [];
  const [command, ...args] = scheduledTaskCommand;
  ...
  try {
    ...
    if (action=== 'install') {
      const schedulerTaskSettingsExecution = await executeSchedulerSettingsPlan(plan, { execFileImpl, timeout });
      return {
        ...base,
...
export async function executeSiteContinuityReconciliationPlan(
  plan: any,
  { dryRun = true, executionTimeoutMs = DEFAULT_RECONCILE_EXECUTION_TIMEOUT_MS, execFileImpl = execFileGoverned, now = () => new Date().toISOString() }: any= {},
) {
  ...
  if (reconciliationPlan.status !== 'ready') {
    return persistReconciliationExecutionResult({
      ...
      filesystem_mutation_admission: 'reconciliation_execution_artifact_write_only',
      command_blockers: selectedSites.flatMap((site: any) => site.command_blockers ?? []),
      results: selectedSites.map((site: any) => ({
  ...
  const completedCount = results.filter((result: any) => result.status === 'completed').length;
  const failedCount = results.filter((result: any) => result.status === 'failed').length;
...
async function recordCloudflareReconciliationExecutionEvidence(result: any, plan: any, { execFileImpl = execFileGoverned, timeout = DEFAULT_RECONCILE_EXECUTION_TIMEOUT_MS }: any= {}) {
  ...
  const siteIds = [...new Set((result.results ?? []).map((entry: any) => entry?.site_id).filter(Boolean))];
  ...
  const recordedCount = records.filter((record: any) => record.status === 'recorded').length;
  const failedCount = records.filter((record: any) => record.status === 'failed').length;
  return {
...
function persistReconciliationExecutionResult(result: any, { dryRun = true, now = () => new Date().toISOString() }: any= {}) {
...
export async function buildSiteContinuitySchedulerPlanWithOptionalRefresh(
  options: any= {},
  { env = process.env, materializeSiteRegistryProjection = materializeCloudflareSiteRegistryProjection }: any= {},
) {
...
function summarizeSiteRegistryProjectionRefresh(result: any) {
  return {
    schema: 'narada.cloudflare_site_registry.local_projection_refresh_summary.v1',
...
export function readLocalSyncArtifactInventory(
  artifactDirectory: any,
  { lastOutputPath = null, configuredSites = [], maxArtifactAgeMinutes = DEFAULT_MAX_SYNC_ARTIFACT_AGE_MINUTES, now = () => new Date().toISOString() }: any= {},
) {
  const configuredSiteRecords = normalizeConfiguredSiteRecords(configuredSites);
  const normalizedConfiguredSites = configuredSiteRecords.map((site: any) => site.site_id);
  ...
  const allArtifactSummaries = readdirSync(artifactDirectory, { withFileTypes: true })
    ...
    .map((entry: any) => readLastSyncArtifact(join(artifactDirectory, entry.name), { now }))
    .filter((artifact: any) => isContinuitySyncArtifactSummary(artifact, lastOutputPath));
  ...
  const needsAttention = artifacts.some((artifact: any) => artifact.status !== 'synced')
    || configuredSiteSyncStatuses.some((site: any) => site.status !== 'synced');
...
export function readLocalInboundPacketInventory(
  inboundDirectory: any,
  { configuredSites = [], maxArtifactAgeMinutes = DEFAULT_MAX_SYNC_ARTIFACT_AGE_MINUTES, now = () => new Date().toISOString() }: any= {},
) {
  const configuredSiteRecords = normalizeConfiguredSiteRecords(configuredSites);
  const normalizedConfiguredSites = configuredSiteRecords.map((site: any) => site.site_id);
  const effectiveMaxArtifactAgeMinutes = normalizeMaxArtifactAgeMinutes(maxArtifactAgeMinutes);
  ...
  const artifacts = readdirSync(inboundDirectory, { withFileTypes: true })
    .filter((entry: any) => entry.isFile() && entry.name.endsWith('.json'))
    .map((entry: any) => readLocalInboundPacketArtifact(join(inboundDirectory, entry.name)))
    .filter((artifact: any) => artifact.artifact_present)
    .sort(compareArtifactSummaries);
  ...
  const needsAttention = artifacts.some((artifact: any) => artifact.status !== 'synced')
    || configuredSiteInboundStatuses.some((site: any) => site.status !== 'synced');
  return {
...
export function readLocalInboundPacketArtifact(outputPath: any) {
  if (!outputPath) {
    return {
      state: 'not_configured',
...
export function buildSiteContinuityReconciliationPlan({
  ...
  const selectedSites = configuredStatuses
    .filter((site: any) => site.status !== 'synced')
    .map((site: any) => buildReconciliationSiteAction(site, { nodeCommand, syncEntryPoint, packetPath, packetDirectory, artifactDirectory }));
  const packetSummary = packetDirectory ? null : readSiteContinuityPacketSummary(packetPath);
  const commandReadyCount = selectedSites.filter((site: any) => site.command_status === 'ready').length;
  return {
...
export function readLocalConfiguredSites({ root, explicitSites = null, siteContinuityBindingRegistryPath = null, sitesFilePath = null, siteRegistryProjectionPath = null }: any= {}) {
  const sources = [];
  ...
  ].find(([, sites]: any) => sites.length > 0) ?? ['not_configured', []];
  ...
  const bindingRecordBySiteId = new Map(normalizeConfiguredSiteRecords(siteContinuityBindingRegistry.site_records ?? [])
    .map((site: any) => [site.site_id, site]));
  const registryRecordBySiteId = new Map(normalizeConfiguredSiteRecords(siteRegistryProjection.site_records ?? [])
    .map((site: any) => [site.site_id, site]));
  const siteRecords = normalizeConfiguredSiteRecords(normalizedSites.map((siteId: any) => ({
...
export function readLastSyncArtifact(
  outputPath: any,
  { now = () => new Date().toISOString(), loopStaleAfterMs = DEFAULT_SITE_CONTINUITY_LOOP_STALE_AFTER_MS }: any= {},
) {
  if (!outputPath) {
    return {
      state: 'not_configured',
...
export function readLastReconciliationExecutionArtifact(outputPath: any) {
  if (!outputPath) {
    return {
      state: 'not_configured',
      artifact_present: false,
  ...
  return {
    ...
    site_ids: [...new Set((artifact.results ?? []).map((entry: any) => normalizeOptionalString(entry?.site_id)).filter(Boolean))],
    result_status_counts: countResultStatuses(artifact.results ?? []),
  };
}

export function readLastScheduledHealthSnapshot(outputPath: any) {
  if (!outputPath) {
    return {
      state: 'not_configured',
      artifact_present: false,
...
export function summarizeScheduledHealthOperatorNextAction(artifact: any) {
...
export function readLocalSchedulerStatus({ root, syncEntryPoint, taskEntryPoint, localRoot, packetPath, packetDirectory = null, outputPath, configuredSites = null }: any) {
...
function buildTaskCommand({ nodeCommand, entrypoint }: any) {
  const parts = [quote(nodeCommand), quote(entrypoint)];
  return parts.join(' ');
}

function buildHiddenTaskCommand({ wrapperPath }: any) {
  return [
    'wscript.exe',
    '//B',
    quote(wrapperPath),
...
function buildSchedulerBatterySettingsCommand(taskName: any) {
...
function splitWindowsSchedulerTaskName(taskName: any) {
...
export function buildHiddenVbsWrapperContent(command: any) {
  return [
...
function writeHiddenWrapper({ wrapperPath, content }: any) {
  mkdirSync(dirname(wrapperPath), { recursive: true });
  writeFileSync(wrapperPath, content, 'utf8');
}

function readEnvKeys(envPath: any) {
  const keys = [];
...
function readConfiguredSitesFromEnvFile(envPath: any) {
...
function readConfiguredSitesFile(sitesFilePath: any) {
...
function readSiteContinuityBindingRegistryFile(bindingRegistryPath: any) {
  ...
  const siteRecords = registry.bindings
    .filter((binding: any) => sites.includes(binding.site_id))
    .map((binding: any) => {
...
function findSiteContinuityEmbodiment(binding: any, embodimentKind: any) {
  return binding?.embodiments?.find((embodiment: any) => embodiment.embodiment_kind === embodimentKind) ?? null;
}

function normalizeConfiguredSiteList(value: any) {
  return normalizeConfiguredSiteRecords(value).map((site: any) => site.site_id);
}

function normalizeConfiguredSiteRecords(value: any) {
  const values = Array.isArray(value) ? value : [value];
  const siteById = new Map();
  for (const item of values) {
    if (!item) continue;
    if (typeof item=== 'string') {
      for (const siteId of item.split(',').map((part: any) => part.trim()).filter(Boolean)) {
  ...
  return [...siteById.values()].sort((left: any, right: any) => left.site_id.localeCompare(right.site_id));
}

function firstPresentObject(key: any, ...values: any[]) {
  const value = values.find((candidate: any) => candidate !== undefined && candidate !== null && candidate !== '');
  return value === undefined ? {} : { [key]: value };
}

function buildConfiguredSiteSyncStatuses(configuredSites: any, artifacts: any, { maxArtifactAgeMinutes = DEFAULT_MAX_SYNC_ARTIFACT_AGE_MINUTES, now = () => new Date().toISOString() }: any= {}) {
  const configuredSiteRecords = normalizeConfiguredSiteRecords(configuredSites);
  ...
  return configuredSiteRecords.map((site: any) => {
    ...
    const artifactAgeMilliseconds: any = Number.isFinite(nowMilliseconds) && artifact.artifact_updated_at
...
function summarizeLoopReportFreshness(loopReport: any, { now = () => new Date().toISOString(), staleAfterMs = DEFAULT_SITE_CONTINUITY_LOOP_STALE_AFTER_MS }: any= {}) {
...
function buildConfiguredSiteInboundStatuses(configuredSites: any, artifacts: any, { maxArtifactAgeMinutes = DEFAULT_MAX_SYNC_ARTIFACT_AGE_MINUTES, now = () => new Date().toISOString() }: any= {}) {
  const configuredSiteRecords = normalizeConfiguredSiteRecords(configuredSites);
  const artifactBySite = new Map();
  for (const artifact of artifacts) {
    if (!artifact?.site_id) continue;
...
function buildConfiguredSiteMetadata(site: any) {
  return {
...
function readSiteContinuityPacketSummary(packetPath: any) {
...
function buildReconciliationSiteAction(site: any, { nodeCommand, syncEntryPoint, packetPath, packetDirectory = null, artifactDirectory }: any) {
...
function resolveReconciliationPacketPath(siteId: any, { packetPath = null, packetDirectory = null }: any= {}) {
  if (packetDirectory) return join(packetDirectory, `${safeFileToken(siteId)}-packet.json`);
  return packetPath;
}

function buildSyncOnceCommand({ nodeCommand, syncEntryPoint, siteId, packetPath, outputPath, localInboundDirectory }: any) {
  return [
    quote(nodeCommand),
...
function countSelectedReasons(selectedSites: any) {
  const counts: any = {};
  for (const site of selectedSites) {
...
function normalizeMaxArtifactAgeMinutes(value: any) {
...
function normalizeExecutionTimeoutMs(value: any) {
...
function stripEnvValueQuotes(value: any) {
...
function normalizeIntervalMinutes(value: any) {
...
function resolvePath(root: any, value: any) {
  return isAbsolute(value) ? value : resolve(root, value);
}

function compareArtifactSummaries(left: any, right: any) {
...
function powershellString(value: any) {
  return `'${String(value).replaceAll("'", "''")}'`;
}

function vbsString(value: any) {
  return `"${String(value).replaceAll('"', '""')}"`;
}
...
function loadLocalEnvFile(envPath: any) {
