...
import {
  materializeCloudflareSiteRegistryProjection, resolveCloudflareSiteRegistryProjectionInputs, } from '../read-models/cloudflare-carrier-site-registry-projection.mjs';
import { resolveAuth as resolveProductReadAuth } from '../shared/cloudflare-carrier-auth-http.mjs';
import { readProductSurface } from '../read-models/cloudflare-carrier-product-read.mjs';
...
export function buildSiteContinuitySchedulerPlan({
  ...
  syncEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-sync.mjs',
  scheduledTaskEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-site-continuity-scheduled-task.mjs',
  localRoot = null,
...
async function executeSchedulerSettingsPlan(plan, { execFileImpl, timeout }) {
  ...
  if (projectionWorkerUrl && operatorSessionFile) {
    ...
    if (lastReconciliationExecution?.artifact_path && Array.isArray(lastReconciliationExecution.site_ids)) {
      for (const siteId of lastReconciliationExecution.site_ids) {
        lines.push(`Reconciliation Execution Record: pnpm --filter @narada2/cloudflare-carrier exec node scripts/workflows/cloudflare-site-continuity-sync.mjs reconciliation-execution-put --site ${siteId} --execution ${lastReconciliationExecution.artifact_path} --url ${projectionWorkerUrl} --operator-session-file ${operatorSessionFile}`);
      }
    }
  }
