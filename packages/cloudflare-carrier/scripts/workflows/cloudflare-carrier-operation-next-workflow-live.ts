...
const productReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-product-read.ts');
const evidenceReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-evidence-read.ts');
const persistenceReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-persistence-read.ts');
const recoveryReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-recovery-read.ts');
const mailboxOutlookDraftReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-mailbox-outlook-draft-read.ts');
const mailboxSendAcceptedReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-mailbox-send-accepted-read.ts');
const mailboxSendConfirmationReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-mailbox-send-confirmation-read.ts');
const directiveDeliveryReviewScript = resolve(scriptDir, './cloudflare-carrier-directive-delivery-review.ts');
const operationScopeReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-scope-read.ts');
const sessionEvidenceReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-session-evidence-read.ts');
const siteAuthorityReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-site-authority-read.ts');
const webhookDelayShadowReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-webhook-delay-shadow-read.ts');
const localIngressRequestReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-local-ingress-request-read.ts');
const localIngressEvidenceReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-local-ingress-evidence-read.ts');
const localIngressProviderLivenessReadScript = resolve(scriptDir, './cloudflare-carrier-local-ingress-provider-liveness-read.ts');
const mailboxDraftReplyProposalReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-mailbox-draft-reply-proposal-read.ts');
const repositoryPublicationProviderLivenessReadScript = resolve(scriptDir, './cloudflare-carrier-repository-publication-provider-liveness-read.ts');
const repositoryPublicationReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-repository-publication-read.ts');
const repositoryPublicationRequestReviewScript = resolve(scriptDir, '../read-models/cloudflare-carrier-repository-publication-request-review.ts');
const siteFileChangeProposalReviewScript = resolve(scriptDir, '../read-models/cloudflare-carrier-site-file-change-proposal-review.ts');
const siteFileMaterializationReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-site-file-materialization-read.ts');
const taskLifecycleReadScript = resolve(scriptDir, '../read-models/cloudflare-carrier-task-lifecycle-read.ts');
const taskLifecycleNextWorkflowScript = resolve(scriptDir, './cloudflare-carrier-task-lifecycle-next-workflow-live.ts');
const taskLifecycleCreateFromDirectiveIntentScript = resolve(scriptDir, '../commands/cloudflare-carrier-task-lifecycle-create-from-directive-intent.ts');
const residentDispatchWindowsFallbackRequestScript = resolve(scriptDir, '../commands/cloudflare-carrier-resident-dispatch-windows-fallback-request.ts');
const residentDispatchWindowsFallbackEvidenceScript = resolve(scriptDir, '../commands/cloudflare-carrier-resident-dispatch-windows-fallback-evidence.ts');
const residentDispatchWindowsFallbackEvidenceReviewScript = resolve(scriptDir, '../read-models/cloudflare-carrier-resident-dispatch-windows-fallback-evidence-review.ts');
const residentDispatchWindowsFallbackExecuteScript = resolve(scriptDir, '../commands/cloudflare-carrier-resident-dispatch-windows-fallback-execute.ts');
const residentDispatchLocalResidentCarrierBridgeScript = resolve(scriptDir, '../commands/cloudflare-carrier-resident-dispatch-local-resident-carrier-bridge.ts');
const residentDispatchLiveSmokeScript = resolve(scriptDir, './cloudflare-carrier-resident-dispatch-live-smoke.ts');
const focusReviewScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-focus-review.ts');
const focusWorkflowScript = resolve(scriptDir, './cloudflare-carrier-operation-focus-workflow-live.ts');
const sessionWorkflowScript = resolve(scriptDir, './cloudflare-carrier-operation-session-workflow-live.ts');
const continuationWorkflowScript = resolve(scriptDir, './cloudflare-carrier-operation-continuation-workflow-live.ts');
const continuityWorkflowScript = resolve(scriptDir, './cloudflare-carrier-operation-continuity-workflow-live.ts');
...
export function parseOperationNextWorkflowLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export function formatOperationNextWorkflowLiveText(result: any) {
  ...
  if (hasWorkerUrl && hasSiteId) {
    lines.push(`Operation List: pnpm --filter @narada-core/cloudflare-carrier product:operation:list:text -- --url ${result.worker_url} --site ${result.site_id} --operator-session-file <operator-session-file>`);
  ...
  if (hasWorkerUrl && hasSiteId && hasSelectedOperationId) {
    lines.push(`Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --operator-session-file <operator-session-file>`);
  }
  ...
  if (hasWorkerUrl && hasSiteId && hasSelectedOperationId && carrierSessionId) {
    lines.push(`Session Evidence: pnpm --filter @narada-core/cloudflare-carrier product:session:evidence:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --carrier-session-id ${carrierSessionId} --operator-session-file <operator-session-file>`);
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${result.worker_url} --site ${result.site_id} --carrier-session-id ${carrierSessionId} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --carrier-session-id ${carrierSessionId} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
  return `${lines.join('\n')}\n`;
}

function buildPostActionWorkflowCommand(result: any) {
  ...
  if (nextAction=== 'start_or_select_session') {
    return {
      ...
      command: `pnpm --filter @narada-core/cloudflare-carrier product:operation:session:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --operator-session-file <operator-session-file> --execute-operation-session`,
    };
  }
  if (nextAction=== 'resume_operation_continuation') {
    return {
      label: 'Continuation Workflow',
      command: `pnpm --filter @narada-core/cloudflare-carrier product:operation:continuation:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --operator-session-file <operator-session-file> --execute-operation-continuation-resume`,
    };
  }
  if (nextAction=== 'refresh_site_continuity_loop') {
    return {
      label: 'Continuity Workflow',
      command: `pnpm --filter @narada-core/cloudflare-carrier product:operation:continuity:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --expected-pre-action refresh_site_continuity_loop --operator-session-file <operator-session-file> --execute-operation-continuity`,
    };
  }
  if (nextAction=== 'review_site_continuity_reconciliation_execution' && focusRef) {
    return {
      label: 'Review Ack',
      command: `pnpm --filter @narada-core/cloudflare-carrier product:operation:focus-review:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.selected_operation_id} --focus-kind ${focusKind ?? 'site_continuity_reconciliation_execution'} --focus-ref ${focusRef} --operator-session-file <operator-session-file>`,
    };
  }
  return null;
}

function hasConcreteSiteAndSelectedOperation(result: any) {
...
export async function runOperationNextWorkflowLive(
  config: any,
  { runNodeScript = defaultRunNodeScript }: any= {},
) {
  ...
  if (routeAction=== 'review_recovery_posture') {
    ...
    if (isRecoveryBridgeActionable(recoveryResult.summary)) {
      ...
      const bridgeWorkflow: any = ROUTE_TO_WORKFLOW.get('bridge_local_resident_carrier_evidence');
      const workflowResult = parseJsonStdout(
...
async function defaultRunNodeScript(args: any, options: any) {
...
function buildOperationListArgs(config: any) {
  const args = [
    productReadScript,
...
function buildOperationReadArgs(config: any, operationId: any) {
  const args = [
    productReadScript,
...
function buildWorkflowArgs(config: any, workflow: any, operationId: any, readSummary: any= {}) {
  const args = [
    workflow.script,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildEvidenceArgs(config: any, operationId: any) {
  const args = [
    evidenceReadScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildRecoveryArgs(config: any, operationId: any) {
  const args = [
    recoveryReadScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildFocusReviewArgs(config: any, operationId: any, evidenceSummary: any= {}) {
...
function inferFocusKindFromRef(focusRef: any) {
...
function appendAuthOptions(args: any, config: any) {
...
function parseJsonStdout(stdout: any, label: any) {
...
function isEvidenceReviewSatisfied(summary: any= {}) {
...
function isEvidenceReviewActionable(summary: any= {}) {
  return Boolean(summary?.reviewable_focus_kind && summary?.reviewable_focus_ref)
    && !isEvidenceReviewSatisfied(summary);
}

function isRecoveryBridgeActionable(summary: any= {}) {
  return summary?.recovery_next_action === 'local_resident_carrier_evidence_not_admitted';
}

function isLocalResidentBridgeActionable(summary: any= {}) {
  return Array.isArray(summary?.local_resident_session_refs)
    && summary.local_resident_session_refs.some((ref: any) => typeof ref === 'string' && ref.trim().length > 0);
}

function shouldRetargetToPostureTarget(summary: any= {}, selectedOperationId: any) {
...
function resolveTaskLifecycleTaskId(readSummary: any= {}) {
  const candidates = [
    readSummary.task_id,
