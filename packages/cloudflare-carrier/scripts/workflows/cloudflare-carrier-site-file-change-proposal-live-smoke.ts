...
export function parseSiteFileChangeProposalLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatSiteFileChangeProposalLiveSmokeText(result: any) {
  const lines = [
    ...
    `Proposal Review: pnpm --filter @narada-core/cloudflare-carrier product:site-file-change:proposal:review:text -- --url ${result.worker_url} --site ${result.site_id} --focus-ref ${result.proposal_id} --operator-session-file <operator-session-file>`,
...
export async function runSiteFileChangeProposalLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.proposals.some((entry: any) => entry.proposal_id === proposalId));
  ...
  assert.ok(operationRead.body.site_file_change_proposals.some((entry: any) => entry.proposal_id === proposalId));
