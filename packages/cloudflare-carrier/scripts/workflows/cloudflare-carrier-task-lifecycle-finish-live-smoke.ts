...
export function parseTaskLifecycleFinishLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleFinishLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleFinishLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'finished' && entry.finish_id));
