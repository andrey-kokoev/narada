...
export function parseLocalIngressProviderLivenessReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readLocalIngressProviderLiveness(config: any, fetchImpl: any= fetch) {
  ...
  if (config.focusHeartbeatId && !heartbeats.some((entry: any) => entry?.local_ingress_provider_heartbeat_id === config.focusHeartbeatId)) {
...
export function summarizeLocalIngressProviderLiveness(body: any= {}, options: any= {}) {
  ...
  const focusedHeartbeats = focusHeartbeatId
    ? heartbeats.filter((entry: any) => entry?.local_ingress_provider_heartbeat_id === focusHeartbeatId)
    : heartbeats;
...
export function formatLocalIngressProviderLivenessReadText(result: any) {
...
function listLocalIngressProviderHeartbeats(body: any= {}) {
