...
export function parsePostureCoherenceLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function runPostureCoherenceLive(config: any, { runNodeScript = defaultRunNodeScript }: any= {}) {
...
function validateSiteListSummary(summary: any= {}, issues: any) {
...
function validateSiteReadSummary(summary: any= {}, issues: any) {
...
function validateOperationListSummary(siteId: any, summary: any= {}, response: any= {}, issues: any) {
  const routeAction = summary.route_next_action ?? null;
...
function issue(scope: any, code: any, details: any= {}) {
...
function buildSiteReadArgs(config: any, siteId: any) {
  return [
    productReadScript,
    '--operation', 'site.read',
    '--url', config.workerUrl,
...
function buildAuthArgs(config: any) {
...
export function formatPostureCoherenceLiveText(result: any) {
  ...
  const lines = [
    ...
    `Operation Count Summary: ${result.sites.map((site: any) => `${site.site_id}:${site.operation_list.operation_count ?? 0}`).join(', ') || 'none'}`,
  ];
  ...
  for (const site of result.sites ?? []) {
    ...
    if (workerUrl && typeof site.site_id=== 'string' && site.site_id.length > 0 && typeof site.operation_list?.next_operation_id === 'string' && site.operation_list.next_operation_id.length > 0) {
      lines.push(`  Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.operation_list.next_operation_id} --operator-session-file <operator-session-file>`);
      if (isActionableOperationListSummary(site.operation_list)) {
        lines.push(`  Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.operation_list.next_operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
      }
    }
  }
...
function isActionableOperationListSummary(summary: any= {}) {
  const routeAction = summary?.route_next_action ?? null;
