...
export function parseTaskLifecycleCreateLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleCreateLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleCreateLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedTask = taskList.body.tasks.find((entry: any) => entry.task_id === created.body.task.task_id);
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id));
