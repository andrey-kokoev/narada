...
export function formatWebhookDelayDirectSourceLiveSmokeText(result: any) {
  const lines = [
...
export async function runWebhookDelayDirectSourceLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listedSamples.body.samples.some((entry: any) => entry.sample?.source_record?.direct_source_url_host === read.body.direct_source_url_host));
  ...
  assert.ok(operationRead.body.webhook_delay_observation_primary_reads.some((entry: any) => entry.observation_id === observationId && entry.record?.source_material_locus === 'direct_remote_metric_source'));
