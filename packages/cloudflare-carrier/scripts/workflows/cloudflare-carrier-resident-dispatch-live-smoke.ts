...
export function parseResidentDispatchLiveSmokeArgs(argv: any= [], env: any= process.env, now: any= () => new Date()) {
  const args = [...argv];
...
export function formatResidentDispatchLiveSmokeText(result: any) {
...
export async function runResidentDispatchLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedDecision = listed.body.dispatch_decisions.find((entry: any) => entry.dispatch_decision_id === config.dispatchDecisionId);
  ...
  assert.ok(operationRead.body.resident_dispatch_decisions.some((entry: any) => entry.dispatch_decision_id === config.dispatchDecisionId));
  assert.ok(operationRead.body.operation_product_surface.resident_dispatch_decision_count >= 1);
  const sessionPresent = operationRead.body.sessions.some((session: any) => session.carrier_session_id === config.carrierSessionId);
...
function isAcceptedDispatchStatus(status: any) {
...
function loadLocalEnv(envPath: any) {
