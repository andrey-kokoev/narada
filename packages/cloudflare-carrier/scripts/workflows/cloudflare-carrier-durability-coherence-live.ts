...
const operationRecoveryScript = resolve(scriptDir, '../read-models/cloudflare-carrier-operation-recovery-read.ts');
const CHILD_STDIO_MAX_BUFFER = 64 * 1024 * 1024;

export function parseDurabilityCoherenceLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
function authFromExplicitFiles({ tokenFile = null, operatorSessionFile = null }: any= {}) {
  if (tokenFile) {
    return {
      kind: 'bearer',
...
function normalizeOperatorSessionCookie(cookie: any) {
  return String(cookie).replace(/^narada_operator_session=/, '').trim();
}

export async function runDurabilityCoherenceLive(config: any, { runNodeScript = defaultRunNodeScript }: any= {}) {
...
function validateSiteDurability(siteId: any, summary: any= {}, issues: any) {
...
function validateOperationDurability(siteId: any, operationId: any, operationReadSummary: any= {}, operationRecoverySummary: any= {}, issues: any) {
...
function issue(siteId: any, code: any, details: any= {}) {
  return { site_id: siteId, code, ...details };
}

function selectOperationId(summary: any= {}) {
...
function buildProductReadArgs(config: any, operation: any, { siteId = null, operationId = null }: any= {}) {
...
function buildOperationRecoveryArgs(config: any, siteId: any, operationId: any) {
  const args = [operationRecoveryScript, '--url', config.workerUrl, '--site', siteId, '--operation-id', operationId, '--format', 'json'];
  appendAuthArgs(args, config);
  return args;
}

function appendAuthArgs(args: any, config: any) {
...
async function runNodeScriptWithRetry(runNodeScript: any, args: any, options: any= {}) {
  try {
...
function isTransientChildReadError(error: any) {
...
export function formatDurabilityCoherenceLiveText(result: any) {
  ...
  if (workerUrl && isActionableSiteRoute(result?.site_list?.route_next_action) && routeSiteId) {
    lines.push(`Site Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:site:next:workflow:live:text -- --url ${workerUrl} --site ${routeSiteId} --operator-session-file <operator-session-file> --execute-site-next`);
  }
  for (const site of result?.sites ?? []) {
    ...
    if (workerUrl && typeof site.site_id=== 'string' && site.site_id.length > 0 && typeof site.selected_operation_id === 'string' && site.selected_operation_id.length > 0) {
      lines.push(`  Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.selected_operation_id} --operator-session-file <operator-session-file>`);
      if (isActionableOperationNextAction(site.operation_read?.workflow_next_action)) {
        lines.push(`  Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.selected_operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
      }
      lines.push(`  Recovery Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:recovery:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.selected_operation_id} --operator-session-file <operator-session-file>`);
      lines.push(`  Persistence Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:persistence:text -- --url ${workerUrl} --site ${site.site_id} --operation-id ${site.selected_operation_id} --operator-session-file <operator-session-file>`);
    }
  }
...
function isActionableSiteRoute(routeAction: any) {
  return routeAction != null && routeAction !== 'monitor_sites';
}

function isActionableSiteNextAction(nextAction: any) {
  return nextAction != null && nextAction !== 'monitor_site';
}

function isActionableOperationNextAction(nextAction: any) {
