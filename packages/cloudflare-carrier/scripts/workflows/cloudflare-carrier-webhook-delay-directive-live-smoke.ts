...
export function formatWebhookDelayDirectiveLiveSmokeText(result: any) {
  const lines = [
    ...
    `Directive Delivery Review: pnpm --filter @narada-core/cloudflare-carrier product:directive:delivery:review:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --focus-ref ${result.directive_record_id} --operator-session-file <operator-session-file>`,
  ];
  return `${lines.join('\n')}\n`;
}

export async function runWebhookDelayDirectiveLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedRecord = listed.body.directive_records.find((entry: any) => entry.directive_record_id === directiveRecordId);
  ...
  assert.ok(operationRead.body.webhook_delay_directive_records.some((entry: any) => entry.directive_record_id === directiveRecordId));
