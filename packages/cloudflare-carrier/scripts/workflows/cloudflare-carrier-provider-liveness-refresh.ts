...
export function parseProviderLivenessRefreshArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatProviderLivenessRefreshText(result: any) {
...
export async function runProviderLivenessRefresh(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  return {
    ...
    status: results.every((result: any) => result.http_status === 200 && result.status === 'ready') ? 'ok' : 'needs_attention',
    ...
    providers: results.map((result: any) => ({ provider: result.provider, status: result.status, http_status: result.http_status })),
  };
}
...
function inspectLocalRoot(path: any) {
  try {
...
function buildRefreshSource(config: any) {
  return {
