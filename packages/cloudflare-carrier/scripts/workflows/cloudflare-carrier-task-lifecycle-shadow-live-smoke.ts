...
export function parseTaskLifecycleShadowLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleShadowLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleShadowLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedRead = listed.body.reads.find((entry: any) => entry.read_id === readId);
  ...
  assert.ok(siteRead.body.task_lifecycle_shadow_reads.some((entry: any) => entry.read_id === readId));
  ...
  assert.ok(operationRead.body.task_lifecycle_shadow_reads.some((entry: any) => entry.read_id === readId));
