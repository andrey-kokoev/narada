...
export function formatMailboxSendLiveSmokeText(result: any) {
  ...
  if (workerUrl && siteId) {
    ...
    lines.push(`Proposal Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:draft-reply-proposal:text -- --url ${workerUrl} --site ${siteId} --focus-ref ${result.proposal_id} --operator-session-file <operator-session-file>`);
    lines.push(`Draft Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:outlook-draft:text -- --url ${workerUrl} --site ${siteId} --focus-ref ${result.draft_create_id} --operator-session-file <operator-session-file>`);
    lines.push(`Send Accepted Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:send-accepted:text -- --url ${workerUrl} --site ${siteId} --focus-ref ${result.send_accepted_id} --operator-session-file <operator-session-file>`);
    lines.push(`Send Confirmation Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:send-confirmation:text -- --url ${workerUrl} --site ${siteId} --focus-ref ${result.send_confirmation_id} --operator-session-file <operator-session-file>`);
  }
...
export async function runMailboxSendLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(sendListed.body.sends.some((entry: any) => entry.send_accepted_id === sendAcceptedId));
  ...
  assert.ok(operationReadAfterSend.body.mailbox_send_accepted_records.some((entry: any) => entry.send_accepted_id === sendAcceptedId));
  ...
  const confirmed: any = await retryConfirmationRead(config, {
    operation: 'mailbox.send_confirmation.read',
  ...
  assert.ok(confirmationListed.body.confirmations.some((entry: any) => entry.send_confirmation_id === sendConfirmationId));
  ...
  assert.ok(operationReadAfterConfirmation.body.mailbox_send_confirmations.some((entry: any) => entry.send_confirmation_id === sendConfirmationId));
...
async function retryConfirmationRead(config: any, body: any, fetchImpl: any) {
  let lastResult = null;
