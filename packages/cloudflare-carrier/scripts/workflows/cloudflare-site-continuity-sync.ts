...
import { classifySiteContinuityExchangePacket } from '../../../site-continuity/src/site-continuity.ts';
...
async function persistLocalLoopReportArtifact({ siteId, loopReport, artifactDirectory, generatedAt }: any) {
  const base = {
...
function classifyRepositoryPublicationRequest(request: any) {
  const errors = [];
...
async function buildRepositoryPublicationExecutionEvidence(request: any, { repoPath, remote, shouldPush }: any) {
...
async function persistLocalInboundPacketArtifact({ siteId, packet, admission, loopReport, inboundDirectory, generatedAt }: any) {
  const base = {
...
async function readGitState(repoPath: any) {
...
async function runGit(repoPath: any, gitArgs: any) {
  try {
...
function classifyBranchRef(requestedBranchRef: any, localBranchRef: any) {
...
function normalizeBranchRef(value: any) {
...
const auth: any = resolveAuth(args, process.env);
...
if (command=== 'repository-publication-execute-pending') {
  ...
  const completedCount = results.filter((result: any) => result.evidence?.publication_status === 'completed').length;
  const refusedCount = results.filter((result: any) => result.evidence?.publication_status === 'refused').length;
  const resolvedCount = results.filter((result: any) => ['completed', 'refused', 'failed'].includes(String(result.evidence?.publication_status ?? ''))).length;
  const heartbeat = await post({
    ...
    params: {
      ...
      status: results.some((result: any) => result.status !== 'evidence_recorded') ? 'needs_attention' : 'ready',
      max_cycles: 1,
      iteration_count: 1,
  ...
  await writeOutput(option('--out'), {
    ...
    status: results.every((result: any) => result.status === 'evidence_recorded') && heartbeat.http_status === 200 && heartbeat.body?.ok !== false ? 'ok' : 'needs_attention',
    site_id: siteId,
    worker_url: workerUrl,
    auth_source: auth.source,
    ...
    evidence_recorded_count: results.filter((result: any) => result.status === 'evidence_recorded').length,
    repository_publication_provider_heartbeat_id: heartbeatId,
...
function requiredOption(name: any) {
...
async function readJsonEnvelope(path: any) {
...
function classifyRepositoryPublicationEvidence(evidence: any) {
  const errors = [];
...
function buildSiteContinuityLoopReport({ siteId, localPacket, cloudflarePacket, pushed, generatedAt }: any) {
...
async function writeOutput(path: any, value: any) {
...
async function post(body: any) {
  const response = await fetch(apiUrl(), {
    ...
    headers: {
      ...
      ...(authHeaders(auth) as unknown as Record<string, string>),
    },
...
function failApi(code: any, response: any) {
  failJson(code, {
...
function failJson(code: any, detail: any= {}) {
...
function formatSiteContinuitySyncText(commandName: any, result: any, { operatorSessionFile: sessionFile = null }: any= {}) {
