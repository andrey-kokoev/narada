...
export function formatMailboxDraftReplyProposalLiveSmokeText(result: any) {
...
export async function runMailboxDraftReplyProposalLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.mailbox_draft_reply_proposals.some((entry: any) => entry.proposal_id === proposalId));
  const mailboxOutlookDraftCreates = operationRead.body.mailbox_outlook_draft_creates ?? [];
  const linkedDraftCreate = mailboxOutlookDraftCreates.find((entry: any) => entry?.proposal_id === proposalId) ?? null;
