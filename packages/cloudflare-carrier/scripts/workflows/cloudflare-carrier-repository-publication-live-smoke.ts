...
export async function parseRepositoryPublicationLiveSmokeArgs(
  argv: any= [],
  env: any= process.env,
  { loadEnv = true, gitHeadShaImpl = gitHeadSha }: any= {},
) {
  const args = [...argv];
  ...
  const repositoryRef = option(args, '--repository-ref') ?? 'github:narada-core/narada';
...
export function formatRepositoryPublicationLiveSmokeText(result: any) {
...
export async function runRepositoryPublicationLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedExecution = executionList.body.executions.find((entry: any) => entry.repository_publication_execution_id === executionId);
...
async function gitHeadSha(cwd: any) {
  const result: any = await execFileGoverned('git', ['rev-parse', 'HEAD'], { cwd, timeout: 30000, windowsHide: true });
