...
export function parseResidentLoopShadowLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatResidentLoopShadowLiveSmokeText(result: any) {
  const lines = [
...
export async function runResidentLoopShadowLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const listedRun = listed.body.loop_runs.find((entry: any) => entry.loop_run_id === loopRunId);
  ...
  assert.ok(operationRead.body.resident_loop_shadow_runs.some((entry: any) => entry.loop_run_id === loopRunId));
