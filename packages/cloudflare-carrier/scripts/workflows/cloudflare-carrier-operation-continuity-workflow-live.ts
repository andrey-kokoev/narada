...
export function parseOperationContinuityWorkflowLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export function formatOperationContinuityWorkflowLiveText(result: any) {
...
function buildPostContinuityWorkflowCommand(result: any) {
  ...
  if (nextAction=== 'review_site_continuity_reconciliation_execution' && focusRef) {
    return {
      label: 'Review Ack',
      command: `pnpm --filter @narada-core/cloudflare-carrier product:operation:focus-review:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --focus-kind ${focusKind ?? 'site_continuity_reconciliation_execution'} --focus-ref ${focusRef} --operator-session-file <operator-session-file>`,
    };
  }
  return null;
}
...
function buildContinuityRunOnceArgs(config: any) {
  const args = [
