...
export function formatWebhookDelayScheduledSourceLiveSmokeText(result: any) {
  const lines = [
...
export async function runWebhookDelayScheduledSourceLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.runs.some((entry: any) => entry.scheduled_run_id === scheduledRunId));
  ...
  assert.ok(operationRead.body.webhook_delay_scheduled_source_reads.some((entry: any) => entry.scheduled_run_id === scheduledRunId));
