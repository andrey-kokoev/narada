...
import { readRepositoryPublicationSurface } from '../read-models/cloudflare-carrier-repository-publication-read.ts';

const VALID_LANES = new Set(['cloudflare', 'windows']);

export function parseRepositoryPublicationReadbackLiveSmokeArgs(argv: any= [], env: any= process.env, options: any= {}) {
  const args = [...argv];
  ...
  if (!lane || !VALID_LANES.has(lane)) throw new Error(`repository_publication_readback_live_smoke_lane_unsupported:${lane}`);
...
export function formatRepositoryPublicationReadbackLiveSmokeText(result: any) {
...
export async function runRepositoryPublicationReadbackLiveSmoke(config: any, fetchImpl: any= fetch) {
  const readPublication = (operation: any, extraParams: any= {}) => readRepositoryPublicationSurface({
  ...
  const requestRecord = (Array.isArray(requestList.response?.requests) ? requestList.response.requests : [])
    .find((entry: any) => entry?.repository_publication_request_id === config.repositoryPublicationRequestId);
  ...
  const admissionRecord = admissions.find((entry: any) => entry?.repository_publication_request_id === config.repositoryPublicationRequestId);
  ...
  const evidenceRecord = config.repositoryPublicationEvidenceId
    ? evidenceRecords.find((entry: any) => entry?.repository_publication_evidence_id === config.repositoryPublicationEvidenceId)
    : evidenceRecords[0] ?? null;
  const executionRecord = config.repositoryPublicationExecutionId
    ? executionRecords.find((entry: any) => entry?.repository_publication_execution_id === config.repositoryPublicationExecutionId)
  ...
  if (operationRead) {
    ...
    if (config.lane=== 'cloudflare') {
      const opExecution = config.repositoryPublicationExecutionId
        ? executions.find((entry: any) => entry?.repository_publication_execution_id === config.repositoryPublicationExecutionId)
        : executions.find((entry: any) => entry?.repository_publication_request_id === config.repositoryPublicationRequestId);
      assert.ok(opExecution, JSON.stringify(operationRead.response));
    }
    if (config.lane=== 'windows') {
      const opEvidence = config.repositoryPublicationEvidenceId
        ? evidence.find((entry: any) => entry?.repository_publication_evidence_id === config.repositoryPublicationEvidenceId)
        : evidence.find((entry: any) => entry?.repository_publication_request_id === config.repositoryPublicationRequestId);
...
function parsePositiveInteger(value: any, fieldName: any) {
...
function resolveBearerFromEnv(env: any, repoRoot: any) {
...
function loadLocalEnv(envPath: any, targetEnv: any= process.env) {
