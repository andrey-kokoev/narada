...
export function parseSiteFileMaterializationLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatSiteFileMaterializationLiveSmokeText(result: any) {
  const lines = [
    ...
    `Materialization Review: pnpm --filter @narada-core/cloudflare-carrier product:site-file:materialization:review:text -- --url ${result.worker_url} --site ${result.site_id} --site-file-materialization-id ${result.materialization_id} --operator-session-file <operator-session-file>`,
...
export async function runSiteFileMaterializationLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.materializations.some((entry: any) => entry.materialization_id === materializationId));
  ...
  assert.ok(operationRead.body.site_file_materializations.some((entry: any) => entry.materialization_id === materializationId));
