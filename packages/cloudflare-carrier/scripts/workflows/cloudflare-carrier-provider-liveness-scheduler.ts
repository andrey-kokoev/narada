...
export function buildProviderLivenessSchedulerPlan({
  ...
  refreshEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-carrier-provider-liveness-refresh.ts',
  scheduledTaskEntrypoint = 'packages/cloudflare-carrier/scripts/workflows/cloudflare-carrier-provider-liveness-scheduled-task.ts',
  localRoot = null,
...
async function executeProviderLivenessSchedulerSettingsPlan(plan: any, { execFileImpl, timeout }: any) {
...
export async function runProviderLivenessSchedulerAction(
  options: any= {},
  { execFileImpl = execFileGoverned }: any= {},
) {
...
async function executeProviderLivenessSchedulerTaskPlan(plan: any, { execFileImpl = execFileGoverned, executionTimeoutMs = DEFAULT_EXECUTION_TIMEOUT_MS }: any= {}) {
...
async function executeProviderLivenessSchedulerReadback(plan: any, { execFileImpl = execFileGoverned, executionTimeoutMs = DEFAULT_EXECUTION_TIMEOUT_MS }: any= {}) {
  const scheduledTaskCommand = plan?.scheduled_task_command ?? [];
  const [command, ...args] = scheduledTaskCommand;
  const base = {
    ...plan,
...
export function summarizeProviderLivenessSchedulerReadback({ state, command, args, stdout, stderr, timeout_ms: timeoutMs, parsed, expectedIntervalMinutes, expectedTaskCommand, expectedTaskEntrypoint, hiddenWrapperReadback = null }: any) {
...
export function formatProviderLivenessSchedulerText(result: any) {
...
export function readLocalSchedulerStatus({ root, refreshEntryPoint, taskEntryPoint, localRoot }: any) {
...
function buildTaskCommand({ nodeCommand, entrypoint, localRoot }: any) {
  return [
    quote(nodeCommand),
    quote(entrypoint),
...
function parseArgs(argv: any) {
  const args: any = { action: 'status', dryRun: true, format: 'json' };
