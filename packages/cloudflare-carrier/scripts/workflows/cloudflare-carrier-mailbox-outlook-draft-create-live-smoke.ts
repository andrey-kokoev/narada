...
export function formatMailboxOutlookDraftCreateLiveSmokeText(result: any) {
...
export async function runMailboxOutlookDraftCreateLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.drafts.some((entry: any) => entry.draft_create_id === draftCreateId));
  ...
  const operationDraft = operationRead.body.mailbox_outlook_draft_creates.find((entry: any) => entry.draft_create_id === draftCreateId);
