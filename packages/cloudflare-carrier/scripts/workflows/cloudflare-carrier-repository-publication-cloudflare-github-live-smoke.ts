...
export function parseRepositoryPublicationCloudflareGithubLiveSmokeArgs(argv: any= [], env: any= process.env, options: any= {}) {
  const args = [...argv];
...
export function formatRepositoryPublicationCloudflareGithubLiveSmokeText(result: any) {
...
export async function runRepositoryPublicationCloudflareGithubLiveSmoke(config: any, fetchImpl: any= fetch) {
  ...
  const storedExecution = executionList.body.executions.find((entry: any) => entry.repository_publication_execution_id === executionId);
  ...
  const operationExecution = operationRead.body.repository_publication_executions.find((entry: any) => entry.repository_publication_execution_id === executionId);
...
function normalizeCommitSha(value: any) {
