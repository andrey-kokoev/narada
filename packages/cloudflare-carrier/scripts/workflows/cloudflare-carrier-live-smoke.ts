...
export function parseLiveSmokeArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
async function postIntelligence(config: any, body: any, fetchImpl: any) {
...
function invocationControl({ intent_id, operation_id, mode }: any) {
  return {
...
async function readExecution(config: any, attemptId: any, fetchImpl: any) {
...
export function formatLiveSmokeText(result: any) {
  ...
  if (hasSiteId && hasCarrierSessionId) {
    lines.push(`Session Evidence: pnpm --filter @narada-core/cloudflare-carrier product:session:evidence:text -- --url ${result.worker_url} --site ${result.site_id} --carrier-session-id ${result.carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${result.worker_url} --site ${result.site_id} --carrier-session-id ${result.carrier_session_id} --operator-session-file <operator-session-file>`);
    lines.push(`Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --carrier-session-id ${result.carrier_session_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`);
  }
  if (hasSiteId && hasOperationId) {
    lines.push(`Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --operator-session-file <operator-session-file>`);
    lines.push(`Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
  }
  return `${lines.join('\n')}\n`;
}

export async function runLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const inputEventKinds = inputDelivery.body.events.map((event: any) => event.event_kind);
  ...
  const providerRequest = inputDelivery.body.events.find((event: any) => event.event_kind === 'provider_request_recorded');
  ...
  const providerOutput = inputDelivery.body.events.find((event: any) => event.event_kind === 'provider_text_delta_recorded');
  assert.equal(typeof providerOutput.payload.text_delta, 'string');
  assert.ok(providerOutput.payload.text_delta.length > 0);
  const turnCompleted = inputDelivery.body.events.find((event: any) => event.event_kind === 'turn_completed');
  ...
  assert.deepEqual(executionRead.body.result.data.transitions.map(({ state }: any) => state), [
  ...
  assert.ok(executionRead.body.result.data.provenance.materializations.every(({ destination }: any) =>
    destination.site_id === config.intelligenceSiteId));
  const inputCompleted = inputDelivery.body.events.find((event: any) => event.event_kind === 'input_completed');
  ...
  const resolverRefusalFailure = resolverRefusal.body.events.find((event: any) => event.event_kind === 'turn_failed');
  ...
  const retryFailureEvent = retryFailure.body.events.find((event: any) => event.event_kind === 'turn_failed');
  ...
  const retrySuccessCompleted = retrySuccess.body.events.find((event: any) => event.event_kind === 'turn_completed');
  ...
  assert.ok(retryExecutionRead.body.result.data.plan_revalidations.some(({ mode }: any) => mode === 'retry'));
  ...
  const uncertainFailure = uncertainDelivery.body.events.find((event: any) => event.event_kind === 'turn_failed');
  ...
  const replayCompleted = replay.body.events.find((event: any) => event.event_kind === 'turn_completed');
  ...
  const taskCreateResult = taskCreate.body.events.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  const taskUpdateResult = taskUpdate.body.events.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  assert.ok(status.body.tasks.some((task: any) => task.task_id === createdTask.task_id && task.status === 'done' && task.note === 'live-smoke'));
  ...
  const liveEventKinds = events.body.events.map((event: any) => event.event_kind);
  ...
  return {
    ...
    intelligence_transition_states: executionRead.body.result.data.transitions.map(({ state }: any) => state),
    ...
    event_kinds: events.body.events.map((event: any) => event.event_kind),
    sequences: events.body.events.map((event: any) => event.sequence),
    next_cursor: events.body.next_cursor,
  };
}

function assertAuthenticatedPrincipal(principal: any) {
...
function assertToolEffectStatus(body: any, expectedPosture: any) {
  ...
  if (body.tool_effect_posture=== 'configured') {
    ...
    assert.deepEqual(body.tool_effect_supported_tools, body.tool_effect_capabilities.map((capability: any) => capability.tool_name));
...
function assertKnownToolCapability(capability: any) {
...
async function getConsole(config: any, fetchImpl: any) {
...
async function post(config: any, body: any, fetchImpl: any) {
...
function resolveBearerToken(args: any, env: any) {
