...
export function parseTaskLifecycleAssignmentWriteLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleAssignmentWriteLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleAssignmentWriteLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'finished' && entry.task_lifecycle_assignment_write_count === 1));
