...
export function parseTaskLifecycleClaimLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatTaskLifecycleClaimLiveSmokeText(result: any) {
  const lines = [
...
export async function runTaskLifecycleClaimLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'claimed'));
