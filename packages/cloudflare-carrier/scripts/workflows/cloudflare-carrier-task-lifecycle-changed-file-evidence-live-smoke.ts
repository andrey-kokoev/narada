...
export function parseChangedFileEvidenceLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatChangedFileEvidenceLiveSmokeText(result: any) {
  const lines = [
...
export async function runChangedFileEvidenceLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'finished' && entry.changed_file_evidence_count === 1));
  ...
  ].map((value: any) => value.replace('role-resolution', 'role_resolution'))).has(operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission), `unexpected changed-file evidence smoke Cloudflare write admission: ${operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission}`);
