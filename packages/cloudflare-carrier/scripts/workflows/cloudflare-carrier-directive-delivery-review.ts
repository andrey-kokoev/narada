...
import { parseProductReadArgs, readProductSurface } from '../read-models/cloudflare-carrier-product-read.ts';

export function parseDirectiveDeliveryReviewArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readDirectiveDeliveryReview(config: any, fetchImpl: any= fetch) {
...
export function summarizeDirectiveDeliveryReview(operationBody: any= {}, directiveRecordsBody: any= {}, directiveDeliveriesBody: any= {}, options: any= {}) {
  ...
  const exactFocusedDirectiveRecord = focusRef
    ? directiveRecords.find((entry: any) => entry?.directive_record_id === focusRef) ?? null
    : null;
  ...
  const focusedDirectiveDeliveries = exactFocusedDirectiveRecord
    ? directiveDeliveries.filter((entry: any) => entry?.directive_record_id === focusedDirectiveRecordId)
    : directiveDeliveries;
  ...
  const deliveredRecordIds = new Set(
    focusedDirectiveDeliveries
      .map((entry: any) => entry?.directive_record_id)
      .filter((value: any) => typeof value === 'string' && value.length > 0),
  );
  const undeliveredDirectiveRecords = focusedDirectiveRecords.filter((entry: any) => !deliveredRecordIds.has(entry?.directive_record_id));
  return {
...
export function formatDirectiveDeliveryReviewText(result: any) {
