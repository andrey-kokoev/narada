...
export function loadLocalEnvFile(envUrlOrPath: any, { env = process.env }: any= {}) {
...
export function stripEnvValueQuotes(value: any) {
...
function hasAnyFlag(argv: any, flags: any) {
  return argv.some((arg: any) => flags.includes(arg));
}
...
function resolveMaybeRelative(basePath: any, value: any) {
...
if (import.meta.url=== pathToFileURL(process.argv[1]).href) {
  ...
  process.argv = [
    process.argv[0],
    fileURLToPath(new URL('./cloudflare-site-continuity-scheduler.ts', import.meta.url)),
  ...
  await import('./cloudflare-site-continuity-scheduler.ts');
}
