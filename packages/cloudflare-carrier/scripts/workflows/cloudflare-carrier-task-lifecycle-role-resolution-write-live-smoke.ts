...
export function parseTaskLifecycleRoleResolutionWriteLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
  ...
  const format = option(args, '--format') ?? env.CLOUDFLARE_TASK_LIFECYCLE_ROLE_RESOLUTION_WRITE_LIVE_SMOKE_FORMAT ?? 'json';
  ...
  const filePath = option(args, '--file') ?? 'packages/cloudflare-carrier/src/cloudflare-worker.ts';
  ...
  if (!['json', 'text'].includes(format)) throw new Error(`role_resolution_write_live_smoke_unknown_format:${format}`);
  if (!auth) throw new Error('role_resolution_write_live_smoke_requires_bearer_token_or_operator_session');
...
export function formatTaskLifecycleRoleResolutionWriteLiveSmokeText(result: any) {
  const lines = [
    ...
    `Role Resolution: resolved_role=${result.resolved_role ?? 'unknown'} authority=${result.role_resolution_authority_admission ?? 'unknown'} roster_read=${result.roster_read_admission ?? 'unknown'} roster_mutation=${result.roster_mutation_admission ?? 'unknown'} mailbox=${result.mailbox_mutation_admission ?? 'unknown'} filesystem=${result.filesystem_mutation_admission ?? 'unknown'} repository_publication=${result.repository_publication_admission ?? 'unknown'}`,
    `Partition: ${result.authority_partition ?? 'unknown'}`,
    `Counts: role_resolution_write=${result.task_lifecycle_role_resolution_write_count ?? 0} write_admissions=${result.task_lifecycle_write_admission_count ?? 0}`,
    `Site Read: pnpm --filter @narada-core/cloudflare-carrier product:site:read:text -- --url ${result.worker_url} --site ${result.site_id} --operator-session-file <operator-session-file>`,
    `Site Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:site:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operator-session-file <operator-session-file> --execute-site-next`,
    `Posture Coherence Review: pnpm --filter @narada-core/cloudflare-carrier product:posture:coherence:live:text -- --url ${result.worker_url} --site ${result.site_id} --operator-session-file <operator-session-file>`,
    `Durability Coherence Review: pnpm --filter @narada-core/cloudflare-carrier product:durability:coherence:live:text -- --url ${result.worker_url} --site ${result.site_id} --operator-session-file <operator-session-file>`,
    `Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${result.worker_url} --site ${result.site_id} --task-id ${result.task_id} --operator-session-file <operator-session-file>`,
    `Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --task-id ${result.task_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`,
    `Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --operator-session-file <operator-session-file>`,
    `Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.operation_id} --operator-session-file <operator-session-file> --execute-operation-next`,
  ];
  return `${lines.join('\n')}\n`;
}

export async function runTaskLifecycleRoleResolutionWriteLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const title = config.title ?? `Cloudflare role resolution write ${suffix}`;

  const created = await postCarrier(config, {
    operation: 'task_lifecycle.task_create.admit',
    request_id: `role_resolution_write_live_create_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: createAdmissionId,
      title,
      description: 'Live proof setup for Cloudflare-owned task lifecycle role resolution write.',
  ...
  const claimed = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_claim_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: claimAdmissionId,
      task_id: taskId,
      claimant_agent_id: config.agentId,
      claimant_principal_id: config.assigneePrincipalId,
  ...
  const reported = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_report_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: reportAdmissionId,
      task_id: taskId,
      reporter_agent_id: config.agentId,
      reporter_principal_id: config.assigneePrincipalId,
      summary: 'Live Cloudflare role resolution write proof setup.',
  ...
  const evidence = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_evidence_${suffix}`,
    params: {
      site_id: config.siteId,
  ...
  const finished = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_finish_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: finishAdmissionId,
      task_id: taskId,
      finalizer_agent_id: config.agentId,
      finalizer_principal_id: config.assigneePrincipalId,
  ...
  const projected = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_projection_${suffix}`,
    params: {
      site_id: config.siteId,
      ...
      source_evidence_ref: `source-evidence:${taskId}:finished-row`,
  ...
  const sourceState = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_source_state_${suffix}`,
    params: {
      site_id: config.siteId,
      ...
      source_state_evidence_ref: `source-state-evidence:${taskId}:projection-row`,
  ...
  const assignment = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_assignment_${suffix}`,
    params: {
      site_id: config.siteId,
      ...
      assignment_evidence_ref: `assignment-evidence:${taskId}:source-state-row`,
  ...
  const refusedRoleResolution = await postCarrier(config, {
    operation: 'task_lifecycle.role_resolution_write.admit',
    request_id: `role_resolution_write_live_refused_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: `${roleResolutionAdmissionId}_refused`,
      task_id: taskId,
      assignee_principal_id: config.assigneePrincipalId,
      ...
      role_resolution_evidence_ref: `role-resolution-evidence:${config.assigneePrincipalId}:membership-row`,
    },
  }, fetchImpl);
  ...
  const roleResolution = await postCarrier(config, {
    operation: 'task_lifecycle.role_resolution_write.admit',
    request_id: `role_resolution_write_live_admitted_${suffix}`,
    params: {
      site_id: config.siteId,
      admission_id: roleResolutionAdmissionId,
      task_id: taskId,
      assignee_principal_id: config.assigneePrincipalId,
      ...
      confirmation_evidence_ref: 'evidence:live-smoke:task-lifecycle-role-resolution-write',
    },
  }, fetchImpl);
  ...
  const operationRead = await postCarrier(config, {
    ...
    request_id: `role_resolution_write_live_operation_read_${suffix}`,
    params: { site_id: config.siteId, operation_id: config.operationId, task_lifecycle_task_limit: 100, task_lifecycle_include_task_ids: [taskId], task_lifecycle_write_admission_limit: 100 },
  }, fetchImpl);
  assert.equal(operationRead.http_status, 200, JSON.stringify(operationRead.body));
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === taskId && entry.task_lifecycle_role_resolution_write_count === 1));
...
async function postCarrier(config: any, body: any, fetchImpl: any) {
...
function resolveBearerFromEnv(args: any, env: any) {
...
function readTokenFile(tokenFilePath: any) {
...
if (process.argv[1]=== scriptPath) {
  const config = parseTaskLifecycleRoleResolutionWriteLiveSmokeArgs(process.argv.slice(2));
  const result = await runTaskLifecycleRoleResolutionWriteLiveSmoke(config);
  if (config.format=== 'text') {
    process.stdout.write(formatTaskLifecycleRoleResolutionWriteLiveSmokeText(result));
  } else {
