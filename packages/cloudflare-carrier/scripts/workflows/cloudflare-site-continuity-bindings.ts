...
async function readScheduledHealthSnapshotForBindingPreparation(healthSnapshotPath: any) {
...
function readCloudflareSiteProjectionRecord(plan: any, siteId: any) {
  ...
  try {
    ...
    return (projection?.sites ?? []).find((site: any) => site?.site_id === siteId) ?? null;
  } catch {
    return null;
  }
}

function fileSafeId(value: any) {
  return String(value ?? 'unknown-site').replace(/[^A-Za-z0-9_.-]+/g, '-').replace(/^-+|-+$/g, '') || 'unknown-site';
}

function buildBindingMaterializationPlan({ argv = [], env = process.env, cwd = process.cwd() }: any= {}) {
  ...
  return {
    ...
    effective_packet_paths: packetPaths.map((packetPath: any) => resolvePath(cwd, packetPath)),
    effective_packet_directories: packetDirectories.map((packetDirectory: any) => resolvePath(cwd, packetDirectory)),
...
async function runSiteContinuityBindingWorkflow(plan: any) {
  switch (plan.action) {
    case 'materialize':
...
function validateExplicitSiteContinuityRefs({ localSiteRef, cloudflareSiteRef, siteRecord }: any) {
  const errors = [];
...
async function admitNextSiteContinuityBinding(plan: any) {
  ...
  const preservedBindings = (existingRegistry.bindings ?? [])
    .filter((binding: any) => binding.site_id !== targetSiteId && binding.relation_id !== nextBinding.relation_id);
  const existingBinding = (existingRegistry.bindings ?? [])
    .find((binding: any) => binding.site_id === targetSiteId || binding.relation_id === nextBinding.relation_id) ?? null;
  const nextRegistry = createSiteContinuityBindingRegistry({
    bindings: [...preservedBindings, nextBinding]
      .sort((left: any, right: any) => left.site_id.localeCompare(right.site_id)),
...
async function prepareNextSiteContinuityBindingPacket(plan: any) {
  const health = await readScheduledHealthSnapshotForBindingPreparation(plan.effective_health_snapshot_path);
  const targetSiteId = plan.target_site_id ?? health.operator_next_target_site_id ?? health.cloudflare_product_next_site_id ?? null;
  const operatorAction = health.operator_next_action ?? null;
  if (operatorAction && operatorAction !== 'bind_cloudflare_product_next_site_locally') {
...
async function materializeSiteContinuityBindingRegistry(plan: any) {
  const packetReads = [];
  const bindings = [];
  ...
  return {
    ...
    sites: registry.bindings.map((binding: any) => binding.site_id).sort((left: any, right: any) => left.localeCompare(right)),
    packet_reads: packetReads.map((read: any) => ({
      path: read.path,
...
async function validateMaterializedSiteContinuityBindingRegistry(plan: any) {
...
async function listMaterializedSiteContinuityBindingRegistry(plan: any) {
  const registry = await readMaterializedSiteContinuityBindingRegistry(plan);
  return {
    ok: true,
    action: 'listed',
    ...
    sites: registry.bindings
      .map((binding: any) => ({
        ...
        embodiments: (binding.embodiments ?? [])
          ...
          .sort((left: any, right: any) => left.embodiment_kind.localeCompare(right.embodiment_kind)),
      }))
      .sort((left: any, right: any) => left.site_id.localeCompare(right.site_id)),
  };
}

async function readMaterializedSiteContinuityBindingRegistry(plan: any) {
...
function collectPacketPaths(args: any, env: any, cwd: any, packetDirectories: any= []) {
  ...
  const directoryPackets = packetDirectories.flatMap((packetDirectory: any) => readPacketDirectoryPaths(cwd, packetDirectory));
  const configured = [...configuredPackets, ...directoryPackets];
  const selected = configured.length > 0 ? configured : DEFAULT_PACKET_PATHS;
  return [...new Set(selected.map((item: any) => String(item).trim()).filter(Boolean))];
}

function collectPacketDirectories(args: any, env: any) {
  ...
  return [...new Set(configuredDirectories.map((item: any) => String(item).trim()).filter(Boolean))];
}

function readPacketDirectoryPaths(cwd: any, packetDirectory: any) {
  ...
  const packetPaths = entries
    .filter((entry: any) => entry.isFile() && entry.name.endsWith('-packet.json'))
    .map((entry: any) => path.join(effectiveDirectory, entry.name))
    .filter((packetPath: any) => statSync(packetPath).isFile())
...
function splitList(value: any) {
  return String(value ?? '').split(/[;,]/u).map((item: any) => item.trim()).filter(Boolean);
}

function resolvePath(cwd: any, value: any) {
  const normalized = String(value ?? '').trim();
  if (!normalized) throw new Error('path_missing');
  return path.resolve(cwd, normalized);
}

function formatSiteContinuityBindingWorkflowText(plan: any, result: any) {
  ...
  if (sites.length > 0) {
    const labels = sites.map((site: any) => (typeof site === 'string' ? site : site?.site_id)).filter(Boolean);
