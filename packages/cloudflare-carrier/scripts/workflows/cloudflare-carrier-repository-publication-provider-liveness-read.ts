...
export function parseRepositoryPublicationProviderLivenessReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readRepositoryPublicationProviderLiveness(config: any, fetchImpl: any= fetch) {
  ...
  if (config.focusHeartbeatId && !heartbeats.some((entry: any) => entry?.repository_publication_provider_heartbeat_id === config.focusHeartbeatId)) {
...
export function summarizeRepositoryPublicationProviderLiveness(body: any= {}, options: any= {}) {
  ...
  const focusedHeartbeats = focusHeartbeatId
    ? heartbeats.filter((entry: any) => entry?.repository_publication_provider_heartbeat_id === focusHeartbeatId)
    : heartbeats;
...
export function formatRepositoryPublicationProviderLivenessReadText(result: any) {
  ...
  if (workerUrl && summary.site_id) {
    ...
    if (actionableNext) {
      lines.push(`Provider Liveness Refresh: pnpm --filter @narada-core/cloudflare-carrier provider-liveness:refresh:live:text -- --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file>`);
    }
  }
  return `${lines.join('\n')}\n`;
}

function listRepositoryPublicationProviderHeartbeats(body: any= {}) {
