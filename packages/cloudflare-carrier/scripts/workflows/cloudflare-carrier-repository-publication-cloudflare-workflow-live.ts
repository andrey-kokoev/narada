...
const cloudflareExecutionScript = resolve(scriptDir, './cloudflare-carrier-repository-publication-cloudflare-github-live-smoke.ts');
const readbackScript = resolve(scriptDir, './cloudflare-carrier-repository-publication-readback-live-smoke.ts');

export function parseRepositoryPublicationCloudflareWorkflowLiveArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export function formatRepositoryPublicationCloudflareWorkflowLiveText(result: any) {
...
function appendOption(args: any, name: any, value: any) {
