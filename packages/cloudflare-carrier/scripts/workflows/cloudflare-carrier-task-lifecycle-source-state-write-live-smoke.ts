...
export function parseTaskLifecycleSourceStateWriteLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
  ...
  const format = option(args, '--format') ?? env.CLOUDFLARE_TASK_LIFECYCLE_SOURCE_STATE_WRITE_LIVE_SMOKE_FORMAT ?? 'json';
  ...
  if (!['json', 'text'].includes(format)) throw new Error(`source_state_write_live_smoke_unknown_format:${format}`);
  if (!auth) throw new Error('source_state_write_live_smoke_requires_bearer_token_or_operator_session');
...
export function formatTaskLifecycleSourceStateWriteLiveSmokeText(result: any) {
  const lines = [
    ...
    `Source State: canonical=${result.canonical_source_state_authority ?? 'unknown'} windows_sqlite=${result.windows_sqlite_source_write_admission ?? 'unknown'} filesystem=${result.filesystem_mutation_admission ?? 'unknown'} repository_publication=${result.repository_publication_admission ?? 'unknown'} mailbox=${result.mailbox_mutation_admission ?? 'unknown'}`,
    `Partition: ${result.authority_partition ?? 'unknown'}`,
    `Counts: source_state_write=${result.task_lifecycle_source_state_write_count ?? 0} write_admissions=${result.task_lifecycle_write_admission_count ?? 0}`,
...
export async function runTaskLifecycleSourceStateWriteLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  const title = config.title ?? `Cloudflare source-state write ${suffix}`;
  ...
  const refusedSourceState = await postCarrier(config, {
  ...
  assert.ok(operationRead.body.task_lifecycle_tasks.some((entry: any) => entry.task_id === created.body.task.task_id && entry.status === 'finished' && entry.task_lifecycle_source_state_write_count === 1));
  ...
  ]).has(operationRead.body.operation_product_surface.task_lifecycle_authority_partition), `unexpected source-state smoke authority partition: ${operationRead.body.operation_product_surface.task_lifecycle_authority_partition}`);
  assert.ok(new Set([
  ...
  ]).has(operationRead.body.operation_product_surface.task_lifecycle_write_admission_posture), `unexpected source-state smoke write posture: ${operationRead.body.operation_product_surface.task_lifecycle_write_admission_posture}`);
  assert.ok(new Set([
  ...
  ]).has(operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission), `unexpected source-state smoke Cloudflare write admission: ${operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission}`);
...
if (process.argv[1]=== scriptPath) {
  const config = parseTaskLifecycleSourceStateWriteLiveSmokeArgs(process.argv.slice(2));
  const result = await runTaskLifecycleSourceStateWriteLiveSmoke(config);
  if (config.format=== 'text') {
    process.stdout.write(formatTaskLifecycleSourceStateWriteLiveSmokeText(result));
  } else {
