...
const taskCreateScript = resolve(scriptDir, '../commands/cloudflare-carrier-task-lifecycle-create.ts');
...
export function parseTaskLifecycleWorkflowLiveArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export function formatTaskLifecycleWorkflowLiveText(result: any) {
  const lines = [
  ...
  if (result.read_after_finish?.operation_id) {
    lines.push(`Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.read_after_finish.operation_id} --operator-session-file <operator-session-file>`);
    lines.push(`Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.read_after_finish.operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
  }
  return `${lines.join('\n')}\n`;
}

export async function runTaskLifecycleWorkflowLive(config: any, { runNodeScript = defaultRunNodeScript }: any= {}) {
...
function buildTaskCreateArgs(config: any) {
  const args = [
    taskCreateScript,
...
function buildTaskClaimArgs(config: any, taskId: any) {
  const args = [
    taskClaimScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildTaskReportArgs(config: any, taskId: any) {
  const args = [
    taskReportScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildTaskFinishArgs(config: any, taskId: any) {
  const args = [
    taskFinishScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
...
function buildTaskReadArgs(config: any, taskId: any) {
  const args = [
    taskReadScript,
    '--url', config.workerUrl,
    '--site', config.siteId,
