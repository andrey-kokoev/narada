...
export function formatTaskLifecycleWorkflowLiveText(result: any) {
  ...
  if (result.read_after_finish?.operation_id) {
    ...
    lines.push(`Operation Next Workflow: pnpm --filter @narada2/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${result.worker_url} --site ${result.site_id} --operation-id ${result.read_after_finish.operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
  }
  return `${lines.join('\n')}\n`;
}
