...
export function parseLocalIngressRequestLiveSmokeArgs(argv: any= [], env: any= process.env, { loadEnv = true }: any= {}) {
  const args = [...argv];
...
export function formatLocalIngressRequestLiveSmokeText(result: any) {
  ...
  if (workerUrl && siteId && requestId) {
    lines.push(`Request Review: pnpm --filter @narada-core/cloudflare-carrier product:local-ingress:request:review:text -- --url ${workerUrl} --site ${siteId} --local-ingress-request-id ${requestId} --operator-session-file <operator-session-file>`);
    lines.push(`Evidence Read: pnpm --filter @narada-core/cloudflare-carrier product:local-ingress:evidence:review:text -- --url ${workerUrl} --site ${siteId} --local-ingress-request-id ${requestId} --operator-session-file <operator-session-file>`);
  }
...
export async function runLocalIngressRequestLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.requests.some((entry: any) => entry.local_ingress_request_id === localIngressRequestId));
