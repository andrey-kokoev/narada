...
export function formatMailboxStatusSourceLiveSmokeText(result: any) {
...
export async function runMailboxStatusSourceLiveSmoke(config: any, { fetchImpl = fetch }: any= {}) {
  ...
  assert.ok(listed.body.reads.some((entry: any) => entry.read_id === readId));
  ...
  assert.ok(operationRead.body.mailbox_status_source_reads.some((entry: any) => entry.read_id === readId));
