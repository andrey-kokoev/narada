...
export function parseWebhookDelayShadowReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readWebhookDelayShadow(config: any, fetchImpl: any= fetch) {
...
export function summarizeWebhookDelayShadow(operationBody: any= {}, shadowReadBody: any= {}, options: any= {}) {
  ...
  const exactFocusedObservation = focusRef
    ? observations.find((entry: any) => entry?.observation_id === focusRef) ?? null
    : null;
...
export function formatWebhookDelayShadowReadText(result: any) {
...
function parseOptionalInteger(value: any, name: any) {
