...
const LIMIT_PARAM_BY_OPERATION: any = {
...
export function parseRepositoryPublicationReadArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export function buildRepositoryPublicationReadParams({ operation, siteId, repositoryPublicationRequestId, limit }: any) {
...
export async function readRepositoryPublicationSurface(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_read_failed:${code}`);
    error.code = code;
  ...
  if (config.operation=== 'repository_publication.evidence.list' && config.focusEvidenceId) {
    ...
    if (!evidence.some((entry: any) => entry?.repository_publication_evidence_id === config.focusEvidenceId)) {
  ...
  if (config.operation=== 'repository_publication.admission.list' && config.focusAdmissionId) {
    ...
    if (!admissions.some((entry: any) => entry?.repository_publication_admission_id === config.focusAdmissionId)) {
  ...
  if (config.operation=== 'repository_publication.cloudflare_execution.list' && config.focusExecutionId) {
    ...
    if (!executions.some((entry: any) => entry?.repository_publication_execution_id === config.focusExecutionId)) {
...
export function summarizeRepositoryPublicationSurface(operation: any, body: any= {}, params: any= {}, options: any= {}) {
  ...
  if (operation=== 'repository_publication.admission.list') {
    ...
    const focusedAdmissions = focusAdmissionId
      ? admissions.filter((entry: any) => entry?.repository_publication_admission_id === focusAdmissionId)
      : admissions;
  ...
  if (operation=== 'repository_publication.evidence.list') {
    ...
    const focusedEvidence = focusEvidenceId
      ? evidence.filter((entry: any) => entry?.repository_publication_evidence_id === focusEvidenceId)
      : evidence;
  ...
  if (operation=== 'repository_publication.cloudflare_execution.list') {
    ...
    const focusedExecutions = focusExecutionId
      ? executions.filter((entry: any) => entry?.repository_publication_execution_id === focusExecutionId)
      : executions;
...
export function formatRepositoryPublicationReadText(result: any) {
  ...
  if (summary.operation=== 'repository_publication.request.list') {
    ...
    if (workerUrl && summary.site_id && summary.latest_operation_id) {
      lines.push(`Operation Review: pnpm --filter @narada-core/cloudflare-carrier product:operation:read:text -- --url ${workerUrl} --site ${summary.site_id} --operation-id ${summary.latest_operation_id} --operator-session-file <operator-session-file>`);
      lines.push(`Operation Next Workflow: pnpm --filter @narada-core/cloudflare-carrier product:operation:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --operation-id ${summary.latest_operation_id} --operator-session-file <operator-session-file> --execute-operation-next`);
    }
    ...
    if (workerUrl && summary.site_id && summary.latest_repository_publication_request_id) {
      lines.push(`Request Read: pnpm --filter @narada-core/cloudflare-carrier product:repository-publication:request:review:text -- --url ${workerUrl} --site ${summary.site_id} --repository-publication-request-id ${summary.latest_repository_publication_request_id} --operator-session-file <operator-session-file>`);
    }
    ...
    if (workerUrl && summary.site_id && summary.repository_publication_request_id) {
      lines.push(`Request Read: pnpm --filter @narada-core/cloudflare-carrier product:repository-publication:request:review:text -- --url ${workerUrl} --site ${summary.site_id} --repository-publication-request-id ${summary.repository_publication_request_id} --operator-session-file <operator-session-file>`);
    }
...
function labelForOperation(operation: any) {
  switch (operation) {
...
function parseOptionalInteger(value: any, fieldName: any) {
...
function parseJsonText(text: any) {
  try {
