...
export function parseLocalIngressEvidenceReadArgs(argv: any= [], env: any= process.env) {
  const config: any = parseProductReadArgs(['--operation', 'local_ingress.evidence.list', ...argv], env);
...
export async function readLocalIngressEvidence(config: any, fetchImpl: any= fetch) {
  ...
  if (
    ...
    && !evidence.some((entry: any) => entry?.local_ingress_evidence_id === config.params.local_ingress_evidence_id)
  ) {
...
export function summarizeLocalIngressEvidence(body: any= {}, options: any= {}) {
  ...
  const focusedEvidence = focusEvidenceId
    ? evidence.filter((entry: any) => entry?.local_ingress_evidence_id === focusEvidenceId)
    : focusRequestId
      ? evidence.filter((entry: any) => entry?.local_ingress_request_id === focusRequestId)
      : evidence;
...
export function formatLocalIngressEvidenceReadText(result: any) {
  ...
  if (workerUrl && hasSiteId && summary.focused_request_id) {
    lines.push(`Request Read: pnpm --filter @narada-core/cloudflare-carrier product:local-ingress:request:review:text -- --url ${workerUrl} --site ${summary.site_id} --local-ingress-request-id ${summary.focused_request_id} --operator-session-file <operator-session-file>`);
  }
