...
export function parseOperationFocusReviewArgs(argv: any= [], env: any= process.env, now: any= () => Date.now()) {
  const args = [...argv];
...
export function buildOperationFocusReviewParams(input: any= {}) {
  const params: any = { site_id: input.siteId };
...
export async function callOperationFocusReview(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`operation_focus_review_failed:${code}`);
    error.code = code;
...
export function summarizeOperationFocusReview(operation: any, body: any= {}, params: any= {}) {
...
export function formatOperationFocusReviewText(result: any) {
  ...
  if (workerUrl && summary.site_id) {
    lines.push(`Review List: pnpm --filter @narada-core/cloudflare-carrier product:operation:focus-review:text -- --operation operation_focus_review.list --url ${workerUrl} --site ${summary.site_id} --operator-session-file <operator-session-file>`);
  }
  return `${lines.join('\n')}\n`;
}
