...
export async function runCloudflareSiteRegistryProjectionCli(argv: any= process.argv.slice(2), env: any= process.env) {
...
export function resolveCloudflareSiteRegistryProjectionInputs({ args = {}, env = process.env, repoRoot = process.cwd() }: any= {}) {
...
async function postCarrier({ workerUrl, bearerToken, body, fetchImpl }: any) {
...
function resolveBearerToken({ args, env, repoRoot }: any) {
...
function loadLocalEnv(envPath: any, env: any) {
...
function requestTimestamp(now: any) {
