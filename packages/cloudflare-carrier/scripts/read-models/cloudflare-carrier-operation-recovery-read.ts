...
export function parseOperationRecoveryReadArgs(argv: any= [], env: any= process.env) {
  return parseProductReadArgs(['--operation', 'operation.read', ...argv], env);
}

export async function readOperationRecovery(config: any, fetchImpl: any= fetch) {
...
export function summarizeOperationRecovery(body: any= {}, options: any= {}) {
...
export function formatOperationRecoveryReadText(result: any) {
  ...
  for (const { label, command } of buildOperationRecoveryWorkflowLinks(result, summary)) {
...
function buildOperationRecoveryWorkflowLinks(result: any, summary: any) {
  const workerUrl = result?.worker_url ?? null;
