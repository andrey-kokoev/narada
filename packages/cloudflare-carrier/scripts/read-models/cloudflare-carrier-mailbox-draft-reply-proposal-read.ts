...
export function parseMailboxDraftReplyProposalReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readMailboxDraftReplyProposal(config: any, fetchImpl: any= fetch) {
  ...
  if (config.focusRef && !proposals.some((entry: any) => entry?.proposal_id === config.focusRef)) {
...
export function summarizeMailboxDraftReplyProposal(body: any= {}, draftCreateBody: any= null, options: any= {}) {
  ...
  const exactFocusedProposals = focusRef
    ? proposals.filter((entry: any) => entry?.proposal_id === focusRef)
    : [];
  ...
  const linkedDraftCreates = focusedProposal
    ? draftCreates.filter((entry: any) => entry?.proposal_id === focusedProposal.proposal_id)
    : [];
  const linkedSendAcceptedIds = linkedDraftCreates.map((entry: any) => entry?.send_accepted_id).filter(Boolean);
  const linkedSendConfirmationIds = linkedDraftCreates.map((entry: any) => entry?.send_confirmation_id).filter(Boolean);
  const latestFocusReview = focusedProposal
    ? focusReviews.find((entry: any) => entry?.focus_kind === 'mailbox_draft_reply_proposal'
  ...
  return {
    ...
    linked_draft_create_ids: linkedDraftCreates.map((entry: any) => entry?.draft_create_id).filter(Boolean),
...
export function formatMailboxDraftReplyProposalReadText(result: any) {
  ...
  if (workerUrl && hasSiteId && summary.linked_draft_create_ids?.[0]) {
    lines.push(`Draft Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:outlook-draft:text -- --url ${workerUrl} --site ${summary.site_id} --focus-ref ${summary.linked_draft_create_ids[0]} --operator-session-file <operator-session-file>`);
  }
  if (workerUrl && hasSiteId && summary.linked_send_accepted_ids?.[0]) {
    lines.push(`Accepted Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:send-accepted:text -- --url ${workerUrl} --site ${summary.site_id} --focus-ref ${summary.linked_send_accepted_ids[0]} --operator-session-file <operator-session-file>`);
  }
  if (workerUrl && hasSiteId && summary.linked_send_confirmation_ids?.[0]) {
    lines.push(`Confirmation Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:send-confirmation:text -- --url ${workerUrl} --site ${summary.site_id} --focus-ref ${summary.linked_send_confirmation_ids[0]} --operator-session-file <operator-session-file>`);
  }
...
function selectFocusedProposal(proposals: any, focusRef: any) {
  if (!Array.isArray(proposals) || proposals.length === 0) return null;
  if (focusRef) {
    const exact = proposals.find((entry: any) => entry?.proposal_id === focusRef);
    if (exact) return exact;
  }
  return proposals[0] ?? null;
}

function listMailboxDraftReplyProposals(body: any= {}) {
...
function listMailboxOutlookDraftCreates(body: any= {}) {
