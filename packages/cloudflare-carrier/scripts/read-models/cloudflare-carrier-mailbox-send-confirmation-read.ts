...
export function parseMailboxSendConfirmationReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
  const parsed: any = parseProductReadArgs(['--operation', 'mailbox.send_confirmation.list', ...argv], env);
...
export async function readMailboxSendConfirmation(config: any, fetchImpl: any= fetch) {
  const product = await readProductSurface(config, fetchImpl);
  const confirmations = listMailboxSendConfirmations(product.response);
  if (config.focusRef && !confirmations.some((entry: any) => entry?.send_confirmation_id === config.focusRef)) {
...
export function summarizeMailboxSendConfirmation(body: any= {}, options: any= {}) {
  const confirmations = listMailboxSendConfirmations(body);
  const focusRef = options.focusRef ?? null;
  const focused = focusRef
    ? confirmations.find((entry: any) => entry?.send_confirmation_id === focusRef) ?? null
    : null;
...
export function formatMailboxSendConfirmationReadText(result: any) {
...
function listMailboxSendConfirmations(body: any= {}) {
