...
export function parseTaskLifecycleReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readCloudflareTaskLifecycle(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`task_lifecycle_read_request_failed:${code}`);
    error.code = code;
...
export function summarizeTaskLifecycleRead(body: any= {}, params: any= {}) {
  ...
  const sessionFocusedTask = params.carrier_session_id
    ? newestTasks.find((entry: any) => entry?.carrier_session_id === params.carrier_session_id) ?? null
    : null;
  const openStatuses = new Set(['open', 'opened', 'claimed', 'active', 'needs_continuation']);
  const operationFocusedTask = params.operation_id
    ? newestTasks.find((entry: any) => openStatuses.has(String(entry?.status ?? '').toLowerCase())) ?? newestTasks[0] ?? null
    : null;
  const focusedTask = tasks.find((entry: any) => entry?.task_id === params.task_lifecycle_task_id)
    ?? sessionFocusedTask
    ?? operationFocusedTask
    ?? tasks[0]
    ?? null;
  const openTaskCount = tasks.filter((entry: any) => openStatuses.has(String(entry?.status ?? '').toLowerCase())).length;
  return {
    ok: body.ok ?? null,
    code: body.code ?? null,
...
export function formatTaskLifecycleReadText(result: any) {
...
function formatTaskLifecycleNextCommands(result: any, summary: any) {
  const workerUrl = result?.worker_url ?? null;
