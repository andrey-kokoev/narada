...
export function parseSiteFileChangeProposalReviewArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readSiteFileChangeProposalReview(config: any, fetchImpl: any= fetch) {
...
export function summarizeSiteFileChangeProposalReview(operationBody: any= {}, proposalsBody: any= {}, materializationsBody: any= {}, options: any= {}) {
  ...
  const linkedMaterializations = focusedProposalId
    ? materializations.filter((entry: any) => entry?.proposal_id === focusedProposalId)
    : [];
  const latestFocusReview = focusedProposalId
    ? focusReviews.find((entry: any) => entry?.focus_kind === 'site_file_change_proposal'
...
export function formatSiteFileChangeProposalReviewText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.focused_proposal_id) {
