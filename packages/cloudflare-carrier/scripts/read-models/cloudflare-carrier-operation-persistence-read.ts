...
export function parseOperationPersistenceReadArgs(argv: any= [], env: any= process.env) {
  return parseProductReadArgs(['--operation', 'operation.read', ...argv], env);
}

export async function readOperationPersistence(config: any, fetchImpl: any= fetch) {
...
export function summarizeOperationPersistence(body: any= {}, options: any= {}) {
  ...
  return {
    ...
    persistence_durable_boundary_keys: durableBoundaries.map((boundary: any) => boundary?.key).filter(Boolean),
    persistence_next_action: persistence?.next_action ?? null,
  };
}

export function formatOperationPersistenceReadText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.operation_id) {
    ...
    lines.push(`Evidence Read: pnpm --filter @narada-core/cloudflare-carrier product:operation:evidence:text -- --url ${workerUrl} --site ${summary.site_id} --operation-id ${summary.operation_id} --operator-session-file <operator-session-file>`);
...
function buildOperationPersistenceWorkflowLinks(result: any, summary: any) {
  const workerUrl = result?.worker_url ?? null;
