...
export function parseRepositoryPublicationReadinessArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readCloudflareRepositoryPublicationReadiness(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`repository_publication_readiness_failed:${code}`);
    error.code = code;
...
export function summarizeRepositoryPublicationReadiness(body: any= {}, params: any= {}) {
  return {
    ok: body.ok ?? null,
    code: body.code ?? null,
...
export function formatRepositoryPublicationReadinessText(result: any) {
  ...
  return [
    ...
    ...summary.missing_configuration.map((entry: any) => `Missing Configuration: ${entry}`),
...
function redactRepositoryPublicationReadinessParams(params: any= {}) {
  return { ...params };
}
