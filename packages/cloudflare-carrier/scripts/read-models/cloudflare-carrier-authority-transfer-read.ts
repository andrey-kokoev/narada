...
import { readRepositoryPublicationSurface } from './cloudflare-carrier-repository-publication-read.ts';
import { readCloudflareRepositoryPublicationReadiness } from './cloudflare-carrier-repository-publication-readiness.ts';
import { readProductSurface } from './cloudflare-carrier-product-read.ts';

export function parseAuthorityTransferReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readAuthorityTransfer(config: any, fetchImpl: any= fetch) {
...
export function summarizeAuthorityTransfer(operationBody: any= {}, readinessSummary: any= {}) {
...
export function formatAuthorityTransferText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.operation_id) {
    ...
    lines.push(`Mailbox Readback Smoke: pnpm --filter @narada-core/cloudflare-carrier mailbox:readback-smoke:live:text -- --url ${workerUrl} --site ${summary.site_id} --operation ${summary.operation_id} --operator-session-file <operator-session-file>`);
  }
  if (workerUrl && summary.site_id && isAuthorityTransferWorkflowAction(summary.next_action)) {
    lines.push(`Site Action Workflow: pnpm --filter @narada-core/cloudflare-carrier product:site:action:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id}${summary.operation_id ? ` --operation-id ${summary.operation_id}` : ''} --operator-session-file <operator-session-file> --execute-site-action`);
  }
...
function isAuthorityTransferWorkflowAction(action: any) {
  return typeof action === 'string'
    && (action.startsWith('transfer_') || action === 'continue_authority_transfer' || action === 'verify_full_cloudflare_authority');
}

async function inferRepositoryPublicationTarget(config: any, fetchImpl: any) {
  ...
  const focusedRequest = requests.find((entry: any) => entry?.operation_id === config.operationParams.operation_id) ?? null;
  return {
...
function redactAuthorityTransferParams(config: any, resolvedReadinessParams: any= config.readinessParams) {
  return {
