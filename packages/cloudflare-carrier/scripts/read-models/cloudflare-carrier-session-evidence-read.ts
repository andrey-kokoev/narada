...
export function parseSessionEvidenceReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readSessionEvidence(config: any, fetchImpl: any= fetch) {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const error: any = new Error(`session_evidence_read_request_failed:${code}`);
    error.code = code;
...
export function summarizeSessionEvidence(body: any= {}, config: any= {}) {
  ...
  return {
    ...
    event_kind_counts: countBy(events, (event: any) => event?.event_kind ?? 'unknown'),
  };
}

export function formatSessionEvidenceText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.carrier_session_id) {
    lines.push(
      `Task Review: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:review:text -- --url ${workerUrl} --site ${summary.site_id} --carrier-session-id ${summary.carrier_session_id} --operator-session-file <operator-session-file>`,
    );
    lines.push(
      `Task Workflow: pnpm --filter @narada-core/cloudflare-carrier product:task-lifecycle:next:workflow:live:text -- --url ${workerUrl} --site ${summary.site_id} --carrier-session-id ${summary.carrier_session_id} --agent-id <agent-id> --operator-session-file <operator-session-file> --execute-task-lifecycle-next`,
    );
  }
...
function parseOptionalInteger(value: any, label: any, { minimum = 0 }: any= {}) {
...
function formatKeyValueMap(entries: any) {
  const items = Object.entries(entries ?? {});
  if (items.length === 0) return 'none';
  return items.map(([key, value]: any) => `${key}=${value}`).join(' ');
}
