...
export function parseMailboxOutlookDraftReadArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
  const parsed: any = parseProductReadArgs(['--operation', 'mailbox.outlook_draft.list', ...argv], env);
...
export async function readMailboxOutlookDraft(config: any, fetchImpl: any= fetch) {
  const product = await readProductSurface(config, fetchImpl);
  const drafts = listMailboxOutlookDrafts(product.response);
  if (config.focusRef && !drafts.some((entry: any) => entry?.draft_create_id === config.focusRef)) {
...
export function summarizeMailboxOutlookDraft(body: any= {}, options: any= {}) {
  const drafts = listMailboxOutlookDrafts(body);
  const focusRef = options.focusRef ?? null;
  const focused = focusRef
    ? drafts.find((entry: any) => entry?.draft_create_id === focusRef) ?? null
    : null;
...
export function formatMailboxOutlookDraftReadText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.latest_send_accepted_id) {
    lines.push(`Accepted Read: pnpm --filter @narada-core/cloudflare-carrier product:mailbox:send-accepted:text -- --url ${workerUrl} --site ${summary.site_id} --focus-ref ${summary.latest_send_accepted_id} --operator-session-file <operator-session-file>`);
  }
...
function listMailboxOutlookDrafts(body: any= {}) {
