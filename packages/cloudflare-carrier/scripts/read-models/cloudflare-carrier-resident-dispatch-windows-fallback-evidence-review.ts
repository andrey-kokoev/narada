...
export function parseResidentDispatchWindowsFallbackEvidenceReviewArgs(argv: any= [], env: any= process.env) {
  const args = [...argv];
...
export async function readResidentDispatchWindowsFallbackEvidenceReview(config: any, fetchImpl: any= fetch) {
...
export function summarizeResidentDispatchWindowsFallbackEvidenceReview(operationBody: any= {}, evidenceBody: any= {}, options: any= {}) {
  ...
  const exactFocusedEvidence = focusRef
    ? evidence.find((entry: any) => entry?.fallback_evidence_id === focusRef) ?? null
    : null;
  ...
  const latestFocusReview = focusedEvidenceId
    ? focusReviews.find((entry: any) => entry?.focus_kind === FOCUS_KIND && entry?.focus_ref === focusedEvidenceId) ?? null
    : null;
  return {
...
export function formatResidentDispatchWindowsFallbackEvidenceReviewText(result: any) {
  ...
  if (workerUrl && summary.site_id && summary.focused_fallback_evidence_id) {
    lines.push(`Review Ack: pnpm --filter @narada-core/cloudflare-carrier product:operation:focus-review:text -- --url ${workerUrl} --site ${summary.site_id}${summary.operation_id ? ` --operation-id ${summary.operation_id}` : ''} --focus-kind ${FOCUS_KIND} --focus-ref ${summary.focused_fallback_evidence_id} --operator-session-file <operator-session-file>`);
  }
  return `${lines.join('\n')}\n`;
}

function selectFocusedEvidence(evidence: any, focusRef: any) {
  if (!Array.isArray(evidence) || evidence.length === 0) return null;
  if (focusRef) {
    const exact = evidence.find((entry: any) => entry?.fallback_evidence_id === focusRef);
