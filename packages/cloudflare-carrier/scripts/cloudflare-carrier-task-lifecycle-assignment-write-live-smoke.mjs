...
if (!bearerToken) throw new Error('assignment_write_live_smoke_requires_--token_or_--token-file_or_CLOUDFLARE_CARRIER_TOKEN');
...
const title = option('--title') ?? `Cloudflare assignment write ${suffix}`;
...
const refusedAssignment = await postCarrier({
...
const assignment = await postCarrier({
  operation: 'task_lifecycle.assignment_write.admit',
...
assert.equal(operationRead.body.operation_product_surface.task_lifecycle_role_resolution_authority_admission, 'not_admitted');
assert.equal(operationRead.body.operation_product_surface.task_lifecycle_authority_partition, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_cloudflare_remaining_windows_effects');
assert.equal(operationRead.body.operation_product_surface.task_lifecycle_write_admission_posture, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted_remaining_external_effects_not_admitted');
assert.equal(operationRead.body.operation_product_surface.task_lifecycle_cloudflare_write_admission, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted');
