...
export class CloudflareCarrierDurableObjectBase {
  ...
  constructor(state: any, env: any= {}, dependencies: any= {}) {
    this.state = state;
    this.env = env;
  ...
  async #handleInLane(request: any) {
  ...
  async #loadOrCreateSession(request: any) {
  ...
  async #storeSnapshot(session: any) {
    await this.state.storage.put(SNAPSHOT_KEY, session.snapshot());
  }

  async #scheduleOperationHeartbeatAlarm(session: any) {
