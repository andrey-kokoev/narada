...
} from './cloudflare-intelligence-management-api.ts';
import worker, { createCloudflareAiProviderAdapter } from './cloudflare-worker.ts';
...
test('D1 management API deploys canonical production authority without conflating transport and semantic actors', async () => {
  ...
  assert.ok(response.body.result.receipts.every(({ actor_id }: any) => actor_id !== PRINCIPAL.principal_id));
...
test('authenticated execution.read returns linked D1 plan, outcome, evidence, and Site provenance', async () => {
  ...
  assert.deepEqual(readback.body.result.data.transitions.map(({ state }: any) => state), [
  ...
  assert.ok(readback.body.result.data.provenance.materializations.every(({ destination }: any) =>
...
test('Worker exposes the management service only through authenticated POST /api/intelligence', async () => {
  ...
  const request = (token: any) => new Request('https://carrier.test/api/intelligence', {
    method: 'POST',
    headers: {
