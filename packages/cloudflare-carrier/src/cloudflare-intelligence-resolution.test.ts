...
} from './cloudflare-intelligence-resolution.ts';
...
const PRODUCTION_TOPOLOGY = PRODUCTION_CATALOG.records.find(
  ({ record_kind }: any) => record_kind === 'route',
).document.topology;

function makeAi(resultOrError: any, calls: any= []) {
  return {
    calls,
...
function catalogWithTimeout(timeoutMs: any) {
  const catalog = structuredClone(PRODUCTION_CATALOG);
  const defaults = catalog.records.find(({ record_id }: any) => record_id === 'policy:narada-cloudflare-defaults');
  const rule = defaults.document.rules.find(({ option }: any) => option === 'timeout_ms');
  rule.value = timeoutMs;
  defaults.source.digest = canonicalSha256(defaults.document);
  return catalog;
}

function makeSequenceAi(sequence: any, calls: any= []) {
  return {
    calls,
    async run(model: any, request: any) {
      calls.push({ model, request });
...
function configuredEnv(overrides: any= {}) {
  return {
...
function carrierContext(overrides: any= {}) {
  return {
...
function productionDeploymentBundle(catalog: any= PRODUCTION_CATALOG): any {
  return {
...
async function seedRegistry(binding: any, catalog: any= PRODUCTION_CATALOG) {
  ...
  try {
    const result = await deployManagementBundle({
      store,
      materialization,
      owningSite: { kind: 'site' as const, id: TARGET_SITE_ID },
...
test('D1 catalog selects the exact offering key; model environment variables have no authority', async () => {
  ...
  const adapter: any = createCloudflareAiProviderAdapter(env);
  ...
  const plan: any = await store.getPlan(result.intelligence.plan_id);
  ...
  const outcome: any = await store.getTerminalOutcomeByAttempt(attempts[0].id);
  assert.equal(outcome.kind, 'success');
  const observations = await store.listInvocationObservations(attempts[0].id);
  assert.ok(observations.length > 0);
  assert.ok(observations.flatMap(({ evidence }: any) => evidence ?? []).every(({ evidence_class }: any) => (
...
test('service bearer resolves through its dedicated workload principal scope', async () => {
  ...
  const store = await D1RegistryStore.open(env.INTELLIGENCE_REGISTRY_DB);
  const plan: any = await store.getPlan(result.intelligence.plan_id);
  const intent: any = await store.getIntent(plan.intent_id);
...
test('provider timeout persists acknowledgment uncertainty rather than provider failure', async () => {
  ...
  let refs: any;
  await assert.rejects(adapter.run(invocation({ operation_id: 'operation:test:timeout' })), (error: any) => {
  ...
  const store = await D1RegistryStore.open(env.INTELLIGENCE_REGISTRY_DB);
  const outcome: any = await store.getTerminalOutcome(refs.outcome_id);
  ...
  assert.equal(
    observations.find(({ kind }: any) => kind === 'transport-acknowledgment')?.status,
    'uncertain',
  );
  ...
  for (const attemptId of [failure.attempt_id, uncertain.attempt_id, recovery.intelligence.attempt_id]) {
    assert.equal(
      (await store.listInvocationObservations(attemptId))
        .find(({ kind }: any) => kind === 'transport-submitted')?.status,
      'not-observed',
    );
  }
...
test('explicit retry appends lineage and preserves the failed predecessor', async () => {
  ...
  const failedOutcome: any = await store.getTerminalOutcome(failedRefs.outcome_id);
  const recoveredOutcome: any = await store.getTerminalOutcome(recovered.intelligence.outcome_id);
...
test('Cloudflare route factorizes runtime topology from provider account access', () => {
  assert.deepEqual(
    PRODUCTION_TOPOLOGY.nodes.map(({ kind }: any) => kind),
    ['client', 'launcher', 'carrier', 'runtime', 'adapter', 'inference-service', 'endpoint'],
  );
  const route = PRODUCTION_CATALOG.records.find(({ record_kind }: any) => record_kind === 'route').document;
  ...
  assert.equal(route.access.credential.id, 'credential-locator:cloudflare-worker-binding');
  assert.deepEqual(PRODUCTION_TOPOLOGY.edges.find(({ id }: any) => id === 'c2').boundary.kinds, [
    'network', 'trust', 'site', 'account',
  ]);
  assert.equal(PRODUCTION_TOPOLOGY.edges.find(({ id }: any) => id === 'c5').kind, 'binding-call');
  assert.deepEqual(PRODUCTION_TOPOLOGY.edges.find(({ id }: any) => id === 'c5').boundary.kinds, ['account', 'trust']);
});
...
test('generated Cloudflare boundary admissions remain valid after the deployment day', () => {
  ...
  for (const edgeId of ['c2', 'c5']) {
    const admission = PRODUCTION_TOPOLOGY.edges.find(({ id }: any) => id === edgeId).boundary.admission;
...
test('same delivery identity is not redispatched and reports metadata-only replay', async () => {
  ...
  const store = await D1RegistryStore.open(env.INTELLIGENCE_REGISTRY_DB);
  const plan: any = await store.getPlan(first.intelligence.plan_id);
...
test('inference failure is durable and distinct from admission refusal', async () => {
  ...
  let refs: any;
  await assert.rejects(adapter.run(invocation({ operation_id: 'operation:test:failure' })), (error: any) => {
    assert.equal(error.code, 'cloudflare_workers_ai_provider_failed');
    refs = error.intelligence;
    return true;
  });
  ...
  const store = await D1RegistryStore.open(env.INTELLIGENCE_REGISTRY_DB);
  const attempt: any = await store.getExecutionAttempt(refs.attempt_id);
  const outcome: any = await store.getTerminalOutcome(refs.outcome_id);
...
test('runtime refuses an empty D1 registry and never synthesizes catalog authority', async () => {
  ...
  await assert.rejects(adapter.run(invocation()), (error: any) => {
