...
async function readExecution(store: any, materialization: any, site: any, request: any) {
  ...
  const routeRevision = snapshot.referenced_revisions.find(({ kind, record_id }: any) =>
  ...
  const routeRecord = routeRevision
    ...
    : null;
  const materializationRevisions = snapshot.referenced_revisions.filter(({ kind }: any) => kind === 'materialization');
  const projections = await Promise.all(materializationRevisions.map(({ immutable_ref }: any) =>
    materialization.getProjectionByEnvelope(immutable_ref)));
  if (!routeRecord || projections.length === 0 || projections.some((projection: any) => !projection)) {
    throw new ManagementError(
      'execution-provenance-unavailable',
      `Execution attempt '${attemptId}' cannot be linked to all immutable route and materialization inputs.`,
      [attemptId, ...(routeRevision ? [routeRevision.immutable_ref] : []), ...materializationRevisions.map(({ immutable_ref }: any) => immutable_ref)],
    );
  }
  if (
    routeRecord.authority?.site_id !== site.id
    || projections.some((projection: any) => projection.envelope.destination.site_id !== site.id)
  ) {
    throw new ManagementError(
      'foreign-locus-read',
  ...
  return {
    ...
    data: {
      ...
      provenance: {
        ...
        materializations: projections.map((projection: any) => ({
...
function statusForManagementResult(result: any) {
...
function apiResponse(site: any, transportAuthorization: any, result: any, status: any= statusForManagementResult(result)) {
  return {
    status,
    body: {
...
function managedRefusal(code: any, message: any, evidenceRefs: any= []) {
  return managementErrorResult(new ManagementError(code, message, evidenceRefs));
}

async function transportSiteIdentity(env: any, body: any, site: any) {
  ...
  if (body?.schema === MANAGEMENT_DEPLOYMENT_BUNDLE_SCHEMA) {
    siteResource = body.catalog?.records
      ...
      .find((document: any) => document?.schema === 'narada.invokable-intelligence.site.v1' && document.id === site.id) ?? null;
  ...
  const bindings = siteResource?.registry_bindings?.filter(({ registry, subject_id }: any) =>
...
async function authorizeSiteMutation(env: any, site: any, transportSite: any, principal: any) {
...
export async function executeCloudflareIntelligenceManagement(body: any, principal: any, env: any= {}) {
