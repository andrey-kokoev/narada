...
} from './cloudflare-product-operation-registry.ts';
...
test('product operation registry has one discoverable owner for every registered operation', async () => {
  ...
  assert.deepEqual(
    [...new Set(definitions.map((definition: any) => definition.bounded_context))].sort(),
    [...CLOUDFLARE_PRODUCT_OPERATION_CONTEXTS].sort(),
  );
  assert.equal(new Set(definitions.map((definition: any) => definition.operation)).size, definitions.length);
