...
export function fakeD1SiteRegistryDatabase(initial: any= {}) {
  const state = {
...
function fakeD1SiteRegistryStatement(state: any, sql: any) {
  ...
  return {
    ...
    async run() {
      if (normalized.startsWith('insert into cloudflare_site_memberships')) {
        ...
        const existing = state.memberships.find((entry: any) => entry.site_id === site_id && entry.principal_id === principal_id);
        ...
        const existing = state.operations.find((entry: any) => entry.operation_id === operation_id);
        ...
        const existing = state.carrierSessions.find((entry: any) => entry.carrier_session_id === carrier_session_id);
        ...
        if (!state.carrierSessions.some((entry: any) => entry.carrier_session_id === carrier_session_id)) {
        ...
        const existing = state.continuityPackets.find((entry: any) => entry.packet_id === packet_id);
        ...
        const existing = state.continuityLoopReports.find((entry: any) => entry.report_id === report_id);
        ...
        const existing = state.continuityReconciliationExecutions.find((entry: any) => entry.execution_id === execution_id);
        ...
        const existing = state.webhookDelayShadowObservations.find((entry: any) => entry.observation_id === observation_id);
        ...
        const existing = state.webhookDelayRemoteSourceSamples.find((entry: any) => entry.sample_id === sample_id);
        ...
        const existing = state.webhookDelayScheduledSourceReads.find((entry: any) => entry.scheduled_run_id === scheduled_run_id);
        ...
        const existing = state.webhookDelayObservationPrimaryReads.find((entry: any) => entry.observation_id === observation_id);
        ...
        const existing = state.webhookDelayDirectiveRecords.find((entry: any) => entry.directive_record_id === directive_record_id);
        ...
        const existing = state.webhookDelayDirectiveDeliveries.find((entry: any) => entry.delivery_id === delivery_id);
        ...
        const existing = state.residentLoopShadowRuns.find((entry: any) => entry.loop_run_id === loop_run_id);
        ...
        const existing = state.mailboxStatusShadowReads.find((entry: any) => entry.read_id === read_id);
        ...
        const existing = state.mailboxStatusSourceReads.find((entry: any) => entry.read_id === read_id);
        ...
        const existing = state.mailboxDraftReplyProposals.find((entry: any) => entry.proposal_id === proposal_id);
        ...
        const existing = state.mailboxOutlookDraftCreates.find((entry: any) => entry.draft_create_id === draft_create_id);
        ...
        const existing = state.mailboxSendAcceptedRecords.find((entry: any) => entry.send_accepted_id === send_accepted_id);
        ...
        const existing = state.mailboxSendConfirmations.find((entry: any) => entry.send_confirmation_id === send_confirmation_id);
        ...
        const existing = state.mailboxSendReviews.find((entry: any) => entry.review_id === review_id);
        ...
        const existing = state.operationFocusReviews.find((entry: any) => entry.review_id === review_id);
        ...
        const existing = state.siteFileChangeProposals.find((entry: any) => entry.proposal_id === proposal_id);
        ...
        const existing = state.siteFileMaterializations.find((entry: any) => entry.materialization_id === materialization_id);
        ...
        const existing = state.localIngressRequests.find((entry: any) => entry.local_ingress_request_id === local_ingress_request_id);
        ...
        const existing = state.localIngressEvidence.find((entry: any) => entry.local_ingress_evidence_id === local_ingress_evidence_id);
        ...
        const existing = state.localIngressProviderHeartbeats.find((entry: any) => entry.local_ingress_provider_heartbeat_id === local_ingress_provider_heartbeat_id);
        ...
        const existing = state.repositoryPublicationRequests.find((entry: any) => entry.repository_publication_request_id === repository_publication_request_id);
        ...
        const existing = state.repositoryPublicationAdmissions.find((entry: any) => entry.repository_publication_admission_id === repository_publication_admission_id);
        ...
        const existing = state.repositoryPublicationExecutions.find((entry: any) => entry.repository_publication_execution_id === repository_publication_execution_id);
        ...
        const existing = state.repositoryPublicationEvidence.find((entry: any) => entry.repository_publication_evidence_id === repository_publication_evidence_id);
        ...
        const existing = state.repositoryPublicationProviderHeartbeats.find((entry: any) => entry.repository_publication_provider_heartbeat_id === repository_publication_provider_heartbeat_id);
        ...
        const existing = state.taskLifecycleShadowReads.find((entry: any) => entry.read_id === read_id);
        ...
        const existing = state.taskLifecycleWriteAdmissions.find((entry: any) => entry.admission_id === admission_id);
        ...
        const existing = state.taskLifecycleTasks.find((entry: any) => entry.site_id === site_id && entry.task_id === task_id);
        ...
        const existing = state.residentDispatchDecisions.find((entry: any) => entry.dispatch_decision_id === dispatch_decision_id);
        ...
        const existing = state.residentDispatchWindowsFallbackRequests.find((entry: any) => entry.fallback_request_id === fallback_request_id);
        ...
        const existing = state.residentDispatchWindowsFallbackEvidence.find((entry: any) => entry.fallback_evidence_id === fallback_evidence_id);
        ...
        const existing = state.carrierSessionEvents.find((entry: any) => entry.carrier_session_id === carrier_session_id && Number(entry.sequence) === Number(sequence));
    ...
    async first() {
      ...
      if (normalized.includes('max(task_number)') && normalized.includes('from cloudflare_task_lifecycle_tasks')) {
        ...
        const maxTaskNumber = state.taskLifecycleTasks
          .filter((entry: any) => entry.site_id === siteId)
          .reduce((max: any, entry: any) => Math.max(max, Number(entry.task_number)), 0);
      ...
      if (normalized.includes('from cloudflare_task_lifecycle_tasks') && normalized.includes('task_id = ?')) {
        ...
        return clone(state.taskLifecycleTasks.find((entry: any) => entry.site_id === siteId && entry.task_id === taskId));
      }
      if (normalized.includes('from cloudflare_mailbox_send_accepted_records') && normalized.includes('send_accepted_id = ?')) {
        ...
        return clone(state.mailboxSendAcceptedRecords.find((entry: any) => entry.site_id === siteId && entry.send_accepted_id === sendAcceptedId));
      }
      return null;
    },
    async all() {
      if (normalized.includes('from cloudflare_sites s join cloudflare_site_memberships m')) {
        ...
        const visibleSiteIds = new Set(state.memberships
          .filter((membership: any) => membership.principal_id === principalId && membership.status === 'active')
          .map((membership: any) => membership.site_id));
        return {
          results: state.sites
            .filter((site: any) => visibleSiteIds.has(site.site_id) && site.status === 'active')
            .sort((left: any, right: any) => left.created_at.localeCompare(right.created_at))
            .map((site: any) => clone(site)),
        };
      }
        ...
        if (normalized.includes('where operation_id = ?')) {
          ...
          return {
            results: state.carrierSessions
              .filter((entry: any) => entry.operation_id === operationId)
              .sort((left: any, right: any) => right.created_at.localeCompare(left.created_at))
      ...
      if (normalized.includes('from cloudflare_mailbox_status_source_reads')) {
        ...
        return {
          results: state.mailboxStatusSourceReads
            .filter((entry: any) => entry.site_id === siteId)
            .sort((left: any, right: any) => right.recorded_at.localeCompare(left.recorded_at) || right.generated_at.localeCompare(left.generated_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      ...
      if (normalized.includes('from cloudflare_site_continuity_reconciliation_executions')) {
        ...
        return {
          results: state.continuityReconciliationExecutions
            .filter((entry: any) => entry.site_id === siteId)
            .sort((left: any, right: any) => right.recorded_at.localeCompare(left.recorded_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      ...
      if (normalized.includes('from cloudflare_carrier_session_events')) {
        ...
        return {
          results: state.carrierSessionEvents
            .filter((entry: any) => entry.carrier_session_id === carrierSessionId)
            .sort((left: any, right: any) => Number(left.sequence) - Number(right.sequence))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      ...
      if (normalized.includes('from cloudflare_site_settings')) {
        const [siteId] = bindings;
        return { results: state.settings.filter((entry: any) => entry.site_id === siteId).map((entry: any) => clone(entry)) };
      }
      ...
      if (normalized.includes('from cloudflare_site_continuity_packets')) {
        ...
        return {
          results: state.continuityPackets
            .filter((entry: any) => entry.site_id === siteId)
            .sort((left: any, right: any) => right.imported_at.localeCompare(left.imported_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      ...
      if (normalized.includes('from cloudflare_webhook_delay_remote_source_samples')) {
        ...
        return {
          results: state.webhookDelayRemoteSourceSamples
            .filter((entry: any) => entry.site_id === siteId && entry.source_adapter_id === sourceAdapterId)
            .sort((left: any, right: any) => right.observed_at.localeCompare(left.observed_at) || right.recorded_at.localeCompare(left.recorded_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      if (normalized.includes('from cloudflare_webhook_delay_scheduled_source_reads')) {
        ...
        return {
          results: state.webhookDelayScheduledSourceReads
            .filter((entry: any) => entry.site_id === siteId)
            .sort((left: any, right: any) => right.recorded_at.localeCompare(left.recorded_at) || right.scheduled_at.localeCompare(left.scheduled_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
      ...
      if (normalized.includes('from cloudflare_local_ingress_evidence')) {
        ...
        return {
          results: state.localIngressEvidence
            .filter((entry: any) => entry.site_id === siteId && (!requestId || entry.local_ingress_request_id === requestId))
            .sort((left: any, right: any) => right.recorded_at.localeCompare(left.recorded_at) || right.generated_at.localeCompare(left.generated_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      ...
      if (normalized.includes('from cloudflare_repository_publication_admissions')) {
        ...
        return {
          results: state.repositoryPublicationAdmissions
            .filter((entry: any) => entry.site_id === siteId && (!requestId || entry.repository_publication_request_id === requestId))
            .sort((left: any, right: any) => right.recorded_at.localeCompare(left.recorded_at) || right.generated_at.localeCompare(left.generated_at))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      ...
      if (normalized.includes('from cloudflare_task_lifecycle_tasks')) {
        ...
        return {
          results: state.taskLifecycleTasks
            .filter((entry: any) => entry.site_id === siteId)
            .sort((left: any, right: any) => Number(left.task_number) - Number(right.task_number))
            .slice(0, Number(limit))
            .map((entry: any) => clone(entry)),
        };
      }
...
function fakeD1Statement(rows: any, sql: any) {
  const normalized = sql.replace(/\s+/g, ' ').trim().toLowerCase();
  let bindings: any[] = [];
  return {
    bind(...values: any[]) {
    ...
    async run() {
      if (normalized.startsWith('insert into narada_tasks')) {
        ...
        const row = rows.find((entry: any) => entry.site_id === siteId && entry.task_id === taskId);
    ...
    async first() {
      if (normalized.startsWith('select coalesce(max(task_number)')) {
        const [siteId] = bindings;
        const max = rows.filter((entry: any) => entry.site_id === siteId).reduce((value: any, entry: any) => Math.max(value, Number(entry.task_number)), 0);
      ...
      if (normalized.includes('where site_id = ? and task_number = ?')) {
        ...
        const row = rows.find((entry: any) => entry.site_id === siteId && Number(entry.task_number) === Number(taskNumber));
        return row ? clone(row) : null;
      }
      return null;
    },
