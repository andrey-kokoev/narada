...
export function cloudflareIntelligenceDiagnosticsEnabled(value: any) {
 ...
 */
export function normalizeCloudflareIntelligenceDiagnostic(value: any, principal: any= null, { enabled = false }: any= {}) {
  ...
  if (principal?.auth_type !== 'service') {
    const error: any = new Error('cloudflare_intelligence_diagnostic_requires_service_principal');
  ...
  if (typeof value !== 'string' || !CLOUDFLARE_INTELLIGENCE_DIAGNOSTIC_MODES.includes(value)) {
    const error: any = new Error(`cloudflare_intelligence_diagnostic_unknown_mode:${String(value)}`);
...
export class CloudflareCarrierRouter {
  ...
  constructor({ now = () => new Date().toISOString(), providerAdapter = null, toolEffectAdapter = null, taskStoreAdapter = null, intelligenceDiagnosticsEnabled = false }: any = {}) {
...
export class CloudflareCarrierSession {
  ...
  handle(request: any) {
    ...
    if (MUTATING_OPERATIONS.has(operation)) {
      ...
      if (isPromiseLike(response)) {
        restoreNow = false;
        return response.then((resolved: any) => {
  ...
  status() {
    ...
    if (isPromiseLike(tasks)) return tasks.then((resolvedTasks: any) => this.#statusWithTasks(resolvedTasks));
    return this.#statusWithTasks(tasks);
  }

  #statusWithTasks(tasks: any) {
    return {
      ok: true,
  ...
  readEvents({ after_sequence = 0, limit = 100 }: any = {}) {
    const boundedLimit = Math.max(0, Math.min(Number(limit) || 100, 500));
    const events = this.events
      .filter((event: any) => event.sequence > after_sequence)
      .slice(0, boundedLimit)
      .map((event: any) => clone(event));
    return {
      ok: true,
      events,
  ...
  snapshot() {
    ...
    return {
      state: clone(state),
      processed_requests: [...processed_requests.entries()].map(([key, value]: any) => [key, clone(value)]),
      events: this.events.map((event: any) => clone(event)),
    };
  }

  static fromSnapshot(snapshot: any, { now = () => new Date().toISOString(), providerAdapter = null, toolEffectAdapter = null, taskStoreAdapter = null, intelligenceDiagnosticsEnabled = false }: any = {}) {
    const session = new CloudflareCarrierSession({
    ...
    session.state = {
      ...
      processed_requests: new Map((snapshot.processed_requests ?? []).map(([key, value]: any) => [key, clone(value)])),
      invocation_authority_context: clone(snapshot.state.invocation_authority_context ?? null),
    };
    session.events = (snapshot.events ?? []).map((event: any) => clone(event));
    return session;
  }

  #handleFresh(request: any) {
  ...
  #directiveEmissionTarget(directiveKind: any, params: any= {}) {
    const spec: any = carrierDirectiveEmitterSpec(directiveKind);
  ...
  #ensureDirectiveEmissionRule(request: any= {}) {
    const params = request.params ?? {};
    ...
    let authorization = this.state.directive_emission_authorizations.find((entry: any) => entry.authorization_id === authorizationId) ?? null;
    let rule = this.state.directive_emission_rules.find((entry: any) => entry.rule_id === ruleId) ?? null;
    const events: any[] = [];
    if (!authorization) {
  ...
  #deliverInput(request: any) {
    ...
    const terminal: any = admission.creates_turn
    ...
    if (isPromiseLike(terminal)) {
      return terminal.then((terminalState: any) => ({
        ok: true,
        operation: 'carrier.input.deliver',
        input_event_id: input.event_id,
  ...
  #recordProviderTurn(input: any, events: any, intelligenceInvocation: any= undefined, intelligenceDiagnostic: any= null) {
  ...
  async #recordProviderExecution(input: any, events: any, intelligenceInvocation: any= undefined, intelligenceDiagnostic: any= null) {
  ...
  async #recordProviderToolCalls(toolCalls: any, turnId: any, events: any, startSequence: any= 2) {
  ...
  async #executeToolEffectAndRecordResult(toolCall: any, events: any, { turn_id = null }: any = {}) {
    ...
    const result: any = await executeToolEffect(this.toolEffectAdapter, toolCall, {
      carrier_session_id: this.state.carrier_session_id,
      agent_id: this.state.agent_id,
      site_id: this.state.site_id,
      turn_id,
  ...
  #recordProviderRefusal(input: any, events: any) {
    const turnId = `turn_${input.event_id}`;
    this.state.active_turn = { turn_id: turnId, input_event_id: input.event_id, state: 'active' };
    events.push(this.#appendEvent('turn_started', { turn_id: turnId, input_event_id: input.event_id }));
    events.push(this.#appendEvent('provider_request_recorded', {
  ...
  #executeCommand(request: any) {
    ...
    if (command === '/tasks' || command === 'tasks') {
      ...
      return isPromiseLike(tasks)
        ? tasks.then((resolvedTasks: any) => this.#simpleCommand(command, { tasks: resolvedTasks }))
  ...
  #goalCommand(command: any, args: any, request: any) {
  ...
  async #taskCommand(command: any, args: any) {
    const words = Array.isArray(args) ? args.map(String) : String(args ?? '').split(/\s+/).filter(Boolean);
  ...
  #taskSnapshot(): any {
    ...
    return Promise.resolve(store.list()).then((tasks: any) => clone(tasks));
  }

  #hostCommand(params: any, request: any) {
  ...
  #simpleCommand(command: any, details: any= {}, ok: any= true) {
  ...
  #appendEvent(event_kind: any, payload: any= {}) {
...
function hostCommandPayload({ target, command_text, principal }: any) {
  return {
...
function assertNoSecretValues(value: any) {
...
function toolEffectPosture(toolEffectAdapter: any) {
...
function normalizeProviderToolCall(rawToolCall: any) {
...
async function executeToolEffect(toolEffectAdapter: any, toolCall: any, context: any) {
...
export function classifyCloudflareCarrierControl(request: any= {}) {
...
export function expectedObserverEventKindsForInput(input: any, state: any) {
  const admission = classifyCarrierInputQueueAdmission(input, state);
  return [
    ...admission.queue_events.map((event: any) => event.event_kind),
    ...admission.admission_events.map((event: any) => event.event_kind),
    ...admission.visible_events.map((event: any) => event.event_kind),
  ];
}

export function observerEvidencePayload(input: any, suppression_reason: any= null) {
  return observerPayload(input, suppression_reason ? { suppression_reason } : {});
}

export function isTerminalState(value: any) {
  return TERMINAL_STATES.has(value);
}

export function cloudflareCarrierSessionMutates(operation: any) {
