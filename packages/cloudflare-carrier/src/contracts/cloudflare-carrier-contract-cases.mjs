...
import {
  ...
  fakeD1TaskDatabase,
} from '../cloudflare-d1-test-fixtures.mjs';
import worker, {
...
function carrierTestGroupForName(name) {
...
test('worker site.read composes site sessions tasks authority events and carrier evidence', async () => {
  ...
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary) => boundary.key === 'site_continuity_packet_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary) => boundary.key === 'site_continuity_loop_report_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary) => boundary.key === 'site_continuity_reconciliation_execution_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary) => boundary.key === 'operation_focus_review_store'), true);
  ...
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary) => boundary.key === 'site_continuity_packet_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary) => boundary.key === 'site_continuity_loop_report_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary) => boundary.key === 'site_continuity_reconciliation_execution_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary) => boundary.key === 'operation_focus_review_store' && boundary.status === 'recoverable'), true);
...
test('worker repository publication selector uses per-request evidence lookup beyond global evidence window', async () => {
  ...
  const laterIso = (offset) => new Date(now + offset * 1000).toISOString();
  const siteDb = fakeD1SiteRegistryDatabase({
    sites: [{
      site_id: siteId,
      site_ref: 'site://fixture',
...
test('worker reads mailbox status from Graph only with Cloudflare-held credentials', async () => {
  ...
  assert.ok(graphUrls.every((entry) => entry.authorization === 'Bearer cloudflare-worker-graph-token'));
  ...
  assert.deepEqual(listedBody.reads.map((entry) => entry.read_id), ['mailbox_status_source_fixture_1']);
...
test('worker creates Outlook drafts through Cloudflare Graph authority without admitting send', async () => {
  ...
  assert.equal(listedBody.mailbox_send_admission, 'not_admitted');
  assert.deepEqual(listedBody.drafts.map((entry) => entry.draft_create_id), ['mailbox_outlook_draft_create_fixture_1']);
...
test('worker records site file change proposals without admitting filesystem or publication mutation', async () => {
  ...
  assert.equal(operationReadAfterReviewBody.operation_activity_timeline.items.some((entry) => entry.activity_kind === 'operation_focus_review_acknowledgement'), true);
...
test('worker admits Cloudflare site file materialization records without Windows filesystem or publication mutation', async () => {
  ...
  assert.deepEqual(siteDb.dump().siteFileMaterializations.map((entry) => entry.materialization_id), ['site_file_materialization_fixture_1']);
  ...
  assert.deepEqual(listedBody.materializations.map((entry) => entry.materialization_id), ['site_file_materialization_fixture_1']);
...
test('worker operation.read does not smear indexed carrier evidence across sibling operations', async () => {
  ...
  assert.equal(body.operation_activity_timeline.items.some((entry) => entry.activity_kind === 'carrier_evidence_event'), false);
