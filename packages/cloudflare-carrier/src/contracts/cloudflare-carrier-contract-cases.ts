...
import {
  ...
  fakeD1TaskDatabase,
} from '../cloudflare-d1-test-fixtures.ts';
import worker, {
...
async function canonicalCloudflareIntelligenceD1(registrySiteId: any= CLOUDFLARE_REGISTRY_SITE_ID) {
  ...
  if (registrySiteId !== CLOUDFLARE_REGISTRY_SITE_ID) {
    const targetSite = catalog.records.find(({ record_id }: any) => record_id === CLOUDFLARE_INTELLIGENCE_SITE_ID);
    const principal = catalog.records.find(({ record_id }: any) => record_id === CLOUDFLARE_INTELLIGENCE_PRINCIPAL_ID);
...
async function canonicalAiWorkerEnv(overrides: any= {}) {
...
function startRequest(extra: any= {}) {
  return {
...
function inputRequest(input: any, extra: any= {}) {
  return {
...
function commandRequest(command: any, args: any= [], extra: any= {}) {
  return {
...
function eventKinds(response: any) {
  return (response.events ?? [response.event]).filter(Boolean).map((event: any) => event.event_kind);
}

function assertValidEvents(response: any) {
...
const CARRIER_TEST_GROUP = new URL(import.meta.url).searchParams.get('group');
if (!CARRIER_TEST_GROUP) {
  throw new Error('cloudflare_carrier_contract_cases_require_focused_suite');
}

function test(name: any, ...args: any[]) {
  if (carrierTestGroupForName(name) === CARRIER_TEST_GROUP) {
    return nodeTest(name, ...args);
  }
  return undefined;
}

function carrierTestGroupForName(name: any) {
  const text = String(name);
  if (/^(cloudflare (operation|authority|session|task|evidence|site|membership) command state|cloudflare task lifecycle write admission)/u.test(text)) {
    return 'boundary-contracts';
  }
  if (/provider-unavailable|router-created sessions|canonical invocation control|intelligence diagnostics|provider adapter completes|provider tool|follow-up tool|malformed tool effect|plain tool effect adapter|throwing tool effect/u.test(text)) {
    return 'intelligence';
  }
  if (/webhook delay|resident (loop|dispatch)|mailbox |site file |repository publication|configured Cloudflare (tool|KV) tool|tool effect classifier/u.test(text)) {
    return 'adapters';
  }
  if (/site\.read|site\.list|operation\.create read|operation\.read (does not smear|routes)|operation\.list yields|focused operation|selectCloudflareFocusedOperation|shouldKeepFocusedOperationProjection|shouldPromoteOperationOperatorFocus|summarizeCloudflareOperation|normalizeCloudflareOperationPosture|task lifecycle shadow|clears task lifecycle external effects|site\.membership\.put/u.test(text)) {
    return 'persistence';
  }
  if (/session\.start|input delivery|observer fixture|heartbeat|directive emitter|goal command|observer mute|unsupported host command|event reads|status reconstructs|durable object|browser API alias/u.test(text)) {
...
test('cloudflare task lifecycle write admission keeps Windows mutation authority until cutover', () => {
  ...
  const admittedChangedFileEvidence = classifyCloudflareTaskLifecycleWriteAdmission({
    ...
    file_path: 'packages/cloudflare-carrier/src/cloudflare-worker.ts',
    reporter_agent_id: 'agent-cloudflare',
  ...
  const admittedAssignmentWrite = classifyCloudflareTaskLifecycleWriteAdmission({
    mutation_class: 'task_assignment_write',
    task_id: 'cloudflare-task-lifecycle-1',
    assignee_agent_id: 'agent-cloudflare',
    ...
    assignment_evidence_ref: 'assignment-evidence:cloudflare-task-lifecycle-row:v1',
    cutover_point_ref: 'cutover:task-assignment-write:v1',
    governed_write_contract_ref: 'contract:task-assignment-write:v1',
    confirmation_evidence_ref: 'evidence:operator-check:task-assignment-write',
  });
  assert.equal(admittedAssignmentWrite.action, 'admit');
  assert.equal(admittedAssignmentWrite.reason, 'cloudflare_task_assignment_write_cutover_admitted');
  assert.equal(admittedAssignmentWrite.write_effect, 'task_lifecycle_assignment_write');
  assert.equal(admittedAssignmentWrite.assignment_write_admission, 'admitted');
  assert.equal(admittedAssignmentWrite.assignment_write_schema, 'narada.sonar.cloudflare_task_lifecycle_assignment_write.v1');
  assert.equal(admittedAssignmentWrite.assignment_authority_ref, 'assignment-authority:cloudflare-task-lifecycle-assignment:v1');
  assert.equal(admittedAssignmentWrite.assignment_schema_ref, 'schema:cloudflare-task-lifecycle-assignment:v1');
  assert.equal(admittedAssignmentWrite.assignment_evidence_ref, 'assignment-evidence:cloudflare-task-lifecycle-row:v1');
  assert.equal(admittedAssignmentWrite.roster_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWrite.role_resolution_authority_admission, 'not_admitted');
  assert.equal(admittedAssignmentWrite.assignment_mailbox_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWrite.assignment_filesystem_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWrite.assignment_repository_publication_admission, 'not_admitted');
  assert.deepEqual(admittedAssignmentWrite.retained_windows_authority, [
    'windows_roster_mutation_store',
    'windows_role_resolution',
  ]);

  const admittedRoleResolutionWrite = classifyCloudflareTaskLifecycleWriteAdmission({
    ...
    role_resolution_authority_ref: 'role-resolution-authority:cloudflare-site-membership:v1',
    roster_source_ref: 'roster-source:cloudflare-site-membership:v1',
    role_resolution_schema_ref: 'schema:cloudflare-task-lifecycle-role-resolution:v1',
    role_resolution_evidence_ref: 'role-resolution-evidence:cloudflare-site-membership-row:v1',
    cutover_point_ref: 'cutover:task-role-resolution-write:v1',
    governed_write_contract_ref: 'contract:task-role-resolution-write:v1',
    confirmation_evidence_ref: 'evidence:operator-check:task-role-resolution-write',
  });
  assert.equal(admittedRoleResolutionWrite.action, 'admit');
  assert.equal(admittedRoleResolutionWrite.reason, 'cloudflare_task_role_resolution_write_cutover_admitted');
  assert.equal(admittedRoleResolutionWrite.write_effect, 'task_lifecycle_role_resolution_write');
  assert.equal(admittedRoleResolutionWrite.role_resolution_write_admission, 'admitted');
  assert.equal(admittedRoleResolutionWrite.role_resolution_write_schema, 'narada.sonar.cloudflare_task_lifecycle_role_resolution_write.v1');
  assert.equal(admittedRoleResolutionWrite.role_resolution_roster_read_admission, 'admitted');
  assert.equal(admittedRoleResolutionWrite.role_resolution_roster_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWrite.role_resolution_mailbox_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWrite.role_resolution_filesystem_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWrite.role_resolution_repository_publication_admission, 'not_admitted');
...
test('observer fixture cases produce shared pipeline event kinds plus terminal completion', () => {
  for (const fixtureCase of inputPipelineCases.cases.filter((entry: any) => entry.expected.admission_event_kinds.some((kind: any) => kind.startsWith('observer_')))) {
    const { router } = startedSession();
...
test('record-only operation heartbeat directive records acceptance without provider turn', () => {
  const fixtureCase = inputPipelineCases.cases.find((entry: any) => entry.name === 'operation_heartbeat_system_directive_record_only');
  ...
  assert.equal(response.events.some((event: any) => event.event_kind === 'turn_started'), false);
  assert.equal(response.events.some((event: any) => event.event_kind === 'provider_request_recorded'), false);
...
test('registered directive emitter routes operation attention through input delivery', () => {
  const fixtureCase = directiveEmitterRegistryCases.cases.find((entry: any) => entry.name === 'operation_attention_runtime_trigger_operator_visible_operation_target');
  const { router } = startedSession();
  const response = router.handle({
...
test('observer mute command suppresses interjections but record-only remains observation evidence', () => {
  ...
  const mutedVisible = inputPipelineCases.cases.find((entry: any) => entry.name === 'muted_conversation_observer_suppressed');
...
test('provider-unavailable posture records terminal failure evidence', () => {
  const { router } = startedSession();
  const input = inputPipelineCases.cases.find((entry: any) => entry.name === 'manual_operator_admitted').input;
  ...
  const providerEvent = response.events.find((event: any) => event.event_kind === 'provider_request_recorded');
...
test('router-created sessions carry configured provider and tool adapters', async () => {
  ...
  const textEvent = response.events.find((event: any) => event.event_kind === 'provider_text_delta_recorded');
...
test('carrier forwards canonical invocation control through the provider tool loop', async () => {
  ...
  const input = {
    ...inputPipelineCases.cases.find((entry: any) => entry.name === 'manual_operator_admitted').input,
  ...
  assert.deepEqual(calls.map(({ intelligence_invocation }: any) => intelligence_invocation), [control, control]);
...
test('intelligence diagnostics are service-only and forwarded as explicit adapter fault modes', async () => {
  ...
  assert.throws(
    ...
    (error: any) => error.code === 'cloudflare_intelligence_diagnostic_requires_service_principal',
  );
  assert.throws(
  ...
  assert.equal(
    response.events.find((event: any) => event.event_kind === 'provider_request_recorded').payload.intelligence_diagnostic,
    'provider-failure',
  );
  const diagnosticProviderEvent = response.events.find((event: any) => event.event_kind === 'provider_request_recorded');
...
test('event reads return ordered events by sequence cursor', () => {
  ...
  assert.deepEqual(read.events.map((event: any) => event.sequence), [2, 3]);
...
test('durable object alarm emits operation heartbeat directive through input delivery', async () => {
  ...
  const kinds = read.events.map((event: any) => event.event_kind);
...
test('durable object facade serializes mutations while provider work is pending', async () => {
  ...
  assert.deepEqual(read.events.map((event: any) => event.sequence), read.events.map((_: any, index: any) => index + 1));
...
test('worker site.read composes site sessions tasks authority events and carrier evidence', async () => {
  ...
  assert.equal(body.authority_events.some((event: any) => event.event_kind === 'carrier_site_binding_admitted'), true);
  assert.equal(body.carrier_evidence[0].carrier_session_id, 'carrier_session_cloudflare_fixture');
  assert.equal(body.carrier_evidence[0].events.some((event: any) => event.event_kind === 'carrier_session_started'), true);
  ...
  assert.equal(body.cloudflare_persistence_posture.active_boundary_count, 12);
  ...
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'session_snapshot'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'carrier_evidence_index'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'site_continuity_packet_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'site_continuity_loop_report_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'site_continuity_reconciliation_execution_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'operation_focus_review_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'site_file_materialization_store'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'local_ingress_request_queue'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'repository_publication_request_queue'), true);
  assert.equal(body.cloudflare_persistence_posture.durable_boundaries.some((boundary: any) => boundary.key === 'repository_publication_evidence_store'), true);
  ...
  assert.equal(body.cloudflare_recovery_posture.recovery_boundary_count, 12);
  assert.equal(body.cloudflare_recovery_posture.recoverable_boundary_count, 12);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'site_continuity_packet_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'site_continuity_loop_report_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'site_continuity_reconciliation_execution_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'operation_focus_review_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'site_file_materialization_store' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'local_ingress_request_queue' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'repository_publication_request_queue' && boundary.status === 'recoverable'), true);
  assert.equal(body.cloudflare_recovery_posture.recovery_boundaries.some((boundary: any) => boundary.key === 'repository_publication_evidence_store' && boundary.status === 'recoverable'), true);
  ...
  const membershipDecision = body.site_authority.decisions.find((decision: any) => decision.mutation_class === 'hosted_site_membership');
  ...
  const localFilesystemDecision = body.site_authority.decisions.find((decision: any) => decision.mutation_class === 'local_repository_filesystem_mutation');
  ...
  assert.deepEqual(classifyCloudflareTaskCommandState({
    ...
    evidence_count: body.carrier_evidence[0].events.filter((event: any) => JSON.stringify(event.payload || {}).includes(body.tasks[0].task_id)).length,
  ...
  assert.deepEqual(classifyCloudflareAuthorityCommandState({
    ...
    refusal_count: body.site_authority.decisions.filter((decision: any) => ['refuse', 'deny'].includes(decision.action)).length,
    unresolved_locus_count: body.site_authority.decisions.filter((decision: any) => !decision.authority_locus || decision.authority_locus === 'unresolved').length,
  ...
  assert.deepEqual(classifyCloudflareEvidenceCommandState(body.carrier_evidence[0].events.find((event: any) => event.event_kind === 'carrier_session_started')), {
    lane: 'authority',
  ...
  const identityContinuity = body.site_continuity.decisions.find((decision: any) => decision.exchange_class === 'site_identity_binding');
  ...
  const mutationExecutionContinuity = body.site_continuity.decisions.find((decision: any) => decision.exchange_class === 'cross_embodiment_mutation_execution');
  ...
  assert.equal(body.cloudflare_product_surface_readiness.required_checks.some((check: any) => check.key === 'local_cloud_continuity_bridge' && check.status === 'attention'), true);
  ...
  assert.equal(operationReadAfterFailedReconciliationExecutionBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'site_continuity_reconciliation_execution' && item.focus_ref === failedReconciliationExecutionId), true, JSON.stringify(operationReadAfterFailedReconciliationExecutionBody.operation_activity_timeline.items));
  assert.equal(operationReadAfterFailedReconciliationExecutionBody.operation_product_surface.activity_timeline.items.some((item: any) => item.activity_kind === 'site_continuity_reconciliation_execution' && item.focus_ref === failedReconciliationExecutionId), true, JSON.stringify(operationReadAfterFailedReconciliationExecutionBody.operation_product_surface.activity_timeline.items));
  ...
  assert.ok(operationReadAfterReconciliationExecutionReviewBody.operation_focus_reviews.some((review: any) => review.review_id === 'operation_focus_review_reconciliation_execution_fixture_1'));
  ...
  assert.equal(operationReadAfterLocalIngressBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'local_ingress_request'), true);
  ...
  const repositoryPublicationRequest = await worker.fetch(jsonRequest({
    ...
    params: {
      ...
      requested_action_summary: 'request governed repository publication without direct Cloudflare git push',
      ...
      evidence_return_contract_ref: 'contract:windows-repository-publication-evidence-return:v1',
      rollback_plan_ref: 'rollback:repository-publication-fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const repositoryPublicationRequestBody = await repositoryPublicationRequest.json();
  assert.equal(repositoryPublicationRequest.status, 200, JSON.stringify(repositoryPublicationRequestBody));
  assert.equal(repositoryPublicationRequestBody.status, 'queued');
  assert.equal(repositoryPublicationRequestBody.repository_publication_request_authority, 'cloudflare_repository_publication_request_queue');
  assert.equal(repositoryPublicationRequestBody.repository_publication_executor_authority, 'windows_repository_publication_executor');
  assert.equal(repositoryPublicationRequestBody.repository_publication_admission, 'pending_windows_publication_admission');
  assert.equal(repositoryPublicationRequestBody.cloudflare_git_push_admission, 'not_admitted');
  assert.equal(repositoryPublicationRequestBody.direct_cloudflare_repository_mutation_admission, 'not_admitted');

  const repositoryPublicationRequestList = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.list',
    request_id: 'request_repository_publication_request_list',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(repositoryPublicationRequestList.status, 200);
  const repositoryPublicationRequestListBody = await repositoryPublicationRequestList.json();
  assert.equal(repositoryPublicationRequestListBody.repository_publication_request_authority, 'cloudflare_repository_publication_request_queue');
  assert.equal(repositoryPublicationRequestListBody.repository_publication_executor_authority, 'windows_repository_publication_executor');
  assert.equal(repositoryPublicationRequestListBody.repository_publication_admission, 'pending_windows_publication_admission');
  assert.equal(repositoryPublicationRequestListBody.cloudflare_git_push_admission, 'not_admitted');
  assert.equal(repositoryPublicationRequestListBody.direct_cloudflare_repository_mutation_admission, 'not_admitted');
  ...
  const repositoryPublicationRequestNextBeforeAdmission = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.next',
    request_id: 'request_repository_publication_request_next_before_admission',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(repositoryPublicationRequestNextBeforeAdmission.status, 200);
  const repositoryPublicationRequestNextBeforeAdmissionBody = await repositoryPublicationRequestNextBeforeAdmission.json();
  assert.equal(repositoryPublicationRequestNextBeforeAdmissionBody.status, 'awaiting_cloudflare_admission');
  assert.equal(repositoryPublicationRequestNextBeforeAdmissionBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(repositoryPublicationRequestNextBeforeAdmissionBody.repository_publication_admission, 'waiting_for_cloudflare_publication_admission');
  assert.equal(repositoryPublicationRequestNextBeforeAdmissionBody.pending_unadmitted_count, 1);
  assert.equal(repositoryPublicationRequestNextBeforeAdmissionBody.request, null);

  const repositoryPublicationEvidenceBeforeAdmission = await worker.fetch(jsonRequest({
    operation: 'repository_publication.evidence.put',
    request_id: 'request_repository_publication_evidence_before_admission',
    params: {
      site_id: 'site_fixture',
      repository_publication_request_id: 'repository-publication-request-fixture',
      publication_execution_id: 'repository-publication-execution-before-admission',
      repository_ref: 'github:andrey-kokoev/narada.sonar',
      branch_ref: 'master',
      source_change_ref: 'local-ingress-execution:local-ingress-execution-fixture',
      windows_admission_action: 'admit',
  ...
  assert.equal(repositoryPublicationEvidenceBeforeAdmission.status, 400);
  const repositoryPublicationEvidenceBeforeAdmissionBody = await repositoryPublicationEvidenceBeforeAdmission.json();
  assert.equal(repositoryPublicationEvidenceBeforeAdmissionBody.code, 'repository_publication_evidence_cloudflare_admission_required');

  const repositoryPublicationAdmission = await worker.fetch(jsonRequest({
    operation: 'repository_publication.admission.classify',
    request_id: 'request_repository_publication_admission_classify',
    params: {
      site_id: 'site_fixture',
      repository_publication_admission_id: 'repository-publication-admission-fixture',
      repository_publication_request_id: 'repository-publication-request-fixture',
      admission_action: 'admit',
      admission_reason: 'cloudflare_repository_publication_request_admitted_for_fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const repositoryPublicationAdmissionBody = await repositoryPublicationAdmission.json();
  assert.equal(repositoryPublicationAdmission.status, 200, JSON.stringify(repositoryPublicationAdmissionBody));
  assert.equal(repositoryPublicationAdmissionBody.schema, 'narada.sonar.cloudflare_repository_publication_admission.v1');
  assert.equal(repositoryPublicationAdmissionBody.status, 'admission_recorded');
  assert.equal(repositoryPublicationAdmissionBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(repositoryPublicationAdmissionBody.repository_publication_executor_authority, 'windows_repository_publication_executor');
  assert.equal(repositoryPublicationAdmissionBody.repository_publication_admission, 'admitted_by_cloudflare_repository_publication');
  assert.equal(repositoryPublicationAdmissionBody.cloudflare_git_push_admission, 'not_admitted');
  assert.equal(repositoryPublicationAdmissionBody.direct_cloudflare_repository_mutation_admission, 'not_admitted');

  const repositoryPublicationAdmissionList = await worker.fetch(jsonRequest({
    operation: 'repository_publication.admission.list',
    request_id: 'request_repository_publication_admission_list',
    params: { site_id: 'site_fixture', repository_publication_request_id: 'repository-publication-request-fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(repositoryPublicationAdmissionList.status, 200);
  const repositoryPublicationAdmissionListBody = await repositoryPublicationAdmissionList.json();
  assert.equal(repositoryPublicationAdmissionListBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(repositoryPublicationAdmissionListBody.repository_publication_admission, 'admitted_by_cloudflare_repository_publication');
  assert.equal(repositoryPublicationAdmissionListBody.admissions.length, 1);
  assert.equal(repositoryPublicationAdmissionListBody.admissions[0].repository_publication_admission_id, 'repository-publication-admission-fixture');
  ...
  assert.equal(repositoryPublicationRequestNextBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(repositoryPublicationRequestNextBody.repository_publication_admission, 'admitted_by_cloudflare_repository_publication');
  ...
  assert.equal(repositoryPublicationRequestNextBody.authority_partition, 'cloudflare_admits_repository_publication_windows_executes_and_returns_evidence');
  assert.equal(repositoryPublicationRequestNextBody.admission.repository_publication_admission_id, 'repository-publication-admission-fixture');
  ...
  assert.equal(repositoryPublicationEvidenceBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  ...
  assert.equal(repositoryPublicationEvidenceBody.repository_publication_admission, 'admitted_by_cloudflare_repository_publication');
  assert.equal(repositoryPublicationEvidenceBody.cloudflare_repository_publication_admission_id, 'repository-publication-admission-fixture');
  assert.equal(repositoryPublicationEvidenceBody.cloudflare_repository_publication_admission_action, 'admit');
  ...
  assert.equal(repositoryPublicationEvidenceListBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  ...
  assert.equal(repositoryPublicationEvidenceListBody.repository_publication_admission, 'resolved_after_cloudflare_repository_publication_admission');
  ...
  assert.equal(repositoryPublicationEvidenceListBody.authority_partition, 'cloudflare_admits_repository_publication_windows_executes_and_cloudflare_records_evidence');
  ...
  assert.equal(repositoryPublicationEvidenceListBody.evidence[0].cloudflare_repository_publication_admission_id, 'repository-publication-admission-fixture');
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_admissions.length, 1);
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.operation_product_surface.repository_publication_admission_count, 1);
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.operation_product_surface.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.operation_product_surface.repository_publication_execution_admission, 'admitted_by_cloudflare_repository_publication');
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.schema, 'narada.cloudflare_repository_publication_operation_posture.v1');
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.state, 'evidence_recorded');
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.repository_publication_request_count, 1);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.repository_publication_admission_count, 1);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.repository_publication_evidence_count, 1);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.repository_publication_provider_heartbeat_count, 1);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.pending_request_count, 0);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.completed_evidence_count, 1);
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.provider_liveness.state, 'fresh');
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.cloudflare_git_push_admission, 'not_admitted');
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(operationReadAfterRepositoryPublicationBody.repository_publication_operation_posture.repository_publication_admission, 'admitted_by_cloudflare_repository_publication');
  ...
  assert.equal(operationReadAfterRepositoryPublicationBody.authority_transfer_posture.remaining_windows_authorities[0].domain, 'mailbox_status');
  assert.equal(operationReadAfterRepositoryPublicationBody.authority_transfer_posture.remaining_windows_authorities[0].authority, 'mailbox_status_source');
  assert.equal(operationReadAfterRepositoryPublicationBody.authority_transfer_posture.next_action, 'transfer_mailbox_status_source_authority');
  assert.equal(operationReadAfterRepositoryPublicationBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'local_ingress').classification, 'cloudflare_governed_windows_executed');
  assert.equal(operationReadAfterRepositoryPublicationBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'repository_publication').classification, 'cloudflare_governed_windows_executed');
  ...
  assert.ok(operationReadAfterRepositoryPublicationBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'repository_publication_request'));
  assert.ok(operationReadAfterRepositoryPublicationBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'repository_publication_evidence'));
  assert.ok(operationReadAfterRepositoryPublicationBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'repository_publication_provider_heartbeat'));
  ...
  const directRepositoryMutationClaim = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.create',
    request_id: 'request_repository_publication_direct_cloudflare_claim',
    params: {
      site_id: 'site_fixture',
      publication_ref: 'repository-publication:invalid-direct-cloudflare-claim',
      requested_action_ref: 'repository-publication-action:invalid-direct-cloudflare-claim',
      repository_ref: 'github:andrey-kokoev/narada.sonar',
      branch_ref: 'master',
      source_change_ref: 'local-ingress-execution:invalid-direct-cloudflare-claim',
      governed_request_contract_ref: 'contract:cloudflare-to-windows-repository-publication-request:v1',
      evidence_return_contract_ref: 'contract:windows-repository-publication-evidence-return:v1',
      rollback_plan_ref: 'rollback:repository-publication-fixture',
      cloudflare_git_push_admission: 'admitted',
  ...
  assert.equal(directRepositoryMutationClaim.status, 400);
  const directRepositoryMutationClaimBody = await directRepositoryMutationClaim.json();
  assert.equal(directRepositoryMutationClaimBody.code, 'repository_publication_cloudflare_git_push_admission_invalid');
  ...
  assert.equal(operationReadAfterLocalIngressEvidenceBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'local_ingress_evidence'), true);
  assert.equal(operationReadAfterLocalIngressEvidenceBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'local_ingress_provider_heartbeat'), true);
...
test('worker executes admitted repository publication through Cloudflare GitHub executor', async () => {
  const siteDb = fakeD1SiteRegistryDatabase({
    ...
    operations: [{ operation_id: 'operation_site_read', site_id: 'site_fixture', display_name: 'Site Read Operation', operation_kind: 'control', status: 'active', created_by_principal_id: 'admin', created_at: clock(), updated_at: clock() }],
  });
  const githubCalls: any[] = [];
  const env = authEnv(fakeDurableObjectNamespace(), {
    ...
    CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_FETCH: async (url: any, init: any) => {
  ...
  assert.equal(readyBody.github_credential_mode, 'github_token');
  ...
  assert.equal(readyBody.github_app_configured, false);
  ...
  assert.equal(notReadyBody.github_credential_mode, 'missing');
  assert.equal(notReadyBody.github_token_configured, false);
  assert.deepEqual(notReadyBody.missing_configuration, [
    'CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_TOKEN',
  ...
  const request = await worker.fetch(jsonRequest({
    ...
    params: {
      site_id: 'site_fixture',
      repository_publication_request_id: 'repository-publication-request-cloudflare-execution-fixture',
      publication_ref: 'repository-publication:cloudflare-execution-fixture',
      requested_action_ref: 'repository-publication-action:cloudflare-execution-fixture',
      requested_action_summary: 'publish admitted commit from Cloudflare GitHub executor',
      repository_ref: 'github:andrey-kokoev/narada.sonar',
      branch_ref: 'cloudflare-publication-fixture',
      ...
      rollback_plan_ref: 'rollback:cloudflare-github-repository-publication-fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(request.status, 200, JSON.stringify(await request.clone().json()));

  const beforeAdmissionExecution = await worker.fetch(jsonRequest({
    operation: 'repository_publication.cloudflare_execution.execute',
    request_id: 'request_repository_publication_cloudflare_execution_before_admission',
    params: { site_id: 'site_fixture', repository_publication_request_id: 'repository-publication-request-cloudflare-execution-fixture' },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(beforeAdmissionExecution.status, 400);
  assert.equal((await beforeAdmissionExecution.json()).code, 'cloudflare_repository_publication_execution_admission_required');

  const admission = await worker.fetch(jsonRequest({
    operation: 'repository_publication.admission.classify',
    request_id: 'request_repository_publication_cloudflare_execution_admission',
    params: {
      site_id: 'site_fixture',
      repository_publication_admission_id: 'repository-publication-admission-cloudflare-execution-fixture',
      repository_publication_request_id: 'repository-publication-request-cloudflare-execution-fixture',
      admission_action: 'admit',
      admission_reason: 'cloudflare_repository_publication_request_admitted_for_cloudflare_github_execution',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  ...
  const execution = await worker.fetch(jsonRequest({
    operation: 'repository_publication.cloudflare_execution.execute',
    request_id: 'request_repository_publication_cloudflare_execution_execute',
    params: {
      site_id: 'site_fixture',
      repository_publication_execution_id: 'repository-publication-execution-cloudflare-fixture',
      repository_publication_request_id: 'repository-publication-request-cloudflare-execution-fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const executionBody = await execution.json();
  assert.equal(execution.status, 200, JSON.stringify(executionBody));
  assert.equal(executionBody.schema, 'narada.sonar.cloudflare_github_repository_publication_execution.v1');
  assert.equal(executionBody.publication_status, 'completed');
  assert.equal(executionBody.repository_publication_executor_authority, 'cloudflare_github_repository_publication_executor');
  assert.equal(executionBody.execution.github_credential_mode, 'github_token');
  assert.equal(executionBody.repository_publication_admission_authority, 'cloudflare_repository_publication_admission_controller');
  assert.equal(executionBody.cloudflare_git_push_admission, 'not_admitted');
  assert.equal(executionBody.direct_cloudflare_repository_mutation_admission, 'admitted_by_cloudflare_github_repository_publication');
  ...
  assert.deepEqual(JSON.parse(githubCalls[0].init.body), { sha: '0123456789abcdef0123456789abcdef01234567', force: false });

  const executionList = await worker.fetch(jsonRequest({
    operation: 'repository_publication.cloudflare_execution.list',
    request_id: 'request_repository_publication_cloudflare_execution_list',
    params: { site_id: 'site_fixture', repository_publication_request_id: 'repository-publication-request-cloudflare-execution-fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const executionListBody = await executionList.json();
  assert.equal(executionList.status, 200, JSON.stringify(executionListBody));
  assert.equal(executionListBody.executions.length, 1);
  assert.equal(executionListBody.executions[0].repository_publication_execution_id, 'repository-publication-execution-cloudflare-fixture');

  const nextAfterExecution = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.next',
    request_id: 'request_repository_publication_cloudflare_execution_next_after_execution',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const nextAfterExecutionBody = await nextAfterExecution.json();
  assert.equal(nextAfterExecution.status, 200, JSON.stringify(nextAfterExecutionBody));
  ...
  assert.equal(readBody.repository_publication_executions.length, 1);
  assert.equal(readBody.repository_publication_operation_posture.repository_publication_execution_count, 1);
  assert.equal(readBody.repository_publication_operation_posture.executor_authority, 'cloudflare_github_repository_publication_executor');
  ...
  assert.equal(readBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'repository_publication').classification, 'cloudflare_owned');
  assert.deepEqual(readBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'site_file_materialization').remaining_windows_authority, []);
  assert.equal(readBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.domain === 'site_file_materialization' && entry.authority === 'repository_publication'), false);
  assert.equal(readBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'local_ingress').classification, 'cloudflare_owned');
  assert.deepEqual(readBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'local_ingress').remaining_windows_authority, []);
  assert.equal(readBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.domain === 'local_ingress'), false);
});
...
test('worker executes admitted repository publication with GitHub App installation authority', async () => {
  ...
  const env = authEnv(fakeDurableObjectNamespace(), {
    CLOUDFLARE_SITE_REGISTRY_DB: siteDb,
    CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_ID: '12345',
    CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_INSTALLATION_ID: '67890',
    CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_PRIVATE_KEY: privateKey,
    CLOUDFLARE_REPOSITORY_PUBLICATION_ALLOWED_REPOSITORIES: 'github:andrey-kokoev/narada.sonar',
    CLOUDFLARE_REPOSITORY_PUBLICATION_ALLOWED_BRANCHES: 'cloudflare-publication-fixture',
    CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_FETCH: async (url: any, init: any) => {
      githubCalls.push({ url, init });
      if (url === 'https://api.github.com/app/installations/67890/access_tokens') {
        ...
        return new Response(JSON.stringify({ token: 'github-app-installation-token-fixture' }), {
          status: 201,
          headers: { 'content-type': 'application/json' },
        });
      }
  ...
  const ready = await worker.fetch(jsonRequest({
    operation: 'repository_publication.cloudflare_execution.readiness',
    request_id: 'request_repository_publication_github_app_readiness_ready',
    params: { site_id: 'site_fixture', repository_ref: 'github:andrey-kokoev/narada.sonar', branch_ref: 'cloudflare-publication-fixture' },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const readyBody = await ready.json();
  assert.equal(ready.status, 200, JSON.stringify(readyBody));
  assert.equal(readyBody.readiness_status, 'ready');
  assert.equal(readyBody.github_credential_mode, 'github_app_installation');
  assert.equal(readyBody.github_token_configured, false);
  assert.equal(readyBody.github_app_configured, true);
  assert.deepEqual(readyBody.missing_configuration, []);
  assert.doesNotMatch(JSON.stringify(readyBody), /PRIVATE KEY|github-app-installation-token-fixture/);

  const request = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.create',
    request_id: 'request_repository_publication_github_app_create',
    params: {
      site_id: 'site_fixture',
      repository_publication_request_id: 'repository-publication-request-github-app-fixture',
      publication_ref: 'repository-publication:github-app-fixture',
      requested_action_ref: 'repository-publication-action:github-app-fixture',
      requested_action_summary: 'publish admitted commit from Cloudflare GitHub App executor',
      repository_ref: 'github:andrey-kokoev/narada.sonar',
      branch_ref: 'cloudflare-publication-fixture',
      source_change_ref: 'git:commit:0123456789abcdef0123456789abcdef01234567',
      governed_request_contract_ref: 'contract:cloudflare-github-app-repository-publication-request:v1',
      evidence_return_contract_ref: 'contract:cloudflare-github-repository-publication-execution-record:v1',
      rollback_plan_ref: 'rollback:cloudflare-github-app-repository-publication-fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(request.status, 200, JSON.stringify(await request.clone().json()));
  ...
  const admission = await worker.fetch(jsonRequest({
    operation: 'repository_publication.admission.classify',
    request_id: 'request_repository_publication_github_app_admission',
    params: {
      site_id: 'site_fixture',
      repository_publication_admission_id: 'repository-publication-admission-github-app-fixture',
      repository_publication_request_id: 'repository-publication-request-github-app-fixture',
      admission_action: 'admit',
      admission_reason: 'cloudflare_repository_publication_request_admitted_for_cloudflare_github_app_execution',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(admission.status, 200, JSON.stringify(await admission.clone().json()));
  ...
  const execution = await worker.fetch(jsonRequest({
    operation: 'repository_publication.cloudflare_execution.execute',
    request_id: 'request_repository_publication_github_app_execute',
    params: {
      site_id: 'site_fixture',
      repository_publication_execution_id: 'repository-publication-execution-github-app-fixture',
      repository_publication_request_id: 'repository-publication-request-github-app-fixture',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  const executionBody = await execution.json();
  assert.equal(execution.status, 200, JSON.stringify(executionBody));
  assert.equal(executionBody.publication_status, 'completed');
  assert.equal(executionBody.execution.github_credential_mode, 'github_app_installation');
  assert.equal(githubCalls.length, 2);
  assert.equal(githubCalls[0].url, 'https://api.github.com/app/installations/67890/access_tokens');
  assert.equal(githubCalls[1].url, 'https://api.github.com/repos/andrey-kokoev/narada.sonar/git/refs/heads/cloudflare-publication-fixture');
  assert.equal(githubCalls[1].init.headers.authorization, 'Bearer github-app-installation-token-fixture');
  assert.deepEqual(JSON.parse(githubCalls[1].init.body), { sha: '0123456789abcdef0123456789abcdef01234567', force: false });
});

test('worker repository publication selector uses per-request evidence lookup beyond global evidence window', async () => {
  const siteId = 'site_fixture';
  const resolvedRequestId = 'repository-publication-request-resolved-old';
  const now = Date.parse(clock());
  const laterIso = (offset: any) => new Date(now + offset * 1000).toISOString();
  const siteDb = fakeD1SiteRegistryDatabase({
    sites: [{
      site_id: siteId,
      site_ref: 'site://fixture',
    ...
    repositoryPublicationRequests: [{
      repository_publication_request_id: resolvedRequestId,
      site_id: siteId,
      generated_at: clock(),
      operation_id: 'operation_site_read',
      task_id: '',
      ...
      governed_request_contract_ref: 'contract:cloudflare-to-windows-repository-publication-request:v1',
      evidence_return_contract_ref: 'contract:windows-repository-publication-evidence-return:v1',
      rollback_plan_ref: 'rollback:resolved-old',
      authority_locus: 'cloudflare_carrier_site',
      target_authority_locus: 'windows_local_repository',
      ...
      request_posture: 'cloudflare_queued_windows_repository_publication_request',
      request_json: '{}',
      recorded_by_principal_id: 'admin',
      recorded_at: clock(),
    }],
    repositoryPublicationAdmissions: [{
      repository_publication_admission_id: 'repository-publication-admission-resolved-old',
      site_id: siteId,
      generated_at: clock(),
      repository_publication_request_id: resolvedRequestId,
      admission_action: 'admit',
      admission_reason: 'fixture_admitted',
      ...
      cloudflare_git_push_admission: 'not_admitted',
      direct_cloudflare_repository_mutation_admission: 'not_admitted',
      admission_posture: 'cloudflare_admitted_windows_repository_publication_request',
      admission_json: '{}',
      recorded_by_principal_id: 'admin',
      recorded_at: clock(),
    }],
    repositoryPublicationEvidence: [
      {
        repository_publication_evidence_id: 'repository-publication-evidence-resolved-old',
        site_id: siteId,
        generated_at: clock(),
        repository_publication_request_id: resolvedRequestId,
        ...
        publication_status: 'completed',
        repository_publication_executor_authority: 'windows_repository_publication_executor',
        published_commit_ref: 'git:commit:resolved-old',
        rollback_evidence_ref: 'rollback:published:resolved-old',
        cloudflare_git_push_admission: 'not_admitted',
        direct_cloudflare_repository_mutation_admission: 'not_admitted',
      ...
      ...Array.from({ length: 101 }, (_: any, index: any) => ({
        repository_publication_evidence_id: `repository-publication-evidence-newer-${index}`,
        site_id: siteId,
        generated_at: laterIso(index + 1),
        repository_publication_request_id: `repository-publication-request-newer-${index}`,
        ...
        publication_status: 'completed',
        repository_publication_executor_authority: 'windows_repository_publication_executor',
        published_commit_ref: `git:commit:newer-${index}`,
        rollback_evidence_ref: `rollback:published:newer-${index}`,
        cloudflare_git_push_admission: 'not_admitted',
        direct_cloudflare_repository_mutation_admission: 'not_admitted',
        evidence_posture: 'windows_repository_publication_resolved_cloudflare_recorded_evidence',
        evidence_json: '{}',
        recorded_by_principal_id: 'admin',
        recorded_at: laterIso(index + 1),
      })),
    ],
  });
  const env = authEnv(fakeDurableObjectNamespace(), { CLOUDFLARE_SITE_REGISTRY_DB: siteDb });
  ...
  const response = await worker.fetch(jsonRequest({
    operation: 'repository_publication.request.next',
    request_id: 'request_repository_publication_next_after_saturated_evidence_history',
    params: { site_id: siteId, limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);

  assert.equal(response.status, 200);
  const body = await response.json();
  assert.equal(body.status, 'drained', JSON.stringify(body, null, 2));
  assert.equal(body.request, null);
  assert.equal(body.repository_publication_dispatch_authority, 'not_observed');
});
...
test('worker site.read can replay carrier evidence from D1 index without durable object binding', async () => {
  ...
  assert.deepEqual(body.carrier_evidence[0].events.map((event: any) => event.event_kind), [
...
test('worker serves minimal authenticated web console shell', async () => {
  ...
  assert.match(html, /Cloudflare GitHub Readiness/);
  assert.match(html, /readRepositoryPublicationReadiness/);
  assert.match(html, /readFocusedRepositoryPublicationReadiness/);
  assert.match(html, /repositoryPublicationReadinessDetail/);
  assert.match(html, /repository_publication\.cloudflare_execution\.readiness/);
  ...
  assert.match(html, /Focused Operation Lifecycle/);
  assert.match(html, /focusedOperationLifecycle/);
  assert.match(html, /focusedOperationLifecycleNextAction/);
  assert.match(html, /applyFocusedOperationLifecycleNextAction/);
  ...
  assert.match(html, /refresh_site_continuity_loop/);
  assert.match(html, /focusContinuityLoopRefresh/);
  ...
  assert.match(html, /operationLifecycleResumeContinuation/);
  assert.match(html, /resumeFocusedOperationContinuation/);
  assert.match(html, /operation_continuation_resumed_by_operator/);
  ...
  assert.match(html, /authorityTransferDetail/);
  assert.match(html, /authorityTransferWorkflow/);
  assert.match(html, /authorityTransferPosture/);
  assert.match(html, /authorityTransferContext/);
  assert.match(html, /renderAuthorityTransfer/);
  assert.match(html, /authorityTransferWorkflowItems/);
  assert.match(html, /renderAuthorityTransferWorkflow/);
  ...
  assert.match(html, /site_file_materializations/);
  assert.match(html, /site_file_materialization_count/);
  assert.match(html, /site_file_materialization_authority/);
  assert.match(html, /cloudflare_site_file_materialization_admission/);
  assert.match(html, /cloudflare_site_file_materialization_executor_authority/);
  assert.match(html, /windows_filesystem_mutation_admission/);
  assert.match(html, /site_file_materialization_repository_publication_admission/);
  assert.match(html, /site_file_materialization_authority_partition/);
  assert.match(html, /site_file_materialization_next_action/);
  assert.match(html, /review_site_file_materialization/);
  ...
  assert.match(html, /Local Ingress Operation Posture/);
  assert.match(html, /narada\.cloudflare_local_ingress_operation_posture\.v1/);
  assert.match(html, /restore_windows_local_ingress_executor/);
  assert.match(html, /local_ingress_operation_posture_requires_windows_executor/);
  ...
  assert.match(html, /Repository Publication/);
  assert.match(html, /repositoryPublicationRequestNavigator/);
...
test('worker records webhook delay observations as Cloudflare shadow-read evidence without dispatching', async () => {
  ...
  assert.deepEqual(listedBody.observations.map((entry: any) => entry.classification_state), ['critical', 'ok']);
  ...
  assert.deepEqual(siteReadBody.webhook_delay_shadow_observations.map((entry: any) => entry.classification_state), ['critical', 'ok']);
  ...
  assert.deepEqual(classifyCloudflareOperationCommandState({
    ...
    evidence_loaded: (operationReadBody.carrier_evidence || []).some((entry: any) => (entry.events || []).length > 0),
...
test('worker records webhook delay observation as Cloudflare primary with Windows fallback', async () => {
  ...
  assert.deepEqual(listedBody.observations.map((entry: any) => entry.observation_id), ['webhook_delay_observation_primary_fixture_1']);
...
test('worker reads webhook delay observation from Cloudflare remote source adapter', async () => {
  ...
  assert.deepEqual(listedSamplesBody.samples.map((sample: any) => sample.sample_id), [
...
test('worker records webhook delay directive intent as dual-recorded carrier input without delivery', async () => {
  ...
  assert.equal(recordedBody.carrier_admission.admission_events.some((event: any) => event.event_kind === 'directive_receipt_recorded'), true);
  assert.equal(recordedBody.carrier_admission.admission_events.some((event: any) => event.event_kind === 'input_admitted_to_turn'), false);
  assert.deepEqual(recordedBody.record.retained_windows_authority, [
    'mailbox_send',
    'local_filesystem_mutation',
    'task_lifecycle_write',
  ...
  assert.deepEqual(listedBody.directive_records.map((entry: any) => entry.directive_record_id), ['webhook_delay_directive_fixture_1']);
...
test('worker delivers webhook delay directive as Cloudflare primary with Windows fallback recorded', async () => {
  ...
  assert.equal(deliveredBody.delivery.events.some((event: any) => event.event_kind === 'carrier_session_started'), false);
  assert.equal(deliveredBody.delivery.events.some((event: any) => event.event_kind === 'directive_receipt_recorded'), true);
  assert.equal(deliveredBody.delivery.events.some((event: any) => event.event_kind === 'input_admitted_to_turn'), true);
  assert.equal(deliveredBody.delivery.events.some((event: any) => event.event_kind === 'provider_request_recorded'), true);
  ...
  assert.equal(siteDb.dump().carrierSessions.some((session: any) => session.carrier_session_id === 'carrier_session_webhook_delay_directive_fixture'), true);
  ...
  assert.deepEqual(listedBody.directive_deliveries.map((entry: any) => entry.delivery_id), ['webhook_delay_directive_delivery_fixture_1']);
  ...
  assert.equal(operationReadBody.sessions.some((session: any) => session.carrier_session_id === 'carrier_session_webhook_delay_directive_fixture'), true);
  assert.equal(operationReadBody.carrier_evidence.some((entry: any) => entry.carrier_session_id === 'carrier_session_webhook_delay_directive_fixture' && entry.events.some((event: any) => event.event_kind === 'directive_receipt_recorded')), true);
});
...
test('worker records resident loop runs as Cloudflare shadow-read evidence without dispatching', async () => {
  ...
  assert.equal(listedBody.dispatch_authority, 'windows_primary_dispatcher');
  assert.equal(listedBody.dispatch_action, 'none');
  assert.deepEqual(listedBody.loop_runs.map((entry: any) => entry.loop_status), ['attention_recorded']);
...
test('worker records mailbox status shadow reads without admitting mailbox mutation', async () => {
  ...
  assert.deepEqual(listedBody.reads.map((entry: any) => entry.read_id), ['mailbox_status_shadow_fixture_1']);
...
test('worker reads mailbox status from Graph only with Cloudflare-held credentials', async () => {
  const siteDb = fakeD1SiteRegistryDatabase({
    sites: [{
      site_id: 'site_fixture',
      site_ref: 'site://fixture',
    ...
    operations: [{
      operation_id: 'operation_mailbox_source',
      site_id: 'site_fixture',
      display_name: 'Mailbox Source Operation',
      operation_kind: 'operating_layer_update',
      status: 'active',
      created_by_principal_id: 'admin',
      created_at: clock(),
  ...
  const envWithoutCredentials = authEnv(fakeDurableObjectNamespace(), {
    CLOUDFLARE_SITE_REGISTRY_DB: siteDb,
    CLOUDFLARE_GRAPH_FETCH: async () => {
      graphFetchCalls += 1;
      return new Response('{}', { status: 500 });
    },
  });
  const missingCredentials = await worker.fetch(jsonRequest({
    operation: 'mailbox.status_source.read',
    request_id: 'request_mailbox_source_missing_credentials',
    params: { site_id: 'site_fixture', account_ref: 'sonar.narada@example.com' },
  }, { token: 'test-admin-token', path: '/api/carrier' }), envWithoutCredentials);
  const missingCredentialsBody = await missingCredentials.json();
  assert.equal(missingCredentialsBody.code, 'graph_credentials_missing');
  assert.equal(missingCredentials.status, 401);
  assert.equal(graphFetchCalls, 0);

  const graphUrls: any[] = [];
  const env = authEnv(fakeDurableObjectNamespace(), {
    CLOUDFLARE_SITE_REGISTRY_DB: siteDb,
    GRAPH_ACCESS_TOKEN: 'cloudflare-worker-graph-token',
    CLOUDFLARE_GRAPH_FETCH: async (url: any, init: any) => {
      graphUrls.push({ url: String(url), authorization: init?.headers?.Authorization });
      if (String(url).includes('/mailFolders/inbox/messages')) {
        return new Response(JSON.stringify({ value: [{ receivedDateTime: '2026-06-08T12:00:00Z' }] }), { status: 200 });
      }
      if (String(url).includes('/mailFolders/drafts')) {
        return new Response(JSON.stringify({ totalItemCount: 1 }), { status: 200 });
      }
      if (String(url).includes('/mailFolders/inbox')) {
        return new Response(JSON.stringify({ unreadItemCount: 2, totalItemCount: 7 }), { status: 200 });
      }
      return new Response(JSON.stringify({ error: { code: 'UnexpectedUrl' } }), { status: 404 });
    },
  });

  const recorded = await worker.fetch(jsonRequest({
    operation: 'mailbox.status_source.read',
    request_id: 'request_mailbox_source_read',
    params: { site_id: 'site_fixture', account_ref: 'sonar.narada@example.com', read_id: 'mailbox_status_source_fixture_1' },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(recorded.status, 200);
  const recordedBody = await recorded.json();
  assert.equal(recordedBody.schema, 'narada.sonar.cloudflare_mailbox_status_source_read.v1');
  assert.equal(recordedBody.mailbox_status_authority, 'cloudflare_graph_mailbox_status_source');
  assert.equal(recordedBody.mailbox_send_admission, 'not_admitted');
  assert.equal(recordedBody.mailbox_mutation_admission, 'not_admitted');
  assert.equal(recordedBody.read.source_locus, 'cloudflare_carrier_site');
  assert.equal(recordedBody.read.unread_count, 2);
  assert.equal(recordedBody.read.pending_draft_count, 1);
  assert.equal(recordedBody.read.latest_message_at, '2026-06-08T12:00:00Z');
  assert.equal(graphUrls.length, 3);
  assert.ok(graphUrls.every((entry: any) => entry.authorization === 'Bearer cloudflare-worker-graph-token'));

  const listed = await worker.fetch(jsonRequest({
    operation: 'mailbox.status_source.list',
    request_id: 'request_mailbox_source_list',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.equal(listedBody.mailbox_status_authority, 'cloudflare_graph_mailbox_status_source');
  assert.deepEqual(listedBody.reads.map((entry: any) => entry.read_id), ['mailbox_status_source_fixture_1']);

  const operationRead = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_mailbox_source_operation_read',
    params: { site_id: 'site_fixture', operation_id: 'operation_mailbox_source', mailbox_status_source_limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(operationRead.status, 200);
  const operationReadBody = await operationRead.json();
  assert.equal(operationReadBody.mailbox_status_source_reads.length, 1);
  assert.equal(operationReadBody.operation_product_surface.mailbox_status_source_read_count, 1);
  assert.equal(operationReadBody.operation_product_surface.mailbox_status_authority, 'cloudflare_graph_mailbox_status_source');
  assert.equal(operationReadBody.operation_product_surface.mailbox_send_admission, 'not_admitted');
  assert.equal(operationReadBody.operation_product_surface.mailbox_mutation_admission, 'not_admitted');
...
test('worker records mailbox draft reply proposals without admitting Outlook draft creation or send', async () => {
  ...
  assert.deepEqual(siteDb.dump().mailboxDraftReplyProposals.map((entry: any) => entry.proposal_id), ['mailbox_draft_reply_fixture_1']);
  ...
  assert.equal(listedBody.mailbox_send_admission, 'not_admitted');
  assert.equal(listedBody.mailbox_mutation_admission, 'not_admitted');
  assert.deepEqual(listedBody.proposals.map((entry: any) => entry.proposal_id), ['mailbox_draft_reply_fixture_1']);
  ...
  assert.equal(operationReadBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'mailbox_draft_reply_proposal'), true);
...
test('worker creates Outlook drafts through Cloudflare Graph authority without admitting send', async () => {
  const siteDb = fakeD1SiteRegistryDatabase({
    sites: [{ site_id: 'site_fixture', site_ref: 'site://fixture', display_name: 'Fixture Site', status: 'active', created_at: clock(), updated_at: clock(), created_by_principal_id: 'admin' }],
    memberships: [{ site_id: 'site_fixture', principal_id: 'admin', role: 'owner', status: 'active', created_at: clock(), updated_at: clock() }],
    operations: [{ operation_id: 'operation_mailbox_draft_create', site_id: 'site_fixture', display_name: 'Mailbox Draft Create Operation', operation_kind: 'operating_layer_update', status: 'active', created_by_principal_id: 'admin', created_at: clock(), updated_at: clock() }],
  });
  ...
  const sourcePayload = {
    ...
    generated_at: '2026-06-08T14:00:00.000Z',
    ...
    body_text: 'Thanks for the report. We are checking the webhook delay now.',
  ...
  const refusedSend = await worker.fetch(jsonRequest({
    ...
    request_id: 'request_mailbox_outlook_draft_refused_send',
    params: { site_id: 'site_fixture', draft_create_id: 'mailbox_outlook_draft_refused_send', source_payload: { ...sourcePayload, mailbox_send_admission: 'admitted' } },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(refusedSend.status, 400);
  assert.equal((await refusedSend.json()).code, 'mailbox_outlook_draft_create_send_admission_invalid');
  assert.equal(graphCalls.length, 0);

  const created = await worker.fetch(jsonRequest({
    operation: 'mailbox.outlook_draft.create',
    request_id: 'request_mailbox_outlook_draft_create',
    params: { site_id: 'site_fixture', draft_create_id: 'mailbox_outlook_draft_create_fixture_1', source_payload: sourcePayload },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(created.status, 200);
  const createdBody = await created.json();
  assert.equal(createdBody.schema, 'narada.sonar.cloudflare_mailbox_outlook_draft_create.v1');
  assert.equal(createdBody.status, 'created');
  assert.equal(createdBody.mailbox_outlook_draft_create_authority, 'cloudflare_graph_outlook_draft_create');
  assert.equal(createdBody.mailbox_outlook_draft_create_admission, 'admitted');
  assert.equal(createdBody.mailbox_send_admission, 'not_admitted');
  assert.equal(createdBody.mailbox_mutation_admission, 'not_admitted');
  assert.equal(createdBody.record.outlook_draft_id, 'outlook-draft-fixture-1');
  assert.equal(graphCalls.length, 1);
  assert.equal(graphCalls[0].method, 'POST');
  assert.match(graphCalls[0].url, /\/users\/help%40global-maxima\.com\/messages$/);
  assert.equal(graphCalls[0].authorization, 'Bearer cloudflare-worker-graph-token');
  assert.equal(graphCalls[0].body.subject, 'Re: support request');
  assert.equal(graphCalls[0].body.toRecipients[0].emailAddress.address, 'customer@example.com');

  const listed = await worker.fetch(jsonRequest({
    operation: 'mailbox.outlook_draft.list',
    request_id: 'request_mailbox_outlook_draft_list',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.equal(listedBody.mailbox_outlook_draft_create_admission, 'admitted');
  assert.equal(listedBody.mailbox_send_admission, 'not_admitted');
  assert.deepEqual(listedBody.drafts.map((entry: any) => entry.draft_create_id), ['mailbox_outlook_draft_create_fixture_1']);

  const operationRead = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_mailbox_outlook_draft_operation_read',
    params: { site_id: 'site_fixture', operation_id: 'operation_mailbox_draft_create', mailbox_outlook_draft_create_limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(operationRead.status, 200);
  const operationReadBody = await operationRead.json();
  assert.equal(operationReadBody.mailbox_outlook_draft_creates.length, 1);
  assert.equal(operationReadBody.operation_product_surface.mailbox_outlook_draft_create_count, 1);
  assert.equal(operationReadBody.operation_product_surface.mailbox_outlook_draft_create_admission, 'admitted');
  assert.equal(operationReadBody.operation_product_surface.mailbox_send_admission, 'not_admitted');
  assert.equal(operationReadBody.operation_product_surface.mailbox_mutation_admission, 'not_admitted');
  ...
  assert.equal(operationReadBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'mailbox_outlook_draft_create'), true);
  assert.equal(operationReadBody.operation_workflow_route.operator_focus.schema, 'narada.cloudflare_operation_operator_focus.v1');
  ...
  assert.deepEqual(sendListedBody.sends.map((entry: any) => entry.send_accepted_id), ['mailbox_send_accepted_fixture_1']);
  ...
  assert.equal(operationReadAfterSendBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.authority === 'mailbox_send'), false);
  assert.equal(operationReadAfterSendBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.authority === 'mailbox_delivery_confirmation'), true);
  assert.equal(operationReadAfterSendBody.authority_transfer_posture.next_action, 'transfer_mailbox_status_source_authority');
  assert.equal(operationReadAfterSendBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'mailbox_send_accepted'), true);
  ...
  assert.deepEqual(confirmationListedBody.confirmations.map((entry: any) => entry.send_confirmation_id), ['mailbox_send_confirmation_fixture_1']);
  ...
  assert.equal(operationReadAfterConfirmationBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.authority === 'mailbox_delivery_confirmation'), false);
  assert.equal(operationReadAfterConfirmationBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'mailbox_send_confirmation'), true);
  ...
  assert.deepEqual(reviewListedBody.reviews.map((entry: any) => entry.review_id), ['mailbox_send_review_fixture_1']);
  ...
  assert.equal(operationReadAfterReviewBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'mailbox_send_review_acknowledgement'), true);
...
test('worker records site file change proposals without admitting filesystem or publication mutation', async () => {
  ...
  assert.equal(recordedBody.record.recorded_by_principal_id, 'admin');
  assert.deepEqual(siteDb.dump().siteFileChangeProposals.map((entry: any) => entry.proposal_id), ['site_file_change_fixture_1']);
  ...
  assert.deepEqual(listedBody.proposals.map((entry: any) => entry.proposal_id), ['site_file_change_fixture_1']);
  ...
  assert.equal(operationReadBody.operation_workflow_route.operator_focus.action, 'review_site_file_change_proposal');
  assert.equal(operationReadBody.operation_workflow_route.operator_focus.focus_kind, 'site_file_change_proposal');
  assert.equal(operationReadBody.operation_workflow_route.operator_focus.focus_ref, 'site_file_change_fixture_1');

  const refusedReview = await worker.fetch(jsonRequest({
    operation: 'operation_focus_review.acknowledge',
    request_id: 'request_site_file_change_focus_review_refused',
    params: { site_id: 'site_fixture', focus_kind: 'site_file_change_proposal', focus_ref: 'missing-site-file-change-proposal' },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  ...
  const focusReview = await worker.fetch(jsonRequest({
    operation: 'operation_focus_review.acknowledge',
    request_id: 'request_site_file_change_focus_review_acknowledge',
    params: {
      site_id: 'site_fixture',
      review_id: 'operation_focus_review_site_file_change_fixture_1',
      focus_kind: 'site_file_change_proposal',
      focus_ref: 'site_file_change_fixture_1',
      review_action: 'acknowledge_operation_focus_review',
      note: 'operator reviewed site file change proposal without filesystem mutation',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  ...
  const operationReadAfterReview = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_site_file_change_operation_read_after_focus_review',
    params: { site_id: 'site_fixture', operation_id: 'operation_site_file_change', site_file_change_proposal_limit: 10, operation_focus_review_limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(operationReadAfterReview.status, 200);
  const operationReadAfterReviewBody = await operationReadAfterReview.json();
  assert.equal(operationReadAfterReviewBody.operation_focus_reviews.length, 1);
  assert.equal(operationReadAfterReviewBody.operation_product_surface.operation_focus_review_count, 1);
  assert.equal(operationReadAfterReviewBody.operation_product_surface.operation_focus_review_authority, 'cloudflare_operator_operation_focus_review');
  assert.equal(operationReadAfterReviewBody.operation_product_surface.operation_focus_review_admission, 'admitted');
  assert.equal(operationReadAfterReviewBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'operation_focus_review_acknowledgement'), true);
  assert.notEqual(operationReadAfterReviewBody.operation_workflow_route.operator_focus?.focus_ref, 'site_file_change_fixture_1');
  assert.notEqual(operationReadAfterReviewBody.operation_workflow_route.next_action, 'review_site_file_change_proposal');
});

test('worker admits Cloudflare site file materialization records without Windows filesystem or publication mutation', async () => {
  const siteDb = fakeD1SiteRegistryDatabase({
    sites: [{
      site_id: 'site_fixture',
      site_ref: 'site://fixture',
    ...
    operations: [{
      operation_id: 'operation_site_file_materialization',
      site_id: 'site_fixture',
      display_name: 'Site File Materialization Operation',
      operation_kind: 'operating_layer_update',
      status: 'active',
      created_by_principal_id: 'admin',
      created_at: clock(),
  ...
  const sourcePayload = {
    ...
    operation_id: 'operation_site_file_materialization',
    task_id: 'cloudflare-task-file-materialization-1',
    proposal_id: 'site_file_change_fixture_1',
    proposal_ref: 'proposal:file-change:1',
    file_path: 'docs/architecture/cloudflare-carrier/target.md',
    content_sha256: '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef',
    content_ref: 'cloudflare-site-file-store:target-md:1',
    materialization_authority_ref: 'cloudflare-carrier:site-file-materialization:v1',
    cutover_point_ref: 'cutover:cloudflare-site-file-materialization:test',
    governed_write_contract_ref: 'contract:cloudflare-site-file-materialization:test',
    confirmation_evidence_ref: 'evidence:operator-confirmed-materialization:test',
    authority_locus: 'cloudflare_carrier_site',
    filesystem_executor_authority: 'cloudflare_site_file_store',
    windows_filesystem_mutation_admission: 'not_admitted',
    repository_publication_admission: 'not_admitted',
  ...
  const refusedCutover = await worker.fetch(jsonRequest({
    operation: 'site_file_materialization.admit',
    request_id: 'request_site_file_materialization_refused_cutover',
    params: {
      site_id: 'site_fixture',
      materialization_id: 'site_file_materialization_refused_cutover',
      source_payload: { ...sourcePayload, cloudflare_site_file_materialization_cutover: false },
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(refusedCutover.status, 400);
  const refusedCutoverBody = await refusedCutover.json();
  assert.equal(refusedCutoverBody.code, 'site_file_materialization_cutover_evidence_required');

  const refusedWindowsMutation = await worker.fetch(jsonRequest({
    operation: 'site_file_materialization.admit',
    request_id: 'request_site_file_materialization_refused_windows_mutation',
    params: {
      site_id: 'site_fixture',
      materialization_id: 'site_file_materialization_refused_windows_mutation',
      source_payload: { ...sourcePayload, windows_filesystem_mutation_admission: 'admitted' },
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(refusedWindowsMutation.status, 400);
  const refusedWindowsMutationBody = await refusedWindowsMutation.json();
  assert.equal(refusedWindowsMutationBody.code, 'site_file_materialization_windows_filesystem_mutation_admission_invalid');

  const recorded = await worker.fetch(jsonRequest({
    operation: 'site_file_materialization.admit',
    request_id: 'request_site_file_materialization_admit',
    params: {
      site_id: 'site_fixture',
      materialization_id: 'site_file_materialization_fixture_1',
      source_payload: sourcePayload,
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(recorded.status, 200);
  const recordedBody = await recorded.json();
  assert.equal(recordedBody.schema, 'narada.sonar.cloudflare_site_file_materialization.v1');
  assert.equal(recordedBody.status, 'admitted');
  assert.equal(recordedBody.site_file_materialization_authority, 'cloudflare_carrier_site');
  assert.equal(recordedBody.cloudflare_site_file_materialization_admission, 'admitted');
  assert.equal(recordedBody.filesystem_executor_authority, 'cloudflare_site_file_store');
  assert.equal(recordedBody.windows_filesystem_mutation_admission, 'not_admitted');
  assert.equal(recordedBody.repository_publication_admission, 'not_admitted');
  assert.equal(recordedBody.write_effect, 'cloudflare_site_file_materialization_record');
  assert.deepEqual(siteDb.dump().siteFileMaterializations.map((entry: any) => entry.materialization_id), ['site_file_materialization_fixture_1']);

  const listed = await worker.fetch(jsonRequest({
    operation: 'site_file_materialization.list',
    request_id: 'request_site_file_materialization_list',
    params: { site_id: 'site_fixture', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.equal(listedBody.site_file_materialization_authority, 'cloudflare_carrier_site');
  assert.equal(listedBody.cloudflare_site_file_materialization_admission, 'admitted');
  assert.equal(listedBody.windows_filesystem_mutation_admission, 'not_admitted');
  assert.equal(listedBody.repository_publication_admission, 'not_admitted');
  assert.equal(listedBody.authority_partition, 'site_file_materialization_cloudflare_owned_windows_filesystem_and_publication_not_admitted');
  assert.deepEqual(listedBody.materializations.map((entry: any) => entry.materialization_id), ['site_file_materialization_fixture_1']);

  const operationRead = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_site_file_materialization_operation_read',
    params: { site_id: 'site_fixture', operation_id: 'operation_site_file_materialization', site_file_materialization_limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(operationRead.status, 200);
  const operationReadBody = await operationRead.json();
  assert.equal(operationReadBody.site_file_materializations.length, 1);
  assert.equal(operationReadBody.operation_product_surface.site_file_materialization_count, 1);
  assert.equal(operationReadBody.operation_product_surface.site_file_materialization_authority, 'cloudflare_carrier_site');
  assert.equal(operationReadBody.operation_product_surface.cloudflare_site_file_materialization_admission, 'admitted');
  assert.equal(operationReadBody.operation_product_surface.cloudflare_site_file_materialization_executor_authority, 'cloudflare_site_file_store');
  ...
  assert.equal(operationReadBody.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'site_file_materialization').classification, 'cloudflare_owned');
  assert.equal(operationReadBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.authority === 'windows_filesystem_executor'), false);
  assert.equal(operationReadBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.authority === 'windows_filesystem_mutation'), false);
  assert.equal(operationReadBody.authority_transfer_posture.remaining_windows_authorities.some((entry: any) => entry.domain === 'site_file_materialization' && entry.authority === 'repository_publication'), true);
});
...
test('worker records task lifecycle shadow reads from Windows without admitting Cloudflare writes', async () => {
  ...
  assert.equal(listedBody.dispatch_authority, 'windows_primary_dispatcher');
  assert.deepEqual(listedBody.reads.map((entry: any) => entry.cloudflare_write_admission), ['not_admitted', 'not_admitted']);
  ...
  assert.deepEqual(admissionListBody.decisions.map((entry: any) => entry.admission_action).sort(), ['admit', 'refuse']);
  ...
  assert.equal(
    focusedTaskLifecycleListBody.tasks.some((task: any) => task.carrier_session_id === 'carrier_session_operation_fixture'),
    true,
  ...
  const refusedAssignmentWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.assignment_write.admit',
    request_id: 'request_task_lifecycle_assignment_write_refused',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_assignment_write_refused_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_agent_id: 'agent-cloudflare',
      assignment_authority_ref: 'assignment-authority:cloudflare-task-lifecycle-assignment:v1',
      assignment_schema_ref: 'schema:cloudflare-task-lifecycle-assignment:v1',
  ...
  assert.equal(refusedAssignmentWrite.status, 403);
  const refusedAssignmentWriteBody = await refusedAssignmentWrite.json();
  assert.equal(refusedAssignmentWriteBody.code, 'task_lifecycle_assignment_write_not_admitted');
  assert.equal(refusedAssignmentWriteBody.decision.reason, 'windows_task_lifecycle_mutation_authority_retained');

  const admittedAssignmentWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.assignment_write.admit',
    request_id: 'request_task_lifecycle_assignment_write_admitted',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_assignment_write_admitted_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_agent_id: 'agent-cloudflare',
      cloudflare_task_assignment_write_cutover: true,
      assignment_authority_ref: 'assignment-authority:cloudflare-task-lifecycle-assignment:v1',
  ...
  assert.equal(admittedAssignmentWrite.status, 200);
  const admittedAssignmentWriteBody = await admittedAssignmentWrite.json();
  assert.equal(admittedAssignmentWriteBody.status, 'task_lifecycle_assignment_written');
  assert.equal(admittedAssignmentWriteBody.write_effect, 'task_lifecycle_assignment_write');
  assert.equal(admittedAssignmentWriteBody.assignment.assignment_authority_admission, 'admitted');
  assert.equal(admittedAssignmentWriteBody.assignment.roster_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.assignment.role_resolution_authority_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.assignment.mailbox_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.assignment.filesystem_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.assignment.repository_publication_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.task.task_lifecycle_assignment_write_admission, 'admitted');
  assert.equal(admittedAssignmentWriteBody.task.task_lifecycle_assignment_write_count, 1);
  assert.equal(admittedAssignmentWriteBody.task.assignment_authority_admission, 'admitted');
  assert.equal(admittedAssignmentWriteBody.task.roster_mutation_admission, 'not_admitted');
  assert.equal(admittedAssignmentWriteBody.task.role_resolution_authority_admission, 'not_admitted');

  const refusedRoleResolutionWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.role_resolution_write.admit',
    request_id: 'request_task_lifecycle_role_resolution_write_refused',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_role_resolution_write_refused_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_principal_id: 'admin',
      role_resolution_authority_ref: 'role-resolution-authority:cloudflare-site-membership:v1',
      roster_source_ref: 'roster-source:cloudflare-site-membership:v1',
      role_resolution_schema_ref: 'schema:cloudflare-task-lifecycle-role-resolution:v1',
      role_resolution_evidence_ref: 'role-resolution-evidence:cloudflare-site-membership-row:v1',
  ...
  assert.equal(refusedRoleResolutionWrite.status, 403);
  const refusedRoleResolutionWriteBody = await refusedRoleResolutionWrite.json();
  assert.equal(refusedRoleResolutionWriteBody.code, 'task_lifecycle_role_resolution_write_not_admitted');
  assert.equal(refusedRoleResolutionWriteBody.decision.reason, 'windows_task_lifecycle_mutation_authority_retained');

  const admittedRoleResolutionWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.role_resolution_write.admit',
    request_id: 'request_task_lifecycle_role_resolution_write_admitted',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_role_resolution_write_admitted_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_principal_id: 'admin',
      cloudflare_task_role_resolution_write_cutover: true,
      role_resolution_authority_ref: 'role-resolution-authority:cloudflare-site-membership:v1',
      ...
      role_resolution_schema_ref: 'schema:cloudflare-task-lifecycle-role-resolution:v1',
      role_resolution_evidence_ref: 'role-resolution-evidence:cloudflare-site-membership-row:v1',
      cutover_point_ref: 'cutover:task-lifecycle-role-resolution-write:v1',
      governed_write_contract_ref: 'contract:task-lifecycle-role-resolution-write:v1',
      confirmation_evidence_ref: 'evidence:operator-check:task-role-resolution-write',
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(admittedRoleResolutionWrite.status, 200);
  const admittedRoleResolutionWriteBody = await admittedRoleResolutionWrite.json();
  assert.equal(admittedRoleResolutionWriteBody.status, 'task_lifecycle_role_resolution_written');
  assert.equal(admittedRoleResolutionWriteBody.write_effect, 'task_lifecycle_role_resolution_write');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.role_resolution_authority_admission, 'admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.roster_read_admission, 'admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.roster_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.mailbox_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.filesystem_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.repository_publication_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWriteBody.role_resolution.resolved_role, 'owner');
  assert.equal(admittedRoleResolutionWriteBody.task.task_lifecycle_role_resolution_write_admission, 'admitted');
  assert.equal(admittedRoleResolutionWriteBody.task.task_lifecycle_role_resolution_write_count, 1);
  assert.equal(admittedRoleResolutionWriteBody.task.role_resolution_authority_admission, 'admitted');
  assert.equal(admittedRoleResolutionWriteBody.task.roster_read_admission, 'admitted');
  assert.equal(admittedRoleResolutionWriteBody.task.roster_mutation_admission, 'not_admitted');
  assert.equal(admittedRoleResolutionWriteBody.task.resolved_assignee_role, 'owner');

  const refusedRosterMutationWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.roster_mutation_write.admit',
    request_id: 'request_task_lifecycle_roster_mutation_write_refused',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_roster_mutation_write_refused_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_principal_id: 'admin',
      roster_mutation_authority_ref: 'roster-mutation-authority:cloudflare-site-membership:v1',
      roster_schema_ref: 'schema:cloudflare-site-membership-roster:v1',
  ...
  assert.equal(refusedRosterMutationWrite.status, 403);
  const refusedRosterMutationWriteBody = await refusedRosterMutationWrite.json();
  assert.equal(refusedRosterMutationWriteBody.code, 'task_lifecycle_roster_mutation_write_not_admitted');
  assert.equal(refusedRosterMutationWriteBody.decision.reason, 'windows_task_lifecycle_mutation_authority_retained');

  const admittedRosterMutationWrite = await worker.fetch(jsonRequest({
    operation: 'task_lifecycle.roster_mutation_write.admit',
    request_id: 'request_task_lifecycle_roster_mutation_write_admitted',
    params: {
      site_id: 'site_fixture',
      admission_id: 'task_lifecycle_roster_mutation_write_admitted_1',
      task_id: admittedCreateBody.task.task_id,
      assignee_principal_id: 'admin',
      cloudflare_task_roster_mutation_write_cutover: true,
      roster_mutation_authority_ref: 'roster-mutation-authority:cloudflare-site-membership:v1',
  ...
  assert.equal(admittedRosterMutationWrite.status, 200);
  const admittedRosterMutationWriteBody = await admittedRosterMutationWrite.json();
  assert.equal(admittedRosterMutationWriteBody.status, 'task_lifecycle_roster_mutation_written');
  assert.equal(admittedRosterMutationWriteBody.write_effect, 'task_lifecycle_roster_mutation_write');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.roster_mutation_admission, 'admitted');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.membership_role, 'owner');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.membership_status, 'active');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.mailbox_mutation_admission, 'not_admitted');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.filesystem_mutation_admission, 'not_admitted');
  assert.equal(admittedRosterMutationWriteBody.roster_mutation.repository_publication_admission, 'not_admitted');
  assert.equal(admittedRosterMutationWriteBody.task.task_lifecycle_roster_mutation_write_admission, 'admitted');
  assert.equal(admittedRosterMutationWriteBody.task.task_lifecycle_roster_mutation_write_count, 1);
  assert.equal(admittedRosterMutationWriteBody.task.roster_mutation_admission, 'admitted');
  assert.equal(admittedRosterMutationWriteBody.task.mailbox_mutation_admission, 'not_admitted');

  const operationReadWithTasks = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_task_lifecycle_task_operation_read',
    params: { site_id: 'site_fixture', operation_id: 'operation_task_lifecycle', task_lifecycle_task_limit: 10, task_lifecycle_write_admission_limit: 20 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  ...
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_assignment_write_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_assignment_write_count, 1);
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].assignment_authority_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_role_resolution_write_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_role_resolution_write_count, 1);
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].role_resolution_authority_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].roster_read_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].resolved_assignee_role, 'owner');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_roster_mutation_write_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].task_lifecycle_roster_mutation_write_count, 1);
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].roster_mutation_authority, 'cloudflare_task_lifecycle_d1');
  assert.equal(operationReadWithTasksBody.task_lifecycle_tasks[0].roster_mutation_admission, 'admitted');
  assert.ok(operationReadWithTasksBody.task_lifecycle_write_admissions.length >= 12);
  ...
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_assignment_write_count, 1);
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_role_resolution_write_count, 1);
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_roster_mutation_write_count, 1);
  ...
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_assignment_authority, 'cloudflare_task_lifecycle_d1');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_role_resolution_authority, 'cloudflare_task_lifecycle_d1');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_roster_mutation_authority, 'cloudflare_task_lifecycle_d1');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_roster_read_admission, 'admitted');
  ...
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_roster_mutation_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_role_resolution_authority_admission, 'admitted');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_authority_partition, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_roster_mutation_cloudflare_remaining_windows_effects');
  assert.equal(operationReadWithTasksBody.operation_product_surface.task_lifecycle_write_admission_posture, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_roster_mutation_admitted_remaining_external_effects_not_admitted');
  ...
  const operationReadWithIncludedTask = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_task_lifecycle_task_operation_read_include_task_id',
    params: {
      site_id: 'site_fixture',
      operation_id: 'operation_task_lifecycle',
      task_lifecycle_task_limit: 0,
      task_lifecycle_include_task_ids: [admittedCreateBody.task.task_id],
    },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(operationReadWithIncludedTask.status, 200);
  const operationReadWithIncludedTaskBody = await operationReadWithIncludedTask.json();
...
test('worker clears task lifecycle external effects after dependent Cloudflare effect domains are evidenced', async () => {
  ...
  assert.equal(body.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'task_lifecycle').classification, 'cloudflare_owned');
  assert.equal(body.authority_transfer_posture.domains.find((domain: any) => domain.domain === 'task_lifecycle').authority_partition, 'task_lifecycle_cloudflare_writes_and_external_effects_cloudflare_owned');
  ...
  assert.equal(body.operation_product_surface.task_lifecycle_write_admission_posture, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_external_effects_admitted');
  assert.equal(body.operation_product_surface.task_lifecycle_authority_partition, 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_external_effects_cloudflare_owned');
});
...
test('worker starts controlled resident dispatch as Cloudflare primary with Windows fallback recorded', async () => {
  ...
  assert.equal(siteDb.dump().carrierSessions.some((session: any) => session.carrier_session_id === 'carrier_session_dispatch_fixture'), true);
  ...
  assert.deepEqual(listedBody.dispatch_decisions.map((entry: any) => entry.decision_state), ['cloudflare_primary_started']);
...
test('worker records resident dispatch Windows fallback request and routes the operation through that governed lane', async () => {
  const failingNamespace = {
    ...
    get() {
      return {
        async fetch() {
          return new Response(JSON.stringify({ ok: false, code: 'carrier_session_start_failed' }), {
            status: 500,
            headers: { 'content-type': 'application/json' },
          });
        },
  ...
  const env = authEnv(failingNamespace, { CLOUDFLARE_SITE_REGISTRY_DB: siteDb });
  ...
  const dispatchedBody = await dispatched.json();
  assert.equal(dispatchedBody.status, 'cloudflare_primary_failed_windows_fallback_available');

  const readBefore = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_operation_read_before_fallback_request',
    params: { site_id: 'site_fixture', operation_id: 'operation_dispatch', resident_dispatch_limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(readBefore.status, 200);
  const readBeforeBody = await readBefore.json();
  assert.equal(readBeforeBody.resident_dispatch_decisions[0].decision_state, 'cloudflare_primary_failed_windows_fallback_available');

  const created = await worker.fetch(jsonRequest({
    operation: 'resident_dispatch.windows_fallback_request.create',
    request_id: 'request_resident_dispatch_windows_fallback_create',
    params: {
      site_id: 'site_fixture',
      source_payload: {
        generated_at: '2026-06-12T16:00:00.000Z',
        operation_id: 'operation_dispatch',
        dispatch_decision_id: 'resident_dispatch_fixture_failed',
        carrier_session_id: 'carrier_session_dispatch_fixture',
  ...
  assert.equal(created.status, 200);
  const createdBody = await created.json();
  assert.equal(createdBody.status, 'recorded');
  assert.equal(createdBody.local_execution_admission, 'pending_windows_admission');
  assert.equal(createdBody.fallback_request.dispatch_decision_id, 'resident_dispatch_fixture_failed');

  const listed = await worker.fetch(jsonRequest({
    operation: 'resident_dispatch.windows_fallback_request.list',
    request_id: 'request_resident_dispatch_windows_fallback_list',
    params: { site_id: 'site_fixture', operation_id: 'operation_dispatch', limit: 10 },
  }, { token: 'test-admin-token', path: '/api/carrier' }), env);
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.equal(listedBody.requests.length, 1);
  assert.equal(listedBody.requests[0].dispatch_decision_id, 'resident_dispatch_fixture_failed');

  const readAfter = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_operation_read_after_fallback_request',
    params: {
      site_id: 'site_fixture',
      operation_id: 'operation_dispatch',
  ...
  assert.equal(readAfterFocusReviewBody.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'operation_focus_review_acknowledgement'), true);
...
test('worker site.membership.put admits owner and exposes membership through site.read', async () => {
  ...
  const readBody = await read.json();
  assert.equal(readBody.memberships.some((membership: any) => (
    membership.principal_id === 'microsoft:tenant:operator'
    && membership.role === 'viewer'
    && membership.status === 'active'
  )), true);
  assert.equal(readBody.authority_events.some((event: any) => event.event_kind === 'site_membership_updated'), true);
});
...
test('worker site.list exposes product statuses across visible sites', async () => {
  ...
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.deepEqual(listedBody.sites.map((site: any) => site.site_id), ['site_alpha', 'site_beta']);
...
test('worker operation.create read and list route through site registry authority', async () => {
  ...
  assert.equal(start.status, 200);
  assert.equal(siteDb.dump().carrierSessions.some((session: any) => session.operation_id === 'operation_control'), true, JSON.stringify(siteDb.dump().carrierSessions));
  ...
  const taskCreateResult = taskCreateBody.events?.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  assert.equal(directSiteTasks.results.some((task: any) => task.carrier_session_id === 'carrier_session_operation_fixture'), true, JSON.stringify(directSiteTasks.results));
  ...
  assert.equal(readBody.sessions.some((session: any) => session.carrier_session_id === 'carrier_session_operation_fixture'), true, JSON.stringify(readBody.sessions));
  assert.equal(readBody.tasks.some((task: any) => task.carrier_session_id === 'carrier_session_operation_fixture'), true, JSON.stringify(readBody.tasks));
  assert.equal(readBody.carrier_evidence.some((entry: any) => entry.carrier_session_id === 'carrier_session_operation_fixture'), true, JSON.stringify(readBody.carrier_evidence));
  ...
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'operation_status_transition' && item.summary === 'active -> inactive'), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'operation_session_binding'), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'operation_task'), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'carrier_evidence_event'), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.focus_kind === 'operation_task' && item.focus_ref === taskCreateSummary.task.task_id), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.focus_kind === 'operation_session' && item.focus_ref === 'carrier_session_operation_fixture'), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.focus_kind === 'operation_authority_event' && item.focus_ref), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  assert.equal(pausedReadBody.operation_activity_timeline.items.some((item: any) => item.focus_kind === 'carrier_evidence_event' && item.focus_ref.startsWith('carrier_session_operation_fixture:')), true, JSON.stringify(pausedReadBody.operation_activity_timeline.items));
  ...
  const pausedAuthorityEvent = pausedReadBody.authority_events.find((event: any) => event.event_kind === 'site_operation_status_updated');
  ...
  assert.deepEqual(resumedReadBody.operation_status_history.transitions.map((transition: any) => transition.to_status), ['inactive', 'active']);
  assert.deepEqual(resumedReadBody.operation_status_history.transitions.map((transition: any) => transition.reason), ['operation_paused_by_operator', 'operation_resumed_by_operator']);
  ...
  assert.equal(resumedReadBody.operation_activity_timeline.items.some((item: any) => item.activity_kind === 'operation_status_transition' && item.summary === 'inactive -> active'), true, JSON.stringify(resumedReadBody.operation_activity_timeline.items));
  ...
  assert.equal(listed.status, 200);
  const listedBody = await listed.json();
  assert.deepEqual(listedBody.operations.map((operation: any) => operation.operation_id), ['operation_control']);
  ...
  assert.equal(siteRead.status, 200);
  const siteReadBody = await siteRead.json();
  assert.equal(siteReadBody.operations.some((operation: any) => operation.operation_id === 'operation_control'), true);
});
...
test('worker browser API alias starts resumes sends input and reads evidence events', async () => {
  ...
  assert.ok(readBody.events.some((event: any) => event.event_kind === 'provider_request_recorded'));
  const providerEvent = readBody.events.find((event: any) => event.event_kind === 'provider_request_recorded');
...
test('configured Cloudflare task tools admit command-triggered task create update and persisted readback', async () => {
  ...
  const createResult = createdBody.events.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  const updateResult = updatedBody.events.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  assert.deepEqual(persistedBody.events.map((event: any) => event.event_kind), [
    'carrier_session_started',
    'tool_call_requested',
...
test('provider tool call can create a Cloudflare Narada task through admitted task effect', async () => {
  ...
  const toolResult = body.events.find((event: any) => event.event_kind === 'tool_result_received');
  ...
  assert.deepEqual(durableEnv.AI.calls[0].request.tools.map((tool: any) => tool.function.name), [
    'cloudflare_carrier_task_create',
    'cloudflare_carrier_task_update',
    'cloudflare_carrier_task_list',
  ]);
...
test('worker provider adapter completes turns through Cloudflare AI binding', async () => {
  ...
  const providerRequest = body.events.find((event: any) => event.event_kind === 'provider_request_recorded');
  ...
  const output = body.events.find((event: any) => event.event_kind === 'provider_text_delta_recorded');
...
test('provider tool calls are denied when the Cloudflare effect adapter is not configured', async () => {
  ...
  const textDeltas = body.events.filter((event: any) => event.event_kind === 'provider_text_delta_recorded');
...
test('configured Cloudflare tool adapter admits only runtime metadata read effects', async () => {
  ...
  assert.equal(response.status, 200);
  const body = await response.json();
  const toolCall = body.events.find((event: any) => event.event_kind === 'provider_tool_call_requested');
...
test('configured Cloudflare KV tool adapter admits read-only key gets', async () => {
  ...
  assert.equal(durableEnv.AI.calls.length, 2);
  assert.deepEqual(durableEnv.AI.calls[0].request.tools.map((tool: any) => tool.function.name), ['cloudflare_carrier_kv_get']);
...
test('configured Cloudflare KV write tool requires write flag and principal authority', async () => {
  ...
  assert.deepEqual(durableEnv.AI.calls[0].request.tools.map((tool: any) => tool.function.name), ['cloudflare_carrier_kv_put']);
...
test('provider follow-up tool calls are processed in bounded batches', async () => {
  ...
  const body = await response.json();
  assert.equal(body.terminal_state, 'completed');
  const toolResults = body.events.filter((event: any) => event.event_kind === 'tool_result_received');
  ...
  const providerToolCalls = body.events.filter((event: any) => event.event_kind === 'provider_tool_call_requested');
  assert.deepEqual(providerToolCalls.map((event: any) => event.payload.sequence), [2, 3]);
  const textDeltas = body.events.filter((event: any) => event.event_kind === 'provider_text_delta_recorded');
  assert.deepEqual(textDeltas.map((event: any) => event.payload.text_delta), [
    'Reading first key.',
    'Reading second key.',
...
test('malformed tool effect result fails turn before invalid evidence append', async () => {
  ...
  assert.equal(response.terminal_state, 'failed');
  assert.equal(response.events.some((event: any) => event.event_kind === 'tool_result_received'), false);
  const failed = response.events.find((event: any) => event.event_kind === 'turn_failed');
  ...
  assert.deepEqual(
    response.events.map((event: any) => event.sequence),
    response.events.map((_: any, index: any) => index + 1),
  );
  assertValidEvents(response);
});
...
test('plain tool effect adapter result remains valid without admission evidence', async () => {
  ...
  assert.equal(response.terminal_state, 'completed');
  const toolResult = response.events.find((event: any) => event.event_kind === 'tool_result_received');
  assert.equal(toolResult.payload.status, 'ok');
...
test('throwing tool effect adapter records failed tool result and provider follow-up', async () => {
  ...
  const textDeltas = response.events.filter((event: any) => event.event_kind === 'provider_text_delta_recorded');
...
test('carrier auth classifier matches revolution bearer token principal shapes', () => {
  const admin: any = authenticateCarrierRequest(jsonRequest({}, { token: 'test-admin-token' }), { ADMIN_BEARER_TOKEN: 'test-admin-token' });
  ...
  const service: any = authenticateCarrierRequest(jsonRequest({}, { token: 'test-service-token' }), { SERVICE_TOKEN: 'test-service-token' });
...
function fakeDurableObjectNamespace(durableEnv: any = {}) {
...
function deferred() {
  ...
  const promise = new Promise<unknown>((promiseResolve, promiseReject) => {
    resolve = (value?: unknown) => promiseResolve(value);
    reject = (reason?: unknown) => promiseReject(reason);
  });
  return { promise, resolve, reject };
}

function authEnv(namespace: any, extra: any = {}) {
  return {
...
function microsoftAuthEnv(extraClaims: any = {}) {
  return {
...
function fakeAiBinding(response: any) {
  const responses = Array.isArray(response) ? [...response] : [response];
  const calls: any[] = [];
  return {
    calls,
    async run(model: any, request: any) {
...
function fakeKvBinding(values: any = {}) {
  const state = { ...values };
  return {
    async get(key: any) {
...
test('worker operation.read does not smear indexed carrier evidence across sibling operations', async () => {
  const now = clock();
  ...
  const read = await worker.fetch(jsonRequest({
    operation: 'operation.read',
    request_id: 'request_operation_read_scope_isolated',
    params: {
      site_id: 'site_fixture',
      operation_id: 'operation_site_read',
  ...
  assert.equal(read.status, 200);
  const body = await read.json();
  assert.equal(body.operation.operation_id, 'operation_site_read');
  assert.equal(body.operation_lifecycle_status.session_count, 0);
  assert.equal(body.operation_lifecycle_status.next_action, 'session');
  assert.equal(body.operation_activity_timeline.items.some((entry: any) => entry.activity_kind === 'carrier_evidence_event'), false);
...
function jsonRequest(body: any, { token = null, cookie = null, path = '/control' }: any = {}) {
