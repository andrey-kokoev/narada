...
function registryError(code: any, message: any, details: any= {}) {
...
function requireOperation(value: any) {
...
export function defineCloudflareOperationHandler(definition: any= {}) {
  return normalizeDefinition(definition);
}

export function createCloudflareOperationRegistry(definitions: any= []) {
  ...
  return Object.freeze({
    ...
    async dispatch(operation: any, context: any) {
      const entry = byOperation.get(operation);
      if (!entry) {
