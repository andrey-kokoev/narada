...
import { cloudflareIntelligenceDiagnosticsEnabled } from './cloudflare-carrier.ts';
...
function secretRef(binding: any, value: any) {
...
function bindingPosture(value: any) {
  return Object.freeze({ configured: configured(value) });
}

export function createCloudflareCarrierConfig(env: any= {}) {
  const bindings = {
