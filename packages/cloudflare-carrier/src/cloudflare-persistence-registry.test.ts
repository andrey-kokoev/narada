...
} from './cloudflare-persistence-registry.ts';
import { createCloudflareD1TaskStoreAdapter } from './cloudflare-d1-task-store-adapter.ts';
import { fakeD1TaskDatabase } from './cloudflare-d1-test-fixtures.ts';

test('carrier persistence manifest gives every runtime table one bounded-context owner', () => {
  const tables = CLOUDFLARE_CARRIER_PERSISTENCE_SCHEMA_MANIFEST.map(({ table }: any) => table);
  ...
  const runtimeDdl = [
    readFileSync(new URL('./cloudflare-worker.ts', import.meta.url), 'utf8'),
    readFileSync(new URL('./cloudflare-d1-task-store-adapter.ts', import.meta.url), 'utf8'),
  ].flatMap((source: any) => [...source.matchAll(/CREATE TABLE IF NOT EXISTS\s+([a-z0-9_]+)/giu)].map(([, table]: any) => table));
  assert.deepEqual([...new Set(runtimeDdl)].sort(), [...tables].sort());

  const carrierTest = readFileSync(new URL('./contracts/cloudflare-carrier-contract-cases.ts', import.meta.url), 'utf8');
  assert.match(carrierTest, /cloudflare-d1-test-fixtures\.ts/);
...
test('persistence repositories reject cross-domain SQL instead of becoming a shared database service', async () => {
  ...
  const registry = createCloudflarePersistenceRegistry(db);
  assert.ok(registry);
  const taskRepository: any = registry.repository('task-store');
  assert.throws(
...
test('SQLite D1 and the reusable carrier fixture preserve task idempotency, ordering, update, and readback semantics', async () => {
  ...
  const exercise = async (store: any) => {
  ...
  assert.deepEqual(sqliteResult.listed.map(({ task_number, status }: any) => ({ task_number, status })), [
