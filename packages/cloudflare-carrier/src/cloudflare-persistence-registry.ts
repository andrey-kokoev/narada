...
const PERSISTENCE_DOMAINS: any = Object.freeze({
  'carrier-events': {
...
export const CLOUDFLARE_CARRIER_PERSISTENCE_SCHEMA_MANIFEST = Object.freeze(
  Object.entries(PERSISTENCE_DOMAINS).flatMap(([domain, definition]: any) => definition.tables.map((table: any) => ({
    domain,
...
const TABLE_TO_DOMAIN = new Map(
  CLOUDFLARE_CARRIER_PERSISTENCE_SCHEMA_MANIFEST.map(({ table, domain }: any) => [table, domain]),
);

export function createCloudflarePersistenceRegistry(db: any) {
  if (!db || typeof db.prepare !== 'function') return null;
  const repositories = new Map(
    Object.keys(PERSISTENCE_DOMAINS).map((domain: any) => [
      domain,
  ...
  return {
    ...
    repository(domain: any) {
      return repositories.get(domain) ?? null;
    },
  };
}

export function createCloudflarePersistenceRepository(db: any, domain: any) {
  if (!db || typeof db.prepare !== 'function') return null;
  ...
  return {
    ...
    prepare,
    async run(sql: any, ...bindings: any[]) {
      return prepare(sql).bind(...bindings).run();
    },
    async first(sql: any, ...bindings: any[]) {
      return prepare(sql).bind(...bindings).first();
    },
    async all(sql: any, ...bindings: any[]) {
      return prepare(sql).bind(...bindings).all();
    },
  };
}

export function persistenceDomainForTable(table: any) {
  return TABLE_TO_DOMAIN.get(table) ?? null;
}

function assertPersistenceSqlScope(sql: any, tables: any, domain: any) {
  const referencedTables = [...String(sql).matchAll(/\b(?:from|join|into|update|table)\s+([a-z0-9_]+)/giu)]
    .map((match: any) => match[1].toLowerCase())
    .filter((table: any) => TABLE_TO_DOMAIN.has(table));
  const foreignTable = referencedTables.find((table: any) => !tables.has(table));
  if (foreignTable) {
