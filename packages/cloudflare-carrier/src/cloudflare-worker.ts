...
import { CloudflareCarrierDurableObjectBase } from './cloudflare-carrier-durable-object.ts';
import { cloudflareCarrierSessionMutates } from './cloudflare-carrier.ts';
import { createCloudflareCarrierHttpRouter } from './cloudflare-http-router.ts';
import { createCloudflareProductOperationRegistry } from './cloudflare-product-operation-registry.ts';
import { createCloudflareProviderAdapter } from './cloudflare-provider-adapter.ts';
import { createCloudflareD1TaskStoreAdapter as createCloudflareD1TaskStoreAdapterBoundary } from './cloudflare-d1-task-store-adapter.ts';
...
} from './cloudflare-operator-console.ts';
import { renderCloudflareOperatorConsoleAsset } from './cloudflare-operator-console-asset.ts';
...
export class CloudflareCarrierDurableObject extends CloudflareCarrierDurableObjectBase {
  constructor(state: any, env: any= {}) {
    super(state, env, {
      createProviderAdapter: (runtimeEnv: any, config: any) => createCloudflareAiProviderAdapter(runtimeEnv, { config }),
      createToolEffectAdapter: (runtimeEnv: any) => createCloudflareToolEffectAdapter(runtimeEnv),
      createTaskStoreAdapter: (runtimeEnv: any) => createCloudflareD1TaskStoreAdapter(runtimeEnv),
      recordEvidenceEvents: recordCloudflareCarrierEvidenceEvents,
    });
  }
}

export function normalizeCloudflareOperationPostureOverview(overview: any= null, route: any= null, focusedLifecycle: any= null, operationCount: any= 0) {
...
function cloudflareSiteProductAttentionAction(attention: any= [], {
...
async function resolveCloudflareGithubRepositoryPublicationCredential(env: any= {}, fetchImpl: any= fetch) {
...
async function createGithubAppJwt(appId: any, privateKeyPem: any) {
...
function pemToPkcs8Bytes(pem: any) {
...
async function getCloudflareTaskLifecycleTask(db: any, siteId: any, taskId: any) {
...
async function importCloudflareContinuityPacket(env: any= {}, packet: any, { imported_by_principal_id = 'unknown-principal' }: any= {}) {
...
async function claimCloudflareTaskLifecycleTask(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
async function reportCloudflareTaskLifecycleTask(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function finishCloudflareTaskLifecycleTask(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareChangedFileEvidence(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareTaskLifecycleProjectionWrite(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareTaskLifecycleSourceStateWrite(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareTaskLifecycleAssignmentWrite(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareTaskLifecycleRoleResolutionWrite(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
async function recordCloudflareTaskLifecycleRosterMutationWrite(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const taskId = String(params.task_id ?? '').trim();
...
function parseCloudflareTaskLifecycleStringList(value: any) {
  if (value == null || value === '') return { ok: true, value: [] };
  if (Array.isArray(value)) {
    if (!value.every((entry: any) => typeof entry === 'string')) return { ok: false, code: 'task_lifecycle_report_string_list_invalid' };
  ...
  if (text.startsWith('[')) {
    ...
    if (!parsed.every((entry: any) => typeof entry === 'string')) return { ok: false, code: 'task_lifecycle_report_string_list_invalid' };
    return { ok: true, value: parsed };
  }
  return { ok: true, value: text.split(',').map((entry: any) => entry.trim()).filter(Boolean) };
}

function parseCloudflareTaskLifecycleVerification(value: any) {
  if (value == null || value === '') return { ok: true, value: [] };
  ...
  return { ok: true, value: parsed.map((item: any) => ({ command: String(item.command), result: String(item.result) })) };
}

function summarizeCloudflareSiteContinuityLoopStatus(siteId: any, loopReports: any= [], { nowMs = Date.now(), staleAfterMs = DEFAULT_SITE_CONTINUITY_LOOP_STALE_AFTER_MS }: any= {}) {
...
function createSiteContinuityLoopReportId(report: any= {}) {
  return [
    'site-continuity-loop',
    report.site_id || 'unknown-site',
    report.generated_at || report.recorded_at || 'unknown-time',
  ].map((part: any) => String(part).replace(/[^A-Za-z0-9_.:-]/g, '_')).join(':');
}

function summarizeSiteContinuityLoopReport(report: any= {}) {
  return {
...
async function importCloudflareContinuityLoopReport(env: any= {}, report: any, { recorded_by_principal_id = 'unknown-principal' }: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
function summarizeCloudflareSiteContinuityReconciliationExecutionStatus(siteId: any, executions: any= []) {
...
function createSiteContinuityReconciliationExecutionId(execution: any= {}, siteId: any= null) {
  return [
...
function inferSiteIdForReconciliationExecution(execution: any= {}, fallbackSiteId: any= null) {
  ...
  const resultSites = [...new Set((execution.results ?? []).map((result: any) => result?.site_id).filter(Boolean))];
  if (resultSites.length === 1) return resultSites[0];
  const selectedSites = [...new Set((execution.plan?.reconciliation_plan?.selected_sites ?? []).map((site: any) => site?.site_id).filter(Boolean))];
  if (selectedSites.length === 1) return selectedSites[0];
  return null;
}

function summarizeSiteContinuityReconciliationExecution(execution: any= {}, siteId: any= null) {
...
async function importCloudflareContinuityReconciliationExecution(env: any= {}, execution: any, { site_id = null, recorded_by_principal_id = 'unknown-principal' }: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
function continuityBindingEmbodiment(binding: any= {}, embodimentKind: any) {
  return (binding.embodiments || []).find((embodiment: any) => embodiment.embodiment_kind === embodimentKind) ?? {};
}

function isDefaultContinuityBridgeRef(value: any) {
  return ['local-windows-site', 'cloudflare-site', 'site-authority-map:v1'].includes(String(value ?? ''));
}

function continuityBridgeRefQuality(binding: any= {}) {
  binding = binding ?? {};
  ...
  return refs.filter((ref: any) => !isDefaultContinuityBridgeRef(ref)).length;
}

function observedContinuityPacketBinding(continuityPackets: any= []) {
  const packetBindings = (Array.isArray(continuityPackets) ? continuityPackets : [])
    .map((record: any) => record?.packet?.binding)
    .filter(Boolean);
  return packetBindings.find((binding: any) => continuityBridgeRefQuality(binding) > 0) ?? packetBindings[0] ?? null;
}

function summarizeLocalCloudContinuityBridge(siteId: any, continuityPackets: any= [], siteContinuity: any= null, continuityStatus: any= null) {
...
function summarizeCloudflareSiteProductOverview(siteProductStatuses: any= [], siteProductProjections: any= []) {
  ...
  const firstActionable = statuses.find((status: any) => status?.next_action && status.next_action !== 'monitor_site');
  const actionableProjection = firstActionable
    ? projections.find((projection: any) => (
  ...
  const actionableSessionRecord = actionableOperationId && Array.isArray(actionableProjection?.sessions)
    ? actionableProjection.sessions.find((item: any) => item?.operation_id === actionableOperationId) ?? actionableProjection.sessions[0] ?? null
    : null;
...
function summarizeCloudflareOperationPostureOverview(operations: any= [], product: any= {}, context: any= {}) {
  const items = cloudflareOperationWorkQueueItems(operations, product, context);
  const healthCounts: any = { ready: 0, needs_attention: 0 };
  const actionCounts: any = {};
  const reasonCounts: any = {};
  const commandStateCounts: any = {};
  ...
  const next: any = items.find((item: any) => item.status === 'needs_attention')
    || items.find((item: any) => item.operation.operation_id === activeOperationId)
    || items[0]
    || null;
...
function summarizeCloudflareSitePostureRoute(overview: any= {}, focusedSiteId: any= '') {
...
function summarizeCloudflareOperationPostureRoute(overview: any= {}, activeOperationId: any= '') {
...
export function summarizeCloudflareOperationWorkflowRoute({
  ...
  const openTasks = (Array.isArray(tasks) ? tasks : []).filter((task: any) => !['done', 'closed', 'cancelled', 'resolved'].includes(String(task.status ?? '').toLowerCase()));
  ...
  const latestDispatchDecision = dispatchDecisions.find((decision: any) => {
    if (!decision) return false;
    if (operationId && decision.operation_id) return decision.operation_id === operationId;
    return true;
  }) ?? null;
  const latestFallbackRequest = fallbackRequests.find((request: any) => {
  ...
  const latestFallbackEvidence = fallbackEvidence.find((entry: any) => {
...
function cloudflareReviewedMailboxSendFocusKeys(mailboxSendReviews: any= []) {
...
function cloudflareReviewedOperationFocusKeys(operationFocusReviews: any= []) {
  const reviewed = new Set();
...
export function summarizeCloudflareOperationOperatorFocus(operationActivityTimeline: any= null, options: any= {}) {
  ...
  for (const [activityKind, action] of priorities) {
    const matchingItems = items.filter((entry: any) => entry?.activity_kind === activityKind);
    ...
    const item = matchingItems.find((entry: any) => {
...
function cloudflareOperationWorkQueueItems(operations: any= [], product: any= {}, context: any= {}) {
  const activeOperationId = context.active_operation_id || product.operation?.operation_id || '';
  return (Array.isArray(operations) ? operations : []).map((operation: any) => {
...
export function selectCloudflareFocusedOperation(operations: any= [], params: any= {}, response: any= {}) {
  const requestedOperationId = String(params.operation_id ?? '').trim();
  const candidates = Array.isArray(operations) ? operations : [];
  if (requestedOperationId) {
    const requested = candidates.find((operation: any) => operation.operation_id === requestedOperationId);
    if (requested) return requested;
  }
  return candidates.find((operation: any) => operation.status === 'active')
    ?? candidates[0]
    ?? response.operation
    ?? null;
}

function cloudflareOperationPathContext(operation: any= {}, product: any= {}, context: any= {}) {
  ...
  const attention = cloudflareOperationAttention(product).filter((item: any) => !item.operation_id || item.operation_id === operationId);
  const openTasks = tasks.filter((task: any) => cloudflareTaskLifecycleStatus(task) === 'open');
  const openAttention = attention.filter((item: any) => item.status !== 'resolved');
...
function cloudflareOperationScopeLoaded(operation: any= {}, product: any= {}, context: any= {}) {
...
function cloudflareOperationSessions(operationId: any, product: any= {}) {
  if (!operationId) return [];
  return (product.sessions || []).filter((session: any) => !session.operation_id || session.operation_id === operationId);
}

function cloudflareOperationLocalResidentSessionInhabitanceCount(operationId: any, product: any= {}) {
...
export function shouldKeepFocusedOperationProjection(focusedProjection: any= null, selectedProjection: any= null) {
...
function mergeLocalResidentCarrierBridgeSessions(sessions: any= [], bridgeRecords: any= []) {
  const sessionList = Array.isArray(sessions) ? sessions : [];
  const bridgeList = Array.isArray(bridgeRecords) ? bridgeRecords : [];
  const seenCarrierSessionIds = new Set(
    sessionList
      .map((session: any) => String(session?.carrier_session_id ?? session?.session_id ?? '').trim())
      .filter(Boolean),
  );
...
function mergeLocalResidentCarrierBridgeEvidence(carrierEvidence: any= [], bridgeRecords: any= []) {
  const evidenceGroups = Array.isArray(carrierEvidence) ? carrierEvidence : [];
  const bridgeList = Array.isArray(bridgeRecords) ? bridgeRecords : [];
  const seenCarrierSessionIds = new Set(
    evidenceGroups
      .filter((group: any) => Array.isArray(group?.events) && group.events.length > 0)
      .map((group: any) => String(group?.carrier_session_id ?? group?.session_id ?? '').trim())
      .filter(Boolean),
  );
...
function cloudflareOperationHasOnlyLocalResidentInhabitance(operation: any= null, {
  sessions = [],
  residentDispatchWindowsFallbackEvidence = [],
}: any= {}) {
...
function cloudflareOperationTasks(operation: any= {}, product: any= {}, context: any= {}) {
  const operationId = operation.operation_id || context.active_operation_id || product.operation?.operation_id || '';
  if (!operationId) return [];
  const operationSessionIds = new Set(
    cloudflareOperationSessions(operationId, product)
      .map((session: any) => String(session?.carrier_session_id ?? '').trim())
      .filter(Boolean),
  );
  return (product.tasks || []).filter((task: any) => (
    task.operation_id === operationId
    || (task.carrier_session_id && operationSessionIds.has(task.carrier_session_id))
  ));
}

function cloudflareOperationCarrierSessionOperationMap(product: any= {}) {
  const entries = Array.isArray(product.sessions) ? product.sessions : [];
  return new Map(
    entries
      .map((session: any) => [String(session?.carrier_session_id ?? '').trim(), String(session?.operation_id ?? '').trim()])
      .filter(([carrierSessionId, operationId]: any) => carrierSessionId && operationId),
  );
}

function cloudflareOperationEventOperationId(event: any= {}, product: any= {}, fallbackCarrierSessionId: any= '') {
...
function cloudflareOperationEvents(operation: any= {}, product: any= {}) {
  const operationId = operation.operation_id || product.operation?.operation_id || '';
  if (!operationId) return [];
  const seen = new Set();
  return (product.carrier_evidence || []).flatMap((entry: any) => (
    (entry.events || []).filter((event: any) => {
      ...
      seen.add(key);
      return true;
    }).map((event: any) => ({ ...event, carrier_session_id: entry.carrier_session_id || event.carrier_session_id || null }))
  ));
}

function cloudflareOperationAttention(product: any= {}) {
  const tasks = product.tasks || [];
  const sessionOperationMap = cloudflareOperationCarrierSessionOperationMap(product);
  const seen = new Set();
  return (product.carrier_evidence || []).flatMap((entry: any) => (entry.events || []).map((event: any) => ({ entry, event })))
    .filter(({ event }: any) => event.event_kind === 'directive_emitted' && event.payload?.directive_kind === 'operation_attention')
    .map(({ entry, event }: any) => {
      ...
      const resolvedByTask = tasks.find((task: any) => {
...
function cloudflareOperationPostureReason(item: any= {}) {
  const action = item.command?.next_action || 'inspect_operation';
...
function cloudflareTaskLifecycleStatus(task: any= {}) {
  const status = String(task.status || '').toLowerCase();
...
async function listCloudflareContinuityPackets(env: any= {}, siteId: any, limit: any= 100) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
...
function parseCloudflareContinuityPacketJson(packetJson: any) {
...
async function listCloudflareContinuityLoopReports(env: any= {}, siteId: any, limit: any= 20) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareContinuityLoopReportSchema(db);
...
async function listCloudflareContinuityReconciliationExecutions(env: any= {}, siteId: any, limit: any= 20) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareContinuityReconciliationExecutionSchema(db);
...
function summarizeCloudflareSiteContinuityStatus(siteId: any, continuityPackets: any= [], siteContinuity: any= null) {
...
function summarizeCloudflareOperationLifecycleStatus({
  ...
  const openTaskCount = taskList.filter((task: any) => !['done', 'closed', 'cancelled'].includes(String(task.status ?? '').toLowerCase())).length;
  const evidenceGroups = Array.isArray(carrierEvidence) ? carrierEvidence : [];
  const evidenceEventCount = evidenceGroups.reduce((count: any, group: any) => count + (Array.isArray(group.events) ? group.events.length : 0), 0);
...
function summarizeCloudflareLocalIngressOperationPosture({
  ...
  const evidenceRequestIds = new Set(evidence.map((item: any) => item?.local_ingress_request_id).filter(Boolean));
  const pendingRequests = requests.filter((request: any) => !evidenceRequestIds.has(request?.local_ingress_request_id));
  ...
  const completedEvidenceCount = evidence.filter((item: any) => String(item?.local_execution_status ?? '') === 'completed').length;
  const refusedEvidenceCount = evidence.filter((item: any) => {
...
function summarizeCloudflareRepositoryPublicationOperationPosture({
  ...
  const resolvedRequestIds = new Set([
    ...executions.map((item: any) => item?.repository_publication_request_id).filter(Boolean),
    ...evidence.map((item: any) => item?.repository_publication_request_id).filter(Boolean),
  ]);
  const pendingRequests = requests.filter((request: any) => !resolvedRequestIds.has(request?.repository_publication_request_id));
  const admittedRequestIds = new Set(admissions.filter((item: any) => item?.admission_action === 'admit').map((item: any) => item.repository_publication_request_id).filter(Boolean));
  const pendingUnadmittedRequests = pendingRequests.filter((request: any) => !admittedRequestIds.has(request?.repository_publication_request_id));
  ...
  const completedExecutionCount = executions.filter((item: any) => String(item?.publication_status ?? '') === 'completed').length;
  const failedExecutionCount = executions.filter((item: any) => String(item?.publication_status ?? '') === 'failed').length;
  const completedEvidenceCount = evidence.filter((item: any) => String(item?.publication_status ?? '') === 'completed').length;
  const refusedEvidenceCount = evidence.filter((item: any) => {
...
function summarizeCloudflareAuthorityTransferPosture({
  ...
  const classifyCounted = (count: any, observedClassification: any, retainedClassification: any= 'windows_retained') => (count > 0 ? observedClassification : retainedClassification);
  ...
  const taskLifecycleCloudflareWriteCount = taskList.reduce((count: any, task: any) => count
  ...
  const counts = domains.reduce((acc: any, item: any) => {
    acc[item.classification] = (acc[item.classification] ?? 0) + 1;
    return acc;
  }, {});
  const remainingWindowsDomains = domains.filter((item: any) => item.remaining_windows_authority.length > 0).map((item: any) => item.domain);
  const remainingWindowsAuthorities = domains.flatMap((item: any) => item.remaining_windows_authority.map((authority: any) => ({ domain: item.domain, authority })));
  return {
...
function summarizeTaskLifecycleSurfaceWriteAdmissionPosture(taskLifecycleTasks: any= [], externalEffectsReady: any= false) {
  const tasks = Array.isArray(taskLifecycleTasks) ? taskLifecycleTasks : [];
  if (tasks.some((task: any) => task.task_lifecycle_roster_mutation_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_roster_mutation_and_external_effects_admitted' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_roster_mutation_admitted_remaining_external_effects_not_admitted';
  if (tasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_external_effects_admitted' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_role_resolution_admitted_remaining_external_effects_not_admitted';
  if (tasks.some((task: any) => task.task_lifecycle_assignment_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_external_effects_admitted' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted_remaining_external_effects_not_admitted';
  if (tasks.some((task: any) => task.task_lifecycle_source_state_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_external_effects_admitted' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_and_source_state_admitted_remaining_external_effects_not_admitted';
  if (tasks.some((task: any) => task.task_lifecycle_projection_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_and_external_effects_admitted' : 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_admitted_remaining_writes_not_admitted';
  if (tasks.some((task: any) => task.changed_file_evidence_count > 0)) return tasks.some((task: any) => task.finish_id) ? (externalEffectsReady ? 'task_create_claim_report_finish_and_changed_file_evidence_and_external_effects_admitted' : 'task_create_claim_report_finish_and_changed_file_evidence_admitted_remaining_writes_not_admitted') : (externalEffectsReady ? 'task_create_claim_report_and_changed_file_evidence_and_external_effects_admitted' : 'task_create_claim_report_and_changed_file_evidence_admitted_remaining_writes_not_admitted');
  if (tasks.some((task: any) => task.finish_id)) return externalEffectsReady ? 'task_create_claim_report_finish_and_external_effects_admitted' : 'task_create_claim_report_and_finish_admitted_remaining_writes_not_admitted';
  if (tasks.some((task: any) => task.report_id)) return externalEffectsReady ? 'task_create_claim_report_and_external_effects_admitted' : 'task_create_claim_and_report_admitted_remaining_writes_not_admitted';
  if (tasks.some((task: any) => task.status === 'claimed')) return externalEffectsReady ? 'task_create_claim_and_external_effects_admitted' : 'task_create_and_claim_admitted_remaining_writes_not_admitted';
...
function summarizeTaskLifecycleSurfaceAuthorityPartition(taskLifecycleTasks: any= [], externalEffectsReady: any= false) {
  const tasks = Array.isArray(taskLifecycleTasks) ? taskLifecycleTasks : [];
  if (tasks.some((task: any) => task.task_lifecycle_roster_mutation_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_roster_mutation_and_external_effects_cloudflare_owned' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_roster_mutation_cloudflare_remaining_windows_effects';
  if (tasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_external_effects_cloudflare_owned' : 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_role_resolution_cloudflare_remaining_windows_effects';
  ...
  if (tasks.some((task: any) => task.task_lifecycle_projection_write_count > 0)) return externalEffectsReady ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_and_external_effects_cloudflare_owned' : 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_cloudflare_remaining_windows';
  if (tasks.some((task: any) => task.changed_file_evidence_count > 0)) return tasks.some((task: any) => task.finish_id) ? (externalEffectsReady ? 'task_create_claim_report_finish_and_changed_file_evidence_and_external_effects_cloudflare_owned' : 'task_create_claim_report_finish_and_changed_file_evidence_cloudflare_remaining_windows') : (externalEffectsReady ? 'task_create_claim_report_and_changed_file_evidence_and_external_effects_cloudflare_owned' : 'task_create_claim_report_and_changed_file_evidence_cloudflare_remaining_windows');
  if (tasks.some((task: any) => task.finish_id)) return externalEffectsReady ? 'task_create_claim_report_finish_and_external_effects_cloudflare_owned' : 'task_create_claim_report_and_finish_cloudflare_remaining_windows';
  if (tasks.some((task: any) => task.report_id)) return externalEffectsReady ? 'task_create_claim_report_and_external_effects_cloudflare_owned' : 'task_create_claim_and_report_cloudflare_remaining_windows';
  if (tasks.some((task: any) => task.status === 'claimed')) return externalEffectsReady ? 'task_create_claim_and_external_effects_cloudflare_owned' : 'task_create_and_claim_cloudflare_remaining_windows';
...
function summarizeCloudflareOperationStatusHistory(authorityEvents: any= [], operation: any= null) {
  const operationId = operation?.operation_id ?? null;
  const transitions = (Array.isArray(authorityEvents) ? authorityEvents : [])
    .filter((event: any) => event?.event_kind === 'site_operation_status_updated')
    .filter((event: any) => !operationId || event?.evidence?.operation_id === operationId)
    .map((event: any) => ({
    ...
    .sort((left: any, right: any) => String(left.recorded_at || '').localeCompare(String(right.recorded_at || '')));
  return {
...
function summarizeCloudflareOperationActivityTimeline({
  ...
  const sorted = items.sort((left: any, right: any) => {
...
function summarizeCloudflareCarrierEvidenceReadStatus({ sessions = [], carrierEvidence = [], params = {} }: any= {}) {
  const sessionList = Array.isArray(sessions) ? sessions : [];
  const evidenceGroups = Array.isArray(carrierEvidence) ? carrierEvidence : [];
  const sessionIds = sessionList.map((session: any) => session.carrier_session_id).filter(Boolean);
  ...
  const evidenceBySession = new Map<any, any>(evidenceGroups.map((entry: any) => [entry?.carrier_session_id, entry] as [any, any]).filter(([id]: any) => Boolean(id)));
  const missingSessionIds = attemptedSessionIds.filter((sessionId: any) => !evidenceBySession.has(sessionId));
  ...
  const failed = evidenceGroups.filter((entry: any) => entry?.ok !== true);
  const readable = evidenceGroups.filter((entry: any) => entry?.ok === true);
  const eventCount = evidenceGroups.reduce((count: any, group: any) => count + (Array.isArray(group?.events) ? group.events.length : 0), 0);
...
function summarizeCloudflarePersistencePosture(env: any= {}, {
  siteId = null,
  operation = null,
  sessions = [],
  tasks = [],
  ...
  const missing = durableBoundaries.filter((boundary: any) => boundary.status !== 'available').map((boundary: any) => boundary.key);
  const warnings = [];
...
function summarizeCloudflareRecoveryPosture({
  ...
  const evidenceGroups = Array.isArray(carrierEvidence) ? carrierEvidence : [];
  const evidenceEventCount = evidenceGroups.reduce((count: any, group: any) => count + (Array.isArray(group?.events) ? group.events.length : 0), 0);
  const evidenceSources = [...new Set(evidenceGroups.map((group: any) => group?.source).filter(Boolean))];
  const evidenceSessionIds = new Set(evidenceGroups.filter((group: any) => group?.ok === true && (group.events || []).length > 0).map((group: any) => group.carrier_session_id));
  const missingEvidenceSessionIds = Array.isArray(carrierEvidenceReadStatus?.missing_session_ids)
    ...
    : sessionList.map((session: any) => session.carrier_session_id).filter((sessionId: any) => sessionId && !evidenceSessionIds.has(sessionId));
  const sessionSnapshotBoundary = (persistencePosture?.durable_boundaries || []).find((boundary: any) => boundary.key === 'session_snapshot');
  const evidenceIndexBoundary = (persistencePosture?.durable_boundaries || []).find((boundary: any) => boundary.key === 'carrier_evidence_index');
  const recoveryBoundaries = (persistencePosture?.durable_boundaries || []).map((boundary: any) => ({
    key: boundary.key,
    substrate: boundary.substrate,
    status: boundary.status === 'available' ? 'recoverable' : 'unavailable',
    authority: boundary.authority,
  }));
  const unavailableRecoveryBoundaries = recoveryBoundaries.filter((boundary: any) => boundary.status !== 'recoverable').map((boundary: any) => boundary.key);
...
function summarizeCloudflareSiteProductStatus({
  ...
  const evidenceGroups = Array.isArray(carrierEvidence) ? carrierEvidence : [];
  const evidenceEventCount = evidenceGroups.reduce((count: any, group: any) => count + (Array.isArray(group.events) ? group.events.length : 0), 0);
  const activeOperationCount = operationList.filter((operation: any) => String(operation.status ?? '').toLowerCase() === 'active').length;
  const activeMembershipCount = membershipList.filter((membership: any) => String(membership.status ?? '').toLowerCase() === 'active').length;
  const openTaskCount = taskList.filter((task: any) => !['done', 'closed', 'cancelled'].includes(String(task.status ?? '').toLowerCase())).length;
  const continuityState = continuityStatus?.state ?? 'unknown';
  const continuityLoopState = continuityLoopStatus?.state ?? 'unknown';
  const continuityLoopFreshnessState = continuityLoopStatus?.freshness_state ?? 'unknown';
...
function normalizeCloudflareSiteProductStatus(status: any= null, operationPostureOverview: any= null, focusedOperationLifecycle: any= null) {
...
function summarizeCloudflareProductSurfaceReadiness({
  ...
  const incomplete = requiredChecks.filter((check: any) => check.status === 'incomplete');
  const attention = requiredChecks.filter((check: any) => check.status === 'attention');
...
function boundedContinuityPacketReadLimit(value: any= 100) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 100;
  return Math.max(1, Math.min(500, Math.trunc(numeric)));
}

async function ensureCloudflareContinuityPacketSchema(db: any) {
...
async function ensureCloudflareContinuityLoopReportSchema(db: any) {
...
async function ensureCloudflareContinuityReconciliationExecutionSchema(db: any) {
...
async function validateCarrierSiteBindingForRequest(body: any, principal: any, env: any= {}) {
...
function validateCarrierSessionAuthorityForRequest(body: any, env: any= {}) {
...
export const cloudflareProductOperationRegistry = createCloudflareProductOperationRegistry({
  dispatch: ({ body, principal, env, operation }: any) => handleSiteProductApiRequestLegacy(
    { ...body, operation },
    principal,
    env,
  ),
...
export async function handleCloudflareWorkerRequest(request: any, env: any) {
...
export async function handleCloudflareScheduled(controller: any, env: any, ctx: any) {
...
async function handleCarrierApiRequest(request: any, env: any) {
  return cloudflareCarrierHttpRouter(request, env);
}

async function routeCarrierSessionRequest(requestUrl: any, body: any, principal: any, env: any) {
...
function isSiteProductOperation(operation: any) {
  return cloudflareProductOperationRegistry.has(operation);
}

function cloudflareSiteAuthorityMap(env: any= {}, siteId: any= 'unknown-site') {
...
function classifyCloudflareSiteAuthority(env: any= {}, siteId: any= 'unknown-site', mutationClass: any) {
...
function cloudflareSiteAuthorityReadModel(env: any= {}, siteId: any= 'unknown-site') {
  const map = cloudflareSiteAuthorityMap(env, siteId);
  return {
    map,
    decisions: map.entries.map((entry: any) => classifySiteAuthorityRequest(map, {
...
function cloudflareSiteContinuityReadModel(env: any= {}, siteId: any= 'unknown-site') {
...
function siteAuthorityDeniedBody(decision: any, operation: any) {
  return {
    ok: false,
    code: 'site_authority_route_denied',
    operation,
...
async function handleSiteProductApiRequest(body: any, principal: any, env: any= {}) {
...
async function handleSiteProductApiRequestLegacy(body: any, principal: any, env: any= {}) {
  const registry = createCloudflareSiteRegistryAdapter(env);
  ...
  if (body.operation === 'task_lifecycle.write_admission.list') {
    ...
    const hasCloudflareTaskCreateAdmission = decisions.some((decision: any) => {
    ...
    const hasCloudflareTaskClaimAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_claim'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskReportAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_report'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskFinishAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_finish'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareChangedFileEvidenceAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'changed_file_evidence'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskProjectionWriteAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_projection_write'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskSourceStateWriteAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_source_state_write'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskAssignmentWriteAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_assignment_write'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskRoleResolutionWriteAdmission = decisions.some((decision: any) => {
      return decision.mutation_class === 'task_role_resolution_write'
        && decision.admission_action === 'admit'
        && decision.mutation_authority === 'cloudflare_task_lifecycle_d1';
    });
    const hasCloudflareTaskRosterMutationWriteAdmission = decisions.some((decision: any) => {
        ...
        if (focusedOperationRead?.ok) {
          ...
          if (
            ...
            for (const candidate of response.operations ?? []) {
              if (!candidate?.operation_id || candidate.operation_id === selectedProjection.focused_operation_lifecycle?.operation?.operation_id) continue;
          ...
          const selectedOperationPostureRoute = summarizeCloudflareOperationPostureRoute(
            ...
            selectedProjection.focused_operation_lifecycle?.operation_id
              ?? selectedProjection.focused_operation_lifecycle?.operation?.operation_id
...
async function buildCloudflareOperationProductProjection(env: any, registry: any, principal: any, response: any, params: any= {}) {
  ...
  try {
    ...
    const operationCarrierSessionIds = new Set(
      sessions
        .filter((session: any) => !operation?.operation_id || session.operation_id === operation.operation_id)
        .map((session: any) => session.carrier_session_id)
        .filter(Boolean),
    );
    const taskLifecycleTasks = operation?.operation_id
      ? siteTaskLifecycleTasks.filter((task: any) => (
    ...
    ?.find((domain: any) => domain.domain === 'task_lifecycle')
    ...
    operation_product_surface: {
      ...
      mailbox_mutation_admission: 'not_admitted',
      mailbox_authority_partition: authorityTransferPosture.domains.find((domain: any) => domain.domain === 'mailbox_status')?.authority_partition ?? 'mailbox_windows_owned',
      ...
      mailbox_draft_reply_authority_partition: authorityTransferPosture.domains.find((domain: any) => domain.domain === 'mailbox_draft_reply')?.authority_partition ?? 'mailbox_draft_reply_windows_owned',
      mailbox_outlook_draft_create_count: mailboxOutlookDraftCreates.length,
      mailbox_outlook_draft_create_authority_partition: authorityTransferPosture.domains.find((domain: any) => domain.domain === 'mailbox_outlook_draft_create')?.authority_partition ?? 'mailbox_outlook_draft_create_not_observed',
      ...
      site_file_change_authority_partition: authorityTransferPosture.domains.find((domain: any) => domain.domain === 'site_file_change_proposal')?.authority_partition ?? 'filesystem_and_publication_windows_owned',
      ...
      site_file_materialization_authority_partition: authorityTransferPosture.domains.find((domain: any) => domain.domain === 'site_file_materialization')?.authority_partition ?? 'materialization_not_observed_filesystem_and_publication_windows_owned',
      ...
      task_lifecycle_task_claim_count: taskLifecycleTasks.filter((task: any) => task.claimed_at || task.claimed_by_agent_id || task.claimed_by_principal_id).length,
      task_lifecycle_task_report_count: taskLifecycleTasks.filter((task: any) => task.report_id).length,
      task_lifecycle_task_finish_count: taskLifecycleTasks.filter((task: any) => task.finish_id).length,
      task_lifecycle_changed_file_evidence_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.changed_file_evidence_count ?? 0), 0),
      task_lifecycle_projection_write_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.task_lifecycle_projection_write_count ?? 0), 0),
      task_lifecycle_source_state_write_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.task_lifecycle_source_state_write_count ?? 0), 0),
      task_lifecycle_assignment_write_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.task_lifecycle_assignment_write_count ?? 0), 0),
      task_lifecycle_role_resolution_write_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.task_lifecycle_role_resolution_write_count ?? 0), 0),
      task_lifecycle_roster_mutation_write_count: taskLifecycleTasks.reduce((count: any, task: any) => count + Number(task.task_lifecycle_roster_mutation_write_count ?? 0), 0),
      ...
      task_lifecycle_default_cloudflare_write_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_source_state_write_count > 0)
      ...
      task_lifecycle_task_claim_authority: taskLifecycleTasks.some((task: any) => task.claimed_at || task.claimed_by_agent_id || task.claimed_by_principal_id) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_task_report_authority: taskLifecycleTasks.some((task: any) => task.report_id) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_task_finish_authority: taskLifecycleTasks.some((task: any) => task.finish_id) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_surface_write_admission_posture: taskLifecycleSurfaceWriteAdmissionPosture,
      task_lifecycle_surface_authority_partition: taskLifecycleSurfaceAuthorityPartition,
      task_lifecycle_changed_file_evidence_authority: taskLifecycleTasks.some((task: any) => task.changed_file_evidence_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_projection_write_authority: taskLifecycleTasks.some((task: any) => task.task_lifecycle_projection_write_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_source_state_authority: taskLifecycleTasks.some((task: any) => task.task_lifecycle_source_state_write_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_windows_sqlite_source_write_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_source_state_write_count > 0) ? 'not_admitted' : 'retained',
      task_lifecycle_assignment_authority: taskLifecycleTasks.some((task: any) => task.task_lifecycle_assignment_write_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_role_resolution_authority: taskLifecycleTasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_roster_mutation_authority: taskLifecycleTasks.some((task: any) => task.task_lifecycle_roster_mutation_write_count > 0) ? 'cloudflare_task_lifecycle_d1' : 'not_observed',
      task_lifecycle_roster_read_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0) ? 'admitted' : 'not_observed',
      task_lifecycle_roster_mutation_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_roster_mutation_write_count > 0) ? 'admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_assignment_write_count > 0) ? 'not_admitted' : 'retained',
      task_lifecycle_role_resolution_authority_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0) ? 'admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_assignment_write_count > 0) ? 'not_admitted' : 'retained',
      ...
      task_lifecycle_cloudflare_write_admission: taskLifecycleTasks.some((task: any) => task.task_lifecycle_roster_mutation_write_count > 0) ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_role_resolution_and_roster_mutation_admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_role_resolution_write_count > 0) ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_role_resolution_admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_assignment_write_count > 0) ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_source_state_write_count > 0) ? 'task_create_claim_report_finish_changed_file_evidence_projection_write_and_source_state_admitted' : taskLifecycleTasks.some((task: any) => task.task_lifecycle_projection_write_count > 0) ? 'task_create_claim_report_finish_changed_file_evidence_and_projection_write_admitted' : taskLifecycleTasks.some((task: any) => task.changed_file_evidence_count > 0) ? (taskLifecycleTasks.some((task: any) => task.finish_id) ? 'task_create_claim_report_finish_and_changed_file_evidence_admitted' : 'task_create_claim_report_and_changed_file_evidence_admitted') : taskLifecycleTasks.some((task: any) => task.finish_id) ? 'task_create_claim_report_and_finish_admitted' : taskLifecycleTasks.some((task: any) => task.report_id) ? 'task_create_claim_and_report_admitted' : taskLifecycleTasks.some((task: any) => task.status === 'claimed') ? 'task_create_and_claim_admitted' : taskLifecycleTasks.length > 0 ? 'task_create_admitted' : 'not_admitted',
      dispatch_authority: WINDOWS_PRIMARY_DISPATCH_AUTHORITY,
    },
    };
  } catch (error: any) {
...
function summarizeCloudflareOperationProjectionError(error: any, context: any= {}) {
  return {
...
async function buildCloudflareSiteProductProjection(env: any, principal: any, response: any, params: any= {}) {
  ...
  function summarizeFocusedOperationArtifacts(focusedOperation: any) {
  ...
  const postureTarget = operationPostureRoute.next_action === 'focus_next_operation'
    ...
    : '';
  if (postureTarget && postureTarget !== (focusedOperation?.operation_id ?? '')) {
    const targetOperation = (Array.isArray(response.operations) ? response.operations : []).find((operation: any) => operation?.operation_id === postureTarget);
    if (targetOperation) {
...
async function startCloudflareResidentDispatchWithWindowsFallback(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
function createResidentDispatchDecision(siteId: any, params: any= {}, principal: any= null) {
...
async function recordCloudflareResidentDispatchDecision(env: any= {}, record: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
async function ensureCloudflareResidentDispatchDecisionSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareResidentDispatchDecisions(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareResidentDispatchDecisionSchema(db);
...
async function createCloudflareResidentDispatchWindowsFallbackRequest(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareResidentDispatchWindowsFallbackRequestSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareResidentDispatchWindowsFallbackRequests(env: any= {}, siteId: any, limit: any, filters: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareResidentDispatchWindowsFallbackRequestSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
  ...
  `).bind(siteId, boundedLimit).all();
  return (rows.results ?? [])
    .filter((row: any) => (!operationId || row.operation_id === operationId) && (!dispatchDecisionId || row.dispatch_decision_id === dispatchDecisionId))
    .map((row: any) => ({
...
function createResidentDispatchWindowsFallbackEvidence(siteId: any, params: any= {}) {
...
async function recordCloudflareResidentDispatchWindowsFallbackEvidence(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createResidentDispatchWindowsFallbackEvidence(siteId, params);
  if (!payload.ok) return payload;
  const evidence: any = payload.evidence;
  const record = {
...
async function ensureCloudflareResidentDispatchWindowsFallbackEvidenceSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareResidentDispatchWindowsFallbackEvidence(env: any= {}, siteId: any, params: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareResidentDispatchWindowsFallbackEvidenceSchema(db);
  ...
  `).bind(siteId, boundedLimit).all();
  return (rows.results ?? [])
    .filter((row: any) => (!fallbackRequestId || row.fallback_request_id === fallbackRequestId) && (!operationId || row.operation_id === operationId) && (!dispatchDecisionId || row.dispatch_decision_id === dispatchDecisionId))
    .map((row: any) => ({
...
function createLocalResidentCarrierBridge(siteId: any, params: any= {}) {
...
async function recordCloudflareLocalResidentCarrierBridge(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createLocalResidentCarrierBridge(siteId, params);
  if (!payload.ok) return payload;
  const bridge: any = payload.bridge;
  const record = {
...
async function ensureCloudflareLocalResidentCarrierBridgeSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareLocalResidentCarrierBridgeRecords(env: any= {}, siteId: any, params: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareLocalResidentCarrierBridgeSchema(db);
  ...
  `).bind(siteId, boundedLimit).all();
  return (rows.results ?? [])
    .filter((row: any) => (!operationId || row.operation_id === operationId)
...
function createResidentDispatchWindowsFallbackRequest(siteId: any, params: any= {}) {
...
function residentDispatchWindowsFallbackRequestId(siteId: any, request: any= {}) {
...
function normalizeNullableWorkerString(value: any) {
  if (value == null) return null;
  const trimmed = String(value).trim();
  return trimmed || null;
}

async function recordCloudflareMailboxStatusShadowRead(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createMailboxStatusShadowRead(siteId, params);
  if (!payload.ok) return payload;
  const read: any = payload.read;
  const record = {
...
async function ensureCloudflareMailboxStatusShadowReadSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxStatusShadowReads(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxStatusShadowReadSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
async function readCloudflareMailboxStatusSource(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  ...
  const inbox: any = await fetchCloudflareGraphJson(env, `${baseUrl}/users/${encodeURIComponent(accountRef)}/mailFolders/inbox?$select=unreadItemCount,totalItemCount`, { headers });
  if (!inbox.ok) return { ok: false, code: 'graph_inbox_status_read_failed', graph_status: inbox.status, graph_error: inbox.error };
  const drafts: any = await fetchCloudflareGraphJson(env, `${baseUrl}/users/${encodeURIComponent(accountRef)}/mailFolders/drafts?$select=totalItemCount`, { headers });
  if (!drafts.ok) return { ok: false, code: 'graph_draft_status_read_failed', graph_status: drafts.status, graph_error: drafts.error };
  const latest: any = await fetchCloudflareGraphJson(env, `${baseUrl}/users/${encodeURIComponent(accountRef)}/mailFolders/inbox/messages?$top=1&$orderby=receivedDateTime%20desc&$select=receivedDateTime`, { headers });
...
async function ensureCloudflareMailboxStatusSourceReadSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxStatusSourceReads(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxStatusSourceReadSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createMailboxDraftReplyProposal(siteId: any, params: any= {}) {
...
function mailboxDraftReplyProposalId(siteId: any, proposal: any) {
...
async function recordCloudflareMailboxDraftReplyProposal(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createMailboxDraftReplyProposal(siteId, params);
  if (!payload.ok) return payload;
  const proposal: any = payload.proposal;
  const record = {
...
async function ensureCloudflareMailboxDraftReplyProposalSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxDraftReplyProposals(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxDraftReplyProposalSchema(db);
...
function createMailboxOutlookDraftCreateRequest(siteId: any, params: any= {}) {
  ...
  const toRecipients = recipients.map((recipient: any) => String(recipient ?? '').trim()).filter(Boolean).slice(0, 25);
...
function mailboxOutlookDraftCreateId(siteId: any, draft: any) {
...
async function createCloudflareMailboxOutlookDraft(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createMailboxOutlookDraftCreateRequest(siteId, params);
  if (!payload.ok) return payload;
  const draft: any = payload.draft;
  const tokenResult = await resolveCloudflareGraphAccessToken(env);
  if (!tokenResult.ok) return tokenResult;
  const baseUrl = String(env.GRAPH_BASE_URL ?? 'https://graph.microsoft.com/v1.0').replace(/\/+$/, '');
  const graphPayload = {
    ...
    toRecipients: draft.to_recipients.map((address: any) => ({ emailAddress: { address } })),
  };
  ...
  const graphBody: any = graphResult.body ?? {};
  const record = {
...
async function ensureCloudflareMailboxOutlookDraftCreateSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxOutlookDraftCreates(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxOutlookDraftCreateSchema(db);
  const boundedLimit = clampInteger(limit, 0, 5000, 25);
...
function createMailboxSendRequest(siteId: any, params: any= {}) {
...
function mailboxSendAcceptedId(siteId: any, sendRequest: any) {
...
async function sendCloudflareMailboxOutlookDraft(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createMailboxSendRequest(siteId, params);
  if (!payload.ok) return payload;
  const sendRequest: any = payload.send_request;
...
async function ensureCloudflareMailboxSendAcceptedSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxSendAcceptedRecords(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxSendAcceptedSchema(db);
  const boundedLimit = clampInteger(limit, 0, 5000, 25);
...
function createMailboxSendConfirmationRequest(siteId: any, params: any= {}) {
...
function mailboxSendConfirmationId(siteId: any, confirmationRequest: any) {
...
async function readCloudflareMailboxSendConfirmation(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createMailboxSendConfirmationRequest(siteId, params);
  if (!payload.ok) return payload;
  const confirmationRequest: any = payload.confirmation_request;
  await ensureCloudflareMailboxSendAcceptedSchema(db);
  ...
  if (!tokenResult.ok) return tokenResult;
  const baseUrl = String(env.GRAPH_BASE_URL ?? 'https://graph.microsoft.com/v1.0').replace(/\/+$/, '');
let graphResult: any = await fetchCloudflareGraphJson(env, `${baseUrl}/users/${encodeURIComponent(accountRef)}/messages/${encodeURIComponent(confirmationRequest.sent_message_ref)}`, {
    method: 'GET',
  ...
  const graphMessage: any = graphResult.body && typeof graphResult.body === 'object' && !Array.isArray(graphResult.body) ? graphResult.body : {};
  const now = new Date().toISOString();
  const record = {
...
async function resolveCloudflareMailboxSentMessageBySubject(env: any, baseUrl: any, accountRef: any, subject: any, accessToken: any) {
  const query = new URLSearchParams({
    '$top': '25',
    '$orderby': 'sentDateTime desc',
  });
  const result: any = await fetchCloudflareGraphJson(env, `${baseUrl}/users/${encodeURIComponent(accountRef)}/mailFolders/SentItems/messages?${query.toString()}`, {
    method: 'GET',
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!result.ok) return result;
  const messages = Array.isArray(result.body?.value) ? result.body.value : [];
  const message = messages.find((entry: any) => String(entry?.subject ?? '') === String(subject));
...
function escapeODataString(value: any) {
  return String(value ?? '').replace(/'/g, "''");
}
async function ensureCloudflareMailboxSendConfirmationSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareMailboxSendConfirmations(env: any = {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxSendConfirmationSchema(db);
  const boundedLimit = clampInteger(limit, 0, 5000, 25);
...
function mailboxSendReviewId(siteId: any, focusKind: any, focusRef: any, generatedAt: any) {
  return `mailbox_send_review_${safeIdToken(siteId)}_${safeIdToken(focusKind)}_${safeIdToken(focusRef)}_${safeIdToken(generatedAt)}`;
}

async function ensureCloudflareMailboxSendReviewSchema(db: any) {
  await db.prepare(`
...
async function recordCloudflareMailboxSendReview(env: any = {}, siteId: any, params: any = {}, principal: any = null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const now = new Date().toISOString();
  ...
  if (focusKind === 'mailbox_send_confirmation') {
    const confirmation = (await listCloudflareMailboxSendConfirmations(env, siteId, 100))
      .find((entry: any) => entry.send_confirmation_id === focusRef);
    ...
    const accepted = (await listCloudflareMailboxSendAcceptedRecords(env, siteId, 100))
      .find((entry: any) => entry.send_accepted_id === focusRef);
...
async function listCloudflareMailboxSendReviews(env: any = {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareMailboxSendReviewSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function operationFocusReviewId(siteId: any, focusKind: any, focusRef: any, generatedAt: any) {
...
async function findCloudflareOperationFocusRecord(env: any = {}, siteId: any, focusKind: any, focusRef: any) {
  const limit = 100;
  const by = (entries: any, idField: any) => (entries || []).find((entry: any) => String(entry?.[idField] ?? '') === focusRef) ?? null;
...
async function recordCloudflareOperationFocusReview(env: any = {}, siteId: any, params: any = {}, principal: any = null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const focusKind = String(params.focus_kind ?? '');
...
async function ensureCloudflareOperationFocusReviewSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareOperationFocusReviews(env: any = {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareOperationFocusReviewSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
async function resolveCloudflareGraphAccessToken(env: any = {}) {
...
async function fetchCloudflareGraphJson(env: any, url: any, init: any= {}) {
...
function cloudflareGraphFetch(env: any, url: any, init: any) {
...
async function recordCloudflareResidentLoopShadowRun(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareResidentLoopShadowRunSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareResidentLoopShadowRuns(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareResidentLoopShadowRunSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createResidentLoopShadowRun(siteId: any, params: any= {}) {
...
function residentLoopShadowRunId(siteId: any, loopRun: any) {
  return `resident_loop_shadow_${safeIdToken(siteId)}_${safeIdToken(loopRun.operation_id)}_${safeIdToken(loopRun.run_started_at)}`;
}

async function readCloudflareTaskLifecycleShadowSource(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
...
function resolveTaskLifecycleShadowReadSourceUrl(env: any= {}, params: any= {}) {
...
async function fetchTaskLifecycleShadowReadSource(env: any= {}, sourceUrl: any, params: any= {}) {
...
async function recordCloudflareTaskLifecycleShadowRead(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareTaskLifecycleShadowReadSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareTaskLifecycleShadowReads(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareTaskLifecycleShadowReadSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
export function classifyCloudflareTaskLifecycleWriteAdmission(request: any= {}, state: any= {}) {
...
async function recordCloudflareTaskLifecycleWriteAdmission(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareTaskLifecycleWriteAdmissionSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareTaskLifecycleWriteAdmissions(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareTaskLifecycleWriteAdmissionSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function taskLifecycleWriteAdmissionId(siteId: any, decision: any, recordedAt: any) {
  return `task_lifecycle_write_admission_${safeIdToken(siteId)}_${safeIdToken(decision.mutation_class)}_${safeIdToken(recordedAt)}`;
}

async function createCloudflareTaskLifecycleTask(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareTaskLifecycleTaskSchema(db: any) {
  await db.prepare(`
...
async function nextCloudflareTaskLifecycleTaskNumber(db: any, siteId: any) {
...
function normalizeCloudflareTaskLifecycleIncludeTaskIds(params: any= {}) {
  ...
  return [...new Set(values.map((value: any) => String(value ?? '').trim()).filter(Boolean))].slice(0, 25);
}

async function listCloudflareTaskLifecycleTasks(env: any= {}, siteId: any, limit: any, params: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareTaskLifecycleTaskSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
  ...
  const focusedTask = carrierSessionId
    ? tasks.find((task: any) => task.carrier_session_id === carrierSessionId) ?? null
    : null;
  if (carrierSessionId) {
    tasks = tasks
      .filter((task: any) => task.carrier_session_id !== carrierSessionId)
      .slice(0, boundedLimit);
    if (focusedTask) tasks.push(focusedTask);
  }
  const seenTaskIds = new Set(tasks.map((task: any) => task.task_id));
...
function formatCloudflareTaskLifecycleTask(row: any) {
...
function createTaskLifecycleShadowRead(siteId: any, params: any= {}) {
  ...
  const tasks = Array.isArray(source.tasks) ? source.tasks.slice(0, clampInteger(source.limit, 0, 100, 25)).map((task: any) => ({
...
function createSiteFileChangeProposal(siteId: any, params: any= {}) {
  ...
  const files = Array.isArray(source.files ?? params.files) ? (source.files ?? params.files).slice(0, 100).map((file: any) => ({
...
function siteFileChangeProposalId(siteId: any, proposal: any) {
...
async function recordCloudflareSiteFileChangeProposal(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareSiteFileChangeProposalSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareSiteFileChangeProposals(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareSiteFileChangeProposalSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createSiteFileMaterialization(siteId: any, params: any= {}) {
...
function siteFileMaterializationId(siteId: any, materialization: any) {
...
async function recordCloudflareSiteFileMaterialization(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createSiteFileMaterialization(siteId, params);
  if (!payload.ok) return payload;
  const materialization: any = payload.materialization;
  const record = {
...
async function ensureCloudflareSiteFileMaterializationSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareSiteFileMaterializations(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareSiteFileMaterializationSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createLocalIngressRequest(siteId: any, params: any= {}) {
  const source = params.source_payload ?? params.payload ?? params.request ?? {};
...
function localIngressRequestId(siteId: any, request: any) {
...
async function createCloudflareLocalIngressRequest(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareLocalIngressRequestSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareLocalIngressRequests(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareLocalIngressRequestSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createRepositoryPublicationRequest(siteId: any, params: any= {}) {
  const source = params.source_payload ?? params.payload ?? params.request ?? {};
...
function repositoryPublicationRequestId(siteId: any, request: any) {
...
async function createCloudflareRepositoryPublicationRequest(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareRepositoryPublicationRequestSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareRepositoryPublicationRequests(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareRepositoryPublicationRequestSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createRepositoryPublicationAdmission(siteId: any, params: any= {}) {
...
function repositoryPublicationAdmissionId(siteId: any, admission: any) {
...
async function recordCloudflareRepositoryPublicationAdmission(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const payload = createRepositoryPublicationAdmission(siteId, params);
  if (!payload.ok) return payload;
  const admission: any = payload.admission;
  const requests = await listCloudflareRepositoryPublicationRequests(env, siteId, 100);
  const request = requests.find((entry: any) => entry.repository_publication_request_id === admission.repository_publication_request_id);
...
async function ensureCloudflareRepositoryPublicationAdmissionSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareRepositoryPublicationAdmissions(env: any= {}, siteId: any, limit: any, repositoryPublicationRequestId: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareRepositoryPublicationAdmissionSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function latestRepositoryPublicationAdmissionForRequest(admissions: any= [], requestId: any) {
  return admissions
    .filter((entry: any) => entry.repository_publication_request_id === requestId)
    .sort((left: any, right: any) => String(right.recorded_at).localeCompare(String(left.recorded_at)) || String(right.generated_at).localeCompare(String(left.generated_at)))[0] ?? null;
}

async function listCloudflarePendingRepositoryPublicationRequests(env: any= {}, siteId: any, limit: any) {
...
async function nextCloudflareRepositoryPublicationRequest(env: any= {}, siteId: any, limit: any) {
...
function parseGithubRepositoryRef(repositoryRef: any) {
...
function parseGitCommitRef(sourceChangeRef: any) {
  const match = String(sourceChangeRef ?? '').trim().match(/^git:commit:([0-9a-f]{40})$/i);
  return match ? match[1].toLowerCase() : null;
}

function cloudflarePublicationAllowedValues(value: any) {
  return String(value ?? '').split(',').map((entry: any) => entry.trim()).filter(Boolean);
}

function isCloudflarePublicationValueAllowed(value: any, allowedValues: any) {
  return allowedValues.includes(value);
}

function githubBranchRefPath(branchRef: any) {
  return normalizeRepositoryBranchRef(branchRef).split('/').map((part: any) => encodeURIComponent(part)).join('/');
}

function normalizeRepositoryBranchRef(branchRef: any) {
  return String(branchRef ?? '').trim().replace(/^refs\/heads\//, '');
}

function readCloudflareGithubRepositoryPublicationReadiness(env: any= {}, siteId: any, params: any= {}) {
...
function readCloudflareGithubRepositoryPublicationCredentialConfig(env: any= {}) {
...
async function executeCloudflareGithubRepositoryPublication(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  ...
  const requests = await listCloudflareRepositoryPublicationRequests(env, siteId, 100);
  const request = requests.find((entry: any) => entry.repository_publication_request_id === repositoryPublicationRequestId);
...
function summarizeGithubPublicationResponse(body: any) {
...
async function recordCloudflareGithubRepositoryPublicationExecution(db: any, execution: any) {
...
async function ensureCloudflareRepositoryPublicationExecutionSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareRepositoryPublicationExecutions(env: any= {}, siteId: any, limit: any, repositoryPublicationRequestId: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareRepositoryPublicationExecutionSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createLocalIngressEvidence(siteId: any, params: any= {}) {
  const source = params.source_payload ?? params.payload ?? params.evidence ?? {};
...
async function recordCloudflareLocalIngressEvidence(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareLocalIngressEvidenceSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareLocalIngressEvidence(env: any= {}, siteId: any, limit: any, localIngressRequestId: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareLocalIngressEvidenceSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createCloudflareLocalIngressProviderHeartbeat(siteId: any, params: any= {}, principal: any= null) {
...
function classifyLocalIngressProviderLiveness(heartbeats: any= [], { nowMs = Date.now(), staleAfterMs = DEFAULT_LOCAL_INGRESS_PROVIDER_STALE_AFTER_MS }: any= {}) {
...
function classifyProviderHeartbeatSchedulerPosture(heartbeat: any= null, { providerKind = 'provider', providerLivenessState = 'unknown' }: any= {}) {
  if (!heartbeat) {
    return {
...
async function recordCloudflareLocalIngressProviderHeartbeat(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareLocalIngressProviderHeartbeatSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareLocalIngressProviderHeartbeats(env: any= {}, siteId: any, limit: any= 20) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
...
function createRepositoryPublicationEvidence(siteId: any, params: any= {}) {
  const source = params.source_payload ?? params.payload ?? params.evidence ?? {};
  const repositoryPublicationRequestId = String(source.repository_publication_request_id ?? params.repository_publication_request_id ?? '');
...
async function recordCloudflareRepositoryPublicationEvidence(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareRepositoryPublicationEvidenceSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareRepositoryPublicationEvidence(env: any= {}, siteId: any, limit: any, repositoryPublicationRequestId: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareRepositoryPublicationEvidenceSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createCloudflareRepositoryPublicationProviderHeartbeat(siteId: any, params: any= {}, principal: any= null) {
  const generatedAt = params.generated_at ?? params.last_run_at ?? new Date().toISOString();
...
function classifyRepositoryPublicationProviderLiveness(heartbeats: any= [], { nowMs = Date.now(), staleAfterMs = DEFAULT_REPOSITORY_PUBLICATION_PROVIDER_STALE_AFTER_MS }: any= {}) {
  const heartbeat = Array.isArray(heartbeats) ? heartbeats[0] ?? null : null;
  if (!heartbeat) {
    return {
...
async function recordCloudflareRepositoryPublicationProviderHeartbeat(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareRepositoryPublicationProviderHeartbeatSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareRepositoryPublicationProviderHeartbeats(env: any= {}, siteId: any, limit: any= 20) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return [];
  if (!siteId || siteId === 'unknown-site') return [];
  await ensureCloudflareRepositoryPublicationProviderHeartbeatSchema(db);
...
async function recordCloudflareWebhookDelayShadowObservation(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
...
async function ensureCloudflareWebhookDelayShadowObservationSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareWebhookDelayShadowObservations(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayShadowObservationSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
async function recordCloudflareWebhookDelayObservationPrimaryRead(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const observation = createWebhookDelayShadowObservation(siteId, params);
...
async function ensureCloudflareWebhookDelayObservationPrimaryReadSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareWebhookDelayObservationPrimaryReads(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayObservationPrimaryReadSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
async function putCloudflareWebhookDelayRemoteSourceSamples(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  ...
  const samples = rawSamples.map((sample: any, index: any) => normalizeWebhookDelayRemoteSourceSample(siteId, sourceAdapterId, sample, index));
  if (samples.length === 0) return { ok: false, code: 'webhook_delay_remote_source_samples_missing' };
  if (samples.some((sample: any) => !sample.ok)) return samples.find((sample: any) => !sample.ok);
  ...
  return {
    ...
    samples: samples.map(({ sample }: any) => sample),
  };
}

async function readCloudflareWebhookDelayRemoteSourceWithWindowsFallback(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const sourceAdapterId = params.source_adapter_id ?? 'sonar_webhook_delay_d1_remote_source_v1';
...
async function readCloudflareWebhookDelayDirectRemoteMetricSource(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
...
function resolveDirectMetricSourceUrl(env: any= {}, params: any= {}) {
...
async function fetchWebhookDelayDirectMetricSource(env: any= {}, sourceUrl: any, params: any= {}) {
  const headers: any = { accept: 'application/json' };
...
function createWebhookDelayDirectMetricSourceSamples(siteId: any, sourceAdapterId: any, sourceId: any, sourceUrl: any, body: any= {}, params: any= {}) {
  ...
  const samples = rows.map((row: any, index: any) => {
...
function safeUrlHost(value: any) {
  try {
...
async function runCloudflareWebhookDelayScheduledSourceRead(env: any= {}, params: any= {}, principal: any= null) {
...
async function recordCloudflareWebhookDelayScheduledSourceRead(env: any= {}, siteId: any, record: any, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
async function listCloudflareWebhookDelayScheduledSourceReads(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayScheduledSourceReadSchema(db);
...
async function ensureCloudflareWebhookDelayScheduledSourceReadSchema(db: any) {
  await db.prepare(`
...
async function ensureCloudflareWebhookDelayRemoteSourceSampleSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareWebhookDelayRemoteSourceSamples(env: any= {}, siteId: any, sourceAdapterId: any= 'sonar_webhook_delay_d1_remote_source_v1', limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayRemoteSourceSampleSchema(db);
  const boundedLimit = clampInteger(limit, 0, 500, 100);
...
function normalizeWebhookDelayRemoteSourceSample(siteId: any, sourceAdapterId: any, input: any= {}, index: any= 0) {
...
function createWebhookDelaySummaryFromRemoteSourceSamples(siteId: any, samples: any= [], params: any= {}) {
  const sorted = [...samples].sort((left: any, right: any) => String(right.observed_at).localeCompare(String(left.observed_at)));
  const latest = sorted.find((sample: any) => sample.sample_role === 'today_latest') ?? sorted[0];
  const comparison = sorted.find((sample: any) => sample.sample_role === 'yesterday_same_clock') ?? null;
  const latestDelay = Number(latest.delay_minutes);
  const comparisonDelay: any = comparison ? Number(comparison.delay_minutes) : null;
  return {
    ...
    today: {
      rows: samples.filter((sample: any) => sample.sample_role === 'today_latest').length || null,
      latest: {
...
function createWebhookDelayShadowObservation(siteId: any, params: any= {}) {
...
function classifyWebhookDelayShadowObservation(observation: any) {
...
function classifyWebhookDelayObservationPrimaryRead(observation: any) {
...
function webhookDelayShadowObservationId(siteId: any, observation: any) {
...
function webhookDelayObservationPrimaryReadId(siteId: any, observation: any) {
...
async function recordCloudflareWebhookDelayDirectiveDualRecord(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const observation: any = createWebhookDelayShadowObservation(siteId, params);
  if (!observation.ok) return observation;
  const classification = classifyWebhookDelayShadowObservation(observation.observation);
...
async function ensureCloudflareWebhookDelayDirectiveDualRecordSchema(db: any) {
  await db.prepare(`
...
async function listCloudflareWebhookDelayDirectiveDualRecords(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayDirectiveDualRecordSchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
async function deliverCloudflareWebhookDelayDirectiveWithWindowsFallback(env: any= {}, siteId: any, params: any= {}, principal: any= null) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
  if (!siteId || siteId === 'unknown-site') return { ok: false, code: 'missing_site_id' };
  const observation: any = createWebhookDelayShadowObservation(siteId, params);
...
async function recordCloudflareWebhookDelayDirectiveDelivery(env: any= {}, record: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return { ok: false, code: 'missing_site_registry_binding' };
...
async function ensureCloudflareWebhookDelayDirectiveDeliverySchema(db: any) {
  await db.prepare(`
...
async function listCloudflareWebhookDelayDirectiveDeliveries(env: any= {}, siteId: any, limit: any) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  if (!db || typeof db.prepare !== 'function' || !siteId) return [];
  await ensureCloudflareWebhookDelayDirectiveDeliverySchema(db);
  const boundedLimit = clampInteger(limit, 0, 100, 25);
...
function createWebhookDelayThresholdPolicy(params: any= {}, classification: any= {}) {
...
function webhookDelayDirectiveRecordId(siteId: any, operationId: any, generatedAt: any) {
  return `webhook_delay_directive_${safeIdToken(siteId)}_${safeIdToken(operationId)}_${safeIdToken(generatedAt)}`;
}

function safeIdToken(value: any) {
...
async function listSiteTasks(env: any= {}, siteId: any) {
...
async function listOperationTasks(env: any= {}, siteId: any, sessions: any= []) {
  const sessionIds = new Set((sessions ?? []).map((session: any) => session.carrier_session_id).filter(Boolean));
  if (sessionIds.size === 0) return [];
  const tasks = await listSiteTasks(env, siteId);
  return tasks.filter((task: any) => sessionIds.has(task.carrier_session_id));
}

async function readCarrierEvidenceForSiteSessions(env: any= {}, sessions: any= [], principal: any= null, params: any= {}) {
  ...
  const evidence: any[] = indexedEvidence?.evidence ?? [];
  const indexedSessionIds = new Set(evidence.map((entry: any) => entry.carrier_session_id).filter(Boolean));
...
function carrierEventsFromResponse(response: any= {}) {
  return (response.events ?? [response.event]).filter((event: any) => event?.schema && event?.carrier_session_id);
}

async function recordCloudflareCarrierEvidenceEvents(env: any= {}, session: any= null, events: any= []) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
...
async function readCloudflareCarrierEvidenceIndex(env: any= {}, sessions: any= [], params: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
  ...
  for (const session of sessionSlice) {
    ...
    const events = (rows.results ?? []).map((row: any) => parseJsonObject(row.event_json)).filter((event: any) => event?.event_kind);
    if (events.length === 0) {
...
async function ensureCloudflareCarrierEvidenceIndexSchema(db: any) {
  await db.prepare(`
...
async function handleOperatorAuthRequest(request: any, env: any= {}) {
  const url = new URL(request.url);
...
async function startMicrosoftLogin(request: any, env: any= {}) {
...
async function completeMicrosoftLogin(request: any, env: any= {}) {
  const config = microsoftOidcConfig(request, env);
  if (!config.ok) return jsonResponse({ ok: false, code: config.code }, config.status);
  const url = new URL(request.url);
  ...
  const validation: any = await validateMicrosoftIdToken(tokenResponse.id_token, pending.value.nonce, config, env);
...
async function captureOperatorSessionCookie(request: any, env: any= {}) {
  const url = new URL(request.url);
  ...
  const auth: any = await authenticateOperatorSessionRequest(request, env);
  if (!auth.ok) {
...
function microsoftOidcConfig(request: any, env: any= {}): any {
...
async function exchangeMicrosoftCodeForTokens(code: any, codeVerifier: any, config: any, env: any= {}) {
...
async function validateMicrosoftIdToken(idToken: any, nonce: any, config: any, env: any= {}) {
  ...
  const key = (jwks.keys ?? []).find((entry: any) => entry.kid === header.kid);
...
function validateMicrosoftClaims(claims: any, nonce: any, config: any) {
...
async function createOperatorSessionForMicrosoftPrincipal(claims: any, env: any= {}) {
  const db = env.CLOUDFLARE_SITE_REGISTRY_DB ?? env.NARADA_SITE_REGISTRY_DB ?? null;
...
async function authenticateCarrierApiRequest(request: any, env: any= {}) {
...
async function authenticateOperatorSessionRequest(request: any, env: any= {}) {
...
async function ensureOperatorSessionSchema(db: any) {
...
function microsoftPrincipalId(claims: any= {}) {
  return `microsoft:${claims.tid}:${claims.oid}`;
}

function validatedOperatorPostLoginRedirect(url: any) {
...
function validateOperatorCaptureReturnTo(value: any) {
...
export function createCloudflareAiProviderAdapter(env: any= {}, { config = createCloudflareCarrierConfig(env) }: any= {}) {
...
function createCloudflareAiProviderAdapterLegacy(env: any= {}, { config = createCloudflareCarrierConfig(env) }: any= {}) {
  ...
  const workersAiTools = toolEffectConfig.tool_definitions.map((tool: any) => ({
    type: 'function',
    function: {
      name: tool.name,
  ...
  const workersAiRequest = (input: any, tool_results: any) => tool_results.length > 0
    ? {
  ...
  const ensureGateway = () => {
    gatewayPromise ??= createCarrierIntelligenceGateway(env, (store: any) => ({
      async invoke({ plan, offering, messages, invocationScope }: any) {
        ...
        try {
          const result: any = await withTimeout(aiBinding.run(modelSlug, workersAiRequest(input, tool_results)), timeoutMs);
          return {
            response: {
  ...
  return {
    ...
    async run({
      ...
      if (intelligence_diagnostic && !intelligenceDiagnosticsEnabled) {
        const error: any = new Error('cloudflare_intelligence_diagnostic_disabled');
      ...
      if (result.kind === 'refusal') {
        const error: any = new Error('intelligence_resolution_refused:' + result.refusal.reason_code + ':' + result.refusal.explanation);
        ...
        error.intelligence = {
          ...
          audit_evidence_ids: result.auditEvidence.map(({ id }: any) => id),
        };
        throw error;
      }
      const intelligence = {
        ...
        audit_evidence_ids: result.auditEvidence.map(({ id }: any) => id),
        observation_ids: result.observations.map(({ id }: any) => id),
        telemetry_ids: result.telemetry.map(({ id }: any) => id),
      ...
      if (result.adapterOutcome.error) {
        const error: any = new Error(result.adapterOutcome.error.message);
...
function createWorkersAiInitialMessages(input: any) {
  return [
    {
      role: 'system',
...
function createWorkersAiToolResultMessages(input: any, toolResults: any) {
  return [
    ...
    {
      role: 'user',
      content: `Carrier tool results:\n${JSON.stringify(toolResults.map((result: any) => ({
...
function cloudflareToolEffectConfig(env: any= {}) {
...
export function createCloudflareToolEffectAdapter(env: any= {}) {
...
function createCloudflareToolEffectAdapterLegacy(env: any= {}) {
  ...
  return {
    ...
    capabilities: config.capabilities.map((capability: any) => ({ ...capability })),
    async execute({ toolCall, context }: any) {
...
export function createCloudflareD1TaskStoreAdapter(env: any= {}) {
  return createCloudflareD1TaskStoreAdapterBoundary(env);
}

function createCloudflareD1TaskStoreAdapterLegacy(env: any= {}) {
  const db = env.CLOUDFLARE_CARRIER_TASK_DB ?? env.NARADA_TASK_DB ?? null;
  if (!db || typeof db.prepare !== 'function') return null;
  return {
    posture: 'cloudflare-d1',
    adapter_kind: 'cloudflare-d1-task-store',
    forSession(context: any= {}) {
      return createD1SessionTaskStore(db, context);
    },
  };
}

function createD1SessionTaskStore(db: any, context: any= {}) {
  ...
  return {
    async create({ title, description = null, status = 'open', source = 'carrier' }: any) {
    ...
    async update({ task_id, status = null, note = null }: any) {
      await ensureSchema();
...
async function findTask(db: any, siteId: any, taskIdOrNumber: any) {
...
function classifyToolEffectAuthority(principal: any, toolName: any) {
...
function capabilityForTool(toolName: any) {
...
export function classifyCloudflareToolEffectAdmission(toolCall: any= {}, state: any= {}) {
...
function parseToolArguments(argumentsSummary: any) {
...
export function authenticateCarrierRequest(request: any, env: any= {}) {
...
function htmlResponse(body: any, status: any= 200) {
...
function operatorRedirectResponse(location: any, status: any= 302, cookies: any= []) {
  const headers = new Headers({ location, 'cache-control': 'no-store' });
  for (const cookie of cookies.filter(Boolean)) headers.append('set-cookie', cookie);
  return new Response(null, { status, headers });
}

async function signedCookie(name: any, value: any, env: any= {}, { maxAge = 300 }: any= {}) {
...
async function readSignedCookie(request: any, name: any, env: any= {}) {
...
function readCookie(request: any, name: any) {
...
function timingSafeEqual(left: any, right: any) {
...
async function sha256Base64Url(value: any) {
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(value));
  return base64UrlEncode(new Uint8Array(digest));
}

function randomBase64Url(byteLength: any) {
  const bytes = new Uint8Array(byteLength);
  crypto.getRandomValues(bytes);
  return base64UrlEncode(bytes);
}

function parseJwtPart(part: any) {
...
function withPrincipalEvidence(body: any, operation: any, principal: any) {
...
function extractWorkersAiText(result: any) {
...
function extractWorkersAiToolCalls(result: any) {
