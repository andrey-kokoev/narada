...
This is a split-by-mutation-class posture, not a full task lifecycle migration: after `task_source_state_write`, Cloudflare owns canonical source-state authority for Cloudflare task lifecycle rows, but Windows-originated task assignment, Windows roster, Windows role resolution, mailbox behavior, filesystem mutation, and repository publication remain refused or Windows-owned until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_claim_report_finish_changed_file_evidence_projection_write_and_source_state_cloudflare_remaining_windows_effects`, alongside explicit retained Windows authority for non-migrated effects.
