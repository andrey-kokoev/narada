...
The first eligible repository-publication mutation class is `repository_publication.cloudflare_github_execution`, but only for explicitly allowed repository and branch refs and only after the Cloudflare Worker reports `repository_publication.cloudflare_execution.readiness` as `ready`. The Worker secret `CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_TOKEN` is credential substrate only: installing it does not itself admit publication authority, does not grant arbitrary GitHub mutation, and must not be echoed in carrier output. An admitted publication requires repository ref, branch ref, commit identity, `repository_publication_authority_ref`, `repository_publication_admission_authority`, `cutover_point_ref`, `governed_write_contract_ref`, and `confirmation_evidence_ref`; the executor authority must be `cloudflare_github_repository_publication_executor`, the admission authority must be `cloudflare_repository_publication_admission_controller`, and direct filesystem mutation remains `not_admitted`. The transfer slice is complete only after live evidence shows secret readiness, allowlist admission, a governed Cloudflare GitHub publication smoke against an allowed non-default cutover branch, and readback of the resulting repository state.

The operator credential cutover procedure for this class is intentionally two-stage. First, install the credential into the Worker secret store through `pnpm --filter @narada2/cloudflare-carrier run repository-publication:secret-put:live` using an explicit token value or token file; this step may prove only secret installation posture. Second, rerun `repository-publication:readiness-smoke:live`; only a `ready` result permits the mutating `repository-publication:cloudflare-github-smoke:live` proof. A `not_ready` readiness result is a refusal boundary, not a fallback to local Windows publication.
