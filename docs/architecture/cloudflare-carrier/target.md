...
This is a split-by-mutation-class posture, not a full task lifecycle migration: SQLite source writes, Windows-originated task assignment, Windows roster, Windows role resolution, mailbox behavior, filesystem mutation, and repository publication remain refused or Windows-owned until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_claim_report_finish_changed_file_evidence_and_projection_write_cloudflare_remaining_windows`, alongside the default retained Windows authority for non-migrated task lifecycle mutations.
