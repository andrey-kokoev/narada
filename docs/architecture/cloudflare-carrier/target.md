...
`operation.read` also exposes `operation_workflow_route` for the focused operation. This route converts durable operation facts into one read-only workflow step: review persistence or recovery posture, start/select a session, read evidence, review continuity, review directive delivery, focus open task work, start resident dispatch, or monitor the operation. It is a client-facing route, not an effect executor; clients must still call the governed operation that owns any mutation.

`operation.read` may expose `mailbox_status_source_reads` as Cloudflare-owned mailbox status source evidence when the Worker holds explicit Microsoft Graph credentials. This is a status read boundary only: it records unread count, draft count, and latest-message timestamp with `mailbox_status_authority = cloudflare_graph_mailbox_status_source`, while `mailbox_send_admission = not_admitted` and `mailbox_mutation_admission = not_admitted`. Without Cloudflare-held credentials, the Worker must refuse the Graph read before making any Graph request.

`operation.read` exposes `mailbox_draft_reply_proposals` as Cloudflare-recorded proposal evidence. A proposal may carry subject, recipients, body preview/hash, rationale, and source-message reference, but it must keep `mailbox_outlook_draft_create_admission = not_admitted`, `mailbox_send_admission = not_admitted`, and `mailbox_mutation_admission = not_admitted`. Cloudflare proposal authority is not Outlook mutation authority; it is the operator-review queue before any draft executor is admitted.

`operation.read` may expose `mailbox_outlook_draft_creates` only as a separate Cloudflare Graph draft-create boundary. That boundary requires explicit Graph credentials and an input with `mailbox_outlook_draft_create_admission = admitted`, while preserving `mailbox_send_admission = not_admitted` and `mailbox_mutation_admission = not_admitted`. Creating a draft in Outlook is therefore not hidden inside proposal recording and does not admit send or broader mailbox mutation.
...
Task lifecycle `external_effects` are not a separate hidden executor. They are cleared only when the external effect domains task lifecycle depends on have their own Cloudflare evidence: mailbox source visibility, Outlook draft creation, mailbox send, mailbox delivery confirmation, Cloudflare site-file materialization, and Cloudflare repository publication. At that point the task lifecycle partition may report `task_lifecycle_cloudflare_writes_and_external_effects_cloudflare_owned`; before that, the task lifecycle row cutovers remain partial and must continue to report `external_effects` as retained Windows authority.
...
The first eligible repository-publication mutation class is `repository_publication.cloudflare_github_execution`, but only for explicitly allowed repository and branch refs and only after the Cloudflare Worker reports `repository_publication.cloudflare_execution.readiness` as `ready`. The credential substrate may be either the legacy PAT secret `CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_TOKEN` or the preferred GitHub App installation credentials `CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_ID`, `CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_INSTALLATION_ID`, and `CLOUDFLARE_REPOSITORY_PUBLICATION_GITHUB_APP_PRIVATE_KEY`. GitHub App mode is the product-shaped path because the Worker mints a short-lived installation token for the allowed repository boundary; installing either credential substrate does not itself admit publication authority, does not grant arbitrary GitHub mutation, and must not echo PATs, private keys, or installation tokens in carrier output. An admitted publication requires repository ref, branch ref, commit identity, `repository_publication_authority_ref`, `repository_publication_admission_authority`, `cutover_point_ref`, `governed_write_contract_ref`, and `confirmation_evidence_ref`; the executor authority must be `cloudflare_github_repository_publication_executor`, the admission authority must be `cloudflare_repository_publication_admission_controller`, and direct filesystem mutation remains `not_admitted`. The transfer slice is complete only after live evidence shows credential readiness, allowlist admission, a governed Cloudflare GitHub publication smoke against an allowed non-default cutover branch, and readback of the resulting repository state.

The operator credential cutover procedure for this class is intentionally two-stage. Preferred path: install GitHub App credentials into the Worker secret store through `pnpm --filter @narada-core/cloudflare-carrier run repository-publication:github-app-secret-put:live -- --app-id <id> --installation-id <id> --private-key-file <path>`. Legacy path: install a PAT through `pnpm --filter @narada-core/cloudflare-carrier run repository-publication:secret-put:live` using an explicit token value or token file. Either step may prove only secret installation posture. Then rerun `repository-publication:readiness-smoke:live`; only a `ready` result with `github_credential_mode = github_app_installation` or `github_token` permits the mutating `repository-publication:cloudflare-github-smoke:live` proof. A `not_ready` readiness result is a refusal boundary, not a fallback to local Windows publication.
