...
This is a split-by-mutation-class posture, not a full task lifecycle migration: after `task_role_resolution_write`, Cloudflare owns create, claim, report, changed-file evidence record, finish, projection-write, source-state, assignment-write, and role-resolution authority for Cloudflare task lifecycle rows, but roster mutation, mailbox behavior, filesystem mutation, and repository publication remain refused or Windows-owned until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_assignment_and_role_resolution_cloudflare_remaining_windows_effects`, alongside explicit retained Windows authority for non-migrated effects.
