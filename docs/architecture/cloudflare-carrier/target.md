...
The second eligible mutation class is `task_claim`, but only for tasks that already exist in the Cloudflare task lifecycle D1 substrate. It may be admitted only when the request carries `cloudflare_task_claim_cutover = true`, `task_id`, claimant identity, `assignment_authority_ref`, `cutover_point_ref`, `governed_write_contract_ref`, and `confirmation_evidence_ref`. The claim transition is opened-only: it records `conflict_policy = opened_only_no_overwrite`, changes the task status from `opened` to `claimed`, records claimant and assignment authority evidence, and returns conflict evidence instead of overwriting an already claimed task. This does not migrate Windows roster, role-resolution, report, finish, changed-file evidence, projection writes, or SQLite mutation authority.

This is a split-by-mutation-class posture, not a full task lifecycle migration: `task_report`, `task_finish`, changed-file evidence, projection writes, SQLite writes, and Windows-originated task assignment remain refused until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_and_claim_cloudflare_remaining_windows`, alongside the default retained Windows authority for non-migrated task lifecycle mutations.
`task_claim` may be cut over only after `task_create` has already established the Cloudflare task row and the claim request carries `cloudflare_task_claim_cutover = true`, `task_id`, a claimant identity, `assignment_authority_ref`, `cutover_point_ref`, `governed_write_contract_ref`, and `confirmation_evidence_ref`. Claim admission is opened-only and no-overwrite: claiming an already claimed task must return conflict evidence rather than silently replacing the assignee.

These are split-by-mutation-class postures, not a full task lifecycle migration. `task_report`, `task_finish`, changed-file evidence, projection writes, and SQLite writes remain refused until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_cloudflare_remaining_windows` or `task_create_and_claim_cloudflare_remaining_windows`, alongside the default retained Windows authority for non-migrated task lifecycle mutations.
