...
This is a split-by-mutation-class posture, not a full task lifecycle migration: after `task_assignment_write`, Cloudflare owns create, claim, report, changed-file evidence record, finish, projection-write, source-state, and assignment-write authority for Cloudflare task lifecycle rows, but Windows roster, Windows role resolution, mailbox behavior, filesystem mutation, and repository publication remain refused or Windows-owned until their own cutover contracts and evidence exist. Read models must expose the authority partition, for example `task_create_claim_report_finish_changed_file_evidence_projection_write_source_state_and_assignment_cloudflare_remaining_windows_effects`, alongside explicit retained Windows authority for non-migrated effects.

The source-state boundary is intentionally separate from projection writes. A `task_source_state_write` admission may become admissible only with `cloudflare_task_source_state_write_cutover = true`, task identity, source-state authority/schema/evidence refs, cutover point, governed write contract, and confirmation evidence. The source-state effect route records canonical Cloudflare D1 source-state authority for Cloudflare task lifecycle rows only; it keeps mailbox mutation, assignment authority, role resolution, filesystem mutation, repository publication, and local resident mechanics outside Cloudflare authority until their own cutover contracts exist.
