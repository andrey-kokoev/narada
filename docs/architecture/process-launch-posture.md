...
- Direct `agent-web-ui --agent` discovery refuses when more than one active matching session exists, even when the identity group is the same. Use an explicit `--session` or launch binding to choose; newest-session ordering is not authority.
