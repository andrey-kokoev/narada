...
- Each planned agent carries `selection_resolution` with the requested value, resolved value, and source for operator surfaces, runtime, and intelligence provider. `registry default` is resolved before handoff; a missing registry value is a refusal, never an ambient runtime fallback.
- The launch registry parser requires an explicit `OperatorSurface` and `Runtime` at agent or registry scope. `Carrier` may be read only as a legacy input alias during compatibility migration; the parser must never synthesize `codex` or derive a runtime from an operator-surface name.
- Interactive terminal/browser adapters preserve the literal `registry default` sentinel through the planning boundary. They must not encode an explicit default choice as an absent option, because absence and default have different provenance.
- Structural workspace-plan preflight runs before smoke or plan-artifact persistence and is repeated at the executor boundary. It rejects incomplete process ownership, missing resolved selections, missing provider selection for NARS, malformed persisted plans, and Web UI projections without their exact launch binding.
- An actual workspace launch then runs an execution-only provider-readiness preflight through `agent-start --preflight-only` for every selected NARS agent. The provider credential and Codex-subscription logic remains owned by `packages/agent-start` and its User Site cache; the executor does not duplicate it. Dry-run intentionally skips this live provider check. All selected agents must return a ready preflight result before any hidden runtime, terminal, or browser projection process is spawned.
- Direct `agent-web-ui --agent` discovery refuses when more than one active matching session exists, even when the identity group is the same. Explicit `--session latest` is a separate opt-in resolver: it requires `--agent` plus an explicit `--site` or `--site-root`, resolves once to the newest active/running matching session with deterministic tie-breaking, and records the concrete session id. It does not change implicit ambiguity behavior; use a concrete `--session` with a launch binding, because `latest` and `--launch-binding` are incompatible.
