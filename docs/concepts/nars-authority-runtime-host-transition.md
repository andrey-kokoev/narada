...
## Implementation Status (Chapter F / Task 2376)

- The local source FSM (drain/seal/prepare/activate and refusal paths) lives in `packages/nars-session-core/src/authority-transition-state.ts`. Its durable source record and session index retain the source host, authority epoch, runtime identity, transition id, handoff evidence, and reconciliation evidence.
- The Cloudflare target side (`prepareTransitionTarget`, `activateTransitionTarget`, `reconcileTransitionOutcome`, and `refuseTransitionSource`) lives in `packages/cloudflare-operator-projection/src/index.ts`. Target activation requires a strictly newer epoch, source seal evidence, and the exact replay boundary; the handoff records event-log, operator-queue, active-turn, artifact, health, MCP-fabric, provider, transport, and reconciliation dispositions.
- A second prepared target for the same source runtime and epoch is refused with `dual_host_authority_conflict`. A target with an active turn, unsupported provider transfer, incompatible MCP fabric, replay mismatch, or unknown transport/mutation outcome remains non-canonical. An unknown prepared transition cannot be activated later without explicit connected/reconnected, known-outcome, verified-replay evidence.
- Post-handoff input is fenced by both the target authority epoch and target runtime identity. Refusal evidence remains durable; canonical target events begin only at the verified replay boundary.
- Harness evidence: `packages/nars-session-core/src/authority-transition-state.test.ts` and `packages/cloudflare-operator-projection/test/authority-host-transition.test.ts` cover local source persistence, competing-target refusal, active-work/provider limits, all handoff dimensions, transport loss and recovery, unknown-outcome fencing, snapshot recovery, and post-handoff epoch fencing.
- The `cloudflare-host → local` direction remains explicitly refused with `transition_direction_refused`; provider-capable authority migration remains an unsupported synthetic slice. Live cross-host transfer still requires a separately authorized live-run task.
