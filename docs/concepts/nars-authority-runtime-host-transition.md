...
- The Cloudflare target side (`prepareTransitionTarget`, `activateTransitionTarget`, `reconcileTransitionOutcome`, and `refuseTransitionSource`) lives in `packages/cloudflare-nars-projection/src/index.ts`. Target activation requires a strictly newer epoch, source seal evidence, and the exact replay boundary; the handoff records event-log, operator-queue, active-turn, artifact, health, MCP-fabric, provider, transport, and reconciliation dispositions.
