...
The implementation lives in the companion `mcp-surfaces` workspace at `packages/shared/mcp-transport/src/mcp-payload-file.ts` (package `@narada-core/mcp-transport`). The former `packages/mcp-transport/src/mcp-payload-file.mjs` path is historical and no longer exists.
