...
Operator Surface message routing uses the same rule but keeps the message grammar explicit: `from` is the speaker/principal and `to` is the recipient address. `--identity` is only a transitional recipient alias. Send dry-runs and failures should show the requested address, current Site, target Site, resolved identity, binding status, and exact repair command. Labels are observations; identity admission is not addressability.
