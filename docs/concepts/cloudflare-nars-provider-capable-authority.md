...
Status: decided (Task 2112); implemented as decided (Task 2113 — closed). Binding vocabulary aligned with the canonical Narada provider names (commit `fa7e2712`). Wire contract aligned with the canonical OpenAI-compatible shape via the shared worker-safe core in `@narada2/carrier-provider-contract` (Task 2144, commit `4e36604a`). Live evidence: Task 2120 (deferred pending a bound provider).
...
- **Wire contract**: the provider call uses the canonical OpenAI-compatible chat-completions shape built by the shared worker-safe core (`@narada2/carrier-provider-contract/openai-compatible-chat`) — the same builder local NARS uses, including kimi/deepseek/openrouter special-casing. Any OpenAI-compatible provider binds directly; no Narada-specific endpoint or relay is required. Providers whose registry `adapter_kind` is not `openai-compatible-chat-completions` (anthropic, codex) refuse turns with typed `provider_adapter_unsupported_on_cloudflare` evidence until implemented. Streaming deltas remain a declared later increment.
...
- **Binding names** (canonical Narada provider vocabulary, same as local NARS and the pwsh SecretStore setup): `NARADA_INTELLIGENCE_PROVIDER` (var, provider id — activates the adapter), `NARADA_AI_API_KEY` (Worker secret, Bearer credential), optional `NARADA_AI_BASE_URL` (var, overrides the registry default base URL) / `NARADA_AI_MODEL` / `NARADA_AI_THINKING`. The Worker resolves the binding from the provider registry through the shared core (`provider-runtime-binding-core`), exactly like local NARS. The Worker secret value mirrors the pwsh SecretStore entry `narada/provider/<provider>/api-key`; that ref comes from the registry as `credential_secret_ref` metadata in `provider_request` events for lineage, never the value. Operator setup: `docs/concepts/cloudflare-nars-web-projection.md` → Deployed Provider-Capable Authority Smoke; `wrangler.toml` carries a commented `[vars]` template.
