...
There are three live smoke lineages and they prove different authority shapes:
...
| `pnpm --filter @narada2/cloudflare-nars-projection smoke:provider-capable-live` | Cloudflare provider-capable NARS authority runtime (`cloudflare_provider_http_adapter`). | Session authority with real provider dispatch from the Worker/DO lane. | A deployed Worker bound with `NARS_PROVIDER_*` env/secret admits operator input, executes a real provider turn, graduates `provider_execution` to `present` with executed evidence, and revokes with post-revoke refusals. Requires operator authorization recorded in Task 2115; planning mode performs no mutation. |
