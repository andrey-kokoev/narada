...
| Local NARS runtime | Cloudflare-hosted browser surface | `projected` | Real local authority; Cloudflare state is a non-canonical projection. Projected input is admitted only by local NARS (`semantic_success_point: nars_admission`). | `packages/cloudflare-operator-projection/test/runtime-surface-quadrants.test.ts`, `test/cloudflare-operator-projection.test.ts`, `test/cloudflare-operator-projection-node.test.ts` |
| Cloudflare-origin synthetic runtime | Local browser surface | `synthetic` | Synthetic authority: ordered synthetic events, replay, health, input admission, revocation are real at the synthetic boundary. Provider execution may graduate `absent → declared → present` per the Task 2112 decision when a provider adapter is configured and a turn completes; local tool/MCP/filesystem/local-artifact authority remain **absent** by hard rule. | `packages/cloudflare-operator-projection/test/runtime-surface-quadrants.test.ts`, `test/provider-executor.test.ts`, `packages/agent-web-ui/test/agent-web-ui-projection.test.ts` |
| Cloudflare-origin synthetic runtime | Cloudflare-hosted browser surface | `synthetic` | Same synthetic slice, consumed from the Cloudflare-hosted shell. Deployed live smoke evidence exists separately and is operator-run only. | `packages/cloudflare-operator-projection/test/runtime-surface-quadrants.test.ts`, `test/cloudflare-operator-projection.test.ts` |
...
- The Cloudflare-origin authority slice is synthetic and does not imply local provider execution semantics. Provider execution on the slice is an explicit graduation: `absent` by default, `declared` when a provider adapter binding is configured, `present` only after a completed provider turn at the authority boundary (evidence ref required). Graduation is proven live on the deployed Worker (Task 2120, 2026-07-19, provider `deepseek-api`, evidence `provider-capable-live-smoke-1784485975919.json`). `local_tool_execution`, `local_mcp`, `local_filesystem_authority`, and `local_artifact_authority` remain `absent` permanently on Cloudflare-origin.
- Remote observation and authority-host transition remain distinct from projection symmetry. Local → cloudflare-host authority transition is implemented: the local drain/seal FSM hands off to the Cloudflare target activation with epoch token and replay boundary, validated by the carrier-protocol transition record; ambiguous dual-host is refused (`dual_host_authority_conflict`), and the cloudflare-host → local direction is durably refused (`transition_direction_refused`) until implemented.
