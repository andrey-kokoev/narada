...
- The Cloudflare-origin authority slice is synthetic and does not imply local provider execution semantics. Provider execution on the slice is an explicit graduation: `absent` by default, `declared` when a provider adapter binding is configured, `present` only after a completed provider turn at the authority boundary (evidence ref required). `local_tool_execution`, `local_mcp`, `local_filesystem_authority`, and `local_artifact_authority` remain `absent` permanently on Cloudflare-origin.
