...
| Task lifecycle projection-write cutover surface | The lower-level `task-lifecycle:projection-write-smoke:live` gate proves task lifecycle projection writes can be admitted as Cloudflare read-model records while SQLite source mutation, filesystem mutation, and repository publication remain outside Cloudflare authority. |
