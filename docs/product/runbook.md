...
The systemd unit (`docs/deployment/systemd/narada-daemon.service`) specifies `Restart=on-failure`.
...
The minimal operator rhythm is documented in [`docs/product/operator-loop.md`](operator-loop.md). The core command:
