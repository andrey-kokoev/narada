...
Agents currently claim tasks by reading Markdown files in `.ai/tasks/`. There is no durable record of which agent claimed which task, when, or under what authority. Multiple agents can simultaneously attempt the same task with no collision detection.
...
Define the assignment record shape. Suggested location: task file front-matter or a parallel `.ai/tasks/assignments/` directory.
...
**Task Assignment Record Format** (`.ai/tasks/assignments/README.md` + JSON files):
