...
`allocateTaskNumber()` loads `.ai/tasks/.registry.json`, increments in memory, and writes it back with `atomicWriteFile()`.
...
- create `.ai/tasks/.registry.lock` with exclusive create (`open(..., 'wx')`)
...
Also ensure `last_allocated` never moves behind the current maximum task number in `.ai/tasks/`.
...
Update `.ai/tasks/20260420-262-review-loop-and-task-number-allocation.md` with corrective notes referencing this task.
...
Added corrective notes section to `.ai/tasks/20260420-262-review-loop-and-task-number-allocation.md` referencing Task 274 and documenting each fix.
