...
Task files in `.ai/tasks/` should be durable task artifacts, not a growing shadow log of status variants. Creating sibling files for execution or completion status causes:
...
- `.ai/tasks/*.md` task files are durable specs/reviews, not execution-log variants
...
Add a repo lint/guard script that scans `.ai/tasks/` and fails if forbidden derivative filenames are present.
...
Remove existing derivative status files from `.ai/tasks/` and fold any necessary durable evidence back into the canonical task file where needed.
...
- [ ] existing derivative task status files in `.ai/tasks/` are removed after content migration where needed
