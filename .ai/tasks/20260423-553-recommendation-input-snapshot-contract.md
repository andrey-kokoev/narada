...
1. **Task State** — `.ai/tasks/*.md` (status, depends_on, continuation_affinity, title, body)
...
4. **Assignment History** — `.ai/tasks/assignments/*.json` (claim/release history, last worker, completion counts)
5. **Work Result Reports** — `.ai/tasks/reports/*.json` (changed_files, work_type, status, quality)
