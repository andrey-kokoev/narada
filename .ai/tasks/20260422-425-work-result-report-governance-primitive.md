...
Add documentation for a durable report record under `.ai/tasks/reports/README.md` or the existing task-governance docs if a better home exists.
...
- **`WorkResultReport` schema** documented in `.ai/tasks/reports/README.md` with full field table and invariants.
...
- **`narada task report` command** implemented in `packages/layers/cli/src/commands/task-report.ts`:
  ...
  - Writes atomic `WorkResultReport` to `.ai/tasks/reports/`
