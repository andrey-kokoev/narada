...
- `.ai/tasks/20260423-501-governed-task-artifact-terminal-state-ownership.md`
...
**chapter-init.ts**: `buildChildTaskBody()` now includes `## Execution Notes` and `## Verification` sections with placeholder comments in every newly created child task. Agents see the expected completion grammar as soon as they open the task file.
