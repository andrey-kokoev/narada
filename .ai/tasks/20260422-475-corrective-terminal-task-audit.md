...
Task 474 introduced the governed task closure invariant and audited the existing `.ai/tasks/` directory. The audit found **119 terminal tasks** (`closed` or `confirmed`) that violate the closure invariant. These tasks were created before the invariant was established and lack one or more of:
...
node -e "
const fs = require('fs');
const files = fs.readdirSync('.ai/tasks').filter(f => f.endsWith('.md'));
let violations = 0;
for (const f of files) {
  const content = fs.readFileSync('.ai/tasks/' + f, 'utf8');
