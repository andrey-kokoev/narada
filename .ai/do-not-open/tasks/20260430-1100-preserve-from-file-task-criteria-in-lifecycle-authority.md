...
- [ ] Reproduce the defect where task markdown created with --from-file contains Acceptance Criteria but task read/lifecycle authority reports empty acceptance_criteria.
- [ ] Parse Acceptance Criteria from --from-file task bodies into lifecycle authority during task creation, or return a clear diagnostic if parsing/admission is impossible.
- [ ] Ensure task read/evidence reports criteria consistently after --from-file creation without requiring a separate task amend command.
- [ ] Add regression tests covering from-file criteria preservation and malformed/unparseable criteria diagnostics.
- [ ] Update task create docs/help if needed to state how --from-file criteria are admitted.
