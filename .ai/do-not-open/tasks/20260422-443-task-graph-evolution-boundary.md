...
The `.ai/do-not-open` area is no longer a loose folder of Markdown files. It is a working governance substrate:
...
Define the **Task Graph Evolution Boundary**: the contract that governs how `.ai/do-not-open` may be created, numbered, renumbered, linked, reviewed, and evolved.
...
Define `.ai/do-not-open` as a governed task graph substrate, not a free-form document folder.
...
This should build on existing `.ai/do-not-open/tasks/tasks/.registry.json` if present. If the registry is insufficient, document required changes.
...
- [x] The contract defines `.ai/do-not-open` as a governed task graph substrate.
