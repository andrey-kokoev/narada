...
status: closed
created: 2026-04-24
closed_at: 2026-04-24T20:46:47.167Z
closed_by: a2
reopened_at: 2026-04-24T20:00:10.860Z
reopened_by: a2
...
2. **Added `.configureHelp({ sortSubcommands: false })`** — preserves registration order instead of alphabetical sorting

### Changes made — Round 2 (reviewer feedback)

**`packages/layers/cli/src/lib/grouped-help.ts` (new):**

3. **Implemented custom `GroupedHelp` class extending Commander's `Help`** — overrides `formatHelp()` to inject native group section headers between commands, eliminating the double-rendered flat list + legend duplication
4. **Defined 10 exhaustive command groups** covering every top-level command:
   - **Runtime** — sync, cycle, integrity, status
   - **Task Governance** — task, chapter
   - **Posture & Loop** — posture, construction-loop, verify
   - **Site & Console** — sites, console, workbench
   - **Operator** — ops, principal, doctor, audit
   - **Setup & Bootstrap** — init, demo, init-repo, want-mailbox, want-workflow, want-posture, setup, preflight, inspect, explain, activate
   - **Maintenance & Recovery** — rebuild-projections, backup, restore, backup-verify, backup-ls, cleanup, derive-work, preview-work, confirm-replay, recover, retry-auth-failed, acknowledge-alert
   - **Draft & Outbound** — reject-draft, mark-reviewed, handled-externally, drafts, show-draft, approve-draft-for-send
   - **Inspection** — show, select, crossing
   - **Deprecated** — rebuild-views

**`packages/layers/cli/src/main.ts`:**

5. **Wired `program.createHelp = () => new GroupedHelp()`** — replaces default flat help with grouped output
6. **Removed redundant `.addHelpText('before', ...)` group legend** — groups are now rendered natively by the Help class

### CLI framework limitation stated

Commander.js 11.1.0 renders commands as a single flat list with no native section headers. `.hideHelp()` was added in v12.0.0 and is unavailable. The `GroupedHelp` subclass overrides `formatHelp()` entirely to reconstruct the output with group headers, which is robust against Commander version changes but duplicates some internal formatting logic.

### Verification

20 focused checks:
- ✓ All 10 group section headers present
- ✓ Task description includes "create"
- ✓ rebuild-views in Deprecated section
- ✓ rebuild-views description contains "(deprecated:"
- ✓ Old stale task description removed
- ✓ principal in Operator section
- ✓ demo in Setup & Bootstrap section
- ✓ backup-verify, backup-ls, retry-auth-failed, acknowledge-alert in Maintenance section
