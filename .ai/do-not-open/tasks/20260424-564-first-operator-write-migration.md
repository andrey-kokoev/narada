...
1. **`packages/layers/cli/src/lib/task-lifecycle-store.ts`**
   - Added `openTaskLifecycleStore(cwd: string)` helper that opens/creates `.ai/do-not-open/tasks/task-lifecycle.db`, initializes schema, and returns `SqliteTaskLifecycleStore`.
