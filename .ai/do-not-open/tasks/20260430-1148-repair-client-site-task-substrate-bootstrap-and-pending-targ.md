...
- [ ] Fresh client Site task preflight/create succeeds or offers a sanctioned repair without manual .ai/do-not-open/tasks creation.
- [ ] Registry lock acquisition does not fail with ENOENT for missing task spec parent directory on a bootstrapped Site.
- [ ] inbox pending --to task:1 stores and displays a normalized task target without duplicated task prefix.
- [ ] Tests cover both task substrate bootstrap repair and pending target normalization.
- [ ] User-facing repair guidance no longer tells the Operator to manually create task substrate directories.
