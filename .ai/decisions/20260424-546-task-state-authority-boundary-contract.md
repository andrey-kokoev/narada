...
Task lifecycle authority currently lives in **markdown front matter** (`.ai/do-not-open/tasks/*.md`). CLI operators read the markdown file, mutate the front matter, and rewrite the file atomically. This means:
...
The following operators currently rewrite `.ai/do-not-open/tasks/*.md` front matter. After the migration, they must **write to SQLite instead** and let the projection layer update the markdown view (or leave markdown as a static authored document):
...
After migration, no correctness logic may query `.ai/do-not-open/tasks/*.md` for `status`, `governed_by`, `closed_at`, or `closed_by`. These queries must target SQLite.
...
-- Assignment history (migrates from .ai/do-not-open/tasks/tasks/assignments/*.json)
...
-- Work result reports (migrates from .ai/do-not-open/tasks/tasks/reports/*.json)
...
The task number allocator (currently `.ai/do-not-open/tasks/tasks/.registry.json`) also moves to SQLite:
...
| **C. Split file** | `.ai/do-not-open/tasks/NNN.md` for spec; `.ai/do-not-open/tasks/NNN.state.json` for lifecycle. | Preserves current file layout | Adds a second file per task |
